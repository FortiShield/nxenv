...
Cypress.Commands.add("create_records", (doc) => {
	return cy
		.call("nxenv.tests.ui_test_helpers.create_if_not_exists", { doc: JSON.stringify(doc) })
		.then((r) => r.message);
});

Cypress.Commands.add("set_value", (doctype, name, obj) => {
	return cy.call("nxenv.client.set_value", {
		doctype,
		name,
		fieldname: obj,
	});
...
Cypress.Commands.add(
	...
	(tablefieldname, row_idx, fieldname, fieldtype = "Data") => {
		let selector = `.nxenv-control[data-fieldname="${tablefieldname}"]`;
...
Cypress.Commands.add("dialog", (opts) => {
	return cy
		...
		.then((nxenv) => {
			...
			var d = new nxenv.ui.Dialog(opts);
			d.show();
			return d;
		});
});
...
Cypress.Commands.add("save", () => {
	cy.intercept("/api/method/nxenv.desk.form.save.savedocs").as("save_call");
...
Cypress.Commands.add("add_role", (user, role) => {
	cy.window()
		.its("nxenv")
		.then((nxenv) => {
			const session_user = nxenv.session.user;
...
const add_remove_role = (action, user, role, session_user) => {
	...
	cy.call("nxenv.tests.ui_test_helpers.add_remove_role", {
		action: action,
		user: user,
		role: role,
	});
...
Cypress.Commands.add("select_listview_row_checkbox", (row_no) => {
	cy.get(".nxenv-list .select-like > .list-row-checkbox").eq(row_no).click();
});
