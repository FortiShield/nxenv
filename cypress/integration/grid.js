context("Grid", () => {
	...
	before(() => {
		...
		return cy
			...
			.its("nxenv")
			.then((nxenv) => {
				return nxenv.call("nxenv.tests.ui_test_helpers.create_contact_phone_nos_records");
			});
	});
	it("update docfield property using update_docfield_property", () => {
		...
		cy.window()
			.its("cur_frm")
			.then((frm) => {
				cy.get('.nxenv-control[data-fieldname="phone_nos"]').as("table");
				...
				cy.get("@table-form")
					.find('.nxenv-control[data-fieldname="is_primary_phone"]')
					.should("be.hidden");
	...
	it("update docfield property using toggle_display", () => {
		...
		cy.window()
			...
			.then((frm) => {
				...
				cy.get(".grid-row-open").as("table-form");
				cy.get("@table-form")
					.find('.nxenv-control[data-fieldname="is_primary_mobile_no"]')
					.should("be.hidden");
				cy.get("@table-form").find(".grid-footer-toolbar").click();
	...
	it("update docfield property using toggle_enable", () => {
		...
		cy.window()
			...
			.then((frm) => {
				...
				cy.get(".grid-row-open").as("table-form");
				cy.get("@table-form")
					.find('.nxenv-control[data-fieldname="phone"] .control-value')
