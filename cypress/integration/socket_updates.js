context("Realtime updates", () => {
	...
	it("Recieves custom messages from server", { scrollBehavior: false }, () => {
		...
		cy.window()
			.its("nxenv")
			.then((nxenv) => {
				nxenv.realtime.on(event, () => handler.handle());
			});
	...
	it("Progress bar", { scrollBehavior: false }, () => {
		...
		cy.call("nxenv.tests.ui_test_helpers.publish_progress", { title }).then(() => {
...
function publish_realtime(args) {
	return cy.call("nxenv.tests.ui_test_helpers.publish_realtime", args);
}
