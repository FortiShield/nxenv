	...
	it("check value changes", () => {
		...
		TEST_CASES.forEach((test_case) => {
			cy.window()
				.its("nxenv")
				.then((nxenv) => {
					nxenv.boot.sysdefaults.currency = test_case.currency;
					nxenv.boot.sysdefaults.currency_precision = test_case.default_precision ?? 2;
					nxenv.boot.sysdefaults.number_format = test_case.number_format ?? "#,###.##";
				});
