context("Workspace Blocks", () => {
	before(() => {
		...
		return cy
			...
			.its("nxenv")
			.then((nxenv) => {
				return nxenv.xcall("nxenv.tests.ui_test_helpers.setup_workflow");
			});
	});
	...
	it("Create Test Page", () => {
		cy.intercept({
			method: "POST",
			url: "api/method/nxenv.desk.doctype.workspace.workspace.new_page",
		}).as("new_page");
	...
	it.skip("Quick List Block", () => {
		...
		cy.intercept({
			method: "GET",
			url: "api/method/nxenv.desk.form.load.getdoctype?**",
		}).as("get_doctype");
		...
		cy.intercept({
			method: "POST",
			url: "api/method/nxenv.desk.reportview.get",
		}).as("refresh-list");
