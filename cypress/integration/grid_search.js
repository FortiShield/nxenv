...
context("Grid Search", () => {
	before(() => {
		...
		return cy
			...
			.then((nxenv) => {
				return nxenv.xcall(
					"nxenv.tests.ui_test_helpers.insert_doctype_with_child_table_record",
					{
	...
	it("Test search row visibility", () => {
		cy.window()
			.its("nxenv")
			.then((nxenv) => {
				nxenv.model.user_settings.save("Doctype With Child Table", "GridView", {
		...
		cy.get('.nxenv-control[data-fieldname="child_table_1"]').as("table");
