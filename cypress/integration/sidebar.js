...
context("Sidebar", () => {
	before(() => {
		...
		return cy
			...
			.its("nxenv")
			.then((nxenv) => {
				return nxenv.call("nxenv.tests.ui_test_helpers.create_blog_post");
			});
	});

	it("Verify attachment visibility config", () => {
		cy.call("nxenv.tests.ui_test_helpers.create_todo", {
	...
	it("Verify attachment accessibility UX", () => {
		cy.call("nxenv.tests.ui_test_helpers.create_todo_with_attachment_limit", {
