...
def update_env(file_name: str, nxenv_tag: str, erpnext_tag: str | None = None):
    text = f"\nNXENV_VERSION={nxenv_tag}"
    if erpnext_tag:
...
def _print_resp(nxenv_tag: str, erpnext_tag: str | None = None):
    print(json.dumps({"nxenv": nxenv_tag, "erpnext": erpnext_tag}))


def main(_args: list[str]) -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--repo", choices=["nxenv", "erpnext"], required=True)
    parser.add_argument(
        "--version", choices=["12", "13", "14", "15", "develop"], required=True
    )
    args = parser.parse_args(_args)

    nxenv_tag = get_latest_tag("nxenv", args.version)
    ...
    if file_name:
        update_env(file_name, nxenv_tag, erpnext_tag)
    _print_resp(nxenv_tag, erpnext_tag)
    return 0
