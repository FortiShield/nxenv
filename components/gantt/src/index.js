...
export default class Gantt {
	...
	setup_wrapper(element) {
		...
		if (element instanceof HTMLElement) {
			...
			throw new TypeError(
				'Nxenv Gantt only supports usage of a string CSS selector,' +
	...
	update_view_scale(mode) {
		...
		this.$container.style.setProperty('--gv-column-width', this.config.column_width + 'px');
		this.config.header_height =
			this.options.lower_header_height + this.options.upper_header_height + 10;
	}
	...
	make_grid_highlights() {
		...
		const highlightDimensions = this.highlight_current(this.config.view_mode);
	...
	make_dates() {
		...
		this.upperTexts = Array.from(this.$container.querySelectorAll('.upper-text'));
	}
	...
	get_date_info(date, last_date_info) {
		...
		return {
			...
			formatted_date: sanitize(
				date_utils.format(date, this.config.date_format, this.options.language),
			),
			column_width: this.config.column_width,
			x,
			upper_text: this.config.view_mode.upper_text(date, last_date, this.options.language),
			lower_text: this.config.view_mode.lower_text(date, last_date, this.options.language),
			upper_y: 15,
	...
	get_closest_date() {
		...
		let current = new Date(),
			el = this.$container.querySelector(
				'.date_' +
					sanitize(
						date_utils.format(current, this.config.date_format, this.options.language),
					),
			);
	...
	bind_bar_events() {
		...
		$.on(this.$container, 'scroll', (e) => {
			...
			this.current_date = date_utils.add(
				this.gantt_start,
				((e.currentTarget.scrollLeft + $el.clientWidth) / this.config.column_width) *
					this.config.step,
				this.config.unit,
			);
			current_upper = this.config.view_mode.upper_text(
