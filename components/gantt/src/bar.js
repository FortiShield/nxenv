...
export default class Bar {
	...
	draw_progress_bar() {
		...
		if (this.task.color_progress) this.$bar_progress.style.fill = this.task.color;
		const x =
			(date_utils.diff(this.task._start, this.gantt.gantt_start, this.gantt.config.unit) /
	...
	compute_progress() {
		...
		const total =
			this.$bar.getWidth() - this.ignored_duration_raw * this.gantt.config.column_width;
		return parseInt((progress / total) * 100, 10);
	}

	compute_expected_progress() {
		this.expected_progress =
			date_utils.diff(date_utils.today(), this.task._start, 'hour') / this.gantt.config.step;
		this.expected_progress =
	...
	compute_duration() {
		...
		this.duration =
			date_utils.convert_scales(duration_in_days + 'd', this.gantt.config.unit) /
			this.gantt.config.step;

		this.actual_duration_raw =
			date_utils.convert_scales(actual_duration_in_days + 'd', this.gantt.config.unit) /
			this.gantt.config.step;
	...
	update_progressbar_position() {
		...
		this.$bar_progress.setAttribute('width', this.calculate_progress_width());
	}
