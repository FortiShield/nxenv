...
export default class AxisChart extends BaseChart {
	...
	calcYAxisParameters(dataValues, withMinimum = "false") {
		...
		if (dataValues instanceof Array) {
			yPts = calcChartIntervals(
				dataValues,
				withMinimum,
				this.config.overrideCeiling,
				this.config.overrideFloor
			);
		...
		let scaleAll = (values, id) => {
			return values.map((val) => {
				...
				if (yAxis instanceof Array) {
					yAxis =
						yAxis.length > 1 ? yAxis.find((axis) => id === axis.axisID) : s.yAxis[0];
				}
		...
		let barsConfigs = barDatasets.map((d) => {
			...
			return [
				...
				function () {
					...
					if (!stacked) {
						xPositions = xPositions.map((p) => p + barWidth * barIndex - barWidth);
					}
		...
		let lineConfigs = lineDatasets.map((d) => {
			...
			return [
				...
				function () {
					...
					let minLine =
						yAxis.positions[0] < yAxis.zeroLine ? yAxis.positions[0] : yAxis.zeroLine;
