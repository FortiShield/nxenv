...
export default class CellManager {
	...
	bindKeyboardNav() {
		...
		["left", "right", "up", "down", "tab", "shift+tab"].map((direction) =>
			this.keyboard.on(direction, () => this.focusCellInDirection(direction))
		);

		["left", "right", "up", "down"].map((direction) =>
			this.keyboard.on(`ctrl+${direction}`, () => focusLastCell(direction))
		);
	...
	bindCopyCellContents() {
		this.keyboard.on("ctrl+c", () => {
			const noOfCellsCopied = this.copyCellContents(
				this.$focusedCell,
	...
	focusCell($cell, { skipClearSelection = 0, skipDOMFocus = 0, skipScrollToCell = 0 } = {}) {
		if (!$cell) return;
	...
	unfocusCell($cell) {
		...
		if (this.lastHeaders) {
			this.lastHeaders.forEach(
				(header) => header && header.classList.remove("dt-cell--highlight")
			);
		}
	}
	...
	focusCellInDirection(direction) {
		if (
			!this.$focusedCell ||
			(this.$editingCell && ["left", "right", "up", "down"].includes(direction))
		) {
			return false;
	...
	getCellHTML(cell) {
		const { rowIndex, colIndex, isHeader, isFilter, isTotalRow } = cell;
	...
	getCellContent(cell, refreshHtml = false) {
		const { isHeader, isFilter, colIndex } = cell;
		...
		if (this.options.treeView && !(isHeader || isFilter) && cell.indent !== undefined) {
			...
			if (firstColumnIndex === cell.colIndex) {
				...
				const toggleHTML = addToggle
					? `<span class="dt-tree-node__toggle" style="left: ${
							padding - leftPadding
					  }${unit}">
		...
		const className = [
			"dt-cell__content",
			isHeader
				? `dt-cell__content--header-${colIndex}`
