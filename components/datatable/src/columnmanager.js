...
import { linkProperties, debounce } from "./utils";
...
export default class ColumnManager {
	...
	bindPerfectColumnWidth() {
		$.on(this.header, "dblclick", ".dt-cell .dt-cell__resize-handle", (e, $handle) => {
			...
			let { borderLeftWidth, borderRightWidth, paddingLeft, paddingRight } = $.getStyle(
				this.bodyScrollable.querySelector(".dt-cell__content")
			);
	...
	applyFilter(filters) {
		this.datamanager.filterRows(filters).then(({ rowsToShow }) => {
