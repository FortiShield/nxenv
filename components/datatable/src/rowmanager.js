...
export default class RowManager {
	...
	checkRow(rowIndex, toggle) {
		...
		$.each(selector(rowIndex), this.bodyScrollable).map((input) => {
	...
	checkAll(toggle) {
		...
		// set checkbox value
		$.each('.dt-cell--col-0 [type="checkbox"]', this.bodyScrollable).map((input) => {
			input.checked = toggle;
		});
		// highlight all
	...
	expandAllNodes() {
		...
		const childrenToShow = rootNodes
			.map((row) => this.getChildrenToShowForNode(row.meta.rowIndex))
			.flat();
		const visibleRowIndices = this.bodyRenderer.visibleRowIndices;
		const rowsToShow = uniq([...childrenToShow, ...visibleRowIndices]).sort(numberSortAsc);
	...
	collapseAllNodes() {
		...
		const rowsToHide = rootNodes
			.map((row) => this.getChildrenToHideForNode(row.meta.rowIndex))
			.flat();
		const visibleRows = this.bodyRenderer.visibleRowIndices;
		const rowsToShow = visibleRows
			.filter((rowIndex) => !rowsToHide.includes(rowIndex))
