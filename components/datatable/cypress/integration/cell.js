describe("Cell", function () {
	...
	it("focuses cell on click", function () {
		cy.clickCell(2, 0).should("have.class", "dt-cell--focus");
	});

	it("not focuses cell which are not focusable", function () {
		cy.clickCell(1, 0).should("not.have.class", "dt-cell--focus");
	});
	...
	it("if editing is false: editing should not activate", function () {
		cy.getCell(3, 0).dblclick({ force: true }).should("not.have.class", "dt-cell--editing");
	});

	it("navigation using arrow keys", function () {
		cy.clickCell(2, 0).type("{rightarrow}");
		...
		cy.get(".dt-cell--focus").should("have.class", "dt-cell--3-1");
		// TODO: test navigation over hidden rows
	});

	it("navigation using ctrl + arrow keys", function () {
		cy.clickCell(2, 0).type("{ctrl}{rightarrow}");
		cy.get(".dt-cell--focus").should("have.class", "dt-cell--7-0");
	});

	it("cell selection using shift + arrow keys", function () {
		cy.getCell(2, 1).type("{shift}{rightarrow}{rightarrow}{downarrow}");
