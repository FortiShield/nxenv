describe("Column", function () {
	...
	it("header dropdown toggles on click", function () {
		cy.getColumnCell(2).find(".dt-dropdown__toggle").as("toggle").click();
		cy.get(".dt-dropdown__list").as("dropdown-list").should("be.visible");
	...
	it("sort ascending button should work", function () {
		...
		cy.window()
			.then((win) => win.datatable.getColumn(2))
			.its("sortOrder")
			.should("eq", "asc");

		cy.window()
			.then((win) => win.datatable.datamanager)
			.its("currentSort.colIndex")
			.should("eq", 2);

		cy.get(".dt-scrollable .dt-row:first").contains("Airi Satou");
	...
	it("resize column using double click", function () {
		...
		cy.getColumnCell(4).should("have.css", "width").and("match", /9\dpx/);
		cy.getCell(4, 1).should("have.css", "width").and("match", /9\dpx/);
	});
});
