...
const baseCSS = {
	...
	plugins: [
		postcss({
			extract: true,
			minimize: production,
			plugins: [customProperties(), nested(), autoprefixer()],
		}),
	],
};
...
const devCjs = merge(baseJS, {
	output: {
		file: "dist/nxenv-datatable.cjs.js",
		format: "cjs",
	},
});
...
const prodIIFE = merge(devIIFE, {
	output: {
		file: "dist/nxenv-datatable.min.js",
	},
	plugins: [uglify()],
});
...
const prodCSS = merge(devCSS, {
	output: {
		file: "dist/nxenv-datatable.min.css",
	},
});
