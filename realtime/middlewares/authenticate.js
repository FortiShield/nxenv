...
function authenticate_with_nxenv(socket, next) {
	...
	socket.nxenv_request = (path, args = {}, opts = {}) => {
	...
	socket
		.nxenv_request("/api/method/nxenv.realtime.get_user_info")
		.then((res) => res.json())
		.then(({ message }) => {
...
function get_site_name(socket) {
	...
	} else if (socket.request.headers["x-nxenv-site-name"]) {
		socket.site_name = get_hostname(socket.request.headers["x-nxenv-site-name"]);
	} else if (
		conf.default_site &&
...
module.exports = authenticate_with_nxenv;
