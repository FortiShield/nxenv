...
def main():
    ...
    init_nxcli_if_not_exist(args)
    create_site_in_nxcli(args)
...
def get_args_parser():
    ...
    parser.add_argument(
        ...
        action="store",
        type=str,
        help="Nxcli directory name, default: nxenv-nxcli",
        default="nxenv-nxcli",
    )  # noqa: E501
    parser.add_argument(
        "-s",
    ...
    parser.add_argument(
        ...
        action="store",
        type=str,
        help="nxenv repo to use, default: https://github.com/nxenv/nxenv",  # noqa: E501
    ...
    parser.add_argument(
        ...
        action="store",
        type=str,
        help="nxenv repo to use, default: version-15",  # noqa: E501
        default="version-15",
    )
    parser.add_argument(
        "-p",
...
def init_nxcli_if_not_exist(args):
    if os.path.exists(args.nxcli_name):
        cprint("Nxcli already exists. Only site will be created", level=3)
        return
    try:
        env = os.environ.copy()
        if args.py_version:
        ...
        init_command += "nxcli init "
        init_command += "--skip-redis-config-generation "
        init_command += "--verbose " if args.verbose else " "
        init_command += f"--nxenv-path={args.nxenv_repo} "
        init_command += f"--nxenv-branch={args.nxenv_branch} "
        init_command += f"--apps_path={args.apps_json} "
        init_command += args.nxcli_name
        command = [
            "/bin/bash",
            "-i",
            "-c",
        ...
        cprint("Configuring Nxcli ...", level=2)
        cprint("Set db_host", level=3)
        if args.db_type:
            cprint(f"Setting db_type to {args.db_type}", level=3)
            subprocess.call(
                ["nxcli", "set-config", "-g", "db_type", args.db_type],
                cwd=os.path.join(os.getcwd(), args.nxcli_name),
            )
        ...
        subprocess.call(
            ["nxcli", "set-config", "-gp", "developer_mode", "1"],
            cwd=os.getcwd() + "/" + args.nxcli_name,
        )
...
def create_site_in_nxcli(args):
    if "mariadb" == args.db_type:
        ...
        cprint("Set db_host", level=3)
        subprocess.call(
            ["nxcli", "set-config", "-g", "db_host", "postgresql"],
            cwd=os.getcwd() + "/" + args.nxcli_name,
        )
        new_site_cmd = [
            "nxcli",
    ...
    apps = os.listdir(f"{os.getcwd()}/{args.nxcli_name}/apps")
    apps.remove("nxenv")
    for app in apps:
