...
- `nxcli`. It is used for development. [Learn more how to start development](development.md).
- `production`.
  - Multi-purpose Python backend. Runs [Werkzeug server](https://werkzeug.palletsprojects.com/en/2.0.x/) with [gunicorn](https://gunicorn.org), queues (via `nxcli worker`), or schedule (via `nxcli schedule`).
...
- `custom`. It is used to build nxcli using `apps.json` file set with `--apps_path` during nxcli initialization. `apps.json` is a json array. e.g. `[{"url":"{{repo_url}}","branch":"{{repo_branch}}"}]`

Image has everything we need to be able to run all processes that Nxenv framework requires (take a look at [Nxcli Procfile reference](https://nxenvframework.com/docs/v14/user/en/nxcli/resources/nxcli-procfile)). We follow [Docker best practices](https://docs.docker.com/develop/develop-images/dockerfile_best-practices/#decouple-applications) and split these processes to different containers.
...
- `configurator`. Updates `common_site_config.json` so Nxenv knows how to access db and redis. It is executed on every `docker-compose up` (and exited immediately). Other services start after this container exits successfully.
