...
```sh
docker-compose exec backend nxcli new-site --mariadb-user-host-login-scope=% --db-root-password <db-password> --admin-password <admin-password> <site-name>
```

If you need to install some app, specify `--install-app`. To see all options, just run `nxcli new-site --help`.
...
```sh
docker-compose exec backend nxcli set-config -g root_login <root-login>
docker-compose exec backend nxcli set-config -g root_password <root-password>
```

Also command is slightly different:

```sh
docker-compose exec backend nxcli new-site --mariadb-user-host-login-scope=% --db-type postgres --admin-password <admin-password> <site-name>
```
...
For full instructions, refer to the [wiki](<https://github.com/nxenv/nxenv/wiki/Using-Nxenv-with-Amazon-RDS-(or-any-other-DBaaS)>). Common question can be found in Issues and on forum.
...
For reference of commands like `backup`, `drop-site` or `migrate` check [official guide](https://nxenvframework.com/docs/v13/user/en/nxcli/nxenv-commands) or run:

```sh
docker-compose exec backend nxcli --help
```
...
```sh
docker-compose exec backend nxcli --site <site-name> migrate
```
