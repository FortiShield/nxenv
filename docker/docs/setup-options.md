...
### Setup Nxenv without proxy and external MariaDB and Redis
...
### Setup Nxenv using containerized MariaDB and Redis with Letsencrypt certificates.
...
Switch to the root of the `nxenv_docker` directory before running the following commands:

```sh
# Update environment variables ERPNEXT_VERSION and NXENV_VERSION
nano .env
