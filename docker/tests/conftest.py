...
def env_file(tmp_path_factory: pytest.TempPathFactory):
    tmp_path = tmp_path_factory.mktemp("nxenv-docker")
    file_path = tmp_path / ".env"
    shutil.copy("example.env", file_path)

    _add_sites_var(file_path)

    for var in ("NXENV_VERSION", "ERPNEXT_VERSION"):
...
def nxenv_setup(compose: Compose):
    compose.stop()
...
@pytest.fixture(scope="session")
def nxenv_site(compose: Compose):
...
def postgres_setup(compose: Compose):
    ...
    compose.nxcli("set-config", "-g", "root_login", "postgres")
    compose.nxcli("set-config", "-g", "root_password", "123")
    yield
    compose.stop()
...
@pytest.fixture
def python_path():
    return "/home/nxenv/nxenv-nxcli/env/bin/python"
