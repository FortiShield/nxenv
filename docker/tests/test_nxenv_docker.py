...
def test_endpoints(url: str, callback: Any, nxenv_site: str):
    check_url_content(
        url=f"http://127.0.0.1{url}", callback=callback, site_name=nxenv_site
    )


@pytest.mark.skipif(
    os.environ["NXENV_VERSION"][0:3] == "v12", reason="v12 doesn't have the asset"
)
def test_assets_endpoint(nxenv_site: str):
    check_url_content(
        url=f"http://127.0.0.1/assets/nxenv/images/nxenv-framework-logo.svg",
        callback=assets_cb,
        site_name=nxenv_site,
    )


def test_files_reachable(nxenv_site: str, tmp_path: Path, compose: Compose):
    content = "lalala\n"
    ...
    compose(
        "cp",
        str(file_path),
        f"backend:/home/nxenv/nxenv-nxcli/sites/{nxenv_site}/public/files/",
    )
...
@pytest.mark.parametrize("service", BACKEND_SERVICES)
@pytest.mark.usefixtures("nxenv_site")
def test_nxenv_connections_in_backends(
    service: str, python_path: str, compose: Compose
):
    filename = "_ping_nxenv_connections.py"
    compose("cp", f"tests/{filename}", f"{service}:/tmp/")
    compose.exec(
        "-w",
...
def test_push_backup(
    ...
    compose.nxcli("--site", nxenv_site, "backup", "--with-files")
    restic_args = [
        "--env=RESTIC_REPOSITORY=s3:http://minio:9000/nxenv",
...
def test_https(nxenv_site: str, compose: Compose):
    compose("-f", "overrides/compose.https.yaml", "up", "-d")
    check_url_content(url="https://127.0.0.1", callback=index_cb, site_name=nxenv_site)
