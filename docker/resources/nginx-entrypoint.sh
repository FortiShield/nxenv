...
if [[ -z "$NXENV_SITE_NAME_HEADER" ]]; then
  # shellcheck disable=SC2016
  echo 'NXENV_SITE_NAME_HEADER defaulting to $host'
  # shellcheck disable=SC2016
  export NXENV_SITE_NAME_HEADER='$host'
fi
...
envsubst '${BACKEND}
  ...
  </templates/nginx/nxenv.conf.template >/etc/nginx/conf.d/nxenv.conf
