...
import nxenv
from nxenv.model.document import Document, read_only_document
...
class TestReadOnlyDocument(IntegrationTestCase):
	def setUp(self):
		...
		self.test_doc = nxenv.get_doc({"doctype": "ToDo", "description": "Test ToDo"})
		self.test_doc.insert()

	def tearDown(self):
		# Delete the test document
		nxenv.delete_doc("ToDo", self.test_doc.name)

	def test_read_only_save(self):
		with read_only_document():
			with self.assertRaises(nxenv.DatabaseModificationError):
				self.test_doc.save()

	def test_read_only_insert(self):
		with read_only_document():
			with self.assertRaises(nxenv.DatabaseModificationError):
				nxenv.get_doc({"doctype": "ToDo", "description": "Another Test ToDo"}).insert()
	...
	def test_nested_read_only_document(self):
		...
		self.assertFalse(hasattr(nxenv.local, "read_only_depth"))

		with read_only_document():
			# Check that read_only_depth is set to 1
			self.assertEqual(nxenv.local.read_only_depth, 1)
			...
			with read_only_document():
				...
				self.assertEqual(nxenv.local.read_only_depth, 2)
	...
	def test_error_log_exception_in_read_only(self):
		with read_only_document():
			...
			error_log = nxenv.get_doc({"doctype": "Error Log", "error": "Test error in read-only mode"})
		...
		nxenv.delete_doc("Error Log", error_log.name)

	def test_read_only_multiple_documents(self):
		doc1 = nxenv.get_doc({"doctype": "ToDo", "description": "Test ToDo 1"})
		doc2 = nxenv.get_doc({"doctype": "ToDo", "description": "Test ToDo 2"})
	...
	def test_read_only_method_call_details(self):
		with read_only_document():
			with self.assertRaises(nxenv.DatabaseModificationError) as cm:
				self.test_doc.save()
	...
	def test_read_only_does_not_affect_reads(self):
		with read_only_document():
			...
			doc = nxenv.get_doc("ToDo", self.test_doc.name)
			self.assertEqual(doc.description, "Test ToDo")

			docs = nxenv.get_all("ToDo", filters={"name": self.test_doc.name})
			self.assertEqual(len(docs), 1)

	def test_read_only_with_new_document_instance(self):
		with read_only_document():
			new_doc = nxenv.new_doc("ToDo")
			with self.assertRaises(nxenv.DatabaseModificationError):
				new_doc.insert()
