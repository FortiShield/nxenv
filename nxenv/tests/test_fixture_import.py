...
import nxenv
from nxenv.core.doctype.data_import.data_import import export_json, import_doc
from nxenv.desk.form.save import savedocs
...
class TestFixtureImport(IntegrationTestCase):
	def create_new_doctype(self, DocType: str) -> None:
		file = nxenv.get_app_path("nxenv", "custom", "fixtures", f"{DocType}.json")
	...
	def insert_dummy_data_and_export(self, DocType: str, dummy_name_list: list[str]) -> str:
		for name in dummy_name_list:
			doc = nxenv.get_doc({"doctype": DocType, "member_name": name})
			doc.insert()
	...
	def test_fixtures_import(self):
		self.assertFalse(nxenv.db.exists("DocType", "temp_doctype"))
		...
		nxenv.db.truncate("temp_doctype")
		...
		self.assertEqual(nxenv.db.count("temp_doctype"), len(dummy_name_list))

		data = nxenv.get_all("temp_doctype", "member_name")
		nxenv.db.truncate("temp_doctype")
	...
	def test_singles_fixtures_import(self):
		self.assertFalse(nxenv.db.exists("DocType", "temp_singles"))
		...
		singles_doctype = nxenv.qb.DocType("Singles")
		truncate_query = (
			nxenv.qb.from_(singles_doctype).delete().where(singles_doctype.doctype == "temp_singles")
		)
		truncate_query.run()

		import_doc(path_to_exported_fixtures)

		data = nxenv.db.get_single_value("temp_singles", "member_name")
		truncate_query.run()
