...
from nxenv.tests.test_api import NxenvAPITestCase, suppress_stdout
...
class TestResourceAPIV2(NxenvAPITestCase):
	version = "v2"
	DOCTYPE = "ToDo"
	...
	def test_execute_doc_method(self):
		...
		self.assertEqual(response.json["data"][0]["name"], "nxenv")
	...
	def test_delete_document_non_existing(self):
		non_existent_doc = nxenv.generate_hash(length=12)
...
class TestMethodAPIV2(NxenvAPITestCase):
	version = "v2"
	...
	def test_shorthand_controller_methods(self):
		...
		expanded_response = self.get(
			self.method("nxenv.core.doctype.user.user.get_all_roles"), {"sid": self.sid}
		)
	...
	def test_add_comment(self):
		comment_txt = nxenv.generate_hash()
		response = self.post(
...
class TestDocTypeAPIV2(NxenvAPITestCase):
	version = "v2"
