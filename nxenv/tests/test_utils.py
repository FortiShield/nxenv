...
import nxenv
from nxenv.installer import parse_app_name
from nxenv.model.document import Document
from nxenv.tests import IntegrationTestCase, MockedRequestTestCase
...
from nxenv.utils.change_log import (
	get_source_url,
	parse_github_url,
)
from nxenv.utils.data import (
	add_to_date,
	add_years,
	cast,
	cint,
...
from nxenv.utils.dateutils import get_dates_from_timegrain
from nxenv.utils.diff import _get_value_from_version, get_version_diff, version_query
from nxenv.utils.identicon import Identicon
from nxenv.utils.image import optimize_image, strip_exif_data
from nxenv.utils.make_random import can_make, get_random, how_many
from nxenv.utils.response import json_handler
from nxenv.utils.synchronization import LockTimeoutError, filelock
	...
	def test_scrub_urls(self):
		html = """
			...
			<div class="more-info">
				...
				<img src="/assets/nxenv/test.jpg">
			</div>
			<div style="background-image: url('/assets/nxenv/bg.jpg')">
		...
		self.assertTrue(f'<img src="{url}/assets/nxenv/test.jpg">' in html)
		self.assertTrue(f"style=\"background-image: url('{url}/assets/nxenv/bg.jpg') !important\"" in html)
...
class TestHTMLUtils(IntegrationTestCase):
	def test_clean_email_html(self):
		from nxenv.utils.html_utils import clean_email_html
	...
	def test_sanitize_html(self):
		from nxenv.utils.html_utils import sanitize_html
...
class TestValidationUtils(IntegrationTestCase):
	def test_valid_url(self):
		...
		self.assertTrue(validate_url("http://nxenv.io", throw=True))
		...
		self.assertRaises(nxenv.ValidationError, validate_url, "nxenv", throw=True)

		# Scheme validation
		self.assertFalse(validate_url("https://google.com", valid_schemes="http"))
		self.assertTrue(validate_url("ftp://nxenv.cloud", valid_schemes=["https", "ftp"]))
		self.assertFalse(validate_url("bolo://nxenv.io", valid_schemes=("http", "https", "ftp", "ftps")))
		self.assertRaises(
	...
	def test_valid_email(self):
		...
		# Valid addresses
		self.assertTrue(validate_email_address("someone@nxenv.com"))
		self.assertTrue(validate_email_address("someone@nxenv.com, anyone@nxenv.io"))
		self.assertTrue(validate_email_address("test%201@nxenv.com"))

		# Invalid address
		self.assertFalse(validate_email_address("someone"))
		self.assertFalse(validate_email_address("someone@----.com"))
		self.assertFalse(validate_email_address("test 1@nxenv.com"))
		self.assertFalse(validate_email_address("test@example.com test2@example.com,undisclosed-recipient"))

		# Invalid with throw
		self.assertRaises(
			nxenv.InvalidEmailAddressError,
			validate_email_address,
			"someone.com",
			throw=True,
		)

		self.assertEqual(validate_email_address("Some%20One@nxenv.com"), "Some%20One@nxenv.com")
		self.assertEqual(
			validate_email_address("erp+Job%20Applicant=JA00004@nxenv.com"),
			"erp+Job%20Applicant=JA00004@nxenv.com",
		)
	...
	def test_validate_name(self):
		...
		for name in invalid_names:
			self.assertRaises(nxenv.InvalidNameError, validate_name, name, True)


class TestImage(IntegrationTestCase):
	def test_strip_exif_data(self):
		original_image = Image.open(nxenv.get_app_path("nxenv", "tests", "data", "exif_sample_image.jpg"))
		original_image_content = open(
			nxenv.get_app_path("nxenv", "tests", "data", "exif_sample_image.jpg"),
			mode="rb",
		).read()
	...
	def test_optimize_image(self):
		image_file_path = nxenv.get_app_path("nxenv", "tests", "data", "sample_image_for_optimization.jpg")
...
class TestPythonExpressions(IntegrationTestCase):
	...
	def test_validation_for_bad_python_expression(self):
		...
		for expr in invalid_expressions:
			self.assertRaises(nxenv.ValidationError, validate_python_code, expr)
...
class TestDiffUtils(IntegrationTestCase):
	...
	def setUpClass(cls):
		super().setUpClass()
		cls.doc = nxenv.get_doc(doctype="Client Script", dt="Client Script", name="test_client_script")
		cls.doc.insert()
		cls.doc.script = "2;"
...
class TestDateUtils(IntegrationTestCase):
	def test_first_day_of_week(self):
		...
		with patch.object(nxenv.utils.data, "get_first_day_of_the_week", return_value="Monday"):
			self.assertEqual(
				nxenv.utils.get_first_day_of_week("2020-12-25"),
				nxenv.utils.getdate("2020-12-21"),
			)
			self.assertEqual(
				nxenv.utils.get_first_day_of_week("2020-12-20"),
				nxenv.utils.getdate("2020-12-14"),
			)

		# Sunday as start of the week
		self.assertEqual(
			nxenv.utils.get_first_day_of_week("2020-12-25"),
			nxenv.utils.getdate("2020-12-20"),
		)
		self.assertEqual(
			nxenv.utils.get_first_day_of_week("2020-12-21"),
			nxenv.utils.getdate("2020-12-20"),
		)

	def test_last_day_of_week(self):
		self.assertEqual(
			nxenv.utils.get_last_day_of_week("2020-12-24"),
			nxenv.utils.getdate("2020-12-26"),
		)
		self.assertEqual(
			nxenv.utils.get_last_day_of_week("2020-12-28"),
			nxenv.utils.getdate("2021-01-02"),
		)

	def test_is_last_day_of_the_month(self):
		self.assertEqual(nxenv.utils.is_last_day_of_the_month("2020-12-24"), False)
		self.assertEqual(nxenv.utils.is_last_day_of_the_month("2020-12-31"), True)
...
class TestXlsxUtils(IntegrationTestCase):
	def test_unescape(self):
		from nxenv.utils.xlsxutils import handle_html
...
class TestLinkTitle(IntegrationTestCase):
	def test_link_title_doctypes_in_boot_info(self):
		...
		from nxenv.boot import get_link_title_doctypes
	...
	def test_link_titles_on_getdoc(self):
		...
		link_titles = nxenv.local.response["_link_titles"]
...
class TestAppParser(MockedRequestTestCase):
	def test_app_name_parser(self):
		self.responses.add(
			"HEAD",
			"https://api.github.com/repos/nxenv/healthcare",
			status=200,
			json={},
		)
		nxcli_path = get_nxcli_path()
		nxenv_app = os.path.join(nxcli_path, "apps", "nxenv")
		self.assertEqual("nxenv", parse_app_name(nxenv_app))
		self.assertEqual("healthcare", parse_app_name("healthcare"))
		self.assertEqual("healthcare", parse_app_name("https://github.com/nxenv/healthcare.git"))
		self.assertEqual("healthcare", parse_app_name("git@github.com:nxenv/healthcare.git"))
		self.assertEqual("healthcare", parse_app_name("nxenv/healthcare@develop"))
...
class TestIntrospectionMagic(IntegrationTestCase):
	...
	def test_get_newargs(self):
		...
		self.assertEqual(nxenv.get_newargs(f, safe_kwargs), safe_kwargs)

		unsafe_args = dict(safe_kwargs)
		unsafe_args.update({"ignore_permissions": True, "flags": {"ignore_mandatory": True}})
		self.assertEqual(nxenv.get_newargs(f, unsafe_args), safe_kwargs)
	...
	def test_strip_off_kwargs_when_not_supported(self):
		...
		self.assertEqual(nxenv.get_newargs(f, args), {"b": 1})

		# No args
		self.assertEqual(nxenv.get_newargs(lambda: None, args), {})
...
class TestLazyLoader(IntegrationTestCase):
	def test_lazy_import_module(self):
		from nxenv.utils.lazy_loader import lazy_import

		with Capturing() as output:
			ls = lazy_import("nxenv.tests.data.load_sleep")
		self.assertEqual(output, [])

		with Capturing() as output:
			ls.time
		self.assertEqual(["Module `nxenv.tests.data.load_sleep` loaded"], output)


class TestIdenticon(IntegrationTestCase):
	def test_get_gravatar(self):
		# info@khulnasoft.com has a gravatar linked so str URL will be returned
		nxenv.flags.in_test = False
		gravatar_url = get_gravatar("info@khulnasoft.com")
		nxenv.flags.in_test = True
		...
		gravatar_url = get_gravatar(f"developers{random_string(6)}@nxenv.io")
		self.assertIsInstance(gravatar_url, str)
...
class TestMiscUtils(IntegrationTestCase):
	...
	def test_get_all_sites(self):
		self.assertIn(nxenv.local.site, get_sites())

	def test_get_site_info(self):
		info = get_site_info()

		installed_apps = [app["app_name"] for app in info["installed_apps"]]
		self.assertIn("nxenv", installed_apps)
...
class TestTypingValidations(IntegrationTestCase):
	...
	def test_validate_whitelisted_api(self):
		...
		for fn, args, kwargs in test_cases:
			with self.assertRaisesRegex(nxenv.NxenvTypeError, self.ERR_REGEX):
				fn(*args, **kwargs)

	def test_validate_whitelisted_doc_method(self):
		report = nxenv.get_last_doc("Report")
...
class TestTBSanitization(IntegrationTestCase):
	def test_traceback_sanitzation(self):
		try:
			...
			args = nxenv._dict({"password": "42", "pwd": "42", "safe": "safe_value"})  # noqa: F841
			raise Exception
		except Exception:
...
class TestRounding(IntegrationTestCase):
	...
	def test_default_rounding(self):
		self.assertEqual(nxenv.get_system_settings("rounding_method"), "Banker's Rounding")
...
class TestArgumentTypingValidations(IntegrationTestCase):
	def test_validate_argument_types(self):
		...
		from nxenv.core.doctype.doctype.doctype import DocType
		from nxenv.utils.typing_validations import (
			NxenvTypeError,
		...
		self.assertEqual(test_simple_types(1, 2, "1"), (1, 2.0, True))
		with self.assertRaises(NxenvTypeError):
		...
		doctype = nxenv.get_last_doc("DocType")
...
class TestChangeLog(IntegrationTestCase):
	def test_get_remote_url(self):
		self.assertIsInstance(get_source_url("nxenv"), str)

	def test_parse_github_url(self):
		# using erpnext as repo in order to be different from the owner
		owner, repo = parse_github_url("https://github.com/nxenv/erpnext.git")
		self.assertEqual(owner, "nxenv")
		self.assertEqual(repo, "erpnext")

		owner, repo = parse_github_url("https://github.com/nxenv/erpnext")
		self.assertEqual(owner, "nxenv")
		self.assertEqual(repo, "erpnext")

		owner, repo = parse_github_url("git@github.com:nxenv/erpnext.git")
		self.assertEqual(owner, "nxenv")
		self.assertEqual(repo, "erpnext")
...
class TestURLTrackers(IntegrationTestCase):
	def test_add_trackers_to_url(self):
		...
		with patch("nxenv.db.get_value") as mock_get_value:
	...
	def test_map_trackers(self):
		...
		expected = {
			"utm_source": nxenv.get_doc("UTM Source", "test_source"),
			"utm_medium": nxenv.get_doc("UTM Medium", "test_medium"),
			"utm_campaign": nxenv.get_doc("UTM Campaign", "test_campaign"),
			"utm_content": "test_content",
		}
