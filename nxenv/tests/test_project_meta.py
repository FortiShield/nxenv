# Copyright (c) 2024, Nxenv Technologies Pvt. Ltd. and Contributors
# License: MIT. See LICENSE
import datetime
from pathlib import Path

import nxenv
from nxenv.tests.classes.integration_test_case import IntegrationTestCase
...
class TestProjectMeta(IntegrationTestCase):
	def test_init_py_tax_paid(self):
		...
		nxenv/__init__.py has grown crazy big and keeps getting bigger. Plot the LOC over time and
		...
		init_py_size = len(Path(nxenv.__file__).read_text().splitlines())
		...
		self.assertLessEqual(
			init_py_size,
			expected,
			"""You have either added too many lines to nxenv/__init__.py or that file hasn't shrunk fast enough. Remember the Zen of Python:\n"""
