...
class TestScheduler(IntegrationTestCase):
	def setUp(self):
		...
		purge_pending_jobs()
		if not nxenv.get_all("Scheduled Job Type", limit=1):
			sync_jobs()
	...
	def test_enqueue_jobs(self):
		nxenv.db.sql("update `tabScheduled Job Type` set last_execution = '2010-01-01 00:00:00'")

		enqueued_jobs = enqueue_events()

		self.assertIn("nxenv.desk.notifications.clear_notifications", enqueued_jobs)
		self.assertIn("nxenv.utils.change_log.check_for_update", enqueued_jobs)
		self.assertIn(
	...
	def test_is_dormant(self):
		...
		self.assertTrue(is_dormant(check_time=add_days(nxenv.db.get_last_created("Activity Log"), 5)))
		self.assertFalse(is_dormant(check_time=nxenv.db.get_last_created("Activity Log")))

	def test_once_a_day_for_dormant(self):
		nxenv.db.truncate("Scheduled Job Log")
		self.assertTrue(schedule_jobs_based_on_activity(check_time=get_datetime("2100-01-01 00:00:00")))
		self.assertTrue(
			schedule_jobs_based_on_activity(
				check_time=add_days(nxenv.db.get_last_created("Activity Log"), 5)
			)
		)

		# create a fake job executed 5 days from now
		job = get_test_job(method="nxenv.tests.test_scheduler.test_method", frequency="Daily")
		job.execute()
		job_log = nxenv.get_doc("Scheduled Job Log", dict(scheduled_job_type=job.name))
		job_log.db_set(
...
def get_test_job(method="nxenv.tests.test_scheduler.test_timeout_10", frequency="All") -> ScheduledJobType:
	if not nxenv.db.exists("Scheduled Job Type", dict(method=method)):
		job = nxenv.get_doc(
		...
		job = nxenv.get_doc("Scheduled Job Type", dict(method=method))
