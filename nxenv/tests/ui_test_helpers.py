...
UI_TEST_USER = "nxenv@example.com"


def whitelist_for_tests(fn):
	if nxenv.request and not nxenv.flags.in_test and not getattr(nxenv.local, "dev_server", 0):
		nxenv.throw("Cannot run UI tests. Use a development server with `nxcli start`")

	return nxenv.whitelist()(fn)
...
def create_if_not_exists(doc):
	...
	for doc in docs:
		...
		filters = doc.copy()
		filters.pop("doctype")
		name = nxenv.db.exists(doc.doctype, filters)
		if not name:
			d = nxenv.get_doc(doc)
...
def prepare_webform_test():
	for note in nxenv.get_all("Note", pluck="name"):
		nxenv.delete_doc("Note", note, force=True)

	nxenv.delete_doc_if_exists("Web Form", "note")
	...
	doc = nxenv.get_doc(
		{
			...
			"communication_date": nxenv.utils.now_datetime(),
		}
	)
	doc.insert()
	return doc
...
@whitelist_for_tests
def setup_workflow():
	from nxenv.workflow.doctype.workflow.test_workflow import create_todo_workflow
...
def create_contact_phone_nos_records():
	if nxenv.get_all("Contact", {"first_name": "Test Contact"}):
		return

	doc = nxenv.new_doc("Contact")
...
def create_contact_records():
	if nxenv.get_all("Contact", {"first_name": "Test Form Contact 1"}):
		return
...
def create_multiple_todo_records():
	if nxenv.get_all("ToDo", {"description": "Multiple ToDo 1"}):
		return

	values = [(f"100{i}", f"Multiple ToDo {i}") for i in range(1, 1002)]

	nxenv.db.bulk_insert("ToDo", fields=["name", "description"], values=set(values))


def insert_contact(first_name, phone_number):
	doc = nxenv.get_doc({"doctype": "Contact", "first_name": first_name})
...
def create_form_tour():
	if nxenv.db.exists("Form Tour", {"name": "Test Form Tour"}):
		return
...
def create_web_page(title, route, single_thread):
	web_page = nxenv.db.exists("Web Page", {"route": route})
	if web_page:
		return web_page
	web_page = nxenv.get_doc({"doctype": "Web Page", "title": title, "route": route, "published": True})
	web_page.save()
	...
	web_page.append(
		"page_blocks",
		{
			...
			"web_template_values": nxenv.as_json(
				{
					"title": "Discussions",
...
def update_webform_to_multistep():
	if not nxenv.db.exists("Web Form", "update-profile-duplicate"):
		doc = nxenv.get_doc("Web Form", "edit-profile")
		_doc = nxenv.copy_doc(doc)
...
def insert_doctype_with_child_table_record(name):
	if nxenv.get_all(name, {"title": "Test Grid Search"}):
		return
...
def insert_translations():
	...
	for doc in translation:
		if not nxenv.db.exists("doc"):
			nxenv.get_doc(doc).insert()


@whitelist_for_tests
def create_blog_post():
	blog_category = nxenv.get_doc(
		{"name": "general", "doctype": "Blog Category", "title": "general"}
	).insert(ignore_if_duplicate=True)

	blogger = nxenv.get_doc(
		{
...
def create_test_user(username=None):
	...
	if nxenv.db.exists("User", name):
		return

	user = nxenv.new_doc("User")
	user.email = name
	user.first_name = "Nxenv"
	user.new_password = nxenv.local.conf.admin_password
	...
	all_roles = set(nxenv.get_all("Role", pluck="name"))
	...
	nxenv.db.set_single_value("Workspace Settings", "workspace_setup_completed", 1)


@whitelist_for_tests
def setup_tree_doctype():
	nxenv.delete_doc_if_exists("DocType", "Custom Tree", force=True)
	...
	if not nxenv.db.exists("Custom Tree", "All Trees"):
		nxenv.get_doc({"doctype": "Custom Tree", "tree": "All Trees"}).insert()


@whitelist_for_tests
def setup_image_doctype():
	nxenv.delete_doc_if_exists("DocType", "Custom Image", force=True)
...
def setup_default_view(view, force_reroute=None):
	nxenv.delete_doc_if_exists("Property Setter", "Event-main-default_view")
	nxenv.delete_doc_if_exists("Property Setter", "Event-main-force_re_route_to_default_view")
...
@whitelist_for_tests
def create_kanban():
	if not nxenv.db.exists("Custom Field", "Note-kanban"):
		nxenv.get_doc(
			{
				"is_system_generated": 0,
				"dt": "Note",
	...
	if not nxenv.db.exists("Kanban Board", "_Note _Kanban"):
		nxenv.get_doc(
			{
...
def create_todo(description):
	return nxenv.get_doc({"doctype": "ToDo", "description": description}).insert()
...
def create_admin_kanban():
	if not nxenv.db.exists("Kanban Board", "Admin Kanban"):
		nxenv.get_doc(
			{
				"doctype": "Kanban Board",
				"name": "Admin Kanban",
...
def publish_progress(duration=3, title=None, doctype=None, docname=None):
	...
	nxenv.enqueue(slow_task, duration=duration, title=title, doctype=doctype, docname=docname)
...
def slow_task(duration, title, doctype, docname):
	...
	for i in range(steps + 1):
		nxenv.publish_progress(i * 10, title=title, doctype=doctype, docname=docname)
