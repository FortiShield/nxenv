...
class UnitTestCase(unittest.TestCase, BaseTestCase):
	"""Unit test class for Nxenv tests.

	This class extends unittest.TestCase and provides additional utilities
	specific to Nxenv framework. It's designed for testing individual
	...
	def setUpClass(cls) -> None:
		...
		cls.module = nxenv.get_module(cls.__module__)
		# Test Environment
		nxenv.set_user("Administrator")
		# Test Environment (cleanup)
		cls.addClassCleanup(nxenv.set_user, "Administrator")
...
def _get_doctype_from_module(cls):
	...
	path = nxenv.get_pymodule_path(cls.__module__)
	try:
