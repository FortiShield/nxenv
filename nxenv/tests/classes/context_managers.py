...
if TYPE_CHECKING:
	from nxenv.model import Document
...
def freeze_time(time_to_freeze: Any, is_utc: bool = False, *args: Any, **kwargs: Any) -> None:
	...
	from nxenv.utils.data import get_datetime, get_system_timezone
...
def patch_hooks(overridden_hooks: dict) -> None:
	...
	with patch.object(nxenv, "get_hooks", patched_hooks):
		yield
...
def change_settings(doctype, settings_dict=None, /, commit=False, **settings) -> None:
	...
	settings = nxenv.get_doc(doctype)
...
def enable_safe_exec() -> None:
	...
	conf = os.path.join(nxenv.local.sites_path, "common_site_config.json")
...
def trace_fields(
	...
	**field_configs: dict[str, list | Callable | None],
	...
	Yields:
	    Document class
	"""
	from nxenv.model.trace import traced_field
...
# NOTE: declare those who should also be made available directly nxenv.tests.* namespace
