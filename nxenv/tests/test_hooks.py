...
import nxenv
from nxenv.cache_manager import clear_controller_cache
from nxenv.desk.doctype.todo.todo import ToDo
...
class TestHooks(IntegrationTestCase):
	...
	def test_override_doctype_class(self):
		...
		# Set hook
		hooks.override_doctype_class = {"ToDo": ["nxenv.tests.test_hooks.CustomToDo"]}

		# Clear cache
		nxenv.client_cache.delete_value("app_hooks")
		clear_controller_cache("ToDo")

		todo = nxenv.get_doc(doctype="ToDo", description="asdf")
	...
	def test_has_permission(self):
		...
		address_has_permission_hook.append("nxenv.tests.test_hooks.custom_has_permission")
		...
		wildcard_has_permission_hook.append("nxenv.tests.test_hooks.custom_has_permission")
		...
		user = nxenv.get_doc("User", username)
		user.add_roles("System Manager")
		address = nxenv.new_doc("Address")
		...
		# Test!
		self.assertTrue(nxenv.has_permission("Address", doc=address, user=username))
		self.assertTrue(nxenv.has_permission("Note", doc=note, user=username))

		address.flags.dont_touch_me = True
		self.assertFalse(nxenv.has_permission("Address", doc=address, user=username))

		note.flags.dont_touch_me = True
		self.assertFalse(nxenv.has_permission("Note", doc=note, user=username))

	def test_ignore_links_on_delete(self):
		email_unsubscribe = nxenv.get_doc(
		...
		self.assertRaises(nxenv.LinkExistsError, email_unsubscribe.delete)
	...
	def test_fixture_prefix(self):
		...
		app = "nxenv"
		if os.path.isdir(nxenv.get_app_path(app, "fixtures")):
			shutil.rmtree(nxenv.get_app_path(app, "fixtures"))
		...
		# every call to nxenv.get_hooks loads the hooks module into cache
		...
		if nxenv._load_app_hooks.__wrapped__ in nxenv.local.request_cache.keys():
			del nxenv.local.request_cache[nxenv._load_app_hooks.__wrapped__]
		self.assertEqual([False], nxenv.get_hooks("fixture_auto_order", app_name=app))
		self.assertEqual(
			[
				{"dt": "User"},
				{"dt": "Contact"},
			...
			nxenv.get_hooks("fixtures", app_name=app),
		)
		...
		self.assertCountEqual(
			["user.json", "contact.json", "role.json"], os.listdir(nxenv.get_app_path(app, "fixtures"))
		)

		hooks.fixture_auto_order = True
		del nxenv.local.request_cache[nxenv._load_app_hooks.__wrapped__]
		self.assertEqual([True], nxenv.get_hooks("fixture_auto_order", app_name=app))

		shutil.rmtree(nxenv.get_app_path(app, "fixtures"))
		export_fixtures(app)
		self.assertCountEqual(
			["1_user.json", "2_contact.json", "3_role.json"],
			os.listdir(nxenv.get_app_path(app, "fixtures")),
		)
...
class TestAPIHooks(NxenvAPITestCase):
	def test_auth_hook(self):
		with self.patch_hooks({"auth_hooks": ["nxenv.tests.test_hooks.custom_auth"]}):
			site_url = nxenv.utils.get_site_url(nxenv.local.site)
			response = self.get(
				site_url + "/api/method/nxenv.auth.get_logged_user",
...
def custom_auth():
	auth_type, token = nxenv.get_request_header("Authorization", "Bearer ").split(" ")
