...
class TestGeoUtils(IntegrationTestCase):
	def setUp(self):
		...
		self.test_location = nxenv.get_doc(
	...
	def test_get_coords_from_not_existable_location(self):
		self.assertRaises(nxenv.ValidationError, get_coords, "ToDo", self.test_filter_todo, "location_field")

	def test_get_coords_from_not_existable_coords(self):
		self.assertRaises(nxenv.ValidationError, get_coords, "ToDo", self.test_filter_todo, "coordinates")
