...
from nxenv.utils import cint, get_test_client, get_url

try:
	_site = nxenv.local.site
except Exception:
	_site = None
...
class ThreadWithReturnValue(Thread):
	...
	def run(self):
		if self._target is not None:
			with patch("nxenv.app.get_site_name", return_value=self.site):
				header_patch = patch("nxenv.get_request_header", new=patch_request_header)
...
class NxenvAPITestCase(IntegrationTestCase):
...
class TestResourceAPI(NxenvAPITestCase):
	DOCTYPE = "ToDo"
	...
	def setUpClass(cls):
		super().setUpClass()
		for _ in range(20):
			doc = nxenv.get_doc({"doctype": "ToDo", "description": nxenv.mock("paragraph")}).insert()
	...
	def tearDownClass(cls):
		for name in cls.GENERATED_DOCUMENTS:
			nxenv.delete_doc_if_exists(cls.DOCTYPE, name)
		nxenv.db.commit()
	...
	def test_get_list_dict(self):
		...
		json = nxenv._dict(response.json)
		self.assertEqual(response.status_code, 200)
	...
	def test_create_document(self):
		data = {"description": nxenv.mock("paragraph"), "sid": self.sid}
	...
	def test_update_document(self):
		generated_desc = nxenv.mock("paragraph")
	...
	def test_run_doc_method(self):
		...
		if response.status_code == 403:
			...
			self.assertEqual(
				response.json.get("exception"), "nxenv.exceptions.PermissionError: Not permitted"
			)
...
class TestMethodAPI(NxenvAPITestCase):
	def test_ping(self):
	...
	def test_get_user_info(self):
		# test 3: test for /api/method/nxenv.realtime.get_user_info
		response = self.get(self.method("nxenv.realtime.get_user_info"))
		self.assertEqual(response.status_code, 200)
		self.assertIsInstance(response.json, dict)
	...
	def test_auth_cycle(self):
		...
		response = self.get(self.method("nxenv.auth.get_logged_user"))
	...
	def test_logs(self):
		method = "nxenv.tests.test_api.test"

		def get_message(resp, msg_type):
			return nxenv.parse_json(nxenv.parse_json(nxenv.parse_json(resp.json)[msg_type])[0])
	...
	def test_array_response(self):
		method = "nxenv.tests.test_api.test_array"
...
class TestReadOnlyMode(NxenvAPITestCase):
	...
	def test_blocked_writes(self):
		with suppress_stdout():
			response = self.post(
				self.resource("ToDo"), {"description": nxenv.mock("paragraph"), "sid": self.sid}
			)
		self.assertEqual(response.status_code, 503)
		self.assertEqual(response.json["exc_type"], "InReadOnlyMode")


class TestWSGIApp(NxenvAPITestCase):
	def test_request_hooks(self):
		self.addCleanup(lambda: _test_REQ_HOOK.clear())

		with self.patch_hooks(
			{
				"before_request": ["nxenv.tests.test_api.before_request"],
				"after_request": ["nxenv.tests.test_api.after_request"],
			}
		):
...
class TestResponse(NxenvAPITestCase):
	def test_generate_pdf(self):
		response = self.get(
			"/api/method/nxenv.utils.print_format.download_pdf",
	...
	def test_binary_and_csv_response(self):
		def download_template(file_type):
			...
			return self.post(
				"/api/method/nxenv.core.doctype.data_import.data_import.download_template",
				{
					"sid": self.sid,
					"doctype": "User",
		...
		from nxenv.desk.utils import provide_binary_file
	...
	def test_download_private_file_with_unique_url(self):
		test_content = nxenv.generate_hash()
		file = nxenv.get_doc(
			{
				"doctype": "File",
...
def generate_admin_keys():
	from nxenv.core.doctype.user.user import generate_keys
