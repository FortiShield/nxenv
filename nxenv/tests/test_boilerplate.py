...
import nxenv
from nxenv.modules.patch_handler import get_all_patches, parse_as_configfile
from nxenv.utils.boilerplate import (
	PatchCreator,
...
class TestBoilerPlate(unittest.TestCase):
	...
	def setUpClass(cls):
		...
		cls.nxcli_path = nxenv.utils.get_nxcli_path()
		cls.apps_dir = os.path.join(cls.nxcli_path, "apps")
	...
	def delete_test_app(cls, app_name):
		test_app_dir = os.path.join(cls.nxcli_path, "apps", app_name)
	...
	def test_create_app(self):
		...
		new_app_dir = os.path.join(self.nxcli_path, self.apps_dir, app_name)
	...
	def test_new_patch_util(self):
		...
		patches_txt = pathlib.Path(pathlib.Path(nxenv.get_app_path("nxenv", "patches.txt")))
		...
		expected_patch = "nxenv.core.doctype.user.patches.delete_all_users"
