...
- Time bound tests: Nxclimarks are done on GHA before adding numbers
...
from nxenv.website.path_resolver import PathResolver
...
class TestPerformance(IntegrationTestCase):
	def reset_request_specific_caches(self):
		...
		nxenv.destroy()  # releases everything on nxenv.local
		nxenv.init(self.TEST_SITE)
		nxenv.connect()
		nxenv.clear_cache()

	def setUp(self) -> None:
		self.HOST = nxenv.utils.get_site_url(nxenv.local.site)
	...
	def test_permitted_fieldnames(self):
		...
		doc = nxenv.new_doc("Prepared Report")
		# load permitted fieldnames once
		doc.permitted_fieldnames

		with patch("nxenv.model.base_document.get_permitted_fields") as mocked:
			doc.as_dict()
			# get_permitted_fields should not be called again
			mocked.assert_not_called()

	def test_set_value_query_count(self):
		nxenv.db.set_value("User", "Administrator", "interest", "Nothing")

		with self.assertQueryCount(1):
			nxenv.db.set_value("User", "Administrator", "interest", "Nothing")

		with self.assertQueryCount(1):
			nxenv.db.set_value("User", {"user_type": "System User"}, "interest", "Nothing")
	...
	def test_get_value_limits(self):
		...
		# Warm up code
		nxenv.db.get_values("User", filters=filters[0], limit=1)
		for filter in filters:
			with self.assertRowsRead(1):
				self.assertEqual(1, len(nxenv.db.get_values("User", filters=filter, limit=1)))
			with self.assertRowsRead(2):
				self.assertEqual(2, len(nxenv.db.get_values("User", filters=filter, limit=2)))

			self.assertEqual(
				len(nxenv.db.get_values("User", filters=filter)), nxenv.db.count("User", filter)
			)

			with self.assertRowsRead(1):
				nxenv.db.get_value("User", filters=filter)

			with self.assertRowsRead(1):
				nxenv.db.exists("User", filter)
	...
	def test_req_per_seconds_basic(self):
		...
		req_count = 1000
		client = NxenvClient(self.HOST, "Administrator", self.ADMIN_PASSWORD)
	...
	def test_consistent_build_version(self):
		from nxenv.utils import get_build_version
	...
	def test_get_list_single_query(self):
		...
		user = nxenv.get_doc("User", TEST_USER)

		nxenv.set_user(TEST_USER)
	...
	def test_no_ifnull_checks(self):
		query = nxenv.get_all("DocType", {"autoname": ("is", "set")}, run=0).lower()
		self.assertNotIn("coalesce", query)
		self.assertNotIn("ifnull", query)

	def test_no_stale_ref_sql(self):
		"""nxenv.db.sql should not hold any internal references to result set.
	...
	def test_one_time_setup(self):
		...
		run = nxenv.qb._BuilderClasss.run

		nxenv.init(site, force=True)
		patched_run = nxenv.qb._BuilderClasss.run

		self.assertIs(run, patched_run, "nxenv.init should run one-time patching code just once")

	def test_cpu_allocation(self):
		from nxenv._optimizations import assign_core
...
@run_only_if(db_type_is.MARIADB)
class TestOverheadCalls(NxenvAPITestCase):
	...
	def test_get_doc_overheads(self):
		sid = self.sid
		tables = len(nxenv.get_meta("User").get_table_fields())
