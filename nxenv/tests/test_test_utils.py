...
class TestTestUtils(IntegrationTestCase):
	...
	def test_document_assertions(self):
		currency = nxenv.new_doc("Currency")
	...
	def test_thread_locals(self):
		nxenv.flags.temp_flag_to_be_discarded = True

	def test_temp_setting_changes(self):
		current_setting = nxenv.get_system_settings("logout_on_password_reset")

		with IntegrationTestCase.change_settings(
			"System Settings", {"logout_on_password_reset": int(not current_setting)}
		):
			updated_settings = nxenv.get_system_settings("logout_on_password_reset")
			self.assertNotEqual(current_setting, updated_settings)

		restored_settings = nxenv.get_system_settings("logout_on_password_reset")
...
def tearDownModule():
	...
	assert "temp_flag_to_be_discarded" not in nxenv.flags
	assert not nxenv.db.exists("Currency", "STONKS")
