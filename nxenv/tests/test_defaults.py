...
class TestDefaults(IntegrationTestCase):
	...
	def test_user_permission_on_defaults(self):
		...
		perm_doc = nxenv.get_doc(
			doctype="User Permission", user=nxenv.session.user, allow="Language", for_value="en-GB"
		...
		nxenv.delete_doc("User Permission", perm_doc.name)
	...
	def test_user_permission_defaults(self):
		...
		perm_doc = nxenv.get_doc(
			doctype="User Permission", user=nxenv.session.user, allow="Country", for_value="India"
		).insert(ignore_permissions=True)

		nxenv.db.set_value("User Permission", perm_doc.name, "is_default", 1)
		set_global_default("Country", "United States")
		self.assertEqual(get_user_default("Country"), "India")

		nxenv.db.set_value("User Permission", perm_doc.name, "is_default", 0)
		clear_user_default("Country")
		self.assertEqual(get_user_default("Country"), None)

		perm_doc = nxenv.get_doc(
			doctype="User Permission", user=nxenv.session.user, allow="Country", for_value="United States"
		).insert(ignore_permissions=True)
