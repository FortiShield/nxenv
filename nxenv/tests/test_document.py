...
import nxenv
from nxenv.app import make_form_dict
from nxenv.core.doctype.doctype.test_doctype import new_doctype
from nxenv.desk.doctype.note.note import Note
from nxenv.model.naming import make_autoname, parse_naming_series, revert_series_if_last
from nxenv.tests import IntegrationTestCase
from nxenv.utils import cint, now_datetime, set_request
...
class TestDocument(IntegrationTestCase):
	...
	def test_load_single(self):
		d = nxenv.get_doc("Website Settings", "Website Settings")
	...
	def test_insert(self):
		...
		self.assertEqual(nxenv.db.get_value("Event", d.name, "subject"), "test-doc-test-event 1")
	...
	def test_website_route_default(self):
		...
		doc = nxenv.get_doc({"doctype": parent, "child_table": [{"some_fieldname": "xasd"}]}).insert()
	...
	def test_insert_with_child(self):
		...
		d.insert()
		self.assertTrue(d.name.startswith("EV"))
		self.assertEqual(nxenv.db.get_value("Event", d.name, "subject"), "test-doc-test-event 2")
	...
	def test_update(self):
		...
		self.assertEqual(nxenv.db.get_value(d.doctype, d.name, "subject"), "subject changed")
	...
	def test_discard_transitions(self):
		...
		d.submit()
		self.assertRaises(nxenv.ValidationError, d.discard)
		d.reload()
	...
	def test_save_on_discard_throws(self):
		from nxenv.desk.doctype.event.event import Event
		...
		self.assertRaises(nxenv.ValidationError, d3.discard)
	...
	def test_mandatory(self):
		...
		nxenv.delete_doc_if_exists("User", "test_mandatory@example.com", 1)
		...
		self.assertRaises(nxenv.MandatoryError, d.insert)

		d.set("first_name", "Test Mandatory")
		d.insert()
		self.assertEqual(nxenv.db.get_value("User", d.name), d.name)

	def test_text_editor_field(self):
		try:
			nxenv.get_doc(doctype="Activity Log", subject="test", message='<img src="test.png" />').insert()
		except nxenv.MandatoryError:
			self.fail("Text Editor false positive mandatory error")

	def test_conflict_validation(self):
		d1 = self.test_insert()
		d2 = nxenv.get_doc(d1.doctype, d1.name)
		d1.save()
		self.assertRaises(nxenv.TimestampMismatchError, d2.save)

	def test_conflict_validation_single(self):
		d1 = nxenv.get_doc("Website Settings", "Website Settings")
		d1.home_page = "test-web-page-1"

		d2 = nxenv.get_doc("Website Settings", "Website Settings")
	...
	def test_permission(self):
		...
		self.assertRaises(nxenv.PermissionError, self.test_insert)
		nxenv.set_user("Administrator")

	def test_permission_single(self):
		nxenv.set_user("Guest")
		d = nxenv.get_doc("Website Settings", "Website Settings")
		self.assertRaises(nxenv.PermissionError, d.save)
		nxenv.set_user("Administrator")

	def test_link_validation(self):
		nxenv.delete_doc_if_exists("User", "test_link_validation@example.com", 1)
		...
		self.assertRaises(nxenv.LinkValidationError, d.insert)
	...
	def test_validate(self):
		...
		self.assertRaises(nxenv.ValidationError, d.validate)
		self.assertRaises(nxenv.ValidationError, d.run_method, "validate")
		self.assertRaises(nxenv.ValidationError, d.save)
	...
	def test_new_doc_with_fields(self):
		user = nxenv.new_doc("User", first_name="wizard")
		self.assertEqual(user.first_name, "wizard")

	def test_update_after_submit(self):
		d = self.test_insert()
		d.starts_on = "2014-09-09"
		self.assertRaises(nxenv.UpdateAfterSubmitError, d.validate_update_after_submit)
	...
	def test_varchar_length(self):
		...
		self.assertRaises(nxenv.CharacterLengthExceededError, d.save)
	...
	def test_naming_series(self):
		...
		for series in data:
			...
			old_current = nxenv.db.get_value("Series", prefix, "current", order_by="name")

			revert_series_if_last(series, name)
			new_current = cint(nxenv.db.get_value("Series", prefix, "current", order_by="name"))

			self.assertEqual(cint(old_current) - 1, new_current)

	def test_non_negative_check(self):
		nxenv.delete_doc_if_exists("Currency", "Nxenv Coin", 1)

		d = nxenv.get_doc(
			{"doctype": "Currency", "currency_name": "Nxenv Coin", "smallest_currency_fraction_value": -1}
		)

		self.assertRaises(nxenv.NonNegativeError, d.insert)

		d.set("smallest_currency_fraction_value", 1)
		d.insert()
		self.assertEqual(nxenv.db.get_value("Currency", d.name), d.name)
	...
	def test_get_formatted(self):
		...
		nxenv.delete_doc_if_exists("Currency", "INR", 1)
		...
		d = nxenv.get_doc({"doctype": "Test Formatted", "currency": 100000})
	...
	def test_limit_for_get(self):
		doc = nxenv.get_doc("DocType", "DocType")
	...
	def test_virtual_fields(self):
		...
		nxenv.db.delete("Custom Field", {"dt": "Note", "fieldname": "age"})
		...
		def patch_note(class_=None):
			return patch("nxenv.controllers", new={nxenv.local.site: {"Note": class_ or CustomTestNote}})

		@contextmanager
		def customize_note(with_options=False):
			options = (
				"nxenv.utils.now_datetime() - nxenv.utils.get_datetime(doc.creation)"
				if with_options
				else ""
			)
			custom_field = nxenv.get_doc(
				{
	...
	def test_doc_events(self):
		...
		for doctype, doc_hooks in nxenv.get_doc_hooks().items():
	...
	def test_error_on_saving_new_doc_with_name(self):
		...
		doc = nxenv.get_doc(
			{
				"doctype": "ToDo",
				"description": "this should raise nxenv.DoesNotExistError",
				"name": "lets-trick-doc-save",
			}
		)

		self.assertRaises(nxenv.DoesNotExistError, doc.save)
	...
	def test_validate_from_to_dates(self):
		...
		self.assertRaises(
			nxenv.exceptions.InvalidDates, doc.validate_from_to_dates, "start_date", "end_date"
		)
	...
	def test_db_set_singles(self):
		...
		c.db_set(key, val)
		changed_val = nxenv.db.get_single_value(c.doctype, key)
...
class TestDocumentWebView(IntegrationTestCase):
	def get(self, path, user="Guest"):
		...
		make_form_dict(nxenv.local.request)
	...
	def test_web_view_link_authentication(self):
		...
		# expire the key
		document_key_doc = nxenv.get_doc("Document Share Key", {"key": document_key})
	...
	def test_base_class_set_correctly_on_has_web_view_change(self):
		...
		from nxenv.modules.utils import get_doc_path, scrub

		nxenv.flags.allow_doctype_export = True

		nxenv.delete_doc_if_exists("DocType", "Test WebViewDocType", force=1)
	...
	def test_bulk_inserts(self):
		from nxenv.model.document import bulk_insert

		doctype = "Role Profile"
		child_field = "roles"
		child_doctype = nxenv.get_meta(doctype).get_field(child_field).options
		...
		def doc_generator():
			for _ in range(21):
				...
				doc.role_profile = nxenv.generate_hash()
		...
		all_docs = set(nxenv.get_all(doctype, pluck="name"))
		all_child_docs = set(
			nxenv.get_all(
