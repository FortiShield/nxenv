...
from nxenv.database.utils import DefaultOrderBy
from nxenv.desk.reportview import get_filters_cond
from nxenv.handler import execute_cmd
from nxenv.model.db_query import DatabaseQuery, get_between_date_filter
from nxenv.permissions import add_user_permission, clear_user_permissions_for_doctype
from nxenv.query_builder import Column
...
class TestDBQuery(IntegrationTestCase):
	...
	def test_child_table_field_syntax(self):
		note = nxenv.get_doc(
			doctype="Note",
			title=f"Test {nxenv.utils.random_string(8)}",
			content="test",
			seen_by=[{"user": "Administrator"}],
		).insert()
		result = nxenv.get_all(
			"Note",
	...
	def test_child_table_join(self):
		nxenv.delete_doc_if_exists("DocType", "Parent DocType 1")
		nxenv.delete_doc_if_exists("DocType", "Parent DocType 2")
		nxenv.delete_doc_if_exists("DocType", "Child DocType")
		# child table
		nxenv.get_doc(
			{
				"doctype": "DocType",
		...
		# clear records
		nxenv.db.delete("Parent DocType 1")
		nxenv.db.delete("Parent DocType 2")
		nxenv.db.delete("Child DocType")
		...
		# test query
		results1 = nxenv.get_all("Parent DocType 1", fields=["name", "child.title as child_title"])
		results2 = nxenv.get_all("Parent DocType 2", fields=["name", "child.title as child_title"])
	...
	def test_link_field_syntax(self):
		todo = nxenv.get_doc(doctype="ToDo", description="Test ToDo", allocated_to="Administrator").insert()
		result = nxenv.get_all(
			"ToDo",
	...
	def test_build_match_conditions(self):
		...
		test2user = nxenv.get_doc("User", "test2@example.com")
		test2user.add_roles("Blogger")
		nxenv.set_user("test2@example.com")
	...
	def test_string_as_field(self):
		self.assertEqual(
			nxenv.get_all("DocType", as_list=True), nxenv.get_all("DocType", fields="name", as_list=True)
		)

	def test_none_filter(self):
		query = nxenv.qb.get_query("DocType", fields="name", filters={"restrict_to_domain": None})
	...
	def test_between_filters_date_bounds(self):
		date_df = nxenv._dict(fieldtype="Date")
		datetime_df = nxenv._dict(fieldtype="Datetime")
		today = nxenv.utils.nowdate()
	...
	def test_ignore_permissions_for_get_filters_cond(self):
		nxenv.set_user("test2@example.com")
		self.assertRaises(nxenv.PermissionError, get_filters_cond, "DocType", dict(istable=1), [])
	...
	def test_query_fields_sanitizer(self):
		...
		with self.assertRaises(nxenv.DataError):
			DatabaseQuery("DocType").execute(
				fields=[
					"name",
					"issingle",
	...
	def test_nested_permission(self):
		...
		from nxenv.core.page.permission_manager.permission_manager import update
	...
	def test_of_not_of_descendant_ancestors(self):
		...
		data = nxenv.get_all("Nested DocType", {"name": ("descendants of", "Level 2 A")})
		self.assertTrue({"name": "Level 3 A"} in data)

		data = nxenv.get_all("Nested DocType", {"name": ("descendants of", "Level 1 A")})
		self.assertTrue({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		self.assertFalse({"name": "Level 2 B"} in data)
		self.assertFalse({"name": "Level 1 B"} in data)
		...
		data = nxenv.get_all("Nested DocType", {"name": ("ancestors of", "Level 2 A")})
		...
		data = nxenv.get_all("Nested DocType", {"name": ("ancestors of", "Level 1 A")})
		self.assertFalse({"name": "Level 3 A"} in data)
		self.assertFalse({"name": "Level 2 A"} in data)
		self.assertFalse({"name": "Level 2 B"} in data)
		self.assertFalse({"name": "Level 1 B"} in data)
		...
		data = nxenv.get_all("Nested DocType", {"name": ("not descendants of", "Level 2 A")})
		self.assertFalse({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		...
		data = nxenv.get_all("Nested DocType", {"name": ("not descendants of", "Level 1 A")})
		self.assertFalse({"name": "Level 3 A"} in data)
		self.assertFalse({"name": "Level 2 A"} in data)
		self.assertTrue({"name": "Level 2 B"} in data)
		...
		data = nxenv.get_all("Nested DocType", {"name": ("not ancestors of", "Level 2 A")})
		self.assertTrue({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		self.assertTrue({"name": "Level 2 B"} in data)
		self.assertTrue({"name": "Level 1 B"} in data)
		...
		data = nxenv.get_all("Nested DocType", {"name": ("not ancestors of", "Level 1 A")})
		self.assertTrue({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		self.assertTrue({"name": "Level 2 B"} in data)
		self.assertTrue({"name": "Level 1 B"} in data)
		...
		data = nxenv.get_all("Nested DocType", {"name": ("ancestors of", "Root")})
		self.assertTrue(len(data) == 0)
		self.assertTrue(
			len(nxenv.get_all("Nested DocType", {"name": ("not ancestors of", "Root")}))
			== len(nxenv.get_all("Nested DocType"))
		)
	...
	def test_is_set_is_not_set(self):
		...
		nxenv.db.set_value("DocType", "Property Setter", "autoname", None, update_modified=False)
	...
	def test_virtual_field_get_list(self):
		try:
			nxenv.get_list("Prepared Report", ["*"])
			nxenv.get_list("Scheduled Job Type", ["*"])
		except Exception:
	...
	def test_prepare_select_args(self):
		# nxenv.get_all inserts modified field into order_by clause
	...
	def test_column_comparison(self):
		...
		users_unedited = nxenv.get_all(
			"User",
			filters={"creation": Column("modified")},
			fields=["name", "creation", "modified"],
			limit=1,
		)
		users_edited = nxenv.get_all(
			"User",
			filters={"creation": ("!=", Column("modified"))},
			fields=["name", "creation", "modified"],
			limit=1,
	...
	def test_cast_name(self):
		...
		nxenv.delete_doc_if_exists("DocType", "autoinc_dt_test")
	...
	def test_fieldname_starting_with_int(self):
		...
		nxenv.delete_doc_if_exists("DocType", "dt_with_int_named_fieldname")
		nxenv.delete_doc_if_exists("DocType", "table_dt")
		...
		dt_data = nxenv.get_doc({"doctype": "dt_with_int_named_fieldname", "1field": "10"}).insert(
			ignore_permissions=True
		)
	...
	def test_permission_query_condition(self):
		from nxenv.desk.doctype.dashboard_settings.dashboard_settings import (
		...
		dashboard_settings = nxenv.db.sql(
			f"""
				SELECT name
	...
	def test_virtual_doctype(self):
		...
		class VirtualDocType:
			...
			def get_list(args=None, limit_page_length=0, doctype=None):
				from nxenv.types.filter import FilterTuple
		...
		with patch(
			"nxenv.controllers",
			new={nxenv.local.site: {"Virtual DocType": VirtualDocType}},
		):
			nxenv.get_all("Virtual DocType", filters={"name": "test"}, fields=["name"], limit=1)

	def test_coalesce_with_in_ops(self):
		self.assertNotIn("ifnull", nxenv.get_all("User", {"first_name": ("in", ["a", "b"])}, run=0))
		self.assertIn("ifnull", nxenv.get_all("User", {"first_name": ("in", ["a", None])}, run=0))
		self.assertIn("ifnull", nxenv.get_all("User", {"first_name": ("in", ["a", ""])}, run=0))
		self.assertIn("ifnull", nxenv.get_all("User", {"first_name": ("in", [])}, run=0))
		self.assertIn("ifnull", nxenv.get_all("User", {"first_name": ("not in", ["a"])}, run=0))
		self.assertIn("ifnull", nxenv.get_all("User", {"first_name": ("not in", [])}, run=0))
		self.assertIn("ifnull", nxenv.get_all("User", {"first_name": ("not in", [""])}, run=0))

		# primary key is never nullable
		self.assertNotIn("ifnull", nxenv.get_all("User", {"name": ("in", ["a", None])}, run=0))
		self.assertNotIn("ifnull", nxenv.get_all("User", {"name": ("in", ["a", ""])}, run=0))
		self.assertNotIn("ifnull", nxenv.get_all("User", {"name": ("in", (""))}, run=0))
		self.assertNotIn("ifnull", nxenv.get_all("User", {"name": ("in", ())}, run=0))

	def test_coalesce_with_datetime_ops(self):
		self.assertNotIn("ifnull", nxenv.get_all("User", {"last_active": (">", "2022-01-01")}, run=0))
		self.assertNotIn("ifnull", nxenv.get_all("User", {"creation": ("<", "2022-01-01")}, run=0))
		self.assertNotIn(
			"ifnull",
			nxenv.get_all("User", {"last_active": ("between", ("2022-01-01", "2023-01-01"))}, run=0),
		)
		self.assertIn("ifnull", nxenv.get_all("User", {"last_active": ("<", "2022-01-01")}, run=0))

	def test_ambiguous_linked_tables(self):
		from nxenv.desk.reportview import get

		if not nxenv.db.exists("DocType", "Related Todos"):
			nxenv.get_doc(
				{
					"doctype": "DocType",
					"custom": 1,
			...
			nxenv.db.delete("Related Todos")

		todo_one = nxenv.get_doc(
			{
				"doctype": "ToDo",
		...
		nxenv.form_dict.doctype = "Related Todos"
	...
	def test_select_star_expansion(self):
		count = nxenv.get_list("Language", ["SUM(1)", "COUNT(*)"], as_list=1, order_by=None)[0]
		self.assertEqual(count[0], nxenv.db.count("Language"))
		self.assertEqual(count[1], nxenv.db.count("Language"))
...
class TestReportView(IntegrationTestCase):
	...
	def test_get_count(self):
		...
		count = execute_cmd("nxenv.desk.reportview.get_count")
		nxenv.local.form_dict = nxenv._dict(
			{
				"doctype": "DocType",
		...
		dict_filter_response = execute_cmd("nxenv.desk.reportview.get_count")
		...
		child_filter_response = execute_cmd("nxenv.desk.reportview.get_count")
		current_value = nxenv.db.sql(
	...
	def test_reportview_get(self):
		...
		response = execute_cmd("nxenv.desk.reportview.get")
	...
	def test_reportview_get_permlevel_system_users(self):
		with setup_patched_blog_post(), setup_test_user(set_user=True):
			...
			self.assertEqual(nxenv.get_list("Blog Post", "published"), [])
	...
	def test_db_filter_not_set(self):
		...
		"""
		nxenv.get_doc({"doctype": "ToDo", "description": "filter test"}).insert()
		nxenv.get_doc({"doctype": "ToDo", "description": "filter test", "reference_name": ""}).insert()
		...
		self.assertEqual(
			len(nxenv.get_all("ToDo", filters={"reference_name": ["is", "not set"]})),
			nxenv.db.count("ToDo", {"reference_name": ["is", "not set"]}),
		)


def add_child_table_to_blog_post():
	child_table = nxenv.get_doc(
		{
			"doctype": "DocType",
			"istable": 1,
			"custom": 1,
...
def create_event(subject="_Test Event", starts_on=None):
	...
	from nxenv.utils import get_datetime
...
def create_nested_doctype():
	if nxenv.db.exists("DocType", "Nested DocType"):
		return
...
def create_nested_doctype_records():
	...
	for r in records:
		d = nxenv.new_doc("Nested DocType")
		d.update(r)
