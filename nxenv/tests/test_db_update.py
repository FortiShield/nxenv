...
class TestDBUpdate(IntegrationTestCase):
	def test_db_update(self):
		doctype = "User"
		nxenv.reload_doctype("User", force=True)
		nxenv.model.meta.trim_tables("User")
		...
		table_columns = nxenv.db.get_table_columns_description(f"tab{doctype}")
		...
		for field_def in field_defs:
			...
			fallback_default = (
				"0" if field_def.get("fieldtype") in nxenv.model.numeric_fieldtypes else "NULL"
			)
	...
	def test_index_and_unique_constraints(self):
		...
		nxenv.db.add_index(doctype, ["email_signature(200)"])
		nxenv.db.updatedb(doctype)
	...
	def test_bigint_conversion(self):
		...
		with self.assertRaises(nxenv.CharacterLengthExceededError):
			nxenv.get_doc(doctype=doctype.name, int_field=2**62 - 1).insert()
	...
	def test_unique_index_on_install(self):
		...
		for dt in nxenv.get_all("DocType", {"is_virtual": 0, "issingle": 0}, pluck="name"):
			doctype = nxenv.get_meta(dt)
	...
	def test_unique_index_on_alter(self):
		...
		new_field = nxenv.copy_doc(doctype.fields[0])
	...
	def test_uuid_varchar_migration(self):
		...
		doctype.save()
		self.assertEqual(nxenv.db.get_column_type(doctype.name, "name"), "uuid")

		doc = nxenv.new_doc(doctype.name).insert()

		doctype.autoname = "hash"
		doctype.save()
		varchar = "varchar" if nxenv.db.db_type == "mariadb" else "character varying"
		self.assertIn(varchar, nxenv.db.get_column_type(doctype.name, "name"))
	...
	def test_uuid_link_field(self):
		...
		self.assertEqual(nxenv.db.get_column_type(uuid_doctype.name, "name"), "uuid")
		...
		self.assertEqual(nxenv.db.get_column_type(referring_doctype.name, link), "uuid")


def get_fieldtype_from_def(field_def):
	fieldtuple = nxenv.db.type_map.get(field_def.fieldtype, ("", 0))
...
def get_other_fields_meta(meta):
	...
	optional_fields = nxenv.db.OPTIONAL_COLUMNS
	if meta.track_seen:
		optional_fields.append("_seen")

	child_table_fields_map = {}
	if meta.istable:
		child_table_fields_map.update({field: ("Data", 0) for field in nxenv.db.CHILD_TABLE_COLUMNS})
	...
	return [
		nxenv._dict({"fieldname": field, "fieldtype": _type, "length": _length})
