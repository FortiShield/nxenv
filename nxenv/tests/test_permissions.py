...
from nxenv.core.doctype.user_permission.user_permission import clear_user_permissions
from nxenv.core.page.permission_manager.permission_manager import add, remove, reset, update
from nxenv.desk.form.load import getdoc
from nxenv.permissions import (
	ALL_USER_ROLE,
	AUTOMATIC_ROLES,
	GUEST_ROLE,
	SYSTEM_USER_ROLE,
...
class TestPermissions(IntegrationTestCase):
	...
	def setUpClass(cls):
		super().setUpClass()
		nxenv.clear_cache(doctype="Blog Post")
		user = nxenv.get_doc("User", "test1@example.com")
		user.add_roles("Website Manager")
		user.add_roles("System Manager")

		user = nxenv.get_doc("User", "test2@example.com")
		user.add_roles("Blogger")

		user = nxenv.get_doc("User", "test3@example.com")
		user.add_roles("Sales User")

		user = nxenv.get_doc("User", "testperm@example.com")
		user.add_roles("Website Manager")
	...
	def setUp(self):
		...
		nxenv.db.delete("User Permission")

		nxenv.set_user("test1@example.com")

	def tearDown(self):
		nxenv.set_user("Administrator")
		nxenv.db.set_value("Blogger", "_Test Blogger 1", "user", None)
	...
	def set_strict_user_permissions(ignore):
		ss = nxenv.get_doc("System Settings")
		ss.apply_strict_user_permissions = ignore
		ss.flags.ignore_mandatory = 1
		ss.save()

	def test_basic_permission(self):
		post = nxenv.get_doc("Blog Post", "-test-blog-post")
	...
	def test_select_permission(self):
		...
		nxenv.clear_cache(doctype="Blog Post")
		nxenv.set_user("test3@example.com")
		...
		self.assertRaises(nxenv.PermissionError, post.save)

		permitted_record = nxenv.get_list("Blog Post", fields="*", limit=1)[0]
		full_record = nxenv.get_all("Blog Post", fields="*", limit=1)[0]
	...
	def test_user_permissions_in_doc(self):
		...
		post1 = nxenv.get_doc("Blog Post", "-test-blog-post-1")
	...
	def test_user_permissions_in_report(self):
		...
		nxenv.set_user("test2@example.com")
		names = [d.name for d in nxenv.get_list("Blog Post", fields=["name", "blog_category"])]

		self.assertTrue("-test-blog-post-1" in names)
		self.assertFalse("-test-blog-post" in names)

	def test_default_values(self):
		doc = nxenv.new_doc("Blog Post")
	...
	def test_user_link_match_doc(self):
		blogger = nxenv.get_doc("Blogger", "_Test Blogger 1")
		blogger.user = "test2@example.com"
		blogger.save()

		nxenv.set_user("test2@example.com")

		post = nxenv.get_doc("Blog Post", "-test-blog-post-2")
		self.assertTrue(post.has_permission("read"))
	...
	def test_user_link_match_report(self):
		...
		names = [d.name for d in nxenv.get_list("Blog Post", fields=["name", "owner"])]
	...
	def test_not_allowed_to_set_user_permissions(self):
		...
		self.assertRaises(
			nxenv.PermissionError, add_user_permission, "Blog Post", "-test-blog-post", "test2@example.com"
		)
	...
	def test_read_if_explicit_user_permissions_are_set(self):
		...
		doc = nxenv.get_doc("Blog Post", "-test-blog-post")
		self.assertTrue(doc.has_permission("read"))

		# and not this one
		doc = nxenv.get_doc("Blog Post", "-test-blog-post-1")
	...
	def test_set_standard_fields_manually(self):
		...
		fake_owner = nxenv.db.get_value("User", {"name": ("!=", nxenv.session.user)})

		d = nxenv.new_doc("ToDo")
	...
	def test_dont_change_standard_constants(self):
		...
		self.assertRaises(nxenv.CannotChangeConstantError, user.save)
	...
	def test_set_only_once(self):
		blog_post = nxenv.get_meta("Blog Post")
		doc = nxenv.get_doc("Blog Post", "-test-blog-post-1")
		doc.db_set("title", "Old")
		blog_post.get_field("title").set_only_once = 1
		doc.title = "New"
		self.assertRaises(nxenv.CannotChangeConstantError, doc.save)
	...
	def test_set_only_once_child_table_rows(self):
		...
		doc = nxenv.get_doc("DocType", "Blog Post")
	...
	def test_user_permission_doctypes(self):
		...
		doc = nxenv.get_doc("Blog Post", "-test-blog-post-2")
		self.assertTrue(doc.has_permission("read"))
	...
	def test_insert_if_owner_with_user_permissions(self):
		...
		nxenv.delete_doc("Blog Post", "-test-blog-post-title")
		...
		self.assertRaises(nxenv.PermissionError, doc.insert)
	...
	def test_strict_user_permissions(self):
		...
		nxenv.set_user("Administrator")
		nxenv.db.delete("Contact")
		nxenv.db.delete("Contact Email")
		nxenv.db.delete("Contact Phone")
		...
		allowed_contact = nxenv.get_doc("Contact", "_Test Contact For _Test Customer")
		other_contact = nxenv.get_doc("Contact", "_Test Contact For _Test Supplier")

		nxenv.set_user("test3@example.com")
		self.assertTrue(allowed_contact.has_permission("read"))
		self.assertTrue(other_contact.has_permission("read"))
		self.assertEqual(len(nxenv.get_list("Contact")), 2)
		...
		self.assertTrue(len(nxenv.get_list("Contact")), 1)
	...
	def test_contextual_user_permission(self):
		...
		posts = nxenv.get_all("Blog Post", fields=["name", "blogger"])
		...
		posts = nxenv.get_list("Blog Post", fields=["name", "blogger"])
	...
	def test_if_owner_permission_overrides_properly(self):
		...
		nxenv.set_user("test2@example.com")
		self.assertEqual(nxenv.get_list("Blog Post"), [])
	...
	def test_if_owner_permission_on_getdoc(self):
		...
		doclist = [d.name for d in nxenv.response.docs]
		self.assertTrue(doc.name in doclist)

		nxenv.set_user("test2@example.com")
		self.assertRaises(nxenv.PermissionError, getdoc, "Blog Post", doc.name)
	...
	def test_if_owner_permission_on_get_list(self):
		...
		nxenv.set_user("test2@example.com")
		self.assertIn(doc.name, nxenv.get_list("Blog Post", pluck="name"))
		...
		nxenv.set_user("test2@example.com")
		self.assertNotIn(doc.name, nxenv.get_list("Blog Post", pluck="name"))
	...
	def test_if_owner_permission_on_delete(self):
		...
		self.assertRaises(nxenv.PermissionError, nxenv.delete_doc, "Blog Post", doc.name)

		nxenv.set_user("test2@example.com")
		nxenv.delete_doc("Blog Post", "-test-blog-post-title-new-1")
	...
	def test_clear_user_permissions(self):
		...
		# reset the user
		nxenv.set_user(current_user)

	def test_child_permissions(self):
		nxenv.set_user("test3@example.com")
		self.assertIsInstance(nxenv.get_list("DefaultValue", parent_doctype="User", limit=1), list)

		# nxenv.get_list
		self.assertRaises(nxenv.PermissionError, nxenv.get_list, "DefaultValue")
		self.assertRaises(nxenv.PermissionError, nxenv.get_list, "DefaultValue", parent_doctype="ToDo")
		self.assertRaises(
			nxenv.PermissionError, nxenv.get_list, "DefaultValue", parent_doctype="DefaultValue"
		)
		...
		self.assertRaises(nxenv.PermissionError, doc.check_permission)
		...
		user = nxenv.get_doc("User", "Administrator")
		doc = user.append("defaults")
		self.assertRaises(nxenv.PermissionError, doc.check_permission)
	...
	def test_select_user(self):
		...
		for gender in ("Male", "Female"):
			if nxenv.db.exists("Gender", gender):
				continue

			nxenv.get_doc({"doctype": "Gender", "gender": gender}).insert()

		# asssign gender to test users
		nxenv.db.set_value("User", "test1@example.com", "gender", "Male")
		nxenv.db.set_value("User", "test2@example.com", "gender", "Female")
		nxenv.db.set_value("User", "test3@example.com", "gender", "Female")
		...
		nxenv.set_user("test3@example.com")
		users = nxenv.get_list("User", pluck="name")
	...
	def test_automatic_permissions(self):
		def assertHasRole(*roles: str | tuple[str, ...]):
			for role in roles:
				self.assertIn(role, nxenv.get_roles())
	...
	def test_overrides_work_as_expected(self):
		...
		custom_role = nxenv.new_doc("Role", role_name=nxenv.generate_hash()).insert().name
		with self.set_user("Administrator"):
