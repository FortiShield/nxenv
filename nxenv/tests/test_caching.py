...
from nxenv.utils.caching import redis_cache, request_cache, site_cache
...
@site_cache
def ping() -> str:
	register_with_external_service(nxenv.local.site)
	return nxenv.local.site
...
class TestSiteCache(NxenvAPITestCase):
...
class TestRedisCache(NxenvAPITestCase):
	...
	def test_user_cache(self):
		...
		def calculate_area(radius: float) -> float:
			nonlocal function_call_count
			PI_APPROX = ENGINEERING_PI if nxenv.session.user == "Engineer" else PI
			function_call_count += 1
...
class TestDocumentCache(NxenvAPITestCase):
	TEST_DOCTYPE = "User"
	TEST_DOCNAME = "Administrator"
	TEST_FIELD = "middle_name"

	def setUp(self) -> None:
		self.test_value = nxenv.generate_hash()

	def test_caching(self):
		nxenv.get_cached_doc(self.TEST_DOCTYPE, self.TEST_DOCNAME)

		with self.assertQueryCount(0):
			doc = nxenv.get_cached_doc(self.TEST_DOCTYPE, self.TEST_DOCNAME)

		doc.db_set(self.TEST_FIELD, self.test_value)
		new_doc = nxenv.get_cached_doc(self.TEST_DOCTYPE, self.TEST_DOCNAME)

		self.assertIsNot(doc, new_doc)  # Shouldn't be same object from nxenv.local
		self.assertEqual(new_doc.get(self.TEST_FIELD), self.test_value)  # Cache invalidated and fetched
		nxenv.db.rollback()

		doc_after_rollback = nxenv.get_cached_doc(self.TEST_DOCTYPE, self.TEST_DOCNAME)
...
class TestRedisWrapper(NxenvAPITestCase):
	def test_delete_keys(self):
		prefix = "test_del_"

		for i in range(5):
			nxenv.cache.set_value(f"{prefix}{i}", 1)

		self.assertEqual(len(nxenv.cache.get_keys(prefix)), 5)
		nxenv.cache.delete_keys(prefix)
		self.assertEqual(len(nxenv.cache.get_keys(prefix)), 0)

	def test_hash(self):
		key = "test_hash"

		# Confirm that there's no data initially
		exists = nxenv.cache.exists(key)
		self.assertFalse(exists)

		# Insert 5 key-value pairs
		for i in range(5):
			nxenv.cache.hset(key, f"key_{i}", f"value_{i}")

		# Check that we have 5 values
		values = nxenv.cache.hgetall(key)
		self.assertEqual(len(values), 5)

		# Check that each value matches
		for i in range(5):
			value = nxenv.cache.hget(key, f"key_{i}")
			self.assertEqual(value, f"value_{i}")

		# Check the keys themselves
		keys = nxenv.cache.hkeys(key)
		for i in range(5):
			self.assertIn(f"key_{i}".encode(), keys)

		# Delete a single key and check that we still have the remaining 4
		nxenv.cache.hdel(key, "key_1")
		values = nxenv.cache.hgetall(key)
		self.assertEqual(len(values), 4)

		# Delete 2 keys and check that we still have the remaining 2
		nxenv.cache.hdel(key, ["key_2", "key_3"])
		values = nxenv.cache.hgetall(key)
		self.assertEqual(len(values), 2)

		# Delete the hash itself and confirm that there's no data
		nxenv.cache.delete_value(key)
		exists = nxenv.cache.exists(key)
		self.assertFalse(exists)

	def test_user_cache_clear(self):
		from nxenv.cache_manager import user_cache_keys

		# Set some keys that a user's cache would usually have
		user1 = nxenv.utils.random_string(10)
		user2 = nxenv.utils.random_string(10)
		for key in user_cache_keys:
			nxenv.cache.hset(key, user1, key)
			nxenv.cache.hset(key, user2, key)

		nxenv.clear_cache(user=user1)

		# Check that the keys for user1 are gone
		for key in set(user_cache_keys) - {"home_page"}:
			self.assertFalse(nxenv.cache.hexists(key, user1))
			self.assertTrue(nxenv.cache.hexists(key, user2))

	def test_doctype_cache_clear(self):
		from nxenv.cache_manager import doctype_cache_keys

		# Set some keys that a user's cache would usually have
		doctype1 = new_doctype(nxenv.utils.random_string(10))
		doctype2 = new_doctype(nxenv.utils.random_string(10))
		for key in doctype_cache_keys:
			nxenv.cache.hset(key, doctype1.name, key)
			nxenv.cache.hset(key, doctype2.name, key)

		nxenv.clear_cache(doctype=doctype1.name)

		# Check that the keys for doctype1 are gone
		for key in doctype_cache_keys:
			self.assertFalse(nxenv.cache.hexists(key, doctype1.name))
			self.assertTrue(nxenv.cache.hexists(key, doctype2.name))

	def test_backward_compat_cache(self):
		self.assertEqual(nxenv.cache, nxenv.cache())
