...
import nxenv
from nxenv.app import process_response
...
class TestCORS(IntegrationTestCase):
	def make_request_and_test(self, origin="http://example.com", absent=False):
		...
		nxenv.utils.set_request(method="OPTIONS", headers=headers)
	...
	def test_valid_origin(self):
		...
		nxenv.conf.allow_cors = "*"
		self.make_request_and_test()

		nxenv.conf.allow_cors = ["http://example.com", "https://example.com"]
		self.make_request_and_test()

	def test_invalid_origin(self):
		nxenv.conf.allow_cors = "http://example1.com"
		self.make_request_and_test(absent=True)

		nxenv.conf.allow_cors = ["http://example1.com", "https://example.com"]
		self.make_request_and_test(absent=True)
