...
import nxenv
from nxenv.utils import get_request_session
...
def make_request(method: str, url: str, auth=None, headers=None, data=None, json=None, params=None):
	...
	try:
		...
		response = nxenv.flags.integration_request = s.request(
		...
		if nxenv.flags.integration_request_doc:
			nxenv.flags.integration_request_doc.log_error()
		else:
			nxenv.log_error()
		raise exc
...
def create_request_log(
	...
	**kwargs,
	...
	integration_request = nxenv.get_doc(
		{
...
def get_json(obj):
	return obj if isinstance(obj, str) else nxenv.as_json(obj, indent=1)
