...
import nxenv
from nxenv.utils import get_request_site_address

CALLBACK_METHOD = "/api/method/nxenv.integrations.google_oauth.callback"
_SCOPES = {
...
_DOMAIN_CALLBACK_METHODS = {
	"mail": "nxenv.email.oauth.authorize_google_access",
	"contacts": "nxenv.integrations.doctype.google_contacts.google_contacts.authorize_access",
	"drive": "nxenv.integrations.doctype.google_drive.google_drive.authorize_access",
	"indexing": "nxenv.website.doctype.website_settings.google_indexing.authorize_access",
}
...
class GoogleOAuth:
	...
	def __init__(self, domain: str, validate: bool = True):
		self.google_settings = nxenv.get_single("Google Settings")
	...
	def validate_google_settings(self):
		...
		if not self.google_settings.enable:
			nxenv.throw(nxenv._("Please enable {} before continuing.").format(google_settings))

		if not (self.google_settings.client_id and self.google_settings.client_secret):
			nxenv.throw(nxenv._("Please update {} before continuing.").format(google_settings))
...
def handle_response(
	...
	if "error" in response:
		nxenv.log_error(nxenv._(error_title), nxenv._(response.get("error_description", error_message)))

		if raise_err:
			nxenv.throw(nxenv._(error_title), GoogleAuthenticationError, nxenv._(error_message))
...
@nxenv.whitelist(methods=["GET"])
def callback(state: str, code: str | None = None, error: str | None = None) -> None:
	"""Common callback for google integrations.
	Invokes functions using `nxenv.get_attr` and also adds required (keyworded) arguments
	along with committing and redirecting us back to nxenv site."""
	...
	if not error:
		if (domain := state.pop("domain")) in _DOMAIN_CALLBACK_METHODS:
			...
			nxenv.get_attr(_DOMAIN_CALLBACK_METHODS[domain])(**state)
	...
	nxenv.local.response["location"] = f"{redirect}?{failure_query_param if error else success_query_param}"
