...
from nxenv.utils.background_jobs import get_queues_timeout
from nxenv.utils.jinja import validate_template
from nxenv.utils.safe_exec import get_safe_globals

WEBHOOK_SECRET_HEADER = "X-Nxenv-Webhook-Signature"
...
class Webhook(Document):
	...
	if TYPE_CHECKING:
		from nxenv.integrations.doctype.webhook_data.webhook_data import WebhookData
		from nxenv.integrations.doctype.webhook_header.webhook_header import WebhookHeader
	...
	def validate_docevent(self):
		if self.webhook_doctype:
			is_submittable = nxenv.get_value("DocType", self.webhook_doctype, "is_submittable")
			if not is_submittable and self.webhook_docevent in [
				"on_submit",
				"on_cancel",
				"on_update_after_submit",
			]:
				nxenv.throw(_("DocType must be Submittable for the selected Doc Event"))

	def validate_condition(self):
		temp_doc = nxenv.new_doc(self.webhook_doctype)
		if self.condition:
			try:
				nxenv.safe_eval(self.condition, eval_locals=get_context(temp_doc))
			except Exception as e:
				nxenv.throw(_("Invalid Condition: {}").format(e))
	...
	def validate_request_url(self):
		try:
			...
			nxenv.throw(_("Check Request URL"), exc=e)
	...
	def validate_repeating_fields(self):
		...
		if len(webhook_data) != len(set(webhook_data)):
			nxenv.throw(_("Same Field is entered more than once"))
	...
	def validate_secret(self):
		if self.enable_security:
			try:
				...
				nxenv.throw(_("Invalid Webhook Secret"))
	...
	def preview_meets_condition(self, preview_document):
		if not self.condition:
			return _("Yes")
		try:
			doc = nxenv.get_cached_doc(self.webhook_doctype, preview_document)
			met_condition = nxenv.safe_eval(self.condition, eval_locals=get_context(doc))
		except Exception as e:
	...
	def preview_request_body(self, preview_document):
		try:
			doc = nxenv.get_cached_doc(self.webhook_doctype, preview_document)
			return nxenv.as_json(get_webhook_data(doc, self))
		except Exception as e:
			nxenv.local.message_log = []
...
def get_context(doc):
	return {"doc": doc, "utils": get_safe_globals().get("nxenv").get("utils")}


def enqueue_webhook(doc, webhook) -> None:
	request_url = headers = data = r = None
	try:
		webhook: Webhook = nxenv.get_doc("Webhook", webhook.get("name"))
		request_url = webhook.request_url
		if webhook.is_dynamic_url:
			request_url = nxenv.render_template(webhook.request_url, get_context(doc))
		headers = get_webhook_headers(doc, webhook)
		data = get_webhook_data(doc, webhook)

	except Exception as e:
		nxenv.logger().debug({"enqueue_webhook_error": e})
	...
	for i in range(3):
		try:
			...
			r.raise_for_status()
			nxenv.logger().debug({"webhook_success": r.text})
			log_request(webhook.name, doc.doctype, doc.name, request_url, headers, data, r)
			break

		except requests.exceptions.ReadTimeout as e:
			nxenv.logger().debug({"webhook_error": e, "try": i + 1})
			log_request(webhook.name, doc.doctype, doc.name, request_url, headers, data)
...
def log_request(
	...
	request_log = nxenv.get_doc(
		{
			"doctype": "Webhook Request Log",
			"webhook": webhook,
			"reference_doctype": doctype,
			"reference_document": docname,
			"user": nxenv.session.user if nxenv.session.user else None,
			"url": url,
			"headers": nxenv.as_json(headers) if headers else None,
			"data": nxenv.as_json(data) if data else None,
...
def get_webhook_data(doc, webhook):
	...
	if webhook.webhook_data:
		...
		data = nxenv.render_template(webhook.webhook_json, get_context(doc))
