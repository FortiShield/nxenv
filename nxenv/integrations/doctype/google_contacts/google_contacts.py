...
class GoogleContacts(Document):
	...
	def validate(self):
		if not nxenv.db.get_single_value("Google Settings", "enable"):
			nxenv.throw(_("Enable Google API in Google Settings."))

	def get_access_token(self):
		if not self.refresh_token:
			button_label = nxenv.bold(_("Allow Google Contacts Access"))
...
def authorize_access(g_contact, reauthorize=False, code=None):
	...
	"""
	contact = nxenv.get_doc("Google Contacts", g_contact)
...
def get_google_contacts_object(g_contact):
	...
	account = nxenv.get_doc("Google Contacts", g_contact)
	oauth_obj = GoogleOAuth("contacts")
...
def sync(g_contact=None):
	...
	google_contacts = nxenv.get_list("Google Contacts", filters=filters)
...
def sync_contacts_from_google_contacts(g_contact):
	...
	nxenv.db.set_value("Google Contacts", account.name, "last_sync_on", nxenv.utils.now_datetime())

	for idx, connection in enumerate(results):
		nxenv.publish_realtime(
			"import_google_contacts", dict(progress=idx + 1, total=len(results)), user=nxenv.session.user
		)
		# Work-around to fix
		# https://github.com/nxenv/nxenv/issues/22648
...
def insert_contacts_to_google_contacts(doc, method=None):
	...
	"""
	if (
		not nxenv.db.exists("Google Contacts", {"name": doc.google_contacts})
	...
	try:
		...
		nxenv.db.set_value("Contact", doc.name, "google_contacts_id", contact.get("resourceName"))
	except HttpError as err:
		nxenv.msgprint(
...
def update_contacts_to_google_contacts(doc, method=None):
	...
	try:
		...
		nxenv.msgprint(_("Contact Synced with Google Contacts."))
	except HttpError as err:
		nxenv.msgprint(
