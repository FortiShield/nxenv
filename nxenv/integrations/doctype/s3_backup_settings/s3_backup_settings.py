...
class S3BackupSettings(Document):
	...
	def validate(self):
		...
		try:
			...
			bucket_name = nxenv.bold(self.bucket)
			if error_code == "403":
...
def take_backup():
	...
	enqueue(
		"nxenv.integrations.doctype.s3_backup_settings.s3_backup_settings.take_backups_s3",
		queue="long",
		timeout=1500,
	)
...
def take_backups_if(freq):
	if cint(nxenv.db.get_single_value("S3 Backup Settings", "enabled")):
		if nxenv.db.get_single_value("S3 Backup Settings", "frequency") == freq:
			take_backups_s3()
