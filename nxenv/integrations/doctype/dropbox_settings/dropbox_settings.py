...
import nxenv
from nxenv import _
from nxenv.integrations.offsite_backup_utils import (
	get_chunk_site,
...
from nxenv.utils import cint, encode, get_backups_path, get_files_path, get_request_site_address
from nxenv.utils.background_jobs import enqueue
from nxenv.utils.backups import new_backup
...
class DropboxSettings(Document):
	...
	def onload(self):
		if not self.app_access_key and nxenv.conf.dropbox_access_key:
			self.set_onload("dropbox_setup_via_site_config", 1)

	def validate(self):
		if self.enabled and self.limit_no_of_backups and self.no_of_backups < 1:
			nxenv.throw(_("Number of DB backups cannot be less than 1"))
...
def take_backup():
	...
	enqueue(
		"nxenv.integrations.doctype.dropbox_settings.dropbox_settings.take_backup_to_dropbox",
		queue="long",
		timeout=1500,
	)
	nxenv.msgprint(_("Queued for backup. It may take a few minutes to an hour."))
...
def take_backups_if(freq):
	if nxenv.db.get_single_value("Dropbox Settings", "backup_frequency") == freq:
...
def take_backup_to_dropbox(retry_count=0, upload_db_backup=True):
	...
	try:
		if cint(nxenv.db.get_single_value("Dropbox Settings", "enabled")):
			validate_file_size()
			...
			if cint(nxenv.db.get_single_value("Dropbox Settings", "send_email_for_successful_backup")):
...
def backup_to_dropbox(upload_db_backup=True):
	...
	if upload_db_backup:
		if nxenv.flags.create_new_backup:
...
def upload_from_folder(path, is_private, dropbox_folder, dropbox_client, did_not_upload, error_log):
	...
	if is_fresh_upload():
		...
		response = nxenv._dict({"entries": []})

	path = str(path)

	for f in nxenv.get_all(
		"File",
		...
		for file_metadata in response.entries:
			try:
				...
				error_log.append(nxenv.get_traceback())
...
def upload_file_to_dropbox(filename, folder, dropbox_client):
	...
	try:
		...
		if isinstance(e.error, dropbox.files.UploadError):
			...
			error += nxenv.get_traceback()
			nxenv.log_error(error)
		else:
			raise
...
def update_file_dropbox_status(file_name):
	nxenv.db.set_value("File", file_name, "uploaded_to_dropbox", 1, update_modified=False)


def is_fresh_upload():
	file_name = nxenv.db.get_value("File", {"uploaded_to_dropbox": 1}, "name")
...
def get_dropbox_settings(redirect_uri=False):
	...
	settings = nxenv.get_doc("Dropbox Settings")
	app_details = {
		"app_key": settings.app_access_key or nxenv.conf.dropbox_access_key,
		"app_secret": settings.get_password(fieldname="app_secret_key", raise_exception=False)
		if settings.app_secret_key
		else nxenv.conf.dropbox_secret_key,
		...
		app_details.update(
			{
				...
				+ "/api/method/nxenv.integrations.doctype.dropbox_settings.dropbox_settings.dropbox_auth_finish"
			}
		)
...
def dropbox_auth_finish():
	...
	if not callback.state or not callback.code:
		nxenv.respond_as_web_page(
			...
			indicator_color="red",
			http_status_code=nxenv.AuthenticationError.http_status_code,
		)
		return
	...
	nxenv.local.response["location"] = "/app/dropbox-settings"
...
def set_dropbox_token(access_token, refresh_token=None):
	...
	dropbox_settings = nxenv.get_single("Dropbox Settings")
