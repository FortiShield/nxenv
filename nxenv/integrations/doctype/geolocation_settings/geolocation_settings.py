...
def autocomplete(txt: str) -> list[dict]:
	...
	settings = nxenv.get_single("Geolocation Settings")
	if not settings.enable_address_autocompletion:
		return []

	if settings.provider == "Geoapify":
		provider = Geoapify(settings.get_password("api_key"), nxenv.local.lang)
		...
		provider = Here(settings.get_password("api_key"), nxenv.local.lang)
	else:
		nxenv.throw(_("This geolocation provider is not supported yet."))
