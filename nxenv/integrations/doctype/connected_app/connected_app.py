...
if any((os.getenv("CI"), nxenv.conf.developer_mode, nxenv.conf.allow_tests)):
...
class ConnectedApp(Document):
	...
	if TYPE_CHECKING:
		from nxenv.integrations.doctype.oauth_scope.oauth_scope import OAuthScope
		from nxenv.integrations.doctype.query_parameters.query_parameters import QueryParameters
	...
	def validate(self):
		base_url = nxenv.utils.get_url()
		callback_path = (
			"/api/method/nxenv.integrations.doctype.connected_app.connected_app.callback/" + self.name
		)
	...
	def initiate_web_application_flow(self, user=None, success_uri=None):
		...
		if not token_cache:
			token_cache = nxenv.new_doc("Token Cache")
	...
	def get_user_token(self, user=None, success_uri=None):
		...
		nxenv.local.response["location"] = redirect
		return redirect
	...
	def get_token_cache(self, user):
		...
		if nxenv.db.exists("Token Cache", token_cache_name):
			token_cache = nxenv.get_doc("Token Cache", token_cache_name)
...
@nxenv.whitelist(methods=["GET"], allow_guest=True)
def callback(code=None, state=None):
	...
	if nxenv.session.user == "Guest":
		nxenv.local.response["type"] = "redirect"
		nxenv.local.response["location"] = "/login?" + urlencode({"redirect-to": nxenv.request.url})
		return

	path = nxenv.request.path[1:].split("/")
	if len(path) != 4 or not path[3]:
		nxenv.throw(_("Invalid Parameters."))

	connected_app = nxenv.get_doc("Connected App", path[3])
	token_cache = nxenv.get_doc("Token Cache", connected_app.name + "-" + nxenv.session.user)

	if state != token_cache.state:
		nxenv.throw(_("Invalid token state! Check if the token has been created by the OAuth user."))
	...
	nxenv.local.response["type"] = "redirect"
	nxenv.local.response["location"] = token_cache.get("success_uri") or connected_app.get_url()


@nxenv.whitelist()
def has_token(connected_app, connected_user=None):
	app = nxenv.get_doc("Connected App", connected_app)
	token_cache = app.get_token_cache(connected_user or nxenv.session.user)
