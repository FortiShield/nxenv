...
import nxenv
from nxenv.integrations.doctype.social_login_key.test_social_login_key import (
...
def get_connected_app():
	...
	connected_app = nxenv.new_doc(doctype)
	connected_app.provider_name = "nxenv"
...
def get_oauth_client():
	oauth_client = nxenv.new_doc("OAuth Client")
...
class TestConnectedApp(IntegrationTestCase):
	def setUp(self):
		...
		Nxenv comes with it's own oAuth2 provider that we can test against. The
		...
		Nxenv's "Authorization URL" and "Access Token URL" (actually they're
	...
	def test_web_application_flow(self):
		...
		resp = oauth2_session.get(urljoin(self.base_url, "/api/method/nxenv.auth.get_logged_user"))
	...
	def tearDown(self):
		...
		if getattr(self, "oauth_client", None):
			tokens = nxenv.get_all("OAuth Bearer Token", filters={"client": self.oauth_client.name})
			for token in tokens:
				doc = nxenv.get_doc("OAuth Bearer Token", token.name)
				doc.delete()

			codes = nxenv.get_all("OAuth Authorization Code", filters={"client": self.oauth_client.name})
			for code in codes:
				doc = nxenv.get_doc("OAuth Authorization Code", code.name)
				doc.delete()
