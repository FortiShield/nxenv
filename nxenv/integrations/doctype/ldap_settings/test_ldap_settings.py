...
import nxenv
from nxenv.exceptions import MandatoryError, ValidationError
...
class LDAP_TestCase:
	...
	def mock_ldap_connection(f):
		...
		def wrapped(self, *args, **kwargs):
			with mock.patch(
				"nxenv.integrations.doctype.ldap_settings.ldap_settings.LDAPSettings.connect_to_ldap",
	...
	def clean_test_users():
		with contextlib.suppress(Exception):
			nxenv.get_doc("User", "posix.user1@unit.testing").delete()
		with contextlib.suppress(Exception):
			nxenv.get_doc("User", "posix.user2@unit.testing").delete()
		with contextlib.suppress(Exception):
			nxenv.get_doc("User", "website_ldap_user@test.com").delete()

	@classmethod
	def setUpClass(cls):
		cls.clean_test_users()
		# Save user data for restoration in tearDownClass()
		cls.user_ldap_settings = nxenv.get_doc("LDAP Settings")
		...
		user = nxenv.get_doc(cls.user1doc)
		...
		user = nxenv.get_doc(cls.user2doc)
		user.insert(ignore_permissions=True)
	...
	def tearDownClass(cls):
		with contextlib.suppress(Exception):
			nxenv.get_doc("LDAP Settings").delete()
	...
	def test_connect_to_ldap(self):
		...
		prevent_connection_parameters = {
			"mode": {
				"IP_V4_ONLY": "Locks the user to IPv4 without nxenv providing a way to configure",
				"IP_V6_ONLY": "Locks the user to IPv6 without nxenv providing a way to configure",
			},
			"auto_bind": {
	...
	def test_update_user_fields(self):
		...
		test_user = nxenv.get_doc("User", test_user_data["email"])
		self.test_class.update_user_fields(test_user, test_user_data)
		updated_user = nxenv.get_doc("User", test_user_data["email"])
		...
		self.assertIn(self.test_class.default_role, nxenv.get_roles(updated_user.name))
	...
	def test_create_website_user(self):
		...
		new_user = nxenv.get_doc("User", new_test_user_data["email"])
	...
	def test_sync_roles(self):
		...
		role_to_group_map = {
			...
			"All": "nxenv_default_all",
			"Guest": "nxenv_default_guest",
			"Desk User": "nxenv_default_desk_user",
		}

		# re-create user1 to ensure clean
		nxenv.get_doc("User", "posix.user1@unit.testing").delete()
		user = nxenv.get_doc(self.user1doc)
		user.insert(ignore_permissions=True)

		for test_user in test_user_data:
			test_user_doc = nxenv.get_doc("User", f"{test_user}@unit.testing")
			test_user_roles = nxenv.get_roles(f"{test_user}@unit.testing")

			self.assertTrue(
				len(test_user_roles) == 2, "User should only be a part of the All and Guest roles"
			)  # check default nxenv roles

			self.test_class.sync_roles(test_user_doc, test_user_data[test_user])  # update user roles

			nxenv.get_doc("User", f"{test_user}@unit.testing")
			updated_user_roles = nxenv.get_roles(f"{test_user}@unit.testing")
	...
	def test_create_or_update_user(self):
		...
		nxenv.get_doc("User", f"{test_user}@unit.testing").delete()

		with self.assertRaises(
			nxenv.exceptions.DoesNotExistError
		):  # ensure user deleted so function can be tested
			nxenv.get_doc("User", f"{test_user}@unit.testing")

		with mock.patch(
			"nxenv.integrations.doctype.ldap_settings.ldap_settings.LDAPSettings.update_user_fields"
		) as update_user_fields_method:
			with mock.patch(
				"nxenv.integrations.doctype.ldap_settings.ldap_settings.LDAPSettings.sync_roles"
	...
	def test_authenticate(self):
		with mock.patch(
			"nxenv.integrations.doctype.ldap_settings.ldap_settings.LDAPSettings.fetch_ldap_groups"
		...
		for username, password in enumerate(invalid_users):
			with self.assertRaises(nxenv.exceptions.ValidationError) as display_massage:
