...
import nxenv
from nxenv import _, safe_encode
from nxenv.model.document import Document
from nxenv.twofactor import authenticate_for_2factor, confirm_otp_token, should_run_2fa
...
class LDAPSettings(Document):
	...
	if TYPE_CHECKING:
		from nxenv.integrations.doctype.ldap_group_mapping.ldap_group_mapping import LDAPGroupMapping
	...
	def connect_to_ldap(self, base_dn, password, read_only=True) -> ldap3.Connection:
		try:
			...
			nxenv.throw(msg, title=_("LDAP Not Installed"))
		except LDAPInvalidCredentialsResult:
			nxenv.throw(_("Invalid username or password"))
		except Exception as ex:
			nxenv.throw(_(str(ex)))

	@staticmethod
	def get_ldap_client_settings() -> dict:
		# return the settings to be used on the client side.
		result = {"enabled": False}
		ldap = nxenv.get_cached_doc("LDAP Settings")
		if ldap.enabled:
			result["enabled"] = True
			result["method"] = "nxenv.integrations.doctype.ldap_settings.ldap_settings.login"
		return result
	...
	def create_or_update_user(self, user_data: dict, groups: list | None = None):
		...
		if nxenv.db.exists("User", user_data["email"]):
			user = nxenv.get_doc("User", user_data["email"])
		...
		if self.default_user_type == "System User":
			...
			role = nxenv.db.get_value("User Type", user.user_type, "role")
	...
	def authenticate(self, username: str, password: str):
		if not self.enabled:
			nxenv.throw(_("LDAP is not enabled."))
		...
		try:
			...
			nxenv.throw(_("Please use a valid LDAP search filter"), title=_("Misconfigured"))
		...
		if conn.search(
			...
			if conn.entries and conn.entries[0]:
				...
				if conn.modify(entry_dn, changes=changes):
					if logout_sessions:
						from nxenv.sessions import clear_sessions

						clear_sessions(user=user, force=True)
					nxenv.msgprint(_("Password changed successfully."))
				else:
					nxenv.throw(_("Failed to change password."))
			else:
				nxenv.throw(_("No Entry for the User {0} found within LDAP!").format(user))
		else:
			nxenv.throw(_("No LDAP User found for email: {0}").format(user))
...
def login():
	# LDAP LOGIN LOGIC
	args = nxenv.form_dict
	ldap: LDAPSettings = nxenv.get_doc("LDAP Settings")

	user = ldap.authenticate(nxenv.as_unicode(args.usr), nxenv.as_unicode(args.pwd))

	nxenv.local.login_manager.user = user.name
	if should_run_2fa(user.name):
		authenticate_for_2factor(user.name)
		if not confirm_otp_token(nxenv.local.login_manager):
			return False
