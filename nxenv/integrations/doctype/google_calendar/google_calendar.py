...
import nxenv
from nxenv import _
from nxenv.integrations.google_oauth import GoogleOAuth
...
from nxenv.utils.password import set_encrypted_password
...
class GoogleCalendar(Document):
	...
	def validate(self):
		google_settings = nxenv.get_single("Google Settings")
		if not google_settings.enable:
			nxenv.throw(_("Enable Google API in Google Settings."))

		if not google_settings.client_id or not google_settings.client_secret:
			nxenv.throw(_("Enter Client Id and Client Secret in Google Settings."))
	...
	def get_access_token(self):
		...
		if not self.refresh_token:
			button_label = nxenv.bold(_("Allow Google Calendar Access"))
			raise nxenv.ValidationError(_("Click on {0} to generate Refresh Token.").format(button_label))
...
def authorize_access(g_calendar, reauthorize=None):
	...
	"""
	google_settings = nxenv.get_doc("Google Settings")
	google_calendar = nxenv.get_doc("Google Calendar", g_calendar)
	google_calendar.check_permission("write")

	redirect_uri = (
		get_request_site_address(True)
		+ "?cmd=nxenv.integrations.doctype.google_calendar.google_calendar.google_callback"
	)

	if not google_calendar.authorization_code or reauthorize:
		nxenv.cache.hset("google_calendar", "google_calendar", google_calendar.name)
		...
		try:
			...
			nxenv.local.response["location"] = "/app/Form/{}/{}".format(
				quote("Google Calendar"), quote(google_calendar.name)
			)

			nxenv.msgprint(_("Google Calendar has been configured."))
...
def google_callback(code=None):
	...
	"""
	google_calendar = nxenv.cache.hget("google_calendar", "google_calendar")
	nxenv.db.set_value("Google Calendar", google_calendar, "authorization_code", code)
	nxenv.db.commit()
...
def sync(g_calendar=None):
	...
	google_calendars = nxenv.get_list("Google Calendar", filters=filters)
...
def get_google_calendar_object(g_calendar):
	...
	google_settings = nxenv.get_doc("Google Settings")
	account = nxenv.get_doc("Google Calendar", g_calendar)
	...
	try:
		if account.google_calendar_id:
			...
			calendar = {
				...
				"timeZone": nxenv.get_system_settings("time_zone"),
			}
...
def sync_events_from_google_calendar(g_calendar, method=None):
	...
	while True:
		try:
			...
			if err.resp.status == 410:
				...
				nxenv.msgprint(msg, title="Invalid Sync Token", indicator="blue")
			else:
				nxenv.throw(msg)
	...
	for idx, event in enumerate(results):
		nxenv.publish_realtime(
			"import_google_calendar", dict(progress=idx + 1, total=len(results)), user=nxenv.session.user
		)
		...
		if event.get("status") == "confirmed":
			...
			if not nxenv.db.exists("Event", {"google_calendar_event_id": event.get("id")}):
			...
			nxenv.get_doc(
				{
					...
					"reference_name": nxenv.db.get_value(
						"Event",
						{
							"google_calendar_id": account.google_calendar_id,
							"google_calendar_event_id": event.get("id"),
...
def insert_event_to_calendar(account, event, recurrence=None):
	"""
	Inserts event in Nxenv Calendar during Sync
	"""
	calendar_event = {
		"doctype": "Event",
	...
	nxenv.get_doc(calendar_event).insert(ignore_permissions=True)


def update_event_in_calendar(account, event, recurrence=None):
	"""
	Updates Event in Nxenv Calendar if any existing Google Calendar Event is updated
	"""
	calendar_event = nxenv.get_doc("Event", {"google_calendar_event_id": event.get("id")})
...
def insert_event_in_google_calendar(doc, method=None):
	...
	if (
		...
		or not nxenv.db.exists("Google Calendar", {"name": doc.google_calendar})
	):
		return
	...
	try:
		...
		nxenv.msgprint(_("Event Synced with Google Calendar."))
	except HttpError as err:
		nxenv.throw(
...
def update_event_in_google_calendar(doc, method=None):
	"""
	Updates Events in Google Calendar if any existing event is modified in Nxenv Calendar
	"""
...
def delete_event_from_google_calendar(doc, method=None):
	"""
	Delete Events from Google Calendar if Nxenv Event is deleted.
	"""

	if not nxenv.db.exists("Google Calendar", {"name": doc.google_calendar}):
		return
