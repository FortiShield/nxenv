...
from nxenv.model.base_document import (
...
from nxenv.utils import cast, cint, cstr
...
def get_meta(doctype: str | dict | DocRef, cached=True) -> "_Meta":
	...
	if cached and (
		...
		if meta := nxenv.client_cache.get_value(key):
			return meta

	meta = Meta(doctype)
	key = f"doctype_meta::{meta.name}"
	nxenv.client_cache.set_value(key, meta)
	return meta
...
def clear_meta_cache(doctype: str = "*"):
	...
	if doctype == "*":
		nxenv.cache.delete_keys(key)
	else:
		nxenv.client_cache.delete_value(key)
...
def get_table_columns(doctype):
	return nxenv.db.get_table_columns(doctype)


def load_doctype_from_file(doctype):
	fname = nxenv.scrub(doctype)
	with open(nxenv.get_app_path("nxenv", "core", "doctype", fname, fname + ".json")) as f:
...
class Meta(Document):
	...
	standard_set_once_fields = (
		nxenv._dict(fieldname="creation", fieldtype="Datetime"),
		nxenv._dict(fieldname="owner", fieldtype="Data"),
	)
	...
	def get_global_search_fields(self):
		...
		if getattr(self, "show_name_in_global_search", None):
			fields.append(nxenv._dict(fieldtype="Data", fieldname="name", label="Name"))
	...
	def _valid_columns(self):
		table_exists = nxenv.db.table_exists(self.name)
	...
	def _valid_fields(self):
		if (nxenv.flags.in_install or nxenv.flags.in_migrate) and self.name in self.special_doctypes:
	...
	def add_custom_fields(self):
		if not nxenv.db.table_exists("Custom Field"):
			return

		custom_fields = nxenv.db.get_values(
			"Custom Field",
	...
	def apply_property_setters(self):
		...
		"""
		if not nxenv.db.table_exists("Property Setter"):
			return

		property_setters = nxenv.db.get_values(
			"Property Setter",
	...
	def get_fields_to_check_permissions(self, user_permission_doctypes):
		...
		if self.name in user_permission_doctypes:
			fields.append(nxenv._dict({"label": "Name", "fieldname": "name", "options": self.name}))
	...
	def get_permitted_fieldnames(
		...
		*,
		...
		if not permission_type:
			permission_type = "select" if nxenv.only_has_select_perm(self.name, user=user) else "read"
		...
		if 0 not in permlevel_access and permission_type in ("read", "select"):
			if nxenv.share.get_shared(self.name, user, rights=[permission_type], limit=1):
	...
	def get_permissions(self, parenttype=None):
		if self.istable and parenttype:
			...
			permissions = nxenv.get_meta(parenttype).permissions
		else:
	...
	def get_dashboard_data(self):
		...
		implemented in other Nxenv applications via hooks.
		"""
		data = nxenv._dict()
		if not self.custom:
			try:
				...
				if hasattr(module, "get_data"):
					data = nxenv._dict(module.get_data())
			except ImportError:
				pass
		...
		if not self.custom:
			for hook in nxenv.get_hooks("override_doctype_dashboards", {}).get(self.name, []):
				data = nxenv._dict(nxenv.get_attr(hook)(data=data))
	...
	def get_web_template(self, suffix=""):
		...
		module_name = nxenv.scrub(self.module)
		doctype = nxenv.scrub(self.name)
		template_path = nxenv.get_module_path(
...
def is_single(doctype):
	try:
		return nxenv.db.get_value("DocType", doctype, "issingle")
	except IndexError:
		raise Exception("Cannot determine whether {} is single".format(doctype))


def get_parent_dt(dt):
	if not nxenv.is_table(dt):
		return ""

	return (
		nxenv.db.get_value(
			"DocField",
			{"fieldtype": ("in", nxenv.model.table_fields), "options": dt},
			"parent",
		)
		or ""
	)
...
def set_fieldname(field_id, fieldname):
	nxenv.db.set_value("DocField", field_id, "fieldname", fieldname)
...
def get_field_currency(df, doc=None):
	...
	if not getattr(nxenv.local, "field_currency", None):
		nxenv.local.field_currency = nxenv._dict()

	if not (
		nxenv.local.field_currency.get((doc.doctype, doc.name), {}).get(df.fieldname)
		or (
			doc.get("parent")
			and nxenv.local.field_currency.get((doc.doctype, doc.parent), {}).get(df.fieldname)
		)
	):
		...
		if ":" in cstr(df.get("options")):
			...
			if len(split_opts) == 3 and doc.get(split_opts[1]):
				currency = nxenv.get_cached_value(split_opts[0], doc.get(split_opts[1]), split_opts[2])
		else:
			...
			if doc.get("parenttype"):
				if currency:
					...
					if nxenv.get_meta(doc.parenttype).has_field(df.get("options")):
						# only get_value if parent has currency field
						currency = nxenv.db.get_value(doc.parenttype, doc.parent, df.get("options"))

		if currency:
			nxenv.local.field_currency.setdefault((doc.doctype, ref_docname), nxenv._dict()).setdefault(
				df.fieldname, currency
			)

	return nxenv.local.field_currency.get((doc.doctype, doc.name), {}).get(df.fieldname) or (
		doc.get("parent") and nxenv.local.field_currency.get((doc.doctype, doc.parent), {}).get(df.fieldname)
	)


def get_field_precision(df, doc=None, currency=None):
	"""get precision based on DocField options and fieldvalue in doc"""
	from nxenv.locale import get_number_format

	if df.precision:
		precision = cint(df.precision)

	elif df.fieldtype == "Currency":
		precision = cint(nxenv.db.get_default("currency_precision"))
		if not precision:
			number_format = get_number_format()
			precision = number_format.precision
	else:
		precision = cint(nxenv.db.get_default("float_precision")) or 3
...
def get_default_df(fieldname):
	if fieldname in (default_fields + child_table_fields):
		if fieldname in ("creation", "modified"):
			return nxenv._dict(fieldname=fieldname, fieldtype="Datetime")

		elif fieldname in ("idx", "docstatus"):
			return nxenv._dict(fieldname=fieldname, fieldtype="Int")

		elif fieldname in ("owner", "modified_by"):
			return nxenv._dict(fieldname=fieldname, fieldtype="Link", options="User")

		return nxenv._dict(fieldname=fieldname, fieldtype="Data")
...
def trim_tables(doctype=None, dry_run=False, quiet=False):
	...
	for doctype in nxenv.get_all("DocType", filters=filters, pluck="name"):
		try:
...
def trim_table(doctype, dry_run=True):
	nxenv.cache.hdel("table_columns", f"tab{doctype}")
	ignore_fields = default_fields + optional_fields + child_table_fields
	columns = nxenv.db.get_table_columns(doctype)
	fields = nxenv.get_meta(doctype, cached=False).get_fieldnames_with_value()
	...
	if columns_to_remove and not dry_run:
		...
		nxenv.db.sql_ddl(f"ALTER TABLE `tab{doctype}` {columns_to_remove}")
