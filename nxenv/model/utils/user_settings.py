...
def get_user_settings(doctype, for_update=False):
	user_settings = nxenv.cache.hget("_user_settings", f"{doctype}::{nxenv.session.user}")

	if user_settings is None:
		user_settings = nxenv.db.sql(
			"""select data from `__UserSettings`
			where `user`=%s and `doctype`=%s""",
			(nxenv.session.user, doctype),
		)
...
def update_user_settings(doctype, user_settings, for_update=False):
	...
	nxenv.cache.hset("_user_settings", f"{doctype}::{nxenv.session.user}", json.dumps(current))


def sync_user_settings():
	"""Sync from cache to database (called asynchronously via the browser)"""
	for key, data in nxenv.cache.hgetall("_user_settings").items():
		key = safe_decode(key)
...
def update_user_settings_data(
	...
	if data:
		...
		if update:
			...
			nxenv.cache.hset("_user_settings", f"{user_setting.doctype}::{user_setting.user}", None)
