...
def set_field_property(filters, key, value):
	...
	docs = [
		nxenv.get_doc("DocType", d.parent)
		for d in nxenv.get_all("DocField", fields=["parent"], filters=filters)
	]
...
class InvalidIncludePath(nxenv.ValidationError):
	pass
	...
	for _ in range(5):
		if "{% include" in content:
			...
			for path in paths:
				...
				with open(nxenv.get_app_path(app, app_path), encoding="utf-8") as f:
					include = f.read()
...
def is_virtual_doctype(doctype: str):
	if nxenv.flags.in_install or nxenv.flags.in_migrate:
		if nxenv.db.has_column("DocType", "is_virtual"):
			return nxenv.db.get_value("DocType", doctype, "is_virtual")
	else:
		return getattr(nxenv.get_meta(doctype), "is_virtual", False)
...
def is_single_doctype(doctype: str) -> bool:
	...
	if nxenv.flags.in_install or nxenv.flags.in_migrate:
		...
		return getattr(nxenv.get_meta(doctype), "issingle", False)
