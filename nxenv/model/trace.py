"""
Traced Fields for Nxenv

This module provides utilities for creating traced fields in Nxenv documents,
...
- Seamlessly integrate with Nxenv's document model

Example of standard usage:
		from nxenv.model.trace import TracedDocument, traced_field
...
See nxenv.tests.classes.context_managers for a context manager built into test classes.
"""
...
class TracedValue:
	"""
	A descriptor class for creating traced fields in Nxenv documents.

	This class allows for monitoring and validating changes to specific fields
	in a Nxenv document. It can enforce forbidden values and apply custom
	validation logic.
	...
	def __set__(self, obj, value):
		...
		if value in self.forbidden_values:
			if nxenv.flags.in_test:
				nxenv.throw(f"{self.field_name} cannot be set to {value}", AssertionError)
			else:
				nxenv.throw(f"{self.field_name} cannot be set to {value}")
		...
		if self.custom_validation:
			try:
				...
				if nxenv.flags.in_test:
					nxenv.throw(str(e), AssertionError)
				else:
					nxenv.throw(str(e))
...
class TracedDocument(Document):
	"""
	A base class for Nxenv documents with traced fields.

	This class extends Nxenv's Document class to provide support for
	traced fields created with TracedValue.

	Attributes:
	        Inherits all attributes from nxenv.model.document.Document
	"""
...
def traced_field(*args, **kwargs):
	...
	This function simplifies the creation of traced fields in Nxenv documents.
...
from nxenv.deprecation_dumpster import model_trace_traced_field_context as traced_field_context
