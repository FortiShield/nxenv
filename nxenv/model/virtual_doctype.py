...
class VirtualDoctype(Protocol):
	...
	- DocType controller has to inherit from `nxenv.model.document.Document` class
	...
	@staticmethod
	def get_list(**kwargs) -> list[nxenv._dict]:
...
def validate_controller(doctype: str) -> None:
	try:
		...
		nxenv.msgprint(_("Failed to import virtual doctype {}, is controller file present?").format(doctype))
		return
		...
		if not isinstance(method, staticmethod):
			nxenv.msgprint(
				_("Virtual DocType {} requires a static method called {} found {}").format(
					nxenv.bold(doctype), nxenv.bold(m), nxenv.bold(_as_str(method))
				),
