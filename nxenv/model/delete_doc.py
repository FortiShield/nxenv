...
from nxenv import _, get_module_path
from nxenv.desk.doctype.tag.tag import delete_tags_for_document
from nxenv.model.docstatus import DocStatus
from nxenv.model.dynamic_links import get_dynamic_link_map
from nxenv.model.naming import revert_series_if_last
from nxenv.model.utils import is_virtual_doctype
from nxenv.utils.file_manager import remove_all
from nxenv.utils.global_search import delete_for_document
from nxenv.utils.password import delete_all_passwords_for
...
def delete_doc(
	...
	# get from form
	if not doctype:
		doctype = nxenv.form_dict.get("dt")
		name = nxenv.form_dict.get("dn")
	...
	for name in names or []:
		if is_virtual:
			nxenv.get_doc(doctype, name).delete()
			continue
		...
		if doctype == "DocType":
			if for_reload:
				...
				doc = nxenv.get_doc(doctype, name)
				if not (doc.custom or nxenv.conf.developer_mode or nxenv.flags.in_patch or force):
					nxenv.throw(_("Standard DocType can not be deleted."))

				update_flags(doc, flags, ignore_permissions)
				check_permission_and_not_submitted(doc)
				# delete custom table fields using this doctype.
				nxenv.db.delete(
					"Custom Field", {"options": name, "fieldtype": ("in", nxenv.model.table_fields)}
				)
				nxenv.db.delete("__global_search", {"doctype": name})

			delete_from_table(doctype, name, ignore_doctypes, None)

			if (
				nxenv.conf.developer_mode
				and not doc.custom
				and not (
					for_reload
			...
			try:
				nxenv.db.get_value(doctype, name, for_update=True, wait=False)
			except nxenv.QueryTimeoutError:
				nxenv.throw(
					_(
						"This document can not be deleted right now as it's being modified by another user. Please try again after some time."
					),
					exc=nxenv.QueryTimeoutError,
				)
			doc = nxenv.get_doc(doctype, name)
			...
			if not for_reload:
				...
				if not force:
					...
					except nxenv.LinkExistsError as e:
			...
			if not for_reload:
				...
				nxenv.enqueue(
					"nxenv.model.delete_doc.delete_dynamic_links",
					doctype=doc.doctype,
					name=doc.name,
...
def add_to_deleted_document(doc):
	...
	if doc.doctype != "Deleted Document" and nxenv.flags.in_install != "nxenv":
		nxenv.get_doc(
...
def delete_from_table(doctype: str, name: str, ignore_doctypes: list[str], doc):
	if doctype != "DocType" and doctype == name:
		nxenv.db.delete("Singles", {"doctype": name})
	else:
		nxenv.db.delete(doctype, {"name": name})
	if doc:
		child_doctypes = [
			d.options for d in doc.meta.get_table_fields() if nxenv.get_meta(d.options).is_virtual == 0
		]
		...
		child_doctypes = nxenv.get_all(
			"DocField",
			fields="options",
			filters={"fieldtype": ["in", nxenv.model.table_fields], "parent": doctype},
			pluck="options",
		)

	child_doctypes_to_delete = set(child_doctypes) - set(ignore_doctypes)
	for child_doctype in child_doctypes_to_delete:
		nxenv.db.delete(child_doctype, {"parenttype": doctype, "parent": name})
...
def check_permission_and_not_submitted(doc):
	# permission
	if not doc.flags.ignore_permissions and nxenv.session.user != "Administrator":
		if doc.doctype == "DocType" and not doc.custom:
			nxenv.throw(_("Only the Administrator can delete a standard DocType."))
		else:
...
def check_if_doc_is_linked(doc, method="Delete"):
	...
	"""
	from nxenv.model.rename_doc import get_link_fields
	...
	if method == "Delete":
		ignored_doctypes.update(nxenv.get_hooks("ignore_links_on_delete"))
	...
	for lf in link_fields:
		...
		if issingle:
			if nxenv.db.get_single_value(link_dt, link_field) == doc.name:
		...
		for item in nxenv.db.get_values(link_dt, {link_field: doc.name}, fields, as_dict=True):
...
def check_if_doc_is_dynamically_linked(doc, method="Delete"):
	"""Raise `nxenv.LinkExistsError` if the document is dynamically linked"""
	for df in get_dynamic_link_map().get(doc.doctype, []):
		ignore_linked_doctypes = doc.get("ignore_linked_doctypes") or []

		if df.parent in nxenv.get_hooks("ignore_links_on_delete") or (
		...
		if meta.issingle:
			...
			for refdoc in nxenv.db.sql(
				...
				if (method == "Delete" and not DocStatus(refdoc.docstatus).is_cancelled()) or (
					...
					if reference_doctype in nxenv.get_hooks("ignore_links_on_delete") or (
...
def clear_timeline_references(link_doctype, link_name):
	nxenv.db.delete("Communication Link", {"link_doctype": link_doctype, "link_name": link_name})
...
def delete_controllers(doctype, module):
	...
	dir_path = os.path.join(module_path, "doctype", nxenv.scrub(doctype))
