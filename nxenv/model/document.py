...
import nxenv
from nxenv import _, is_whitelisted, msgprint
from nxenv.core.doctype.file.utils import relink_mismatched_files
from nxenv.core.doctype.server_script.server_script_utils import run_server_script_for_doc_event
from nxenv.desk.form.document_follow import follow_document
from nxenv.integrations.doctype.webhook import run_webhooks
from nxenv.model import optional_fields, table_fields
from nxenv.model.base_document import BaseDocument, get_controller
from nxenv.model.docstatus import DocStatus
from nxenv.model.naming import set_new_name, validate_name
from nxenv.model.utils import is_virtual_doctype, simple_singledispatch
from nxenv.model.workflow import set_workflow_state_on_action, validate_workflow
from nxenv.types import DF, DocRef
from nxenv.utils import compare, cstr, date_diff, file_lock, flt, get_table_name, now
from nxenv.utils.data import get_absolute_url, get_datetime, get_timedelta, getdate
from nxenv.utils.global_search import update_global_search
...
def get_doc(*args, **kwargs) -> "Document":
	"""Return a `nxenv.model.Document` object.
...
def read_only_guard(func):
	...
	def wrapper(self, *args, **kwargs):
		if getattr(nxenv.local, "read_only_depth", 0) > 0:
			...
			if getattr(nxenv.local, "read_only_context", None):
				error_msg += f" ({nxenv.local.read_only_context})"
			raise nxenv.DatabaseModificationError(error_msg)
		return func(self, *args, **kwargs)
...
def read_only_document(context=None):
	...
	if not hasattr(nxenv.local, "read_only_depth"):
		nxenv.local.read_only_depth = 0

	nxenv.local.read_only_depth += 1
	if context:
		nxenv.local.read_only_context = context

	try:
		yield
	finally:
		nxenv.local.read_only_depth -= 1
		if nxenv.local.read_only_depth == 0:
			if hasattr(nxenv.local, "read_only_context"):
				del nxenv.local.read_only_context
			del nxenv.local.read_only_depth
...
class Document(BaseDocument, DocRef):
	...
	def load_from_db(self) -> "Self":
		...
		if not getattr(self, "_metaclass", False) and self.meta.issingle:
			single_doc = nxenv.db.get_singles_dict(self.doctype, for_update=self.flags.for_update)
			if not single_doc:
				single_doc = nxenv.new_doc(self.doctype, as_dict=True)
			...
			if not d:
				nxenv.throw(
					_("{0} {1} not found").format(_(self.doctype), self.name), nxenv.DoesNotExistError
				)
	...
	def load_children_from_db(self):
		for df in self._get_table_fields():
			...
			# During nxenv is installed, the property "is_virtual" is not available in tabDocType, so
			# we need to filter those cases for the access to nxenv.db.get_value() as it would crash otherwise.
	...
	def check_permission(self, permtype="read", permlevel=None):
		"""Raise `nxenv.PermissionError` if not permitted"""
		if not self.has_permission(permtype):
			self.raise_no_permission_to(permtype)

	def has_permission(self, permtype="read", *, debug=False, user=None) -> bool:
		"""
		Call `nxenv.permissions.has_permission` if `ignore_permissions` flag isn't truthy
		...
		return nxenv.permissions.has_permission(self.doctype, permtype, self, debug=debug, user=user)

	def raise_no_permission_to(self, perm_type):
		"""Raise `nxenv.PermissionError`."""
		nxenv.flags.error_message = _(
			"You need the '{0}' permission on {1} {2} to perform this action."
		).format(
			_(perm_type),
			nxenv.bold(_(self.doctype)),
			self.name or "",
		)
		raise nxenv.PermissionError
	...
	def insert(
		...
		if not (nxenv.flags.in_migrate or nxenv.local.flags.in_install or nxenv.flags.in_setup_wizard):
			if nxenv.get_cached_value("User", nxenv.session.user, "follow_created_documents"):
				follow_document(self.doctype, self.name, nxenv.session.user)
		return self

	def check_if_locked(self):
		if self.creation and self.is_locked:
			raise nxenv.DocumentLockedError
	...
	def _save(self, ignore_permissions=None, ignore_version=None) -> "Self":
		...
		self.flags.ignore_version = nxenv.flags.in_test if ignore_version is None else ignore_version
	...
	def validate_amended_from(self):
		if nxenv.db.get_value(self.doctype, self.get("amended_from"), "docstatus") != 2:
			message = _(
				"{0} cannot be amended because it is not cancelled. Please cancel the document before creating an amendment."
			).format(nxenv.utils.get_link_to_form(self.doctype, self.get("amended_from")))
			nxenv.throw(message, title=_("Amendment Not Allowed"))

	def copy_attachments_from_amended_from(self):
		"""Copy attachments from `amended_from`"""
		from nxenv.desk.form.load import get_attachments
	...
	def set_new_name(self, force=False, set_name=None, set_child_names=True):
		"""Calls `nxenv.naming.set_new_name` for parent and child docs."""
	...
	def update_single(self, d):
		...
		nxenv.db.delete("Singles", {"doctype": self.doctype})
		...
		if self.doctype in nxenv.db.value_cache:
			del nxenv.db.value_cache[self.doctype]

	def set_user_and_timestamp(self):
		self._original_modified = self.modified
		self.modified = now()
		self.modified_by = nxenv.session.user
		...
		if self.is_new() and not (
			nxenv.flags.in_install or nxenv.flags.in_patch or nxenv.flags.in_migrate
		):
		...
		nxenv.flags.currently_saving.append((self.doctype, self.name))
	...
	def _validate_non_negative(self):
		def get_msg(df):
			if self.get("parentfield"):
				return "{} {} #{}: {} {}".format(
					...
					nxenv.bold(_(df.label, context=df.parent)),
				)
			else:
				return _("Value cannot be negative for {0}: {1}").format(
					_(df.parent), nxenv.bold(_(df.label, context=df.parent))
				)
		...
		for df in self.meta.get(
			...
			if flt(self.get(df.fieldname)) < 0:
				msg = get_msg(df)
				nxenv.throw(msg, nxenv.NonNegativeError, title=_("Negative Value"))
	...
	def validate_workflow(self):
		...
		if nxenv.flags.in_install == "nxenv":
			return
				...
				if fail:
					nxenv.throw(
						_("Value cannot be changed for {0}").format(
							nxenv.bold(self.meta.get_label(field.fieldname))
						),
						exc=nxenv.CannotChangeConstantError,
					)
	...
	def apply_fieldlevel_read_permissions(self):
		...
		for table_field in self.meta.get_table_fields():
			all_fields += nxenv.get_meta(table_field.options).fields or []
		...
		for table_field in self.meta.get_table_fields():
			for df in nxenv.get_meta(table_field.options).fields or []:
	...
	def validate_higher_perm_levels(self):
		...
		if self.flags.ignore_permissions or nxenv.flags.in_install:
			return
		...
		for df in self.meta.get_table_fields():
			high_permlevel_fields = nxenv.get_meta(df.options).get_high_permlevel_fields()
			if high_permlevel_fields:
	...
	def get_permissions(self):
		if self.meta.istable:
			...
			permissions = nxenv.get_meta(self.parenttype).permissions
		else:
	...
	def _set_defaults(self):
		...
		if self.is_new():
			new_doc = nxenv.new_doc(self.doctype, as_dict=True)
			self.update_if_missing(new_doc)

		# children
		for df in self.meta.get_table_fields():
			new_doc = nxenv.new_doc(df.options, parent_doc=self, parentfield=df.fieldname, as_dict=True)
	...
	def check_if_latest(self):
		...
		if cstr(previous.modified) != cstr(self._original_modified):
			nxenv.msgprint(
				...
				raise_exception=nxenv.TimestampMismatchError,
			)
	...
	def check_docstatus_transition(self, to_docstatus):
		...
		if to_docstatus == DocStatus.draft():
			if self.docstatus.is_draft():
				...
				raise nxenv.DocstatusTransitionError(
					_("Cannot change docstatus from 0 (Draft) to 2 (Cancelled)")
				)
			else:
				raise nxenv.ValidationError(_("Invalid docstatus"), self.docstatus)
			...
			raise nxenv.ValidationError(_("Cannot edit cancelled document"))
	...
	def _validate_mandatory(self):
		...
		if nxenv.flags.print_messages:
			print(self.as_json().encode("utf-8"))

		raise nxenv.MandatoryError(
	...
	def _validate_links(self):
		...
		if invalid_links:
			...
			nxenv.throw(_("Could not find {0}").format(msg), nxenv.LinkValidationError)

		if cancelled_links:
			msg = ", ".join(each[2] for each in cancelled_links)
			nxenv.throw(_("Cannot link cancelled document: {0}").format(msg), nxenv.CancelledLinkError)
	...
	def run_notifications(self, method):
		...
		if (
			method == "onload"
			or (nxenv.flags.in_import and nxenv.flags.mute_emails)
		...
		from nxenv.email.doctype.notification.notification import evaluate_alert
		...
		if self.flags.notifications is None:
			...
			self.flags.notifications = nxenv.cache.hget("notifications", self.doctype, _get_notifications)
	...
	def _rename(self, name: str, merge: bool = False, force: bool = False, validate_rename: bool = True):
		"""Rename the document. Triggers nxenv.rename_doc, then reloads."""
	...
	def discard(self):
		...
		if not self.docstatus == DocStatus.draft():
			raise nxenv.ValidationError(_("Only draft documents can be discarded"), self.docstatus)
	...
	def load_doc_before_save(self, *, raise_exception: bool = False):
		...
		try:
			self._doc_before_save = nxenv.get_doc(self.doctype, self.name, for_update=True)
	...
	def run_post_save_methods(self):
		...
		if (self.doctype, self.name) in nxenv.flags.currently_saving:
			nxenv.flags.currently_saving.remove((self.doctype, self.name))

	def clear_cache(self):
		nxenv.clear_document_cache(self.doctype, self.name)
	...
	def reset_seen(self):
		...
		if (
			...
			nxenv.db.set_value(
				self.doctype, self.name, "_seen", json.dumps([nxenv.session.user]), update_modified=False
			)

	def notify_update(self):
		"""Publish realtime that the current document is modified"""
		if nxenv.flags.in_patch:
			return
		...
		if not self.meta.get("read_only") and not self.meta.get("issingle") and not self.meta.get("istable"):
			data = {"doctype": self.doctype, "name": self.name, "user": nxenv.session.user}
			nxenv.publish_realtime("list_update", data, after_commit=True)
	...
	def db_set(self, fieldname, value=None, update_modified=True, notify=False, commit=False):
		...
		:param commit: default False. run nxenv.db.commit()
		"""
		...
		if update_modified and (self.doctype, self.name) not in nxenv.flags.currently_saving:
			# don't update modified timestamp if called from post save methods
			# like on_update or on_submit
			self.set("modified", now())
			self.set("modified_by", nxenv.session.user)
	...
	def db_get(self, fieldname):
		...
		return nxenv.db.get_value(self.doctype, self.name, fieldname)

	def check_no_back_links_exist(self):
		"""Check if document links to any active document before Cancel."""
		from nxenv.model.delete_doc import check_if_doc_is_dynamically_linked, check_if_doc_is_linked
	...
	def hook(f):
		...
		def compose(fn, *hooks):
			def runner(self, method, *args, **kwargs):
				...
				for f in hooks:
					try:
						nxenv.db._disable_transaction_control += 1
	...
	def validate_table_has_rows(self, parentfield, raise_exception=None):
		...
		if not (isinstance(self.get(parentfield), list) and len(self.get(parentfield)) > 0):
			...
			nxenv.throw(
				_("Table {0} cannot be empty").format(label), raise_exception or nxenv.EmptyTableError
			)
	...
	def round_floats_in(self, doc, fieldnames=None):
		...
		rounding_method = nxenv.get_system_settings("rounding_method")
		...
		return nxenv.get_doc(
			{
				...
				"comment_email": comment_email or nxenv.session.user,
	...
	def add_seen(self, user=None):
		...
		if self.meta.track_seen and not nxenv.flags.read_only and not self.meta.issingle:
			_seen = self.get("_seen") or []
			_seen = nxenv.parse_json(_seen)
	...
	def add_viewed(self, user=None, force=False, unique_views=False):
		...
		if unique_views and nxenv.db.exists(
	...
	def get_document_share_key(self, expires_on=None, no_expiry=False):
		...
		if existing_key:
			doc = nxenv.get_doc("Document Share Key", existing_key)
		else:
			doc = nxenv.new_doc("Document Share Key")
	...
	def set_onload(self, key, value):
		if not self.get("__onload"):
			self.set("__onload", nxenv._dict())
		self.get("__onload")[key] = value

	def get_onload(self, key=None):
		if not key:
			return self.get("__onload", nxenv._dict())
	...
	def queue_action(self, action, **kwargs):
		...
		from nxenv.utils.background_jobs import enqueue
		...
		return enqueue(
			"nxenv.model.document.execute_action",
	...
	def lock(self, timeout=None):
		...
		nxenv.local.locked_documents.append(self)

	def unlock(self):
		"""Delete the lock file for this document"""
		file_lock.delete_lock(self.get_signature())
		if self in nxenv.local.locked_documents:
			nxenv.local.locked_documents.remove(self)
		...
		if date_diff(to_date, from_date) < 0:
			...
			if self.meta.istable:
				table_row = _("{0} row #{1}: ").format(
					_(nxenv.unscrub(self.parentfield)),
					self.idx,
				)
			...
			nxenv.throw(
				...
				+ _("{0} must be after {1}").format(
					nxenv.bold(_(self.meta.get_label(to_date_field))),
					nxenv.bold(_(self.meta.get_label(from_date_field))),
				),
				nxenv.exceptions.InvalidDates,
			)

	def get_assigned_users(self):
		assigned_users = nxenv.get_all(
			"ToDo",
			fields=["allocated_to"],
			filters={
	...
	def add_tag(self, tag):
		...
		from nxenv.desk.doctype.tag.tag import DocTags
	...
	def deferred_insert(self) -> None:
		...
		from nxenv.deferred_insert import deferred_insert
...
def execute_action(__doctype, __name, __action, **kwargs):
	...
	doc = nxenv.get_doc(__doctype, __name)
	doc.unlock()
	try:
		...
		if nxenv.message_log:
			msg = nxenv.message_log[-1].get("message")
		else:
			msg = "<pre><code>" + nxenv.get_traceback() + "</pre></code>"
...
def bulk_insert(
	...
	doctype_meta = nxenv.get_meta(doctype)
	...
	for child_table in doctype_meta.get_table_fields():
		valid_column_map[child_table.options] = nxenv.get_meta(child_table.options).get_valid_columns()
...
def _document_values_generator(
	...
	for doc in documents:
		...
		doc.owner = doc.modified_by = nxenv.session.user
...
def unlock_document(doctype: str, name: str):
	nxenv.get_doc(doctype, name).unlock()
	nxenv.msgprint(nxenv._("Document Unlocked"), alert=True)
