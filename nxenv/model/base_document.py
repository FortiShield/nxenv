...
from nxenv.model.dynamic_links import invalidate_distinct_link_doctypes
from nxenv.model.naming import set_new_name
from nxenv.model.utils.link_count import notify_link_count
...
from nxenv.utils.defaults import get_not_null_defaults
...
def get_controller(doctype):
	...
	For `custom` type, return `nxenv.model.document.Document`.

	:param doctype: DocType name as string.
	"""

	if nxenv.local.dev_server or nxenv.flags.in_migrate:
		return import_controller(doctype)

	site_controllers = nxenv.controllers.setdefault(nxenv.local.site, {})
...
def import_controller(doctype):
	from nxenv.model.document import Document
	from nxenv.utils.nestedset import NestedSet

	module_name = "Core"
	if doctype not in DOCTYPES_FOR_DOCTYPE:
		doctype_info = nxenv.db.get_value("DocType", doctype, fieldname="*")
		if doctype_info:
	...
	module_path = None
	class_overrides = nxenv.get_hooks("override_doctype_class")
	...
	if class_overrides and class_overrides.get(doctype):
		...
		custom_module = nxenv.get_module(module_path)
		custom_class_ = getattr(custom_module, custom_classname, None)
		if not issubclass(custom_class_, class_):
			original_class_path = nxenv.bold(f"{class_.__module__}.{class_.__name__}")
			nxenv.throw(
				f"{doctype}: {nxenv.bold(import_path)} must be a subclass of {original_class_path}",
...
class BaseDocument:
	...
	@cached_property
	def meta(self):
		return nxenv.get_meta(self.doctype)
	...
	def get_db_value(self, key):
		return nxenv.db.get_value(self.doctype, self.name, key)
	...
	def _init_child(self, value, key):
		...
		if not getattr(value, "name", None):
			...
			value.__dict__["__temporary_name"] = nxenv.generate_hash(length=10)
			...
			if df:
				if is_virtual_field:
					...
					if (prop := getattr(type(self), fieldname, None)) and is_a_property(prop):
						...
						from nxenv.utils.safe_exec import get_safe_globals

						value = nxenv.safe_eval(
							code=options,
				...
				if isinstance(value, list) and df.fieldtype not in table_fields:
					nxenv.throw(_("Value for {0} cannot be a list").format(_(df.label, context=df.parent)))
	...
	def get_valid_columns(self) -> list[str]:
		if self.doctype not in nxenv.local.valid_columns:
			if self.doctype in DOCTYPES_FOR_DOCTYPE:
				from nxenv.model.meta import get_table_columns

				valid = get_table_columns(self.doctype)
			else:
				valid = self.meta.get_valid_columns()

			nxenv.local.valid_columns[self.doctype] = valid

		return nxenv.local.valid_columns[self.doctype]
	...
	def as_json(self):
		return nxenv.as_json(self.as_dict())
	...
	def db_insert(self, ignore_if_duplicate=False):
		...
		if ignore_if_duplicate and nxenv.db.db_type == "postgres":
			conflict_handler = "on conflict (name) do nothing"

		if not self.creation:
			self.creation = self.modified = now()
			self.owner = self.modified_by = nxenv.session.user
		...
		try:
			...
			if nxenv.db.is_primary_key_violation(e):
				...
				if not ignore_if_duplicate:
					nxenv.msgprint(
						_("{0} {1} already exists").format(_(self.doctype), nxenv.bold(self.name)),
						title=_("Duplicate Name"),
						indicator="red",
					)
					raise nxenv.DuplicateEntryError(self.doctype, self.name, e)

			elif nxenv.db.is_unique_key_violation(e):
				# unique constraint
	...
	def db_update(self):
		...
		try:
			...
			if nxenv.db.is_unique_key_violation(e):
				self.show_unique_validation_message(e)
			else:
				raise
	...
	def show_unique_validation_message(self, e):
		if nxenv.db.db_type != "postgres":
			...
			nxenv.msgprint(_("{0} must be unique").format(label or fieldname))

		# this is used to preserve traceback
		raise nxenv.UniqueValidationError(self.doctype, self.name, e)
	...
	def update_modified(self):
		...
		if getattr(self.meta, "issingle", False):
			nxenv.db.set_single_value(self.doctype, "modified", self.modified, update_modified=False)
		else:
			nxenv.db.set_value(self.doctype, self.name, "modified", self.modified, update_modified=False)
		...
		for df in self.meta.get_link_fields() + self.meta.get("fields", {"fieldtype": ("=", "Dynamic Link")}):
			...
			if docname:
				if df.fieldtype == "Link":
					doctype = df.options
					if not doctype:
						nxenv.throw(_("Options not set for link field {0}").format(df.fieldname))
				else:
					doctype = self.get(df.options)
					if not doctype:
						nxenv.throw(_("{0} must be set first").format(self.meta.get_label(df.options)))
				...
				if not meta.get("is_virtual"):
					if not fields_to_fetch:
						...
						values = _dict(name=nxenv.db.get_value(doctype, docname, "name", cache=True))
					else:
						...
						values = (
							nxenv.db.get_value(doctype, docname, values_to_fetch, as_dict=True)
							or empty_values
						)
				...
				if meta.get("is_virtual"):
					values = nxenv.get_doc(doctype, docname).as_dict()
				...
				if values:
					...
					if not values.name:
						...
						and nxenv.get_meta(doctype).is_submittable
						and cint(nxenv.db.get_value(doctype, docname, "docstatus")) == DocStatus.cancelled()
					):
	...
	def set_fetch_from_value(self, doctype, df, values):
		...
		if df.fieldtype in ["Small Text", "Text", "Data"]:
			from nxenv.model.meta import get_default_df

			fetch_from_df = get_default_df(fetch_from_fieldname) or nxenv.get_meta(doctype).get_field(
	...
	def _validate_selects(self):
		...
		for df in self.meta.get_select_fields():
			...
			if value not in options and not (nxenv.flags.in_test and value.startswith("_T-")):
	...
	def _validate_data_fields(self):
		...
		# data_field options defined in nxenv.model.data_field_options
		for data_field in self.meta.get_data_fields():
			...
			if data_field_options == "Email":
				if (self.owner in nxenv.STANDARD_USERS) and (data in nxenv.STANDARD_USERS):
					continue
	...
	def _validate_constants(self):
		if nxenv.flags.in_import or self.is_new() or self.flags.ignore_validate_constants:
			return

		constants = [d.fieldname for d in self.meta.get("fields", {"set_only_once": ("=", 1)})]
		if constants:
			values = nxenv.db.get_value(self.doctype, self.name, constants, as_dict=True)
		...
		for fieldname in constants:
			...
			if self.get(fieldname) != value:
				nxenv.throw(
					...
					nxenv.CannotChangeConstantError,
				)
	...
	def _validate_code_fields(self):
		for field in self.meta.get_code_fields():
			...
			if language == "Python":
				nxenv.utils.validate_python_code(code_string, fieldname=field.label, is_expression=False)

			elif language == "PythonExpression":
				nxenv.utils.validate_python_code(code_string, fieldname=field.label)
	...
	def throw_length_exceeded_error(self, df, max_length, value):
		...
		nxenv.throw(
			_("{0}: '{1}' ({3}) will get truncated, as max characters allowed is {2}").format(
				reference, nxenv.bold(_(df.label, context=df.parent)), max_length, value
			),
			nxenv.CharacterLengthExceededError,
			title=_("Value too big"),
		)

	def _validate_update_after_submit(self):
		# get the full doc with children
		db_values = nxenv.get_doc(self.doctype, self.name).as_dict()
				...
				if self_value != db_value:
					nxenv.throw(
						_("{0} Not allowed to change {1} after submission from {2} to {3}").format(
							...
							nxenv.bold(db_value),
							nxenv.bold(self_value),
						),
						nxenv.UpdateAfterSubmitError,
	...
	def _save_passwords(self):
		...
		from nxenv.utils.password import remove_encrypted_password, set_encrypted_password
	...
	def get_formatted(
		...
		from nxenv.utils.formatters import format_value
		...
		if (
			...
			currency = nxenv.db.get_value("Currency", currency_value, cache=True)
	...
	def reset_values_if_no_permlevel_access(self, has_access_to, high_permlevel_fields):
		...
		if to_reset:
			if self.is_new():
				...
				ref_doc = nxenv.new_doc(self.doctype)
			else:
	...
	def _extract_images_from_editor(self):
		from nxenv.core.doctype.file.utils import extract_images_from_doc
