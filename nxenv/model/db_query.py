...
from nxenv.database.utils import DefaultOrderBy, FallBackDateTimeStr, NestedSetHierarchy
from nxenv.model import get_permitted_fields, optional_fields
from nxenv.model.meta import get_table_columns
from nxenv.model.utils import is_virtual_doctype
from nxenv.model.utils.user_settings import get_user_settings, update_user_settings
from nxenv.query_builder.utils import Column
from nxenv.types import Filters, FilterSignature, FilterTuple
...
from nxenv.utils.data import DateTimeLikeObject, get_datetime, getdate, sbool
...
class DatabaseQuery:
	def __init__(self, doctype, user=None):
		...
		self.or_conditions = []
		self.fields = None
		self.user = user or nxenv.session.user
	...
	def get_meta(self, doctype: str):
		if doctype not in self._metas:
			self._metas[doctype] = nxenv.get_meta(doctype)
	...
	def execute(
		...
		*,
		...
		if as_list and not isinstance(self.fields, (Sequence | str)) and len(self.fields) > 1:
			nxenv.throw(_("Fields must be a list or tuple when as_list is enabled"))
		...
		if is_virtual_doctype(self.doctype):
			...
			return nxenv.call(controller.get_list, args=kwargs, **kwargs)
	...
	def build_and_run(self):
		...
		if nxenv.db.db_type == "postgres" and args.order_by and args.group_by:
	...
	def prepare_args(self):
		...
		for child in self.tables[1:]:
			...
			args.tables += f" {self.join} {child} on ({child}.parenttype = {nxenv.db.escape(self.doctype)} and {child}.parent = {parent_name})"
	...
	def sanitize_fields(self):
		...
		def _raise_exception():
			nxenv.throw(_("Use of sub-query or function is restricted"), nxenv.DataError)
		...
		for field in self.fields:
			...
			if SUB_QUERY_PATTERN.match(field):
				...
				if function in blacklisted_functions:
					nxenv.throw(
						_("Use of function {0} in field is restricted").format(function), exc=nxenv.DataError
					)
			...
			if self.strict:
				if STRICT_FIELD_PATTERN.match(field):
					nxenv.throw(_("Illegal SQL Query"))
	...
	def _set_permission_map(self, doctype: str, parent_doctype: str | None = None):
		ptype = "select" if nxenv.only_has_select_perm(doctype) else "read"
		nxenv.has_permission(
			doctype,
			ptype=ptype,
	...
	def get_table_columns(self):
		...
		except nxenv.db.TableMissingError:
			if self.ignore_ddl:
				return None
			else:
				raise
	...
	def apply_fieldlevel_read_permissions(self):
		...
		Note: Does not apply to `nxenv.model.core_doctype_list`
		...
		Example:
		        ...
		        - Query: fields=["*"]
		        - Result: fields=["title", ...] // will also include Nxenv's meta field like `name`, `owner`, etc.
		"""
		from nxenv.desk.reportview import extract_fieldnames
		...
		for i, field in enumerate(self.fields):
			...
			# labels / pseudo columns or nxenv internals
			...
			# handle child / joined table fields
				...
				if wrap_grave_quotes(table) in self.query_tables:
					...
					raise nxenv.PermissionError(ch_doctype)
	...
	def prepare_filter_condition(self, ft: FilterTuple) -> str:
		...
		from nxenv.boot import get_additional_filters_from_hooks
		...
		if f.operator.lower() in NestedSetHierarchy:
			...
			lft, rgt = "", ""
			if f.value:
				lft, rgt = nxenv.db.get_value(ref_doctype, f.value, ["lft", "rgt"]) or (0, 0)
			...
			fallback = "''"
			value = [nxenv.db.escape((cstr(v)).strip(), percent=False) for v in nodes]
			if len(value):
				value = f"({', '.join(value)})"
			else:
				value = "('')"
		...
		if f.operator.lower() in ("in", "not in"):
			...
			if value is None:
				...
				fallback = "''"
				value = [nxenv.db.escape((cstr(v) or "").strip(), percent=False) for v in values]
				if len(value):
					value = f"({', '.join(value)})"
				else:
					value = "('')"
			...
			if isinstance(f.value, Column):
				...
				quote = '"' if nxenv.conf.db_type == "postgres" else "`"
				value = f"{tname}.{quote}{f.value.name}{quote}"

			# escape value
			elif escape and isinstance(value, str):
				value = f"{nxenv.db.escape(value, percent=False)}"
		...
		if (
			...
			if f.operator.lower() == "like" and nxenv.conf.get("db_type") == "postgres":
				f.operator = "ilike"
	...
	def build_match_conditions(self, as_condition=True) -> str | list:
		...
		if not self.user:
			self.user = nxenv.session.user

		if not self.tables:
			self.extract_tables()

		role_permissions = nxenv.permissions.get_role_permissions(self.doctype_meta, user=self.user)
		if (
			...
			only_if_shared = True
			self.shared = nxenv.share.get_shared(self.doctype, self.user)
			if not self.shared:
				nxenv.throw(_("No permission to read {0}").format(_(self.doctype)), nxenv.PermissionError)
			else:
			...
			if requires_owner_constraint(role_permissions):
				...
				self.match_conditions.append(
					f"`tab{self.doctype}`.`owner` = {nxenv.db.escape(self.user, percent=False)}"
				)

			# add user permission only if role has read perm
			elif role_permissions.get("read") or role_permissions.get("select"):
				# get user permissions
				user_permissions = nxenv.permissions.get_user_permissions(self.user)
	...
	def get_share_condition(self):
		return (
			...
			+ f" in ({', '.join(nxenv.db.escape(s, percent=False) for s in self.shared)})"
		)
	...
	def add_user_permissions(self, user_permissions):
		...
		for df in doctype_link_fields:
			...
			if user_permission_values:
				docs = []
				if nxenv.get_system_settings("apply_strict_user_permissions"):
					condition = ""
				else:
				...
				if docs:
					values = ", ".join(nxenv.db.escape(doc, percent=False) for doc in docs)
	...
	def get_permission_query_conditions(self) -> str:
		conditions = []
		hooks = nxenv.get_hooks("permission_query_conditions", {})
		condition_methods = hooks.get(self.doctype, []) + hooks.get("*", [])
		for method in condition_methods:
			if c := nxenv.call(nxenv.get_attr(method), self.user, doctype=self.doctype):
				conditions.append(c)

		if permission_script_name := get_server_script_map().get("permission_query", {}).get(self.doctype):
			script = nxenv.get_doc("Server Script", permission_script_name)
	...
	def validate_order_by_and_group_by(self, parameters: str):
		...
		if "select" in _lower and "from" in _lower:
			nxenv.throw(_("Cannot use sub-query in order by"))
		...
		for field in parameters.split(","):
			...
			if full_field_name:
				...
				if tbl not in self.tables:
					if tbl.startswith("`"):
						tbl = tbl[4:-1]
					nxenv.throw(_("Please select atleast 1 column from {0} to sort/group").format(tbl))

			if function in blacklisted_sql_functions:
				nxenv.throw(_("Cannot use {0} in order/group by").format(field))
...
def check_parent_permission(parent, child_doctype):
	if parent:
		...
		if child_doctype and not (
			nxenv.db.exists("DocField", {"parent": parent, "options": child_doctype})
			or nxenv.db.exists("Custom Field", {"dt": parent, "options": child_doctype})
		):
			raise nxenv.PermissionError

		if nxenv.permissions.has_permission(parent):
			return
...
def has_any_user_permission_for_doctype(doctype, user, applicable_for):
	user_permissions = nxenv.permissions.get_user_permissions(user=user)
...
def get_between_date_filter(value, df=None):
	...
	from_date = nxenv.utils.nowdate()
	to_date = nxenv.utils.nowdate()
	...
	if fieldtype == "Datetime":
		cond = f"'{nxenv.db.format_datetime(from_date)}' AND '{nxenv.db.format_datetime(to_date)}'"
	else:
		cond = f"'{nxenv.db.format_date(from_date)}' AND '{nxenv.db.format_date(to_date)}'"
...
def get_additional_filter_field(additional_filters_config, f, value):
	...
	f = nxenv._dict(nxenv.get_attr(additional_filter["get_field"])())
	if f.query_value:
		for option in f.options:
			option = nxenv._dict(option)
