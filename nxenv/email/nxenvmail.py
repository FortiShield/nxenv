...
import nxenv
from nxenv import _
from nxenv.nxenvclient import NxenvClient, NxenvOAuth2Client
from nxenv.utils import convert_utc_to_system_timezone, get_datetime, get_system_timezone


class NxenvMail:
	"""Class to interact with the Nxenv Mail API."""
	...
	def get_client(
		...
		api_secret: str | None = None,
		access_token: str | None = None,
	) -> NxenvClient | NxenvOAuth2Client:
		"""Returns a NxenvClient or NxenvOAuth2Client instance."""

		if hasattr(nxenv.local, "nxenv_mail_clients"):
			if client := nxenv.local.nxenv_mail_clients.get(mailbox):
				return client
		else:
			nxenv.local.nxenv_mail_clients = {}

		client = (
			NxenvOAuth2Client(url=site, access_token=access_token)
			if access_token
			else NxenvClient(url=site, api_key=api_key, api_secret=api_secret)
		)
		nxenv.local.nxenv_mail_clients[mailbox] = client
	...
	def request(
		...
		"""Makes a request to the Nxenv Mail API."""
	...
	def send_raw(
		...
		"""Sends an email using the Nxenv Mail API."""
	...
	def pull_raw(self, limit: int = 50, last_synced_at: str | None = None) -> dict[str, str | list[str]]:
		"""Pulls emails from the mailbox using the Nxenv Mail API."""
		...
		headers = {"X-Site": nxenv.utils.get_url()}
