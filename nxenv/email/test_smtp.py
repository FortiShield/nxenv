...
class TestSMTP(IntegrationTestCase):
	...
	def test_get_email_account(self):
		existing_email_accounts = nxenv.get_all(
		...
		for email_account in existing_email_accounts:
			nxenv.db.set_value("Email Account", email_account["name"], unset_details)

		# remove mail_server config so that test@example.com is not created
		mail_server = nxenv.conf.get("mail_server")
		del nxenv.conf["mail_server"]

		nxenv.local.outgoing_email_account = {}
		...
		nxenv.conf["mail_server"] = mail_server
		for email_account in existing_email_accounts:
			set_details = {
			...
			nxenv.db.set_value("Email Account", email_account["name"], set_details)
...
def create_email_account(email_id, password, enable_outgoing, default_outgoing=0, append_to=None):
	...
	email_account = nxenv.new_doc("Email Account")
