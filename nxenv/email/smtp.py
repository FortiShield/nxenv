...
from nxenv.utils import cint, cstr, get_traceback


class InvalidEmailCredentials(nxenv.ValidationError):
	pass
...
class SMTPServer:
	def __init__(
		...
		if not self.server:
			nxenv.msgprint(
				...
				raise_exception=nxenv.OutgoingEmailError,
			)
	...
	def session(self):
		...
		try:
			...
			if not _session:
				nxenv.msgprint(
					_("Could not connect to outgoing email server"), raise_exception=nxenv.OutgoingEmailError
				)
			...
			if self.use_oauth:
				...
				if res[0] != 235:
					nxenv.msgprint(res[1], raise_exception=nxenv.OutgoingEmailError)
	...
	def _enqueue_connection_closure(self):
		if nxenv.request and hasattr(nxenv.request, "after_response"):
			nxenv.request.after_response.add(self.quit)
		elif nxenv.job:
			nxenv.job.after_job.add(self.quit)
		elif not nxenv.flags.in_test:
			# Console?
			import atexit
