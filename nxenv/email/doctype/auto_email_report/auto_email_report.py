...
import nxenv
from nxenv import _
from nxenv.desk.query_report import build_xlsx_data
...
from nxenv.utils.csvutils import to_csv
...
class AutoEmailReport(Document):
	...
	def autoname(self):
		...
		if nxenv.db.exists("Auto Email Report", self.name):
	...
	def sender_email(self):
		return nxenv.db.get_value("Email Account", self.sender, "email_id")
	...
	def validate_report_count(self):
		count = nxenv.db.count("Auto Email Report", {"user": self.user, "enabled": 1})

		max_reports_per_user = (
			cint(nxenv.local.conf.max_reports_per_user)  # kept for backward compatibilty
			or cint(nxenv.get_system_settings("max_auto_email_report_per_user"))
			or 20
		)
		...
		if count > max_reports_per_user + (-1 if self.flags.in_insert else 0):
			...
			nxenv.throw(msg, title=_("Report limit reached"))
		...
		if self.format not in valid_report_formats:
			nxenv.throw(
				_("{0} is not a valid report format. Report format should one of the following {1}").format(
					nxenv.bold(self.format), nxenv.bold(", ".join(valid_report_formats))
				)
			)

	def validate_mandatory_fields(self):
		# Check if all Mandatory Report Filters are filled by the User
		filters = nxenv.parse_json(self.filters) if self.filters else {}
		filter_meta = nxenv.parse_json(self.filter_meta) if self.filter_meta else {}
		throw_list = [
	...
	def get_report_content(self):
		...
		report = nxenv.get_doc("Report", self.report)

		self.filters = nxenv.parse_json(self.filters) if self.filters else {}
		...
		# add serial numbers
		columns.insert(0, nxenv._dict(fieldname="idx", label="", width="30px"))
		...
		if self.format == "HTML":
			...
			nxenv.throw(_("Invalid Output Format"))

	def get_html_table(self, columns=None, data=None):
		date_time = global_date_format(now()) + " " + format_time(now())
		report_doctype = nxenv.db.get_value("Report", self.report, "ref_doctype")

		return nxenv.render_template(
			"nxenv/templates/emails/auto_email_report.html",
			{
				"title": self.name,
	...
	def prepare_dynamic_filters(self):
		self.filters = nxenv.parse_json(self.filters)
	...
	def send(self):
		if self.filter_meta and not self.filters:
			nxenv.throw(_("Please set filters value in Report Filter table."))
...
def download(name):
	...
	auto_email_report = nxenv.get_doc("Auto Email Report", name)
	auto_email_report.check_permission()
	data = auto_email_report.get_report_content()

	if not data:
		nxenv.msgprint(_("No Data"))
		return

	nxenv.local.response.filecontent = data
	nxenv.local.response.type = "download"
	nxenv.local.response.filename = auto_email_report.get_file_name()
...
def send_daily():
	...
	enabled_reports = nxenv.get_all(
		"Auto Email Report", filters={"enabled": 1, "frequency": ("in", ("Daily", "Weekdays", "Weekly"))}
	)

	for report in enabled_reports:
		auto_email_report = nxenv.get_doc("Auto Email Report", report.name)
...
def send_monthly():
	...
	for report in nxenv.get_all("Auto Email Report", {"enabled": 1, "frequency": "Monthly"}):
		nxenv.get_doc("Auto Email Report", report.name).send()
	...
	for row in data:
		...
		for col in columns:
			...
			if col.fieldtype == "Link":
				...
				doc = None
				if doc_name and col.get("parent") and not nxenv.get_meta(col.parent).istable:
					if nxenv.db.exists(col.parent, doc_name):
						doc = nxenv.get_doc(col.parent, doc_name)

				# Pass the Document to get the currency based on docfield option
				row[col.fieldname] = nxenv.format_value(row[col.fieldname], col, doc=doc)
	return columns, data
