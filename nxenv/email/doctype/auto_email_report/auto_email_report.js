...
nxenv.ui.form.on("Auto Email Report", {
	refresh: function (frm) {
		...
		if (!frm.is_new()) {
			frm.add_custom_button(__("Download"), function () {
				var w = window.open(
					nxenv.urllib.get_full_url(
						"/api/method/nxenv.email.doctype.auto_email_report.auto_email_report.download?" +
							"name=" +
			...
			frm.add_custom_button(__("Send Now"), function () {
				nxenv.call({
					method: "nxenv.email.doctype.auto_email_report.auto_email_report.send_now",
					args: { name: frm.doc.name },
					callback: function () {
						nxenv.msgprint(__("Scheduled to send"));
					},
				});
			});
		} else {
			if (!frm.doc.user) {
				frm.set_value("user", nxenv.session.user);
			}
			if (!frm.doc.email_to) {
				frm.set_value("email_to", nxenv.session.user);
			}
		}
	...
	fetch_report_filters(frm) {
		if (
			...
			nxenv.call({
				method: "nxenv.desk.query_report.get_script",
				args: {
					report_name: frm.doc.report,
				},
				callback: function (r) {
					nxenv.dom.eval(r.message.script || "");
					frm.script_setup_for = frm.doc.report;
	...
	show_filters: async function (frm) {
		...
		wrapper.empty();
		let reference_report = nxenv.query_reports[frm.doc.report];
		if (!reference_report || !reference_report.filters) {
			reference_report = await nxenv.model.with_doc("Report", frm.doc.report);
		}
		if (
			...
			if (
				...
				if (frm.doc.filters) {
					...
					nxenv.db.get_value("Report", frm.doc.report, "json", (r) => {
						if (r && r.json) {
				...
				report_filters = nxenv.query_reports[frm.doc.reference_report].filters;
			} else {
				filters = JSON.parse(frm.doc.filters || "{}");
			...
			report_filters.forEach((f) => {
				...
				$("<td>" + nxenv.format(filters[f.fieldname], f) + "</td>")
					.css(css)
					.appendTo(row);
			});

			table.on("click", function () {
				var dialog = new nxenv.ui.Dialog({
