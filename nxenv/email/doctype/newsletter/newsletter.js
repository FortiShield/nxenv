...
nxenv.ui.form.on("Newsletter", {
	refresh(frm) {
		let doc = frm.doc;
		let can_write = nxenv.boot.user.can_write.includes(doc.doctype);
		if (!frm.is_new() && !frm.is_dirty() && !doc.email_sent && can_write) {
			...
			frm.add_custom_button(
				...
				() => {
					...
					nxenv.confirm(__("Are you sure you want to send this newsletter now?"), () => {
						frm.events.send_emails(frm);
					});
				},
				__("Send")
		...
		if (frm.is_new() && !doc.sender_email) {
			let { fullname, email } = nxenv.user_info(doc.owner);
	...
	send_emails(frm) {
		nxenv.dom.freeze(__("Queuing emails..."));
		frm.call("send_emails").then(() => {
			frm.refresh();
			nxenv.dom.unfreeze();
			nxenv.show_alert(
				__("Queued {0} emails", [nxenv.utils.shorten_number(frm.doc.total_recipients)])
			);
		});
	},

	schedule_send_dialog(frm) {
		let hours = nxenv.utils.range(24);
	...
	update_schedule_message(frm) {
		if (!frm.doc.email_sent && frm.doc.schedule_send) {
			let datetime = nxenv.datetime.global_date_format(frm.doc.schedule_send);
