...
import nxenv
from nxenv.email.doctype.newsletter.exceptions import (
	NewsletterAlreadySentError,
	NoRecipientFoundError,
)
from nxenv.email.doctype.newsletter.newsletter import (
	Newsletter,
...
from nxenv.utils import add_days, getdate
...
class TestNewsletterMixin:
	...
	def tearDown(self):
		...
		for newsletter in newsletters:
			...
			nxenv.delete_doc("Newsletter", newsletter)
			nxenv.db.delete("Newsletter Email Group", {"parent": newsletter})
			newsletters.remove(newsletter)

	def setup_email_group(self):
		if not nxenv.db.exists("Email Group", "_Test Email Group"):
			nxenv.get_doc({"doctype": "Email Group", "title": "_Test Email Group"}).insert()
		...
		for email in emails:
			...
			try:
				...
				nxenv.db.set_value(doctype, email_filters, "unsubscribed", 0)
	...
	def get_newsletter(**kwargs) -> "Newsletter":
		...
		similar_newsletters = nxenv.get_all(doctype, newsletter_content, pluck="name")

		for similar_newsletter in similar_newsletters:
			nxenv.delete_doc(doctype, similar_newsletter)

		newsletter = nxenv.get_doc({"doctype": doctype, **newsletter_content, **kwargs})
...
class TestNewsletter(TestNewsletterMixin, IntegrationTestCase):
	def test_send(self):
		...
		email_queue_list = [nxenv.get_doc("Email Queue", e.name) for e in nxenv.get_all("Email Queue")]
	...
	def test_unsubscribe(self):
		...
		group = nxenv.get_all("Newsletter Email Group", filters={"parent": name}, fields=["email_group"])
	...
	def test_send_scheduled_email_error_handling(self):
		...
		job_path = "nxenv.email.doctype.newsletter.newsletter.Newsletter.queue_all"
		m = MagicMock(side_effect=nxenv.OutgoingEmailError)

		with self.assertRaises(nxenv.OutgoingEmailError):
	...
	def test_retry_partially_sent_newsletter(self):
		...
		# retry
		newsletter.send_emails()
		self.assertEqual(nxenv.db.count("Email Queue"), 4)
