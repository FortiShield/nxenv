...
import nxenv.utils
from nxenv import _
from nxenv.email.doctype.email_group.email_group import add_subscribers
from nxenv.rate_limiter import rate_limit
from nxenv.utils.safe_exec import is_job_queued
from nxenv.utils.verified_command import get_signed_params, verify_request
...
class Newsletter(WebsiteGenerator):
	...
	if TYPE_CHECKING:
		from nxenv.email.doctype.newsletter_attachment.newsletter_attachment import NewsletterAttachment
		from nxenv.email.doctype.newsletter_email_group.newsletter_email_group import NewsletterEmailGroup
	...
	def get_sending_status(self):
		...
		emails_queued = is_job_queued(
			job_name=nxenv.utils.get_job_name("send_bulk_emails_for", self.doctype, self.name),
			queue="long",
		)
	...
	def send_test_email(self, email):
		test_emails = nxenv.utils.validate_email_address(email, throw=True)
		self.send_newsletter(emails=test_emails, test_email=True)
		nxenv.msgprint(_("Test email sent to {0}").format(email), alert=True)
	...
	def validate_newsletter_status(self):
		if self.email_sent:
			nxenv.throw(_("Newsletter has already been sent"), exc=NewsletterAlreadySentError)

		if self.get("__islocal"):
			nxenv.throw(_("Please save the Newsletter before sending"), exc=NewsletterNotSavedError)

	def validate_newsletter_recipients(self):
		if not self.newsletter_recipients:
			nxenv.throw(_("Newsletter should have atleast one recipient"), exc=NoRecipientFoundError)

	def validate_sender_address(self):
		"""Validate self.send_from is a valid email address or not."""
		if self.sender_email:
			nxenv.utils.validate_email_address(self.sender_email, throw=True)
			self.send_from = (
	...
	def validate_publishing(self):
		if self.send_webview_link and not self.published:
			nxenv.throw(_("Newsletter must be published to send webview link in email"))

	def validate_scheduling_date(self):
		if getattr(nxenv.flags, "is_scheduler_running", False):
			return

		if (
			self.schedule_sending
			and nxenv.utils.get_datetime(self.schedule_send) < nxenv.utils.now_datetime()
		):
			nxenv.throw(_("Past dates are not allowed for Scheduling."))
	...
	def queue_all(self):
		...
		self.email_sent_at = nxenv.utils.now()
	...
	def send_newsletter(self, emails: list[str], test_email: bool = False):
		...
		sender = self.send_from or nxenv.utils.get_formatted_email(self.owner)
		args = self.as_dict()
		args["message"] = self.get_message(medium="email")

		is_auto_commit_set = bool(nxenv.db.auto_commit_on_many_writes)
		nxenv.db.auto_commit_on_many_writes = not nxenv.flags.in_test
		...
		nxenv.sendmail(
			...
			if test_email
			else "/api/method/nxenv.email.doctype.newsletter.newsletter.newsletter_email_read",
		)

		nxenv.db.auto_commit_on_many_writes = is_auto_commit_set

	def get_message(self, medium=None) -> str:
		message = self.message
		if self.content_type == "Markdown":
			message = nxenv.utils.md_to_html(self.message_md)
		if self.content_type == "HTML":
			message = self.message_html

		html = nxenv.render_template(message, {"doc": self.as_dict()})
	...
	def add_source(self, html: str, medium="None") -> str:
		...
		for link in links:
			...
			if href and not href.startswith("#"):
				if not nxenv.utils.is_site_link(href):
					continue
				new_href = nxenv.utils.add_trackers_to_url(
	...
	def get_email_groups(self) -> list[str]:
		...
		return [x.email_group for x in self.email_group] or nxenv.get_all(
...
def confirmed_unsubscribe(email, group):
	...
	doc = nxenv.get_doc("Email Group Member", {"email": email, "email_group": group})
...
def subscribe(email, email_group=None):
	...
	api_endpoint = nxenv.utils.get_url(
		"/api/method/nxenv.email.doctype.newsletter.newsletter.confirm_subscription"
	)
	...
	email_confirmation_template = nxenv.db.get_value(
	...
	if email_confirmation_template:
		...
		email_template = nxenv.get_doc("Email Template", email_confirmation_template)
		email_subject = email_template.subject
		content = nxenv.render_template(email_template.response, args)
	else:
...
def confirm_subscription(email, email_group=None):
	...
	try:
		group = nxenv.get_doc("Email Group", email_group)
	except nxenv.DoesNotExistError:
		group = nxenv.get_doc({"doctype": "Email Group", "title": email_group}).insert(
	...
	if welcome_url:
		nxenv.local.response["type"] = "redirect"
		nxenv.local.response["location"] = welcome_url
	else:
...
def send_scheduled_email():
	...
	nxenv.flags.is_scheduler_running = True

	scheduled_newsletter = nxenv.get_all(
		"Newsletter",
		filters={
			"schedule_send": ("<=", nxenv.utils.now_datetime()),
			"email_sent": False,
	...
	for newsletter_name in scheduled_newsletter:
		try:
			newsletter = nxenv.get_doc("Newsletter", newsletter_name)
			...
			nxenv.db.set_value("Newsletter", newsletter_name, "email_sent", 0)
			newsletter.log_error("Failed to send newsletter")

		if not nxenv.flags.in_test:
			nxenv.db.commit()

	nxenv.flags.is_scheduler_running = False
...
def newsletter_email_read(recipient_email=None, reference_doctype=None, reference_name=None):
	...
	verify_request()
	try:
		doc = nxenv.get_cached_doc("Newsletter", reference_name)
		if doc.add_viewed(recipient_email, force=True, unique_views=True):
			newsletter = nxenv.qb.DocType("Newsletter")
			(
				nxenv.qb.update(newsletter)
		...
		nxenv.response.update(nxenv.utils.get_imaginary_pixel_response())


def get_default_email_group():
	return _("Website", lang=nxenv.db.get_default("language"))
