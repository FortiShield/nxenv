...
def get_test_notification(config):
	try:
		notification = nxenv.get_doc(doctype="Notification", **config).insert()
		yield notification
	finally:
		notification.delete()
		nxenv.db.commit()
...
class TestNotification(IntegrationTestCase):
	def setUp(self):
		...
		if not nxenv.db.exists("Notification", {"name": "ToDo Status Update"}, "name"):
			notification = nxenv.new_doc("Notification")
		...
		if not nxenv.db.exists("Notification", {"name": "Contact Status Update"}, "name"):
			notification = nxenv.new_doc("Notification")
	...
	def test_new_and_save(self):
		...
		communication = nxenv.new_doc("Communication")
		...
		self.assertEqual(nxenv.db.get_value("Communication", communication.name, "subject"), "__testing__")

	def test_condition(self):
		"""Check notification is triggered based on a condition."""
		event = nxenv.new_doc("Event")
		event.subject = "test"
	...
	def test_invalid_condition(self):
		...
		recipent = nxenv.new_doc("Notification Recipient")
		recipent.receiver_by_document_field = "owner"

		notification.recipents = recipent
		notification.condition = "test"

		self.assertRaises(nxenv.ValidationError, notification.save)
		notification.delete()
	...
	def test_minutes_positive_offset(self):
		...
		with get_test_notification(notification) as n:
			nxenv.db.delete("Notification Log", {"subject": n.subject})
			# Run the offset alerts
			trigger_notifications(None, "offset")

			# Check if the notification was triggered
			self.assertEqual(1, nxenv.db.count("Notification Log", {"subject": n.subject}))
	...
	def test_minutes_offset_validation(self):
		...
		self.assertRaises(nxenv.ValidationError, notification.insert)
	...
	def test_date_changed(self):
		...
		nxenv.get_doc(
			"Scheduled Job Type",
			dict(method="nxenv.email.doctype.notification.notification.trigger_daily_alerts"),
		).execute()
		...
		event.starts_on = nxenv.utils.add_days(nxenv.utils.nowdate(), 2) + " 12:00:00"
		event.save()
	...
	def test_cc_jinja(self):
		nxenv.db.delete("User", {"email": "test_jinja@example.com"})
		nxenv.db.delete("Email Queue")
		nxenv.db.delete("Email Queue Recipient")

		test_user = nxenv.new_doc("User")
		...
		self.assertTrue(nxenv.db.get_value("Email Queue Recipient", {"recipient": "test_jinja@example.com"}))
	...
	def test_notification_value_change_casted_types(self):
		...
		with get_test_notification(notification) as n:
			...
			user.birth_date = nxenv.utils.add_days(user.birth_date, 1)
			user.save()

			user.reload()
			user.birth_date = nxenv.utils.getdate(user.birth_date)
			user.save()
			self.assertEqual(1, nxenv.db.count("Notification Log", {"subject": n.subject}))

	@classmethod
	def tearDownClass(cls):
		nxenv.delete_doc_if_exists("Notification", "ToDo Status Update")
		nxenv.delete_doc_if_exists("Notification", "Contact Status Update")
		...
		notification = nxenv.get_doc(
			{
				...
				"message": "{% set val = nxenv.get_doc('ToDo', doc.name) %} ToDo allocated to {{ doc.allocated_to }}",
				"channel": "Email",
...
❯ nxcli run-tests --module nxenv.email.doctype.notification.test_notification --case TestNotificationOffsetRange
...
# 		self.event = nxenv.new_doc("Event")
...
# 		nxenv.db.delete("Event", {"name": self.event.name})
# 		nxenv.db.delete("Notification", {"name": self.notification["name"]})
# 		nxenv.db.delete("Notification Log", {"subject": self.notification["subject"]})
...
# 						self.assertEqual(
# 							1,
# 							nxenv.db.count("Notification Log", {"subject": n.subject}),
