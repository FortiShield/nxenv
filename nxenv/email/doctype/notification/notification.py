...
import nxenv
from nxenv import _
from nxenv.core.doctype.role.role import get_info_based_on_role, get_user_info
from nxenv.core.doctype.sms_settings.sms_settings import send_sms
from nxenv.desk.doctype.notification_log.notification_log import enqueue_create_notification
from nxenv.integrations.doctype.slack_webhook_url.slack_webhook_url import send_slack_message
from nxenv.model.document import Document
from nxenv.modules.utils import export_module_json, get_doc_module
from nxenv.utils import add_to_date, cast, now_datetime, nowdate, validate_email_address
...
class Notification(Document):
	...
	if TYPE_CHECKING:
		from nxenv.email.doctype.notification_recipient.notification_recipient import NotificationRecipient
	...
	def preview_meets_condition(self, preview_document):
		if not self.condition:
			return _("Yes")
		try:
			doc = nxenv.get_cached_doc(self.document_type, preview_document)
			context = get_context(doc)
			return _("Yes") if nxenv.safe_eval(self.condition, eval_locals=context) else _("No")
	...
	def preview_message(self, preview_document):
		try:
			...
			msg = nxenv.render_template(self.message, context)
			if self.channel == "SMS":
				return nxenv.utils.strip_html_tags(msg)
			return msg
		except Exception as e:
	...
	def preview_subject(self, preview_document):
		try:
			...
			if not self.subject:
				return _("No subject")
			if "{" in self.subject:
				return nxenv.render_template(self.subject, context)
			return self.subject
		except Exception as e:
	...
	def validate(self):
		...
		if self.event in ("Days Before", "Days After") and not self.date_changed:
			nxenv.throw(_("Please specify which date field must be checked"))

		if self.event in ("Minutes Before", "Minutes After"):
			if not self.datetime_changed:
				nxenv.throw(_("Please specify which datetime field must be checked"))
			if not self.minutes_offset:
				nxenv.throw(_("Please specify the minutes offset"))
		...
		if self.event == "Value Change" and not self.value_changed:
			nxenv.throw(_("Please specify which value field must be checked"))

		self.validate_forbidden_document_types()
		self.validate_condition()
		self.validate_standard()
		nxenv.cache.hdel("notifications", self.document_type)
...
"""
	...
	def validate_standard(self):
		if self.is_standard and self.enabled and not nxenv.conf.developer_mode:
			nxenv.throw(
				_("Cannot edit Standard Notification. To edit, please disable this and duplicate it")
			)

	def validate_condition(self):
		temp_doc = nxenv.new_doc(self.document_type)
		if self.condition:
			try:
				nxenv.safe_eval(self.condition, None, get_context(temp_doc.as_dict()))
			except Exception:
				nxenv.throw(_("The Condition '{0}' is invalid").format(self.condition))

	def validate_forbidden_document_types(self):
		if self.document_type in FORBIDDEN_DOCUMENT_TYPES or (
			nxenv.get_meta(self.document_type).istable and self.event not in DATE_BASED_EVENTS
		):
	...
	def get_documents_for_today(self):
		...
		for d in doc_list:
			doc = nxenv.get_doc(self.document_type, d.name)

			if self.condition and not nxenv.safe_eval(self.condition, None, get_context(doc)):
				continue
	...
	def queue_send(self, doc, enqueue_after_commit=True):
		...
		Example:
		              ...
		              ```python
		              notification = nxenv.get_doc("Notification", "My Notification", ignore_permissions=True)
		...
		return enqueue(
			"nxenv.email.doctype.notification.notification.evaluate_alert",
			doc=doc,
			alert=self,
		...
		if self.set_property_after_alert:
			...
			try:
				if allow_update and not doc.flags.in_notification_update:
					...
					if doc.meta.get_field(fieldname).fieldtype in nxenv.model.numeric_fieldtypes:
						value = nxenv.utils.cint(value)
	...
	def create_system_notification(self, doc, context):
		...
		if "{" in subject:
			subject = nxenv.render_template(self.subject, context)
		...
		notification_doc = {
			...
			"email_content": nxenv.render_template(self.message, context),
	...
	def send_an_email(self, doc, context):
		...
		from nxenv.core.doctype.communication.email import _make as make_communication
		...
		sender = None
		message = nxenv.render_template(self.message, context)
		...
		if doc.doctype != "Communication":
			...
			comm = nxenv.get_doc("Communication", communication)
	...
	def send_sms(self, doc, context):
		send_sms(
			...
			msg=nxenv.utils.strip_html_tags(nxenv.render_template(self.message, context)),
		)
	...
	def get_list_of_recipients(self, doc, context):
		...
		for recipient in self.recipients:
			if recipient.condition:
				if not nxenv.safe_eval(recipient.condition, None, context):
					continue
	...
	def get_attachment(self, doc):
		...
		print_settings = nxenv.get_doc("Print Settings", "Print Settings")
			...
			return [
				{
					...
					"lang": nxenv.db.get_value("Print Format", self.print_format, "default_print_language")
	...
	def get_template(self, md_as_html=False):
		...
		path = os.path.join(os.path.dirname(module.__file__), nxenv.scrub(self.name))
		extension = FORMATS.get(self.message_type, ".md")
		file_path = path + extension
		...
		if extension == ".md":
			return nxenv.utils.md_to_html(template)
...
def get_documents_for_today(notification):
	notification = nxenv.get_doc("Notification", notification)
...
def trigger_notifications(doc, method=None):
	if nxenv.flags.in_import or nxenv.flags.in_patch:
	...
	if method == "daily":
		...
		for d in doc_list:
			alert = nxenv.get_doc("Notification", d.name)
...
def evaluate_alert(doc: Document, alert, event=None):
	...
	try:
		if isinstance(alert, str):
			alert = nxenv.get_doc("Notification", alert)

		context = get_context(doc)

		if alert.condition:
			if not nxenv.safe_eval(alert.condition, None, context):
				return

		if event == "Value Change" and not doc.is_new():
			if not nxenv.db.has_column(doc.doctype, alert.value_changed):
		...
		message = _("Error while evaluating Notification {0}. Please fix your template.").format(
			nxenv.utils.get_link_to_form("Notification", alert.name)
		)
		nxenv.throw(message, title=_("Error in Notification"))
	except Exception as e:
		title = str(e)
		message = nxenv.get_traceback(with_context=True)
		nxenv.log_error(title=title, message=message)
		msg = f"<details><summary>{title}</summary>{message}</details>"
		nxenv.throw(msg, title=_("Error in Notification"))


def get_context(doc):
	Nxenv = namedtuple("Nxenv", ["nxenv"])
	nxenv = Nxenv(nxenv=get_safe_globals().get("nxenv"))
	return {
		"doc": doc,
		"nowdate": nowdate,
		"nxenv": nxenv.nxenv,
...
def get_emails_from_template(template, context):
	...
	emails = nxenv.render_template(template, context) if "{" in template else template
