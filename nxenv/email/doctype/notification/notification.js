...
nxenv.notification = {
	setup_fieldname_select: function (frm) {
		...
		nxenv.model.with_doctype(frm.doc.document_type, function () {
			...
			let get_receiver_fields = function (
				...
				let extract_receiver_field = function (df) {
					...
					if (nxenv.model.table_fields.includes(df.fieldtype)) {
						let child_fields = nxenv.get_doc("DocType", df.options).fields;
			...
			let fields = nxenv.get_doc("DocType", frm.doc.document_type).fields;
			let options = $.map(fields, function (d) {
				return nxenv.model.no_value_type.includes(d.fieldtype)
					? null
...
nxenv.ui.form.on("Notification", {
	...
	refresh: function (frm) {
		nxenv.notification.setup_fieldname_select(frm);
		nxenv.notification.setup_example_message(frm);
		...
		frm.get_field("is_standard").toggle(nxenv.boot.developer_mode);
		frm.trigger("event");
		if (frm.doc.document_type) {
	...
	view_properties: function (frm) {
		nxenv.route_options = { doc_type: frm.doc.document_type };
	...
	event: function (frm) {
		...
		frm.add_custom_button(__("Get Alerts for Today"), function () {
			nxenv.call({
				method: "nxenv.email.doctype.notification.notification.get_documents_for_today",
				args: {
					notification: frm.doc.name,
				},
				callback: function (r) {
					if (r.message && r.message.length > 0) {
						nxenv.msgprint(r.message.toString());
					} else {
						nxenv.msgprint(__("No alerts for today"));
					}
				},
			});
		});
