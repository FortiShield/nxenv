	...
	refresh: function (frm) {
		if (!frm.is_new()) {
			frm.add_custom_button(
				...
				function () {
					nxenv.prompt(
						...
						function (data) {
							nxenv.call({
								method: "nxenv.email.doctype.email_group.email_group.import_from",
								args: {
									name: frm.doc.name,
			...
			frm.add_custom_button(
				...
				function () {
					nxenv.prompt(
						...
						function (data) {
							nxenv.call({
								method: "nxenv.email.doctype.email_group.email_group.add_subscribers",
								args: {
									name: frm.doc.name,
			...
			frm.add_custom_button(
				__("New Newsletter"),
				function () {
					nxenv.route_options = { email_group: frm.doc.name };
					nxenv.new_doc("Newsletter");
				},
				__("Action")
			);
		}
