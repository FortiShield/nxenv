...
from nxenv.utils import parse_addr, validate_email_address
...
class EmailGroup(Document):
	...
	def onload(self):
		singles = [d.name for d in nxenv.get_all("DocType", "name", {"issingle": 1})]
		self.get("__onload").import_types = [
			{"value": d.parent, "label": f"{d.parent} ({d.label})"}
			for d in nxenv.get_all("DocField", ("parent", "label"), {"options": "Email"})
	...
	def import_from(self, doctype):
		...
		for user in nxenv.get_all(doctype, [email_field, unsubscribed_field or "name"]):
			with contextlib.suppress(nxenv.UniqueValidationError, nxenv.InvalidEmailAddressError):
		...
		nxenv.msgprint(_("{0} subscribers added").format(added))
	...
	def on_trash(self):
		for d in nxenv.get_all("Email Group Member", "name", {"email_group": self.name}):
			nxenv.delete_doc("Email Group Member", d.name, force=True)


@nxenv.whitelist()
def import_from(name, doctype):
	nlist = nxenv.get_doc("Email Group", name)
...
def add_subscribers(name, email_list):
	...
	template = nxenv.db.get_value("Email Group", name, "welcome_email_template")
	welcome_email = nxenv.get_doc("Email Template", template) if template else None
	...
	for email in email_list:
		...
		if parsed_email:
			if not nxenv.db.get_value("Email Group Member", {"email_group": name, "email": parsed_email}):
				nxenv.get_doc(
					{"doctype": "Email Group Member", "email_group": name, "email": parsed_email}
				).insert(ignore_permissions=nxenv.flags.ignore_permissions)
			...
			nxenv.msgprint(_("{0} is not a valid Email Address").format(email))

	nxenv.msgprint(_("{0} subscribers added").format(count))

	return nxenv.get_doc("Email Group", name).update_total_subscribers()
...
def send_welcome_email(welcome_email, email, email_group):
	...
	message = nxenv.render_template(welcome_email.response_, args)
	nxenv.sendmail(email, subject=welcome_email.subject, message=message)
