...
function oauth_access(frm) {
	nxenv.model.with_doc("Connected App", frm.doc.connected_app, () => {
		const connected_app = nxenv.get_doc("Connected App", frm.doc.connected_app);
		return nxenv.call({
			doc: connected_app,
...
function set_default_max_attachment_size(frm) {
	if (frm.doc.__islocal && !frm.doc["attachment_limit"]) {
		nxenv.call({
			method: "nxenv.core.api.file.get_max_file_size",
			callback: function (r) {
				if (!r.exc) {
	...
	switch (frm.doc.service) {
		...
		case "Nxenv Mail":
			frm.set_df_property(
				"api_secret",
				"description",
				'To know more visit <a href="https://github.com/nxenv/mail"> here'
			);
			break;
	}
}
nxenv.ui.form.on("Email Account", {
	service: function (frm) {
		$.each(nxenv.email_defaults[frm.doc.service], function (key, value) {
			frm.set_value(key, value);
		});
		if (!frm.doc.use_imap) {
			$.each(nxenv.email_defaults_pop[frm.doc.service], function (key, value) {
				frm.set_value(key, value);
			});
		}
	...
	onload: function (frm) {
		...
		frm.set_query(
			"append_to",
			"nxenv.email.doctype.email_account.email_account.get_append_to"
		);
		frm.set_query("append_to", "imap_folder", function () {
			return {
				query: "nxenv.email.doctype.email_account.email_account.get_append_to",
			};
		});
	...
	refresh: function (frm) {
		...
		if (nxenv.route_flags.delete_user_from_locals && nxenv.route_flags.linked_user) {
			delete nxenv.route_flags.delete_user_from_locals;
			delete locals["User"][nxenv.route_flags.linked_user];
		}

		if (!frm.is_dirty() && frm.doc.enable_incoming) {
			frm.add_custom_button(__("Pull Emails"), () => {
				nxenv.dom.freeze(__("Pulling emails..."));
				frm.call({
					method: "pull_emails",
					...
					if (!(r._server_messages && r._server_messages.length)) {
						nxenv.show_alert({ message: __("Emails Pulled"), indicator: "green" });
					}
				});
			});
		}
	...
	validate_nxenv_mail_settings: function (frm) {
		if (frm.doc.service == "Nxenv Mail") {
			nxenv.call({
				doc: frm.doc,
				method: "validate_nxenv_mail_settings",
			});
		}
	},
	...
	show_oauth_authorization_message(frm) {
		if (
			...
			nxenv.call({
				method: "nxenv.integrations.doctype.connected_app.connected_app.has_token",
				args: {
	...
	domain: nxenv.utils.debounce((frm) => {
		if (frm.doc.domain) {
			nxenv.call({
	...
	email_sync_option: function (frm) {
		...
		if (frm.doc.email_sync_option == "ALL") {
			...
			nxenv.confirm(msg, null, function () {
	...
	warn_autoreply_on_incoming: function (frm) {
		if (frm.doc.enable_incoming && frm.doc.enable_auto_reply && frm.doc.__islocal) {
			...
			nxenv.confirm(msg, null, function () {
				...
				nxenv.show_alert({ message: __("Disabled Auto Reply"), indicator: "blue" });
			});
		}
	},
});
