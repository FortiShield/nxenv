...
import nxenv
from nxenv import _, are_emails_muted, safe_encode
from nxenv.desk.form import assign_to
from nxenv.email.doctype.email_domain.email_domain import EMAIL_DOMAIN_FIELDS
from nxenv.email.nxenvmail import NxenvMail
from nxenv.email.receive import EmailServer, InboundMail, SentEmailInInboxError
from nxenv.email.smtp import SMTPServer
from nxenv.email.utils import get_port
from nxenv.model.document import Document
from nxenv.utils import cint, comma_or, cstr, parse_addr, validate_email_address
from nxenv.utils.background_jobs import enqueue, get_jobs
from nxenv.utils.jinja import render_template
...
def cache_email_account(cache_name):
	def decorator_cache_email_account(func):
		...
		def wrapper_cache_email_account(*args, **kwargs):
			if not hasattr(nxenv.local, cache_name):
				setattr(nxenv.local, cache_name, {})

			cached_accounts = getattr(nxenv.local, cache_name)
...
class EmailAccount(Document):
	...
	if TYPE_CHECKING:
		from nxenv.email.doctype.imap_folder.imap_folder import IMAPFolder
		...
		service: DF.Literal[
			"", "Nxenv Mail", "GMail", "Sendgrid", "SparkPost", "Yahoo Mail", "Outlook.com", "Yandex.Mail"
		]
	...
	def validate(self):
		...
		if self.login_id_is_different:
			if not self.login_id:
				nxenv.throw(_("Login Id is required"))
		else:
			self.login_id = None

		if self.service == "Nxenv Mail":
			self.use_imap = 0
			self.always_use_account_email_id_as_sender = 1

			if self.auth_method == "Basic" or self.get_oauth_token():
				self.validate_nxenv_mail_settings()

		# validate the imap settings
		if self.enable_incoming and self.use_imap and len(self.imap_folder) <= 0:
			nxenv.throw(_("You need to set one IMAP folder for {0}").format(nxenv.bold(self.email_id)))

		if nxenv.local.flags.in_patch or nxenv.local.flags.in_test:
			return
		...
		if (
			not nxenv.local.flags.in_install
			and not self.awaiting_password
			and not self.service == "Nxenv Mail"
		):
			if validate_oauth or self.password or self.smtp_server in ("127.0.0.1", "localhost"):
				...
				if self.enable_incoming or (self.enable_outgoing and not self.no_smtp_authentication):
					if not use_oauth:
						nxenv.throw(_("Password is required or select Awaiting Password"))

		if self.notify_if_unreplied:
			if not self.send_notification_to:
				nxenv.throw(_("{0} is mandatory").format(self.meta.get_label("send_notification_to")))
			...
			for folder in self.imap_folder:
				if folder.append_to:
					...
					if folder.append_to not in valid_doctypes:
						nxenv.throw(_("Append To can be one of {0}").format(comma_or(valid_doctypes)))

	@nxenv.whitelist()
	def validate_nxenv_mail_settings(self):
		if self.service == "Nxenv Mail":
			nxenv_mail_client = self.get_nxenv_mail_client()
			nxenv_mail_client.validate(for_inbound=self.enable_incoming, for_outbound=self.enable_outgoing)

	def validate_smtp_conn(self):
		if not self.smtp_server:
			nxenv.throw(_("SMTP Server is required"))
	...
	def before_save(self):
		...
		if not self.enable_incoming and self.default_incoming:
			...
			messages.append(
				_("{} has been disabled. It can only be enabled if {} is checked.").format(
					nxenv.bold(_("Default Incoming")),
					nxenv.bold(_("Enable Incoming")),
				)
			)
		if not self.enable_outgoing and self.default_outgoing:
			...
			messages.append(
				_("{} has been disabled. It can only be enabled if {} is checked.").format(
					nxenv.bold(_("Default Outgoing")),
					nxenv.bold(_("Enable Outgoing")),
				)
			)
		if messages:
			if len(messages) == 1:
	...
	def there_must_be_only_one_default(self):
		...
		for field in ("default_incoming", "default_outgoing"):
			...
			for email_account in nxenv.get_all("Email Account", filters={field: 1}):
				if email_account.name == self.name:
					continue

				email_account = nxenv.get_doc("Email Account", email_account.name)
				email_account.set(field, 0)
				email_account.save()

	@nxenv.whitelist()
	def get_domain_values(self, domain: str):
		return nxenv.db.get_value("Email Domain", domain, EMAIL_DOMAIN_FIELDS, as_dict=True)
	...
	def get_incoming_server(self, in_receive=False, email_sync_rule="UNSEEN"):
		...
		if not args.get("host"):
			nxenv.throw(_("{0} is required").format("Email Server"))

		email_server = EmailServer(nxenv._dict(args))
	...
	def check_email_server_connection(self, email_server, in_receive):
		...
		try:
			...
			if in_receive:
				...
				description = nxenv.message_log.pop() if nxenv.message_log else "Socket Error"
	...
	def _password(self):
		raise_exception = not (
			self.auth_method == "OAuth" or self.no_smtp_authentication or nxenv.flags.in_test
		)
	...
	def from_record(cls, record):
		email_account = nxenv.new_doc(cls.DOCTYPE)
		email_account.update(record)
		return email_account

	@classmethod
	def find(cls, name):
		return nxenv.get_doc(cls.DOCTYPE, name)

	@classmethod
	def find_one_by_filters(cls, **kwargs) -> "EmailAccount":
		name = nxenv.db.get_value(cls.DOCTYPE, kwargs)
	...
	def get_account_details_from_site_config(cls):
		if not nxenv.conf.get("mail_server"):
			return {}
		...
		field_to_conf_name_map = {
			...
			"name": {"conf_names": ("email_sender_name",), "default": "Nxenv"},
		...
		for doc_field_name, d in field_to_conf_name_map.items():
			...
			value = [nxenv.conf.get(k) for k in conf_names if nxenv.conf.get(k)]
	...
	def get_nxenv_mail_client(self):
		return self._nxenv_mail_client

	@functools.cached_property
	def _nxenv_mail_client(self):
		if self.auth_method == "OAuth":
			if access_token := self.get_access_token():
				return NxenvMail(self.nxenv_mail_site, self.email_id, access_token=access_token)

			nxenv.throw(
				_("Please Authorize OAuth for Email Account {0}").format(
					nxenv.bold(self.email_account_name)
				),
				title=_("Nxenv Mail OAuth Error"),
			)
		else:
			return NxenvMail(
				self.nxenv_mail_site, self.email_id, self.api_key, self.get_password("api_secret")
			)
	...
	def handle_incoming_connect_error(self, description):
		if self.get_failed_attempts_count() > 5:
			...
			nxenv.enqueue(self._disable_broken_incoming_account, description=description)
		else:
			self.set_failed_attempts_count(self.get_failed_attempts_count() + 1)

	def _disable_broken_incoming_account(self, description):
		if nxenv.flags.in_test:
			return
	...
	def set_failed_attempts_count(self, value):
		nxenv.cache.set_value(f"{self.name}:email-account-failed-attempts", value)

	def get_failed_attempts_count(self):
		return cint(nxenv.cache.get_value(f"{self.name}:email-account-failed-attempts"))
	...
	def receive(self):
		...
		for mail in inbound_mails:
			try:
				...
				try:
					...
					if self.use_imap:
						self.handle_bad_emails(mail.uid, mail.raw_message, nxenv.get_traceback())
					exceptions.append(nxenv.get_traceback())
				except Exception:
					nxenv.db.rollback()
				else:
					nxenv.db.commit()
		...
		if exceptions:
			raise Exception(nxenv.as_json(exceptions))
	...
	def get_inbound_mails(self) -> list[InboundMail]:
		...
		try:
			if self.service == "Nxenv Mail":
				nxenv_mail_client = self.get_nxenv_mail_client()
				messages = nxenv_mail_client.pull_raw(last_synced_at=self.last_synced_at)
	...
	def send_auto_reply(self, communication, email):
		...
		from nxenv.core.doctype.communication.email import (
		...
		if self.enable_auto_reply:
			...
			nxenv.sendmail(
				...
				or nxenv.get_template("templates/emails/auto_reply.html").render(communication.as_dict()),
	...
	def on_trash(self):
		...
		Communication = nxenv.qb.DocType("Communication")
		nxenv.qb.update(Communication).set(Communication.email_account, "").where(
	...
	def after_rename(self, old, new, merge=False):
		nxenv.db.set_value("Email Account", new, "email_account_name", new)
	...
	def check_automatic_linking_email_account(self):
		if self.enable_automatic_linking:
			if not self.enable_incoming:
				nxenv.throw(_("Automatic Linking can be activated only if Incoming is enabled."))

			if nxenv.db.exists(
				"Email Account",
				{"enable_automatic_linking": 1, "name": ("!=", self.name)},
			):
				nxenv.throw(_("Automatic Linking can be activated only for one Email Account."))
	...
	def get_oauth_token(self):
		if self.auth_method == "OAuth":
			connected_app = nxenv.get_doc("Connected App", self.connected_app)
...
def get_append_to(doctype=None, txt=None, searchfield=None, start=None, page_len=None, filters=None):
	...
	email_append_to_list = [
		dt.name for dt in nxenv.get_all("DocType", filters=filters, fields=["name", "email_append_to"])
	]
	# Set Email Append To DocTypes set via Customize Form
	email_append_to_list.extend(
		dt.doc_type
		for dt in nxenv.get_list(
			"Property Setter",
...
def notify_unreplied():
	...
	for email_account in nxenv.get_all(
		"Email Account",
		"name",
		...
		if append_to:
			...
			for comm in nxenv.get_all(
				...
				comm = nxenv.get_doc("Communication", comm.name)

				if nxenv.db.get_value(comm.reference_doctype, comm.reference_name, "status") == "Open":
...
def pull(now=False):
	...
	from nxenv.integrations.doctype.connected_app.connected_app import has_token

	doctype = nxenv.qb.DocType("Email Account")
	email_accounts = (
	...
	for email_account in email_accounts:
		...
		if now:
			...
			queued_jobs = get_jobs(site=nxenv.local.site, key="job_name")[nxenv.local.site]
...
def pull_emails(email_account: str) -> None:
	...
	nxenv.has_permission("Email Account", "read", throw=True)
	...
	if job_name not in queued_jobs:
		...
		nxenv.msgprint(_("Emails are already being pulled from this account."))


def pull_from_email_account(email_account):
	"""Runs within a worker process"""
	email_account = nxenv.get_doc("Email Account", email_account)
...
def setup_user_email_inbox(email_account, awaiting_password, email_id, enable_outgoing, used_oauth):
	...
	from nxenv.core.doctype.user.user import ask_pass_update
	...
	user_names = nxenv.db.get_values("User", {"email": email_id}, as_dict=True)
	if not user_names:
		return
	...
	if update_user_email_settings:
		UserEmail = nxenv.qb.DocType("User Email")
		nxenv.qb.update(UserEmail).set(UserEmail.awaiting_password, (awaiting_password or 0)).set(
		...
		users = " and ".join([nxenv.bold(user.get("name")) for user in user_names])
		nxenv.msgprint(_("Enabled email inbox for user {0}").format(users))
	ask_pass_update()
...
def remove_user_email_inbox(email_account):
	...
	for user in users:
		doc = nxenv.get_doc("User", user.get("name"))
...
def set_email_password(email_account, password):
	account = nxenv.get_doc("Email Account", email_account)
