nxenv.listview_settings["Email Queue"] = {
...
function show_toggle_sending_button(list_view) {
	...
	const sending_disabled = cint(nxenv.sys_defaults.suspend_email_queue);
	const label = sending_disabled ? __("Resume Sending") : __("Suspend Sending");

	list_view.page.add_inner_button(label, async () => {
		await nxenv.xcall(
			"nxenv.email.doctype.email_queue.email_queue.toggle_sending",
		...
		nxenv.sys_defaults.suspend_email_queue = sending_disabled ? 0 : 1;
