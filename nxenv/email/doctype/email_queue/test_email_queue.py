...
import nxenv
from nxenv.email.doctype.email_queue.email_queue import SendMailContext, get_email_retry_limit
...
class TestEmailQueue(IntegrationTestCase):
	def test_email_queue_deletion_based_on_modified_date(self):
		...
		new_record = nxenv.copy_doc(old_record)
		new_record.insert()

		EmailQueue.clear_old_logs()

		self.assertFalse(nxenv.db.exists("Email Queue", old_record.name))
		self.assertFalse(nxenv.db.exists("Email Queue Recipient", {"parent": old_record.name}))

		self.assertTrue(nxenv.db.exists("Email Queue", new_record.name))
		self.assertTrue(nxenv.db.exists("Email Queue Recipient", {"parent": new_record.name}))

	def test_failed_email_notification(self):
		subject = nxenv.generate_hash()
		email_record = nxenv.new_doc("Email Queue")
		email_record.sender = "Test <test@example.com>"
		email_record.message = textwrap.dedent(
			f"""\
		MIME-Version: 1.0
		Message-Id: {nxenv.generate_hash()}
		X-Original-From: Test <test@example.com>
		Subject: {subject}
		From: Test <test@example.com>
		To: <!--recipient-->
		Date: {nxenv.utils.now_datetime().strftime('%a, %d %b %Y %H:%M:%S %z')}
		Reply-To: test@example.com
		X-Nxenv-Site: {nxenv.local.site}
		"""
		)
		...
		notification_log = nxenv.db.get_value(
			"Notification Log",
	...
	def test_perf_reusing_smtp_server(self):
		...
		self.assertTrue(nxenv.new_doc("Email Queue").get_email_account()._from_site_config)

		def get_server(q):
			return q.get_email_account().get_smtp_server()

		self.assertIs(get_server(nxenv.new_doc("Email Queue")), get_server(nxenv.new_doc("Email Queue")))

		q1 = nxenv.new_doc("Email Queue", email_account="_Test Email Account 1")
		q2 = nxenv.new_doc("Email Queue", email_account="_Test Email Account 1")
		self.assertIsNot(get_server(nxenv.new_doc("Email Queue")), get_server(q1))
