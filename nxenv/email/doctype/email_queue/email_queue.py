...
import nxenv
from nxenv import _, safe_encode, task
from nxenv.core.utils import html2text
from nxenv.database.database import savepoint
from nxenv.email.doctype.email_account.email_account import EmailAccount
from nxenv.email.email_body import add_attachment, get_email, get_formatted_html
from nxenv.email.nxenvmail import NxenvMail
from nxenv.email.queue import get_unsubcribed_url, get_unsubscribe_message
from nxenv.email.smtp import SMTPServer
from nxenv.model.document import Document
from nxenv.query_builder import DocType, Interval
...
from nxenv.utils.verified_command import get_signed_params
...
class EmailQueue(Document):
	...
	if TYPE_CHECKING:
		from nxenv.email.doctype.email_queue_recipient.email_queue_recipient import EmailQueueRecipient
	...
	def prevent_email_queue_delete(self):
		if nxenv.session.user != "Administrator":
			nxenv.throw(_("Only Administrator can delete Email Queue"))

	def get_duplicate(self, recipients):
		values = self.as_dict()
		del values["name"]
		duplicate = nxenv.get_doc(values)
	...
	def update_db(self, commit=False, **kwargs):
		nxenv.db.set_value(self.DOCTYPE, self.name, kwargs)
		if commit:
			nxenv.db.commit()
	...
	def update_status(self, status, commit=False, **kwargs):
		...
		if self.communication:
			communication_doc = nxenv.get_doc("Communication", self.communication)
	...
	def get_email_account(self, raise_error=False):
		if self.email_account:
			return nxenv.get_cached_doc("Email Account", self.email_account)
	...
	def can_send_now(self):
		if (
			nxenv.are_emails_muted()
			or not self.is_to_be_sent()
			or cint(nxenv.db.get_default("suspend_email_queue")) == 1
		):
			return False

		return True

	def send(self, smtp_server_instance: SMTPServer = None, nxenv_mail_client: NxenvMail = None):
		"""Send emails to recipients."""
		if not self.can_send_now():
			return

		with SendMailContext(self, smtp_server_instance, nxenv_mail_client) as ctx:
			...
			for recipient in self.recipients:
				...
				elif not nxenv.flags.in_test or nxenv.flags.testing_email:
					if ctx.email_account_doc.service == "Nxenv Mail":
						is_newsletter = self.reference_doctype == "Newsletter"
						ctx.nxenv_mail_client.send_raw(
							sender=self.sender,
			...
			if nxenv.flags.in_test and not nxenv.flags.testing_email:
				nxenv.flags.sent_mail = message
				return
	...
	def clear_old_logs(days=30):
		...
		"""
		days = days or 31
		email_queue = nxenv.qb.DocType("Email Queue")
		email_recipient = nxenv.qb.DocType("Email Queue Recipient")

		# Delete queue table
		(
			nxenv.qb.from_(email_queue).delete().where(email_queue.creation < (Now() - Interval(days=days)))
		).run()
		...
		(
			nxenv.qb.from_(email_recipient)
			.delete()
			.where(email_recipient.creation < (Now() - Interval(days=days)))
		).run()


from nxenv.deprecation_dumpster import send_mail as _send_mail
...
class SendMailContext:
	...
	def fetch_outgoing_server(self):
		...
		if self.email_account_doc.service == "Nxenv Mail":
			if not self.nxenv_mail_client:
				self.nxenv_mail_client = self.email_account_doc.get_nxenv_mail_client()
	...
	def __exit__(self, exc_type, exc_val, exc_tb):
		if exc_type:
			update_fields = {"error": nxenv.get_traceback()}
	...
	def get_tracker_str(self, recipient_email) -> str:
		...
		if self.queue_doc.get("email_read_tracker_url"):
			...
			tracker_url = f"{get_url()}/api/method/nxenv.core.doctype.communication.email.mark_email_as_seen?name={self.queue_doc.communication}"
	...
	def include_attachments(self, message):
		...
		for attachment in attachments:
			...
			if file_filters:
				_file = nxenv.get_doc("File", file_filters)
				...
				print_format_file = nxenv.attach_print(**attachment)
	...
	def _store_file(self, file_name, content):
		if not nxenv.get_system_settings("store_attached_pdf_document"):
			return

		file_data = nxenv._dict(file_name=file_name, is_private=1)
		...
		if nxenv.db.exists("File", file_data):
			return

		file = nxenv.new_doc("File", **file_data)
...
def retry_sending(queues: str | list[str]):
	if not nxenv.has_permission("Email Queue", throw=True):
		return
	...
	email_queue = nxenv.qb.DocType("Email Queue")
	nxenv.qb.update(email_queue).set(email_queue.status, "Not Sent").set(email_queue.modified, now()).set(
		email_queue.modified_by, nxenv.session.user
...
def toggle_sending(enable):
	...
	nxenv.db.set_default("suspend_email_queue", 0 if sbool(enable) else 1)


def on_doctype_update():
	"""Add index in `tabCommunication` for `(reference_doctype, reference_name)`"""
	nxenv.db.add_index("Email Queue", ("status", "send_after", "priority", "creation"), "index_bulk_flush")

	nxenv.db.add_index("Email Queue", ["message_id(140)"])


def get_email_retry_limit():
	return cint(nxenv.db.get_system_setting("email_retry_limit")) or 3
...
class QueueBuilder:
	...
	def __init__(
		...
		:param unsubscribe_method: URL method for unsubscribe. Default is `/api/method/nxenv.email.queue.unsubscribe`.
	...
	def unsubscribe_method(self):
		return self._unsubscribe_method or "/api/method/nxenv.email.queue.unsubscribe"
	...
	def get_unsubscribed_user_emails(self):
		...
		if len(all_ids) > 0:
			unsubscribed = (
				nxenv.qb.from_(EmailUnsubscribe)
	...
	def process(self, send_now=False) -> EmailQueue | None:
		...
		if not queue_separately:
			...
			if send_now and len(final_recipients) >= 1000:
				...
				send_now = False
			for recipients in nxenv.utils.create_batch(final_recipients, 1000):
				nxenv.enqueue(
					self.send_emails,
					queue_data=queue_data,
					final_recipients=recipients,
					job_name=nxenv.utils.get_job_name(
						"send_bulk_emails_for", self.reference_doctype, self.reference_name
					),
					now=nxenv.flags.in_test or send_now,
					queue="long",
				)
	...
	def send_emails(self, queue_data, final_recipients):
		...
		nxenv_mail_client = None
		smtp_server_instance = None
		for r in final_recipients:
			recipients = list(set([r, *self.final_cc(), *self.bcc]))
			q = EmailQueue.new({**queue_data, **{"recipients": recipients}}, ignore_permissions=True)
			if not nxenv_mail_client and not smtp_server_instance:
				email_account = q.get_email_account(raise_error=True)

				if email_account.service == "Nxenv Mail":
					nxenv_mail_client = email_account.get_nxenv_mail_client()
				else:
					smtp_server_instance = email_account.get_smtp_server()

			with suppress(Exception):
				q.send(smtp_server_instance=smtp_server_instance, nxenv_mail_client=nxenv_mail_client)
	...
	def as_dict(self, include_recipients=True):
		...
		except nxenv.InvalidEmailAddressError:
