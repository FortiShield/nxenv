...
class EmailDomain(Document):
	...
	def validate(self):
		...
		if nxenv.local.flags.in_patch or nxenv.local.flags.in_test or nxenv.local.flags.in_install:
			return
	...
	def on_update(self):
		...
		for email_account in nxenv.get_all("Email Account", filters={"domain": self.name}):
			try:
