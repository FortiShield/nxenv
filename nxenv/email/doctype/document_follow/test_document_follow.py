...
import nxenv
import nxenv.desk.form.document_follow as document_follow
from nxenv.desk.form.assign_to import add
from nxenv.desk.form.document_follow import get_document_followed_by_user
from nxenv.desk.form.utils import add_comment
from nxenv.desk.like import toggle_like
from nxenv.query_builder import DocType
from nxenv.query_builder.functions import Cast_
from nxenv.share import add as share
...
class TestDocumentFollow(IntegrationTestCase):
	...
	def tearDown(self):
		...
		nxenv.db.delete("Document Follow")
	...
	doc = nxenv.get_doc(
		{
			...
			"doc.starts_on": nxenv.utils.now(),
			"doc.ends_on": nxenv.utils.add_days(nxenv.utils.now(), 5),
...
def get_user(document_follow=None):
	...
	if nxenv.db.exists("User", "test@docsub.com"):
		doc = nxenv.delete_doc("User", "test@docsub.com")
	doc = nxenv.new_doc("User")
