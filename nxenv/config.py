...
import nxenv
from nxenv import _dict, get_file_json
from nxenv.exceptions import IncorrectSitePath
from nxenv.utils.caching import site_cache
...
def get_site_config(
	...
	*,
	...
	sites_path = sites_path or getattr(nxenv.local, "sites_path", ".")
	site_path = site_path or getattr(nxenv.local, "site_path", None)
...
def _get_site_config(sites_path: str, site_path: str) -> _dict[str, Any]:
	...
	if site_path:
		...
		if os.path.exists(site_config):
			try:
				...
				click.secho(f"{nxenv.local.site}/site_config.json is invalid", fg="red")
				print(error)
				raise
		elif nxenv.local.site and not nxenv.local.flags.new_site:
			error_msg = f"{nxenv.local.site} does not exist."
	...
	config["redis_queue"] = (
		os.environ.get("NXENV_REDIS_QUEUE") or config.get("redis_queue") or "redis://127.0.0.1:11311"
	)
	config["redis_cache"] = (
		os.environ.get("NXENV_REDIS_CACHE") or config.get("redis_cache") or "redis://127.0.0.1:13311"
	)
	config["db_type"] = os.environ.get("NXENV_DB_TYPE") or config.get("db_type") or "mariadb"
	config["db_socket"] = os.environ.get("NXENV_DB_SOCKET") or config.get("db_socket")
	config["db_host"] = os.environ.get("NXENV_DB_HOST") or config.get("db_host") or "127.0.0.1"
	config["db_port"] = int(
		os.environ.get("NXENV_DB_PORT") or config.get("db_port") or db_default_ports(config["db_type"])
	)

	# Set the user as database name if not set in config
	config["db_user"] = os.environ.get("NXENV_DB_USER") or config.get("db_user") or config.get("db_name")

	# vice versa for dbname if not defined
	config["db_name"] = os.environ.get("NXENV_DB_NAME") or config.get("db_name") or config["db_user"]

	# read password
	config["db_password"] = os.environ.get("NXENV_DB_PASSWORD") or config.get("db_password")
