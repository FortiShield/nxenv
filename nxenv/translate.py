...
from nxenv.utils import cstr, get_nxcli_path, is_html, strip, strip_html_tags, unique
...
def get_language(lang_list: list | None = None) -> str:
	"""Set `nxenv.local.lang` from HTTP headers at beginning of request
	...
	"""
	is_logged_in = nxenv.session.user != "Guest"

	# fetch language from form_dict
	if nxenv.form_dict._lang:
		language = get_lang_code(nxenv.form_dict._lang or get_parent_language(nxenv.form_dict._lang))
		if language:
			return language
	...
	accept_language = list(nxenv.request.accept_languages.values())
	...
	return nxenv.get_system_settings("language") or "en"
...
def get_user_lang(user: str | None = None) -> str:
	"""Set nxenv.local.lang from user preferences on session beginning or resumption"""
	user = user or nxenv.session.user

	# User.language => Session Defaults => nxenv.local.lang => 'en'
	return (
		nxenv.get_cached_value("User", user, "language")
		or nxenv.get_system_settings("language")
		or nxenv.local.lang
		or "en"
	)
...
def get_lang_code(lang: str) -> str | None:
	return nxenv.db.get_value("Language", {"name": lang}) or nxenv.db.get_value(
...
def set_default_language(lang):
	...
	if nxenv.db.get_default("lang") != lang:
		nxenv.db.set_default("lang", lang)
	nxenv.local.lang = lang


def get_lang_dict():
	"""Return all languages in dict format, full name is the key e.g. `{"english":"en"}`."""
	return dict(
		nxenv.get_all("Language", fields=["language_name", "name"], order_by="creation", as_list=True)
	)
...
def get_messages_for_boot():
	...
	return get_all_translations(nxenv.local.lang)


@nxenv.whitelist(allow_guest=True)
def get_app_translations():
	if nxenv.session.user != "Guest":
		language = nxenv.db.get_value("User", nxenv.session.user, "language")
	else:
		language = nxenv.db.get_single_value("System Settings", "language")
...
def get_all_translations(lang: str) -> dict[str, str]:
	...
	try:
		return nxenv.cache.hget(MERGED_TRANSLATION_KEY, lang, generator=_merge_translations)
	except Exception:
		...
		nxenv.logger().error("Unable to load translations", exc_info=True)
		return {}
...
def get_translations_from_apps(lang, apps=None):
	...
	translations = {}
	from nxenv.gettext.translate import get_translations_from_mo

	for app in apps or nxenv.get_installed_apps(_ensure_on_nxcli=True):
...
def get_translations_from_csv(lang, app):
	return get_translation_dict_from_file(
		os.path.join(nxenv.get_app_path(app, "translations"), lang + ".csv"), lang, app
	)
	...
	if os.path.exists(path):
		...
		for item in csv_content:
			if len(item) == 3 and item[2]:
				...
				nxenv.log_error(message=msg, title="Error in translation file")
				if throw:
					nxenv.throw(msg, title="Error in translation file")
...
def get_user_translations(lang):
	...
	return nxenv.cache.hget(USER_TRANSLATION_KEY, lang, generator=_read_from_db)


def clear_cache():
	"""Clear all translation assets from :meth:`nxenv.cache`"""
	nxenv.cache.delete_value(
...
def get_messages_for_app(app, deduplicate=True):
	...
	messages = []
	modules = [nxenv.unscrub(m) for m in nxenv.local.app_modules[app]]
	...
	if modules:
		if isinstance(modules, str):
			modules = [modules]
		filtered_doctypes = (
			nxenv.qb.from_("DocType").where(Field("module").isin(modules)).select("name").run(pluck=True)
		)
		...
		# pages
		filtered_pages = (
			nxenv.qb.from_("Page").where(Field("module").isin(modules)).select("name", "title").run()
		)
...
def get_messages_from_navbar():
	...
	labels = nxenv.get_all("Navbar Item", filters={"item_label": ("is", "set")}, pluck="item_label")
...
def get_messages_from_doctype(name):
	...
	from nxenv.gettext.extractors.utils import is_translatable

	messages = []
	meta = nxenv.get_meta(name)
	...
	if not meta.custom:
		doctype_file_path = nxenv.get_module_path(meta.module, "doctype", meta.name, meta.name)
...
def get_messages_from_workflow(doctype=None, app_name=None):
	...
	workflows = []
	if doctype:
		workflows = nxenv.get_all("Workflow", filters={"document_type": doctype})
	else:
		fixtures = nxenv.get_hooks("fixtures", app_name=app_name) or []
		for fixture in fixtures:
			if isinstance(fixture, str) and fixture == "Workflow":
				workflows = nxenv.get_all("Workflow")
				break
			elif isinstance(fixture, dict) and fixture.get("dt", fixture.get("doctype")) == "Workflow":
				workflows.extend(nxenv.get_all("Workflow", filters=fixture.get("filters")))

	messages = []
	document_state = DocType("Workflow Document State")
	for w in workflows:
		states = nxenv.db.get_values(
			document_state,
...
def get_messages_from_page(name):
	"""Return all translatable strings from a :class:`nxenv.core.doctype.Page`."""
	return _get_messages_from_page_or_report("Page", name)


def get_messages_from_report(name):
	"""Return all translatable strings from a :class:`nxenv.core.doctype.Report`."""
	from nxenv.gettext.extractors.utils import is_translatable

	report = nxenv.get_doc("Report", name)
	messages = _get_messages_from_page_or_report(
		"Report", name, nxenv.db.get_value("DocType", report.ref_doctype, "module")
	)
...
def _get_messages_from_page_or_report(doctype, name, module=None):
	if not module:
		module = nxenv.db.get_value(doctype, name, "module")

	doc_path = nxenv.get_module_path(module, doctype, name)

	messages = get_messages_from_file(os.path.join(doc_path, nxenv.scrub(name) + ".py"))
...
def get_server_messages(app):
	"""Extracts all translatable strings (tagged with :func:`nxenv._`) from Python modules
	inside an app"""
	messages = []
	file_extensions = (".py", ".html", ".js", ".vue")
	app_walk = os.walk(nxenv.get_app_path(app))
...
def get_messages_from_include_files(app_name=None):
	...
	messages = []
	app_include_js = nxenv.get_hooks("app_include_js", app_name=app_name) or []
	web_include_js = nxenv.get_hooks("web_include_js", app_name=app_name) or []
	include_js = app_include_js + web_include_js

	for js_path in include_js:
		file_path = bundled_asset(js_path)
		relative_path = os.path.join(nxenv.local.sites_path, file_path.lstrip("/"))
...
def get_all_messages_from_js_files(app_name=None):
	...
	messages = []
	for app in [app_name] if app_name else nxenv.get_installed_apps(_ensure_on_nxcli=True):
		if os.path.exists(nxenv.get_app_path(app, "public")):
			for basepath, folders, files in os.walk(nxenv.get_app_path(app, "public")):  # noqa: B007
				if "nxenv/public/js/lib" in basepath:
					continue
...
def get_messages_from_file(path: str) -> list[tuple[str, str, str | None, int]]:
	...
	from nxenv.gettext.extractors.utils import extract_messages_from_code

	nxenv.flags.setdefault("scanned_files", set())
	# TODO: Find better alternative
	# To avoid duplicate scan
	if path in nxenv.flags.scanned_files:
		return []

	nxenv.flags.scanned_files.add(path)
	...
	with open(path) as sourcefile:
		...
		return [
			(os.path.relpath(path, nxcli_path), message, context, line)
...
def update_translations(lang, untranslated_file, translated_file, app="_ALL_APPS"):
	...
	for key, value in zip(
		nxenv.get_file_items(untranslated_file, ignore_empty_lines=False),
		nxenv.get_file_items(translated_file, ignore_empty_lines=False),
		strict=False,
	):
...
def import_translations(lang, path):
	...
	for app in nxenv.get_all_apps(True):
...
def migrate_translations(source_app, target_app):
	...
	strings_in_source_app = [m[1] for m in nxenv.translate.get_messages_for_app(source_app)]
	strings_in_target_app = [m[1] for m in nxenv.translate.get_messages_for_app(target_app)]

	strings_in_target_app_but_not_in_source_app = list(
		set(strings_in_target_app) - set(strings_in_source_app)
	)

	languages = nxenv.translate.get_all_languages()

	source_app_translations_dir = nxenv.get_app_path(source_app, "translations")
	target_app_translations_dir = nxenv.get_app_path(target_app, "translations")
...
def write_translations_file(app, lang, full_dict=None, app_messages=None):
	...
	tpath = nxenv.get_app_path(app, "translations")
	nxenv.create_folder(tpath)
	write_csv_file(os.path.join(tpath, lang + ".csv"), app_messages, full_dict or get_all_translations(lang))


def send_translations(translation_dict):
	"""Append translated dict in `nxenv.local.response`"""
	if "__messages" not in nxenv.local.response:
		nxenv.local.response["__messages"] = {}

	nxenv.local.response["__messages"].update(translation_dict)
...
def update_translations_for_source(source=None, translation_dict=None):
	...
	translation_records = nxenv.db.get_values(
...
def get_all_languages(with_language_name: bool = False) -> list:
	...
	def get_all_language_with_name():
		return nxenv.get_all("Language", ["language_code", "language_name"], {"enabled": 1})

	if with_language_name:
		return nxenv.cache.get_value("languages_with_name", get_all_language_with_name)
	else:
		languages = nxenv.client_cache.get_value("languages")
		if not languages:
			languages = nxenv.get_all("Language", filters={"enabled": 1}, pluck="name")
			nxenv.client_cache.set_value("languages", languages)
		return languages


def get_preferred_language_cookie():
	return nxenv.request.cookies.get("preferred_language")


def get_translated_doctypes():
	dts = nxenv.get_all("DocType", {"translated_doctype": 1}, pluck="name")
...
def print_language(language: str):
	...
	with print_language("de"):
	    html = nxenv.get_print(...)
	```
	"""
	if not language or language == nxenv.local.lang:
		# do nothing
		yield
		return

	# remember original values
	_lang = nxenv.local.lang
	_jenv = nxenv.local.jenv

	# set language, empty any existing lang_full_dict and jenv
	nxenv.local.lang = language
	nxenv.local.jenv = None

	yield

	# restore original values
	nxenv.local.lang = _lang
	nxenv.local.jenv = _jenv
