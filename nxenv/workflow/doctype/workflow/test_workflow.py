...
class TestWorkflow(IntegrationTestCase):
	...
	def setUp(self):
		self.patcher = patch("nxenv.attach_print", return_value={})
		self.patcher.start()
		nxenv.db.delete("Workflow Action")
		self.workflow = create_todo_workflow()

	def tearDown(self):
		nxenv.set_user("Administrator")
		self.patcher.stop()
		nxenv.delete_doc("Workflow", "Test ToDo")
	...
	def test_if_workflow_actions_were_processed_using_role(self):
		...
		workflow_actions = nxenv.get_all("Workflow Action", fields=["*"])
...
def create_todo_workflow():
	from nxenv.tests.ui_test_helpers import UI_TEST_USER

	if nxenv.db.exists("Workflow", "Test ToDo"):
		nxenv.delete_doc("Workflow", "Test ToDo")

	TEST_ROLE = "Test Approver"

	if not nxenv.db.exists("Role", TEST_ROLE):
		nxenv.get_doc(doctype="Role", role_name=TEST_ROLE).insert(ignore_if_duplicate=True)
		if nxenv.db.exists("User", UI_TEST_USER):
			nxenv.get_doc("User", UI_TEST_USER).add_roles(TEST_ROLE)

	workflow = nxenv.new_doc("Workflow")
...
def create_new_todo():
	return nxenv.get_doc(doctype="ToDo", description="workflow " + random_string(10)).insert()
