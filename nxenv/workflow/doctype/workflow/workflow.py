...
class Workflow(Document):
	...
	if TYPE_CHECKING:
		...
		from nxenv.workflow.doctype.workflow_document_state.workflow_document_state import (
			WorkflowDocumentState,
		)
		from nxenv.workflow.doctype.workflow_transition.workflow_transition import WorkflowTransition
	...
	def on_update(self):
		nxenv.clear_cache(doctype=self.document_type)

	def create_custom_field_for_workflow_state(self):
		nxenv.clear_cache(doctype=self.document_type)
		meta = nxenv.get_meta(self.document_type)
	...
	def validate_docstatus(self):
		def get_state(state):
			...
			nxenv.throw(nxenv._("{0} not a valid State").format(state))
		...
		for t in self.transitions:
			...
			if state.doc_status == "2":
				nxenv.throw(
					nxenv._("Cannot change state of Cancelled Document. Transition row {0}").format(t.idx)
				)
			...
			if state.doc_status == "0" and next_state.doc_status == "2":
				nxenv.throw(nxenv._("Cannot cancel before submitting. See Transition {0}").format(t.idx))
...
def get_workflow_state_count(doctype, workflow_state_field, states):
	nxenv.has_permission(doctype=doctype, ptype="read", throw=True)
	states = nxenv.parse_json(states)

	if workflow_state_field in nxenv.get_meta(doctype).get_valid_columns():
