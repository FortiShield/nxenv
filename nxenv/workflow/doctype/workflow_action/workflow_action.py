...
import nxenv
from nxenv import _
from nxenv.desk.form.utils import get_pdf_link
from nxenv.desk.notifications import clear_doctype_notifications
from nxenv.email.doctype.email_template.email_template import get_email_template
from nxenv.model.document import Document
from nxenv.model.workflow import (
	apply_workflow,
	get_workflow_name,
...
from nxenv.utils import get_datetime, get_url
from nxenv.utils.background_jobs import enqueue
from nxenv.utils.data import get_link_to_form
from nxenv.utils.user import get_users_with_role
...
class WorkflowAction(Document):
	...
	if TYPE_CHECKING:
		...
		from nxenv.workflow.doctype.workflow_action_permitted_role.workflow_action_permitted_role import (
...
def on_doctype_update():
	...
	nxenv.db.add_index("Workflow Action", ["reference_name", "reference_doctype", "status"])
...
def has_permission(doc, user):
	...
	return not permitted_roles.isdisjoint(nxenv.get_roles(user))
...
def confirm_action(doctype, docname, user, action):
	...
	logged_in_user = nxenv.session.user
...
def return_success_page(doc):
	nxenv.respond_as_web_page(
		...
		_("{0}: {1} is set to state {2}").format(
			doc.get("doctype"), nxenv.bold(doc.get("name")), nxenv.bold(get_doc_workflow_state(doc))
		),
...
def return_link_expired_page(doc, doc_workflow_state):
	nxenv.respond_as_web_page(
		...
		_("Document {0} has been set to state {1} by {2}").format(
			nxenv.bold(doc.get("name")),
			nxenv.bold(doc_workflow_state),
			nxenv.bold(nxenv.get_value("User", doc.get("modified_by"), "full_name")),
		),
		indicator_color="blue",
	)
...
def get_allowed_roles(user, workflow, workflow_state):
	user = user if user else nxenv.session.user

	allowed_roles = nxenv.get_all(
		"Workflow Transition",
		fields="allowed",
	...
	user_roles = set(nxenv.get_roles(user))
...
def update_completed_workflow_actions_using_role(user=None, workflow_action=None):
	...
	(
		nxenv.qb.update(WorkflowAction)
	...
	for transition in transitions:
		...
		for user in filtered_users:
			if not user_data_map.get(user):
				user_data_map[user] = nxenv._dict(
					{
						"possible_actions": [],
						"email": nxenv.db.get_value("User", user, "email"),
					}
				)
...
def get_workflow_action_url(action, doc, user):
	apply_action_method = "/api/method/nxenv.workflow.doctype.workflow_action.workflow_action.apply_action"
...
def get_confirm_workflow_action_url(doc, action, user):
	confirm_action_method = (
		"/api/method/nxenv.workflow.doctype.workflow_action.workflow_action.confirm_action"
	)
...
def get_common_email_args(doc):
	...
	lang = doc.get("language") or (
		nxenv.get_cached_value("Print Format", print_format, "default_print_language")
		if print_format
		else None
	)
...
def get_email_template_from_workflow(doc):
	...
	template_name = nxenv.db.get_value(
