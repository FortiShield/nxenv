"""
NxenvClient is a library that helps you connect with other nxenv systems
"""
...
class NxenvException(Exception):
	pass
...
class NxenvClient:
	def __init__(
		...
		api_key=None,
		api_secret=None,
		nxenv_authorization_source=None,
	):
		import requests
		...
		self.nxenv_authorization_source = nxenv_authorization_source
	...
	def setup_key_authentication_headers(self):
		if self.api_key and self.api_secret:
			...
			if self.nxenv_authorization_source:
				auth_source = {"Nxenv-Authorization-Source": self.nxenv_authorization_source}
	...
	def insert(self, doc):
		...
		res = self.session.post(
			...
			data={"data": nxenv.as_json(doc)},
			verify=self.verify,
			headers=self.headers,
		)
		return nxenv._dict(self.post_process(res))
	...
	def insert_many(self, docs):
		...
		return self.post_request({"cmd": "nxenv.client.insert_many", "docs": nxenv.as_json(docs)})
	...
	def update(self, doc):
		...
		res = self.session.put(
			url, data={"data": nxenv.as_json(doc)}, verify=self.verify, headers=self.headers
		)
		return nxenv._dict(self.post_process(res))
	...
	def bulk_update(self, docs):
		...
		return self.post_request({"cmd": "nxenv.client.bulk_update", "docs": nxenv.as_json(docs)})
	...
	def delete(self, doctype, name):
		...
		return self.post_request({"cmd": "nxenv.client.delete", "doctype": doctype, "name": name})

	def submit(self, doc):
		"""Submit remote document

		:param doc: dict or Document object to be submitted remotely"""
		return self.post_request({"cmd": "nxenv.client.submit", "doc": nxenv.as_json(doc)})
	...
	def get_value(self, doctype, fieldname=None, filters=None):
		...
		return self.get_request(
			{
				"cmd": "nxenv.client.get_value",
				"doctype": doctype,
				"fieldname": fieldname or "name",
				"filters": nxenv.as_json(filters),
			}
		)
	...
	def cancel(self, doctype, name):
		...
		return self.post_request({"cmd": "nxenv.client.cancel", "doctype": doctype, "name": name})
	...
	def migrate_doctype(self, doctype, filters=None, update=None, verbose=1, exclude=None, preprocess=None):
		...
		for doc in docs:
			...
			if doctype != "User" and not nxenv.db.exists("User", doc.get("owner")):
				nxenv.get_doc(
					{
						"doctype": "User",
						"email": doc.get("owner"),
	...
	def migrate_single(self, doctype):
		...
		doc.modified = nxenv.db.get_single_value(doctype, "modified")
	...
	def post_process(self, response):
		...
		if rjson and ("exc" in rjson) and rjson["exc"]:
			try:
				...
				exc = "NxenvClient Request Failed\n\n" + exc
			except Exception:
				exc = rjson["exc"]

			raise NxenvException(exc)
		if "message" in rjson:
...
class NxenvOAuth2Client(NxenvClient):
