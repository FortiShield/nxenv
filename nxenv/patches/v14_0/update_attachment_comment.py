...
def execute():
	...
	for name, content in nxenv.get_all(
		...
		nxenv.db.set_value("Comment", name, "content", content, update_modified=False)
	...
	for name, content in nxenv.get_all(
		...
		if content and content.startswith("Removed "):
			nxenv.db.set_value("Comment", name, "content", content[8:], update_modified=False)
