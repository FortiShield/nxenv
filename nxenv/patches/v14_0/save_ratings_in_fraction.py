...
def execute():
	...
	rating_fields = nxenv.get_all(
		"DocField", fields=["parent", "fieldname"], filters={"fieldtype": "Rating"}
	)

	custom_rating_fields = nxenv.get_all(
	...
	for _field in rating_fields + custom_rating_fields:
		...
		if (
			...
			and nxenv.db.get_column_type(doctype_name, field) == RATING_FIELD_TYPE
		):
			continue
		...
		nxenv.db.change_column_type(doctype_name, column=field, type=RATING_FIELD_TYPE, nullable=True)

		# update data: int => decimal
		nxenv.qb.update(doctype).set(doctype[field], doctype[field] / 5).run()
