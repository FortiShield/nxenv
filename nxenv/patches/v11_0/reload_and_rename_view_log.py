...
def execute():
	if nxenv.db.table_exists("View log"):
		...
		nxenv.db.sql("CREATE TABLE `ViewLogTemp` AS SELECT * FROM `tabView log`")

		# deleting old View log table
		nxenv.db.sql("DROP table `tabView log`")
		nxenv.delete_doc("DocType", "View log")

		# reloading view log doctype to create `tabView Log` table
		nxenv.reload_doc("core", "doctype", "view_log")

		# Move the data to newly created `tabView Log` table
		nxenv.db.sql("INSERT INTO `tabView Log` SELECT * FROM `ViewLogTemp`")
		nxenv.db.commit()

		# Delete temporary table
		nxenv.db.sql("DROP table `ViewLogTemp`")
	else:
		nxenv.reload_doc("core", "doctype", "view_log")
