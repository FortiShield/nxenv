...
def execute():
	custom_doctypes = nxenv.get_all("DocType", filters={"custom": 1})
	...
	for doctype in custom_doctypes:
		...
		custom_fields = nxenv.get_all("Custom Field", filters={"dt": doctype.name}, fields=["*"])

		property_setter_map = {}

		for prop in property_setters:
			property_setter_map[prop.field_name] = prop
			nxenv.db.delete("Property Setter", {"name": prop.name})

		meta = nxenv.get_meta(doctype.name)
		...
		for cf in custom_fields:
			...
			if field:
				...
				df = nxenv.new_doc("DocField", parent_doc=meta, parentfield="fields")
				df.update(cf)
				meta.fields.append(df)
			nxenv.db.delete("Custom Field", {"name": cf.name})
