...
def execute():
	nxenv.reload_doctype("Event")
	# Rename "Cancel" to "Cancelled"
	nxenv.db.sql("""UPDATE tabEvent set event_type='Cancelled' where event_type='Cancel'""")
	...
	for event in events:
		if event.ref_type and event.ref_name:
			try:
				e = nxenv.get_doc("Event", event.name)
				e.append(
					"event_participants",
				...
				nxenv.log_error(nxenv.get_traceback())
