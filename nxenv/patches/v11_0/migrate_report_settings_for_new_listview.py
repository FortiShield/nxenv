...
def execute():
	...
	reports = nxenv.get_all("Report", {"report_type": "Report Builder"})

	for report_name in reports:
		settings = nxenv.db.get_value("Report", report_name, "json")
		if not settings:
			continue

		settings = nxenv._dict(json.loads(settings))
		...
		nxenv.db.set_value("Report", report_name, "json", json.dumps(settings))
