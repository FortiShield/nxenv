...
from nxenv.utils.dashboard import get_dashboards_with_link


def execute():
	if (
		not nxenv.db.table_exists("Dashboard Chart")
		or not nxenv.db.table_exists("Number Card")
		or not nxenv.db.table_exists("Dashboard")
	):
		return

	nxenv.reload_doc("desk", "doctype", "dashboard_chart")
	nxenv.reload_doc("desk", "doctype", "number_card")
	nxenv.reload_doc("desk", "doctype", "dashboard")
...
def get_modified_docs(doctype):
	return nxenv.get_all(doctype, filters={"owner": "Administrator", "modified_by": ["!=", "Administrator"]})


def rename_modified_doc(docname, doctype):
	new_name = docname + " Custom"
	try:
		nxenv.rename_doc(doctype, docname, new_name)
	except nxenv.ValidationError:
