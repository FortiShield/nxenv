...
def execute():
	...
	nxenv.reload_doc("website", "doctype", "color")
	nxenv.reload_doc("website", "doctype", "website_theme", force=True)

	for theme in nxenv.get_all("Website Theme"):
...
def setup_color_record(doc):
	...
	for color_field in color_fields:
		...
		if not color_code or nxenv.db.exists("Color", color_code):
			continue
