import nxenv
from nxenv.model.naming import is_autoincremented
...
def execute():
	...
	for doctype in possible_log_types:
		if (
			nxenv.db.exists("DocType", doctype)
			and _is_implicit_int_pk(doctype)
			and not is_autoincremented(doctype)
		):
			nxenv.db.change_column_type(
				doctype,
				"name",
				type=f"varchar({nxenv.db.VARCHAR_LEN})",
				nullable=True,
			)
...
def _is_implicit_int_pk(doctype: str) -> bool:
	...
	if nxenv.db.db_type == "mariadb":
		...
		values = (nxenv.db.cur_db_name,)

	col_type = nxenv.db.sql(query, values)
