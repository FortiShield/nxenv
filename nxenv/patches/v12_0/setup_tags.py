...
def execute():
	nxenv.delete_doc_if_exists("DocType", "Tag Category")
	nxenv.delete_doc_if_exists("DocType", "Tag Doc Category")

	nxenv.reload_doc("desk", "doctype", "tag")
	nxenv.reload_doc("desk", "doctype", "tag_link")

	tag_list = []
	tag_links = []
	time = nxenv.utils.get_datetime()

	for doctype in nxenv.get_list("DocType", filters={"istable": 0, "issingle": 0, "is_virtual": 0}):
		if not nxenv.db.count(doctype.name) or not nxenv.db.has_column(doctype.name, "_user_tags"):
			continue

		for _user_tags in nxenv.db.sql(
			...
			for tag in _user_tags.get("_user_tags").split(",") if _user_tags.get("_user_tags") else []:
				...
				tag_link_name = nxenv.generate_hash(length=10)
				tag_links.append(
