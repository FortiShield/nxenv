...
def execute():
	...
	all_tables = nxenv.db.get_tables()
	final_deletion_map = nxenv._dict()

	for table in all_tables:
		indexes_to_keep_map = nxenv._dict()
		indexes_to_delete = []
		index_info = nxenv.db.sql(
		...
		for index in index_list:
			try:
				if is_clustered_index(table_name, index):
					continue
				nxenv.db.sql_ddl(f"ALTER TABLE `{table_name}` DROP INDEX `{index}`")
