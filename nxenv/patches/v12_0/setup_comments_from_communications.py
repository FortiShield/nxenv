...
def execute():
	...
	if nxenv.db.count("Communication", filters=dict(communication_type="Comment")) > 20000:
		nxenv.db.auto_commit_on_many_writes = True

	for comment in nxenv.get_all("Communication", fields=["*"], filters=dict(communication_type="Comment")):
	...
	# clean up
	nxenv.db.delete("Communication", {"communication_type": "Comment"})
