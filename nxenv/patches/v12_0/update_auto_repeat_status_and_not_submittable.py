...
def execute():
	...
	nxenv.reload_doc("automation", "doctype", "Auto Repeat")
	nxenv.db.sql("update `tabDocPerm` set submit=0, cancel=0, amend=0 where parent='Auto Repeat'")
	nxenv.db.sql("update `tabAuto Repeat` set docstatus=0 where docstatus=1 or docstatus=2")

	for entry in nxenv.get_all("Auto Repeat"):
		doc = nxenv.get_doc("Auto Repeat", entry.name)

		# create custom field for allow auto repeat
		fields = nxenv.get_meta(doc.reference_doctype).fields
