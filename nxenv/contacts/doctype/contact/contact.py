...
import nxenv
from nxenv import _
from nxenv.contacts.address_and_contact import set_link_title
from nxenv.core.doctype.access_log.access_log import make_access_log
from nxenv.core.doctype.dynamic_link.dynamic_link import deduplicate_dynamic_links
from nxenv.model.document import Document
from nxenv.model.naming import append_number_if_name_exists
from nxenv.utils import cstr, has_gravatar
...
class Contact(Document):
	...
	if TYPE_CHECKING:
		from nxenv.contacts.doctype.contact_email.contact_email import ContactEmail
		from nxenv.contacts.doctype.contact_phone.contact_phone import ContactPhone
		from nxenv.core.doctype.dynamic_link.dynamic_link import DynamicLink
	...
	def autoname(self):
		...
		if nxenv.db.exists("Contact", self.name):
	...
	def validate(self):
		...
		if self.get("sync_with_google_contacts") and not self.get("google_contacts"):
			nxenv.throw(_("Select Google Contacts to which contact should be synced."))

		deduplicate_dynamic_links(self)

	def set_user(self):
		if not self.user and self.email_id:
			self.user = nxenv.db.get_value("User", {"email": self.email_id})
	...
	def add_email(self, email_id, is_primary=0, autosave=False):
		if not nxenv.db.exists("Contact Email", {"email_id": email_id, "parent": self.name}):
	...
	def add_phone(self, phone, is_primary_phone=0, is_primary_mobile_no=0, autosave=False):
		if not nxenv.db.exists("Contact Phone", {"phone": phone, "parent": self.name}):
			self.append(
				"phone_nos",
				{
					"phone": phone,
	...
	def set_primary_email(self):
		...
		if len([email.email_id for email in self.email_ids if email.is_primary]) > 1:
			nxenv.throw(_("Only one {0} can be set as primary.").format(nxenv.bold(_("Email ID"))))
	...
	def set_primary(self, fieldname):
		...
		if len(is_primary) > 1:
			nxenv.throw(
				_("Only one {0} can be set as primary.").format(nxenv.bold(nxenv.unscrub(fieldname)))
			)
...
def download_vcard(contact: str):
	...
	contact = nxenv.get_doc("Contact", contact)
	contact.check_permission()

	vcard = contact.get_vcard()
	make_access_log(doctype="Contact", document=contact.name, file_type="vcf")

	nxenv.response["filename"] = f"{contact.name}.vcf"
	nxenv.response["filecontent"] = vcard.serialize().encode("utf-8")
	nxenv.response["type"] = "binary"
...
def download_vcards(contacts: str):
	...
	from nxenv.utils.data import now

	contact_ids = nxenv.parse_json(contacts)

	vcards = []
	for contact_id in contact_ids:
		contact = nxenv.get_doc("Contact", contact_id)
		contact.check_permission()
		vcard = contact.get_vcard()
	...
	nxenv.response["filename"] = f"{timestamp} Contacts.vcf"
	nxenv.response["filecontent"] = "\n".join(vcards).encode("utf-8")
	nxenv.response["type"] = "binary"
...
def invite_user(contact: str):
	...
	if not contact.email_id:
		nxenv.throw(_("Please set Email Address"))
...
def update_contact(doc, method):
	...
	contact_name = nxenv.db.get_value("Contact", {"email_id": doc.name})
	if contact_name:
...
def contact_query(doctype, txt, searchfield, start, page_len, filters):
	from nxenv.desk.reportview import get_match_cond

	doctype = "Contact"
	if not nxenv.get_meta(doctype).get_field(searchfield) and searchfield not in nxenv.db.DEFAULT_COLUMNS:
		return []
...
def get_contacts_linked_from(doctype, docname, fields=None):
	...
	link_fields = nxenv.get_meta(doctype).get("fields", {"fieldtype": "Link", "options": "Contact"})
	if not link_fields:
		return []

	contact_names = nxenv.get_value(doctype, docname, fieldname=[f.fieldname for f in link_fields])
	if not contact_names:
		return []

	return nxenv.get_list("Contact", fields=fields, filters={"name": ("in", contact_names)})
...
def get_contact_display_list(doctype: str, name: str) -> list[dict]:
	from nxenv.contacts.doctype.address.address import get_condensed_address

	if not nxenv.has_permission("Contact", "read"):
		return []

	contact_list = nxenv.get_list(
		"Contact",
		filters=[
			["Dynamic Link", "link_doctype", "=", doctype],
	...
	for contact in contact_list:
		contact["email_ids"] = nxenv.get_all(
			"Contact Email",
			filters={"parenttype": "Contact", "parent": contact.name, "is_primary": 0},
			fields=["email_id"],
		)

		contact["phone_nos"] = nxenv.get_all(
			"Contact Phone",
			filters={
				"parenttype": "Contact",
				"parent": contact.name,
		...
		if contact.address and nxenv.has_permission("Address", "read"):
			address = nxenv.get_doc("Address", contact.address)
