...
import nxenv
from nxenv.contacts.doctype.address.address import address_query, get_address_display
...
class TestAddress(IntegrationTestCase):
	def test_template_works(self):
		if not nxenv.db.exists("Address Template", "India"):
			nxenv.get_doc({"doctype": "Address Template", "country": "India", "is_default": 1}).insert()

		if not nxenv.db.exists("Address", "_Test Address-Office"):
			nxenv.get_doc(
				{
		...
		address = nxenv.get_list("Address")[0].name
		display = get_address_display(nxenv.get_doc("Address", address).as_dict())
