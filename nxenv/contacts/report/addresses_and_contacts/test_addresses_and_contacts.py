...
from nxenv.contacts.report.addresses_and_contacts.addresses_and_contacts import get_data
from nxenv.tests import IntegrationTestCase


def get_custom_linked_doctype():
	if bool(nxenv.get_all("DocType", filters={"name": "Test Custom Doctype"})):
		return
...
def create_linked_address(link_list):
	if nxenv.flags.test_address_created:
		return

	address = nxenv.get_doc(
		{
			"doctype": "Address",
	...
	address.insert()
	nxenv.flags.test_address_created = True

	return address.name


def create_linked_contact(link_list, address):
	if nxenv.flags.test_contact_created:
		return
	...
	nxenv.flags.test_contact_created = True
