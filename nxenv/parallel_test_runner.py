...
class ParallelTestRunner:
	def __init__(self, app, site, build_number=1, total_builds=1, dry_run=False):
		self.app = app
		self.site = site
		self.build_number = nxenv.utils.cint(build_number) or 1
		self.total_builds = nxenv.utils.cint(total_builds)
	...
	def before_test_setup(self):
		...
		for fn in nxenv.get_hooks("before_tests", app_name=self.app):
			nxenv.get_attr(fn)()

		test_module = nxenv.get_module(f"{self.app}.tests")
	...
	def run_tests_for_file(self, file_info):
		...
		from nxenv.deprecation_dumpster import compat_preload_test_records_upfront
	...
	def get_module(self, path, filename):
		app_path = nxenv.get_app_path(self.app)
...
class ParallelTestWithOrchestrator(ParallelTestRunner):
	...
	def __init__(self, app, site):
		...
		if not self.orchestrator_url:
			...
			click.echo("Pass public URL after hosting https://github.com/nxenv/test-orchestrator")
			sys.exit(1)

		self.ci_build_id = os.environ.get("CI_BUILD_ID")
		self.ci_instance_id = os.environ.get("CI_INSTANCE_ID") or nxenv.generate_hash(length=10)
