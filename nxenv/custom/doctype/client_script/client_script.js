...
nxenv.ui.form.on("Client Script", {
	setup(frm) {
	...
	refresh(frm) {
		if (frm.doc.dt && frm.doc.script) {
			frm.add_custom_button(__("Go to {0}", [frm.doc.dt]), () =>
				nxenv.set_route("List", frm.doc.dt, "List")
			);
		}

		if (frm.doc.view == "Form") {
			frm.add_custom_button(__("Add script for Child Table"), () => {
				nxenv.model.with_doctype(frm.doc.dt, () => {
					const child_tables = nxenv.meta
			...
			if (!frm.is_new()) {
				frm.add_custom_button(__("Compare Versions"), () => {
					new nxenv.ui.DiffView("Client Script", "script", frm.doc.name);
				});
			}
		}
	...
	dt(frm) {
		frm.toggle_display("view", !nxenv.boot.single_types.includes(frm.doc.dt));
	...
	view(frm) {
		let has_form_boilerplate = frm.doc.script.includes("nxenv.ui.form.on");
...
nxenv.ui.form.on('Task',  'validate',  function(frm) {
...
nxenv.ui.form.on('Sales Invoice',  {
    validate: function(frm) {
