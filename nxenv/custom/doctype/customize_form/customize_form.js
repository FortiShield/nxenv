...
nxenv.provide("nxenv.customize_form");

nxenv.ui.form.on("Customize Form", {
	setup: function (frm) {
		...
		frm.set_query("doc_type", function () {
			return {
				filters: [
					...
					["DocType", "restrict_to_domain", "in", nxenv.boot.active_domains],
				],
			};
		});
	...
	refresh: function (frm) {
		...
		if (frm.doc.doc_type) {
			nxenv.model.with_doctype(frm.doc.doc_type).then(() => {
				frm.page.set_title(__("Customize Form - {0}", [__(frm.doc.doc_type)]));
				nxenv.customize_form.set_primary_action(frm);

				frm.add_custom_button(
					__("Go to {0} List", [__(frm.doc.doc_type)]),
					function () {
						nxenv.set_route("List", frm.doc.doc_type);
					},
					__("Actions")
				);
				...
				frm.add_custom_button(
					__("Set Permissions"),
					function () {
						nxenv.set_route("permission-manager", frm.doc.doc_type);
					},
					__("Actions")
				);
				...
				frm.add_custom_button(
					...
					function () {
						nxenv.customize_form.confirm(__("Remove all customizations?"), frm);
					},
					__("Actions")
				);
				...
				frm.toggle_display(
					["queue_in_background"],
					nxenv.get_meta(frm.doc.doc_type).is_submittable || 0
				);
	...
	set_default_doc_type(frm) {
		let doc_type;
		if (nxenv.route_options && nxenv.route_options.doc_type) {
			doc_type = nxenv.route_options.doc_type;
				...
				return frm.call({
					...
					callback: function (r) {
						if (!r.exc) {
							...
							nxenv.customize_form.clear_locals_and_refresh(frm);
						}
					},
				});
			}
	...
	async trim_table(frm) {
		let dropped_columns = await nxenv.xcall(
			"nxenv.custom.doctype.customize_form.customize_form.get_orphaned_columns",
			{ doctype: frm.doc.doc_type }
		);

		if (!dropped_columns?.length) {
			nxenv.toast(__("This doctype has no orphan fields to trim"));
			return;
		}
		let msg = __(
			...
			frm.add_custom_button(
				...
				function () {
					nxenv.prompt(
						...
						function (data) {
							nxenv.call({
								method: "nxenv.modules.utils.export_customizations",
								args: {
									doctype: frm.doc.doc_type,
									module: data.module,
	...
	setup_sort_order(frm) {
		...
		if (frm.doc.doc_type) {
			var fields = $.map(frm.doc.fields, function (df) {
				return nxenv.model.is_value_type(df.fieldtype) ? df.fieldname : null;
			});
	...
	setup_default_views(frm) {
		nxenv.model.set_default_views_for_doctype(frm.doc.doc_type, frm);
	},
});

// can't delete standard fields
nxenv.ui.form.on("Customize Form Field", {
	before_fields_remove: function (frm, doctype, name) {
		const row = nxenv.get_doc(doctype, name);
	...
	fields_add: function (frm, cdt, cdn) {
		var f = nxenv.model.get_doc(cdt, cdn);
...
nxenv.ui.form.on("DocType Link", {
	before_links_remove: function (frm, doctype, name) {
		let row = nxenv.get_doc(doctype, name);
		...
		if (!(row.custom || row.__islocal)) {
			nxenv.msgprint(__("Cannot delete standard link. You can hide it if you want"));
			throw "cannot delete standard link";
		}
	},
	links_add: function (frm, cdt, cdn) {
		let f = nxenv.model.get_doc(cdt, cdn);
		f.custom = 1;
	},
...
nxenv.ui.form.on("DocType Action", {
	before_actions_remove: function (frm, doctype, name) {
		...
		if (!(row.custom || row.__islocal)) {
			nxenv.msgprint(__("Cannot delete standard action. You can hide it if you want"));
...
nxenv.ui.form.on("DocType State", {
	before_states_remove: function (frm, doctype, name) {
		...
		if (!(row.custom || row.__islocal)) {
			nxenv.msgprint(__("Cannot delete standard document state."));
...
nxenv.customize_form.save_customization = function (frm) {
	if (frm.doc.doc_type) {
		return frm.call({
			doc: frm.doc,
			freeze: true,
...
nxenv.customize_form.update_fields_from_form_builder = function (frm) {
	let form_builder = nxenv.form_builder;
...
nxenv.customize_form.set_primary_action = function (frm) {
...
nxenv.customize_form.confirm = function (msg, frm) {
	...
	nxenv.customize_form.confirm.dialog = d;
	d.show();
};

nxenv.customize_form.clear_locals_and_refresh = function (frm) {
	delete frm.doc.__unsaved;
	// clear doctype from locals
	nxenv.model.clear_doc("DocType", frm.doc.doc_type);
	delete nxenv.meta.docfield_copy[frm.doc.doc_type];
	frm.refresh();
};

function render_form_builder(frm) {
	if (nxenv.form_builder && nxenv.form_builder.doctype === frm.doc.doc_type) {
		nxenv.form_builder.setup_page_actions();
		nxenv.form_builder.store.fetch();
		return;
	}

	if (nxenv.form_builder) {
		nxenv.form_builder.wrapper = $(frm.fields_dict["form_builder"].wrapper);
		nxenv.form_builder.frm = frm;
		nxenv.form_builder.doctype = frm.doc.doc_type;
		nxenv.form_builder.customize = true;
		nxenv.form_builder.init(true);
		nxenv.form_builder.store.fetch();
	} else {
		nxenv.require("form_builder.bundle.js").then(() => {
			nxenv.form_builder = new nxenv.ui.FormBuilder({
...
extend_cscript(cur_frm.cscript, new nxenv.model.DocTypeController({ frm: cur_frm }));
