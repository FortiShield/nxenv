...
nxenv.ui.form.on("Custom Field", {
	...
	dt: function (frm) {
		...
		return nxenv.call({
			method: "nxenv.custom.doctype.custom_field.custom_field.get_fields_label",
	...
	label: function (frm) {
		if (frm.doc.label && nxenv.utils.has_special_chars(frm.doc.label)) {
			...
			frm.add_custom_button(__("Rename Fieldname"), () => {
				nxenv.prompt(
					...
					function (data) {
						nxenv
							.xcall(
								"nxenv.custom.doctype.custom_field.custom_field.rename_fieldname",
								{
...
nxenv.utils.has_special_chars = function (t) {
	var iChars = "!@#$%^&*()+=-[]\\';,./{}|\":<>?";
