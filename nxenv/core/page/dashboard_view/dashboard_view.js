...
nxenv.pages["dashboard-view"].on_page_load = function (wrapper) {
	...
	nxenv.dashboard = new Dashboard(wrapper);
	$(wrapper).bind("show", function () {
		nxenv.dashboard.show();
	});
};
...
class Dashboard {
	...
	show() {
		...
		if (this.route.length > 1) {
			...
			// last opened
			if (nxenv.last_dashboard) {
				nxenv.set_re_route("dashboard-view", nxenv.last_dashboard);
			} else {
				// default dashboard
				nxenv.db.get_list("Dashboard", { filters: { is_default: 1 } }).then((data) => {
					if (data && data.length) {
						nxenv.set_re_route("dashboard-view", data[0].name);
					} else {
						// no default, get the latest one
						nxenv.db.get_list("Dashboard", { limit: 1 }).then((data) => {
							if (data && data.length) {
								nxenv.set_re_route("dashboard-view", data[0].name);
							} else {
								// create a new dashboard!
								nxenv.new_doc("Dashboard");
							}
						});
					}
				});
	...
	show_dashboard(current_dashboard_name) {
		...
		this.charts = {};
		nxenv.last_dashboard = current_dashboard_name;
	}

	set_breadcrumbs() {
		nxenv.breadcrumbs.add("Desk", "Dashboard");
	}

	refresh() {
		nxenv.run_serially([() => this.render_cards(), () => this.render_charts()]);
	}

	render_charts() {
		return this.get_permitted_items(
			"nxenv.desk.doctype.dashboard.dashboard.get_permitted_charts"
		).then((charts) => {
			if (!charts.length) {
				nxenv.msgprint(
			...
			nxenv.dashboard_utils.get_dashboard_settings().then((settings) => {
				...
				this.chart_group = new nxenv.widget.WidgetGroup({
					title: null,
	...
	render_cards() {
		return this.get_permitted_items(
			"nxenv.desk.doctype.dashboard.dashboard.get_permitted_cards"
		).then((cards) => {
			if (!cards.length) {
				return;
			}
			...
			this.number_card_group = new nxenv.widget.WidgetGroup({
				container: this.container,
				type: "number_card",
				columns: 3,
				options: {
	...
	set_dropdown() {
		...
		this.page.add_menu_item(__("Edit"), () => {
			nxenv.set_route("Form", "Dashboard", nxenv.dashboard.dashboard_name);
		});
		...
		nxenv.db.get_list("Dashboard").then((dashboards) => {
			dashboards.map((dashboard) => {
				let name = dashboard.name;
				if (name != this.dashboard_name) {
					this.page.add_menu_item(
						name,
						() => nxenv.set_route("dashboard-view", name),
						1
					);
				}
			});
