nxenv.pages["permission-manager"].on_page_load = (wrapper) => {
	let page = nxenv.ui.make_app_page({
		parent: wrapper,
	...
	$(nxenv.render_template("permission_manager_help", {})).appendTo(page.main);
	wrapper.permission_engine = new nxenv.PermissionEngine(wrapper);
};

nxenv.pages["permission-manager"].refresh = function (wrapper) {
	wrapper.permission_engine.set_from_route();
};

nxenv.PermissionEngine = class PermissionEngine {
	constructor(wrapper) {
	...
	setup_page() {
		this.doctype_select = this.wrapper.page.add_field({
			...
			options: "DocType",
			change: function () {
				nxenv.set_route("permission-manager", this.get_value());
			},
		});
		...
		this.page.add_inner_button(__("Set User Permissions"), () => {
			return nxenv.set_route("List", "User Permission");
		});
		this.set_from_route();
	}
	...
	set_from_route() {
		...
		if (nxenv.get_route()[1]) {
			this.doctype_select.set_value(nxenv.get_route()[1]);
		} else if (nxenv.route_options) {
			if (nxenv.route_options.doctype) {
				this.doctype_select.set_value(nxenv.route_options.doctype);
			}
			if (nxenv.route_options.role) {
				this.role_select.set_value(nxenv.route_options.role);
			}
	...
	reset_std_permissions(data) {
		let doctype = this.get_doctype();
		let d = nxenv.confirm(__("Reset Permissions for {0}?", [__(doctype)]), () => {
			return nxenv
				.call({
					module: "nxenv.core",
					page: "permission_manager",
		...
		data.message.forEach((d) => {
			let rights = this.rights
				.filter((r) => d[r])
				.map((r) => {
					return __(toTitle(nxenv.unscrub(r)));
				});
	...
	add_delete_button(row, d) {
		$(
			`<button class='btn btn-danger btn-remove-perm btn-xs'>${nxenv.utils.icon(
				"delete"
			)}</button>`
		)
			...
			.on("click", () => {
				return nxenv.call({
					...
					callback: (r) => {
						if (r.exc) {
							nxenv.msgprint(__("Did not remove"));
						} else {
							this.refresh();
						}
					},
	...
	add_check_events() {
		...
		this.body.on("click", ".show-user-permissions", () => {
			nxenv.route_options = { allow: this.get_doctype() || "" };
	...
	get_link_fields(doctype) {
		return nxenv.get_children("DocType", doctype, "fields", {
			fieldtype: "Link",
