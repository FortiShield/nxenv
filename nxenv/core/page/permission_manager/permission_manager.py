...
from nxenv.modules.import_file import get_file_path, read_doc_from_file
...
from nxenv.utils.user import get_users_with_role as _get_user_with_role
...
def get_roles_and_doctypes():
	...
	if nxenv.session.user != "Administrator":
		custom_user_type_roles = nxenv.get_all("User Type", filters={"is_standard": 0}, fields=["role"])
...
def get_permissions(doctype: str | None = None, role: str | None = None):
	...
	if role:
		...
		if nxenv.session.user != "Administrator":
			custom_roles = nxenv.get_all("Role", filters={"is_custom": 1}, pluck="name")
			filters["role"] = ["not in", custom_roles]

		out = nxenv.get_all("Custom DocPerm", fields="*", filters=filters, order_by="permlevel")
		if not out:
			out = nxenv.get_all("DocPerm", fields="*", filters=filters, order_by="permlevel")
	...
	for d in out:
		...
		if meta := nxenv.get_meta(d.parent):
...
def update(doctype: str, role: str, permlevel: int, ptype: str, value=None, if_owner=0) -> str | None:
	...
	if ptype == "report" and value == "1" and if_owner == "1":
		nxenv.throw(_("Cannot set 'Report' permission if 'Only If Creator' permission is set"))
	...
	nxenv.db.after_commit.add(clear_cache)
...
def remove(doctype, role, permlevel, if_owner=0):
	...
	custom_docperms = nxenv.db.get_values(
		"Custom DocPerm", {"parent": doctype, "role": role, "permlevel": permlevel, "if_owner": if_owner}
	)
	for name in custom_docperms:
		nxenv.delete_doc("Custom DocPerm", name, ignore_permissions=True, force=True)

	if not nxenv.get_all("Custom DocPerm", {"parent": doctype}):
		nxenv.throw(_("There must be atleast one permission rule."), title=_("Cannot Remove"))
