...
import nxenv
from nxenv import _
from nxenv.core.api.file import (
	create_new_folder,
	get_attached_images,
	get_files_in_folder,
	move_file,
...
from nxenv.core.doctype.file.exceptions import FileTypeNotAllowed
from nxenv.core.doctype.file.utils import get_corrupted_image_msg, get_extension
from nxenv.desk.form.utils import add_comment
from nxenv.exceptions import ValidationError
from nxenv.tests import IntegrationTestCase, UnitTestCase
from nxenv.utils import get_files_path, set_request
...
def make_test_doc(ignore_permissions=False):
	...
	d.description = "Test"
	d.assigned_by = nxenv.session.user
...
def make_test_image_file(private=False):
	file_path = nxenv.get_app_path("nxenv", "tests/data/sample_image_for_optimization.jpg")
	with open(file_path, "rb") as f:
		file_content = f.read()

	test_file = nxenv.get_doc(
		{
			"doctype": "File",
	...
	# remove those flags
	_test_file: File = nxenv.get_doc("File", test_file.name)
...
class TestSimpleFile(IntegrationTestCase):
	...
	def test_save(self):
		_file = nxenv.get_doc("File", {"file_url": self.saved_file_url})
...
class TestFSRollbacks(IntegrationTestCase):
	def test_rollback_from_file_system(self):
		file_name = content = nxenv.generate_hash()
		file = nxenv.new_doc("File", file_name=file_name, content=content).insert()
...
class TestExtensionValidations(IntegrationTestCase):
	...
	def test_allowed_extension(self):
		...
		file_name = content = nxenv.generate_hash()
		bad_file = nxenv.new_doc("File", file_name=f"{file_name}.png", content=content)
		self.assertRaises(FileTypeNotAllowed, bad_file.insert)

		bad_file = nxenv.new_doc("File", file_name=f"{file_name}.csv", content=content).insert()
		nxenv.db.rollback()
...
class TestBase64File(IntegrationTestCase):
	def setUp(self):
		...
		_file: nxenv.Document = nxenv.get_doc(
			{
				"doctype": "File",
	...
	def test_saved_content(self):
		_file: nxenv.Document = nxenv.get_doc("File", {"file_url": self.saved_file_url})
		content = _file.get_content()
...
class TestSameFileName(IntegrationTestCase):
	def test_saved_content(self):
		...
		_file = nxenv.get_doc("File", {"file_url": self.saved_file_url1})
		content1 = _file.get_content()
		self.assertEqual(content1, self.test_content1)
		_file = nxenv.get_doc("File", {"file_url": self.saved_file_url2})
	...
	def test_saved_content_private(self):
		...
		_file = nxenv.get_doc("File", {"file_url": _file1.file_url})
		self.assertEqual(_file.get_content(), test_content1)

		_file = nxenv.get_doc("File", {"file_url": _file2.file_url})
...
class TestSameContent(IntegrationTestCase):
	...
	def test_attachment_limit(self):
		...
		self.assertRaises(nxenv.exceptions.AttachmentLimitReached, file2.insert)
	...
	def test_utf8_bom_content_decoding(self):
		...
		_file.save()
		saved_file = nxenv.get_doc("File", _file.name)
...
class TestFile(IntegrationTestCase):
	...
	def tearDown(self):
		try:
			nxenv.get_doc("File", {"file_name": "file_copy.txt"}).delete()
		except nxenv.DoesNotExistError:
			pass

	def delete_test_data(self):
		test_file_data = nxenv.get_all(
			"File",
			pluck="name",
	...
	def tests_after_upload(self):
		...
		file_folder = nxenv.db.get_value("File", self.saved_name, "folder")
		self.assertEqual(file_folder, _("Home/Test Folder 1"))

	def test_file_copy(self):
		folder = self.get_folder("Test Folder 2", "Home")

		file = nxenv.get_doc("File", {"file_name": "file_copy.txt"})
	...
	def test_folder_copy(self):
		...
		file = nxenv.get_doc("File", {"file_name": "folder_copy.txt"})
		file_copy_txt = nxenv.get_value("File", {"file_name": "file_copy.txt"})
		if file_copy_txt:
			nxenv.get_doc("File", file_copy_txt).delete()

		self.assertEqual(_("Home/Test Folder 1/Test Folder 3"), file.folder)

	def test_default_folder(self):
		d = nxenv.get_doc({"doctype": "File", "file_name": _("Test_Folder"), "is_folder": 1})
		d.save()
	...
	def test_on_delete(self):
		...
		self.assertEqual(nxenv.db.get_value("File", _("Home/Test Folder 1"), "file_size"), 0)
		...
		folder = nxenv.get_doc("File", "Home/Test Folder 1/Test Folder 3")
	...
	def test_same_file_url_update(self):
		...
		file2 = nxenv.get_doc("File", file2.name)
	...
	def test_file_url_validation(self):
		test_file: File = nxenv.new_doc("File")
		test_file.update({"file_name": "logo", "file_url": "https://nxenv.io/files/nxenv.png"})
	...
	def test_make_thumbnail(self):
		# test web image
		test_file: File = nxenv.get_doc(
			{
				"doctype": "File",
				"file_name": "logo",
				"file_url": nxenv.utils.get_url("/_test/assets/image.jpg"),
			}
		...
		test_file = nxenv.get_doc(
			{
				"doctype": "File",
				"file_name": "logo",
				"file_url": nxenv.utils.get_url("/_test/assets/image"),
			}
		).insert(ignore_permissions=True)
		...
		test_file.db_set("thumbnail_url", None)
		test_file.reload()
		test_file.file_url = nxenv.utils.get_url("unknown.jpg")
		test_file.make_thumbnail(suffix="xs")
		self.assertEqual(
			nxenv.message_log[0].get("message"),
			f"File '{nxenv.utils.get_url('unknown.jpg')}' not found",
		)
		self.assertEqual(test_file.thumbnail_url, None)

	def test_file_unzip(self):
		file_path = nxenv.get_app_path("nxenv", "www/_test/assets/file.zip")
		public_file_path = nxenv.get_site_path("public", "files")
		try:
			import shutil
...
class TestAttachment(IntegrationTestCase):
	...
	def tearDownClass(cls):
		nxenv.db.rollback()
		nxenv.delete_doc("DocType", cls.test_doctype)

	def test_file_attachment_on_update(self):
		doc = nxenv.get_doc(doctype=self.test_doctype, title="test for attachment on update").insert()
...
class TestAttachmentsAccess(IntegrationTestCase):
	def setUp(self) -> None:
		nxenv.db.delete("File", {"is_folder": 0})
...
class TestFileUtils(IntegrationTestCase):
	def test_extract_images_from_doc(self):
		is_private = not nxenv.get_meta("ToDo").make_attachments_public
		...
		self.assertTrue(nxenv.db.exists("File", {"attached_to_name": todo.name, "is_private": is_private}))
		...
		filename = nxenv.db.exists("File", {"attached_to_name": todo.name})
		self.assertIn(f'<img src="{nxenv.get_doc("File", filename).file_url}', todo.description)
	...
	def test_extract_images_from_comment(self):
		...
		"""
		is_private = not nxenv.get_meta("ToDo").make_attachments_public
		test_doc = nxenv.get_doc(doctype="ToDo", description="comment test").insert()
		comment = add_comment(
			"ToDo",
			test_doc.name,
		...
		self.assertTrue(
			nxenv.db.exists("File", {"attached_to_name": test_doc.name, "is_private": is_private})
		)
	...
	def test_extract_images_from_communication(self):
		...
		"""
		is_private = not nxenv.get_meta("Communication").make_attachments_public
		communication = nxenv.get_doc(
		...
		self.assertTrue(
			nxenv.db.exists("File", {"attached_to_name": communication.name, "is_private": is_private})
		)
...
class TestFileOptimization(IntegrationTestCase):
	...
	def test_optimize_svg(self):
		file_path = nxenv.get_app_path("nxenv", "tests/data/sample_svg.svg")
		with open(file_path, "rb") as f:
			file_content = f.read()
		test_file = nxenv.get_doc(
	...
	def test_optimize_folder(self):
		test_folder = nxenv.get_doc("File", "Home/Attachments")
...
class TestGuestFileAndAttachments(IntegrationTestCase):
	...
	def tearDown(self) -> None:
		nxenv.set_user("Administrator")
		nxenv.db.rollback()
		nxenv.delete_doc("DocType", "Test For Attachment")
	...
	def test_attach_unattached_guest_file(self):
		...
		d = nxenv.new_doc("Test For Attachment")
	...
	def test_private_remains_private_even_if_same_hash(self):
		file_name = "test" + nxenv.generate_hash()
		content = file_name.encode()

		doc_pub: File = nxenv.new_doc("File")  # type: ignore
		doc_pub.file_url = f"/files/{file_name}.txt"
		doc_pub.content = content
		doc_pub.save()

		doc_pri: File = nxenv.new_doc("File")  # type: ignore
