nxenv.ui.form.on("File", {
	...
	preview_file: function (frm) {
		...
		if (nxenv.utils.is_image_file(frm.doc.file_url)) {
			$preview = $(`<div class="img_preview">
				<img
					class="img-responsive"
					src="${nxenv.utils.escape_html(frm.doc.file_url)}"
					onerror="${frm.toggle_display("preview", false)}"
				/>
			</div>`);
		} else if (nxenv.utils.is_video_file(frm.doc.file_url)) {
			$preview = $(`<div class="img_preview">
				<video width="480" height="320" controls>
					<source src="${nxenv.utils.escape_html(frm.doc.file_url)}">
			...
			$preview = $(`<div class="img_preview">
				<object style="background:#323639;" width="100%">
					<embed
						...
						width="100%"
						height="1190"
						src="${nxenv.utils.escape_html(frm.doc.file_url)}" type="application/pdf"
					>
				</object>
			</div>`);
			...
			$preview = $(`<div class="img_preview">
				<audio width="480" height="60" controls>
					<source src="${nxenv.utils.escape_html(frm.doc.file_url)}" type="audio/mpeg">
	...
	optimize: function (frm) {
		nxenv.show_alert(__("Optimizing image..."));
		frm.call("optimize_file").then(() => {
			nxenv.show_alert(__("Image optimized"));
		});
	},

	unzip: function (frm) {
		nxenv.call({
			method: "nxenv.core.api.file.unzip_file",
			args: {
				name: frm.doc.name,
			},
			callback: function () {
