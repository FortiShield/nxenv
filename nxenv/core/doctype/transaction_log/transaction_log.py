...
from nxenv.utils import cint, now_datetime
...
class TransactionLog(Document):
	...
	def hash_line(self):
		sha = hashlib.sha256()
		sha.update(
			nxenv.safe_encode(str(self.row_index))
			+ nxenv.safe_encode(str(self.timestamp))
			+ nxenv.safe_encode(str(self.data))
		)
	...
	def hash_chain(self):
		sha = hashlib.sha256()
		sha.update(
			nxenv.safe_encode(str(self.transaction_hash)) + nxenv.safe_encode(str(self.previous_hash))
		)
		return sha.hexdigest()
...
def get_current_index():
	...
	current = (
		nxenv.qb.from_(series).where(series.name == "TRANSACTLOG").for_update().select("current")
	).run()
	...
	if current and current[0][0] is not None:
		...
		nxenv.db.sql("INSERT INTO `tabSeries` (name, current) VALUES ('TRANSACTLOG', 1)")
		current = 1
	return current
