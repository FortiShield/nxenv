nxenv.ui.form.on("System Settings", {
	refresh: function (frm) {
		nxenv.call({
			method: "nxenv.core.doctype.system_settings.system_settings.load",
			callback: function (data) {
				nxenv.all_timezones = data.message.timezones;
				frm.set_df_property("time_zone", "options", nxenv.all_timezones);

				$.each(data.message.defaults, function (key, val) {
					frm.set_value(key, val, null, true);
					nxenv.sys_defaults[key] = val;
				});
				if (frm.re_setup_moment) {
					nxenv.app.setup_moment();
	...
	after_save: function (frm) {
		...
		const attr_tuples = [
			[frm.doc.language, nxenv.boot.sysdefaults.language, nxenv.boot.user.language],
			[frm.doc.rounding_method, nxenv.boot.sysdefaults.rounding_method], // no user override.
		];
	...
	rounding_method: function (frm) {
		if (frm.doc.rounding_method == nxenv.boot.sysdefaults.rounding_method) return;
		let msg = __(
		...
		nxenv.confirm(
			...
			() => {},
			() => {
				frm.set_value("rounding_method", nxenv.boot.sysdefaults.rounding_method);
			}
		);
	},
