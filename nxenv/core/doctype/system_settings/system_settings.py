...
from nxenv.utils import cint, today
...
class SystemSettings(Document):
	...
	def validate(self):
		from nxenv.twofactor import toggle_two_factor_auth
		...
		if enable_password_policy and minimum_password_score <= 0:
			nxenv.throw(_("Please select Minimum Password Score"))
		...
		if self.session_expiry:
			...
			if len(parts) != 2 or not (cint(parts[0]) or cint(parts[1])):
				nxenv.throw(_("Session Expiry must be in format {0}").format("hh:mm"))

		if self.enable_two_factor_auth:
			if self.two_factor_method == "SMS":
				if not nxenv.db.get_single_value("SMS Settings", "sms_gateway_url"):
					nxenv.throw(
		...
		nxenv.flags.update_last_reset_password_date = False
		if self.force_user_to_reset_password and not cint(
			nxenv.db.get_single_value("System Settings", "force_user_to_reset_password")
		):
			nxenv.flags.update_last_reset_password_date = True
	...
	def validate_user_pass_login(self):
		...
		social_login_enabled = nxenv.db.exists("Social Login Key", {"enable_social_login": 1})
		ldap_enabled = nxenv.db.get_single_value("LDAP Settings", "enabled")
		login_with_email_link_enabled = nxenv.db.get_single_value("System Settings", "login_with_email_link")
	...
	def validate_backup_limit(self):
		if not self.backup_limit or self.backup_limit < 1:
			nxenv.msgprint(_("Number of backups must be greater than zero."), alert=True)
			self.backup_limit = 1
	...
	def on_update(self):
		...
		if nxenv.flags.update_last_reset_password_date:
			update_last_reset_password_date()

	def set_defaults(self):
		from nxenv.translate import set_default_language

		for df in self.meta.get("fields"):
			if df.fieldtype not in no_value_fields and self.has_value_changed(df.fieldname):
				nxenv.db.set_default(df.fieldname, self.get(df.fieldname))
...
@nxenv.whitelist()
def load():
	from nxenv.utils.momentjs import get_all_timezones

	if "System Manager" not in nxenv.get_roles():
		nxenv.throw(_("Not permitted"), nxenv.PermissionError)

	all_defaults = nxenv.db.get_defaults()
	defaults = {}

	for df in nxenv.get_meta("System Settings").get("fields"):
...
cache_key = nxenv.get_document_cache_key("System Settings", "System Settings")


def get_system_settings(key: str):
	"""Return the value associated with the given `key` from System Settings DocType."""
	if not (system_settings := getattr(nxenv.local, "system_settings", None)):
		try:
			system_settings = nxenv.client_cache.get_doc("System Settings")
			nxenv.local.system_settings = system_settings
		except nxenv.DoesNotExistError:  # possible during new install
...
def clear_system_settings_cache():
	nxenv.client_cache.delete_value(cache_key)
	nxenv.cache.delete_value("system_settings")
	nxenv.cache.delete_value("time_zone")
