...
from nxenv.utils import cstr, get_site_url
...
class TestAccessLog(IntegrationTestCase):
	def setUp(self):
		...
		generate_keys(nxenv.session.user)
		nxenv.db.commit()
		generated_secret = nxenv.utils.password.get_decrypted_password(
			"User", nxenv.session.user, fieldname="api_secret"
		)
		...
		self.file_name = nxenv.utils.random_string(10) + ".txt"
		self.test_content = nxenv.utils.random_string(1024)
	...
	def test_make_full_access_log(self):
		...
		last_doc = nxenv.get_last_doc("Access Log")
	...
	def test_private_file_download(self):
		...
		new_private_file = nxenv.get_doc(
			{
				"doctype": self.test_doctype,
		...
		private_file_link = get_site_url(nxenv.local.site) + new_private_file.file_url
