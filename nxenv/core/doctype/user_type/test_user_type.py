...
class TestUserType(IntegrationTestCase):
	...
	def test_add_select_perm_doctypes(self):
		...
		doc = nxenv.get_meta("Contact")
		link_fields = doc.get_link_fields()
		select_doctypes = nxenv.get_all(
	...
	def test_print_share_email_default(self):
		...
		perm = nxenv.get_all("Custom DocPerm", filters={"role": "_Test User Type"}, fields=["*"])[0]
...
def create_user_type(user_type):
	if nxenv.db.exists("User Type", user_type):
		nxenv.delete_doc("User Type", user_type)

	user_type_limit = {nxenv.scrub(user_type): 1}
...
def create_role():
	if not nxenv.db.exists("Role", "_Test User Type"):
		nxenv.get_doc(
