...
from nxenv.permissions import add_permission, add_user_permission
from nxenv.utils import get_link_to_form
from nxenv.utils.modules import get_modules_from_app
...
class UserType(Document):
	...
	if TYPE_CHECKING:
		from nxenv.core.doctype.user_document_type.user_document_type import UserDocumentType
		from nxenv.core.doctype.user_select_document_type.user_select_document_type import (
			UserSelectDocumentType,
		)
		from nxenv.core.doctype.user_type_module.user_type_module import UserTypeModule
	...
	def clear_cache(self):
		...
		if not self.is_standard:
			nxenv.cache.delete_value("non_standard_user_types")
	...
	def on_trash(self):
		if self.is_standard:
			nxenv.throw(_("Standard user type {0} can not be deleted.").format(nxenv.bold(self.name)))

	def set_modules(self):
		if not self.user_doctypes:
			return

		modules = nxenv.get_all(
			"DocType",
	...
	def validate_document_type_limit(self):
		limit = nxenv.conf.get("user_type_doctype_limit", {}).get(nxenv.scrub(self.name))

		if not limit and nxenv.session.user != "Administrator":
			nxenv.throw(
				_("User does not have permission to create the new {0}").format(nxenv.bold(_("User Type"))),
	...
	def validate_role(self):
		if not self.role:
			nxenv.throw(_("The field {0} is mandatory").format(nxenv.bold(_("Role"))))

		if not nxenv.db.get_value("Role", self.role, "is_custom"):
			nxenv.throw(
				_("The role {0} should be a custom role.").format(
					nxenv.bold(get_link_to_form("Role", self.role))
				)
			)

	def update_users(self):
		for row in nxenv.get_all("User", filters={"user_type": self.name}):
			user = nxenv.get_cached_doc("User", row.name)
	...
	def add_select_perm_doctypes(self):
		if nxenv.flags.ignore_select_perm:
			return
	...
	def remove_permission_for_deleted_doctypes(self):
		...
		for perm in nxenv.get_all(
			"Custom DocPerm", filters={"role": self.role, "parent": ["not in", doctypes]}
		):
			nxenv.delete_doc("Custom DocPerm", perm.name)


def add_role_permissions(doctype, role):
	name = nxenv.get_value("Custom DocPerm", dict(parent=doctype, role=role, permlevel=0))
...
def get_user_linked_doctypes(doctype, txt, searchfield, start, page_len, filters):
	modules = [d.get("module_name") for d in get_modules_from_app("nxenv")]
...
def user_linked_with_permission_on_doctype(doc, user):
	...
	if not doc.user_id_field:
		nxenv.throw(_("User Id Field is mandatory in the user type {0}").format(nxenv.bold(doc.name)))

	if nxenv.db.get_value(doc.apply_user_permission_on, {doc.user_id_field: user}, "name"):
		return True
	else:
		label = nxenv.get_meta(doc.apply_user_permission_on).get_field(doc.user_id_field).label
		...
		nxenv.msgprint(
			_(
				...
				nxenv.bold(label),
				nxenv.bold(user),
				nxenv.bold(doc.apply_user_permission_on),
			)
		)
...
def apply_permissions_for_non_standard_user_type(doc, method=None):
	...
	if not nxenv.db.table_exists("User Type") or nxenv.flags.in_migrate:
		return

	user_types = nxenv.cache.get_value(
	...
	for user_type, data in user_types.items():
		...
		if doc.get(data[1]) and (
			...
			if not perm_data:
				...
				user_perm = nxenv.get_doc("User Permission", perm_data[0])
