...
import nxenv
from nxenv import _
from nxenv.core.doctype.recorder.db_optimizer import DBOptimizer, DBTable
from nxenv.custom.doctype.property_setter.property_setter import make_property_setter
from nxenv.model.document import Document
from nxenv.recorder import RECORDER_REQUEST_HASH
from nxenv.recorder import get as get_recorder_data
from nxenv.utils import cint, cstr, evaluate_filters, get_table_name
...
class Recorder(Document):
	...
	if TYPE_CHECKING:
		from nxenv.core.doctype.recorder_query.recorder_query import RecorderQuery
		from nxenv.core.doctype.recorder_suggested_index.recorder_suggested_index import (
...
def serialize_request(request):
	request = nxenv._dict(request)
	if request.get("calls"):
		for i in request.calls:
			i["stack"] = nxenv.as_json(i["stack"])
			i["explain_result"] = nxenv.as_json(i["explain_result"])
	request.update(
		name=request.get("uuid"),
		number_of_queries=request.get("queries"),
		time_in_queries=request.get("time_queries"),
		request_headers=nxenv.as_json(request.get("headers", {}), indent=4),
		form_dict=nxenv.as_json(request.get("form_dict", {}), indent=4),
...
def add_indexes(indexes):
	...
	for index in indexes:
		nxenv.enqueue(_add_index, table=index["table"], column=index["column"])
	nxenv.msgprint(_("Enqueued creation of indexes"), alert=True)


def _add_index(table, column):
	doctype = get_doctype_name(table)
	nxenv.db.add_index(doctype, [column])
...
def optimize(recorder_id: str):
	nxenv.only_for("Administrator")
	nxenv.enqueue(_optimize, recorder_id=recorder_id, queue="long")


def _optimize(recorder_id):
	record: Recorder = nxenv.get_doc("Recorder", recorder_id)
	...
	data = nxenv.cache.hget(RECORDER_REQUEST_HASH, record.name)
	data["suggested_indexes"] = [{"table": idx[0][0], "column": idx[0][1]} for idx in suggested_indexes]
	nxenv.cache.hset(RECORDER_REQUEST_HASH, record.name, data)
	nxenv.publish_realtime("recorder-analysis-complete", user=nxenv.session.user)
	nxenv.msgprint(_("Query analysis complete. Check suggested indexes."), realtime=True, alert=True)
...
def _optimize_query(query):
	...
	for table in tables:
		...
		if not stats:
			return
		db_table = DBTable.from_nxenv_ouput(stats)
...
def _fetch_table_stats(doctype: str, columns: list[str]) -> dict | None:
	...
	if not nxenv.db.table_exists(doctype):
		return

	table = get_table_name(doctype, wrap_in_backticks=True)

	schema = []
	for field in nxenv.db.sql(f"describe {table}", as_dict=True):
		schema.append(
			{
				"column": field["Field"],
				"type": field["Type"],
	...
	indexes = []
	for idx in nxenv.db.sql(f"show index from {table}", as_dict=True):
		indexes.append(
			{
...
def _get_column_cardinality(table, column):
	return nxenv.db.sql(f"select count(distinct {column}) from {table}")[0][0]
