nxenv.listview_settings["Recorder"] = {
	...
	onload(listview) {
		...
		listview.page.add_button(__("Clear"), () => {
			nxenv.xcall("nxenv.recorder.delete").then(listview.refresh);
		});
		...
		listview.page.add_menu_item(__("Import"), () => {
			new nxenv.ui.FileUploader({
				...
				on_success: (file) => {
					...
					nxenv.call({
						method: "nxenv.recorder.import_data",
						args: {
							file: file.file_url,
						},
						callback: function () {
		...
		listview.page.add_menu_item(__("Export"), () => {
			nxenv.call({
				method: "nxenv.recorder.export_data",
				callback: function (r) {
	...
	fetch_recorder_status(listview) {
		return nxenv.xcall("nxenv.recorder.status").then((status) => {
	...
	stop_recorder(listview) {
		let me = this;
		nxenv.xcall("nxenv.recorder.stop", {}).then(() => {
			listview.refresh();
	...
	start_recorder(listview) {
		...
		nxenv.prompt(
			...
			(values) => {
				nxenv.xcall("nxenv.recorder.start", values).then(() => {
					listview.refresh();
