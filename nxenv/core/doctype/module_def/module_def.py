...
from nxenv.modules.export_file import delete_folder
...
class ModuleDef(Document):
	...
	def on_update(self):
		...
		nxenv.clear_cache()
		if not self.custom and nxenv.conf.get("developer_mode"):
	...
	def create_modules_folder(self):
		...
		module_path = nxenv.get_app_path(self.app_name, self.name)
	...
	def add_to_modules_txt(self):
		...
		modules = None
		if not nxenv.local.module_app.get(nxenv.scrub(self.name)):
			with open(nxenv.get_app_path(self.app_name, "modules.txt")) as f:
				content = f.read()
			...
			if modules:
				with open(nxenv.get_app_path(self.app_name, "modules.txt"), "w") as f:
					f.write("\n".join(modules))
	...
	def on_trash(self):
		...
		if not nxenv.conf.get("developer_mode") or nxenv.flags.in_uninstall or self.custom:
			return

		if nxenv.local.module_app.get(nxenv.scrub(self.name)):
			nxenv.db.after_commit.add(self.delete_module_from_file)

	def delete_module_from_file(self):
		delete_folder(self.module_name, "Module Def", self.name)
		modules = []

		modules_txt = Path(nxenv.get_app_path(self.app_name, "modules.txt"))
...
def get_installed_apps():
	return json.dumps(nxenv.get_installed_apps())
