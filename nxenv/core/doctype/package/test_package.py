...
class TestPackage(IntegrationTestCase):
	def test_package_release(self):
		...
		# make release
		nxenv.get_doc(doctype="Package Release", package="Test Package", publish=1).insert()

		self.assertTrue(os.path.exists(nxenv.get_site_path("packages", "test-package")))
		self.assertTrue(
			os.path.exists(nxenv.get_site_path("packages", "test-package", "test_module_for_package"))
		)
		self.assertTrue(
			os.path.exists(
				nxenv.get_site_path(
...
def make_test_package():
	if not nxenv.db.exists("Package", "Test Package"):
		nxenv.get_doc(
...
def make_test_module():
	if not nxenv.db.exists("Module Def", "Test Module for Package"):
		nxenv.get_doc(
			doctype="Module Def",
...
def make_test_doctype():
	if not nxenv.db.exists("DocType", "Test DocType for Package"):
		nxenv.get_doc(
			doctype="DocType",
...
def make_test_server_script():
	if not nxenv.db.exists("Server Script", "Test Script for Package"):
		nxenv.get_doc(
			...
			script='nxenv.msgprint("Test")',
		).insert()


def make_test_web_page():
	if not nxenv.db.exists("Web Page", "test-web-page-for-package"):
		nxenv.get_doc(
			doctype="Web Page",
			module="Test Module for Package",
