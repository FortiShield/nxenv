...
from nxenv.model.sync import get_doc_files
from nxenv.modules.import_file import import_doc, import_file_by_path
from nxenv.utils import get_files_path
...
class PackageImport(Document):
	...
	def import_package(self):
		...
		if not attachment:
			nxenv.throw(nxenv._("Please attach the package"))

		attachment = attachment[0]

		# get package_name from file (package_name-0.0.0.tar.gz)
		package_name = attachment.file_name.split(".", 1)[0].rsplit("-", 1)[0]
		if not os.path.exists(nxenv.get_site_path("packages")):
			os.makedirs(nxenv.get_site_path("packages"))
		...
		subprocess.check_output(
			[
				...
				"-C",
				nxenv.get_site_path("packages"),
			]
		)

		package_path = nxenv.get_site_path("packages", package_name)

		# import Package
		with open(os.path.join(package_path, package_name + ".json")) as packagefile:
			doc_dict = json.loads(packagefile.read())

		nxenv.flags.package = import_doc(doc_dict)
