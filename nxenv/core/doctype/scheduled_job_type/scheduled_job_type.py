...
from nxenv.utils import get_datetime, now_datetime
...
class ScheduledJobType(Document):
	...
	def validate(self):
		...
		if self.frequency == "Cron":
			if not self.cron_format:
				nxenv.throw(_("Cron format is required for job types with Cron frequency."))
			try:
	...
	def enqueue(self, force=False) -> bool:
		...
		if self.is_event_due() or force:
			if not self.is_job_in_queue():
				enqueue(
					"nxenv.core.doctype.scheduled_job_type.scheduled_job_type.run_scheduled_job",
				...
				nxenv.logger("scheduler").error(
					f"Skipped queueing {self.method} because it was found in queue for {nxenv.local.site}"
				)
	...
	def get_next_execution(self):
		CRON_MAP = {
			...
			"Hourly": "0 * * * *",
			"Hourly Long": "0 * * * *",
			"All": f"*/{(nxenv.get_conf().scheduler_interval or 240) // 60} * * * *",
		}
	...
	def execute(self):
		if nxenv.job:
			nxenv.job.frequency = self.frequency
			nxenv.job.cron_format = self.cron_format

		self.scheduler_log = None
		try:
			self.log_status("Start")
			if self.server_script:
				script_name = nxenv.db.get_value("Server Script", self.server_script)
				if script_name:
					nxenv.get_doc("Server Script", script_name).execute_scheduled_method()
			else:
				nxenv.get_attr(self.method)()
			nxenv.db.commit()
	...
	def log_status(self, status):
		# log file
		nxenv.logger("scheduler").info(f"Scheduled Job {status}: {self.method} for {nxenv.local.site}")
	...
	def update_scheduler_log(self, status):
		...
		if not self.scheduler_log:
			self.scheduler_log = nxenv.get_doc(
		...
		if status == "Failed":
			self.scheduler_log.db_set("details", nxenv.get_traceback(with_context=True))
		if status == "Start":
			self.db_set("last_execution", now_datetime(), update_modified=False)
		nxenv.db.commit()
	...
	def on_trash(self):
		nxenv.db.delete("Scheduled Job Log", {"scheduled_job_type": self.name})
...
def execute_event(doc: str):
	...
	doc = json.loads(doc)
	nxenv.get_doc("Scheduled Job Type", doc.get("name")).enqueue(force=True)
	return doc
...
def run_scheduled_job(scheduled_job_type: str, job_type: str | None = None):
	...
	try:
		nxenv.get_doc("Scheduled Job Type", scheduled_job_type).execute()
	except Exception:
		print(nxenv.get_traceback())


def sync_jobs(hooks: dict | None = None):
	nxenv.reload_doc("core", "doctype", "scheduled_job_type")
	scheduler_events = hooks or nxenv.get_hooks("scheduler_events")
...
def insert_single_event(frequency: str, event: str, cron_format: str | None = None):
	...
	if not nxenv.db.exists("Scheduled Job Type", {"method": event, "frequency": frequency, **cron_expr}):
