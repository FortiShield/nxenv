...
class TestScheduledJobType(IntegrationTestCase):
	def setUp(self):
		nxenv.db.rollback()
		nxenv.db.truncate("Scheduled Job Type")
		sync_jobs()
		nxenv.db.commit()
	...
	def test_throws_on_duplicate_job(self):
		job_config = dict(
			...
			method="nxenv.desk.notifications.clear_notifications",
			frequency="Weekly",
		)
		nxenv.get_doc(job_config).insert()

		duplicate_job = nxenv.get_doc(job_config)
	...
	def test_sync_jobs(self):
		all_job = nxenv.get_doc("Scheduled Job Type", dict(method="nxenv.email.queue.flush"))
		self.assertEqual(all_job.frequency, "All")

		daily_job = nxenv.get_doc(
			"Scheduled Job Type", dict(method="nxenv.desk.notifications.clear_notifications")
		)
		self.assertEqual(daily_job.frequency, "Daily")

		# check if cron jobs are synced
		cron_job = nxenv.get_doc("Scheduled Job Type", dict(method="nxenv.oauth.delete_oauth2_data"))
		...
		updated_scheduler_events = {"hourly": ["nxenv.email.queue.flush"]}
		sync_jobs(updated_scheduler_events)
		updated_scheduled_job = nxenv.get_doc("Scheduled Job Type", {"method": "nxenv.email.queue.flush"})
	...
	def test_weekly_job(self):
		job = nxenv.get_doc(
			"Scheduled Job Type",
			dict(method="nxenv.social.doctype.energy_point_log.energy_point_log.send_weekly_summary"),
		)
		job.db_set("last_execution", "2019-01-01 00:00:00")
	...
	def test_monthly_job(self):
		job = nxenv.get_doc(
			"Scheduled Job Type",
			dict(method="nxenv.email.doctype.auto_email_report.auto_email_report.send_monthly"),
		)
		job.db_set("last_execution", "2019-01-01 00:00:00")
	...
	def test_cron_job(self):
		# runs every 15 mins
		job = nxenv.get_doc("Scheduled Job Type", dict(method="nxenv.oauth.delete_oauth2_data"))
		job.db_set("last_execution", "2019-01-01 00:00:00")
	...
	def test_cold_start(self):
		...
		job = nxenv.new_doc("Scheduled Job Type")
		job.frequency = "Daily"
