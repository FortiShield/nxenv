...
from nxenv.website.path_resolver import validate_path
...
class Role(Document):
	...
	def before_rename(self, old, new, merge=False):
		if old in STANDARD_ROLES:
			nxenv.throw(nxenv._("Standard roles cannot be renamed"))

	def after_insert(self):
		nxenv.cache.hdel("roles", "Administrator")
	...
	def disable_role(self):
		if self.name in STANDARD_ROLES:
			nxenv.throw(nxenv._("Standard roles cannot be disabled"))
		else:
			self.remove_roles()

	def validate_homepage(self):
		if nxenv.request and self.home_page:
	...
	def remove_roles(self):
		nxenv.db.delete("Has Role", {"role": self.name})
		nxenv.clear_cache()
	...
	def update_user_type_on_change(self):
		...
		users_with_same_user_type = nxenv.get_all("User", {"user_type": role_user_type}, pluck="name")

		for user_name in set(users_with_role) - set(users_with_same_user_type):
			user = nxenv.get_doc("User", user_name)
...
def get_user_info(users, field="email"):
	...
	info_list = []
	for user in users:
		user_info, enabled = nxenv.db.get_value("User", user.get("user_name"), [field, "enabled"])
...
def get_users(role):
	return [
		d.parent
		for d in nxenv.get_all("Has Role", filters={"role": role, "parenttype": "User"}, fields=["parent"])
	]
