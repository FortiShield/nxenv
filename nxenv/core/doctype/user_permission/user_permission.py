...
from nxenv.desk.form.linked_with import get_linked_doctypes
...
class UserPermission(Document):
	...
	def on_update(self):
		nxenv.cache.hdel("user_permissions", self.user)
		nxenv.publish_realtime("update_user_permissions", user=self.user, after_commit=True)
	...
	def validate_user_permission(self):
		...
		duplicate_exists = nxenv.get_all(
			self.doctype,
			filters={
				"allow": self.allow,
	...
	def validate_default_permission(self):
		...
		if overlap_exists:
			ref_link = nxenv.get_desk_link(self.doctype, overlap_exists[0].name)
			nxenv.throw(_("{0} has already assigned default value for {1}.").format(ref_link, self.allow))
...
def get_user_permissions(user=None):
	...
	if nxenv.request and nxenv.local.form_dict.cmd == "get_user_permissions":
	...
	cached_user_permissions = nxenv.cache.hget("user_permissions", user)
	...
	try:
		for perm in nxenv.get_all(
			...
			meta = nxenv.get_meta(perm.allow)
			add_doc_to_perm(perm, perm.for_value, perm.is_default)

			if meta.is_nested_set() and not perm.hide_descendants:
				decendants = nxenv.db.get_descendants(perm.allow, perm.for_value)
				for doc in decendants:
					add_doc_to_perm(perm, doc, False)

		out = nxenv._dict(out)
		nxenv.cache.hset("user_permissions", user, out)
	except nxenv.db.SQLError as e:
...
def clear_user_permissions(user, for_doctype):
	nxenv.only_for("System Manager")
	total = nxenv.db.count("User Permission", {"user": user, "allow": for_doctype})
...
def insert_user_perm(
	...
	user_perm = nxenv.new_doc("User Permission")
	user_perm.user = user
