...
from nxenv.core.doctype.user_permission.user_permission import (
	add_user_permissions,
	remove_applicable,
)
from nxenv.permissions import add_permission, has_user_permission
from nxenv.tests import IntegrationTestCase, UnitTestCase
from nxenv.website.doctype.blog_post.test_blog_post import make_test_blog
...
class TestUserPermission(IntegrationTestCase):
	def setUp(self):
		...
		nxenv.db.delete("User Permission", {"user": ("in", test_users)})
		nxenv.delete_doc_if_exists("DocType", "Person")
		nxenv.db.sql_ddl("DROP TABLE IF EXISTS `tabPerson`")
		nxenv.delete_doc_if_exists("DocType", "Doc A")
		nxenv.db.sql_ddl("DROP TABLE IF EXISTS `tabDoc A`")
	...
	def test_default_user_permission_validation(self):
		...
		self.assertRaises(nxenv.ValidationError, add_user_permissions, param)
	...
	def test_default_user_permission_corectness(self):
		...
		add_user_permissions(param)
		nxenv.db.delete("User Permission", filters={"for_value": test_blog.name})
		nxenv.delete_doc("Blog Post", test_blog.name)
	...
	def test_default_user_permission(self):
		...
		for category in ["general", "public"]:
			if not nxenv.db.exists("Blog Category", category):
				nxenv.get_doc({"doctype": "Blog Category", "title": category}).insert()
		...
		nxenv.set_user("test_user_perm1@example.com")
	...
	def test_for_applicable_on_update_from_apply_to_all(self):
		...
		removed_apply_to_all = nxenv.db.exists("User Permission", get_exists_param(user))
		is_created_applicable_first = nxenv.db.exists(
			"User Permission", get_exists_param(user, applicable="Comment")
		)
		is_created_applicable_second = nxenv.db.exists(
	...
	def test_for_apply_to_all_on_update_from_applicable(self):
		...
		is_created = add_user_permissions(param)
		is_created_apply_to_all = nxenv.db.exists("User Permission", get_exists_param(user))
		removed_applicable_first = nxenv.db.exists(
			"User Permission", get_exists_param(user, applicable="Comment")
		)
		removed_applicable_second = nxenv.db.exists(
			"User Permission", get_exists_param(user, applicable="Contact")
		)
	...
	def test_user_perm_for_nested_doctype(self):
		...
		if not nxenv.db.exists("DocType", "Person"):
			doc = new_doctype(
				"Person",
		...
		parent_record = nxenv.get_doc({"doctype": "Person", "person_name": "Parent", "is_group": 1}).insert()

		child_record = nxenv.get_doc(
			{
				"doctype": "Person",
				"person_name": "Child",
				"is_group": 0,
		...
		self.assertTrue(has_user_permission(nxenv.get_doc("Person", parent_record.name), user.name))
		self.assertTrue(has_user_permission(nxenv.get_doc("Person", child_record.name), user.name))

		#  give access of Parent DocType to Blogger role
		add_permission("Person", "Blogger")
		nxenv.set_user(user.name)
		visible_names = nxenv.get_list(
			doctype="Person",
			pluck="person_name",
		)

		user_permission = nxenv.get_doc(
		...
		# hides child records
		self.assertTrue(has_user_permission(nxenv.get_doc("Person", parent_record.name), user.name))
		self.assertFalse(has_user_permission(nxenv.get_doc("Person", child_record.name), user.name))

		visible_names_after_hide_descendants = nxenv.get_list(
			"Person",
			pluck="person_name",
		)
	...
	def test_user_perm_on_new_doc_with_field_default(self):
		...
		nxenv.set_user("new_doc_test@example.com")

		new_doc = nxenv.new_doc("Doc A")
	...
	def test_user_perm_on_new_doc_with_user_default(self):
		"""Test User Perm impact on nxenv.new_doc. with *user* default value"""
		from nxenv.core.doctype.session_default_settings.session_default_settings import (
		...
		if not nxenv.db.exists("Session Default", {"ref_doctype": "DocType"}):
		...
		nxenv.set_user("user_default_test@example.com")
...
def create_user(email, *roles):
	...
	if nxenv.db.exists("User", email):
		return nxenv.get_doc("User", email)
