...
class ActivityLog(Document):
	...
	def set_ip_address(self):
		if self.operation in ("Login", "Logout"):
			self.ip_address = nxenv.local.request_ip
	...
	def clear_old_logs(days=None):
		...
		nxenv.db.delete(doctype, filters=(doctype.creation < (Now() - Interval(days=days))))


def on_doctype_update():
	"""Add indexes in `tabActivity Log`"""
	nxenv.db.add_index("Activity Log", ["reference_doctype", "reference_name"])
	nxenv.db.add_index("Activity Log", ["timeline_doctype", "timeline_name"])
