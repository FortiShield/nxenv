...
class TestDocumentNamingRule(IntegrationTestCase):
	def test_naming_rule_by_series(self):
		naming_rule = nxenv.get_doc(
			doctype="Document Naming Rule", document_type="ToDo", prefix="test-todo-", prefix_digits=5
		).insert()

		todo = nxenv.get_doc(
			doctype="ToDo", description="Is this my name " + nxenv.generate_hash()
		).insert()
	...
	def test_naming_rule_by_condition(self):
		...
		# another rule
		naming_rule_1 = nxenv.copy_doc(naming_rule)
		...
		naming_rule_2 = nxenv.copy_doc(naming_rule)
		...
		todo = nxenv.get_doc(
			doctype="ToDo", priority="High", description="Is this my name " + nxenv.generate_hash()
		).insert()

		todo_1 = nxenv.get_doc(
			doctype="ToDo", priority="Medium", description="Is this my name " + nxenv.generate_hash()
		).insert()

		todo_2 = nxenv.get_doc(
			doctype="ToDo", priority="Low", description="Is this my name " + nxenv.generate_hash()
		).insert()
