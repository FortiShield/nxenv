...
nxenv.ui.form.on("Document Naming Rule", {
	...
	document_type: (frm) => {
		...
		if (frm.doc.document_type) {
			nxenv.model.with_doctype(frm.doc.document_type, () => {
				let fieldnames = nxenv
					.get_meta(frm.doc.document_type)
					.fields.filter((d) => {
						return nxenv.model.no_value_type.indexOf(d.fieldtype) === -1;
					})
					.map((d) => {
