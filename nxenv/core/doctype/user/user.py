...
import nxenv.share
from nxenv import STANDARD_USERS, _, msgprint, throw
from nxenv.apps import get_default_path
from nxenv.auth import MAX_PASSWORD_SIZE
from nxenv.core.doctype.user_type.user_type import user_linked_with_permission_on_doctype
...
from nxenv.utils.data import sha256_hash
from nxenv.utils.password import check_password, get_password_reset_limit
from nxenv.utils.password import update_password as _update_password
from nxenv.utils.user import get_system_managers
from nxenv.website.utils import get_home_page, is_signup_disabled
...
class User(Document):
	...
	if TYPE_CHECKING:
		...
		from nxenv.core.doctype.defaultvalue.defaultvalue import DefaultValue
		from nxenv.core.doctype.has_role.has_role import HasRole
		from nxenv.core.doctype.user_email.user_email import UserEmail
		from nxenv.core.doctype.user_role_profile.user_role_profile import UserRoleProfile
		from nxenv.core.doctype.user_social_login.user_social_login import UserSocialLogin
	...
	def after_insert(self):
		...
		nxenv.cache.delete_key("users_for_mentions")
		nxenv.cache.delete_key("enabled_users")
	...
	def validate(self):
		...
		if (self.name not in ["Administrator", "Guest"]) and (not self.get_social_login_userid("nxenv")):
			self.set_social_login_userid("nxenv", nxenv.generate_hash(length=39))
	...
	def populate_role_profile_roles(self):
		...
		for role_profile in self.role_profiles:
			role_profile = nxenv.get_cached_doc("Role Profile", role_profile.role_profile)
	...
	from nxenv.deprecation_dumpster import validate_roles
	...
	def validate_allowed_modules(self):
		if self.module_profile:
			module_profile = nxenv.get_doc("Module Profile", self.module_profile)
	...
	def validate_user_image(self):
		if self.user_image and len(self.user_image) > 2000:
			nxenv.throw(_("Not a valid User Image."))

	def on_update(self):
		# clear new password
		self.share_with_self()
		clear_notifications(user=self.name)
		nxenv.clear_cache(user=self.name)
		now = nxenv.flags.in_test or nxenv.flags.in_install
		self.send_password_notification(self.__new_password)
		nxenv.enqueue(
			"nxenv.core.doctype.user.user.create_contact",
			user=self,
		...
		if self.name not in STANDARD_USERS and not self.user_image:
			nxenv.enqueue(
				"nxenv.core.doctype.user.user.update_gravatar",
				name=self.name,
				now=now,
				enqueue_after_commit=True,
			)
		...
		if self.time_zone:
			nxenv.defaults.set_default("time_zone", self.time_zone, self.name)

		if self.has_value_changed("language"):
			locale_keys = ("date_format", "time_format", "number_format", "first_day_of_the_week")
			if self.language:
				language = nxenv.get_doc("Language", self.language)
				for key in locale_keys:
					value = language.get(key)
					if value:
						nxenv.defaults.set_default(key, value, self.name)
			else:
				for key in locale_keys:
					nxenv.defaults.clear_default(key, parent=self.name)
	...
	def has_website_permission(self, ptype, user, verbose=False):
		...
		return self.name == nxenv.session.user
	...
	def check_enable_disable(self):
		...
		if not cint(self.enabled) and self.name in STANDARD_USERS:
			nxenv.throw(_("User {0} cannot be disabled").format(self.name))

		# clear sessions if disabled
		if not cint(self.enabled) and getattr(nxenv.local, "login_manager", None):
			nxenv.local.login_manager.logout(user=self.name)
	...
	def set_system_user(self):
		...
		if self.user_type and not nxenv.get_cached_value("User Type", self.user_type, "is_standard"):
	...
	def set_roles_and_modules_based_on_user_type(self):
		user_type_doc = nxenv.get_cached_doc("User Type", self.user_type)
	...
	def validate_share(self, docshare):
		...
		# 		if docshare.share != 1:
		# 			nxenv.throw(_("Sorry! User should have complete access to their own record."))
		# 	else:
		# 		nxenv.throw(_("Sorry! Sharing with Website User is prohibited."))
	...
	def reset_password(self, send_email=False, password_expired=False):
		from nxenv.utils import get_url
	...
	def password_reset_mail(self, link):
		reset_password_template = nxenv.db.get_system_setting("reset_password_template")
	...
	def send_welcome_mail_to_user(self):
		...
		subject = None
		method = nxenv.get_hooks("welcome_email")
		if method:
			subject = nxenv.get_attr(method[-1])()
		if not subject:
			site_name = nxenv.db.get_default("site_name") or nxenv.get_conf().get("site_name")
			if site_name:
				subject = _("Welcome to {0}").format(site_name)
			else:
				subject = _("Complete Registration")

		welcome_email_template = nxenv.db.get_system_setting("welcome_email_template")
	...
	def send_login_mail(self, subject, template, add_args, now=None, custom_template=None):
		...
		from nxenv.utils import get_url
		from nxenv.utils.user import get_user_fullname

		created_by = get_user_fullname(nxenv.session["user"])
		if created_by == "Guest":
		...
		sender = (
			nxenv.session.user not in STANDARD_USERS and get_formatted_email(nxenv.session.user)
		) or None

		if custom_template:
			from nxenv.email.doctype.email_template.email_template import get_email_template
	...
	def on_trash(self):
		...
		self.enabled = 0
		if getattr(nxenv.local, "login_manager", None):
			nxenv.local.login_manager.logout(user=self.name)

		# delete todos
		nxenv.db.delete("ToDo", {"allocated_to": self.name})
		todo_table = DocType("ToDo")
		(
			nxenv.qb.update(todo_table)
			.set(todo_table.assigned_by, None)
			.where(todo_table.assigned_by == self.name)
		).run()

		# delete events
		nxenv.db.delete("Event", {"owner": self.name, "event_type": "Private"})

		# delete shares
		nxenv.db.delete("DocShare", {"user": self.name})
		# unlink contact
		table = DocType("Contact")
		nxenv.qb.update(table).where(table.user == self.name).set(table.user, None).run()

		# delete notification settings
		nxenv.delete_doc("Notification Settings", self.name, ignore_permissions=True)
		...
		nxenv.db.delete("User Permission", {"user": self.name})

		# Delete OAuth data
		nxenv.db.delete("OAuth Authorization Code", {"user": self.name})
		nxenv.db.delete("Token Cache", {"user": self.name})

		# Delete EPS data
		nxenv.db.delete("Energy Point Log", {"user": self.name})

		# Remove user link from Workflow Action
		nxenv.db.set_value("Workflow Action", {"user": self.name}, "user", None)

		# Delete user's List Filters
		nxenv.db.delete("List Filter", {"for_user": self.name})

		# Remove user from Note's Seen By table
		seen_notes = nxenv.get_all("Note", filters=[["Note Seen By", "user", "=", self.name]], pluck="name")
		for note_id in seen_notes:
			note = nxenv.get_doc("Note", note_id)
	...
	def before_rename(self, old_name, new_name, merge=False):
		...
		if merge:
			nxenv.delete_doc("Notification Settings", old_name, ignore_permissions=True)

		nxenv.clear_cache(user=old_name)
	...
	def after_rename(self, old_name, new_name, merge=False):
		tables = nxenv.db.get_tables()
		for tab in tables:
			desc = nxenv.db.get_table_columns_description(tab)
		...
		if nxenv.db.exists("Notification Settings", old_name):
			nxenv.rename_doc("Notification Settings", old_name, new_name, force=True, show_alert=False)

		# set email
		nxenv.db.set_value("User", new_name, "email", new_name)
	...
	def remove_disabled_roles(self):
		disabled_roles = [d.name for d in nxenv.get_all("Role", filters={"disabled": 1})]
	...
	def validate_username(self):
		if not self.username and self.is_new() and self.first_name:
			self.username = nxenv.scrub(self.first_name)
		...
		if self.username_exists():
			if self.user_type == "System User":
				nxenv.msgprint(_("Username {0} already exists").format(self.username))
	...
	def suggest_username(self):
		...
		# @firstname
		username = _check_suggestion(nxenv.scrub(self.first_name))

		if not username:
			# @firstname_last_name
			username = _check_suggestion(nxenv.scrub("{} {}".format(self.first_name, self.last_name or "")))

		if username:
			nxenv.msgprint(_("Suggested Username: {0}").format(username))

		return username

	def username_exists(self, username=None):
		return nxenv.db.get_value("User", {"username": username or self.username, "name": ("!=", self.name)})
	...
	def validate_user_email_inbox(self):
		...
		if len(email_accounts) != len(set(email_accounts)):
			nxenv.throw(_("Email Account added multiple times"))
	...
	def find_by_credentials(cls, user_name: str, password: str, validate_password: bool = True):
		...
		login_with_mobile = cint(nxenv.get_system_settings("allow_login_using_mobile_number"))
		login_with_username = cint(nxenv.get_system_settings("allow_login_using_user_name"))
		...
		users = nxenv.get_all("User", fields=["name", "enabled"], or_filters=or_filters, limit=1)
		if not users:
			return
	...
	def check_roles_added(self):
		...
		nxenv.msgprint(
			_("Newly created user {0} has no roles enabled.").format(nxenv.bold(self.name)),
			title=_("No Roles Specified"),
			indicator="orange",
			primary_action={
				"label": _("Add Roles"),
				"client_action": "nxenv.set_route",
...
def get_all_roles():
	...
	roles = nxenv.get_all(
		"Role",
		filters={
			"name": ("not in", nxenv.permissions.AUTOMATIC_ROLES),
			"disabled": 0,
		},
...
def get_roles(arg=None):
	...
	return nxenv.get_roles(nxenv.form_dict.get("uid", nxenv.session.user))


@nxenv.whitelist()
def get_perm_info(role):
	"""get permission info"""
	from nxenv.permissions import get_all_perms

	return get_all_perms(role)


@nxenv.whitelist(allow_guest=True, methods=["POST"])
def update_password(
	...
	if len(new_password) > MAX_PASSWORD_SIZE:
		nxenv.throw(_("Password size exceeded the maximum allowed size."))
	...
	if res.get("message"):
		nxenv.local.response.http_status_code = 410
		return res["message"]
	else:
		user = res["user"]

	logout_all_sessions = cint(logout_all_sessions) or nxenv.get_system_settings("logout_on_password_reset")
	_update_password(user, new_password, logout_all_sessions=cint(logout_all_sessions))

	user_doc, redirect_url = reset_user_data(user)

	# get redirect url from cache
	redirect_to = nxenv.cache.hget("redirect_after_login", user)
	if redirect_to:
		redirect_url = redirect_to
		nxenv.cache.hdel("redirect_after_login", user)

	nxenv.local.login_manager.login_as(user)

	nxenv.db.set_value("User", user, "last_password_reset_date", today())
	nxenv.db.set_value("User", user, "reset_password_key", "")
...
def test_password_strength(new_password: str, key=None, old_password=None, user_data: tuple | None = None):
	from nxenv.utils.password_strength import test_password_strength as _test_password_strength
	...
	enable_password_policy = nxenv.get_system_settings("enable_password_policy")

	if not enable_password_policy:
		return {}

	if not user_data:
		user_data = nxenv.db.get_value(
			"User", nxenv.session.user, ["first_name", "middle_name", "last_name", "email", "birth_date"]
		)

	if new_password:
		result = _test_password_strength(new_password, user_inputs=user_data)
		password_policy_validation_passed = False
		minimum_password_score = cint(nxenv.get_system_settings("minimum_password_score"))
...
def has_email_account(email: str):
	return nxenv.get_list("Email Account", filters={"email_id": email})


@nxenv.whitelist(allow_guest=False)
...
def ask_pass_update():
	...
	from nxenv.utils import set_default

	password_list = nxenv.get_all(
	...
	if key:
		...
		if result.user:
			reset_password_link_expiry = cint(
				nxenv.get_system_settings("reset_password_link_expiry_duration")
			)
			if (
		...
		# verify old password
		nxenv.local.login_manager.check_password(nxenv.session.user, old_password)
...
def reset_user_data(user):
	user_doc = nxenv.get_doc("User", user)
...
@nxenv.whitelist(methods=["POST"])
def verify_password(password):
	nxenv.local.login_manager.check_password(nxenv.session.user, password)


@nxenv.whitelist(allow_guest=True)
def sign_up(email: str, full_name: str, redirect_to: str) -> tuple[int, str]:
	if is_signup_disabled():
		nxenv.throw(_("Sign Up is disabled"), title=_("Not Allowed"))

	user = nxenv.db.get("User", {"email": email})
	if user:
		if user.enabled:
		...
		if nxenv.db.get_creation_count("User", 60) > 300:
		...
		from nxenv.utils import random_string
		...
		default_role = nxenv.db.get_single_value("Portal Settings", "default_role")
		if default_role:
			user.add_roles(default_role)

		if redirect_to:
			nxenv.cache.hset("redirect_after_login", user.name, redirect_to)
...
def reset_password(user: str) -> str:
	try:
		user: User = nxenv.get_doc("User", user)
		...
		nxenv.local.response["http_status_code"] = 404
...
def user_query(doctype, txt, searchfield, start, page_len, filters):
	...
	if filters:
		if not (filters.get("ignore_user_type") and nxenv.session.data.user_type == "System User"):
...
def get_website_users():
	...
	return nxenv.db.count("User", filters={"enabled": True, "user_type": "Website User"})
...
def get_permission_query_conditions(user):
	if user == "Administrator":
		...
		return """(`tabUser`.name not in ({standard_users}))""".format(
			standard_users=", ".join(nxenv.db.escape(user) for user in STANDARD_USERS)
		)
...
def notify_admin_access_to_system_manager(login_manager=None):
	if (
		...
		and nxenv.local.conf.notify_admin_access_to_system_manager
	):
		site = '<a href="{0}" target="_blank">{0}</a>'.format(nxenv.local.request.host_url)
		date_and_time = "<b>{}</b>".format(format_datetime(now_datetime(), format_string="medium"))
		ip_address = nxenv.local.request_ip
...
def handle_password_test_fail(feedback: dict):
	...
	nxenv.throw(msg=" ".join([warning, *suggestions]), title=_("Invalid Password"))


def update_gravatar(name):
	gravatar = has_gravatar(name)
	if gravatar:
		nxenv.db.set_value("User", name, "user_image", gravatar)
...
def throttle_user_creation():
	...
	if nxenv.db.get_creation_count("User", 60) > nxenv.local.conf.get("throttle_user_limit", 60):
		nxenv.throw(_("Throttled"))


@nxenv.whitelist()
def get_module_profile(module_profile: str):
	module_profile = nxenv.get_doc("Module Profile", {"module_profile_name": module_profile})
...
def create_contact(user, ignore_links=False, ignore_mandatory=False):
	...
	if not contact_name:
		...
		except nxenv.DuplicateEntryError:
			pass
	else:
		try:
			contact = nxenv.get_doc("Contact", contact_name)
		...
		except nxenv.TimestampMismatchError:
			raise nxenv.RetryBackgroundJobError
...
def generate_keys(user: str):
	...
	user_details: User = nxenv.get_doc("User", user)
	api_secret = nxenv.generate_hash(length=15)
	# if api key is not set generate api key
	if not user_details.api_key:
		api_key = nxenv.generate_hash(length=15)
...
def switch_theme(theme):
	if theme in ["Dark", "Light", "Automatic"]:
		nxenv.db.set_value("User", nxenv.session.user, "desk_theme", theme)


def get_enabled_users():
	def _get_enabled_users():
		enabled_users = nxenv.get_all("User", filters={"enabled": "1"}, pluck="name")
		return enabled_users

	return nxenv.cache.get_value("enabled_users", _get_enabled_users)
...
def impersonate(user: str, reason: str):
	...
	impersonator = nxenv.session.user
	nxenv.get_doc(
		{
