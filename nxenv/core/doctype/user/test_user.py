...
from nxenv.core.doctype.user.user import (
	User,
...
from nxenv.desk.notifications import extract_mentions
...
user_module = nxenv.core.doctype.user.user
...
class TestUser(IntegrationTestCase):
	def tearDown(self):
		...
		nxenv.db.set_single_value("System Settings", "enable_password_policy", 0)
		nxenv.db.set_single_value("System Settings", "minimum_password_score", "")
	...
	def test_user_type(self):
		user_id = nxenv.generate_hash() + "@example.com"
		new_user = nxenv.get_doc(doctype="User", email=user_id, first_name="Tester").insert()
		self.assertEqual(new_user.user_type, "Website User")

		# social login userid for nxenv
		self.assertTrue(new_user.social_logins[0].userid)
		self.assertEqual(new_user.social_logins[0].provider, "nxenv")
		...
		nxenv.delete_doc("User", new_user.name)

	def test_delete(self):
		nxenv.get_doc("User", "test@example.com").add_roles("_Test Role 2")
		self.assertRaises(nxenv.LinkExistsError, delete_doc, "Role", "_Test Role 2")
		nxenv.db.delete("Has Role", {"role": "_Test Role 2"})
		delete_doc("Role", "_Test Role 2")

		if nxenv.db.exists("User", "_test@example.com"):
			delete_contact("_test@example.com")
			delete_doc("User", "_test@example.com")

		user = nxenv.copy_doc(self.globalTestRecords["User"][1])
		user.email = "_test@example.com"
		user.insert()

		nxenv.get_doc({"doctype": "ToDo", "description": "_Test"}).insert()

		delete_contact("_test@example.com")
		delete_doc("User", "_test@example.com")

		self.assertTrue(
			not nxenv.db.sql("""select * from `tabToDo` where allocated_to=%s""", ("_test@example.com",))
		)

		nxenv.copy_doc(self.globalTestRecords["Role"][1]).insert()

	def test_get_value(self):
		self.assertEqual(nxenv.db.get_value("User", "test@example.com"), "test@example.com")
		self.assertEqual(nxenv.db.get_value("User", {"email": "test@example.com"}), "test@example.com")
		self.assertEqual(
			nxenv.db.get_value("User", {"email": "test@example.com"}, "email"), "test@example.com"
		)
		self.assertEqual(
			nxenv.db.get_value("User", {"email": "test@example.com"}, ["first_name", "email"]),
		...
		test_user = nxenv.db.sql("select * from tabUser where name='test@example.com'", as_dict=True)[0]
		self.assertEqual(
			nxenv.db.get_value("User", {"email": "test@example.com"}, "*", as_dict=True), test_user
		)

		self.assertEqual(nxenv.db.get_value("User", "xxxtest@example.com"), None)

		nxenv.db.set_single_value("Website Settings", "_test", "_test_val")
		self.assertEqual(nxenv.db.get_value("Website Settings", None, "_test"), "_test_val")
		self.assertEqual(nxenv.db.get_value("Website Settings", "Website Settings", "_test"), "_test_val")

	def test_high_permlevel_validations(self):
		user = nxenv.get_meta("User")
		self.assertTrue("roles" in [d.fieldname for d in user.get_high_permlevel_fields()])

		me = nxenv.get_doc("User", "testperm@example.com")
	...
	def test_delete_user(self):
		...
		delete_contact(new_user.name)
		nxenv.delete_doc("User", new_user.name)
		self.assertFalse(nxenv.db.exists("User", new_user.name))
	...
	def test_password_strength(self):
		...
		with change_settings("System Settings", enable_password_policy=1, minimum_password_score=2):
			...
			self.assertRaises(
				nxenv.exceptions.ValidationError, handle_password_test_fail, result["feedback"]
			)
			self.assertRaises(
				nxenv.exceptions.ValidationError, handle_password_test_fail, result
			...
			self.assertRaises(nxenv.exceptions.ValidationError, user.save)
			user.reload()
	...
	def test_comment_mentions(self):
		...
		nxenv.delete_doc("User Group", "Team")
		doc = nxenv.get_doc(
			{
				"doctype": "User Group",
				"name": "Team",
	...
	def test_rate_limiting_for_reset_password(self):
		url = get_url()
		data = {"cmd": "nxenv.core.doctype.user.user.reset_password", "user": "test@test.com"}

		# Clear rate limit tracker to start fresh
		key = f"rl:{data['cmd']}:{data['user']}"
		nxenv.cache.delete(key)
	...
	def test_user_rename(self):
		...
		nxenv.rename_doc("User", user.name, new_name)
		self.assertTrue(nxenv.db.exists("Notification Settings", new_name))

		nxenv.delete_doc("User", new_name)

	def test_signup(self):
		import nxenv.website.utils

		random_user = nxenv.mock("email")
		random_user_name = nxenv.mock("name")
		# disabled signup
		with patch.object(user_module, "is_signup_disabled", return_value=True):
			self.assertRaisesRegex(
				nxenv.exceptions.ValidationError,
				"Sign Up is disabled",
				sign_up,
				random_user,
				random_user_name,
		...
		self.assertEqual(nxenv.cache.hget("redirect_after_login", random_user), "/welcome")

		# re-register
		self.assertTupleEqual(sign_up(random_user, random_user_name, "/welcome"), (0, "Already Registered"))

		# disabled user
		user = nxenv.get_doc("User", random_user)
		user.enabled = 0
		user.save()
		...
		with patch.object(user_module.nxenv.db, "get_creation_count", return_value=301):
			self.assertRaisesRegex(
				nxenv.exceptions.ValidationError,
				"Throttled",
				sign_up,
	...
	def test_reset_password(self):
		...
		nxenv.local.request_ip = "127.0.0.69"

		nxenv.set_user("testpassword@example.com")
		test_user = nxenv.get_doc("User", "testpassword@example.com")
		...
		self.assertRaises(nxenv.exceptions.AuthenticationError, verify_password, old_password)
		...
		# test API endpoint
		with patch.object(user_module.nxenv, "sendmail") as sendmail:
			nxenv.clear_messages()
			test_user = nxenv.get_doc("User", "test2@example.com")
...
class TestImpersonation(NxenvAPITestCase):
	def test_impersonation(self):
		with test_user(roles=["System Manager"], commit=True) as user:
			self.post(
				self.method("nxenv.core.doctype.user.user.impersonate"),
				{"user": user.name, "reason": "test", "sid": self.sid},
			)
			resp = self.get(self.method("nxenv.auth.get_logged_user"))
...
def test_user(
	*, first_name: str | None = None, email: str | None = None, roles: list[str], commit=False, **kwargs
	...
	try:
		first_name = first_name or nxenv.generate_hash()
		email = email or (first_name + "@example.com")
		user: User = nxenv.new_doc(
			"User",
		...
		commit and nxenv.db.commit()


def delete_contact(user):
	nxenv.db.delete("Contact", {"email_id": user})
	nxenv.db.delete("Contact Email", {"email_id": user})
