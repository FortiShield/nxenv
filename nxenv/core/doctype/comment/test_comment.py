...
import nxenv
from nxenv.templates.includes.comments.comments import add_comment
from nxenv.tests import IntegrationTestCase, UnitTestCase
from nxenv.tests.test_model_utils import set_user
...
class TestComment(IntegrationTestCase):
	def test_comment_creation(self):
		test_doc = nxenv.get_doc(doctype="ToDo", description="test")
		test_doc.insert()
	...
	def test_public_comment(self):
		...
		nxenv.db.delete("Comment", {"reference_doctype": "Blog Post"})
		add_comment_args = {
	...
	def test_user_not_logged_in(self):
		some_system_user = nxenv.db.get_value("User", {"name": ("not in", nxenv.STANDARD_USERS)})
