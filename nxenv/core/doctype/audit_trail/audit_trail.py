...
from nxenv.utils import compare
...
class AuditTrail(Document):
	...
	def validate_fields(self):
		...
		for field in fields_dict:
			if not fields_dict[field]:
				nxenv.throw(_("{} field cannot be empty.").format(nxenv.bold(field)))

	def validate_document(self):
		if not nxenv.db.exists(self.doctype_name, self.document):
			nxenv.throw(
				_("The selected document {0} is not a {1}.").format(
					nxenv.bold(self.document), nxenv.bold(self.doctype_name)
				)
			)
	...
	def compare_document(self):
		...
		self.amended_docs = [nxenv.get_doc(self.doctype_name, name) for name in amended_document_names]
	...
	def get_amended_documents(self):
		...
		creation = nxenv.db.get_value(self.doctype_name, self.document, "creation")
		while (
			curr_doc
			...
			curr_doc = nxenv.db.get_value(self.doctype_name, curr_doc, "amended_from")
			creation = nxenv.db.get_value(self.doctype_name, curr_doc, "creation")
...
def filter_fields_for_gridview(row):
	grid_row = {}
	meta = nxenv.get_meta(row.doctype)
	for field in meta.fields:
