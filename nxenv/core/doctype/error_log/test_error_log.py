...
from nxenv.utils.error import _is_ldap_exception, guess_exception_source
...
_RAW_EXC = """
   File "apps/nxenv/nxenv/model/document.py", line 1284, in runner
     add_to_return_value(self, fn(self, *args, **kwargs))
                               ^^^^^^^^^^^^^^^^^^^^^^^^^
   File "apps/nxenv/nxenv/model/document.py", line 933, in fn
...
_THROW_EXC = """
   ...
   File "apps/erpnext/erpnext/selling/doctype/sales_order/sales_order.py", line 58, in onload
     nxenv.throw("what")
   File "apps/nxenv/nxenv/__init__.py", line 550, in throw
     msgprint(
   File "apps/nxenv/nxenv/__init__.py", line 518, in msgprint
     _raise_exception()
   File "apps/nxenv/nxenv/__init__.py", line 467, in _raise_exception
     raise raise_exception(msg)
 nxenv.exceptions.ValidationError: what
"""
...
class TestExceptionSourceGuessing(IntegrationTestCase):
	@patch.object(nxenv, "get_installed_apps", return_value=["nxenv", "erpnext", "3pa"])
