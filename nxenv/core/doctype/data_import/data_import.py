...
import nxenv
from nxenv import _
from nxenv.core.doctype.data_import.exporter import Exporter
from nxenv.core.doctype.data_import.importer import Importer
from nxenv.model import core_doctypes_list
from nxenv.model.document import Document
from nxenv.modules.import_file import import_file_by_path
from nxenv.utils.background_jobs import enqueue, is_job_enqueued
from nxenv.utils.csvutils import validate_google_sheets_url
...
class DataImport(Document):
	...
	def set_delimiters_flag(self):
		if self.import_file:
			nxenv.flags.delimiter_options = self.delimiter_options or ","

	def validate_doctype(self):
		if self.reference_doctype in BLOCKED_DOCTYPES:
			nxenv.throw(_("Importing {0} is not allowed.").format(self.reference_doctype))
	...
	def start_import(self):
		...
		run_now = nxenv.flags.in_test or nxenv.conf.developer_mode
		if is_scheduler_inactive() and not run_now:
			nxenv.throw(_("Scheduler is inactive. Cannot import data."), title=_("Scheduler Inactive"))
...
def get_preview_from_template(data_import, import_file=None, google_sheets_url=None):
	return nxenv.get_doc("Data Import", data_import).get_preview_from_template(
...
def form_start_import(data_import: str):
	return nxenv.get_doc("Data Import", data_import).start_import()


def start_import(data_import):
	"""This method runs in background job"""
	data_import = nxenv.get_doc("Data Import", data_import)
	try:
	...
	nxenv.publish_realtime("data_import_refresh", {"data_import": data_import.name})
...
def download_template(doctype, export_fields=None, export_records=None, export_filters=None, file_type="CSV"):
	...
	export_fields = nxenv.parse_json(export_fields)
	export_filters = nxenv.parse_json(export_filters)
...
def download_errored_template(data_import_name):
	data_import = nxenv.get_doc("Data Import", data_import_name)
...
def get_import_status(data_import_name):
	...
	total_payload_count = nxenv.db.get_value("Data Import", data_import_name, "payload_count")
...
def get_import_logs(data_import: str):
	doc = nxenv.get_doc("Data Import", data_import)
...
def import_file(doctype, file_path, import_type, submit_after_import=False, console=False):
	...
	data_import = nxenv.new_doc("Data Import")
...
def export_json(doctype, path, filters=None, or_filters=None, name=None, order_by="creation asc"):
	...
	out = []
	if name:
		out.append(nxenv.get_doc(doctype, name).as_dict())
	elif nxenv.db.get_value("DocType", doctype, "issingle"):
		out.append(nxenv.get_doc(doctype).as_dict())
	else:
		for doc in nxenv.get_all(
			doctype,
			fields=["name"],
			filters=filters,
			...
			out.append(nxenv.get_doc(doctype, doc.name).as_dict())
	post_process(out)
	...
	with open(path, "w") as outfile:
		outfile.write(nxenv.as_json(out, ensure_ascii=False))


def export_csv(doctype, path):
	from nxenv.core.doctype.data_export.exporter import export_data

	with open(path, "wb") as csvfile:
		export_data(doctype=doctype, all_doctypes=True, template=True, with_data=True)
		csvfile.write(nxenv.response.result.encode("utf-8"))
