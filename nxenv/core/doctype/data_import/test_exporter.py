...
from nxenv.core.doctype.data_import.test_importer import create_doctype_if_not_exists
...
class TestExporter(IntegrationTestCase):
	...
	def test_exports_specified_fields(self):
		if not nxenv.db.exists(doctype_name, "Test"):
			doc = nxenv.get_doc(
			...
			doc = nxenv.get_doc(doctype_name, "Test")
	...
	def test_export_csv_response(self):
		...
		self.assertEqual(nxenv.response["doctype"], doctype_name)
