...
import nxenv
from nxenv import _
from nxenv.model import display_fieldtypes, no_value_fields
from nxenv.model import table_fields as table_fieldtypes
from nxenv.utils import flt, format_duration, groupby_metric
from nxenv.utils.csvutils import build_csv_response
from nxenv.utils.xlsxutils import build_xlsx_response
...
class Exporter:
	...
	def serialize_exportable_fields(self):
		...
		for key, exportable_fields in self.exportable_fields.items():
			for _df in exportable_fields:
				...
				if isinstance(_df, nxenv.core.doctype.docfield.docfield.DocField):
					df = _df.as_dict()
				else:
					df = _df.copy()
	...
	def get_data_to_export(self):
		...
		if not nxenv.permissions.can_export(self.doctype):
			if nxenv.permissions.can_export(self.doctype, is_owner=True):
				for doc in data:
					if doc.get("owner") != nxenv.session.user:
		...
		for i, df in enumerate(self.fields):
			if df.parent == doctype:
				...
				if df.fieldtype == "Text Editor" and value:
					value = nxenv.core.utils.html2text(value)
				row[i] = value
		return rows
