nxenv.listview_settings["DocType"] = {
	...
	new_doctype_dialog(args) {
		...
		let non_developer = nxenv.session.user !== "Administrator" || !nxenv.boot.developer_mode;
		let fields = [
			{
				label: __("Name"),
				fieldname: "name",
		...
		let new_d = new nxenv.ui.Dialog({
			...
			primary_action(values) {
				...
				nxenv.db
					...
					.then((doc) => {
						nxenv.set_route("Form", "DocType", doc.name);
					});
			},
			secondary_action_label: __("Cancel"),
			secondary_action() {
				new_d.hide();
				if (nxenv.get_route()[0] === "Form") {
