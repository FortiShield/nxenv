...
import nxenv
from nxenv import _
from nxenv.cache_manager import clear_controller_cache, clear_user_cache
...
from nxenv.database.schema import validate_column_length, validate_column_name
from nxenv.desk.notifications import delete_notification_count_for, get_filters_for
from nxenv.desk.utils import validate_route_conflict
from nxenv.model import (
	child_table_fields,
	data_field_options,
	default_fields,
	no_value_fields,
...
from nxenv.model.docfield import supports_translation
from nxenv.model.document import Document
from nxenv.model.meta import Meta
from nxenv.modules import get_doc_path, make_boilerplate
from nxenv.modules.import_file import get_file_path
from nxenv.permissions import ALL_USER_ROLE, AUTOMATIC_ROLES, SYSTEM_USER_ROLE
from nxenv.query_builder.functions import Concat
from nxenv.utils import cint, flt, get_datetime, is_a_property, random_string
from nxenv.website.utils import clear_cache

if TYPE_CHECKING:
	from nxenv.custom.doctype.customize_form.customize_form import CustomizeForm
...
class InvalidFieldNameError(nxenv.ValidationError):
	pass


class UniqueFieldnameError(nxenv.ValidationError):
	pass


class IllegalMandatoryError(nxenv.ValidationError):
	pass


class DoctypeLinkError(nxenv.ValidationError):
	pass


class WrongOptionsDoctypeLinkError(nxenv.ValidationError):
	pass


class HiddenAndMandatoryWithoutDefaultError(nxenv.ValidationError):
	pass


class NonUniqueError(nxenv.ValidationError):
	pass


class CannotIndexedError(nxenv.ValidationError):
	pass


class CannotCreateStandardDoctypeError(nxenv.ValidationError):
	pass
...
class DocType(Document):
	...
	if TYPE_CHECKING:
		from nxenv.core.doctype.docfield.docfield import DocField
		from nxenv.core.doctype.docperm.docperm import DocPerm
		from nxenv.core.doctype.doctype_action.doctype_action import DocTypeAction
		from nxenv.core.doctype.doctype_link.doctype_link import DocTypeLink
		from nxenv.core.doctype.doctype_state.doctype_state import DocTypeState
	...
	def validate(self):
		...
		if not self.is_new():
			self.before_update = nxenv.get_doc("DocType", self.name)
		...
		if self.default_print_format and not self.custom:
			nxenv.throw(_("Standard DocType cannot have default print format, use Customize Form"))
	...
	def check_indexing_for_dashboard_links(self):
		...
		for d in self.fields:
			if d.fieldtype == "Link" and not d.unique and not d.search_index:
				referred_as_link = nxenv.db.exists(
					"DocType Link",
	...
	def check_developer_mode(self):
		...
		if nxenv.flags.in_patch or nxenv.flags.in_test:
			return

		if not nxenv.conf.get("developer_mode") and not self.custom:
			nxenv.throw(
		...
		if self.is_virtual and self.custom:
			nxenv.throw(_("Not allowed to create custom Virtual DocType."), CannotCreateStandardDoctypeError)

		if nxenv.conf.get("developer_mode"):
	...
	def setup_fields_to_fetch(self):
		...
		try:
			old_meta = nxenv.get_meta(nxenv.get_doc("DocType", self.name), cached=False)
			old_fields_to_fetch = [df.fieldname for df in old_meta.get_fields_to_fetch()]
		except nxenv.DoesNotExistError:
			old_fields_to_fetch = []

		new_meta = nxenv.get_meta(self, cached=False)
	...
	def validate_website(self):
		...
		if self.has_web_view:
			...
			if "route" not in [d.fieldname for d in self.fields]:
				nxenv.throw(_('Field "route" is mandatory for Web Views'), title="Missing Field")
	...
	def validate_virtual_doctype_methods(self):
		...
		from nxenv.model.virtual_doctype import validate_controller
	...
	def ensure_minimum_max_attachment_limit(self):
		...
		from nxenv.model import attachment_fieldtypes
		...
		if total_attach_fields > self.max_attachments:
			...
			field_label = nxenv.bold(self.meta.get_field("max_attachments").label)
			nxenv.msgprint(
	...
	def change_modified_of_parent(self):
		...
		if nxenv.flags.in_import:
			return
		parent_list = nxenv.get_all(
			"DocField", "parent", dict(fieldtype=["in", nxenv.model.table_fields], options=self.name)
		)
		for p in parent_list:
			nxenv.db.set_value("DocType", p.parent, {})
	...
	def on_update(self):
		...
		try:
			nxenv.db.updatedb(self.name, Meta(None, bootstrap=self))
		...
		allow_doctype_export = (
			...
			and (nxenv.conf.developer_mode or nxenv.flags.allow_doctype_export)
		)
		...
		nxenv.clear_cache(doctype=self.name)

		# clear user cache so that on the next reload this doctype is included in boot
		clear_user_cache(nxenv.session.user)

		if not nxenv.flags.in_install and hasattr(self, "before_update"):
	...
	def setup_autoincrement_and_sequence(self):
		...
		name_type = f"varchar({nxenv.db.VARCHAR_LEN})"

		if self.autoname == "autoincrement":
			name_type = "bigint"
			nxenv.db.create_sequence(self.name, check_not_exists=True)
	...
	def sync_global_search(self):
		...
		if set(global_search_fields_before_update) != set(global_search_fields_after_update):
			now = (not nxenv.request) or nxenv.flags.in_test or nxenv.flags.in_install
			nxenv.enqueue("nxenv.utils.global_search.rebuild_for_doctype", now=now, doctype=self.name)
	...
	def set_base_class_for_controller(self):
		...
		scrubbed_name = nxenv.scrub(self.name)
		scrubbed_module = nxenv.scrub(self.module)
		controller_path = nxenv.get_module_path(
			scrubbed_module, "doctype", scrubbed_name, f"{scrubbed_name}.py"
		)

		document_cls_tag = f"class {despaced_name}(Document)"
		document_import_tag = "from nxenv.model.document import Document"
		website_generator_cls_tag = f"class {despaced_name}(WebsiteGenerator)"
		website_generator_import_tag = "from nxenv.website.website_generator import WebsiteGenerator"
	...
	def before_rename(self, old, new, merge=False):
		...
		if not self.custom and nxenv.session.user != "Administrator":
			nxenv.throw(_("DocType can only be renamed by Administrator"))

		self.check_developer_mode()
		self.validate_name(new)

		if merge:
			nxenv.throw(_("DocType can not be merged"))
	...
	def after_rename(self, old, new, merge=False):
		...
		if self.issingle:
			nxenv.db.sql("""update tabSingles set doctype=%s where doctype=%s""", (new, old))
			nxenv.db.sql(
	...
	def rename_files_and_folders(self, old, new):
		...
		for fname in os.listdir(new_path):
			if nxenv.scrub(old) in fname:
				old_file_name = os.path.join(new_path, fname)
				new_file_name = os.path.join(new_path, fname.replace(nxenv.scrub(old), nxenv.scrub(new)))
				shutil.move(old_file_name, new_file_name)

		self.rename_inside_controller(new, old, new_path)
		nxenv.msgprint(_("Renamed files and replaced code in controllers, please check!"))

	def rename_inside_controller(self, new, old, new_path):
		for fname in ("{}.js", "{}.py", "{}_list.js", "{}_calendar.js", "test_{}.py", "test_{}.js"):
			fname = os.path.join(new_path, fname.format(nxenv.scrub(new)))
		...
		doctype_json_path = os.path.join(new_path, f"{nxenv.scrub(new)}.json")
		current_data = nxenv.get_file_json(doctype_json_path)
	...
	def preserve_naming_series_options_in_property_setter(self):
		...
		if not (
			nxenv.db.table_exists(self.name)
			and nxenv.get_all(self.name, fields=["name"], limit=1, as_list=True)
		):
			return

		existing_property_setter = nxenv.db.get_value(
	...
	def make_controller_template(self):
		...
		if self.has_web_view:
			templates_path = nxenv.get_module_path(
				nxenv.scrub(self.module), "doctype", nxenv.scrub(self.name), "templates"
			)
	...
	def export_types_to_controller(self):
		...
		from nxenv.types.exporter import TypeExporter
		...
		if any(nxenv.get_hooks("export_python_type_annotations", app_name=app)):
	...
	def make_repeatable(self):
		...
		if self.allow_auto_repeat:
			if not nxenv.db.exists(
				"Custom Field", {"fieldname": "auto_repeat", "dt": self.name}
			) and not nxenv.db.exists("DocField", {"fieldname": "auto_repeat", "parent": self.name}):
	...
	def validate_nestedset(self):
		...
		if not self.nsm_parent_field:
			field_label = nxenv.bold(_("Parent Field (Tree)"))
			nxenv.throw(_("{0} is a mandatory field").format(field_label), nxenv.MandatoryError)

		# check if field is valid
		fieldnames = [df.fieldname for df in self.fields]
		if self.nsm_parent_field and self.nsm_parent_field not in fieldnames:
			nxenv.throw(_("Parent Field must be a valid fieldname"), InvalidFieldNameError)
	...
	def add_nestedset_fields(self):
		...
		parent_field_name = nxenv.scrub(parent_field_label)
		self.append(
			"fields",
			{
	...
	def get_max_idx(self):
		...
		max_idx = nxenv.db.sql("""select max(idx) from `tabDocField` where parent = %s""", self.name)
	...
	def validate_name(self, name=None):
		...
		max_length = nxenv.db.MAX_COLUMN_LENGTH - 3
		...
		if WHITESPACE_PADDING_PATTERN.search(name):
			nxenv.throw(_("DocType's name should not start or end with whitespace"), nxenv.NameError)
	...
	def check_pending_migration(self) -> bool:
		...
		file = Path(get_file_path(nxenv.scrub(self.module), self.doctype, self.name))
		content = json.loads(file.read_text())
		if content.get("modified") and get_datetime(self.modified) < get_datetime(content.get("modified")):
			nxenv.msgprint(
				_(
					"This doctype has pending migrations, run 'nxcli migrate' before modifying the doctype to avoid losing changes."
				),
				alert=True,
				indicator="yellow",
			)
...
def validate_series(dt, autoname=None, name=None):
	...
	if not autoname and dt.get("fields", {"fieldname": "naming_series"}):
		...
		if not dt.get("fields", {"fieldname": fieldname}):
			nxenv.throw(
				_("Fieldname called {0} must exist to enable autonaming").format(nxenv.bold(fieldname)),
	...
	if autoname and autoname.startswith("field:"):
		...
		if not field or field not in [df.fieldname for df in dt.fields]:
			nxenv.throw(_("Invalid fieldname '{0}' in autoname").format(field))
		else:
			for df in dt.fields:
	...
	if autoname and autoname.startswith("format:"):
		from nxenv.model.naming import BRACED_PARAMS_HASH_PATTERN

		if len(BRACED_PARAMS_HASH_PATTERN.findall(autoname)) > 1:
			nxenv.throw(_("Only one set of {#} pattern is allowed in the format string"))
	...
	if (
		...
		if used_in:
			nxenv.throw(_("Series {0} already used in {1}").format(prefix, used_in[0][0]))
...
def validate_empty_name(dt, autoname):
	...
	if not autoname and not (dt.issingle or dt.istable):
		...
		if not controller or (not hasattr(controller, "autoname")):
			nxenv.toast(_("Warning: Naming is not set"), indicator="yellow")
...
def validate_autoincrement_autoname(dt: Union[DocType, "CustomizeForm"]) -> bool:
	...
	def get_autoname_before_save(dt: Union[DocType, "CustomizeForm"]) -> str:
		if dt.doctype == "Customize Form":
			property_value = nxenv.db.get_value(
			...
			if not property_value:
				return nxenv.db.get_value("DocType", dt.doc_type, "autoname") or ""
	...
	if not dt.is_new():
		...
		if (is_autoname_autoincrement and autoname_before_save != "autoincrement") or (
			...
			if dt.doctype == "Customize Form":
				nxenv.throw(_("Cannot change to/from autoincrement autoname in Customize Form"))

			if nxenv.get_meta(dt.name).issingle:
				return False

			if not nxenv.get_all(dt.name, limit=1):
...
def change_name_column_type(doctype_name: str, type: str) -> None:
	...
	nxenv.db.change_column_type(*args)


def validate_links_table_fieldnames(meta):
	"""Validate fieldnames in Links table"""
	if not meta.links or nxenv.flags.in_patch or nxenv.flags.in_fixtures or nxenv.flags.in_migrate:
		return
	...
	for index, link in enumerate(meta.links, 1):
		...
		if not link.parent_doctype:
			...
			nxenv.throw(message, nxenv.ValidationError, _("Parent Missing"))

		if not link.table_fieldname:
			message = _("Document Links Row #{0}: Table Fieldname is mandatory for internal links").format(
				index
			)
			nxenv.throw(message, nxenv.ValidationError, _("Table Fieldname Missing"))

		if meta.name == link.parent_doctype:
			field_exists = link.table_fieldname in fieldnames
		else:
			field_exists = nxenv.get_meta(link.parent_doctype).has_field(link.table_fieldname)

		if not field_exists:
			message = _("Document Links Row #{0}: Could not find field {1} in {2} DocType").format(
				index, nxenv.bold(link.table_fieldname), nxenv.bold(meta.name)
			)
			nxenv.throw(message, nxenv.ValidationError, _("Invalid Table Fieldname"))
...
def _test_connection_query(doctype, field, idx):
	...
	try:
		nxenv.get_all(doctype, filters=filters, limit=1, distinct=True, ignore_ifnull=True)
	except Exception as e:
		nxenv.clear_last_message()
		msg = _("Document Links Row #{0}: Invalid doctype or fieldname.").format(idx)
		msg += "<br>" + str(e)
		nxenv.throw(msg, InvalidFieldNameError)


def validate_fields_for_doctype(doctype):
	meta = nxenv.get_meta(doctype, cached=False)
...
def validate_fields(meta: Meta):
	...
	:param meta: `nxenv.model.meta.Meta` object to check."""
	...
	def check_link_table_options(docname, d):
		if nxenv.flags.in_patch or nxenv.flags.in_fixtures:
			return
		...
		if d.fieldtype in ("Link", *table_fields):
			...
			if d.options != d.parent:
				options = nxenv.db.get_value("DocType", d.options, "name")
				if not options:
					nxenv.throw(
	...
	def check_hidden_and_mandatory(docname, d):
		if d.hidden and d.reqd and not d.default and not nxenv.flags.in_migrate:
			nxenv.throw(
	...
	def check_width(d):
		if d.fieldtype == "Currency" and cint(d.width) < 100:
			nxenv.throw(_("Max width for type Currency is 100px in row {0}").format(d.idx))
	...
	def check_precision(d):
		if (
			...
			nxenv.throw(_("Precision should be between 1 and 6"))
	...
	def check_unique_and_text(docname, d):
		...
		if getattr(d, "unique", False):
			...
			if not d.get("__islocal") and nxenv.db.has_column(d.parent, d.fieldname):
				has_non_unique_values = nxenv.db.sql(
	...
	def check_fold(fields):
		...
		for i, f in enumerate(fields):
			if f.fieldtype == "Fold":
				if fold_exists:
					nxenv.throw(_("There can be only one Fold in a form"))
				fold_exists = True
				if i < len(fields) - 1:
					nxt = fields[i + 1]
					if nxt.fieldtype != "Section Break":
						nxenv.throw(_("Fold must come before a Section Break"))
				else:
					nxenv.throw(_("Fold can not be at the end of the form"))
	...
	def check_search_fields(meta, fields):
		...
		for fieldname in search_fields:
			...
			if (fieldtype_mapper.get(fieldname) in no_value_fields) or (fieldname not in fieldname_list):
				nxenv.throw(_("Search field {0} is not valid").format(fieldname))
	...
	def check_title_field(meta):
		...
		if meta.title_field not in fieldname_list:
			nxenv.throw(_("Title field must be a valid fieldname"), InvalidFieldNameError)
	...
	def check_image_field(meta):
		...
		if not df:
			nxenv.throw(_("Image field must be a valid fieldname"), InvalidFieldNameError)
		if df[0].fieldtype != "Attach Image":
			nxenv.throw(_("Image field must be of type Attach Image"), InvalidFieldNameError)
	...
	def check_is_published_field(meta):
		...
		if meta.is_published_field not in fieldname_list:
			nxenv.throw(_("Is Published Field must be a valid fieldname"), InvalidFieldNameError)
	...
	def check_website_search_field(meta):
		...
		if meta.website_search_field not in fieldname_list:
			nxenv.throw(_("Website Search Field must be a valid fieldname"), InvalidFieldNameError)
	...
	def check_timeline_field(meta):
		...
		if meta.timeline_field not in fieldname_list:
			nxenv.throw(_("Timeline field must be a valid fieldname"), InvalidFieldNameError)

		df = meta.get("fields", {"fieldname": meta.timeline_field})[0]
		if df.fieldtype not in ("Link", "Dynamic Link"):
			nxenv.throw(_("Timeline field must be a Link or Dynamic Link"), InvalidFieldNameError)
	...
	def check_illegal_depends_on_conditions(docfield):
		...
		for field in depends_on_fields:
			...
			if depends_on and ("=" in depends_on) and DEPENDS_ON_PATTERN.match(depends_on):
				nxenv.throw(_("Invalid {0} condition").format(nxenv.unscrub(field)), nxenv.ValidationError)
	...
	def validate_fetch_from(field):
		...
		if source_field == field.fieldname:
			...
			nxenv.throw(msg, title=_("Recursive Fetch From"))
	...
	def validate_data_field_type(docfield):
		...
		if docfield.fieldtype == "Data" and not (docfield.oldfieldtype and docfield.oldfieldtype != "Data"):
			if docfield.options and (docfield.options not in data_field_options):
				df_str = nxenv.bold(_(docfield.label, context=docfield.parent))
				text_str = (
				...
				nxenv.msgprint(text_str + df_options_str, title="Invalid Data Field", alert=True)

	def check_child_table_option(docfield):
		if nxenv.flags.in_fixtures:
			return
		if docfield.fieldtype not in ["Table MultiSelect", "Table"]:
			return

		doctype = docfield.options
		child_doctype_meta = nxenv.get_meta(doctype)

		if not child_doctype_meta.istable:
			nxenv.throw(
				_("Option {0} for field {1} is not a child table").format(
					nxenv.bold(doctype), nxenv.bold(docfield.fieldname)
				),
	...
	def check_max_height(docfield):
		if getattr(docfield, "max_height", None) and (docfield.max_height[-2:] not in ("px", "em")):
			nxenv.throw(f"Max for {nxenv.bold(docfield.fieldname)} height must be in px, em, rem")
	...
	def check_no_of_ratings(docfield):
		if docfield.fieldtype == "Rating":
			if docfield.options and (int(docfield.options) > 10 or int(docfield.options) < 3):
				nxenv.throw(_("Options for Rating field can range from 3 to 10"))
	...
	for d in fields:
		...
		if not nxenv.flags.in_migrate or in_ci:
...
def validate_permissions_for_doctype(doctype, for_remove=False, alert=False):
	...
	doctype = nxenv.get_doc("DocType", doctype)
...
def clear_permissions_cache(doctype):
	from nxenv.cache_manager import clear_user_cache
...
def validate_permissions(doctype, for_remove=False, alert=False):
	...
	if not permissions and alert:
		nxenv.msgprint(_("No Permissions Specified"), alert=True, indicator="orange")
	...
	def check_atleast_one_set(d):
		if not d.select and not d.read and not d.write and not d.submit and not d.cancel and not d.create:
			nxenv.throw(_("{0}: No basic permissions set").format(get_txt(d)))
	...
	def check_permission_dependency(d):
		if d.cancel and not d.submit:
			nxenv.throw(_("{0}: Cannot set Cancel without Submit").format(get_txt(d)))

		if (d.submit or d.cancel or d.amend) and not d.write:
			nxenv.throw(_("{0}: Cannot set Submit, Cancel, Amend without Write").format(get_txt(d)))
		if d.amend and not d.write:
			nxenv.throw(_("{0}: Cannot set Amend without Cancel").format(get_txt(d)))
		if d.get("import") and not d.create:
			nxenv.throw(_("{0}: Cannot set Import without Create").format(get_txt(d)))
	...
	def remove_rights_for_single(d):
		...
		if d.report:
			nxenv.msgprint(_("Report cannot be set for Single types"))
			d.report = 0
			d.set("import", 0)
			d.set("export", 0)
	...
	def check_if_submittable(d):
		if d.submit and not issubmittable:
			nxenv.throw(_("{0}: Cannot set Assign Submit if not Submittable").format(get_txt(d)))
		elif d.amend and not issubmittable:
			nxenv.throw(_("{0}: Cannot set Assign Amend if not Submittable").format(get_txt(d)))

	def check_if_importable(d):
		if d.get("import") and not isimportable:
			nxenv.throw(_("{0}: Cannot set import as {1} is not importable").format(get_txt(d), doctype))

	def validate_permission_for_all_role(d):
		if nxenv.session.user == "Administrator":
			return
		...
		if doctype.custom:
			if d.role in AUTOMATIC_ROLES:
				nxenv.throw(
					...
					).format(d.idx, nxenv.bold(_(d.role))),
					title=_("Permissions Error"),
				)

			roles = [row.name for row in nxenv.get_all("Role", filters={"is_custom": 1})]
...
def make_module_and_roles(doc, perm_fieldname="permissions"):
	...
	try:
		if (
			...
			and not nxenv.db.exists("Domain", doc.restrict_to_domain)
		):
			nxenv.get_doc(doctype="Domain", domain=doc.restrict_to_domain).insert()

		if "tabModule Def" in nxenv.db.get_tables() and not nxenv.db.exists("Module Def", doc.module):
			m = nxenv.get_doc({"doctype": "Module Def", "module_name": doc.module})
			if nxenv.scrub(doc.module) in nxenv.local.module_app:
				m.app_name = nxenv.local.module_app[nxenv.scrub(doc.module)]
			else:
				m.app_name = "nxenv"
			m.flags.ignore_mandatory = m.flags.ignore_permissions = True
			if nxenv.flags.package:
				m.package = nxenv.flags.package.name
				m.custom = 1
			m.insert()
		...
		for role in list(set(roles)):
			if nxenv.db.table_exists("Role", cached=False) and not nxenv.db.exists("Role", role):
				r = nxenv.new_doc("Role")
				r.role_name = role
				r.desk_access = 1
		...
		pass
	except nxenv.db.ProgrammingError as e:
		if nxenv.db.is_table_missing(e):
			pass
		else:
			raise
...
def check_fieldname_conflicts(docfield):
	...
	doc = nxenv.get_doc({"doctype": docfield.dt})
	...
	if docfield.fieldname in method_list + property_list:
		nxenv.msgprint(msg, raise_exception=not docfield.is_virtual)


def clear_linked_doctype_cache():
	nxenv.cache.delete_value("linked_doctypes_without_ignore_user_permissions_enabled")
...
def check_email_append_to(doc):
	...
	if doc.subject_field and not subject_field:
		nxenv.throw(_("Select a valid Subject field for creating documents from Email"))
	...
	if subject_field and subject_field.fieldtype not in [
		...
		nxenv.throw(_("Subject Field type should be Data, Text, Long Text, Small Text, Text Editor"))
	...
	if doc.sender_field and not sender_field:
		nxenv.throw(_("Select a valid Sender Field for creating documents from Email"))

	if sender_field.options != "Email":
		nxenv.throw(_("Sender Field should have Email in options"))
...
def get_row_size_utilization(doctype: str) -> float:
	...
	nxenv.has_permission("DocType", throw=True)
	try:
		return flt(nxenv.db.get_row_size(doctype) / nxenv.db.MAX_ROW_SIZE_LIMIT * 100, 2)
	except Exception:
		return 0.0
