...
nxenv.ui.form.on("DocType", {
	onload: function (frm) {
		if (frm.is_new() && !frm.doc?.fields) {
			nxenv.listview_settings["DocType"].new_doctype_dialog();
		}
	...
	after_save: function (frm) {
		if (
			nxenv.form_builder &&
			nxenv.form_builder.doctype === frm.doc.name &&
			nxenv.form_builder.store
		) {
	...
	refresh: function (frm) {
		frm.set_query("role", "permissions", function (doc) {
			if (doc.custom && nxenv.session.user != "Administrator") {
				return {
					query: "nxenv.core.doctype.role.role.role_query",
					filters: [["Role", "name", "!=", "All"]],
				};
			}
		});

		if (nxenv.session.user !== "Administrator" || !nxenv.boot.developer_mode) {
			if (frm.is_new()) {
		...
		if (!frm.is_new() && !frm.doc.istable) {
			...
			frm.add_custom_button(button_text, () => {
				window.open(`/app/${nxenv.router.slug(frm.doc.name)}`);
			});
		}

		const customize_form_link = `<a href="/app/customize-form">${__("Customize Form")}</a>`;
		if (!nxenv.boot.developer_mode && !frm.doc.custom) {
		...
		} else if (nxenv.boot.developer_mode) {
			frm.dashboard.clear_comment();
			let msg = __(
	...
	setup_default_views: (frm) => {
		nxenv.model.set_default_views_for_doctype(frm.doc.name, frm);
	},
...
function render_form_builder(frm) {
	if (nxenv.form_builder && nxenv.form_builder.doctype === frm.doc.name) {
	...
	if (nxenv.form_builder) {
		...
		nxenv.form_builder.doctype = frm.doc.name;
		nxenv.form_builder.customize = false;
