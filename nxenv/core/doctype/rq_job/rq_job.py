...
from nxenv.utils.background_jobs import get_queues, get_redis_conn
...
class RQJob(Document):
	...
	def stop_job(self):
		try:
			...
			nxenv.msgprint(_("Job is not running."), title=_("Invalid Operation"))
...
def serialize_job(job: Job) -> nxenv._dict:
	...
	if job_name == "nxenv.utils.background_jobs.run_doc_method":
	...
	return nxenv._dict(
		...
		exc_info=exc_info,
		arguments=nxenv.as_json(job.kwargs),
		timeout=job.timeout,
...
def for_current_site(job: Job) -> bool:
	return job.kwargs.get("site") == nxenv.local.site
...
@nxenv.whitelist()
def stop_job(job_id):
	nxenv.get_doc("RQ Job", job_id).stop_job()
