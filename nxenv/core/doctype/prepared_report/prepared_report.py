...
import nxenv
from nxenv.database.utils import dangerously_reconnect_on_connection_abort
from nxenv.desk.form.load import get_attachments
from nxenv.desk.query_report import generate_report_result
...
class PreparedReport(Document):
	...
	def clear_old_logs(days=30):
		prepared_reports_to_delete = nxenv.get_all(
			"Prepared Report",
			filters={"creation": ["<", nxenv.utils.add_days(nxenv.utils.now(), -days)]},
		)

		for batch in nxenv.utils.create_batch(prepared_reports_to_delete, 100):
	...
	def on_trash(self):
		...
		with suppress(Exception):
			job = nxenv.get_doc("RQ Job", self.job_id)
			job.stop_job() if self.status == "Started" else job.delete()

	def after_insert(self):
		timeout = nxenv.get_value("Report", self.report_name, "timeout")
		enqueue(
			generate_report,
			queue="long",
	...
	def get_prepared_data(self, with_file_name=False):
		if attachments := get_attachments(self.doctype, self.name):
			...
			attached_file = nxenv.get_doc("File", attachment.name)
...
def generate_report(prepared_report):
	...
	instance: PreparedReport = nxenv.get_doc("Prepared Report", prepared_report)
	report = nxenv.get_doc("Report", instance.report_name)
	...
	try:
		...
		if report.report_type == "Custom Report":
			...
			report = nxenv.get_doc("Report", reference_report)
		...
		_save_error(instance, error=nxenv.get_traceback(with_context=True))

	instance.report_end_time = nxenv.utils.now()
...
def process_filters_for_prepared_report(filters: dict[str, Any] | str) -> str:
	...
	# PS: nxenv.as_json sorts keys
	return nxenv.as_json(filters, indent=None, separators=(",", ":"))
...
def expire_stalled_report():
	nxenv.db.set_value(
		...
		{
			"status": "Failed",
			"error_message": nxenv._("Report timed out."),
		},
...
def delete_prepared_reports(reports):
	reports = nxenv.parse_json(reports)
	for report in reports:
		prepared_report = nxenv.get_doc("Prepared Report", report["name"])
...
def create_json_gz_file(data, dt, dn, report_name):
	...
	json_filename = "{}_{}.json.gz".format(
		nxenv.scrub(report_name), nxenv.utils.data.format_datetime(nxenv.utils.now(), "Y-m-d-H-M")
	)
	encoded_content = nxenv.safe_encode(nxenv.as_json(data, indent=None, separators=(",", ":")))
...
def download_attachment(dn):
	pr = nxenv.get_doc("Prepared Report", dn)
	if not pr.has_permission("read"):
		nxenv.throw(nxenv._("Cannot Download Report due to insufficient permissions"))

	data, file_name = pr.get_prepared_data(with_file_name=True)
	nxenv.local.response.filename = file_name[:-3]
	nxenv.local.response.filecontent = data
	nxenv.local.response.type = "binary"
...
def get_permission_query_condition(user):
	...
	reports = [nxenv.db.escape(report) for report in user.get_all_reports().keys()]
