...
import nxenv
from nxenv.desk.query_report import generate_report_result, get_report_doc
...
class TestPreparedReport(IntegrationTestCase):
	...
	def tearDownClass(cls):
		for r in nxenv.get_all("Prepared Report", pluck="name"):
			nxenv.delete_doc("Prepared Report", r, force=True, delete_permanently=True)

		nxenv.db.commit()

	@timeout(seconds=20)
	def wait_for_status(self, report, status):
		nxenv.db.commit()  # Flush changes first
		while True:
			nxenv.db.rollback()  # read new data
			report.reload()
	...
	def test_queueing(self):
		...
		doc = nxenv.get_last_doc("Prepared Report")
	...
	def test_start_status_and_kill_jobs(self):
		with test_report(report_type="Query Report", query="select sleep(10)") as report:
			...
			doc.delete()
			time.sleep(1)
			job = nxenv.get_doc("RQ Job", job_id)
...
def test_report(**args):
	try:
		...
		if not report.report_name:
			report.report_name = nxenv.generate_hash()
