...
nxenv.ui.form.on("Server Script", {
	setup: function (frm) {
	...
	refresh: function (frm) {
		...
		if (!frm.is_new()) {
			frm.add_custom_button(__("Compare Versions"), () => {
				new nxenv.ui.DiffView("Server Script", "script", frm.doc.name);
			});
		}

		nxenv
			.call("nxenv.core.doctype.server_script.server_script.get_autocompletion_items")
			.then((r) => r.message)
			.then((items) => {
	...
	check_safe_exec(frm) {
		nxenv.xcall("nxenv.core.doctype.server_script.server_script.enabled").then((enabled) => {
			if (enabled === false) {
				let docs_link =
					"https://nxenvframework.com/docs/user/en/desk/scripting/server-script";
...
<p>Payment processing events have a special state. See the <a href="https://github.com/nxenv/payments/blob/develop/payments/controllers/payment_controller.py">PaymentController in Nxenv Payments</a> for details.</p>
<pre>
	<code>
...
if nxenv.form_dict.message == "ping":
	nxenv.response['message'] = "pong"
else:
	nxenv.response['message'] = "ok"
</code></pre>
