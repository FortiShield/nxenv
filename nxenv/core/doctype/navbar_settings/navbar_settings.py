...
class NavbarSettings(Document):
	...
	if TYPE_CHECKING:
		from nxenv.core.doctype.navbar_item.navbar_item import NavbarItem
...
def get_app_logo():
	app_logo = nxenv.get_website_settings("app_logo") or nxenv.get_cached_value(
		"Navbar Settings",
		"Navbar Settings",
		"app_logo",
	)
	...
	if not app_logo:
		logos = nxenv.get_hooks("app_logo_url")
		app_logo = logos[0]
		if len(logos) == 2:
			app_logo = logos[1]
...
def get_navbar_settings():
	return nxenv.get_single("Navbar Settings")
...
def sync_table(key, hook):
	...
	for item in nxenv.get_hooks(hook):
