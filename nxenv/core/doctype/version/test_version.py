...
class TestVersion(IntegrationTestCase):
	def test_get_diff(self):
		...
		old_doc = nxenv.get_doc("Event", test_records[0])
	...
	def test_no_version_on_new_doc(self):
		from nxenv.desk.form.load import get_versions

		t = nxenv.get_doc(doctype="ToDo", description="something")
		t.save(ignore_version=False)

		self.assertFalse(get_versions(t))

		t = nxenv.get_doc(t.doctype, t.name)
