...
import nxenv
from nxenv import _
from nxenv.defaults import clear_default, set_default
...
class Language(Document):
	...
	def on_update(self):
		nxenv.cache.delete_value("languages_with_name")
		nxenv.client_cache.delete_value("languages")
	...
	def update_user_defaults(self):
		...
		should be updated.
		"""
		users = nxenv.get_all("User", filters={"language": self.name}, pluck="name")
...
def sync_languages():
	"""Create Language records from nxenv/geo/languages.csv"""
	from csv import DictReader

	with open(nxenv.get_app_path("nxenv", "geo", "languages.csv")) as f:
		reader = DictReader(f)
		for row in reader:
			if not nxenv.db.exists("Language", row["language_code"]):
				doc = nxenv.new_doc("Language")
				doc.update(row)
				doc.insert()
