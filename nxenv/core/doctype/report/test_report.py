...
import nxenv
from nxenv.core.doctype.user_permission.test_user_permission import create_user
from nxenv.custom.doctype.customize_form.customize_form import reset_customization
from nxenv.desk.query_report import add_total_row, run, save_report
from nxenv.desk.reportview import delete_report
from nxenv.desk.reportview import save_report as _save_report
...
class TestReport(IntegrationTestCase):
	...
	def test_report_builder(self):
		if nxenv.db.exists("Report", "User Activity Report"):
			nxenv.delete_doc("Report", "User Activity Report")

		with open(os.path.join(os.path.dirname(__file__), "user_activity_report.json")) as f:
			nxenv.get_doc(json.loads(f.read())).insert()

		report = nxenv.get_doc("Report", "User Activity Report")
	...
	def test_query_report(self):
		report = nxenv.get_doc("Report", "Permitted Documents For User")
	...
	def test_save_or_delete_report(self):
		...
		try:
			...
			nxenv.set_user("test_report_owner@example.com")
			self.assertRaises(nxenv.PermissionError, delete_report, report.name)
	...
	def test_custom_report(self):
		...
		custom_report = nxenv.get_doc("Report", custom_report_name)
		columns, result = custom_report.run_query_report(user=nxenv.session.user)

		self.assertListEqual(["email"], [column.get("fieldname") for column in columns])
		admin_dict = nxenv.core.utils.find(result, lambda d: d["name"] == "Administrator")
	...
	def test_report_permissions(self):
		nxenv.set_user("test@example.com")
		nxenv.db.delete("Has Role", {"parent": nxenv.session.user, "role": "Test Has Role"})
		nxenv.db.commit()
		if not nxenv.db.exists("Role", "Test Has Role"):
			nxenv.get_doc({"doctype": "Role", "role_name": "Test Has Role"}).insert(ignore_permissions=True)

		if not nxenv.db.exists("Report", "Test Report"):
			report = nxenv.get_doc(
				{
					"doctype": "Report",
					"ref_doctype": "User",
			...
			report = nxenv.get_doc("Report", "Test Report")
	...
	def test_report_custom_permissions(self):
		nxenv.set_user("test@example.com")
		nxenv.db.delete("Custom Role", {"report": "Test Custom Role Report"})
		nxenv.db.commit()  # nosemgrep
		if not nxenv.db.exists("Report", "Test Custom Role Report"):
			report = nxenv.get_doc(
				{
					"doctype": "Report",
					"ref_doctype": "User",
			...
			report = nxenv.get_doc("Report", "Test Custom Role Report")
	...
	def test_format_method(self):
		if nxenv.db.exists("Report", "User Activity Report Without Sort"):
			nxenv.delete_doc("Report", "User Activity Report Without Sort")
		with open(os.path.join(os.path.dirname(__file__), "user_activity_report_without_sort.json")) as f:
			nxenv.get_doc(json.loads(f.read())).insert()

		report = nxenv.get_doc("Report", "User Activity Report Without Sort")
		columns, data = report.get_data()
	...
	def test_non_standard_script_report(self):
		...
		if not nxenv.db.exists("Report", report_name):
			report = nxenv.get_doc(
				{
					"doctype": "Report",
					"ref_doctype": "User",
...
totals = {}
for user in nxenv.get_all('User', fields = ['name', 'user_type', 'creation']):
...
"""
	...
	def test_script_report_with_columns(self):
		...
		if nxenv.db.exists("Report", report_name):
			nxenv.delete_doc("Report", report_name)
...
"""
	...
	def test_toggle_disabled(self):
		...
		reports = nxenv.get_all(doctype="Report", limit=1)
		...
		doc = nxenv.get_doc("Report", report_name)
		with self.assertRaises(nxenv.exceptions.ValidationError):
	...
	def test_cte_in_query_report(self):
		...
		if nxenv.db.db_type == "mariadb":
			...
			self.assertRaises(nxenv.PermissionError, report.execute_query_report, filters={})
