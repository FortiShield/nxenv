...
from nxenv.permissions import rights
...
def execute(filters=None):
	...
	data = nxenv.get_list(doctype, fields=fields, as_list=True, user=user)

	if show_permissions:
		columns = columns + [nxenv.unscrub(right) + ":Check:80" for right in rights]
		data = list(data)
		for i, doc in enumerate(data):
			permission = nxenv.permissions.get_doc_permissions(nxenv.get_doc(doctype, doc[0]), user)
...
def get_columns_and_fields(doctype):
	...
	fields = ["name"]
	for df in nxenv.get_meta(doctype).fields:
...
def query_doctypes(doctype, txt, searchfield, start, page_len, filters):
	...
	user_perms = nxenv.utils.user.UserPermissions(user)
	...
	single_doctypes = [d[0] for d in nxenv.db.get_values("DocType", {"issingle": 1})]

	return [
		[dt]
		for dt in can_read
		if txt.lower().replace("%", "") in nxenv._(dt).lower()
