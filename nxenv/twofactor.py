...
from nxenv.utils import cint, get_datetime, get_url, time_diff_in_seconds
from nxenv.utils.background_jobs import enqueue
from nxenv.utils.password import decrypt, encrypt

PARENT_FOR_DEFAULTS = "__2fa"


def get_default(key):
	return nxenv.db.get_default(key, parent=PARENT_FOR_DEFAULTS)


def set_default(key, value):
	nxenv.db.set_default(key, value, parent=PARENT_FOR_DEFAULTS)


def clear_default(key):
	nxenv.defaults.clear_default(key, parent=PARENT_FOR_DEFAULTS)
...
def toggle_two_factor_auth(state, roles=None):
	...
	for role in roles or []:
		role = nxenv.get_doc("Role", {"role_name": role})
...
def two_factor_is_enabled(user=None):
	...
	enabled = cint(nxenv.get_system_settings("enable_two_factor_auth"))
	if enabled:
		bypass_two_factor_auth = cint(nxenv.get_system_settings("bypass_2fa_for_retricted_ip_users"))
		if bypass_two_factor_auth and user:
			...
			if restrict_ip_list and nxenv.local.request_ip:
...
def two_factor_is_enabled_for_(user):
	...
	role_doctype = nxenv.qb.DocType("Role")
	no_of_users = nxenv.db.count(
		role_doctype,
...
def get_verification_method():
	return nxenv.get_system_settings("two_factor_method")


def confirm_otp_token(login_manager, otp=None, tmp_id=None):
	"""Confirm otp matches."""
	from nxenv.auth import get_login_attempt_tracker

	if not otp:
		otp = nxenv.form_dict.get("otp")
	if not otp:
	...
	if not tmp_id:
		tmp_id = nxenv.form_dict.get("tmp_id")
	hotp_token = nxenv.cache.get(tmp_id + "_token")
	otp_secret = nxenv.cache.get(tmp_id + "_otp_secret")
	if not otp_secret:
	...
	if hotp_token:
		if hotp.verify(otp, int(hotp_token)):
			nxenv.cache.delete(tmp_id + "_token")
...
def get_verification_obj(user, token, otp_secret):
	otp_issuer = nxenv.get_system_settings("otp_issuer_name")
	verification_method = get_verification_method()
...
def process_2fa_for_sms(user, token, otp_secret):
	...
	phone = nxenv.db.get_value("User", user, ["phone", "mobile_no"], as_dict=1)
...
def get_email_subject_for_2fa(kwargs_dict):
	...
	subject_template = _("Login Verification Code from {}").format(
		nxenv.get_system_settings("otp_issuer_name")
	)
	return nxenv.render_template(subject_template, kwargs_dict)
...
def get_email_body_for_2fa(kwargs_dict):
	...
	"""
	return nxenv.render_template(body_template, kwargs_dict)
...
def get_link_for_qrcode(user, totp_uri):
	...
	key = nxenv.generate_hash(length=20)
	key_user = f"{key}_user"
	key_uri = f"{key}_uri"
	lifespan = int(nxenv.get_system_settings("lifespan_qrcode_image")) or 240
	nxenv.cache.set_value(key_uri, totp_uri, expires_in_sec=lifespan)
	nxenv.cache.set_value(key_user, user, expires_in_sec=lifespan)
...
def send_token_via_sms(otpsecret, token=None, phone_no=None):
	...
	try:
		from nxenv.core.doctype.sms_settings.sms_settings import send_request
	except Exception:
		return False
	...
	ss = nxenv.get_doc("SMS Settings", "SMS Settings")
...
def send_token_via_email(user, token, otp_secret, otp_issuer, subject=None, message=None):
	...
	user_email = nxenv.db.get_value("User", user, "email")
	if not user_email:
		return False
	hotp = pyotp.HOTP(otp_secret)
...
def create_barcode_folder():
	...
	folder = nxenv.db.exists("File", {"file_name": folder_name})
	if folder:
		return folder
	folder = nxenv.get_doc({"doctype": "File", "file_name": folder_name, "is_folder": 1, "folder": "Home"})
...
def delete_qrimage(user, check_expiry=False):
	...
	for barcode in user_barcodes:
		if check_expiry and not should_remove_barcode_image(barcode):
			continue
		barcode = nxenv.get_doc("File", barcode.name)
		nxenv.delete_doc("File", barcode.name, ignore_permissions=True)


def delete_all_barcodes_for_users():
	"""Task to delete all barcodes for user."""

	users = nxenv.get_all("User", {"enabled": 1})
	for user in users:
...
def should_remove_barcode_image(barcode):
	...
	if isinstance(barcode, str):
		barcode = nxenv.get_doc("File", barcode)
	lifespan = nxenv.get_system_settings("lifespan_qrcode_image") or 240
...
def disable():
	nxenv.db.set_single_value("System Settings", "enable_two_factor_auth", 0)


@nxenv.whitelist()
def reset_otp_secret(user: str):
	if nxenv.session.user != user:
		nxenv.only_for("System Manager", message=True)

	settings = nxenv.get_cached_doc("System Settings")
	...
	otp_issuer = settings.otp_issuer_name or "Nxenv Framework"
	user_email = nxenv.get_cached_value("User", user, "email")
	...
	nxenv.msgprint(_("OTP Secret has been reset. Re-registration will be required on next login."))
