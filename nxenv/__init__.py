...
"""
Nxenv - Low Code Open Source Framework in Python and JS

Nxenv, pronounced fra-pay, is a full stack, batteries-included, web
...
Read the documentation: https://nxenvframework.com/docs
"""
...
import nxenv
from nxenv.query_builder.utils import (
	get_query,
	get_query_builder,
	patch_query_aggregation,
	patch_query_execute,
)
from nxenv.utils.caching import request_cache
from nxenv.utils.data import bold, cint, cstr, safe_decode, safe_encode, sbool
...
if TYPE_CHECKING:  # pragma: no cover
	...
	from nxenv.database.mariadb.database import MariaDBDatabase
	from nxenv.database.postgres.database import PostgresDatabase
	from nxenv.email.doctype.email_queue.email_queue import EmailQueue
	from nxenv.model.document import Document
	from nxenv.query_builder.builder import MariaDB, Postgres
	from nxenv.types.lazytranslatedstring import _LazyTranslate
	from nxenv.utils.redis_wrapper import ClientCache, RedisWrapper
...
def _(msg: str, lang: str | None = None, context: str | None = None) -> str:
	...
	"""
	from nxenv.translate import get_all_translations
...
def set_user_lang(user: str, user_language: str | None = None) -> None:
	"""Guess and set user language for the session. `nxenv.local.lang`"""
	from nxenv.translate import get_user_lang
...
def init(site: str, sites_path: str = ".", new_site: bool = False, force: bool = False) -> None:
	"""Initialize nxenv for the current site. Reset thread locals `nxenv.local`"""
	...
	local.site = site
	local.site_name = site  # implicitly scopes nxcli
	...
	local.conf = get_site_config(sites_path=sites_path, site_path=site_path, cached=bool(nxenv.request))
	...
	if not _one_time_setup.get(local.conf.db_type):
		patch_query_execute()
		patch_query_aggregation()
		nxenv._optimizations.register_fault_handler()
		_one_time_setup[local.conf.db_type] = True

	setup_module_map(include_all_apps=not (nxenv.request or nxenv.job or nxenv.flags.in_migrate))
...
def connect(site: str | None = None, db_name: str | None = None, set_admin_as_user: bool = True) -> None:
	...
	:param site: (Deprecated) If site is given, calls `nxenv.init`.
	:param db_name: (Deprecated) Optional. Will use from `site_config.json`.
	:param set_admin_as_user: Set Administrator as current user.
	"""
	from nxenv.database import get_db
	...
	if site:
		...
		deprecation_warning(
			"unknown",
			"v17",
			"Calling nxenv.connect with the site argument is deprecated and will be removed in next major version. "
			"Instead, explicitly invoke nxenv.init(site) prior to calling nxenv.connect(), if initializing the site is necessary.",
		)
		init(site)
	if db_name:
		from nxenv.deprecation_dumpster import deprecation_warning
		...
		deprecation_warning(
			"unknown",
			"v17",
			"Calling nxenv.connect with the db_name argument is deprecated and will be removed in next major version. "
			"Instead, explicitly invoke nxenv.init(site) with the right config prior to calling nxenv.connect(), if necessary.",
		)
...
def setup_redis_cache_connection():
	"""Defines `nxenv.cache` as `RedisWrapper` instance"""
	from nxenv.utils.redis_wrapper import ClientCache, setup_cache
...
def get_traceback(with_context: bool = False) -> str:
	...
	from nxenv.utils import get_traceback
...
def print_sql(enable: bool = True) -> None:
	if nxenv.conf.allow_tests and nxenv.conf.developer_mode:
		cache.set_value("flag_print_sql", enable)
	else:
		nxenv.throw("`nxenv.print_sql` only works in `developer_mode` with `allow_tests` enabled on site.")
	...
	*,
	...
	def _raise_exception():
		if raise_exception:
			...
			if out.__nxenv_exc_id:
				exc.__nxenv_exc_id = out.__nxenv_exc_id
			raise exc
	...
	if raise_exception:
		...
		out.__nxenv_exc_id = generate_hash()
...
def toast(message: str, indicator: Literal["blue", "green", "orange", "red", "yellow"] | None = None):
	nxenv.msgprint(message, indicator=indicator, alert=True)
...
def throw(
	...
	:param msg: Message.
	:param exc: Exception class. Default `nxenv.ValidationError`
	:param title: [optional] Message title. Default: "Message".
	:param is_minimizable: [optional] Allow users to minimize the modal
	:param wide: [optional] Show wide modal
	:param as_list: [optional] If `msg` is a list, render as un-ordered list.
...
def create_folder(path, with_init=False):
	...
	from nxenv.utils import touch_file
...
def get_user():
	from nxenv.utils.user import UserPermissions
...
def get_roles(username=None) -> list[str]:
	...
	return nxenv.permissions.get_roles(username or local.session.user)
...
def sendmail(
	...
	if as_markdown:
		from nxenv.utils import md_to_html
	...
	from nxenv.email.doctype.email_queue.email_queue import QueueBuilder
...
def whitelist(allow_guest=False, xss_safe=False, methods=None):
	...
	def innerfn(fn):
		from nxenv.utils.typing_validations import validate_argument_types
...
def is_whitelisted(method):
	from nxenv.utils import sanitize_html
...
def read_only():
	def innfn(fn):
		...
		def wrapper_fn(*args, **kwargs):
			# nxenv.read_only could be called from nested functions, in such cases don't swap the
			# connection again.
...
def only_for(roles: list[str] | tuple[str] | str, message=False):
	"""
	Raises `nxenv.PermissionError` if the user does not have any of the permitted roles.
...
def only_has_select_perm(doctype, user=None, ignore_permissions=False):
	...
	from nxenv.permissions import get_role_permissions
...
def has_permission(
	...
	*,
	...
	Raise `nxenv.PermissionError` if user isn't permitted and `throw` is truthy
	...
	out = nxenv.permissions.has_permission(
		doctype,
		ptype,
		doc=doc,
		user=user,
	...
	if throw and not out:
		...
		nxenv.flags.error_message = _("No permission for {0}").format(document_label)
...
def has_website_permission(doc=None, ptype="read", user=None, verbose=False, doctype=None):
	"""Raises `nxenv.PermissionError` if not permitted.
...
def get_precision(
	...
	from nxenv.model.meta import get_field_precision
...
def new_doc(
	...
	**kwargs,
	...
	from nxenv.model.create_new import get_new_doc
...
def set_value(doctype, docname, fieldname, value=None):
	"""Set document value. Calls `nxenv.client.set_value`"""
	import nxenv.client

	return nxenv.client.set_value(doctype, docname, fieldname, value)


def get_cached_doc(*args: Any, **kwargs: Any) -> "Document":
	"""Identical to `nxenv.get_doc`, but return from cache if available."""
...
def get_doc(**kwargs: dict) -> "_NewDocument":
	...
	Not recommended. Use `nxenv.new_doc` instead."""
	pass
...
def get_doc(*args: Any, **kwargs: Any) -> "Document":
	"""Return a `nxenv.model.document.Document` object of the given type and name.
	...
	Examples:
	        ...
	        todo = nxenv.get_doc({"doctype":"ToDo", "description": "test"})
	        todo.insert()

	        # open an existing document
	        todo = nxenv.get_doc("ToDo", "TD0001")

	"""
	import nxenv.model.document

	return nxenv.model.document.get_doc(*args, **kwargs)
...
def get_single(doctype):
	"""Return a `nxenv.model.document.Document` object of the given Single doctype."""
	return get_doc(doctype, doctype)


def get_meta(doctype, cached=True):
	"""Get `nxenv.model.meta.Meta` instance of given doctype name."""
	import nxenv.model.meta

	return nxenv.model.meta.get_meta(doctype, cached=cached)


def get_meta_module(doctype):
	import nxenv.modules

	return nxenv.modules.load_doctype_module(doctype)
...
def delete_doc(
	...
	"""Delete a document. Calls `nxenv.model.delete_doc.delete_doc`.
	...
	import nxenv.model.delete_doc

	return nxenv.model.delete_doc.delete_doc(
		doctype,
		name,
		force,
		ignore_doctypes,
...
def reload_doc(
	...
	return nxenv.modules.reload_doc(module, dt, dn, force=force, reset_permissions=reset_permissions)
...
def rename_doc(
	...
	*,
	...
	Calls `nxenv.model.rename_doc.rename_doc`
	"""
...
@request_cache
def get_installed_apps(*, _ensure_on_nxcli: bool = False) -> list[str]:
	"""
	Get list of installed apps in current site.

	:param _ensure_on_nxcli: Only return apps that are present on nxcli.
	"""
...
def _load_app_hooks(app_name: str | None = None):
	...
	hooks = {}
	apps = [app_name] if app_name else get_installed_apps(_ensure_on_nxcli=True)
...
def setup_module_map(include_all_apps: bool = True) -> None:
	...
	:param: include_all_apps: Include all apps on nxcli, or just apps installed on the site.
	:return: Nothing
	"""
	if include_all_apps:
	...
	if not local.app_modules:
		...
		if include_all_apps:
			...
			apps = get_installed_apps(_ensure_on_nxcli=True)
...
def get_file_items(path, raise_not_found=False, ignore_empty_lines=True):
	...
	if content:
		content = nxenv.utils.strip(content)
...
def redirect(url):
	...
	from nxenv.exceptions import Redirect
...
def redirect_to_message(title, html, http_status_code=None, context=None, indicator_color=None):
	...
	Example Usage:
	        nxenv.redirect_to_message(_('Thank you'), "<div><p>You will receive an email at test@example.com</p></div>")
...
def build_match_conditions(doctype, as_condition=True):
	...
	import nxenv.desk.reportview

	return nxenv.desk.reportview.build_match_conditions(doctype, as_condition=as_condition)


def get_list(doctype, *args, **kwargs):
	"""List database query via `nxenv.model.db_query`. Will also check for permissions.
	...
	Example usage:
	        ...
	        # simple dict filter
	        nxenv.get_list("ToDo", fields=["name", "description"], filters = {"owner":"test@example.com"})

	        # filter as a list of lists
	        nxenv.get_list("ToDo", fields="*", filters = [["modified", ">", "2014-01-01"]])
	"""
	import nxenv.model.db_query

	return nxenv.model.db_query.DatabaseQuery(doctype).execute(*args, **kwargs)


def get_all(doctype, *args, **kwargs):
	"""List database query via `nxenv.model.db_query`. Will **not** check for permissions.
	Parameters are same as `nxenv.get_list`
	...
	Example usage:
	        ...
	        # simple dict filter
	        nxenv.get_all("ToDo", fields=["name", "description"], filters = {"owner":"test@example.com"})

	        # filter as a list of lists
	        nxenv.get_all("ToDo", fields=["*"], filters = [["modified", ">", "2014-01-01"]])
	"""
...
from nxenv.deprecation_dumpster import nxenv_get_test_records as get_test_records
...
def format_value(*args, **kwargs):
	...
	import nxenv.utils.formatters

	return nxenv.utils.formatters.format_value(*args, **kwargs)
...
def get_print(
	...
	from nxenv.utils.pdf import get_pdf
	from nxenv.website.serve import get_response_without_exception_handling
...
def attach_print(
	...
	from nxenv.translate import print_language
	from nxenv.utils import scrub_urls
	from nxenv.utils.pdf import get_pdf
...
def publish_progress(*args, **kwargs):
	...
	"""
	import nxenv.realtime

	return nxenv.realtime.publish_progress(*args, **kwargs)
...
def publish_realtime(*args, **kwargs):
	...
	return nxenv.realtime.publish_realtime(*args, **kwargs)


def local_cache(namespace, key, generator, regenerate_if_none=False):
	"""A key value store for caching within a request

	:param namespace: nxenv.local.cache[namespace]
	:param key: nxenv.local.cache[namespace][key] used to retrieve value
...
def enqueue(*args, **kwargs):
	...
	"""
	import nxenv.utils.background_jobs

	return nxenv.utils.background_jobs.enqueue(*args, **kwargs)
...
def enqueue_doc(*args, **kwargs):
	...
	return nxenv.utils.background_jobs.enqueue_doc(*args, **kwargs)
...
def logger(
	...
	from nxenv.utils.logger import get_logger
...
def safe_eval(code, eval_globals=None, eval_locals=None):
	...
	from nxenv.utils.safe_exec import safe_eval
...
def get_active_domains():
	from nxenv.core.doctype.domain_settings.domain_settings import get_active_domains
...
def mock(type, size=1, locale="en"):
	...
	from nxenv.utils import squashify
...
def validate_and_sanitize_search_inputs(fn):
	...
	def wrapper(*args, **kwargs):
		from nxenv.desk.search import sanitize_searchfield
...
import nxenv._optimizations
from nxenv.cache_manager import clear_cache, reset_metadata_version

# Backward compatibility
from nxenv.config import get_common_site_config, get_site_config
from nxenv.core.doctype.system_settings.system_settings import get_system_settings
from nxenv.utils import parse_json
from nxenv.utils.error import log_error

nxenv._optimizations.optimize_all()
