...
class RealTimeClient {
	...
	get_host(port = 9000) {
		...
		if (window.dev_server) {
			...
			port = nxenv.boot.socketio_port || port.toString() || "9000";
			if (parts.length > 2) {
				host = parts[0] + ":" + parts[1];
			}
			host = host + ":" + port;
		}
		return host + `/${nxenv.boot.sitename}`;
	}
	...
	doc_subscribe(doctype, docname) {
		...
		nxenv.flags.doc_subscribe = true;

		// throttle to 1 per sec
		setTimeout(function () {
			nxenv.flags.doc_subscribe = false;
		}, 1000);
...
nxenv.realtime = new RealTimeClient();

// backward compatibility
nxenv.socketio = nxenv.realtime;
