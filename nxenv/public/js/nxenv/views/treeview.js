...
nxenv.provide("nxenv.treeview_settings");
nxenv.provide("nxenv.views.trees");
window.cur_tree = null;

nxenv.views.TreeFactory = class TreeFactory extends nxenv.views.Factory {
	make(route) {
		nxenv.model.with_doctype(route[1], function () {
			var options = {
				doctype: route[1],
				meta: nxenv.get_meta(route[1]),
			};

			if (
				!nxenv.treeview_settings[route[1]] &&
				!nxenv.meta.get_docfield(route[1], "is_group")
			) {
				nxenv.msgprint(__("Tree view is not available for {0}", [route[1]]));
				return false;
			}
			$.extend(options, nxenv.treeview_settings[route[1]] || {});
			nxenv.views.trees[options.doctype] = new nxenv.views.TreeView(options);
		});
	}
	...
	on_show() {
		...
		let treeview = nxenv.views.trees[route[1]];
...
nxenv.views.TreeView = class TreeView {
	constructor(opts) {
		var me = this;
		...
		this.get_tree_nodes = me.opts.get_tree_nodes || "nxenv.desk.treeview.get_children";
	...
	get_permissions() {
		this.can_read = nxenv.model.can_read(this.doctype);
		this.can_create =
			nxenv.boot.user.can_create.indexOf(this.doctype) !== -1 ||
			nxenv.boot.user.in_create.indexOf(this.doctype) !== -1;
		this.can_write = nxenv.model.can_write(this.doctype);
		this.can_delete = nxenv.model.can_delete(this.doctype);
	}
	make_page() {
		var me = this;
		if (!this.opts || !this.opts.do_not_make_page) {
			this.parent = nxenv.container.add_page(this.page_name);
			$(this.parent).addClass("treeview");
			nxenv.ui.make_app_page({ parent: this.parent, single_column: true });
			this.page = this.parent.page;
			nxenv.container.change_to(this.page_name);
			nxenv.breadcrumbs.add(
			...
			this.page.main.addClass("nxenv-card");
		} else {
			this.page = this.opts.page;
			$(this.page[0]).addClass("nxenv-card");
		}

		if (nxenv.meta.has_field(me.doctype, "disabled")) {
	...
	make_filters() {
		var me = this;
		nxenv.treeview_settings.filters = [];
		$.each(this.opts.filters || [], function (i, filter) {
			if (nxenv.route_options && nxenv.route_options[filter.fieldname]) {
				filter.default = nxenv.route_options[filter.fieldname];
			}
	...
	rebuild_tree() {
		let me = this;
		nxenv.call({
			method: "nxenv.utils.nestedset.rebuild_tree",
			args: {
				doctype: me.doctype,
			},
			callback: function (r) {
	...
	select_node(node) {
		...
		if (this.opts.view_template) {
			...
			$(
				nxenv.render_template(me.opts.view_template, {
					data: node.data,
					doctype: me.doctype,
				})
		...
		var toolbar = [
			{
				...
				click: function (node) {
					nxenv.set_route("Form", me.doctype, node.label);
				},
			},
			{
				label: __("Add Child"),
			...
			},
			{
				...
				condition: function (node) {
					...
					if (me.doctype && nxenv.get_meta(me.doctype)) {
						if (!nxenv.get_meta(me.doctype).allow_rename) allow_rename = false;
					}
					return !node.is_root && me.can_write && allow_rename;
				},
				click: function (node) {
					nxenv.model.rename_doc(me.doctype, node.label, function (new_name) {
			...
			},
			{
				...
				click: function (node) {
					nxenv.model.delete_doc(me.doctype, node.label, function () {
						node.parent.remove();
					});
				},
				btnClass: "hidden-xs",
	...
	new_node() {
		...
		if (!(node && node.expandable)) {
			nxenv.msgprint(__("Select a group node first."));
			return;
		}
		...
		d.set_primary_action(__("Create New"), function () {
			...
			d.hide();
			nxenv.dom.freeze(__("Creating {0}", [me.doctype]));

			$.extend(args, v);
			return nxenv.call({
				method: me.opts.add_tree_node || "nxenv.desk.treeview.add_node",
				args: args,
				callback: function (r) {
					if (!r.exc) {
	...
	print_tree() {
		if (!nxenv.model.can_print(this.doctype)) {
			nxenv.msgprint(__("You are not allowed to print this report"));
			return false;
		}
		var tree = $(".tree:visible").html();
		var me = this;
		nxenv.ui.get_print_settings(false, function (print_settings) {
			var title = __(me.docname || me.doctype);
			nxenv.render_tree({ title: title, tree: tree, print_settings: print_settings });
			nxenv.call({
				method: "nxenv.core.doctype.access_log.access_log.make_access_log",
				args: {
					doctype: me.doctype,
	...
	set_menu_item() {
		...
		this.menu_items = [
			{
				label: __("View List"),
				action: function () {
					nxenv.set_route("List", me.doctype);
				},
			},
			{
				label: __("Print"),
		...
		if (
			...
			nxenv.meta.has_field(me.doctype, "lft") &&
			nxenv.meta.has_field(me.doctype, "rgt")
		) {
