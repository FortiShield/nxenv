...
nxenv.last_edited_communication = {};
const separator_element = "<div>---</div>";

nxenv.views.CommunicationComposer = class {
	constructor(opts) {
		$.extend(this, opts);
		if (!this.doc) {
	...
	get_fields() {
		...
		const fields = [
			...
			{
				fieldtype: "Button",
				label: nxenv.utils.icon("down", "xs"),
			...
			},
			{
				...
				fieldname: "content",
				onchange: nxenv.utils.debounce(this.save_as_draft.bind(this), 300),
			},
			{
				fieldtype: "Button",
			...
			{ fieldtype: "Section Break" },
			{
				...
				default: nxenv.boot.user.send_me_a_copy,
			},
			{
		...
		const email_accounts = nxenv.boot.email_accounts.filter((account) => {
			return (
		...
		if (email_accounts.length) {
			...
			} else if (this.user_email_accounts.includes(nxenv.session.user_email)) {
				this["sender"] = nxenv.session.user_email;
			}
		}
	...
	guess_language() {
		...
		if (print_format != "Standard") {
			print_format_lang = nxenv.get_doc(
				"Print Format",
				print_format
			)?.default_print_language;
		}

		let lang = document_lang || print_format_lang || nxenv.boot.lang;
	...
	toggle_more_options(show_options) {
		...
		const label = nxenv.utils.icon(show_options ? "up-line" : "down", "xs");
	...
	setup_multiselect_queries() {
		["recipients", "cc", "bcc"].forEach((field) => {
			this.dialog.fields_dict[field].get_data = () => {
				...
				nxenv.call({
					method: "nxenv.email.get_contact_list",
					args: args,
					callback: (r) => {
	...
	setup_email_template() {
		...
		function add_template() {
			...
			nxenv.call({
				method: "nxenv.email.doctype.email_template.email_template.get_email_template",
				args: {
		...
		nxenv.utils.add_select_group_button(clear_and_add_template, email_template_actions);
	}
	...
	get_last_edited_communication(clear) {
		if (!nxenv.last_edited_communication[this.doctype]) {
			nxenv.last_edited_communication[this.doctype] = {};
		}

		if (clear || !nxenv.last_edited_communication[this.doctype][this.key]) {
			nxenv.last_edited_communication[this.doctype][this.key] = {};
		}

		return nxenv.last_edited_communication[this.doctype][this.key];
	}
	...
	async set_values() {
		...
		const subject = this.subject ? nxenv.utils.html2text(this.subject) : "";
	...
	setup_print() {
		...
		if (this.frm) {
			const print_formats = nxenv.meta.get_print_formats(this.frm.meta.name);
	...
	setup_attach() {
		...
		attach
			...
			.on("click", () => new nxenv.ui.FileUploader(args));
			...
			if (files.length) {
				$.each(files, (i, f) => {
					...
					if (!attachment_rows.find(`[data-file-name="${f.name}"]`).length) {
						f.file_url = nxenv.urllib.get_full_url(f.file_url);
	...
	setup_email() {
		...
		$(fields.send_me_a_copy.input).on("click", () => {
			...
			nxenv.db.set_value("User", nxenv.session.user, "send_me_a_copy", val);
			nxenv.boot.user.send_me_a_copy = val;
		});
	}
	...
	send_email(btn, form_values, selected_attachments, print_html, print_format) {
		...
		if (!form_values.recipients) {
			nxenv.msgprint(__("Enter Email Recipient(s)"));
			return;
		}
		...
		if (this.frm && !nxenv.model.can_email(this.doc.doctype, this.frm)) {
			nxenv.msgprint(__("You are not allowed to send emails related to this document"));
			return;
		}

		return nxenv.call({
			method: "nxenv.core.doctype.communication.email.make",
			args: {
				...
				sender_full_name: form_values.sender ? nxenv.user.full_name() : undefined,
			...
			callback(r) {
				if (!r.exc) {
					nxenv.utils.play_sound("email");

					if (r.message["emails_not_sent_to"]) {
						nxenv.msgprint(
							__("Email not sent to {0} (unsubscribed / disabled)", [
								nxenv.utils.escape_html(r.message["emails_not_sent_to"]),
							])
						);
					}
					...
					nxenv.msgprint(__("There were errors while sending email. Please try again."));
	...
	is_print_letterhead_checked() {
		if (this.frm && $(this.frm.wrapper).find(".form-print-wrapper").is(":visible")) {
			...
			return (
				nxenv.model.get_doc(":Print Settings", "Print Settings") || { with_letterhead: 1 }
			).with_letterhead
				? 1
				: 0;
		}
	...
	async get_signature(sender_email) {
		let signature = nxenv.boot.user.email_signature;
		...
		if (!signature) {
			...
			const email_accounts = await nxenv.db.get_list("Email Account", {
				filters: filters,
		...
		if (!nxenv.utils.is_html(signature)) {
	...
	get_earlier_reply() {
		...
		const communication_date = nxenv.datetime.global_date_format(
	...
	html2text(html) {
		...
		const text = nxenv.utils.html2text(html);
		return text.replace(/\n{3,}/g, "\n\n");
	}
};
