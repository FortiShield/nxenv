...
nxenv.views.DashboardView = class DashboardView extends nxenv.views.ListView {
	get view_name() {
		return "Dashboard";
	}
	...
	setup_defaults() {
		return super.setup_defaults().then(() => {
			...
			this.dashboard_settings =
				nxenv.get_user_settings(this.doctype)["dashboard_settings"] || null;
		});
	}
	...
	save_dashboard_customization() {
		...
		nxenv.model.user_settings.save(
			this.doctype,
			"dashboard_settings",
			this.dashboard_settings
		);
	...
	reset_dashboard_customization() {
		nxenv.confirm(__("Are you sure you want to reset all customizations?"), () => {
			this.dashboard_settings = null;
			nxenv.model.user_settings
		...
		const dialog = new nxenv.ui.Dialog({
			...
			fields: [
				...
				},
				{
					...
					get_query: () => {
						return {
							query: "nxenv.desk.doctype.dashboard_chart.dashboard_chart.get_charts_for_user",
							filters: {
								document_type: this.doctype,
							},
						};
			...
			primary_action: (values) => {
				...
				if (chart.new_or_existing == "New Chart") {
					...
					nxenv
						.xcall(
							"nxenv.desk.doctype.dashboard_chart.dashboard_chart.create_dashboard_chart",
							{ args: chart }
						)
						.then((doc) => {
	...
	get_field_options() {
		...
		nxenv.get_meta(this.doctype).fields.map((df) => {
