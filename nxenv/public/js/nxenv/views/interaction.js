...
nxenv.provide("nxenv.interaction_settings");

nxenv.views.InteractionComposer = class InteractionComposer {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	}
	...
	get_event_categories() {
		let me = this;
		nxenv.model.with_doctype("Event", () => {
			let categories = nxenv.meta
	...
	setup_attach() {
		...
		attach.find(".add-more-attachments a").on("click", () => new nxenv.ui.FileUploader(args));
		this.render_attach();
	}
	...
	create_interaction(btn, form_values, selected_attachments) {
		...
		if (!("owner" in interaction_values)) {
			interaction_values["owner"] = nxenv.session.user;
		}
		if (!("assigned_by" in interaction_values) && interaction_values["doctype"] == "ToDo") {
			interaction_values["assigned_by"] = nxenv.session.user;
		}
		return nxenv.call({
	...
	add_attachments(doc, attachments) {
		nxenv.call({
			method: "nxenv.utils.file_manager.add_attachments",
			args: {
				doctype: doc.doctype,
				name: doc.name,
