...
nxenv.views.GanttView = class GanttView extends nxenv.views.ListView {
	get view_name() {
		return "Gantt";
	}
	...
	prepare_tasks() {
		...
		this.tasks = this.data.map(function (item) {
			...
			if (item.color && nxenv.ui.color.validate_hex(item.color)) {
	...
	render_gantt() {
		...
		this.gantt = new Gantt(this.$result[0], this.tasks, {
			...
			on_click: (task) => {
				nxenv.set_route("Form", task.doctype, task.id);
			},
			on_date_change: (task, start, end) => {
				if (!me.can_write) return;
				nxenv.db.set_value(task.doctype, task.id, {
	...
	set_colors() {
		...
		let style = classes
			.map((c) => {
				...
				const progress_color = nxenv.ui.color.get_contrast_color(bar_color);
				return `
	...
	get required_libs() {
		return [
			"assets/nxenv/node_modules/nxenv-gantt/dist/nxenv-gantt.css",
			"assets/nxenv/node_modules/nxenv-gantt/dist/nxenv-gantt.min.js",
		];
	}
};
