...
nxenv.views.MapView = class MapView extends nxenv.views.ListView {
	get view_name() {
		return "Map";
	}
	...
	render_map_view() {
		...
		this.map = L.map(this.map_id).setView(
			nxenv.utils.map_defaults.center,
			nxenv.utils.map_defaults.zoom
		);
	...
	get_coords() {
		let get_coords_method =
			(this.settings && this.settings.get_coords_method) || "nxenv.geo.utils.get_coords";
	...
	get required_libs() {
		return [
			"assets/nxenv/js/lib/leaflet/leaflet.css",
			"assets/nxenv/js/lib/leaflet/leaflet.js",
		];
	}
};
