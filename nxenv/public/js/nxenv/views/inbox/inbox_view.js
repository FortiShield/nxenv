/**
 * nxenv.views.InboxView
 */

nxenv.provide("nxenv.views");

nxenv.views.InboxView = class InboxView extends nxenv.views.ListView {
	static load_last_view() {
		const route = nxenv.get_route();
		if (!route[3] && nxenv.boot.email_accounts.length) {
			let email_account;
			if (nxenv.boot.email_accounts[0].email_id == "All Accounts") {
				email_account = "All Accounts";
			} else {
				email_account = nxenv.boot.email_accounts[0].email_account;
			}
			nxenv.set_route("List", "Communication", "Inbox", email_account);
			return true;
		...
		function is_valid(email_account) {
			return nxenv.boot.email_accounts.find((d) => d.email_account === email_account);
		}
	}
	...
	setup_defaults() {
		...
		this.email_account = nxenv.get_route()[3];
	...
	get_seen_class(doc) {
		return Boolean(doc.seen) || JSON.parse(doc._seen || "[]").includes(nxenv.session.user)
			? ""
			: "bold";
	}
	...
	get_meta_html(email) {
		...
		if (email.reference_doctype && email.reference_doctype !== this.doctype) {
			link = `<a class="text-muted grey"
				href="${nxenv.utils.get_form_link(email.reference_doctype, email.reference_name)}"
	...
	get_inbox_filters() {
		...
		if (email_account === "Sent") {
			...
			filters = default_filters.concat([
				...
				["Communication", "email_account", "in", nxenv.boot.all_accounts, true],
			]);
		} else {
			var op = "=";
			if (email_account == "All Accounts") {
				op = "in";
				email_account = nxenv.boot.all_accounts;
			}
	...
	get_no_result_message() {
		...
		} else if (!email_account && !nxenv.boot.email_accounts.length) {
		...
		const html = nxenv.model.can_create(args.doctype)
			? `<p>${args.msg}</p>
			<p>
	...
	make_new_doc() {
		if (!this.email_account && !nxenv.boot.email_accounts.length) {
			nxenv.route_options = {
				email_id: nxenv.session.user_email,
			};
			nxenv.new_doc("Email Account");
		} else {
