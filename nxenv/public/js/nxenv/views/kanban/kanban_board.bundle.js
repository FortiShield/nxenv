...
(function () {
	var method_prefix = "nxenv.desk.doctype.kanban_board.kanban_board.";
	...
	var store = createStore({
		...
		actions: {
			...
			add_column: function (context, col) {
				if (nxenv.model.can_create("Custom Field")) {
			...
			add_card: function (context, { card_title, column_title }) {
				var state = context.state;
				var doc = nxenv.model.get_new_doc(state.doctype);
				...
				if (field && !quick_entry) {
					...
					nxenv.new_doc(state.doctype, doc);
				}
			},
	...
	nxenv.views.KanbanBoard = function (opts) {
		var self = {};
		...
		function prepare() {
			...
			if (self.$kanban_board.length === 0) {
				self.$kanban_board = $(nxenv.render_template("kanban_board"));
		...
		function make_columns() {
			...
			columns.filter(is_active_column).map(function (col) {
				nxenv.views.KanbanBoardColumn(col, self.$kanban_board, self.board_perms);
			});
		}
		...
		function bind_add_column() {
			...
			const is_custom_field = nxenv.meta.get_docfield(doctype, fieldname)?.is_custom_field;
	...
	nxenv.views.KanbanBoardColumn = function (column, wrapper, board_perms) {
		var self = {};
		var filtered_cards = [];
		...
		function make_dom() {
			self.$kanban_column = $(
				nxenv.render_template("kanban_column", {
					title: column.title,
					doctype: store.state.doctype,
					indicator: nxenv.scrub(column.indicator, "-"),
				})
			).appendTo(wrapper);
			// add task, archive
			...
			if (order) {
				...
				filtered_cards.forEach(function (card) {
					if (order.indexOf(card.name) === -1) {
						nxenv.views.KanbanBoardCard(card, self.$kanban_cards);
					}
				});
				order.forEach(function (name) {
					if (!filtered_cards_names.includes(name)) return;
					nxenv.views.KanbanBoardCard(get_card(name), self.$kanban_cards);
				});
			} else {
		...
		function setup_sortable() {
			...
			if (!nxenv.model.can_write(store.state.doctype)) return;
		...
		function bind_add_card() {
			...
			if (!nxenv.model.can_create(store.state.doctype)) {
		...
		function bind_options() {
			...
			get_column_indicators(function (indicators) {
				let html = `<li class="button-group">${indicators
					.map((indicator) => {
						let classname = nxenv.scrub(indicator, "-");
	...
	nxenv.views.KanbanBoardCard = function (card, wrapper) {
		var self = {};
		...
		function make_dom() {
			var opts = {
				name: card.name,
				title: nxenv.utils.html2text(card.title),
				disable_click: card._disable_click ? "disable-click" : "",
				creation: card.creation,
				doc_content: get_doc_content(card),
				image_url: cur_list.get_image_url(card),
				form_link: nxenv.utils.get_form_link(card.doctype, card.name),
			};

			self.$card = $(nxenv.render_template("kanban_card", opts)).appendTo(wrapper);

			if (!nxenv.model.can_write(card.doctype)) {
		...
		function get_doc_content(card) {
			...
			for (let field_name of cur_list.board.fields) {
				let field =
					nxenv.meta.docfield_map[card.doctype]?.[field_name] ||
					nxenv.model.get_std_field(field_name);
				let label = cur_list.board.show_labels
					? `<span>${__(field.label, null, field.parent)}: </span>`
					: "";
				let value = nxenv.format(card.doc[field_name], field);
				fields.push(`
		...
		function render_card_meta() {
			...
			if (card.color && nxenv.ui.color.validate_hex(card.color)) {
				const $div = $("<div>");
				$("<div></div>")
					.css({
						width: "30px",
		...
		function get_assignees_group() {
			return nxenv.avatar_group(card.assigned_list, 3, {
		...
		function show_assign_to_dialog(e) {
			e.preventDefault();
			e.stopPropagation();
			self.assign_to = new nxenv.ui.form.AssignToDialog({
				obj: self,
				method: "nxenv.desk.form.assign_to.add",
	...
	function fetch_customization(doctype) {
		return new Promise(function (resolve) {
			nxenv.model.with_doc("Customize Form", "Customize Form", function () {
				var doc = nxenv.get_doc("Customize Form");
	...
	function insert_doc(doc) {
		return nxenv.call({
			method: "nxenv.client.insert",
			args: {
				doc: doc,
			},
			callback: function () {
				nxenv.model.clear_doc(doc.doctype, doc.name);
				nxenv.show_alert({ message: __("Saved"), indicator: "green" }, 1);
			},
		});
	}
	...
	function get_column_indicators(callback) {
		nxenv.model.with_doctype("Kanban Board Column", function () {
			var meta = nxenv.get_meta("Kanban Board Column");
			var indicators;
