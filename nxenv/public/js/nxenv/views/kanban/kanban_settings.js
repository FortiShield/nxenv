export default class KanbanSettings {
	constructor({ kanbanview, doctype, meta, settings }) {
		if (!doctype) {
			nxenv.throw(__("DocType required"));
		}
	...
	make() {
		...
		this.dialog.set_primary_action(__("Save"), () => {
			...
			nxenv.call({
				method: "nxenv.desk.doctype.kanban_board.kanban_board.save_settings",
				args: {
		...
		for (let fieldname of this.fields) {
			...
			fields += `
				<div class="control-input flex align-center form-control fields_order sortable"
					...
					<div class="row">
						<div class="col-md-1">
							${nxenv.utils.icon("drag", "xs", "", "", "sortable-handle")}
						</div>
	...
	get_docfield(field_name) {
		return (
			nxenv.meta.get_docfield(this.doctype, field_name) ||
	...
	get_multiselect_fields() {
		...
		const ignore_fieldtypes = [
			...
			"Code",
			"Color",
			...nxenv.model.no_value_type,
		];

		return nxenv.model.std_fields
