...
nxenv.views.ReportView = class ReportView extends nxenv.views.ListView {
	get view_name() {
		return "Report";
	}
	...
	setup_events() {
		...
		nxenv.realtime.on("list_update", (data) => this.on_update(data));
	}
	...
	setup_sort_selector() {
		this.sort_selector = new nxenv.ui.SortSelector({
		...
		this.group_by_control = new nxenv.ui.GroupBy(this);
	...
	before_refresh() {
		if (this.report_doc) {
			// don't parse nxenv.route_options if this is a Custom Report
	...
	set_dirty_state_for_custom_report() {
		...
		if (!nxenv.utils.deep_equal(current_settings, report_settings)) {
	...
	prepare_data(r) {
		let data = r.message || {};
		data = nxenv.utils.dict(data.keys, data.values);
	...
	on_update(data) {
		if (this.doctype === data.doctype && data.name) {
			...
			const flash_row = data.user !== nxenv.session.user;
	...
	setup_charts() {
		...
		for (let col of this.columns) {
			...
			// numeric values in y
			if (col.docfield && nxenv.model.numeric_fieldtypes.includes(col.docfield.fieldtype)) {
				y_fields.push({
					label: col.content,
					fieldname: col.id,
					value: col.id,
			...
			fields: [
				...
				},
				{
					...
					default: defaults.chart_type
						? nxenv.utils.to_title_case(defaults.chart_type)
						: "Bar",
				},
			],
			primary_action: (data) => {
	...
	make_chart() {
		...
		this.chart = new nxenv.Chart(this.$charts_wrapper.find(".charts-inner-wrapper")[0], {
			...
			tooltipOptions: {
				formatTooltipY: (value) => {
					return nxenv.format(value, get_df(this.chart_args.y_axes[0]), {
	...
	render_editing_input(colIndex, value, parent) {
		...
		if (col.docfield.fieldtype === "Text Editor") {
			...
			// make control
			control = nxenv.ui.form.make_control({
				df: col.docfield,
				parent: parent,
				render_input: true,
			});
	...
	evaluate_read_only_depends_on(expression, data) {
		...
		if (typeof expression === "boolean") {
			...
			try {
				out = nxenv.utils.eval(expression.substr(5), { doc: data });
				if (parent && parent.istable && expression.includes("is_submittable")) {
					out = true;
				}
			} catch (e) {
				nxenv.throw(__('Invalid "depends_on" expression'));
			}
	...
	is_editable(df, data) {
		return (
			df &&
			nxenv.model.can_write(this.doctype) &&
			...
			nxenv.model.is_non_std_field(df.fieldname) &&
	...
	set_default_fields() {
		...
		const fields = this.meta.fields.filter((df) => {
			return (
				...
				nxenv.perm.has_perm(this.doctype, df.permlevel, "read") &&
				nxenv.model.is_value_type(df.fieldtype) &&
				!df.report_hide
			);
		});
	...
	get_fields() {
		let fields = this.fields.map((f) => {
			let column_name = nxenv.model.get_full_column_name(f[0], f[1]);
			if (f[1] !== this.doctype) {
				// child table field
		...
		const cdt_name_fields = this.get_unique_cdt_in_view().map(
			(cdt) => nxenv.model.get_full_column_name("name", cdt) + " as " + `'${cdt}:name'`
		);
	...
	add_currency_column(fieldname, doctype, col_index) {
		...
		const df = nxenv.meta.get_docfield(doctype, fieldname);
		if (
			df &&
			df.fieldtype === "Currency" &&
			df.options &&
			!df.options.includes(":") &&
			nxenv.meta.has_field(doctype, df.options)
		) {
			...
			const field_label = nxenv.meta.get_label(doctype, field[0]);
			nxenv.show_alert(
	...
	remove_column_from_datatable(column) {
		...
		if (field[0] === "name") {
			this.refresh();
			nxenv.throw(__("Cannot remove ID field"));
		}
	...
	get_columns_for_picker() {
		...
		const standard_fields_filter = (df) => !nxenv.model.no_value_type.includes(df.fieldtype);

		let doctype_fields = nxenv.meta.get_docfields(this.doctype).filter(standard_fields_filter);

		// filter out docstatus field from picker
		let std_fields = nxenv.model.std_fields.filter(
		...
		if (this.data) {
			has_status_values = nxenv.get_indicator(this.data[0], this.doctype);
		}

		if (!nxenv.meta.has_field(this.doctype, "status") && has_status_values) {
			doctype_fields = [
				{
					label: __("Status"),
					fieldname: "docstatus",
		...
		const table_fields = nxenv.meta.get_table_fields(this.doctype);

		table_fields.forEach((df) => {
			const cdt = df.options;
			const child_table_fields = nxenv.meta
				.get_docfields(cdt)
	...
	get_dialog_fields() {
		...
		const table_fields = nxenv.meta.get_table_fields(this.doctype).filter((df) => !df.hidden);
		...
		for (let f of this.fields) {
			...
			if (f[0] !== "docstatus") {
				...
				if (
					...
					!nxenv.meta.has_field(this.doctype, "status")
				) {
	...
	build_column(c) {
		...
		let docfield = nxenv.meta.docfield_map[doctype || this.doctype][fieldname];
		...
		if (!docfield) {
			docfield = nxenv.model.get_std_field(fieldname, true);
			...
			if (docfield) {
				...
				if (fieldname == "docstatus" && !nxenv.meta.has_field(this.doctype, "status")) {
		...
		const editable =
			nxenv.model.is_non_std_field(fieldname) && !docfield.read_only && !docfield.is_virtual;

		const align = (() => {
			const is_numeric = nxenv.model.is_numeric_field(docfield);
			if (is_numeric) {
				return "right";
			}
		...
		return {
			...
			format: (value, row, column, data) => {
				...
				return nxenv.format(value, column.docfield, { always_show_decimals: true }, doc);
			},
		};
	}
		...
		if (this.add_totals_row) {
			...
			const totals_row = this.columns.map((col, i) => {
				return {
					...
					format: (value) => {
						let formatted_value = nxenv.format(
							value,
							col.docfield,
							{
								always_show_decimals: true,
	...
	format_total_cell(formatted_value, df) {
		...
		if (nxenv.model.is_numeric_field(df.docfield)) {
	...
	build_row(d) {
		return this.columns.map((col) => {
			...
			if (col.field === "docstatus" && !nxenv.meta.has_field(this.doctype, "status")) {
				// get status from docstatus
				let status = nxenv.get_indicator(d, this.doctype);
				if (status) {
	...
	save_report(save_type) {
		const _save_report = (name) => {
			...
			return nxenv.call({
				method: "nxenv.desk.reportview.save_report",
				args: {
					name: name,
					doctype: this.doctype,
				...
				callback: (r) => {
					if (r.exc) {
						nxenv.msgprint(__("Report was not saved (there were errors)"));
						return;
					}
					if (r.message != this.report_name) {
						// Rerender the reports dropdown,
						// so that this report is included in the dropdown as well.
						nxenv.boot.user.all_reports[r.message] = {
							ref_doctype: this.doctype,
							report_type: "Report Builder",
							title: r.message,
						};

						nxenv.set_route("List", this.doctype, "Report", r.message);
						return;
					}
	...
	delete_report() {
		return nxenv.call({
			method: "nxenv.desk.reportview.delete_report",
	...
	get_report_doc() {
		return new Promise((resolve) => {
			nxenv.model.with_doc("Report", this.report_name, () => {
				resolve(nxenv.get_doc("Report", this.report_name));
			});
		});
	}
		...
		return (
			...
			filters
				.map((f) => {
					...
					const docfield = nxenv.meta.get_docfield(doctype, fieldname);
					const label = `<b>${__(nxenv.meta.get_label(doctype, fieldname))}</b>`;
					switch (condition) {
						case "=":
							return __("{0} is equal to {1}", [
								label,
								nxenv.format(value, docfield),
							]);
						case "!=":
						...
						case "Between":
							return __("{0} is between {1} and {2}", [
								__(label),
								nxenv.format(value[0], docfield),
								nxenv.format(value[1], docfield),
							]);
						case "Timespan":
						...
						case "in":
							return __("{0} is one of {1}", [
								__(label),
								nxenv.utils.comma_or(value.map((v) => nxenv.format(v, docfield))),
							]);
						case "not in":
	...
	get_columns_totals(data) {
		...
		this.columns.forEach((col, i) => {
			const totals = data.reduce((totals, d) => {
				if (col.id in d && nxenv.model.is_numeric_field(col.docfield)) {
					totals += flt(d[col.id]);
					return totals;
				}
			}, 0);
	...
	report_menu_items() {
		let items = [
			...
			},
			{
				...
				action: () => {
					...
					nxenv.ui.get_print_settings(false, (print_settings) => {
			...
			},
			{
				...
				action: () => {
					...
					nxenv.utils.setup_search(d.$body, ".unit-checkbox", ".label-area");
					d.show();
				},
			},
		];

		if (nxenv.model.can_export(this.doctype)) {
			items.push({
				label: __("Export"),
				action: () => {
					...
					const d = nxenv.report_utils.get_export_dialog(
						__(this.doctype),
						extra_fields,
						(data) => {
							args.cmd = "nxenv.desk.reportview.export_query";
		...
		items.push({
			...
			action: () => {
				if (this.report_name) {
					...
					nxenv.msgprint(__("Please save the report first"));
				}
			},
		});
		...
		const can_edit_or_delete = (action) => {
			...
			return (
				...
				(nxenv.model[method]("Report") || this.report_doc.owner === nxenv.session.user)
			);
		};
		...
		if (can_edit_or_delete("delete")) {
			items.push({
				label: __("Delete"),
				action: () =>
					nxenv.confirm("Are you sure you want to delete this report?", () =>
						this.delete_report()
					),
		...
		// user permissions
		if (this.report_name && nxenv.user.has_role("System Manager")) {
			items.push({
				...
				action: () => {
					...
					nxenv.set_route("List", "User Permission", args);
				},
			});
		}
	...
	parse_filters_from_route_options() {
		if (nxenv.route_options?._group_by) {
			try {
				let config = JSON.parse(nxenv.route_options._group_by);
				this.group_by_control.apply_settings({
					group_by: config[0],
					aggregate_on: config[1],
					aggregate_function: config[2],
				});
				delete nxenv.route_options["_group_by"];
			} catch (e) {
