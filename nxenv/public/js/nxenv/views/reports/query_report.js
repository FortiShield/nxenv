...
nxenv.provide("nxenv.widget.utils");
nxenv.provide("nxenv.views");
nxenv.provide("nxenv.query_reports");

nxenv.standard_pages["query-report"] = function () {
	var wrapper = nxenv.container.add_page("query-report");
	...
	nxenv.query_report = new nxenv.views.QueryReport({
		parent: wrapper,
	});

	$(wrapper).bind("show", function () {
		nxenv.query_report.show();
	});
};

nxenv.views.QueryReport = class QueryReport extends nxenv.views.BaseList {
	show() {
	...
	init() {
		...
		this.init_promise = nxenv.run_serially(tasks);
		return this.init_promise;
	}
	...
	setup_defaults() {
		...
		this.refresh = nxenv.utils.throttle(this.refresh, 300);
	...
	update_url_with_filters() {
		if (nxenv.get_route_str() == this.page_name) {
	...
	setup_events() {
		nxenv.realtime.on("report_generated", (data) => {
			...
			if (data.report_name) {
				...
				if (data.name == this.prepared_report_doc_name) {
					...
					nxenv.show_alert({ message: alert_message, indicator: "orange" });
				}
			}
		});
	...
	load() {
		if (nxenv.get_route().length < 2) {
			this.toggle_nothing_to_show(true);
			return;
		}

		let route_options = {};
		route_options = Object.assign(route_options, nxenv.route_options);

		if (this.report_name !== nxenv.get_route()[1]) {
			// different report
			this.load_report(route_options);
		} else if (nxenv.has_route_options()) {
	...
	add_card_button_to_toolbar() {
		if (!nxenv.model.can_create("Number Card")) return;
	...
	add_chart_buttons_to_toolbar(show) {
		if (!nxenv.model.can_create("Dashboard Chart")) return;
		if (show) {
	...
	add_card_to_dashboard() {
		let field_options = nxenv.report_utils.get_field_options_from_report(
			this.columns,
			this.raw_data
		);
		const dashboard_field = nxenv.dashboard_utils.get_dashboard_link_field();
		const set_standard = nxenv.boot.developer_mode;

		const dialog = new nxenv.ui.Dialog({
	...
	add_chart_to_dashboard() {
		if (this.chart_fields || this.chart_options) {
			...
			nxenv.msgprint(__("Please Set Chart"));
		}
	}
	...
	create_number_card(values, dashboard_name, card_name, set_standard) {
		...
		this.add_to_dashboard(
			"nxenv.desk.doctype.number_card.number_card.create_report_number_card",
			args,
			dashboard_name,
			card_name,
			"Number Card"
	...
	create_dashboard_chart(chart_args, dashboard_name, chart_name, set_standard) {
		let args = {
			...
			chart_type: "Report",
			report_name: this.report_name,
			type: chart_args.chart_type || nxenv.model.unscrub(chart_args.type),
		...
		this.add_to_dashboard(
			"nxenv.desk.doctype.dashboard_chart.dashboard_chart.create_report_chart",
			args,
			dashboard_name,
			chart_name,
			"Dashboard Chart"
	...
	add_to_dashboard(method, args, dashboard_name, name, doctype) {
		nxenv.xcall(method, { args: args }).then(() => {
			let message;
			if (dashboard_name) {
			...
			nxenv.msgprint(message, __("New {0} Created", [__(doctype)]));
		});
	}
	...
	get_report_doc() {
		return nxenv.model
			...
			.then(() => nxenv.model.with_doctype(this.report_doc?.ref_doctype));
	}

	get_report_settings() {
		return new Promise((resolve, reject) => {
			if (nxenv.query_reports[this.report_name]) {
				this.report_settings = nxenv.query_reports[this.report_name];
				resolve();
			} else {
				nxenv
					.xcall("nxenv.desk.query_report.get_script", {
						report_name: this.report_name,
					})
					.then((settings) => {
						nxenv.dom.eval(settings.script || "");
						nxenv.after_ajax(() => {
							...
							nxenv.query_reports[this.report_name] = this.report_settings;
	...
	get_local_report_settings(custom_report_name) {
		let report_script_name =
			this.report_doc.report_type === "Custom Report"
				...
				: this.report_name;
		return nxenv.query_reports[report_script_name] || {};
	}
	...
	setup_progress_bar() {
		...
		this.interval = setInterval(function () {
			seconds_elapsed += 1;
			nxenv.show_progress(__("Preparing Report"), seconds_elapsed, execution_time);
		}, 1000);
	}
	...
	evaluate_depends_on_value(expression, filter_label) {
		...
		if (doc) {
			if (typeof expression === "boolean") {
				...
				try {
					out = nxenv.utils.eval(expression.substr(5), { doc });
				} catch (e) {
					nxenv.throw(
	...
	set_route_filters(route_options) {
		if (!route_options) route_options = nxenv.route_options;
	...
	refresh(have_filters_changed) {
		...
		return new Promise((resolve) => {
			this.last_ajax = nxenv.call({
				method: "nxenv.desk.query_report.run",
				type: "GET",
				args: {
					report_name: this.report_name,
					filters: filters,
			...
			.then((r) => {
				...
				if (data.result && data.result.length) {
					...
					if (this.chart_options) {
						...
						this.$chart.empty();
						if (this.chart_fields) {
							this.chart_options = nxenv.report_utils.make_chart_options(
								this.columns,
								this.raw_data,
								this.chart_fields
							);
	...
	render_summary(data) {
		data.forEach((summary) => {
			nxenv.utils.build_summary_item(summary).appendTo(this.$summary);
		});
	...
	get_query_params() {
		const query_string = nxenv.utils.get_query_string(nxenv.get_route_str());
		return nxenv.utils.get_query_params(query_string);
	}
	...
	add_prepared_report_buttons(doc) {
		if (doc) {
			this.page.add_inner_button(
				...
				function () {
					window.open(
						nxenv.urllib.get_full_url(
							"/api/method/nxenv.core.doctype.prepared_report.prepared_report.download_attachment?" +
								"dn=" +
			...
			let pretty_diff = nxenv.datetime.comment_when(doc.report_end_time);
			const days_old = nxenv.datetime.get_day_diff(
				nxenv.datetime.now_datetime(),
				doc.report_end_time
			);
			if (days_old > 1) {
		...
		this.primary_action_map = {
			...
			Edit: {
				label: __("Edit"),
				click: () => {
					nxenv.set_route(nxenv.get_route());
				},
			},
			Rebuild: {
				label: __("Rebuild"),
	...
	show_warning_or_generate_report() {
		nxenv
			.xcall(
				"nxenv.core.doctype.prepared_report.prepared_report.get_reports_in_queued_state",
				{
			...
			.then((reports) => {
				...
				if (reports.length) {
					...
					this.prepared_report_dialog = nxenv.warn(
					...
					nxenv.utils.bind_actions_with_object(
	...
	delete_old_queued_reports() {
		...
		nxenv
			.xcall("nxenv.core.doctype.prepared_report.prepared_report.delete_prepared_reports", {
	...
	generate_background_report() {
		...
		if (!missing_mandatory.length) {
			...
			return new Promise((resolve) =>
				nxenv.call({
					method: "nxenv.core.doctype.prepared_report.prepared_report.make_prepared_report",
					args: {
						report_name: this.report_name,
						filters: filters,
					},
				...
				let alert_message =
					...
					`</a>`;
				nxenv.show_alert({ message: alert_message, indicator: "orange" }, 10);
				this.toggle_nothing_to_show(true);
			});
		}
	}
	...
	render_datatable() {
		...
		if (data.length > 1000000) {
			...
			this.toggle_message(true, `${nxenv.utils.icon("solid-warning")} ${msg}`);
			return;
		}
		...
		if (
			...
			let datatable_options = {
				...
				data: data,
				inlineFilters: true,
				language: nxenv.boot.lang,
				translations: nxenv.utils.datatable.get_translations(),
				treeView: this.tree_report,
				layout: "fixed",
				cellHeight: 33,
				showTotalRow: this.raw_data.add_total_row && !this.report_settings.tree,
				direction: nxenv.utils.is_rtl() ? "rtl" : "ltr",
				hooks: {
					columnTotal: nxenv.utils.report_column_total,
				},
			};
	...
	get_chart_options(data) {
		...
		options.axisOptions = {
			...
			numberFormatter: nxenv.utils.format_chart_axis_number,
		};
		options.height = 280;
		return options;
	}
	...
	render_chart(options) {
		this.$chart.empty();
		this.$chart.show();
		this.chart = new nxenv.Chart(this.$chart[0], options);
	}
	...
	open_create_chart_dialog() {
		...
		function preview_chart() {
			...
			if (values.x_field && values.y_fields.length) {
				let options = nxenv.report_utils.make_chart_options(
					me.columns,
					me.raw_data,
					values
				);
				...
				wrapper.empty();
				new nxenv.Chart(wrapper[0], options);
	...
	prepare_columns(columns) {
		return columns.map((column) => {
			column = nxenv.report_utils.prepare_field_from_column(column);
			...
			if (column.fieldtype === "Date") {
				compareFn = (cell, keyword) => {
					...
					const keywordValue = nxenv.datetime.user_to_obj(keyword);
					const cellValue = nxenv.datetime.str_to_obj(cell.content);
	...
	set_breadcrumbs() {
		...
		const ref_doctype = nxenv.get_meta(this.report_doc.ref_doctype);
		nxenv.breadcrumbs.add(ref_doctype.module);
	}

	make_access_log(method, file_format) {
		nxenv.call("nxenv.core.doctype.access_log.access_log.make_access_log", {
	...
	pdf_report(print_settings) {
		const base_url = nxenv.urllib.get_base_url();
		const print_css = nxenv.boot.print_css;
		const landscape = print_settings.orientation == "Landscape";
		...
		const content = nxenv.render_template(template, {
			title: __(this.report_name),
			subtitle: filters_html,
		...
		const html = nxenv.render_template("print_template", {
			title: __(this.report_name),
			content: content,
			base_url: base_url,
			print_css: print_css,
			...
			landscape: landscape,
			columns: columns,
			lang: nxenv.boot.lang,
			layout_direction: nxenv.utils.is_rtl() ? "rtl" : "ltr",
			can_use_smaller_font: this.report_doc.is_standard === "Yes" && custom_format ? 0 : 1,
		});
		...
		nxenv.render_pdf(html, print_settings);
	}
	...
	get_filters_html_for_print() {
		...
		return Object.keys(applied_filters)
			.map((fieldname) => {
				const docfield = nxenv.query_report.get_filter(fieldname).df;
				const value = applied_filters[fieldname];

				if (nxenv.utils.is_empty(value) || docfield.hidden_due_to_dependency) {
					return null;
				}

				return `<div class="filter-row">
					<b>${__(docfield.label, null, docfield.parent)}:</b> ${nxenv.format(value, docfield)}
				</div>`;
			})
			.join("");
	}
	...
	export_report() {
		...
		this.export_dialog = nxenv.report_utils.get_export_dialog(
			__(this.report_name),
			extra_fields,
			({
				file_format,
				...
				for (const [key, value] of Object.entries(filters)) {
					const df = nxenv.query_report.get_filter(key).df;
				...
				const args = {
					cmd: "nxenv.desk.query_report.export_query",
					report_name: this.report_name,
				...
				open_url_post(nxenv.request.url, args);
		...
		return rows.map((row) => {
			...
			return row.slice(standard_column_count).map((cell, i) => {
				if (cell.column.fieldtype === "Duration") {
					cell.content = nxenv.utils.get_formatted_duration(cell.content);
				}
	...
	get_menu_items() {
		let items = [
			...
			{
				label: __("Edit"),
				action: () => nxenv.set_route("Form", "Report", this.report_name),
				condition: () => nxenv.user.is_report_manager(),
				standard: true,
			},
			{
				label: __("Print"),
				action: () => {
					let dialog = nxenv.ui.get_print_settings(
						false,
				...
				condition: () => nxenv.model.can_print(this.report_doc.ref_doctype),
				standard: true,
			},
			{
				label: __("PDF"),
			...
			},
			{
				...
				condition: () => nxenv.model.can_export(this.report_doc.ref_doctype),
				standard: true,
			},
			{
				label: __("Setup Auto Email"),
				action: () =>
					nxenv.set_route("List", "Auto Email Report", { report: this.report_name }),
				standard: true,
			},
			{
				label: __("Add Column"),
				action: () => {
					let d = new nxenv.ui.Dialog({
						...
						fields: [
							{
								...
								change: () => {
									...
									nxenv.model.with_doctype(doctype, () => {
										let options = nxenv.meta
											.get_docfields(doctype)
											.filter(nxenv.model.is_value_type)
											.map((df) => ({
												label: df.label,
												value: df.fieldname,
											}));
						...
						primary_action: (values) => {
							...
							let df = nxenv.meta.get_docfield(values.doctype, values.field);
							...
							custom_columns.push({
								fieldname: this.columns
									...
									? df.fieldname + "-" + nxenv.scrub(values.doctype)
									: df.fieldname,
							...
							nxenv.call({
								method: "nxenv.desk.query_report.get_data_for_custom_field",
								args: {
									field: values.field,
									doctype: values.doctype,
									names: Array.from(
			...
			},
			{
				...
				condition: () => nxenv.user.has_role("System Manager"),
				standard: true,
			},
		];

		if (nxenv.user.is_report_manager()) {
			items.push({
				label: __("Save"),
				action: () => {
					let d = new nxenv.ui.Dialog({
						...
						primary_action: (values) => {
							nxenv.call({
								method: "nxenv.desk.query_report.save_report",
								args: {
								...
								callback: function (r) {
									this.show_save = false;
									d.hide();
									nxenv.set_route("query-report", r.message);
								},
							});
						},
					});
	...
	add_custom_column(
		...
		this.data.forEach((row) => {
			if (column[0].fieldname.includes("-")) {
				row[column_field + "-" + nxenv.scrub(new_column_data.doctype)] =
