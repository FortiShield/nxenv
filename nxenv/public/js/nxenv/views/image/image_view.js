/**
 * nxenv.views.ImageView
 */
nxenv.provide("nxenv.views");

nxenv.views.ImageView = class ImageView extends nxenv.views.ListView {
	get view_name() {
		return "Image";
	}
	...
	item_html(item) {
		...
		const escaped_title = nxenv.utils.escape_html(title);
	...
	get_attached_images() {
		return nxenv
			.call({
				method: "nxenv.core.api.file.get_attached_images",
				args: {
	...
	setup_gallery() {
		var me = this;
		this.gallery = new nxenv.views.GalleryView({
			doctype: this.doctype,
			items: this.items,
			wrapper: this.$result,
	...
	get required_libs() {
		return ["assets/nxenv/node_modules/photoswipe/src/photoswipe.css", "photoswipe.bundle.js"];
	}
};

nxenv.views.GalleryView = class GalleryView {
	constructor(opts) {
		$.extend(this, opts);
		var me = this;
		me.prepare();
	...
	prepare() {
		...
		if (this.pswp_root.length === 0) {
			var pswp = nxenv.render_template("photoswipe_dom");
	...
	_show(docname) {
		...
		// init
		this.pswp = new nxenv.PhotoSwipe(options);
		this.pswp.init();
	}
};
