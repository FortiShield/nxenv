...
nxenv.provide("nxenv.views.calendar");
nxenv.provide("nxenv.views.calendars");

nxenv.views.CalendarView = class CalendarView extends nxenv.views.ListView {
	static load_last_view() {
		const route = nxenv.get_route();
		if (route.length === 3) {
			const doctype = route[1];
			const user_settings = nxenv.get_user_settings(doctype)["Calendar"] || {};
	...
	setup_defaults() {
		return super.setup_defaults().then(() => {
			...
			this.calendar_settings = nxenv.views.Calendar[this.doctype] || {};
			this.calendar_name = nxenv.get_route()[3];
		});
	}
	...
	render() {
		...
		this.load_lib
			...
			.then((options) => {
				this.calendar = new nxenv.views.Calendar(options);
			});
	}
	...
	get_calendar_preferences() {
		...
		return new Promise((resolve) => {
			if (calendar_name === "default") {
				Object.assign(options, nxenv.views.calendar[this.doctype]);
				resolve(options);
			} else {
				nxenv.model.with_doc("Calendar View", calendar_name, () => {
					const doc = nxenv.get_doc("Calendar View", calendar_name);
					if (!doc) {
						nxenv.show_alert(
						...
						nxenv.set_route("List", this.doctype, "Calendar", "default");
						return;
					}
...
nxenv.views.Calendar = class Calendar {
	constructor(options) {
		$.extend(this, options);
	...
	make_page() {
		...
		$.each(nxenv.boot.calendars, function (i, doctype) {
			if (nxenv.model.can_read(doctype)) {
				me.page.add_menu_item(__(doctype), function () {
					nxenv.set_route("List", doctype, "Calendar");
				});
			}
		});
	...
	make() {
		...
		this.footnote_area = nxenv.utils.set_footnote(
			this.footnote_area,
			this.$wrapper,
		...
		this.fullCalendar = new nxenv.FullCalendar(this.$cal[0], this.cal_options);
	...
	set_css() {
		...
		this.$wrapper
			...
			.attr("class", "")
			.html(nxenv.utils.icon("left"));
		this.$wrapper
			.find(`.fc-next-button span`)
			.attr("class", "")
			.html(nxenv.utils.icon("right"));
		if (this.$wrapper.find(".fc-today-button svg").length == 0)
			this.$wrapper.find(".fc-today-button").prepend(nxenv.utils.icon("today"));
	...
	get_system_datetime(date) {
		return nxenv.datetime.convert_to_system_tz(date, true);
	}
	setup_options(defaults) {
		var me = this;
		defaults.meridiem = "false";
		this.cal_options = {
			plugins: nxenv.FullCalendar.Plugins,
			initialView: defaults.initialView || "dayGridMonth",
			locale: nxenv.boot.lang,
			firstDay: 1,
			headerToolbar: {
			...
			events: function (info, successCallback, failureCallback) {
				return nxenv.call({
					method: me.get_events_method || "nxenv.desk.calendar.get_events",
					type: "GET",
			...
			eventClick: function (info) {
				...
				if (nxenv.model.can_read(doctype)) {
					nxenv.set_route("Form", doctype, info.event.id);
				}
			},
			...
			select: function (info) {
				...
				var event = nxenv.model.get_new_doc(me.doctype);
				...
				nxenv.set_route("Form", me.doctype, event.name);
			},
	...
	prepare_events(events) {
		...
		return (events || []).map((d) => {
			d.id = d.name;
			d.editable = nxenv.model.can_write(d.doctype || me.doctype);
			...
			if (d.convertToUserTz) {
				d.start = nxenv.datetime.convert_to_user_tz(d.start);
				d.end = nxenv.datetime.convert_to_user_tz(d.end);
			}

			// show event on single day if start or end date is invalid
			if (!nxenv.datetime.validate(d.start) && d.end) {
				d.start = nxenv.datetime.add_days(d.end, -1);
			}

			if (d.start && !nxenv.datetime.validate(d.end)) {
				d.end = nxenv.datetime.add_days(d.start, 1);
			}

			me.prepare_colors(d);

			d.title = nxenv.utils.html2text(d.title);
	...
	prepare_colors(d) {
		...
		if (this.get_css_class) {
			...
			if (color_name.startsWith("#")) {
				color_name = nxenv.ui.color.validate_hex(color_name) ? color_name : "blue";
			}

			d.backgroundColor = nxenv.ui.color.get(color_name, "extra-light");
			d.textColor = nxenv.ui.color.get(color_name, "dark");
		} else {
			color = d.color;
			if (!nxenv.ui.color.validate_hex(color) || !color) {
				color = nxenv.ui.color.get("blue", "extra-light");
			}
			d.backgroundColor = color;
			d.textColor = nxenv.ui.color.get_contrast_color(color);
		}
		return d;
	}
	update_event(event, revertFunc) {
		var me = this;
		nxenv.model.remove_from_locals(me.doctype, event.id);
		return nxenv.call({
			method: me.update_event_method || "nxenv.desk.calendar.update_event",
			args: me.get_update_args(event),
			callback: function (r) {
				if (r.exc) {
					nxenv.show_alert(__("Unable to update event"));
					revertFunc();
				}
			},
			error: function () {
