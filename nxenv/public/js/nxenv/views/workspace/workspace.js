...
nxenv.standard_pages["Workspaces"] = function () {
	var wrapper = nxenv.container.add_page("Workspaces");
	...
	nxenv.workspace = new nxenv.views.Workspace(wrapper);
	$(wrapper).bind("show", function () {
		nxenv.workspace.show();
	});
};

nxenv.views.Workspace = class Workspace {
	constructor(wrapper) {
		this.wrapper = $(wrapper);
		this.page = wrapper.page;
		this.blocks = nxenv.workspace_block.blocks;
		...
		this.sidebar = nxenv.app.sidebar;
		this.app_switcher_menu = nxenv.app.app_switcher_menu;
		this.sidebar.setup_pages();
		this.cached_pages = $.extend(true, {}, nxenv.boot.sidebar_pages);
		this.has_access = nxenv.boot.sidebar_pages.has_access;
		this.has_create_access = nxenv.boot.sidebar_pages.has_create_access;
		this.show();
	...
	`).appendTo(this.body);
		...
		this.body.find(".btn-edit-workspace").on("click", async () => {
			...
			this.editor.isReady.then(() => {
				...
				nxenv.app.sidebar.toggle_sorting();
			});
		});
	}

	get_pages() {
		return nxenv.xcall("nxenv.desk.desktop.get_workspace_sidebar_items", null, "GET");
	}
	...
	show() {
		...
		if (!nxenv.router.current_route[0]) {
			nxenv.route_flags.replace_route = true;
			nxenv.set_route(nxenv.router.slug(page.public ? page.name : "private/" + page.name));
			return;
		}
	...
	get_data(page) {
		return nxenv
			.call({
				method: "nxenv.desk.desktop.get_desktop_page",
				args: {
			...
			.then((data) => {
				...
				return nxenv.dashboard_utils.get_dashboard_settings().then((settings) => {
					if (settings) {
	...
	get_page_to_show() {
		...
		if (nxenv.boot.user.default_workspace) {
			default_page = {
				name: nxenv.boot.user.default_workspace.name,
				public: nxenv.boot.user.default_workspace.public,
			};
		} else if (
		...
		if (this.sidebar.all_pages.length) {
			...
			if (!this._page.public) {
				...
				if (!app && this._page.module) {
					app = nxenv.boot.module_app[nxenv.router.slug(this._page.module)];
				}
				if (!app) app = "nxenv";
			}
			...
			if (this.pages && this.pages[current_page.name]) {
				...
				await nxenv.after_ajax(() => this.get_data(current_page));
			}
			...
			this.wrapper
				.find(".workspace-icon")
				.html(nxenv.utils.icon(this._page.icon || "folder-normal", "md"));
	...
	setup_customization_buttons(page) {
		...
		this.page.set_secondary_action(__("Discard"), async () => {
			...
			await this.editor.readOnly.toggle();
			this.is_read_only = true;
			nxenv.boot.sidebar_pages = this.cached_pages;
			this.reload();
			nxenv.show_alert({ message: __("Customizations Discarded"), indicator: "info" });
		});

		if (page.name && this.has_access) {
			this.page.add_inner_button(__("Settings"), () => {
				nxenv.set_route(`workspace/${page.name}`);
			});
		}
	}
	...
	initialize_new_page() {
		...
		const d = new nxenv.ui.Dialog({
			...
			primary_action: (values) => {
				...
				let name = values.title + (values.is_public ? "" : "-" + nxenv.session.user);
				let blocks = [
					{
						type: "header",
				...
				let new_page = {
					...
					for_user: values.is_public ? "" : nxenv.session.user,
					icon: values.icon,
					indicator_color: values.indicator_color,
					parent_page: values.parent || "",
					is_editable: true,
					selected: true,
					app: nxenv.current_app,
					type: values.type,
					...
					this.editor
						...
						.then(async () => {
							...
							this.create_page(new_page).then(() => {
								...
								let route = pre_url + nxenv.router.slug(new_page.title);
	...
	create_page(new_page) {
		return new Promise((resolve) => {
			nxenv.call({
				method: "nxenv.desk.doctype.workspace.workspace.new_page",
				args: {
					new_page: new_page,
				},
				callback: (r) => {
					if (r.message) {
						...
						nxenv.boot.sidebar_pages = r.message;
						this.sidebar.setup_pages();

						if (!nxenv.boot.app_data_map["private"] && new_page.public === 0) {
	...
	initialize_editorjs(blocks) {
		this.tools = {
			...
			HeaderSize: nxenv.workspace_block.tunes["header_size"],
		};
	...
	save_page(page) {
		...
		return this.editor
			...
			.then((outputData) => {
				...
				page.content = JSON.stringify(blocks);
				nxenv.call({
					method: "nxenv.desk.doctype.workspace.workspace.save_page",
					args: {
						name: page.name,
	...
	create_page_skeleton() {
		...
		this.body.prepend(nxenv.render_template("workspace_loading_skeleton"));
	...
	register_awesomebar_shortcut() {
		"abcdefghijklmnopqrstuvwxyz".split("").forEach((letter) => {
			...
			nxenv.ui.keys.add_shortcut({ shortcut: letter, ...default_shortcut });
			nxenv.ui.keys.add_shortcut({ shortcut: `shift+${letter}`, ...default_shortcut });
		});
	}
};
