...
nxenv.realtime.on("build_event", (data) => {
	if (data.success) {
			...
			for (let file of changed_files) {
				if (file.includes(".bundle.")) {
					...
					if (parts.length === 2) {
						...
						nxenv.assets._executed = nxenv.assets._executed.filter(
		...
		// update assets json
		nxenv.call("nxenv.sessions.get_boot_assets_json").then((r) => {
			if (r.message) {
				nxenv.boot.assets_json = r.message;

				if (nxenv.hot_update) {
					nxenv.hot_update.forEach((callback) => {
						callback();
					});
				}
			}
