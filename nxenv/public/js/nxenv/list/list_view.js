...
nxenv.views.ListView = class ListView extends nxenv.views.BaseList {
	static load_last_view() {
		const route = nxenv.get_route();
		const doctype = route[1];

		if (route.length === 2) {
			const user_settings = nxenv.get_user_settings(doctype);
			const last_view = user_settings.last_view;
			nxenv.set_route(
				"list",
				nxenv.router.doctype_layout || doctype,
				nxenv.views.is_valid(last_view) ? last_view.toLowerCase() : "list"
			);
			return true;
		}
		return false;
	...
	constructor(opts) {
		super(opts);
		this.show();
		this.debounced_refresh = nxenv.utils.debounce(
	...
	has_permissions() {
		return nxenv.perm.has_perm(this.doctype, 0, "read");
	}
	...
	check_permissions() {
		if (!this.has_permissions()) {
			nxenv.set_route("");
			nxenv.throw(__("Not permitted to view {0}", [this.doctype]));
		}
	}
	...
	validate_filters(filters) {
		...
		valid_fields = valid_fields.concat(nxenv.model.std_fields_list);
	...
	show_restricted_list_indicator_if_applicable() {
		const match_rules_list = nxenv.perm.get_match_rules(this.doctype);
		if (match_rules_list.length) {
			this.restricted_list = $(
				`<button class="btn btn-xs restricted-button flex align-center">
					${nxenv.utils.icon("restriction", "xs")}
				</button>`
			)
	...
	show_restrictions(match_rules_list = []) {
		nxenv.msgprint(
			nxenv.render_template("list_view_permission_restrictions", {
	...
	async set_fields() {
		...
		await Promise.all(
			fields.map((f) => {
				return new Promise((resolve) => {
					const df =
						typeof f === "string" ? nxenv.meta.get_docfield(this.doctype, f) : f;
					if (
						df &&
						df.fieldtype == "Link" &&
						nxenv.boot.link_title_doctypes.includes(df.options)
					) {
						nxenv.model.with_doctype(df.options, () => {
							const meta = nxenv.get_meta(df.options);
		...
		this.fields.forEach((f) => {
			const df = nxenv.meta.get_docfield(f[1], f[0]);
	...
	patch_refresh_and_load_lib() {
		...
		this.refresh = nxenv.utils.throttle(this.refresh, 1000);
		this.load_lib = new Promise((resolve) => {
			if (this.required_libs) {
				nxenv.require(this.required_libs, resolve);
			} else {
				resolve();
			}
		});
	...
	set_primary_action() {
		if (this.can_create && !nxenv.boot.read_only) {
			const doctype_name = __(nxenv.router.doctype_layout) || __(this.doctype);
	...
	make_new_doc() {
		...
		this.filter_area.get().forEach((f) => {
			if (f[2] === "=" && nxenv.model.is_non_std_field(f[1])) {
				options[f[1]] = f[3];
			}
		});
		nxenv.new_doc(doctype, options);
	}
	...
	setup_columns() {
		...
		const get_df = nxenv.meta.get_docfield.bind(null, this.doctype);
		...
		if (nxenv.has_indicator(this.doctype)) {
			// indicator
			this.columns.push({
				type: "Status",
			});
		...
		this.columns = this.columns.concat(
			fields_in_list_view
				.filter((df) => {
					if (nxenv.has_indicator(this.doctype) && df.fieldname === "status") {
						return false;
					}
	...
	before_refresh() {
		if (nxenv.route_options && this.filter_area) {
	...
	before_render() {
		...
		nxenv.model.user_settings.save(this.doctype, "last_view", this.view_name);
		...
		const $columns = this.columns
			.map((col) => {
				let classes = [
					...
					nxenv.model.is_numeric_field(col.df) ? "text-right" : "",
				].join(" ");
	...
	get_column_html(col, doc) {
		...
		let translated_doctypes = nxenv.boot?.translated_doctypes || [];
		...
		const format = () => {
			if (df.fieldtype === "Percent") {
				...
				return nxenv.format(value, df, null, doc);
			}
		};
		...
		const field_html = () => {
			...
			if (strip_html_required) {
				...
				_value =
					typeof value_display === "string"
						? nxenv.utils.escape_html(value_display)
						: value_display;
			}
			...
			if (df.fieldtype === "Image") {
				html = df.options
					...
					: `<div class="missing-image small">
						${nxenv.utils.icon("restriction")}
					</div>`;
			} else if (df.fieldtype === "Select") {
				html = `<span class="filterable indicator-pill ${nxenv.utils.guess_colour(
					_value
				)} ellipsis"
			...
			} else if (nxenv.model.html_fieldtypes.includes(df.fieldtype)) {
				html = `<span class="ellipsis">
					${_value}
				</span>`;
			} else {
				html = `<a class="filterable ellipsis"
					data-filter="${fieldname},=,${nxenv.utils.escape_html(value)}">
					${format()}
				</a>`;
			}

			return `<span class="ellipsis"
				title="${__(label)}: ${nxenv.utils.escape_html(_value)}">
				${html}
			</span>`;
		};
		...
		const css_class = [
			"list-row-col ellipsis",
			class_map[col.type],
			nxenv.model.is_numeric_field(df) ? "text-right" : "",
		].join(" ");
	...
	get_meta_html(doc) {
		...
		if (assigned_users.length) {
			assigned_to = `<div class="list-assignments d-flex align-items-center">
					${nxenv.avatar_group(assigned_users, 3, { filterable: true })[0].outerHTML}
				</div>`;
		}
		...
		if (this.list_view_settings && !this.list_view_settings.disable_comment_count) {
			comment_count = `<span class="comment-count d-flex align-items-center">
				${nxenv.utils.icon("es-line-chat-alt")}
	...
	get_form_link(doc) {
		...
		return `/app/${encodeURIComponent(
			nxenv.router.slug(nxenv.router.doctype_layout || this.doctype)
		)}/${encodeURIComponent(cstr(doc.name))}`;
	}

	get_seen_class(doc) {
		const seen_by = doc._seen ? JSON.parse(doc._seen) : [];
		return seen_by.includes(nxenv.session.user) ? "" : "bold";
	}

	get_like_html(doc) {
		const liked_by = doc._liked_by ? JSON.parse(doc._liked_by) : [];
		const is_liked = liked_by.includes(nxenv.session.user);
		const title = liked_by.map((u) => nxenv.user_info(u).fullname).join(", ");
	...
	get_subject_text(doc, title) {
		...
		if (nxenv.model.html_fieldtypes.includes(subject_field.fieldtype)) {
			// NOTE: this is very slow, so only do it for HTML fields
			return nxenv.utils.html2text(value);
		} else {
			return value;
		}
	}
	...
	get_indicator_html(doc, show_workflow_state) {
		const indicator = nxenv.get_indicator(doc, this.doctype, show_workflow_state);
	...
	get_indicator_dot(doc) {
		const indicator = nxenv.get_indicator(doc, this.doctype);
		if (!indicator) return "";
	...
	get_image_url(doc) {
		...
		if (window.cordova && !nxenv.utils.is_url(url)) {
			url = nxenv.base_url + url;
		}
		return url || null;
	}
	...
	setup_filterable() {
		...
		this.$result.on("click", ".filterable", (e) => {
			...
			const filters_to_apply = filters.map((f) => {
				f = f.split(",");
				if (f[2] === "Today") {
					f[2] = nxenv.datetime.get_today();
				} else if (f[2] == "User") {
					f[2] = nxenv.session.user;
				}
	...
	setup_like() {
		this.$result.on("click", ".like-action", (e) => {
			...
			nxenv.ui.toggle_like($this, doctype, name);
	...
	setup_realtime_updates() {
		...
		nxenv.realtime.doctype_subscribe(this.doctype);
		nxenv.realtime.off("list_update");
		nxenv.realtime.on("list_update", (data) => {
	...
	disable_realtime_updates() {
		nxenv.realtime.doctype_unsubscribe(this.doctype);
	...
	process_document_refreshes() {
		...
		nxenv.call(call_args).then(({ message }) => {
			if (!message) return;
			const data = nxenv.utils.dict(message.keys, message.values);
	...
	save_view_user_settings(obj) {
		return nxenv.model.user_settings.save(this.doctype, this.view_name, obj);
	}

	on_update() {}

	update_url_with_filters() {
		if (nxenv.get_route_str() == this.page_name && !this.report_name) {
	...
	get_menu_items() {
		...
		if (nxenv.model.can_import(doctype, null, this.meta)) {
			items.push({
				label: __("Import", null, "Button in list view menu"),
				action: () =>
					nxenv.set_route("list", "data-import", {
		...
		if (nxenv.model.can_create("Custom Field") && nxenv.model.can_create("Property Setter")) {
			items.push({
				label: __("Customize", null, "Button in list view menu"),
				action: () => {
					if (!this.meta) return;
					if (this.meta.custom) {
						nxenv.set_route("form", "doctype", doctype);
					} else if (!this.meta.custom) {
						nxenv.set_route("form", "customize-form", {
							doc_type: doctype,
						});
					}
				},
		...
		if (nxenv.user.has_role("System Manager") && nxenv.boot.developer_mode) {
			// edit doctype
			items.push({
				label: __("Edit DocType", null, "Button in list view menu"),
				action: () => nxenv.set_route("form", "doctype", doctype),
				standard: true,
			});
		}
	...
	show_list_settings() {
		nxenv.model.with_doctype(this.doctype, () => {
			new ListSettings({
				listview: this,
				doctype: this.doctype,
				settings: this.list_view_settings,
				meta: nxenv.get_meta(this.doctype),
			});
		});
	}
	...
	get_workflow_action_menu_items() {
		...
		if (nxenv.model.has_workflow(this.doctype)) {
			const actions = nxenv.workflow.get_all_transition_actions(this.doctype);
			actions.forEach((action) => {
				workflow_actions.push({
					...
					action: () => {
						...
						nxenv
							.xcall("nxenv.model.workflow.bulk_workflow_approval", {
	...
	toggle_workflow_actions() {
		if (!nxenv.model.has_workflow(this.doctype)) return;
		...
		nxenv
			.xcall("nxenv.model.workflow.get_common_transition_actions", {
				docs: checked_items,
				doctype: this.doctype,
			})
			.then((actions) => {
	...
	get_actions_menu_items() {
		...
		const is_field_editable = (field_doc) => {
			return (
				field_doc.fieldname &&
				nxenv.model.is_value_type(field_doc) &&
		...
		const bulk_edit = () => {
			return {
				...
				action: () => {
					...
					nxenv.meta.get_docfields(doctype).forEach((field_doc) => {
		...
		// bulk printing
		if (nxenv.model.can_print(doctype)) {
			actions_menu_items.push(bulk_printing());
		}

		// bulk submit
		if (
			nxenv.model.is_submittable(doctype) &&
			has_submit_permission(doctype) &&
			!nxenv.model.has_workflow(doctype)
		) {
			actions_menu_items.push(bulk_submit());
		}

		// bulk cancel
		if (nxenv.model.can_cancel(doctype) && !nxenv.model.has_workflow(doctype)) {
			actions_menu_items.push(bulk_cancel());
		}

		// bulk delete
		if (nxenv.model.can_delete(doctype) && !nxenv.model.has_workflow(doctype)) {
	...
	parse_filters_from_route_options() {
		...
		if (!params.toString() && nxenv.route_options) {
			params = new Map(Object.entries(nxenv.route_options));
		}
		...
		params.forEach((value, field) => {
			...
			if (!doctype) {
				doctype = nxenv.meta.get_doctype_for_field(this.doctype, field);
			}
...
nxenv.get_list_view = (doctype) => {
	let route = `List/${doctype}/List`;
	return nxenv.views.list_view[route];
};
