...
nxenv.views.ListViewSelect = class ListViewSelect {
	constructor(opts) {
		$.extend(this, opts);
	...
	set_current_view() {
		...
		const view_name = nxenv.utils.to_title_case(route[2] || "");
		if (route.length > 2 && nxenv.views.view_modes.includes(view_name)) {
	...
	set_route(view, calendar_name) {
		...
		if (search_params) {
			nxenv.route_options = Object.fromEntries(search_params);
		}
	...
	setup_views() {
		const views = {
			...
			Calendar: {
				condition: nxenv.views.calendar[this.doctype],
			...
			Inbox: {
				condition: this.doctype === "Communication" && nxenv.boot.email_accounts.length,
				action: () => this.set_route("inbox"),
				current_view_handler: () => {
					const accounts = this.get_email_accounts();
					let default_action;
					if (has_common(nxenv.user_roles, ["System Manager", "Administrator"])) {
						default_action = {
							label: __("New Email Account"),
							action: () => nxenv.new_doc("Email Account"),
						};
					}
			...
			Tree: {
				condition:
					nxenv.treeview_settings[this.doctype] || nxenv.get_meta(this.doctype).is_tree,
		...
		nxenv.views.view_modes.forEach((view) => {
	...
	setup_kanban_switcher(kanbans) {
		...
		if (can_create) {
			this.page.add_custom_menu_item(
				...
				() => nxenv.views.KanbanView.show_kanban_dialog(this.doctype),
				true
			);
		}
	}
	...
	get_page_name() {
		return nxenv.utils.to_title_case(nxenv.get_route().slice(-1)[0] || "");
	}
	...
	get_reports() {
		...
		var reports =
			Object.values(nxenv.boot.user.all_reports).sort((a, b) =>
	...
	setup_kanban_boards() {
		function fetch_kanban_board(doctype) {
			nxenv.db.get_value("Kanban Board", { reference_doctype: doctype }, "name", (board) => {
				if (!$.isEmptyObject(board)) {
					nxenv.set_route("list", doctype, "kanban", board.name);
				} else {
					nxenv.views.KanbanView.show_kanban_dialog(doctype);
				}
			});
		}

		const last_opened_kanban =
			nxenv.model.user_settings[this.doctype]["Kanban"]?.last_kanban_board;
		if (!last_opened_kanban) {
			fetch_kanban_board(this.doctype);
		} else {
			nxenv.db.exists("Kanban Board", last_opened_kanban).then((exists) => {
				if (exists) {
					nxenv.set_route("list", this.doctype, "kanban", last_opened_kanban);
				} else {
					fetch_kanban_board(this.doctype);
				}
			});
	...
	get_calendars() {
		...
		return nxenv.db
			...
			.then((result) => {
				...
				if (nxenv.views.calendar[this.doctype]) {
	...
	get_email_accounts() {
		let accounts_to_add = [];
		let accounts = nxenv.boot.email_accounts;
	...
	slug() {
		return nxenv.router.slug(nxenv.router.doctype_layout || this.doctype);
	}
};
