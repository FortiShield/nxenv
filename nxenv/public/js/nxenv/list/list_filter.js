...
export default class ListFilter {
	constructor({ wrapper, doctype }) {
		...
		this.can_add_global = nxenv.user.has_role(["System Manager", "Administrator"]);
		this.filters = [];
		this.make();
		this.bind();
		this.refresh();
	...
	make() {
		...
		this.filter_input = nxenv.ui.form.make_control({
			df: {
				fieldtype: "Data",
		...
		this.is_global_input = nxenv.ui.form.make_control({
			df: {
				fieldtype: "Check",
				label: __("Is Global"),
			},
	...
	filter_template(filter) {
		return `<div class="list-link filter-pill list-sidebar-button btn btn-default" data-name="${
			...
			<a class="remove">${nxenv.utils.icon("close")}</a>
		</div>`;
	}
	...
	save_filter(filter_name) {
		return nxenv.db.insert({
			doctype: "List Filter",
			reference_doctype: this.list_view.doctype,
			filter_name,
			for_user: this.is_global_input.get_value() ? "" : nxenv.session.user,
	...
	remove_filter(name) {
		if (!name) return;
		return nxenv.db.delete_doc("List Filter", name);
	}
