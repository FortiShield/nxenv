export default class BulkOperations {
	constructor({ doctype }) {
		if (!doctype) nxenv.throw(__("Doctype required"));
		this.doctype = doctype;
	}

	print(docs) {
		const print_settings = nxenv.model.get_doc(":Print Settings", "Print Settings");
		const allow_print_for_draft = cint(print_settings.allow_print_for_draft);
		const is_submittable = nxenv.model.is_submittable(this.doctype);
		...
		const valid_docs = docs
			.filter((doc) => {
				return (
					...
					nxenv.user.has_role("Administrator")
				);
			})
			.map((doc) => doc.name);
		...
		if (invalid_docs.length > 0) {
			nxenv.msgprint(__("You selected Draft or Cancelled documents"));
			return;
		}

		if (valid_docs.length === 0) {
			nxenv.msgprint(__("Select atleast 1 record for printing"));
			return;
		}
		...
		const dialog = new nxenv.ui.Dialog({
			...
			fields: [
				...
				},
				{
					...
					fieldname: "print_sel",
					options: nxenv.meta.get_print_formats(this.doctype),
					default: nxenv.get_meta(this.doctype).default_print_format,
				},
				{
					fieldtype: "Select",
					label: __("Page Size"),
					fieldname: "page_size",
					options: nxenv.meta.get_print_sizes(),
		...
		dialog.set_primary_action(__("Print"), (args) => {
			if (!args) return;
			const default_print_format = nxenv.get_meta(this.doctype).default_print_format;
			...
			if (args.page_size === "Custom") {
				if (args.page_height === 0 || args.page_width === 0) {
					nxenv.throw(__("Page height and width cannot be zero"));
				}
			...
			if (args.background_print) {
				nxenv
					.call("nxenv.utils.print_format.download_multi_pdf_async", {
					...
					.then((response) => {
						...
						nxenv.realtime.task_subscribe(task_id);
						nxenv.realtime.on(`task_complete:${task_id}`, (data) => {
							nxenv.msgprint({
							...
							nxenv.realtime.task_unsubscribe(task_id);
							nxenv.realtime.off(`task_complete:${task_id}`);
						});
					});
			} else {
				const w = window.open(
					"/api/method/nxenv.utils.print_format.download_multi_pdf?" +
						"doctype=" +
	...
	delete(docnames, done = null) {
		nxenv
			.call({
				method: "nxenv.desk.reportview.delete_items",
				freeze: true,
				freeze_message:
					docnames.length <= 10
			...
			.then((r) => {
				...
				if (failed.length && !r._server_messages) {
					nxenv.throw(__("Cannot delete {0}", [failed.map((f) => f.bold()).join(", ")]));
				}
	...
	assign(docnames, done) {
		if (docnames.length > 0) {
			const assign_to = new nxenv.ui.form.AssignToDialog({
				obj: this,
				method: "nxenv.desk.form.assign_to.add_multiple",
				doctype: this.doctype,
				docname: docnames,
				bulk_assign: true,
				re_assign: true,
			...
			nxenv.msgprint(__("Select records for assignment"));
		}
	}
	...
	clear_assignment(docnames, done) {
		if (docnames.length > 0) {
			nxenv
				.call({
					method: "nxenv.desk.form.assign_to.remove_multiple",
					args: {
						doctype: this.doctype,
						names: docnames,
			...
			nxenv.msgprint(__("Select records for removing assignment"));
		}
	}
	...
	apply_assignment_rule(docnames, done) {
		if (docnames.length > 0) {
			nxenv
				.call("nxenv.automation.doctype.assignment_rule.assignment_rule.bulk_apply", {
					doctype: this.doctype,
					docnames: docnames,
				})
				.then(() => done());
	...
	submit_or_cancel(docnames, action = "submit", done = null) {
		...
		nxenv.realtime.task_subscribe(task_id);
		return nxenv
			.xcall("nxenv.desk.doctype.bulk_update.bulk_update.submit_cancel_or_update_docs", {
				doctype: this.doctype,
				action: action,
				docnames: docnames,
				task_id: task_id,
			...
			.then((failed_docnames) => {
				if (failed_docnames?.length) {
					const comma_separated_records = nxenv.utils.comma_and(failed_docnames);
					switch (action) {
						case "submit":
							nxenv.throw(__("Cannot submit {0}.", [comma_separated_records]));
							break;
						case "cancel":
							nxenv.throw(__("Cannot cancel {0}.", [comma_separated_records]));
							break;
						default:
							nxenv.throw(__("Cannot {0} {1}.", [action, comma_separated_records]));
					}
				}
				if (failed_docnames?.length < docnames.length) {
					nxenv.utils.play_sound(action);
					if (done) done();
				}
			})
			.finally(() => {
	...
	edit(docnames, field_mappings, done) {
		...
		const dialog = new nxenv.ui.Dialog({
			...
			primary_action: ({ value }) => {
				...
				nxenv
					.call({
						method: "nxenv.desk.doctype.bulk_update.bulk_update.submit_cancel_or_update_docs",
						args: {
							doctype: this.doctype,
							freeze: true,
							docnames: docnames,
					...
					.then((r) => {
						...
						dialog.hide();
						nxenv.show_alert(__("Updated successfully"));
					});
			},
		...
		const dialog = new nxenv.ui.Dialog({
			...
			fields: [
				{
					...
					get_data: function (txt) {
						return nxenv.db.get_link_options("Tag", txt);
					},
				},
			],
			...
			primary_action: () => {
				...
				if (args && args.tags) {
					...
					nxenv.call({
						method: "nxenv.desk.doctype.tag.tag.add_tags",
						args: {
							tags: args.tags,
							dt: this.doctype,
							docs: docnames,
	...
	export(doctype, docnames) {
		nxenv.require("data_import_tools.bundle.js", () => {
			const data_exporter = new nxenv.data_import.DataExporter(
				doctype,
				"Insert New Records"
			);
