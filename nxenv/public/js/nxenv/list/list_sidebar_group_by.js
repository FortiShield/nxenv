...
nxenv.views.ListGroupBy = class ListGroupBy {
	constructor(opts) {
		$.extend(this, opts);
		this.make_wrapper();
	...
	render_group_by_items() {
		let get_item_html = (fieldname) => {
			...
			if (fieldname === "assigned_to") {
				...
				label = nxenv.meta.get_label(this.doctype, fieldname);
				let docfield = nxenv.meta.get_docfield(this.doctype, fieldname);
				if (!docfield) {
					return;
				}
			...
			return `<div class="group-by-field list-link">
						...
						href="#" onclick="return false;">
							...
							<span>${nxenv.utils.icon("select", "xs")}</span>
						</a>
	...
	setup_search($dropdown) {
		nxenv.utils.setup_search($dropdown, ".group-by-item", ".group-by-value", "data-name");
	}
	...
	get_group_by_count(field) {
		...
		return nxenv.call("nxenv.desk.listview.get_group_by_count", args).then((r) => {
			let field_counts = r.message || [];
			field_counts = field_counts.filter((f) => f.count !== 0);
			let current_user = field_counts.find((f) => f.name === nxenv.session.user);
			field_counts = field_counts.filter(
				(f) => !["Guest", "Administrator", nxenv.session.user].includes(f.name)
			);
			// Set nxenv.session.user on top of the list
	...
	get_dropdown_html(field, fieldtype, applied = false) {
		...
		if (field.name == null) {
			label = __("Not Set");
		} else if (field.name === nxenv.session.user) {
			label = __("Me");
		...
		let applied_html = applied
			? `<span class="applied"> ${nxenv.utils.icon("tick", "xs")} </span>`
			: "";
