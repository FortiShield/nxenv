...
nxenv.views.ListSidebar = class ListSidebar {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	}
	...
	make() {
		var sidebar_content = nxenv.render_template("list_sidebar", { doctype: this.doctype });
	...
	setup_views() {
		...
		if (this.doctype === "Communication" && nxenv.boot.email_accounts.length) {
			this.sidebar.find('.list-link[data-view="Inbox"]').removeClass("hide");
			show_list_link = true;
		}

		if (nxenv.treeview_settings[this.doctype] || nxenv.get_meta(this.doctype).is_tree) {
			this.sidebar.find(".tree-link").removeClass("hide");
		}

		this.current_view = "List";
		var route = nxenv.get_route();
		if (route.length > 2 && nxenv.views.view_modes.includes(route[2])) {
	...
	setup_kanban_boards() {
		...
		nxenv.views.KanbanView.setup_dropdown_in_sidebar(this.doctype, $dropdown);
	}

	setup_keyboard_shortcuts() {
		this.sidebar.find(".list-link > a, .list-link > .btn-group > a").each((i, el) => {
			nxenv.ui.keys.get_shortcut_group(this.page).add($(el));
		});
	}

	setup_list_group_by() {
		this.list_group_by = new nxenv.views.ListGroupBy({
			doctype: this.doctype,
			sidebar: this,
			list_view: this.list_view,
			page: this.page,
	...
	get_stats() {
		...
		nxenv.call({
			method: "nxenv.desk.reportview.get_sidebar_stats",
			type: "GET",
			args: {
				stats: me.stats,
				doctype: me.doctype,
			...
			callback: function (r) {
				...
				nxenv.utils.setup_search(stats_dropdown, ".stat-link", ".stat-label");
			},
		});
	}
	...
	render_stat(stats) {
		...
		let tag_list = $(nxenv.render_template("list_sidebar_stat", args)).on(
			"click",
			".stat-link",
			(e) => {
	...
	add_insights_banner() {
		try {
			...
			const link = "https://nxenv.io/s/insights";
			const cta = "Nxenv Insights";
