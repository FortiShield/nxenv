...
nxenv.views.BaseList = class BaseList {
	constructor(opts) {
	...
	setup_defaults() {
		...
		this.page_title = this.page_title || nxenv.router.doctype_layout || __(this.doctype);
		this.meta = nxenv.get_meta(this.doctype);
		this.settings = nxenv.listview_settings[this.doctype] || {};
		this.user_settings = nxenv.get_user_settings(this.doctype);

		this.start = 0;
		this.page_length = nxenv.is_large_screen() ? 100 : 20;
		this.selected_page_count = this.page_length;
		this.data = [];
		this.method = "nxenv.desk.reportview.get";

		this.can_create = nxenv.model.can_create(this.doctype);
	...
	get_list_view_settings() {
		return nxenv
			.call("nxenv.desk.listview.get_list_settings", {
	...
	async set_fields() {
		let fields = [].concat(nxenv.model.std_fields_list, this.meta.title_field);
	...
	_add_field(fieldname, doctype) {
		...
		const is_valid_field =
			nxenv.model.std_fields_list.includes(fieldname) ||
			nxenv.meta.has_field(doctype, fieldname) ||
			fieldname === "_seen";
	...
	set_stats() {
		...
		this.workflow_state_fieldname = nxenv.workflow.get_state_fieldname(this.doctype);
		if (this.workflow_state_fieldname) {
			if (!nxenv.workflow.workflows[this.doctype]["override_status"]) {
	...
	fetch_meta() {
		return nxenv.model.with_doctype(this.doctype);
	}
	...
	setup_view_menu() {
		if (nxenv.boot.desk_settings.view_switcher && !this.meta.force_re_route_to_default_view) {
			const icon_map = {
				Image: "image-view",
				List: "list",
				Report: "small-file",
			...
			this.views_list = new nxenv.views.ListViewSelect({
				doctype: this.doctype,
	...
	set_breadcrumbs() {
		nxenv.breadcrumbs.add(this.meta.module, this.doctype);
	}
	...
	setup_side_bar() {
		...
		this.list_sidebar = new nxenv.views.ListSidebar({
			doctype: this.doctype,
			stats: this.stats,
	...
	setup_list_wrapper() {
		this.$nxenv_list = $('<div class="nxenv-list">').appendTo(this.page.main);
	}
	...
	setup_result_area() {
		...
		this.$nxenv_list.append(this.$result);
	}
	...
	setup_no_result_area() {
		...
		`).hide();
		this.$nxenv_list.append(this.$no_result);
	}

	setup_freeze_area() {
		this.$freeze = $('<div class="freeze"></div>').hide();
		this.$nxenv_list.append(this.$freeze);
	}
	...
	setup_paging_area() {
		...
		this.$nxenv_list.append(this.$paging_area);
	...
	get_fields() {
		...
		return this.fields.map((f) => nxenv.model.get_full_column_name(f[0], f[1]));
	}

	get_group_by() {
		let name_field = this.fields && this.fields.find((f) => f[0] == "name");
		if (name_field) {
			return nxenv.model.get_full_column_name(name_field[0], name_field[1]);
		}
		return null;
	}
	...
	prepare_data(r) {
		...
		Object.assign(nxenv.boot.user_info, data.user_info);
...
class FilterArea {
	constructor(list_view) {
		...
		this.debounced_refresh_list_view = nxenv.utils.debounce(
	...
	clear(refresh = true) {
		...
		return nxenv.run_serially(promises).then(() => {
			this.trigger_refresh = true;
			...
			doctype_fields
				.filter(
					(df) =>
						...
						(df.in_standard_filter && nxenv.model.is_value_type(df.fieldtype))
				)
				.map((df) => {
					...
					if (
						df.fieldtype == "Link" &&
						df.options &&
						nxenv.boot.treeviews.includes(df.options)
					) {
						condition = "descendants of (inclusive)";
					}
	...
	make_filter_list() {
			...
			<div class="btn-group">
				<button class="btn btn-default btn-sm filter-button">
					<span class="filter-icon button-icon">
						${nxenv.utils.icon("es-line-filter")}
					</span>
				...
				<button class="btn btn-default btn-sm filter-x-button" title="${__("Clear all filters")}">
					<span class="filter-icon button-icon">
						${nxenv.utils.icon("es-small-close")}
					</span>
				</button>
			</div>
		...
		this.filter_list = new nxenv.ui.FilterGroup({
...
nxenv.views.is_valid = (view_mode) => nxenv.views.view_modes.includes(view_mode);
