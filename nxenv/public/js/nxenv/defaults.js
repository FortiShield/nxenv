...
nxenv.defaults = {
	get_user_default: function (key) {
		let defaults = nxenv.boot.user.defaults;
		let d = defaults[key];
		if (!d && nxenv.defaults.is_a_user_permission_key(key)) {
			d = defaults[nxenv.model.scrub(key)];
		...
		if (!nxenv.defaults.in_user_permission(key, d)) {
			return;
		}
	...
	get_user_defaults: function (key) {
		var defaults = nxenv.boot.user.defaults;
		var d = defaults[key];

		if (nxenv.defaults.is_a_user_permission_key(key)) {
			if (d && $.isArray(d) && d.length === 1) {
				// Use User Permission value when only when it has a single value
				d = d[0];
			} else {
				d = defaults[key] || defaults[nxenv.model.scrub(key)];
			}
		}
		if (!$.isArray(d)) d = [d];
		...
		d.filter((item) => {
			if (nxenv.defaults.in_user_permission(key, item)) {
				return item;
			}
		});
		return d;
	...
	set_user_default_local: function (key, value) {
		nxenv.boot.user.defaults[key] = value;
	},
	get_default: function (key) {
		...
		if (nxenv.defaults.is_a_user_permission_key(key)) {
			if (value && $.isArray(value) && value.length === 1) {
				...
				value = defaults[nxenv.model.scrub(key)];
			}
		}

		if (!nxenv.defaults.in_user_permission(key, value)) {
			return;
		}
	...
	is_a_user_permission_key: function (key) {
		return key.indexOf(":") === -1 && key !== nxenv.model.scrub(key);
	},

	in_user_permission: function (key, value) {
		let user_permission = this.get_user_permissions()[nxenv.model.unscrub(key)];
	...
	update_user_permissions: function () {
		const method = "nxenv.core.doctype.user_permission.user_permission.get_user_permissions";
		nxenv.call(method).then((r) => {
			if (r.message) {
	...
	load_user_permission_from_boot: function () {
		if (nxenv.boot.user.user_permissions) {
			this._user_permissions = Object.assign({}, nxenv.boot.user.user_permissions);
		} else {
