		...
		this.dialog = new nxenv.ui.Dialog({
			...
			primary_action: (data) => {
				...
				if (!this.editing && !data.name) {
					data.name = `${this.type}-${this.label}-${nxenv.utils.get_random(20)}`;
				}
	...
	get_title() {
		if (this.editing) {
			switch (this.type) {
				...
				default:
					return __("Edit {0}", [__(nxenv.model.unscrub(this.type))]);
			}
		}
		...
		switch (this.type) {
			...
			default:
				return __("New {0}", [__(nxenv.model.unscrub(this.type))]);
		}
	}
...
class QuickListDialog extends WidgetDialog {
	...
	generate_filter_from_json() {
		if (this.values && this.values.quick_list_filter) {
			this.filters = nxenv.utils.get_filter_from_json(
...
class ShortcutDialog extends WidgetDialog {
				...
				onchange: () => {
					if (this.dialog.get_value("type") == "DocType") {
						this.dialog.fields_dict.link_to.get_query = () => {
							return {
								query: "nxenv.core.report.permitted_documents_for_user.permitted_documents_for_user.query_doctypes",
								filters: {
			...
			},
			{
				...
				onchange: () => {
					...
					if (doctype && this.dialog.get_value("type") == "DocType") {
						nxenv.model.with_doctype(doctype, async () => {
							let meta = nxenv.get_meta(doctype);

							if (doctype && nxenv.boot.single_types.includes(doctype)) {
								this.hide_filters();
							} else if (doctype) {
							...
							if (nxenv.boot.calendars.includes(doctype)) views.push("Calendar");

							const response = await nxenv.db.get_value(
								"Kanban Board",
			...
			},
			{
				...
				depends_on: (state) => {
					if (this.dialog) {
						...
						let is_single = nxenv.boot.single_types.includes(doctype);
	...
	process_data(data) {
		...
		data.label = data.label ? data.label : nxenv.model.unscrub(data.link_to);

		if (data.url) {
			let _url = data.url;
			if (data.url.startsWith("/")) {
				_url = nxenv.urllib.get_base_url() + data.url;
			}
			!validate_url(_url) &&
				nxenv.throw({
...
class NumberCardDialog extends WidgetDialog {
			...
			return [
				{
					...
					get_query: () => {
						return {
							query: "nxenv.desk.doctype.number_card.number_card.get_cards_for_user",
							filters: {
	...
	set_aggregate_function_fields() {
		...
		if (this.document_type && nxenv.get_meta(this.document_type)) {
			nxenv.get_meta(this.document_type).fields.map((df) => {
		...
		return [
			{
				...
				get_query: () => {
					return {
						query: "nxenv.desk.doctype.custom_html_block.custom_html_block.get_custom_blocks_for_user",
					};
				},
			},
		];
