...
nxenv.provide("nxenv.dashboards");
nxenv.provide("nxenv.dashboards.chart_sources");
...
export default class ChartWidget extends Widget {
	...
	render_time_series_filters() {
		...
		nxenv.dashboard_utils.render_chart_filters(filters, "chart-actions", this.action_area, 0);
	}
			...
			filters = [
				{
					...
					options: nxenv.dashboard_utils.get_years_since_creation(
						nxenv.boot.user.creation
					),
	...
	render_date_range_field() {
		if (!this.date_field_wrapper || !this.date_field_wrapper.is(":visible")) {
			...
			this.date_range_field = nxenv.ui.form.make_control({
				df: {
					fieldtype: "DateRange",
					fieldname: "from_date",
	...
	get_report_chart_data(result) {
		if (result.chart && this.chart_doc.use_report_chart) {
			...
			let columns = result.columns.map((col) => {
				return nxenv.report_utils.prepare_field_from_column(col);
			});

			return nxenv.report_utils.make_chart_options(columns, result, chart_fields).data;
		}
	}
	...
	prepare_chart_actions() {
		let actions = [
			...
			},
			{
				...
				action: "action-edit",
				handler: () => {
					nxenv.set_route("Form", "Dashboard Chart", this.chart_doc.name);
				},
			},
			{
		...
		if (this.chart_doc.document_type) {
			actions.push({
				...
				action: "action-list",
				handler: () => {
					nxenv.set_route("List", this.chart_doc.document_type);
				},
			});
			...
			actions.push({
				...
				action: "action-list",
				handler: () => {
					nxenv.set_route("query-report", this.chart_doc.report_name, this.filters);
				},
			});
		}
	...
	setup_filter_dialog(fields) {
		...
		if (this.chart_doc.chart_type == "Report") {
			...
			nxenv.query_report = new nxenv.views.QueryReport({ filters: dialog.fields_list });
			nxenv.query_reports[this.chart_doc.report_name].onload &&
				nxenv.query_reports[this.chart_doc.report_name].onload(nxenv.query_report);
		}
	...
	save_chart_config_for_user(config, reset = 0) {
		...
		nxenv.xcall("nxenv.desk.doctype.dashboard_settings.dashboard_settings.save_chart_config", {
			reset: reset,
	...
	create_filter_group_and_add_filters() {
		...
		this.filters &&
			nxenv.model.with_doctype(this.chart_doc.document_type, () => {
	...
	fetch(filters, refresh = false, args) {
		...
		return nxenv.xcall(method, args);
	}
	...
	async render() {
		let setup_dashboard_chart = () => {
			...
			if (!this.dashboard_chart) {
				this.dashboard_chart = nxenv.utils.make_chart(this.chart_wrapper[0], chart_args);
			} else {
		...
		if (!this.data || !this.data.labels || !Object.keys(this.data).length) {
			...
			if (this.chart_doc.document_type) {
				nxenv.model.with_doctype(this.chart_doc.document_type, setup_dashboard_chart);
			} else {
	...
	get_chart_args() {
		...
		if (this.chart_doc.document_type) {
			let doctype_meta = nxenv.get_meta(this.chart_doc.document_type);
	...
	update_chart_object() {
		nxenv.db.get_doc("Dashboard Chart", this.chart_doc.name).then((doc) => {
			this.chart_doc = doc;
	...
	prepare_chart_object() {
		if (this.chart_doc.type == "Heatmap" && !this.chart_doc.heatmap_year) {
			this.chart_doc.heatmap_year = nxenv.dashboard_utils.get_year(
				nxenv.datetime.now_date()
			);
		}
	...
	set_chart_filters() {
		...
		let chart_saved_filters = nxenv.dashboard_utils.get_all_filters(this.chart_doc);

		if (this.chart_doc.chart_type == "Report") {
			return nxenv.dashboard_utils
				.get_filters_for_chart_type(this.chart_doc)
				.then((filters) => {
					...
					this.filters =
						nxenv.utils.parse_array(user_saved_filters) ||
						nxenv.utils.parse_array(this.filters) ||
						nxenv.utils.parse_array(chart_saved_filters);
				});
		} else {
			this.filters =
				nxenv.utils.parse_array(user_saved_filters) ||
	...
	get_settings() {
		return nxenv.model.with_doc("Dashboard Chart", this.chart_name).then((chart_doc) => {
			if (chart_doc) {
				this.chart_doc = chart_doc;
				if (this.chart_doc.chart_type == "Custom") {
					// custom source
					if (nxenv.dashboards.chart_sources[this.chart_doc.source]) {
						this.settings = nxenv.dashboards.chart_sources[this.chart_doc.source];
						return Promise.resolve();
					} else {
						const method =
							"nxenv.desk.doctype.dashboard_chart_source.dashboard_chart_source.get_config";
						return nxenv
							.xcall(method, { name: this.chart_doc.source })
							.then((config) => {
								nxenv.dom.eval(config);
								this.settings =
									nxenv.dashboards.chart_sources[this.chart_doc.source];
							});
					}
				} else if (this.chart_doc.chart_type == "Report") {
					this.settings = {
					...
					this.settings = {
						method: "nxenv.desk.doctype.dashboard_chart.dashboard_chart.get",
					};
					return Promise.resolve();
				}
			}
