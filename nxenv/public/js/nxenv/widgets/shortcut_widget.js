...
export default class ShortcutWidget extends Widget {
	...
	setup_events() {
		this.widget.click((e) => {
			...
			let filters = nxenv.utils.get_filter_from_json(this.stats_filter);
	...
	set_actions() {
		...
		$(nxenv.utils.icon("es-line-arrow-up-right", "xs", "", "", "ml-2")).appendTo(
			this.action_area
		);
		...
		let filters = nxenv.utils.process_filter_expression(this.stats_filter);

		if (
			this.type == "DocType" &&
			this.doc_view != "New" &&
			!nxenv.boot.single_types.includes(this.link_to)
		) {
			nxenv.db
				.count(this.link_to, {
					filters: filters || [],
