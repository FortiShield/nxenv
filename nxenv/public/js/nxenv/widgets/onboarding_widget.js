...
export default class OnboardingWidget extends Widget {
	...
	add_step(step, index) {
		...
		let $step = $(`<a class="onboarding-step ${status}">
				<div class="step-title">
					<div class="step-index step-pending">${nxenv.utils.icon(
						"es-line-success",
						"md",
						"",
						"",
					...
					<div class="step-index step-skipped">${nxenv.utils.icon(
					...
					<div class="step-index step-complete">${nxenv.utils.icon(
						"es-solid-success",
						"md",
						"",
						"",
	...
	show_step(step) {
		...
		let actions = {
			...
			"Create Entry": (step) => {
				if (step.is_complete) {
					nxenv.set_route(`/app/List/${step.reference_document}`);
				} else {
		...
		const set_description = () => {
			let content = __(step.description)
				? nxenv.markdown(__(step.description))
				: `<h1>${__(step.title)}</h1>`;

			if (step.action === "Create Entry") {
				// add a secondary action to view list
				content += `<p>
					<a href='/app/${nxenv.router.slug(step.reference_document)}'>
		...
		const toggle_video = () => {
			...
			let plyr = new nxenv.Plyr(video[0], {
				hideControls: true,
				resetOnEnd: true,
			});
	...
	go_to_page(step) {
		...
		nxenv.set_route(step.path).then(() => {
			let message =
	...
	open_report(step) {
		let route = nxenv.utils.generate_route({
		...
		let current_route = nxenv.get_route();

		nxenv.set_route(route).then(() => {
			let msg_dialog = nxenv.msgprint({
				message: __(step.report_description),
				title: __(step.reference_report),
				primary_action: {
					action: () => {
						nxenv.set_route(current_route).then(() => {
							this.mark_complete(step);
						});
						msg_dialog.hide();
					},
			...
			nxenv.msg_dialog.custom_onhide = () => this.mark_complete(step);
		});
	}
	...
	show_form_tour(step) {
		let route;
		if (step.is_single) {
			route = nxenv.router.slug(step.reference_document);
		} else {
			route = `${nxenv.router.slug(step.reference_document)}/new`;
		}

		let current_route = nxenv.get_route();

		nxenv.route_hooks = {};
		nxenv.route_hooks.after_load = (frm) => {
			const on_finish = () => {
				let msg_dialog = nxenv.msgprint({
	...
	update_settings(step) {
		...
		nxenv.route_hooks.after_save = (frm) => {
			let success = false;
			let args = {};
			...
			if (success) {
				...
				args.secondary_action = {
					action: () => nxenv.set_route(current_route),
					label: __("Go Back"),
				};
			...
			nxenv.msgprint(args);
			nxenv.msg_dialog.custom_onhide = () => custom_onhide();
		};

		nxenv.set_route("Form", step.reference_document);
	}
	...
	async create_entry(step) {
		...
		if (step.is_submittable) {
			nxenv.route_hooks.after_save = () => {
				nxenv.msgprint({
					message: __("Submit this document to complete this step."),
					title: __("Document Saved"),
				});
			};
			nxenv.route_hooks.after_submit = callback;
		} else {
			nxenv.route_hooks.after_save = callback;
		}

		nxenv.set_route("Form", step.reference_document, docname);
	}

	show_quick_entry(step) {
		let current_route = nxenv.get_route_str();
		nxenv.ui.form.make_quick_entry(
			step.reference_document,
			() => {
				if (nxenv.get_route_str() != current_route) {
					let success_dialog = nxenv.msgprint({
						message: __("Let's take you back to onboarding"),
						title: __("Document Saved"),
						primary_action: {
							action: () => {
	...
	show_video(step) {
		nxenv.help.show_video(step.video_url, step.title);
		this.mark_complete(step);
	}
	...
	update_step_status(step, status, value, callback) {
		...
		nxenv
			.call("nxenv.desk.desktop.update_onboarding_step", {
				name: step.name,
				field: status,
				value: value,
			})
	...
	show_success() {
		...
		let success_state_image =
			this.success_state_image || "/assets/nxenv/images/ui-states/success-color.png";
		let documentation = "";
		if (this.docs_url) {
			documentation = __(
	...
	is_dismissed() {
		...
		if (Object.keys(dismissed).includes(this.title)) {
			...
			let today = new Date();
			let diff = nxenv.datetime.get_hour_diff(today, last_hidden);
			return diff < 24;
		}
		return false;
	}
	...
	set_actions() {
		...
		dismiss.on("click", () => {
			let dismissed = JSON.parse(localStorage.getItem("dismissed-onboarding") || "{}");
			dismissed[this.title] = nxenv.datetime.now_datetime();

			localStorage.setItem("dismissed-onboarding", JSON.stringify(dismissed));
			this.delete(true, true);
			this.widget.closest(".ce-block").hide();
			nxenv.telemetry.capture("dismissed_" + nxenv.scrub(this.title), "nxenv_onboarding");
		});
		...
		return nxenv.model
			...
			.then((onboarding_doc) => {
				if (onboarding_doc) {
					...
					const method =
						"nxenv.desk.doctype.onboarding_step.onboarding_step.get_onboarding_steps";
					return nxenv
	...
	async get_first_document(doctype) {
		const { message } = await nxenv.db.get_value("Form Tour", { reference_doctype: doctype }, [
			"first_document",
		]);
		let docname;

		if (message.first_document) {
			await nxenv.db.get_list(doctype, { order_by: "creation" }).then((res) => {
