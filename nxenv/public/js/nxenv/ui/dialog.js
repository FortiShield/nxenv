...
nxenv.ui.open_dialogs = [];

nxenv.ui.Dialog = class Dialog extends nxenv.ui.FieldGroup {
	constructor(opts) {
		super();
		this.display = false;
		this.is_dialog = true;
	...
	make() {
		...
		this.$wrapper
			.on("hide.bs.modal", function () {
				...
				nxenv.ui.form.get_open_grid_form?.()?.hide_form();

				if (nxenv.ui.open_dialogs[nxenv.ui.open_dialogs.length - 1] === me) {
					nxenv.ui.open_dialogs.pop();
					if (nxenv.ui.open_dialogs.length) {
						window.cur_dialog =
							nxenv.ui.open_dialogs[nxenv.ui.open_dialogs.length - 1];
					} else {
						window.cur_dialog = null;
					}
				}
			...
			.on("shown.bs.modal", function () {
				...
				nxenv.ui.open_dialogs.push(me);
				me.focus_on_first_input();
				me.hide_scrollbar(true);
				me.on_page_show && me.on_page_show();
				$(document).trigger("nxenv.ui.Dialog:shown");
	...
	toggle_minimize() {
		...
		this.get_minimize_btn().html(nxenv.utils.icon(icon));
