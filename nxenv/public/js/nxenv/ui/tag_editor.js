nxenv.ui.TagEditor = class TagEditor {
	constructor(opts) {
		/* docs:
		Arguments
	...
	setup_tags() {
		...
		this.tags = new nxenv.ui.Tags({
			...
			onTagRemove: (tag) => {
				if (!me.refreshing) {
					return nxenv.call({
						method: "nxenv.desk.doctype.tag.tag.remove_tag",
						args: me.get_args(tag),
						callback: function (r) {
	...
	setup_awesomplete() {
		...
		$input.on("input", function (e) {
			...
			nxenv.call({
				method: "nxenv.desk.doctype.tag.tag.get_tags",
				args: {
