nxenv.provide("nxenv.ui.keys");

let shortcut_groups = new WeakMap();
let shortcut_group_list = [];
nxenv.ui.keys.shortcut_groups = shortcut_groups;

nxenv.ui.keys.get_shortcut_group = (parent) => {
	// parent must be an object
	if (!shortcut_groups.has(parent)) {
		shortcut_groups.set(parent, new nxenv.ui.keys.AltShortcutGroup());
	}
...
nxenv.ui.keys.bind_shortcut_group_event = () => {
	...
	$(document).on("keydown", (e) => {
		let key = (nxenv.ui.keys.key_map[e.which] || "").toLowerCase();
...
nxenv.ui.keys.AltShortcutGroup = class AltShortcutGroup {
	constructor() {
		this.shortcuts_dict = {};
	...
	is_taken(letter) {
		let is_in_global_shortcut = nxenv.ui.keys.standard_shortcuts
			.filter((s) => !s.page)
