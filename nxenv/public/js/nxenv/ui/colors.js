...
nxenv.ui.color = {
	get: function (color_name, shade) {
		...
		return nxenv.ui.color_map;
	},
	get_color: function (color_name) {
		const color_names = Object.keys(nxenv.ui.color_map);
		if (color_names.includes(color_name)) {
			return nxenv.ui.color_map[color_name];
		} else {
	...
	all: function () {
		return Object.values(nxenv.ui.color_map).reduce((acc, curr) => acc.concat(curr), []);
	},
	names: function () {
		return Object.keys(nxenv.ui.color_map);
	},
	...
	get_color_name: function (hex) {
		for (const key in nxenv.ui.color_map) {
			const colors = nxenv.ui.color_map[key];
...
nxenv.ui.color_map = nxenv.ui.color.get_color_map();
