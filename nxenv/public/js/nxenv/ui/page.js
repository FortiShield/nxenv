 ...
 *
 * @returns {nxenv.ui.Page}
 */

/**
 * @typedef {Object} nxenv.ui.Page
 */

nxenv.ui.make_app_page = function (opts) {
	opts.parent.page = new nxenv.ui.Page(opts);
	return opts.parent.page;
};

nxenv.ui.pages = {};

nxenv.ui.Page = class Page {
	constructor(opts) {
		$.extend(this, opts);
		...
		this.make();
		nxenv.ui.pages[nxenv.get_route_str()] = this;
	}
	...
	load_lib(callback) {
		nxenv.require(this.required_libs, callback);
	}

	add_main_section() {
		$(nxenv.render_template("page", {})).appendTo(this.wrapper);
	...
	setup_sidebar_toggle() {
		...
		if (this.disable_sidebar_toggle || !sidebar_wrapper.length) {
			...
			sidebar_toggle.click(() => {
				if (nxenv.utils.is_xs() || nxenv.utils.is_sm()) {
	...
	add_action_icon(icon, click, css_class = "", tooltip_label) {
		...
		if (!tooltip_label) {
			...
			tooltip_label = nxenv.unscrub(icon);
		}
	...
	get_icon_label(icon, label) {
		...
		return `${icon ? nxenv.utils.icon(icon_name, size) : ""} <span class="hidden-xs"> ${__(
			label
		)} </span>`;
	}
	...
	set_action(btn, opts) {
		...
		nxenv.ui.keys.get_shortcut_group(this).add(btn, text_span.length ? text_span : btn);
	}
	...
	add_dropdown_item({
		...
		if (icon) {
			$icon = `<span class="menu-item-icon">${nxenv.utils.icon(icon)}</span>`;
		}
		...
		if (shortcut) {
			...
			`);
			nxenv.ui.keys.add_shortcut(shortcut_obj);
		} else {
			$li = $(`
				<li>
					<a class="grey-link dropdown-item" href="#" onClick="return false;">
		...
		// alt shortcut
		nxenv.ui.keys.get_shortcut_group(parent.get(0)).add($link, $link.find(".menu-item-label"));
	...
	set_title(title, icon = null, strip = true, tab_title = "", tooltip_label = "") {
		...
		this.title = title;
		nxenv.utils.set_title(tab_title || title);
		if (icon) {
			title = `${nxenv.utils.icon(icon)} ${title}`;
		}
	...
	add_button(label, click, opts) {
		...
		let button = $(`<button
			class="btn ${opts.btn_class || "btn-default"} ${opts.btn_size || "btn-sm"} ellipsis">
				${opts.icon ? nxenv.utils.icon(opts.icon) : ""}
				${label}
		</button>`);
	...
	add_dropdown_button(parent, label, click, icon) {
		nxenv.ui.toolbar.add_dropdown_button(parent, label, click, icon);
	}
	...
	add_field(df, parent) {
		...
		var f = nxenv.ui.form.make_control({
			df: df,
