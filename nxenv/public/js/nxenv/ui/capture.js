// nxenv.ui.Capture
// Author - Achilles Rasquinha <achilles@nxenv.io>
 ...
 *
 * @example
 * nxenv._.get_data_uri(video)
 * // returns "data:image/pngbase64,..."
 */
nxenv._.get_data_uri = (element) => {
...
/**
 * @description Nxenv's Capture object.
 *
 * @example
 * const capture = nxenv.ui.Capture()
 * capture.show()
 *
 ...
 */
nxenv.ui.Capture = class {
	constructor(options = {}) {
		this.options = nxenv.ui.Capture.OPTIONS;
	...
	set_options(options) {
		this.options = { ...nxenv.ui.Capture.OPTIONS, ...options };
	...
	show() {
		...
		if (cint(nxenv.boot.sysdefaults.force_web_capture_mode_for_uploads)) {
			this.show_for_desktop();
		} else if (nxenv.is_mobile()) {
	...
	build_dialog() {
		...
		me.$template = $(nxenv.ui.Capture.TEMPLATE);
	...
	show_for_desktop() {
		...
		this.render_stream()
			...
			.catch((err) => {
				if (me.options.error) {
					nxenv.show_alert(nxenv.ui.Capture.ERR_MESSAGE, 3);
				}
	...
	setup_take_photo_action() {
		...
		this.dialog.set_primary_action(__("Take Photo"), () => {
			const data_url = nxenv._.get_data_uri(me.video);
...
nxenv.ui.Capture.OPTIONS = {
	title: __("Camera"),
	animate: false,
	error: false,
};
nxenv.ui.Capture.ERR_MESSAGE = __("Unable to load camera.");
nxenv.ui.Capture.TEMPLATE = `
<div class="nxenv-capture">
	<div class="embed-responsive embed-responsive-16by9 fc-stream-container">
		<video class="fc-stream embed-responsive-item">${nxenv.ui.Capture.ERR_MESSAGE}</video>
	</div>
