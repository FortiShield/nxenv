nxenv.ui.FilterGroup = class {
	...
	add_filters(filters) {
		...
		return nxenv.run_serially(promises).then(() => this.update_filters());
	}
	...
	validate_args(doctype, fieldname) {
		if (
			...
			nxenv.model.is_non_std_field(fieldname)
		) {
			nxenv.msgprint({
	...
	_push_new_filter(doctype, fieldname, condition, value, hidden = false) {
		...
		let filter = new nxenv.ui.Filter(args);
	...
	filter_exists(filter_value) {
		...
		return this.filters
			...
			.some((f) => {
				...
				return nxenv.utils.arrays_equal(f_value.slice(0, 4), filter_value.slice(0, 4));
			});
	}
