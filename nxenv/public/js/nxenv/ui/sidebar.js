nxenv.ui.Sidebar = class Sidebar {
	constructor() {
		this.items = {};
	...
	make_dom() {
		...
		this.wrapper = $(nxenv.render_template("sidebar")).prependTo("body");

		this.app_switcher_dropdown = $(
			nxenv.render_template("apps_switcher", {
				app_logo_url: nxenv.boot.app_data[0].app_logo_url,
				app_title: __(nxenv.boot.app_data[0].app_title),
			})
		...
		this.apps_switcher = new nxenv.ui.AppsSwitcher(this);
	...
	set_all_pages() {
		this.sidebar_pages = nxenv.boot.sidebar_pages;
	...
	set_default_app() {
		...
		nxenv.boot.app_data.sort((a, b) => (a.workspaces.length < b.workspaces.length ? 1 : -1));
		nxenv.current_app = nxenv.boot.app_data[0].app_name;
	}
	...
	setup_pages() {
		...
		if (this.all_pages) {
			nxenv.workspaces = {};
			nxenv.workspace_list = [];
			nxenv.workspace_map = {};
			for (let page of this.all_pages) {
				nxenv.workspaces[nxenv.router.slug(page.name)] = {
					name: page.name,
					public: page.public,
				};
				if (!page.app && page.module) {
					page.app = nxenv.boot.module_app[nxenv.slug(page.module)];
				}
				nxenv.workspace_map[page.name] = page;
				nxenv.workspace_list.push(page);
			}
			this.make_sidebar();
		}
		this.set_hover();
	...
	make_sidebar() {
		...
		let app_workspaces = nxenv.boot.app_data_map[nxenv.current_app || "nxenv"].workspaces;

		let parent_pages = this.all_pages.filter((p) => !p.parent_page).uniqBy((p) => p.name);
		if (nxenv.current_app === "private") {
		...
		this.wrapper.find(".selected").length &&
			!nxenv.dom.is_element_in_viewport(this.wrapper.find(".selected")) &&
	...
	sidebar_item_container(item) {
		...
		if (item.type === "Link") {
			if (item.link_type === "Report") {
				path = nxenv.utils.generate_route({
					type: item.link_type,
					name: item.link_to,
				...
				path = nxenv.utils.generate_route({ type: item.link_type, name: item.link_to });
			}
		} else if (item.type === "URL") {
			path = item.external_link;
		} else {
			if (item.public) {
				path = "/app/" + nxenv.router.slug(item.name);
			} else {
				path = "/app/private/" + nxenv.router.slug(item.name.split("-")[0]);
			}
		}
						...
						<span class="sidebar-item-icon" item-icon=${item.icon || "folder-normal"}>
							${
								item.public || item.icon
									? nxenv.utils.icon(item.icon || "folder-normal", "md")
	...
	add_toggle_children(item, sidebar_control, item_container) {
		...
		let $drop_icon = $(`<button class="btn-reset drop-icon hidden">`)
			.html(nxenv.utils.icon(drop_icon, "sm"))
		...
		for (let container of this.$sidebar.find(".nested-container")) {
			this.sorting_items[this.sorting_items.length] = Sortable.create(container, {
				...
				onEnd: () => {
					...
					nxenv.xcall(
						"nxenv.desk.doctype.workspace_settings.workspace_settings.set_sequence",
						{
	...
	reload() {
		return nxenv.workspace.get_pages().then((r) => {
