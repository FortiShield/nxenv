...
nxenv.ui.ThemeSwitcher = class ThemeSwitcher {
	constructor() {
		this.setup_dialog();
		this.refresh();
	}
	...
	bind_events() {
		this.dialog.$wrapper.on("keydown", (e) => {
			...
			const key = nxenv.ui.keys.get_key(e);
			let increment_by;
	...
	toggle_theme(theme) {
		...
		nxenv.show_alert(__("Theme Changed"), 3);

		nxenv.xcall("nxenv.core.doctype.user.user.switch_theme", {
			theme: toTitle(theme),
		});
	}
...
nxenv.ui.add_system_theme_switch_listener = () => {
	nxenv.ui.dark_theme_media_query.addEventListener("change", () => {
		nxenv.ui.set_theme();
	});
};

nxenv.ui.dark_theme_media_query = window.matchMedia("(prefers-color-scheme: dark)");

nxenv.ui.set_theme = (theme) => {
	const root = document.documentElement;
	let theme_mode = root.getAttribute("data-theme-mode");
	if (!theme) {
		if (theme_mode === "automatic") {
			theme = nxenv.ui.dark_theme_media_query.matches ? "dark" : "light";
		}
	}
