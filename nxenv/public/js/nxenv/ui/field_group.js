...
nxenv.ui.FieldGroup = class FieldGroup extends nxenv.ui.form.Layout {
	constructor(opts) {
		super(opts);
		this.dirty = false;
		this.fetch_dict = {};
		...
		if (this.fields) {
			...
			$.each(this.fields_list, function (i, field) {
				if (field.df["default"]) {
					...
					if (def_value == "Today" && field.df["fieldtype"] == "Date") {
						def_value = nxenv.datetime.get_today();
					}
			...
			$(this.wrapper)
				...
				.on("change awesomplete-selectcomplete", () => {
					this.dirty = true;
					nxenv.run_serially([() => nxenv.timeout(0.1), () => me.refresh_dependency()]);
				});
		}
	}
