...
nxenv.provide("nxenv.ui.keys.handlers");

nxenv.ui.keys.setup = function () {
	$(window).on("keydown", function (e) {
		var key = nxenv.ui.keys.get_key(e);
		if (nxenv.ui.keys.handlers[key]) {
			var out = null;
			for (var i = 0, l = nxenv.ui.keys.handlers[key].length; i < l; i++) {
				var handler = nxenv.ui.keys.handlers[key][i];
...
nxenv.ui.keys.standard_shortcuts = standard_shortcuts;
nxenv.ui.keys.add_shortcut = ({
	...
	nxenv.ui.keys.off(shortcut, page);
	// attach new handler
	nxenv.ui.keys.on(shortcut, handler);
...
nxenv.ui.keys.show_keyboard_shortcut_dialog = () => {
	if (nxenv.ui.keys.is_dialog_shown) return;
		...
		let html = shortcuts
			...
			.map((shortcut) => {
				let shortcut_label = shortcut.shortcut
					.split("+")
					.map(nxenv.utils.to_title_case)
					.join("+");
				if (nxenv.utils.is_mac()) {
	...
	let dialog = new nxenv.ui.Dialog({
		...
		on_hide() {
			nxenv.ui.keys.is_dialog_shown = false;
		},
	});
	...
	dialog.show();
	nxenv.ui.keys.is_dialog_shown = true;
};

nxenv.ui.keys.get_key = function (e) {
	var keycode = e.keyCode || e.which;
	var key = nxenv.ui.keys.key_map[keycode] || String.fromCharCode(keycode);
...
nxenv.ui.keys.on = function (key, handler) {
	if (!nxenv.ui.keys.handlers[key]) {
		nxenv.ui.keys.handlers[key] = [];
	}
	nxenv.ui.keys.handlers[key].push(handler);
};

nxenv.ui.keys.off = function (key, page) {
	let handlers = nxenv.ui.keys.handlers[key];
	if (!handlers || handlers.length === 0) return;
	nxenv.ui.keys.handlers[key] = handlers.filter((h) => {
...
nxenv.ui.keys.add_shortcut({
	...
	action: function (e) {
		...
		nxenv.app.trigger_primary_action();
		e.preventDefault();
		return false;
	},
...
nxenv.ui.keys.on("escape", function (e) {
	handle_escape_key();
});

nxenv.ui.keys.on("esc", function (e) {
	handle_escape_key();
});

nxenv.ui.keys.on("enter", function (e) {
	if (window.cur_dialog && cur_dialog.confirm_dialog) {
		cur_dialog.get_primary_btn().trigger("click");
	}
});

nxenv.ui.keys.on("ctrl+down", function (e) {
	const grid_row = nxenv.ui.form.get_open_grid_form();
...
nxenv.ui.keys.on("ctrl+up", function (e) {
	const grid_row = nxenv.ui.form.get_open_grid_form();
...
"abcdefghijklmnopqrstuvwxyz".split("").forEach((letter, i) => {
	nxenv.ui.keys.key_map[65 + i] = letter;
});
