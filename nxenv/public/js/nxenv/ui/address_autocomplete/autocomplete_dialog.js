...
nxenv.ui.AddressAutocompleteDialog = class AddressAutocompleteDialog {
	constructor(opts) {
	...
	_get_dialog() {
		...
		const dialog = new nxenv.ui.Dialog({
			...
			fields: [
				{
					...
					reqd: 1,
					get_query:
						"nxenv.integrations.doctype.geolocation_settings.geolocation_settings.autocomplete",
					onchange: () => {
						// Disable "Create Address" button if mandatory fields are missing
						nxenv.model.with_doctype("Address", () => {
							const address = this.parse_selected_value();
							const mandatory_fields = nxenv
								.get_meta("Address")
								.fields.filter(
									(field) =>
										field.reqd &&
			...
			primary_action: () => {
				...
				nxenv.db.insert(address).then((doc) => {
