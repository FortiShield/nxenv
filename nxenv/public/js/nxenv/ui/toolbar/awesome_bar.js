...
nxenv.provide("nxenv.tags");

nxenv.search.AwesomeBar = class AwesomeBar {
	setup(element) {
		var me = this;
		...
		var awesomplete = new Awesomplete(input, {
			...
			item: function (item, term) {
				...
				let target = "#";
				if (d.route) {
					target = nxenv.router.make_url(
						nxenv.router.convert_from_standard_route(
							nxenv.router.get_route_from_arguments(
			...
			nxenv.utils.debounce(function (e) {
				...
				if (txt && txt.length > 1) {
					...
					me.options = me.options.concat(
						me.deduplicate(nxenv.search.utils.get_recent_pages(txt || ""))
					);
					me.options = me.options.concat(nxenv.search.utils.get_frequent_links());
				}
				me.add_help();
		...
		$input.on("awesomplete-select", function (e) {
			...
			if (item.route_options) {
				nxenv.route_options = item.route_options;
			}
		...
		nxenv.search.utils.setup_recent();
	}
	...
	add_help() {
		this.options.push({
			...
			onclick: function () {
				...
				nxenv.msgprint(txt, __("Search Help"));
			},
		});
	}
	...
	build_options(txt) {
		var options = nxenv.search.utils
			.get_creatables(txt)
			.concat(
				nxenv.search.utils.get_search_in_list(txt),
				nxenv.search.utils.get_doctypes(txt),
				nxenv.search.utils.get_reports(txt),
				nxenv.search.utils.get_pages(txt),
				nxenv.search.utils.get_workspaces(txt),
				nxenv.search.utils.get_dashboards(txt),
				nxenv.search.utils.get_recent_pages(txt || ""),
				nxenv.search.utils.get_executables(txt),
				nxenv.search.utils.get_marketplace_apps(txt)
			);
		if (txt.charAt(0) === "#") {
			options = nxenv.tags.utils.get_tags(txt);
		}
	...
	make_global_search(txt) {
		...
		// 		<span class="ellipsis">Search for ${nxenv.utils.xss_sanitise(txt).bold()}</span>
		// 		<kbd>↵</kbd>
		// 	</span>
		// `);

		// search_text.click(() => {
		// 	nxenv.searchdialog.search.init_search(txt, "global_search");
		// });
		...
		this.options.push({
			label: `
				<span class="flex justify-between text-medium">
					<span class="ellipsis">${__("Search for {0}", [nxenv.utils.xss_sanitise(txt).bold()])}</span>
					<kbd>↵</kbd>
				</span>
			`,
	...
	make_search_in_current(txt) {
		...
		if (route[0] === "List" && txt.indexOf(" in") === -1) {
			...
			const doctype = nxenv.container.page?.list_view?.doctype;
			if (!doctype) return;
		...
		if (first == parseInt(first) || first === "(" || first === "=") {
			...
			try {
				...
				this.options.push({
					...
					default: "Calculator",
					onclick: function () {
						nxenv.msgprint(formatted_value, __("Result"));
					},
				});
			} catch (e) {
				// pass
	...
	make_random(txt) {
		if (txt.toLowerCase().includes("random")) {
			this.options.push({
				...
				value: nxenv.utils.get_random(16),
				onclick: function () {
					nxenv.msgprint(nxenv.utils.get_random(16), __("Result"));
				},
			});
		}
	}
