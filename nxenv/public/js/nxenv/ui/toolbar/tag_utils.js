...
nxenv.tags.utils = {
	get_tags: function (txt) {
		...
		if (!nxenv.tags.tags) {
			nxenv.tags.utils.fetch_tags();
			return [];
		}

		nxenv.tags.tags.forEach((tag) => {
			const search_result = nxenv.search.utils.fuzzy_search(txt, tag, true);
			if (search_result.score) {
				out.push({
					...
					onclick() {
						...
						nxenv.searchdialog.search.init_search("#".concat(tag), "tags");
					},
				});
			}
		});
	...
	fetch_tags() {
		nxenv.call({
			method: "nxenv.desk.doctype.tag.tag.get_tags_list_for_awesomebar",
			callback: function (r) {
				if (r && r.message) {
					nxenv.tags.tags = $.extend([], r.message);
				}
			},
		});
	},
	...
	get_tag_results: function (tag) {
		...
		return new Promise(function (resolve) {
			nxenv.call({
				method: "nxenv.desk.doctype.tag.tag.get_documents_for_tag",
				args: {
					tag: tag,
				},
				callback: function (r) {
