...
$.extend(nxenv.model, {
	...
	init: function () {
		...
		nxenv.realtime.on("doc_update", function (data) {
			var doc = locals[data.doctype] && locals[data.doctype][data.name];

			if (doc) {
				// current document is dirty, show message if its not me
				if (
					nxenv.get_route()[0] === "Form" &&
					cur_frm.doc.doctype === doc.doctype &&
					cur_frm.doc.name === doc.name
				) {
					if (data.modified !== cur_frm.doc.modified && !nxenv.ui.form.is_saving) {
						if (!cur_frm.is_dirty()) {
					...
					if (!doc.__unsaved) {
						...
						nxenv.model.remove_from_locals(doc.doctype, doc.name);
					} else {
	...
	is_value_type: function (fieldtype) {
		...
		return nxenv.model.no_value_type.indexOf(fieldtype) === -1;
	},

	is_non_std_field: function (fieldname) {
		return ![...nxenv.model.std_fields_list, ...nxenv.model.child_table_field_list].includes(
			fieldname
		);
	},
	...
	get_std_field: function (fieldname, ignore = false) {
		var docfield = $.map(
			[].concat(nxenv.model.std_fields).concat(nxenv.model.std_fields_table),
			function (d) {
		...
		if (!docfield.length) {
			...
			if (ignore) {
				...
				nxenv.msgprint(__("Unknown Column: {0}", [fieldname]));
			}
		}
		return docfield[0];
	},
	...
	set_in_localstorage: function (doctype, docs) {
		try {
			...
			nxenv.model.clear_local_storage();
			localStorage["_doctype:" + doctype] = JSON.stringify(docs);
		}
	},
	...
	with_doctype: function (doctype, callback, async) {
		if (locals.DocType[doctype]) {
			...
			let cached_docs = nxenv.model.get_from_localstorage(doctype);
			...
			return nxenv.call({
				method: "nxenv.desk.form.load.getdoctype",
				type: "GET",
				args: {
					doctype: doctype,
					with_parent: 1,
				...
				callback: function (r) {
					if (r.exc) {
						nxenv.msgprint(__("Unable to load: {0}", [__(doctype)]));
						throw "No doctype";
					}
					if (r.message == "use_cache") {
						nxenv.model.sync(meta);
					} else {
						nxenv.model.set_in_localstorage(doctype, r.docs);
						meta = r.docs[0];
					}
					nxenv.model.init_doctype(meta);

					if (r.user_settings) {
						// remember filters and other settings from last view
						nxenv.model.user_settings[doctype] = JSON.parse(r.user_settings);
						nxenv.model.user_settings[doctype].updated_on = moment().toString();
					}
					callback && callback(r);
				},
			});
	...
	init_doctype: function (meta) {
		if (meta.name === "DocType") {
			...
			nxenv.meta.__doctype_meta = JSON.parse(JSON.stringify(meta));
		}
		...
		if (meta.__templates) {
			$.extend(nxenv.templates, meta.__templates);
		}
	},
	...
	with_doc: function (doctype, name, callback) {
		return new Promise((resolve) => {
			...
			if (
				...
				resolve(nxenv.get_doc(doctype, name));
			} else {
				return nxenv.call({
					method: "nxenv.desk.form.load.getdoc",
					type: "GET",
					args: {
						doctype: doctype,
						name: name,
	...
	get_docinfo: function (doctype, name) {
		return (nxenv.model.docinfo[doctype] && nxenv.model.docinfo[doctype][name]) || null;
	},

	set_docinfo: function (doctype, name, key, value) {
		if (nxenv.model.docinfo[doctype] && nxenv.model.docinfo[doctype][name]) {
			nxenv.model.docinfo[doctype][name][key] = value;
		}
	},

	get_shared: function (doctype, name) {
		return nxenv.model.get_docinfo(doctype, name).shared;
	},

	get_server_module_name: function (doctype) {
		var dt = nxenv.model.scrub(doctype);
		var module = nxenv.model.scrub(locals.DocType[doctype].module);
		var app = nxenv.boot.module_app[module];
	...
	can_create: function (doctype) {
		return nxenv.boot.user.can_create.indexOf(doctype) !== -1;
	},

	can_select: function (doctype) {
		if (nxenv.boot.user) {
			return nxenv.boot.user.can_select.indexOf(doctype) !== -1;
		}
	},

	can_read: function (doctype) {
		if (nxenv.boot.user) {
			return nxenv.boot.user.can_read.indexOf(doctype) !== -1;
		}
	},

	can_write: function (doctype) {
		return nxenv.boot.user.can_write.indexOf(doctype) !== -1;
	},

	can_get_report: function (doctype) {
		return nxenv.boot.user.can_get_report.indexOf(doctype) !== -1;
	},

	can_delete: function (doctype) {
		if (!doctype) return false;
		return nxenv.boot.user.can_delete.indexOf(doctype) !== -1;
	},

	can_submit: function (doctype) {
		if (!doctype) return false;
		return nxenv.boot.user.can_submit.indexOf(doctype) !== -1;
	},

	can_cancel: function (doctype) {
		if (!doctype) return false;
		return nxenv.boot.user.can_cancel.indexOf(doctype) !== -1;
	},

	has_workflow: function (doctype) {
		return nxenv.get_list("Workflow", { document_type: doctype, is_active: 1 }).length;
	},
	...
	is_single: function (doctype) {
		if (!doctype) return false;
		return nxenv.boot.single_types.indexOf(doctype) != -1;
	},
	...
	can_import: function (doctype, frm, meta = null) {
		...
		if (nxenv.user_roles.includes("System Manager")) return true;

		if (frm) return frm.perm[0].import === 1;
		return nxenv.boot.user.can_import.indexOf(doctype) !== -1;
	},
	...
	can_export: function (doctype, frm) {
		...
		return nxenv.boot.user.can_export.indexOf(doctype) !== -1;
	},

	can_print: function (doctype, frm) {
		if (frm) return frm.perm[0].print === 1;
		return nxenv.boot.user.can_print.indexOf(doctype) !== -1;
	},

	can_email: function (doctype, frm) {
		if (frm) return frm.perm[0].email === 1;
		return nxenv.boot.user.can_email.indexOf(doctype) !== -1;
	},

	can_share: function (doctype, frm) {
		let disable_sharing = cint(nxenv.sys_defaults.disable_document_sharing);

		if (disable_sharing && nxenv.session.user !== "Administrator") {
			return false;
		}
		...
		return nxenv.boot.user.can_share.indexOf(doctype) !== -1;
	},

	has_value: function (dt, dn, fn) {
		// return true if property has value
		var val = locals[dt] && locals[dt][dn] && locals[dt][dn][fn];
		var df = nxenv.meta.get_docfield(dt, fn, dn);

		let ret;
		if (nxenv.model.table_fields.includes(df.fieldtype)) {
			ret = false;
	...
	get_list: function (doctype, filters) {
		...
		return nxenv.utils.filter_dict(docsdict, filters);
	},
	...
	get_value: function (doctype, filters, fieldname, callback) {
		if (callback) {
			...
			if (
				...
				var l = nxenv.get_list(doctype, filters);
	...
	set_value: function (
		...
		$.each(to_update, (key, value) => {
			if (doc && doc[key] !== value) {
				...
				doc[key] = value;
				tasks.push(() => nxenv.model.trigger(key, value, doc, skip_dirty_trigger));
			} else {
	...
	on: function (doctype, fieldname, fn) {
		...
		*/
		/* example: nxenv.model.on("Customer", "age", function(fieldname, value, doc) {
		  if(doc.age < 16) {
		   	nxenv.msgprint("Warning, Customer must atleast be 16 years old.");
		    raise "CustomerAgeError";
		  }
		}) */
		nxenv.provide("nxenv.model.events." + doctype);
		if (!nxenv.model.events[doctype][fieldname]) {
			nxenv.model.events[doctype][fieldname] = [];
		}
		nxenv.model.events[doctype][fieldname].push(fn);
	},
		...
		function enqueue_events(events) {
			...
			for (const fn of events) {
				...
				tasks.push(() => {
					...
					// or use the default promise nxenv.after_ajax
					if (return_value && return_value.then) {
						return return_value;
					} else {
						return nxenv.after_server_call();
					}
				});
			}
		}

		if (nxenv.model.events[doc.doctype]) {
			enqueue_events(nxenv.model.events[doc.doctype][fieldname]);
			enqueue_events(nxenv.model.events[doc.doctype]["*"]);
		}
	...
	get_doc: function (doctype, name) {
		...
		if ($.isPlainObject(name)) {
			var doc = nxenv.get_list(doctype, name);
	...
	get_children: function (doctype, parent, parentfield, filters) {
		...
		if ($.isPlainObject(doctype)) {
			...
			doc = nxenv.get_doc(doctype, parent);
		}

		var children = doc[parentfield] || [];
		if (filters) {
			return nxenv.utils.filter_dict(children, filters);
		} else {
			return children;
		}
	},
	...
	remove_from_locals: function (doctype, name) {
		...
		if (nxenv.views.formview[doctype]) {
			delete nxenv.views.formview[doctype].frm.opendocs[name];
		}
	},
	...
	get_no_copy_list: function (doctype) {
		...
		var docfields = nxenv.get_meta(doctype).fields || [];
	...
	delete_doc: function (doctype, docname, callback) {
		let title = docname;
		const title_field = nxenv.get_meta(doctype).title_field;
		if (nxenv.get_meta(doctype).autoname == "hash" && title_field) {
			const value = nxenv.model.get_value(doctype, docname, title_field);
			if (value) {
				title = `${value} (${docname})`;
			}
		}
		nxenv.confirm(__("Permanently delete {0}?", [title.bold()]), function () {
			return nxenv.call({
				method: "nxenv.client.delete",
				args: {
					doctype: doctype,
					name: docname,
				},
				...
				callback: function (r, rt) {
					if (!r.exc) {
						nxenv.utils.play_sound("delete");
						nxenv.model.clear_doc(doctype, docname);
	...
	round_floats_in: function (doc, fieldnames) {
		...
		if (!fieldnames) {
			fieldnames = nxenv.meta.get_fieldnames(doc.doctype, doc.parent, {
	...
	validate_missing: function (doc, fieldname) {
		if (!doc[fieldname]) {
			nxenv.throw(
				__("Please specify") +
					": " +
					__(nxenv.meta.get_label(doc.doctype, fieldname, doc.parent || doc.name))
			);
		}
	},
	...
	is_numeric_field: function (fieldtype) {
		...
		return nxenv.model.numeric_fieldtypes.includes(fieldtype);
	},
	...
	set_default_views_for_doctype(doctype, frm) {
		nxenv.model.with_doctype(doctype, () => {
			...
			if (meta.is_calendar_and_gantt && nxenv.views.calendar[doctype]) {
			...
			if (doctype === "Communication" && nxenv.boot.email_accounts.length) {
...
// legacy
nxenv.get_doc = nxenv.model.get_doc;
nxenv.get_children = nxenv.model.get_children;
nxenv.get_list = nxenv.model.get_list;
