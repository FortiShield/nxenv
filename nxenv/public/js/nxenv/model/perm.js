...
$.extend(nxenv.perm, {
	...
	has_perm: (doctype, permlevel = 0, ptype = "read", doc) => {
		const perms = nxenv.perm.get_perm(doctype, doc);
	...
	get_perm: (doctype, doc) => {
		...
		if (doc && !doc.__islocal) {
			return nxenv.perm._get_perm(doctype, doc);
		}

		return (nxenv.perm.doctype_perm[doctype] ??= nxenv.perm._get_perm(doctype));
	},
	...
	_get_perm: (doctype, doc) => {
		...
		const user = nxenv.session.user;

		if (user === "Administrator" || nxenv.user_roles.includes("Administrator")) {
			perm[0].read = 1;
		}

		if (!meta) return perm;

		perm = nxenv.perm.get_role_permissions(meta);
		const base_perm = perm[0];

		if (doc) {
			// apply user permissions via docinfo (which is processed server-side)
			let docinfo = nxenv.model.get_docinfo(doctype, doc.name);
			...
			// if owner
			if (doc.owner !== user) {
				for (const right of nxenv.perm.rights) {
			...
			if (docinfo && docinfo.shared) {
				for (const s of docinfo.shared) {
					...
					if (s.read) {
						...
						base_perm.email =
							nxenv.boot.user.can_email.indexOf(doctype) !== -1 ? 1 : 0;
						base_perm.print =
							nxenv.boot.user.can_print.indexOf(doctype) !== -1 ? 1 : 0;
					}
				}
			}
		}

		if (!base_perm.read && nxenv.model.can_read(doctype)) {
			// read via sharing
			base_perm.read = 1;
		}
	...
	get_role_permissions: (meta) => {
		...
		*/
		...
		(meta.permissions || []).forEach((p) => {
			...
			if (nxenv.user_roles.includes(p.role)) {
				nxenv.perm.rights.forEach((right) => {
					if (!p[right]) return;
	...
	get_match_rules: (doctype, ptype) => {
		...
		let perm = nxenv.perm.get_perm(doctype);

		let user_permissions = nxenv.defaults.get_user_permissions();

		if (user_permissions && !$.isEmptyObject(user_permissions)) {
			let rules = {};
			let fields_to_check = nxenv.meta.get_fields_to_check_permissions(doctype);
			$.each(fields_to_check, (i, df) => {
				const user_permissions_for_doctype = user_permissions[df.options] || [];
				const allowed_records = nxenv.perm.get_allowed_docs_for_doctype(
		...
		if (base_perm.read && !base_perm.rights_without_if_owner.has("read")) {
			match_rules.push({ Owner: nxenv.session.user });
		}
		return match_rules;
	},
	...
	get_field_display_status: (df, doc, perm, explain) => {
		...
		if (!perm && doc) {
			perm = nxenv.perm.get_perm(doc.doctype, doc);
		}
	...
	is_visible: (df, doc, perm) => {
		if (typeof df === "string") {
			// df is fieldname
			df = nxenv.meta.get_docfield(doc.doctype, df, doc.parent || doc.name);
		}

		let status = nxenv.perm.get_field_display_status(df, doc, perm);
	...
	get_allowed_docs_for_doctype: (user_permissions, doctype) => {
		...
		return nxenv.perm.filter_allowed_docs_for_doctype(user_permissions, doctype, false);
	},
