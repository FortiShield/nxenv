...
$.extend(nxenv.model, {
	...
	get_new_doc: function (doctype, parent_doc, parentfield, with_mandatory_children) {
		...
		var doc = {
			docstatus: 0,
			doctype: doctype,
			name: nxenv.model.get_new_name(doctype),
			__islocal: 1,
			__unsaved: 1,
			owner: nxenv.session.user,
		};
		nxenv.model.set_default_values(doc, parent_doc);
		...
		if (parent_doc) {
			...
			nxenv.provide("nxenv.model.docinfo." + doctype + "." + doc.name);
		}

		nxenv.model.add_to_locals(doc);

		if (with_mandatory_children) {
			nxenv.model.create_mandatory_children(doc);
		}
		...
		if (nxenv.route_options && nxenv.route_options.name_field) {
			var meta = nxenv.get_meta(doctype);
			// set title field / name as name
			if (meta.autoname && meta.autoname.indexOf("field:") !== -1) {
				doc[meta.autoname.substr(6)] = nxenv.route_options.name_field;
			} else if (meta.autoname && meta.autoname === "prompt") {
				doc.__newname = nxenv.route_options.name_field;
			} else if (meta.title_field) {
				doc[meta.title_field] = nxenv.route_options.name_field;
			}

			delete nxenv.route_options.name_field;
		}

		// set route options
		if (nxenv.route_options && !doc.parent) {
			$.each(nxenv.route_options, function (fieldname, value) {
				var df = nxenv.meta.has_field(doctype, fieldname);
				if (df && !df.no_copy) {
					doc[fieldname] = value;
				}
			});
	...
	make_new_doc_and_get_name: function (doctype, with_mandatory_children) {
		return nxenv.model.get_new_doc(doctype, null, null, with_mandatory_children).name;
	},

	get_new_name: function (doctype) {
		// random hash is added to idenity mislinked files when doc is not saved and file is uploaded.
		return nxenv.router.slug(`new-${doctype}-${nxenv.utils.get_random(10)}`);
	},

	set_default_values: function (doc, parent_doc) {
		let doctype = doc.doctype;
		let docfields = nxenv.meta.get_docfields(doctype);
		let updated = [];

		// Table types should be initialized
		let fieldtypes_without_default = nxenv.model.no_value_type.filter(
			(fieldtype) => !nxenv.model.table_fields.includes(fieldtype)
		);
		docfields.forEach((f) => {
			if (
			...
			let v = nxenv.model.get_default_value(f, doc, parent_doc);
			if (v) {
	...
	create_mandatory_children: function (doc) {
		var meta = nxenv.get_meta(doc.doctype);
		if (meta && meta.istable) return;

		// create empty rows for mandatory table fields
		nxenv.meta.get_docfields(doc.doctype).forEach(function (df) {
			if (df.fieldtype === "Table" && df.reqd) {
				nxenv.model.add_child(doc, df.fieldname);
			}
		});
	},
	...
	get_default_value: function (df, doc, parent_doc) {
		var user_default = "";
		var user_permissions = nxenv.defaults.get_user_permissions();
		let allowed_records = [];
		let default_doc = null;
		let value = null;
		if (user_permissions) {
			({ allowed_records, default_doc } = nxenv.perm.filter_allowed_docs_for_doctype(
		...
		if (df.fieldtype === "Link" && df.options !== "User") {
			...
			if (has_user_permissions && default_doc) {
				...
				if (!df.ignore_user_permissions) {
					var user_defaults = nxenv.defaults.get_user_defaults(df.options);
				...
				if (!user_default) {
					user_default = nxenv.defaults.get_user_default(df.fieldname);
				}
				if (
					!user_default &&
					...
					user_default = nxenv.boot.user.last_selected_values[df.options];
				}
		...
		if (!value || df["default"]) {
			...
			if (default_val == "__user" || default_val.toLowerCase() == "user") {
				value = nxenv.session.user;
			} else if (default_val == "user_fullname") {
				value = nxenv.session.user_fullname;
			} else if (default_val == "Today") {
				value = nxenv.datetime.get_today();
			} else if ((default_val || "").toLowerCase() === "now") {
				if (df.fieldtype == "Time") {
					value = nxenv.datetime.now_time();
				} else {
					// datetime fields are stored in system tz
					value = nxenv.datetime.system_datetime();
				}
			} else if (default_val[0] === ":") {
				var boot_doc = nxenv.model.get_default_from_boot_docs(df, doc, parent_doc);
	...
	get_default_from_boot_docs: function (df, doc, parent_doc) {
		...
		if (nxenv.get_list(df["default"]).length > 0) {
			var ref_fieldname = df["default"].slice(1).toLowerCase().replace(" ", "_");
			var ref_value = parent_doc
				? parent_doc[ref_fieldname]
				: nxenv.defaults.get_user_default(ref_fieldname);
			var ref_doc = ref_value ? nxenv.get_doc(df["default"], ref_value) : null;
	...
	add_child: function (parent_doc, doctype, parentfield, idx) {
		...
		if (arguments.length === 2) {
			parentfield = doctype;
			doctype = nxenv.meta.get_field(parent_doc.doctype, parentfield).options;
		}

		// create row doc
		idx = idx ? idx - 0.1 : (parent_doc[parentfield] || []).length + 1;

		var child = nxenv.model.get_new_doc(doctype, parent_doc, parentfield);
		child.idx = idx;
	...
	copy_doc: function (doc, from_amend, parent_doc, parentfield) {
		...
		let newdoc = nxenv.model.get_new_doc(doc.doctype, parent_doc, parentfield);

		for (let key in doc) {
			// don't copy name and blank fields
			let df = nxenv.meta.get_docfield(doc.doctype, key);
			...
			if (df && !is_internal_field && !is_blocked_field && !is_no_copy && !is_password) {
				...
				if (nxenv.model.table_fields.includes(df.fieldtype)) {
					for (let i = 0, j = value.length; i < j; i++) {
						let d = value[i];
						nxenv.model.copy_doc(d, from_amend, newdoc, df.fieldname);
					}
				} else {
					newdoc[key] = doc[key];
				}
	...
	open_mapped_doc: function (opts) {
		...
		return nxenv.call({
			type: "POST",
			method: "nxenv.model.mapper.make_mapped_doc",
			args: {
				method: opts.method,
			...
			callback: function (r) {
				if (!r.exc) {
					...
					nxenv.set_route("Form", r.message.doctype, r.message.name);
				}
			},
		});
	},
...
nxenv.create_routes = {};
nxenv.new_doc = function (doctype, opts, init_callback) {
	if (doctype === "File") {
	...
	return new Promise((resolve) => {
		if (opts && $.isPlainObject(opts)) {
			nxenv.route_options = opts;
		}
		nxenv.model.with_doctype(doctype, function () {
			if (nxenv.create_routes[doctype]) {
				nxenv.set_route(nxenv.create_routes[doctype]).then(() => resolve());
			} else {
				nxenv.ui.form.make_quick_entry(doctype, null, init_callback).then(() => resolve());
			}
		});
	});
};
