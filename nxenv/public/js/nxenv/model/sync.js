...
Object.assign(nxenv.model, {
	docinfo: {},
	sync: function (r) {
		/* docs:
			extract docs, docinfo (attachments, comments, assignments)
			from incoming request and set in `locals` and `nxenv.model.docinfo`
		*/
		var isPlain;
		...
		if (r.docs) {
			for (var i = 0, l = r.docs.length; i < l; i++) {
				...
				if (locals[d.doctype] && locals[d.doctype][d.name]) {
					// update values
					nxenv.model.update_in_locals(d);
				} else {
					nxenv.model.add_to_locals(d);
				}
				...
				if (d.localname) {
					nxenv.model.rename_after_save(d, i);
				}
			}
		}

		nxenv.model.sync_docinfo(r);
		return r.docs;
	},

	rename_after_save: (d, i) => {
		nxenv.model.new_names[d.localname] = d.name;
		$(document).trigger("rename", [d.doctype, d.localname, d.name]);
		delete locals[d.doctype][d.localname];

		// update docinfo to new dict keys
		if (i === 0) {
			nxenv.model.docinfo[d.doctype][d.name] = nxenv.model.docinfo[d.doctype][d.localname];
			nxenv.model.docinfo[d.doctype][d.localname] = undefined;
		}
	},
	...
	sync_docinfo: (r) => {
		...
		if (r.docinfo) {
			...
			if (!nxenv.model.docinfo[doctype]) {
				nxenv.model.docinfo[doctype] = {};
			}
			nxenv.model.docinfo[doctype][name] = r.docinfo;

			// copy values to nxenv.boot.user_info
			Object.assign(nxenv.boot.user_info, r.docinfo.user_info);
		}
	...
	add_to_locals: function (doc) {
		...
		if (!doc.name && doc.__islocal) {
			...
			if (!doc.parentfield) nxenv.model.clear_doc(doc);

			doc.name = nxenv.model.get_new_name(doc.doctype);

			if (!doc.parentfield)
				nxenv.provide("nxenv.model.docinfo." + doc.doctype + "." + doc.name);
		}
	...
	update_in_locals: function (doc) {
		...
		for (let fieldname in doc) {
			let df = nxenv.meta.get_field(doc.doctype, fieldname);
			if (df && nxenv.model.table_fields.includes(df.fieldtype)) {
				// table
				...
				for (let i = 0; i < doc[fieldname].length; i++) {
					...
					if (local_d) {
						...
						if (!d.name) {
							...
							d.name = nxenv.model.get_new_name(doc.doctype);
						}
