...
// depends on nxenv.versions to manage versioning

nxenv.require = function (items, callback) {
	if (typeof items === "string") {
		items = [items];
	}
	items = items.map((item) => nxenv.assets.bundled_asset(item));

	return new Promise((resolve) => {
		nxenv.assets.execute(items, () => {
			resolve();
			callback && callback();
		});
	});
...
class AssetManager {
	...
	init_local_storage() {
		...
		if (nxenv.boot) localStorage.metadata_version = nxenv.boot.metadata_version;
	}
	...
	execute(items, callback) {
		...
		const version_string =
			nxenv.boot.developer_mode || window.dev_server ? Date.now() : window._version_number;
