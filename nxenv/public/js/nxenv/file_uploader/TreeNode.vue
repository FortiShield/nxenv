	...
	<div class="tree-node" :class="{ opened: node.open }">
		...
		>
			...
			<a
				...
				v-html="nxenv.utils.icon('external-link', 'sm')"
				@click.stop
			/>
		</span>
		<div v-if="node.file_url && nxenv.utils.is_image_file(node.file_url)">
...
let icon = computed(() => {
	let icons = {
		open: nxenv.utils.icon("folder-open", "md"),
		closed: nxenv.utils.icon("folder-normal", "md"),
		leaf: nxenv.utils.icon("primitive-dot", "xs"),
		search: nxenv.utils.icon("search"),
	};
...
let open_file = (filename) => {
	return nxenv.utils.get_form_link("File", filename);
};
