...
nxenv.dom = {
	...
	get_unique_id: function () {
		const id = "unique-" + nxenv.dom.id_count;
		nxenv.dom.id_count++;
		return id;
	},
	set_unique_id: function (ele) {
		...
		var id = "unique-" + nxenv.dom.id_count;
		$ele.attr("id", id);
		nxenv.dom.id_count++;
		return id;
	},
	...
	add: function (parent, newtag, className, cs, innerHTML, onclick) {
		if (parent && parent.substr) parent = nxenv.dom.by_id(parent);
	...
	freeze: function (msg, css_class) {
		...
		nxenv.dom.freeze_count++;
	},
	unfreeze: function () {
		if (!nxenv.dom.freeze_count) return; // anything open?
		nxenv.dom.freeze_count--;
		if (!nxenv.dom.freeze_count) {
	...
	scroll_to_section(section_name) {
		setTimeout(() => {
			...
			if (section.length) {
				...
				nxenv.ui.scroll(section.parent().parent());
			}
		}, 200);
	},
...
nxenv.ellipsis = function (text, max) {
	if (!max) max = 20;
	text = cstr(text);
	if (text.length > max) {
...
nxenv.run_serially = function (tasks) {
...
nxenv.load_image = (src, onload, onerror, preprocess = () => {}) => {
...
nxenv.timeout = (seconds) => {
	return new Promise((resolve) => {
		setTimeout(() => resolve(), seconds * 1000);
	});
};

nxenv.scrub = nxenv.slug = function (text, spacer = "_") {
	return text.replace(/ /g, spacer).toLowerCase();
};

nxenv.unscrub = function (txt) {
	return nxenv.model.unscrub(txt);
};
	...
	if (remove_action) {
		let remove_btn = $(`
			<span class="remove-btn cursor-pointer">
				${nxenv.utils.icon("close", "sm", "es-icon")}
			</span>
		`);
...
nxenv.get_modal = function (title, content) {
				...
				<div class="modal-header">
					...
					<div class="modal-actions">
						<button class="btn btn-modal-minimize btn-link hide">
							${nxenv.utils.icon("collapse")}
						</button>
						<button class="btn btn-modal-close btn-link" data-dismiss="modal">
							${nxenv.utils.icon("close-alt", "sm", "close-alt")}
						</button>
					</div>
				</div>
...
nxenv.is_online = function () {
	if (nxenv.boot.developer_mode == 1) {
...
nxenv.create_shadow_element = function (wrapper, html, css, js) {
	let random_id = "custom-block-" + nxenv.utils.get_random(5).toLowerCase();
	...
	class CustomBlock extends HTMLElement {
		constructor() {
			...
			div.innerHTML = nxenv.dom.remove_script_and_style(html);

			// link global desk css
			let link = document.createElement("link");
			link.rel = "stylesheet";
			link.href = nxenv.assets.bundled_asset("desk.bundle.css");
