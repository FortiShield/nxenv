nxenv.ui.form.MultiSelectDialog = class MultiSelectDialog {
	constructor(opts) {
		...
		if (!this.for_select) {
			nxenv.model.with_doctype(this.doctype, () => this.init());
		} else {
			this.init();
		}
	}
	...
	make_new_document(e) {
		...
		if (e) {
			...
			nxenv.new_doc(this.doctype, true);
		}
	}
	...
	setup_child_datatable() {
		...
		this.child_datatable = new nxenv.DataTable(this.$child_wrapper.get(0), {
	...
	get_primary_filters() {
		...
		if ($.isArray(this.setters)) {
			...
			Object.keys(this.setters).forEach((setter, index) => {
				let df_prop = nxenv.meta.docfield_map[this.doctype][setter];
		...
		if (this.allow_child_item_selection) {
			this.child_doctype = nxenv.meta.get_docfield(
				this.doctype,
				...
				${
					head
						? `<span class="ellipsis text-muted" title="${__(
								nxenv.model.unscrub(column)
						  )}">${__(nxenv.model.unscrub(column))}</span>`
						: column !== "name"
						? `<span class="ellipsis result-row" title="${__(
								result[column] || ""
						  )}">${__(result[column] || "")}</span>`
						: `<a href="${
								"/app/" + nxenv.router.slug(me.doctype) + "/" + result[column] ||
								""
	...
	render_result_list(results, more = 0, empty = true) {
		...
		if (!nxenv.flags.auto_scroll && empty) {
			this.empty_list();
		}
		more_btn.hide();
	...
	async perform_search(args) {
		const res = await nxenv.call({
			type: "GET",
			method: "nxenv.desk.search.search_widget",
			no_spinner: true,
			args: args,
		});
