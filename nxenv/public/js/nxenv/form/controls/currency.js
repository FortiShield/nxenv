nxenv.ui.form.ControlCurrency = class ControlCurrency extends nxenv.ui.form.ControlFloat {
	...
	get_precision() {
		...
		if (typeof this.df.precision != "number" && !this.df.precision) {
			if (nxenv.boot.sysdefaults.currency_precision) {
				this.df.precision = nxenv.boot.sysdefaults.currency_precision;
			} else {
