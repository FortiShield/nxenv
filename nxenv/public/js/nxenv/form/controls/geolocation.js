...
nxenv.ui.form.ControlGeolocation = class ControlGeolocation extends nxenv.ui.form.ControlData {
	...
	set_disp_area(value) {
		...
		if (!this.map_id) {
			this.map_id = nxenv.dom.get_unique_id();
			this.map_area = $(
		...
		if (this.frm) {
			...
			$(document).on("nxenv.ui.Dialog:shown", () => {
				this.make_map();
				if (value) {
					this.bind_leaflet_data(value);
				}
	...
	customize_draw_controls() {
		...
		L.Icon.Default.imagePath = nxenv.utils.map_defaults.image_path;
	}

	bind_leaflet_map() {
		this.map = L.map(this.map_id);
		this.map.setView(nxenv.utils.map_defaults.center, nxenv.utils.map_defaults.zoom);

		L.tileLayer(nxenv.utils.map_defaults.tiles, nxenv.utils.map_defaults.options).addTo(
			this.map
		);
		...
		return new L.Control.Draw({
			...
			draw: {
				...
				polygon: {
					...
					drawError: {
						color: nxenv.ui.color.get("orange"), // Color the shape will turn when intersects
