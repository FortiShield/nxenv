nxenv.ui.form.ControlTime = class ControlTime extends nxenv.ui.form.ControlDate {
	...
	set_time_options() {
		...
		this.time_format = nxenv.defaultTimeFormat;
		this.datepicker_options = {
			language: "en",
			timepicker: true,
			onlyTimepicker: true,
			timeFormat: time_format.toLowerCase().replace("mm", "ii"),
			startDate: nxenv.datetime.now_time(true),
			onSelect: () => {
	...
	set_input(value) {
		...
		let time_format = nxenv.sys_defaults.time_format || "HH:mm:ss";
		if (!should_refresh) {
		...
		if (should_refresh) {
			this.datepicker.selectDate(
				nxenv.datetime.moment_to_date_obj(moment(value, time_format))
			);
		}
	}
	set_datepicker() {
		...
		this.datepicker.$datepicker.find('[data-action="today"]').click(() => {
			this.datepicker.selectDate(nxenv.datetime.now_time(true));
	...
	set_description() {
		...
		const { time_zone } = nxenv.sys_defaults;
	...
	format_for_input(value) {
		if (value) {
			return nxenv.datetime.str_to_user(value, true);
		}
		return "";
	}
	validate(value) {
		if (value && !nxenv.datetime.validate(value)) {
			...
			let time_format =
				sysdefaults && sysdefaults.time_format ? sysdefaults.time_format : "HH:mm:ss";
			nxenv.msgprint(__("Time {0} must be in format: {1}", [value, time_format]));
			return "";
		}
		return value;
	}
