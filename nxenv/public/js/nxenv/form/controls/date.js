nxenv.ui.form.ControlDate = class ControlDate extends nxenv.ui.form.ControlData {
	...
	set_formatted_input(value) {
		...
		if (should_refresh) {
			this.datepicker.selectDate(nxenv.datetime.str_to_obj(value));
		}
	}
	set_date_options() {
		// webformTODO:
		...
		let lang = "en";
		nxenv.boot.user && (lang = nxenv.boot.user.language);
		...
		this.date_format = nxenv.defaultDateFormat;
		this.datepicker_options = {
			...
			firstDay: nxenv.datetime.get_first_day_of_the_week_index(),
			onSelect: () => {
	...
	get_now_date() {
		return nxenv.datetime.convert_to_system_tz(nxenv.datetime.now_date(true), false).toDate();
	}
	set_t_for_today() {
		var me = this;
		this.$input.on("keydown", function (e) {
			if (e.which === 84) {
				...
				if (me.df.fieldtype == "Date") {
					me.set_value(nxenv.datetime.nowdate());
				}
				if (me.df.fieldtype == "Datetime") {
					me.set_value(nxenv.datetime.now_datetime());
				}
				if (me.df.fieldtype == "Time") {
					me.set_value(nxenv.datetime.now_time());
				}
				return false;
			}
		});
	...
	eval_expression(value, type) {
		...
		switch (type) {
			case "date":
				...
				value = nxenv.datetime.user_to_str(value, false);
				break;

			case "time":
				system_fmt = "HH:mm:ss";
				value = nxenv.datetime.user_to_str(value, true);
				break;
	...
	format_for_input(value) {
		if (value) {
			return nxenv.datetime.str_to_user(value, false, true);
		}
		return "";
	}
	validate(value) {
		if (value && !nxenv.datetime.validate(value)) {
			let sysdefaults = nxenv.sys_defaults;
			let date_format =
				sysdefaults && sysdefaults.date_format ? sysdefaults.date_format : "yyyy-mm-dd";
			nxenv.msgprint(__("Date {0} must be in format: {1}", [value, date_format]));
			return "";
		}
		return value;
	}
