nxenv.ui.form.ControlPassword = class ControlPassword extends nxenv.ui.form.ControlData {
	...
	make_input() {
		...
		this.toggle_password.on("click", () => {
			if (this.$input.attr("type") === "password") {
				...
				this.toggle_password.html(nxenv.utils.icon("hide", "sm"));
			} else {
				this.$input.attr("type", "password");
				this.toggle_password.html(nxenv.utils.icon("unhide", "sm"));
			}
		});
	...
	get_password_strength(value) {
		...
		nxenv.call({
			type: "POST",
			method: "nxenv.core.doctype.user.user.test_password_strength",
			args: {
				new_password: value || "",
			},
			callback: function (r) {
