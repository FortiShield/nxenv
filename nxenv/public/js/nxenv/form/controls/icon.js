...
nxenv.ui.form.ControlIcon = class ControlIcon extends nxenv.ui.form.ControlData {
	make_input() {
	...
	get_all_icons() {
		...
		$("#all-symbols > svg > symbol[id]").each(function () {
			this.id.includes("icon-") && nxenv.symbols.push(this.id.replace("icon-", ""));
		});
	}
	...
	make_icon_input() {
		...
		if (!this.selected_icon) {
			this.selected_icon = $(
				`<div class="selected-icon">${nxenv.utils.icon("folder-normal", "md")}</div>`
			);
