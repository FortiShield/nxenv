...
nxenv.ui.form.ControlPhone = class ControlPhone extends nxenv.ui.form.ControlData {
	async make_input() {
	...
	async setup_country_codes() {
		...
		if (data) {
			...
			const data = await nxenv.xcall("nxenv.geo.country_info.get_country_timezone_info");
	...
	input_events() {
		this.$input.keydown((e) => {
			...
			if ([nxenv.ui.keyCode.BACKSPACE].includes(key_code)) {
	...
	setup_country_code_picker() {
		...
		if (!this.selected_icon.length) {
			this.selected_icon = $(
				`<div class="selected-phone">${nxenv.utils.icon("down", "sm")}</div>`
			);
