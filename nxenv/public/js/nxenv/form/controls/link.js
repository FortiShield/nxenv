...
nxenv.ui.form.recent_link_validations = {};

nxenv.ui.form.ControlLink = class ControlLink extends nxenv.ui.form.ControlData {
		...
		$(`<div class="link-field ui-front" style="position: relative;">
			...
			<span class="link-btn">
				<a class="btn-clear" style="display: inline-block;" title="${__("Clear Link")}">
					${nxenv.utils.icon("close", "xs", "es-icon")}
				</a>
				<a class="btn-open" style="display: inline-block;" title="${__("Open Link")}">
					${nxenv.utils.icon("arrow-right", "xs")}
				</a>
			</span>
	...
	show_link_and_clear_buttons() {
		if (this.$input.val() && this.get_options()) {
			...
			this.$link_open.attr("href", nxenv.utils.get_form_link(doctype, name));
	...
	get_reference_doctype() {
		...
		if (this.doctype) return this.doctype;
			...
			return nxenv.get_route && nxenv.get_route()[0] === "List"
				? nxenv.get_route()[1]
				: null;
		}
	}
	...
	is_translatable() {
		return (nxenv.boot?.translated_doctypes || []).includes(this.get_options());
	}
	is_title_link() {
		return (nxenv.boot?.link_title_doctypes || []).includes(this.get_options());
	}
	async set_link_title(value) {
		...
		const link_title =
			nxenv.utils.get_link_title(doctype, value) ||
			(await nxenv.utils.fetch_link_title(doctype, value));
	...
	parse_validate_and_set_in_model(value, e, label) {
		...
		if (label) {
			...
			nxenv.utils.add_link_title(this.df.options, value, label);
		}
	...
	new_doc() {
		...
		if (df && df.get_route_options_for_new_doc) {
			nxenv.route_options = df.get_route_options_for_new_doc(this);
		} else {
			nxenv.route_options = {};
		}

		// partially entered name field
		nxenv.route_options.name_field = this.get_label_value();

		// reference to calling link
		nxenv._from_link = nxenv.utils.deep_clone(this);
		nxenv._from_link_scrollY = $(document).scrollTop();

		nxenv.ui.form.make_quick_entry(doctype, (doc) => {
	...
	setup_awesomeplete() {
		...
		this.awesomplete = new Awesomplete(me.input, {
			...
			item: function (item) {
				...
				return $(`<div role="option">`)
					...
					.html(`<p title="${nxenv.utils.escape_html(_label)}">${html}</p>`)
					.get(0);
			},
			sort: function () {
				return 0;
		...
		this.$input.on(
			"input",
			nxenv.utils.debounce(function (e) {
				...
				var args = {
					...
					page_length: cint(nxenv.boot.sysdefaults?.link_field_results_limit) || 10,
				};
					...
					callback: function (r) {
						...
						if (!me.df.only_select) {
							...
							let custom__link_options =
								nxenv.ui.form.ControlLink.link_options &&
								nxenv.ui.form.ControlLink.link_options(me);
						...
						r.message.forEach((item) => {
							nxenv.utils.add_link_title(doctype, item.value, item.label);
						});
					},
				});
			}, 500)
		...
		this.$input.on("awesomplete-select", function (e) {
			...
			if (me.df.remember_last_selected_value) {
				nxenv.boot.user.last_selected_values[me.df.options] = item.value;
			}
	...
	toggle_href(doctype) {
		if (nxenv.model.can_select(doctype) && !nxenv.model.can_read(doctype)) {
	...
	get_filter_description(filters) {
		...
		function get_filter_description(filter) {
			...
			let docfield = nxenv.meta.get_docfield(doctype, fieldname);
			let label = docfield ? docfield.label : nxenv.model.unscrub(fieldname);
	...
	set_custom_query(args) {
		const is_valid_value = (value, key) => {
			...
			if (this.frm) {
				let field = nxenv.meta.get_docfield(this.frm.doctype, key);
	...
	parse_filters(link_filters) {
		...
		link_filters.forEach((filter) => {
			...
			if (value?.startsWith?.("eval:")) {
				...
				value = nxenv.utils.eval(value, context);
			}
	...
	validate_link_and_fetch(value) {
		...
		if (value) {
			return nxenv
				.xcall("nxenv.client.validate_link", {
					doctype: options,
					docname: value,
