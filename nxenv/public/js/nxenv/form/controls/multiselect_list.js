nxenv.ui.form.ControlMultiSelectList = class ControlMultiSelectList extends (
	nxenv.ui.form.ControlData
) {
	...
	set_selectable_items(options) {
		let html = options
			.map((option) => {
				...
				<div class="multiselect-check">${nxenv.utils.icon("tick", "xs")}</div>
			</li>`;
			})
			.join("");
		if (!html) {
