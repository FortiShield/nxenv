nxenv.ui.form.ControlCode = class ControlCode extends nxenv.ui.form.ControlText {
	make_input() {
	...
	setup_copy_button() {
		...
		this.copy_button.on("click", () => {
			nxenv.utils.copy_to_clipboard(this.get_model_value() || this.get_value());
		});
	...
	make_ace_editor() {
		...
		let change_content = nxenv.utils.debounce(() => {
	...
	set_language() {
		...
		this.editor.setKeyboardHandler(
			`ace/keyboard/${nxenv.boot.user.code_editor_type || "vscode"}`
		);
	}
	...
	load_lib() {
		...
		if (nxenv.boot.developer_mode) {
			this.root_lib_path = "/assets/nxenv/node_modules/ace-builds/src-noconflict/";
		} else {
			this.root_lib_path = "/assets/nxenv/node_modules/ace-builds/src-min-noconflict/";
		}

		this.library_loaded = new Promise((resolve) => {
			nxenv.require(this.root_lib_path + "ace.js", () => {
