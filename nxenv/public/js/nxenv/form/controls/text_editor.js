...
nxenv.ui.form.ControlTextEditor = class ControlTextEditor extends nxenv.ui.form.ControlCode {
	make_wrapper() {
		super.make_wrapper();
	}
	...
	bind_events() {
		this.quill.on(
			"text-change",
			nxenv.utils.debounce((delta, oldDelta, source) => {
		...
		$(this.quill.root).on("keydown", (e) => {
			const key = nxenv.ui.keys && nxenv.ui.keys.get_key(e);
	...
	get_mention_options() {
		...
		return {
			...
			source: nxenv.utils.debounce(async function (search_term, renderList) {
				let method =
					me.mention_search_method || "nxenv.desk.search.get_names_for_mentions";
				let values = await nxenv.xcall(method, {
					search_term,
				});
			...
			renderItem(item) {
				...
				return `${value} ${item.is_group ? nxenv.utils.icon("users") : ""}`;
			},
		};
	}
	...
	parse(value) {
		...
		return nxenv.dom.remove_script_and_style(value);
	}
