nxenv.ui.form.Control = class BaseControl {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	...
	make_wrapper() {
		this.$wrapper = $("<div class='nxenv-control'></div>").appendTo(this.parent);
	...
	get_status(explain) {
		...
		var status = nxenv.perm.get_field_display_status(
			this.df,
			nxenv.model.get_doc(this.doctype, this.docname),
		...
		let value = nxenv.model.get_value(this.doctype, this.docname, this.df.fieldname);
		...
		if (
			...
			!this.only_input &&
			is_null(value) &&
			cint(nxenv.boot.sysdefaults.hide_empty_read_only_fields) &&
	...
	show_translatable_button(value) {
		...
		if (
			...
			!nxenv.model.can_write("Translation") ||
			!value
		)
			return;

		// Disable translation in website
		if (!nxenv.views || !nxenv.views.TranslationManager) return;
		...
		$(translation_btn)
			...
			.on("click", () => {
				if (!this.doc.__islocal) {
					new nxenv.views.TranslationManager({
						df: this.df,
