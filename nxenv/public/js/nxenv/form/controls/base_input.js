nxenv.ui.form.ControlInput = class ControlInput extends nxenv.ui.form.Control {
	...
	make_wrapper() {
		if (this.only_input) {
			this.$wrapper = $('<div class="form-group nxenv-control">').appendTo(this.parent);
		} else {
			this.$wrapper = $(
				`<div class="nxenv-control">
	...
	refresh_input() {
		...
		if (me.disp_status != "None") {
			// refresh value
			if (me.frm) {
				me.value = nxenv.model.get_value(me.doctype, me.docname, me.df.fieldname);
			} else if (me.doc) {
				...
				if (is_fetch_from_read_only) {
					$(me.disp_area).attr(
						...
						__(
							...
							me.df.fetch_from.split(".").map((value) => __(nxenv.unscrub(value)))
						)
					);
				}
			}
	...
	set_disp_area(value) {
		...
		if (["Data", "Long Text", "Small Text", "Text", "Password"].includes(this.df.fieldtype)) {
			value = nxenv.utils.escape_html(value);
		}
		let doc = this.doc || (this.frm && this.frm.doc);
		let display_value = nxenv.format(value, this.df, { no_icon: true, inline: true }, doc);
