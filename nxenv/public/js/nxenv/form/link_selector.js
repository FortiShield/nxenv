...
nxenv.ui.form.LinkSelector = class LinkSelector {
	constructor(opts) {
		...
		if (this.doctype != "[Select]") {
			nxenv.model.with_doctype(this.doctype, function (r) {
				me.make();
			});
		} else {
			this.make();
				...
				if (results.length) {
					...
					$(
						'<p><br><span class="text-muted">' +
							__("No Results") +
							"</span>" +
							(nxenv.model.can_create(me.doctype)
	...
	set_in_grid(value) {
		return new Promise((resolve) => {
			if (this.qty_fieldname) {
						...
						if (!updated) {
							...
							nxenv.run_serially([
								...
								() => {
									...
									return nxenv.model.set_value(d.doctype, d.name, args);
								},
								() => nxenv.show_alert(__("Added {0} ({1})", [value, data.qty])),
								() => resolve(),
							]);
						}
					},
				...
				nxenv.model.set_value(d.doctype, d.name, this.fieldname, value).then(() => {
					nxenv.show_alert(__("{0} {1} added", [this.dynamic_link_reference, value]));
					resolve();
				});
			} else {
				let d = this.target.add_new_row();
				nxenv.model.set_value(d.doctype, d.name, this.fieldname, value).then(() => {
					nxenv.show_alert(__("{0} added", [value]));
					resolve();
				});
			}
		});
...
nxenv.link_search = function (doctype, args, callback, btn) {
	if (!args) {
		args = {
			txt: "",
		};
