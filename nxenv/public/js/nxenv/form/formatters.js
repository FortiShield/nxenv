...
nxenv.provide("nxenv.form.formatters");

nxenv.form.link_formatters = {};

nxenv.form.formatters = {
	...
	_apply_custom_formatter: function (value, df) {
		...
		example:
			nxenv.meta.docfield_map[df.parent][df.fieldname].formatter = (value) => {
		...
		*/
		...
		if (df) {
			const std_df =
				nxenv.meta.docfield_map[df.parent] &&
				nxenv.meta.docfield_map[df.parent][df.fieldname];
	...
	Data: function (value, df) {
		...
		return nxenv.form.formatters._apply_custom_formatter(value, df);
	},
	Autocomplete: function (value, df) {
		return __(nxenv.form.formatters["Data"](value, df));
	},
	...
	Float: function (value, docfield, options, doc) {
		...
		var precision =
			docfield.precision ||
			cint(nxenv.boot.sysdefaults && nxenv.boot.sysdefaults.float_precision) ||
			null;
		if (docfield.options && docfield.options.trim()) {
			...
			return nxenv.form.formatters.Currency(value, docfield, options, doc);
		} else {
			// show 1.000000 as 1
			...
			return nxenv.form.formatters._right(format_number(value, null, precision), options);
		}
	},
	Int: function (value, docfield, options) {
		...
		if (cstr(docfield.options).trim() === "File Size") {
			return nxenv.form.formatters.FileSize(value);
		}
		return nxenv.form.formatters._right(value == null ? "" : cint(value), options);
	},
	Percent: function (value, docfield, options) {
		...
		const precision =
			...
			cint(nxenv.boot.sysdefaults && nxenv.boot.sysdefaults.float_precision) ||
			2;
		return nxenv.form.formatters._right(flt(value, precision) + "%", options);
	},
	...
	Currency: function (value, docfield, options, doc) {
		...
		var currency = nxenv.meta.get_field_currency(docfield, doc);

		let precision;
		if (typeof docfield.precision == "number") {
			precision = docfield.precision;
		} else {
			precision = cint(docfield.precision || nxenv.boot.sysdefaults.currency_precision || 2);
		}
		...
		if (precision > 2) {
			...
			if (decimals.length < 3 || decimals.length < precision) {
				const fraction =
					nxenv.model.get_value(":Currency", currency, "fraction_units") || 100; // if not set, minimum 2.
		...
		if (options && options.only_value) {
			...
			return nxenv.form.formatters._right(value, options);
		}
	},
	...
	Link: function (value, docfield, options, doc) {
		...
		let link_title = nxenv.utils.get_link_title(doctype, value);
		...
		if (nxenv.form.link_formatters[doctype]) {
			// don't apply formatters in case of composite (parent field of same type)
			if (doc && doctype !== doc.doctype) {
				value = nxenv.form.link_formatters[doctype](value, doc, docfield);
			}
		}
		...
		if (docfield && docfield.link_onclick) {
			...
			if (nxenv.model.can_read(doctype)) {
				const a = document.createElement("a");
				a.href = `/app/${encodeURIComponent(
					nxenv.router.slug(doctype)
	...
	Date: function (value) {
		if (!nxenv.datetime.str_to_user) {
			return value;
		}
		if (value) {
			value = nxenv.datetime.str_to_user(value, false, true);
			// handle invalid date
	...
	DateRange: function (value) {
		if (Array.isArray(value)) {
			return __("{0} to {1}", [
				nxenv.datetime.str_to_user(value[0]),
				nxenv.datetime.str_to_user(value[1]),
			]);
		} else {
			return value || "";
		}
	...
	Datetime: function (value) {
		if (value) {
			return moment(nxenv.datetime.convert_to_user_tz(value)).format(
				nxenv.boot.sysdefaults.date_format.toUpperCase() +
					" " +
					(nxenv.boot.sysdefaults.time_format || "HH:mm:ss")
			);
		} else {
			return "";
		}
	...
	Text: function (value, df) {
		if (value) {
			...
			if (!match) {
				value = nxenv.utils.replace_newlines(value);
			}
		}

		return nxenv.form.formatters.Data(value, df);
	},
	Time: function (value) {
		if (value) {
			value = nxenv.datetime.str_to_user(value, true);
		}
	...
	Duration: function (value, docfield) {
		if (value) {
			let duration_options = nxenv.utils.get_duration_options(docfield);
			value = nxenv.utils.get_formatted_duration(value, duration_options);
		}
	...
	SmallText: function (value) {
		return nxenv.form.formatters.Text(value);
	},
	TextEditor: function (value) {
		let formatted_value = nxenv.form.formatters.Text(value);
	...
	WorkflowState: function (value) {
		var workflow_state = nxenv.get_doc("Workflow State", value);
		if (workflow_state) {
			return repl(
	...
	TableMultiSelect: function (rows, df, options) {
		...
		const formatted_values = rows.map((row) => {
			...
			return `<span class="text-nowrap">
				${nxenv.format(value, link_field, options, row)}
			</span>`;
		});
	...
	Icon: (value) => {
		return value
			? `<div>
			<div class="selected-icon">${nxenv.utils.icon(value, "md")}</div>
...
nxenv.form.get_formatter = function (fieldtype) {
	if (!fieldtype) fieldtype = "Data";
	return nxenv.form.formatters[fieldtype.replace(/ /g, "")] || nxenv.form.formatters.Data;
};

nxenv.format = function (value, df, options, doc) {
	...
	var formatter = df.formatter || nxenv.form.get_formatter(fieldtype);

	var formatted = formatter(value, df, options, doc);

	if (typeof formatted == "string") formatted = nxenv.dom.remove_script_and_style(formatted);

	return formatted;
};

nxenv.get_format_helper = function (doc) {
	var helper = {
		get_formatted: function (fieldname) {
			var df = nxenv.meta.get_docfield(doc.doctype, fieldname);
			if (!df) {
				console.log("fieldname not found: " + fieldname);
			}
			return nxenv.format(doc[fieldname], df, { inline: 1 }, doc);
		},
	};
	$.extend(helper, doc);
	return helper;
...
nxenv.form.link_formatters["User"] = function (value, doc, docfield) {
