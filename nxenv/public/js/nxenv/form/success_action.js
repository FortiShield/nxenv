...
nxenv.provide("nxenv.success_action");

nxenv.ui.form.SuccessAction = class SuccessAction {
	constructor(form) {
		this.form = form;
		this.load_setting();
	}
	...
	load_setting() {
		this.setting = nxenv.boot.success_action.find(
	...
	show_alert() {
		nxenv.db.get_list(this.form.doctype, { limit: 2 }).then((result) => {
	...
	get default_actions() {
		return {
			new: {
				label: __("New"),
				action: (frm) => nxenv.new_doc(frm.doctype),
			},
			print: {
				label: __("Print"),
			...
			list: {
				label: __("View All"),
				action: (frm) => {
					nxenv.set_route("List", frm.doctype);
				},
			},
		};
	}
