...
nxenv.ui.form.Sidebar = class {
	constructor(opts) {
		$.extend(this, opts);
	}

	make() {
		var sidebar_content = nxenv.render_template("form_sidebar", {
			doctype: this.frm.doctype,
			frm: this.frm,
			can_write: nxenv.model.can_write(this.frm.doctype, this.frm.docname),
		});
		...
		nxenv.ui.form.setup_user_image_event(this.frm);
	...
	setup_keyboard_shortcuts() {
		...
		nxenv.ui.keys.get_shortcut_group(this.page).add(assignment_link);
	}
	...
	refresh() {
		if (this.frm.doc.__islocal) {
			...
			nxenv.ui.form.set_user_image(this.frm);
		}
	}
	...
	refresh_web_view_count() {
		if (this.frm.doc.route && cint(nxenv.boot.website_tracking_enabled)) {
			...
			nxenv.utils.get_page_view_count(route).then((res) => {
				this.sidebar
					.find(".pageview-count")
					.removeClass("hidden")
	...
	refresh_creation_modified() {
		...
		let avatar_group = nxenv.avatar_group(user_list, 5, {
			align: "left",
			overlap: true,
		});
			...
			nxenv.call({
				...
				callback: function (res) {
					...
					el.on("click", function () {
						nxenv.set_route("Form", "Auto Repeat", me.frm.doc.auto_repeat);
					});
				},
			});
		}
	...
	show_webhook_request_log_status() {
		...
		if (docinfo.webhook_request_log_exists) {
			...
			el.show();
			el.on("click", () => {
				nxenv.set_route("List", "Webhook Request Log", {
					reference_doctype: this.frm.doc.doctype,
	...
	make_tags() {
		...
		this.frm.tags = new nxenv.ui.TagEditor({
			parent: tags_parent,
	...
	make_attachments() {
		var me = this;
		this.frm.attachments = new nxenv.ui.form.Attachments({
	...
	make_assignments() {
		this.frm.assign_to = new nxenv.ui.form.AssignTo({
	...
	make_shared() {
		this.frm.shared = new nxenv.ui.form.Share({
			frm: this.frm,
	...
	make_review() {
		...
		if (nxenv.boot.energy_points_enabled && !this.frm.is_new()) {
			this.frm.reviews = new nxenv.ui.form.Review({
	...
	reload_docinfo(callback) {
		nxenv.call({
			method: "nxenv.desk.form.load.get_docinfo",
			args: {
				doctype: this.frm.doctype,
