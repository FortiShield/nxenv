...
nxenv.ui.form.Review = class Review {
	constructor({ parent, frm }) {
		this.parent = parent;
		this.frm = frm;
		this.points = nxenv.boot.points;
	...
	update_points() {
		return nxenv
			.xcall("nxenv.social.doctype.energy_point_log.energy_point_log.get_energy_points", {
				user: nxenv.session.user,
			})
			.then((data) => {
				nxenv.boot.points = data;
				this.points = data;
			});
	}
	...
	show_review_dialog() {
		...
		const review_dialog = new nxenv.ui.Dialog({
			title: __("Add Review"),
			fields: [
				{
					fieldname: "to_user",
			...
			primary_action: (values) => {
				...
				if (values.points > this.points.review_points) {
					return nxenv.msgprint(__("You do not have enough points"));
				}
				nxenv
					.xcall("nxenv.social.doctype.energy_point_log.energy_point_log.review", {
						doc: {
	...
	setup_detail_popover(el, data) {
		let subject = "";
		let fullname = nxenv.user.full_name(data.user);
		let timestamp = `<span class="text-muted">${nxenv.datetime.comment_when(
			data.creation
		)}</span>`;
			...
			content: () => {
				return `
					<div class="text-medium">
						...
						<div class="subject">
							${nxenv.utils.icon("review")}
							${subject}

							<p class="mt-1">
								<!-- ${nxenv.avatar("shivam@example.com", "avatar-xs")} -->
								<span>${nxenv.user.full_name(data.owner)}</span> - ${timestamp}
							</p>
						</div>
					</div>
				`;
