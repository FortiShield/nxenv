...
nxenv.ui.form.AssignTo = class AssignTo {
	constructor(opts) {
		$.extend(this, opts);
	...
	render(assignments) {
		...
		let avatar_group = nxenv.avatar_group(assigned_users, 5, {
			align: "left",
			overlap: true,
		});
		...
		avatar_group.click(() => {
			new nxenv.ui.form.AssignmentDialog({
	...
	add() {
		...
		if (this.frm.is_new()) {
			nxenv.throw(__("Please save the document before assignment"));
			return;
		}

		if (!me.assign_to) {
			me.assign_to = new nxenv.ui.form.AssignToDialog({
	...
	remove(owner) {
		if (this.frm.is_new()) {
			nxenv.throw(__("Please save the document before removing assignment"));
			return;
		}

		return nxenv
			.xcall("nxenv.desk.form.assign_to.remove", {
...
nxenv.ui.form.AssignToDialog = class AssignToDialog {
	constructor(opts) {
		$.extend(this, opts);
	...
	assign_to_me() {
		...
		if (me.dialog.get_value("assign_to_me")) {
			assign_to.push(nxenv.session.user);
		}
		...
		return [
			...
			},
			{
				...
				get_data: function (txt) {
					return nxenv.db.get_link_options("User", txt, {
...
nxenv.ui.form.AssignmentDialog = class {
	constructor(opts) {
		this.frm = opts.frm;
	...
	make() {
		...
		this.assignment_list.removeClass("nxenv-control");
	...
	add_assignment(assignment) {
		return nxenv
			.xcall("nxenv.desk.form.assign_to.add", {
				doctype: this.frm.doctype,
				name: this.frm.docname,
	...
	remove_assignment(assignment) {
		return nxenv.xcall("nxenv.desk.form.assign_to.remove", {
			doctype: this.frm.doctype,
			name: this.frm.docname,
			assign_to: assignment,
		});
	...
	close_assignment(assignment) {
		return nxenv.xcall("nxenv.desk.form.assign_to.close", {
			doctype: this.frm.doctype,
			name: this.frm.docname,
			assign_to: assignment,
		});
	...
	get_assignment_row(assignment) {
		const row = $(`
			<div class="dialog-assignment-row" data-user="${assignment}">
				<div class="assignee">
					${nxenv.avatar(assignment)}
					${nxenv.user.full_name(assignment)}
				</div>
		...
		if (assignment === nxenv.session.user || this.frm.perm[0].write) {
			btn_group.append(`
				<button type="button" class="btn btn-default remove-btn" title="${__("Cancel")}">
				${nxenv.utils.icon("close")}
				</button>
			`);
