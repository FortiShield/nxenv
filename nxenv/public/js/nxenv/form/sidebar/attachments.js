...
nxenv.ui.form.Attachments = class Attachments {
	constructor(opts) {
		$.extend(this, opts);
	...
	add_attachment(attachment) {
		...
		let file_label = `
			<a href="${file_url}" target="_blank" title="${nxenv.utils.escape_html(file_name)}"
		...
		if (nxenv.model.can_write(this.frm.doctype, this.frm.name)) {
			remove_action = function (target_id) {
				nxenv.confirm(__("Are you sure you want to delete the attachment?"), function () {
		...
		const icon = `<a href="/app/file/${fileid}" class="attachment-icon">
				${nxenv.utils.icon(attachment.is_private ? "es-line-lock" : "es-line-unlock", "sm ml-0")}
			</a>`;

		$(`<div class="attachment-row"></div>`)
			.append(nxenv.get_data_pill(file_label, fileid, remove_action, icon))
	...
	remove_attachment(fileid, callback) {
		...
		var me = this;
		return nxenv.call({
			method: "nxenv.desk.form.utils.remove_attach",
			type: "DELETE",
			args: {
				fid: fileid,
				dt: me.frm.doctype,
			...
			callback: function (r, rt) {
				if (r.exc) {
					if (!r._server_messages) nxenv.msgprint(__("There were errors"));
					return;
				}
