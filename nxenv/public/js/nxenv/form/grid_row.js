...
export default class GridRow {
	...
	set_docfields(update = false) {
		if (this.doc && this.parent_df.options) {
			nxenv.meta.make_docfield_copy_for(
				this.parent_df.options,
				this.doc.name,
				this.docfields
			);
			const docfields = nxenv.meta.get_docfields(this.parent_df.options, this.doc.name);
			if (update) {
			...
			if (this.frm) {
				nxenv
					.run_serially([
						...
						() => {
							nxenv.model.clear_doc(this.doc.doctype, this.doc.name);
		...
		nxenv.prompt(
			...
			function (values) {
				if (me.doc._sortable === false) {
					nxenv.msgprint(__("Cannot move row"));
					return;
				}
	...
	render_template() {
		...
		this.row_display = $('<div class="row-data sortable-handle template-row"></div>')
			.appendTo(this.row)
			.html(
				nxenv.render(this.grid.template, {
					doc: this.doc ? nxenv.get_format_helper(this.doc) : null,
					frm: this.frm,
					row: this,
				})
			);
	...
	render_row(refresh) {
		...
		if (!this.row_index && !this.show_search) {
			...
			nxenv.utils.only_allow_num_decimal(this.row_index.find("input"));
		}
		...
		if (this.configure_columns && this.frm) {
			this.configure_columns_button = $(`
				<div class="col grid-static-col pointer">
					<a>${nxenv.utils.icon("setting-gear", "sm", "", "filter: opacity(0.5)")}</a>
				</div>
			`)
				.appendTo(this.row)
				.on("click", () => {
	...
	configure_dialog_for_columns_selector() {
		this.grid_settings_dialog = new nxenv.ui.Dialog({
		...
		d.set_primary_action(__("Add"), () => {
			...
			if (selected_fields) {
				selected_fields.forEach((selected_column) => {
					let docfield = nxenv.meta.get_docfield(this.grid.doctype, selected_column);
	...
	prepare_columns_for_dialog(selected_fields) {
		...
		const blocked_fields = nxenv.model.no_value_type;
	...
	render_selected_columns() {
		...
		if (this.selected_columns_for_grid) {
			this.selected_columns_for_grid.forEach((d) => {
				let docfield = nxenv.meta.get_docfield(this.grid.doctype, d.fieldname);
					...
					<div class='control-input flex align-center form-control fields_order sortable-handle sortable'
						...
						<div class='row'>
							<div class='col-1' style='padding-top: 4px;'>
								<a style='cursor: grabbing;'>${nxenv.utils.icon("drag", "xs")}</a>
							</div>
		...
		$(this.fields_html_wrapper)
			...
			.change((event) => {
				if (cint(event.target.value) === 0) {
					...
					nxenv.throw(__("Column width cannot be zero."));
				}
		...
		$(this.fields_html_wrapper)
			...
			.click((event) => {
				...
				if (selected_columns_for_grid && selected_columns_for_grid.length === 0) {
					nxenv.throw(__("At least one column is required to show in the grid."));
				}
	...
	update_user_settings_for_grid() {
		...
		nxenv.model.user_settings.save(this.frm.doctype, "GridView", value).then((r) => {
			nxenv.model.user_settings[this.frm.doctype] = r.message || r;
			this.grid.reset_grid();
		});
	}

	reset_user_settings_for_grid() {
		nxenv.model.user_settings.save(this.frm.doctype, "GridView", null).then((r) => {
			nxenv.model.user_settings[this.frm.doctype] = r.message || r;
			this.grid.reset_grid();
		});
	}
	...
	setup_columns() {
		...
		this.grid.visible_columns.forEach((col, ci) => {
			...
			let txt = this.doc
				? nxenv.format(this.doc[df.fieldname], df, null, this.doc)
	...
	make_search_column(df, colsize) {
		...
		["Currency", "Float", "Int", "Percent", "Rating"].includes(df.fieldtype) &&
			nxenv.utils.only_allow_num_decimal($search_input);
	...
	make_column(df, colsize, txt, ci) {
		...
		var $col = $(
			...
			.on("click", function (event) {
				if (nxenv.ui.form.editable_row !== me) {
	...
	toggle_editable_row(show) {
		...
		if (
			...
			if (nxenv.ui.form.editable_row && nxenv.ui.form.editable_row !== this) {
			...
			nxenv.ui.form.editable_row = this;
			return false;
		} else {
			...
			nxenv.ui.form.editable_row = null;
		}
	}
	...
	make_control(column) {
		...
		var field = nxenv.ui.form.make_control({
			df: df,
			parent: parent,
			only_input: true,
			with_link_btn: true,
	...
	set_arrow_keys(field) {
		...
		if (field.$input) {
			field.$input.on("keydown", function (e) {
				var { ESCAPE, TAB, UP: UP_ARROW, DOWN: DOWN_ARROW } = nxenv.ui.keyCode;
	...
	show_form() {
		...
		nxenv.dom.freeze("", "dark grid-form");
		if (cur_frm) cur_frm.cur_grid = this;
		this.wrapper.addClass("grid-row-open");
		if (
			!nxenv.dom.is_element_in_viewport(this.wrapper) &&
			!nxenv.dom.is_element_in_modal(this.wrapper)
		) {
			// -15 offset to make form look visually centered
			nxenv.utils.scroll_to(this.wrapper, true, -15);
		}
	...
	hide_form() {
		...
		if (!nxenv.dom.is_element_in_modal(this.row)) {
			nxenv.utils.scroll_to(this.row, true, 15);
		}
		this.refresh();
	...
	refresh_field(fieldname, txt) {
		...
		if (df && this.doc) {
			txt = nxenv.format(this.doc[fieldname], df, null, this.doc);
		}

		if (!txt && this.frm) {
			txt = nxenv.format(this.doc[fieldname], df, null, this.frm.doc);
		}
