...
nxenv.ui.form.Footer = class FormFooter {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	...
	make() {
		this.wrapper = $(nxenv.render_template("form_footer", {})).appendTo(this.parent);
		this.wrapper.find(".btn-save").click(() => {
			this.frm.save("Save", null, this);
		});
	}
	make_comment_box() {
		this.frm.comment_box = nxenv.ui.form.make_control({
			...
			on_submit: (comment) => {
				if (strip_html(comment).trim() != "" || comment.includes("img")) {
					...
					nxenv
						.xcall("nxenv.desk.form.utils.add_comment", {
							reference_doctype: this.frm.doctype,
							reference_name: this.frm.docname,
							content: comment,
							comment_email: nxenv.session.user,
							comment_by: nxenv.session.user_fullname,
						})
						.then((comment) => {
							let comment_item =
	...
	make_like() {
		...
		nxenv.ui.setup_like_popover(this.wrapper.find(".form-stats-likes"), ".like-icon");

		this.like_icon.on("click", () => {
			nxenv.ui.toggle_like(this.like_wrapper, this.frm.doctype, this.frm.doc.name, () => {
				this.refresh_like();
			});
		});
	}
	...
	refresh_like() {
		...
		const liked = nxenv.ui.is_liked(this.frm.doc);
		this.like_wrapper
