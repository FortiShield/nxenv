...
class FormTimeline extends BaseTimeline {
	...
	setup_timeline_actions() {
		if (nxenv.model.can_email(null, this.frm)) {
		...
		if (this.frm.meta.allow_events_in_timeline) {
			let create_event = () => {
				const args = {
					...
					txt: nxenv.markdown(this.frm.comment_box.get_value()),
				};
				return new nxenv.views.InteractionComposer(args);
			};
	...
	setup_document_email_link() {
		...
		if (doc_info.document_email) {
			...
			this.document_email_link_wrapper.find(".document-email-link").on("click", (e) => {
				...
				nxenv.utils.copy_to_clipboard(text);
			});
		}
	}
	...
	render_timeline_items() {
		...
		nxenv.utils.bind_actions_with_object(this.timeline_items_wrapper, this);
	}

	add_web_page_view_count() {
		if (this.frm.doc.route && cint(nxenv.boot.website_tracking_enabled)) {
			nxenv.utils.get_page_view_count(this.frm.doc.route).then((res) => {
	...
	async get_more_communication_timeline_contents() {
		...
		let response = await nxenv.call({
			method: "nxenv.desk.form.load.get_communications",
			args: {
	...
	get_communication_timeline_content(doc, allow_reply = true) {
		doc._url = nxenv.utils.get_form_link("Communication", doc.name);
		...
		doc.content = nxenv.dom.remove_script_and_style(doc.content);
		let communication_content = $(nxenv.render_template("timeline_message_box", { doc }));
		if (allow_reply) {
	...
	get_comment_timeline_content(doc) {
		doc.content = nxenv.dom.remove_script_and_style(doc.content);
		const comment_content = $(nxenv.render_template("timeline_message_box", { doc }));
	...
	get_milestone_timeline_contents() {
		...
		(this.doc_info.milestones || []).forEach((milestone_log) => {
			const field = nxenv.meta.get_label(this.frm.doctype, milestone_log.track_field);
	...
	get_custom_timeline_contents() {
		...
		(this.doc_info.additional_timeline_content || []).forEach((custom_item) => {
			custom_timeline_contents.push({
				...
				content:
					custom_item.content ||
					nxenv.render_template(custom_item.template, custom_item.template_data),
			});
		});
	...
	get_energy_point_timeline_contents() {
		...
		(this.doc_info.energy_point_logs || []).forEach((log) => {
			...
			energy_point_timeline_contents.push({
				...
				content: nxenv.energy_points.format_form_log(log),
			});
		});
	...
	setup_reply(communication_box, communication_doc) {
		...
		let reply = $(`<a class="action-btn reply">${nxenv.utils.icon("reply", "md")}</a>`).click(
			() => {
				this.compose_mail(communication_doc);
			}
		);
		let reply_all = $(
			`<a class="action-btn reply-all">${nxenv.utils.icon("reply-all", "md")}</a>`
		).click(() => {
	...
	compose_mail(communication_doc = null, reply_all = false) {
		const args = {
			...
			frm: this.frm,
			recipients:
				communication_doc && communication_doc.sender != nxenv.session.user_email
		...
		const email_accounts = nxenv.boot.email_accounts
			.filter((account) => {
				return (
		...
		if (communication_doc && args.is_a_reply) {
			args.cc = "";
			if (
				email_accounts.includes(nxenv.session.user_email) &&
				communication_doc.sender != nxenv.session.user_email
			) {
				...
				args.cc =
					recipients.filter((r) => r != nxenv.session.user_email).join(", ") + ", ";
			}
			if (reply_all) {
		...
		if (this.frm.doctype === "Communication") {
			...
			const comment_value = nxenv.markdown(this.frm.comment_box.get_value());
			args.message = strip_html(comment_value) ? comment_value : "";
		}

		new nxenv.views.CommunicationComposer(args);
	}
	...
	setup_comment_actions(comment_wrapper, doc) {
		...
		if (
			nxenv.model.can_delete("Comment") &&
			(nxenv.session.user == doc.owner || nxenv.user.has_role("System Manager"))
		) {
			const delete_option = $(`
				<li>
		...
		let edit_button = $();
		let current_user = nxenv.session.user;
	...
	make_editable(container) {
		return nxenv.ui.form.make_control({
			parent: container,
			df: {
				fieldtype: "Comment",
				fieldname: "comment",
	...
	update_comment(name, content) {
		return nxenv.xcall("nxenv.desk.form.utils.update_comment", { name, content }).then(() => {
	...
	delete_comment(comment_name) {
		nxenv.confirm(__("Delete comment?"), () => {
			return nxenv
				.xcall("nxenv.client.delete", {
					doctype: "Comment",
					name: comment_name,
				})
				.then(() => {
	...
	copy_link(ev) {
		let doc_link = nxenv.urllib.get_full_url(
			nxenv.utils.get_form_link(this.frm.doctype, this.frm.docname)
		);
		let element_id = $(ev.currentTarget).closest(".timeline-content").attr("id");
		nxenv.utils.copy_to_clipboard(`${doc_link}#${element_id}`);
	}
}
