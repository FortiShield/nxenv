...
nxenv.quick_edit = function (doctype, name) {
	if (!name) name = doctype; // single
	nxenv.db.get_doc(doctype, name).then((doc) => {
		nxenv.ui.form.make_quick_entry(doctype, null, null, doc);
	});
};

nxenv.ui.form.make_quick_entry = (doctype, after_insert, init_callback, doc, force) => {
	var trimmed_doctype = doctype.replace(/ /g, "");
	var controller_name = "QuickEntryForm";

	if (nxenv.ui.form[trimmed_doctype + "QuickEntryForm"]) {
		controller_name = trimmed_doctype + "QuickEntryForm";
	}

	nxenv.quick_entry = new nxenv.ui.form[controller_name](
		doctype,
		after_insert,
		init_callback,
		doc,
	...
	return nxenv.quick_entry.setup();
};

nxenv.ui.form.QuickEntryForm = class QuickEntryForm extends nxenv.ui.Dialog {
		...
		return new Promise((resolve) => {
			nxenv.model.with_doctype(this.doctype, () => {
				...
				if (this.is_quick_entry() || this.force) {
					...
					nxenv.quick_entry = null;
					nxenv.set_route("Form", this.doctype, this.doc.name).then(() => resolve(this));
	...
	check_quick_entry_doc() {
		if (!this.doc) {
			this.doc = nxenv.model.get_new_doc(this.doctype, null, null, true);
		}
	}
	...
	render_dialog() {
		...
		this.onhide = () => (nxenv.quick_entry = null);
		this.show();
			...
			if (data) {
				...
				me.script_manager.trigger("validate").then(() => {
					me.insert().then(() => {
						...
						me.dialog.hide();
						setTimeout(function () {
							nxenv.show_alert({ message: messagetxt, indicator: "green" }, 3);
						}, 500);
					});
				});
			}
	...
	insert() {
		...
		return new Promise((resolve) => {
			me.update_doc();
			nxenv.call({
				method: "nxenv.client.save",
				args: {
					doc: me.dialog.doc,
				},
				callback: function (r) {
					if (
						nxenv.model.is_submittable(me.doctype) &&
						!nxenv.model.has_workflow(me.doctype)
					) {
						nxenv.run_serially([
							() => (me.dialog.working = true),
							() => {
	...
	process_after_insert(r) {
		...
		} else if (nxenv._from_link) {
			nxenv.ui.form.update_calling_link(this.doc);
	...
	open_form_if_not_list() {
		...
		if (route && !(route[0] === "List" && route[1] === doc.doctype)) {
			nxenv.run_serially([() => nxenv.set_route("Form", doc.doctype, doc.name)]);
		}
	}
	...
	open_doc(set_hooks) {
		...
		if (set_hooks && this.after_insert) {
			nxenv.route_options = nxenv.route_options || {};
			nxenv.route_options.after_save = (frm) => {
				this.after_insert(frm);
			};
		}
		this.doc.__run_link_triggers = false;
		nxenv.set_route("Form", this.doctype, this.doc.name);
	}
