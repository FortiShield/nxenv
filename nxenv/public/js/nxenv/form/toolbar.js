...
nxenv.ui.form.Toolbar = class Toolbar {
	constructor(opts) {
		$.extend(this, opts);
		this.refresh();
	...
	set_title() {
		...
		if (this.frm.is_new()) {
			...
			if (
				...
				this.page.$sub_title_area.on("click", (event) => {
					...
					nxenv.utils.copy_to_clipboard(this.frm.docname);
				});
			}
		} else {
			title = this.frm.docname;
		...
		if (this.frm.meta.title_field) {
			nxenv.utils.set_title(title + " - " + this.frm.docname);
		}
	...
	rename_document_title(input_name, input_title, merge = false) {
		...
		let rename_document = () => {
			if (input_name != docname) nxenv.realtime.doctype_subscribe(doctype, input_name);
			return nxenv
				.xcall("nxenv.model.rename_doc.update_document_title", {
					doctype,
					docname,
					name: input_name,
					title: input_title,
	...
	set_indicator() {
		var indicator = nxenv.get_indicator(this.frm.doc);
		if (
	...
	add_discard() {
		if (
			nxenv.model.is_submittable(this.frm.doc.doctype) &&
	...
	add_print() {
		...
		if (
			...
			if (nxenv.model.can_print(null, this.frm) && !this.frm.meta.issingle) {
				this.page.add_menu_item(
					__("Print"),
					() => {
						this.frm.print_doc();
	...
	add_email() {
		// email
		if (nxenv.model.can_email(null, this.frm) && this.frm.doc.docstatus < 2) {
			this.page.add_menu_item(
				__("Email"),
				() => {
					this.frm.email_doc();
	...
	add_duplicate() {
		if (nxenv.boot.user.can_create.includes(this.frm.doctype) && !this.frm.meta.allow_copy) {
			this.page.add_menu_item(
				__("Duplicate"),
				() => {
					this.frm.copy_doc();
	...
	add_delete() {
		...
		if (
			...
			!this.frm.doc.__islocal &&
			!nxenv.model.is_single(this.frm.doctype) &&
			nxenv.model.can_delete(this.frm.doctype)
		) {
			this.page.add_menu_item(
				__("Delete"),
				() => {
	...
	add_follow() {
		if (this.frm.meta.track_changes && nxenv.boot.user.document_follow_notify) {
	...
	add_copy_to_clipboard() {
		...
		this.page.add_menu_item(
			...
			() => {
				nxenv.utils.copy_to_clipboard(JSON.stringify(this.frm.doc));
			},
			true
		);
	}
	...
	add_auto_repeat() {
		...
		if (this.can_repeat()) {
			this.page.add_menu_item(
				__("Repeat"),
				() => {
					nxenv.utils.new_auto_repeat_prompt(this.frm);
				},
				true
			);
		}
	...
	add_new() {
		...
		if (p[CREATE] && !this.frm.meta.issingle && !this.frm.meta.in_create) {
			this.page.add_menu_item(
				...
				() => {
					nxenv.new_doc(this.frm.doctype, true);
				},
				true,
				{
					shortcut: "Ctrl+B",
	...
	add_audit_trail() {
		if (
			...
			nxenv.model.get_value("DocType", this.frm.doc.doctype, "track_changes")
		) {
			this.page.add_menu_item(
				__("View Audit Trail"),
				() => {
					nxenv.set_route("audit-trail");
				},
				true
			);
		}
	...
	make_customize_buttons() {
		...
		if (nxenv.model.can_create("DocType")) {
			if (nxenv.boot.developer_mode && !is_doctype_form) {
				// edit doctype
				this.page.add_menu_item(
					__("Edit DocType"),
					() => {
						nxenv.set_route("Form", "DocType", this.frm.doctype);
					},
					true
				);
			}
	...
	has_workflow() {
		if (this._has_workflow === undefined)
			this._has_workflow = nxenv.model.has_workflow(this.frm.doctype);
	...
	show_linked_with() {
		if (!this.frm.linked_with) {
			this.frm.linked_with = new nxenv.ui.form.LinkedWith({
				frm: this.frm,
			});
		}
	...
	set_page_actions(status) {
		...
		if (status === "Edit") {
			...
			if (this.has_workflow()) {
				nxenv
					.xcall("nxenv.model.workflow.can_cancel_document", {
	...
	follow() {
		...
		nxenv
			.call("nxenv.desk.form.document_follow.update_follow", {
