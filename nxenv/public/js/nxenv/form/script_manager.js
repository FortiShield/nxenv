...
nxenv.provide("nxenv.ui.form.handlers");
...
nxenv.ui.form.get_event_handler_list = function (doctype, fieldname) {
	if (!nxenv.ui.form.handlers[doctype]) {
		nxenv.ui.form.handlers[doctype] = {};
	}
	if (!nxenv.ui.form.handlers[doctype][fieldname]) {
		nxenv.ui.form.handlers[doctype][fieldname] = [];
	}
	return nxenv.ui.form.handlers[doctype][fieldname];
};

nxenv.ui.form.on = nxenv.ui.form.on_change = function (doctype, fieldname, handler) {
	var add_handler = function (fieldname, handler) {
		var handler_list = nxenv.ui.form.get_event_handler_list(doctype, fieldname);
...
nxenv.ui.form.off = function (doctype, fieldname, handler) {
	var handler_list = nxenv.ui.form.get_event_handler_list(doctype, fieldname);
...
nxenv.ui.form.trigger = function (doctype, fieldname) {
	cur_frm.script_manager.trigger(fieldname, doctype);
};

nxenv.ui.form.ScriptManager = class ScriptManager {
	constructor(opts) {
		$.extend(this, opts);
	}
	...
	trigger(event_name, doctype, name) {
		...
		this.frm.selected_doc = nxenv.get_doc(doctype, name);
	...
	has_handler(event_name) {
		...
		return (
			nxenv.ui.form.handlers[this.frm.doctype] &&
			nxenv.ui.form.handlers[this.frm.doctype][event_name]
		);
	}
	...
	get_handlers(event_name, doctype) {
		...
		if (nxenv.ui.form.handlers[doctype] && nxenv.ui.form.handlers[doctype][event_name]) {
			$.each(nxenv.ui.form.handlers[doctype][event_name], function (i, fn) {
	...
	setup() {
		...
		$.each(this.frm.fields, function (i, field) {
			...
			if (nxenv.model.table_fields.includes(field.df.fieldtype)) {
				$.each(
					nxenv.meta.get_docfields(field.df.options, me.frm.docname),
					function (i, df) {
						setup_add_fetch(df);
					}
				);
		...
		// css
		doctype.__css && nxenv.dom.set_style(doctype.__css);

		this.trigger("setup");
	}

	log_error(caller, e) {
		nxenv.show_alert({ message: __("Error in Client Script."), indicator: "error" });
