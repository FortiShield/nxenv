...
nxenv.provide("nxenv.model.docinfo");
...
nxenv.ui.form.Controller = class FormController {
	constructor(opts) {
		$.extend(this, opts);
	}
};

nxenv.ui.form.Form = class NxenvForm {
	constructor(doctype, parent, in_form, doctype_layout_name) {
		...
		this.sections = [];
		this.grids = [];
		this.cscript = new nxenv.ui.form.Controller({ frm: this });
		this.events = {};
		this.fetch_dict = {};
		this.parent = parent;
		this.doctype_layout = nxenv.get_meta(doctype_layout_name);
		this.undo_manager = new UndoManager({ frm: this });
		this.setup_meta(doctype);
		this.debounced_reload_doc = nxenv.utils.debounce(this.reload_doc.bind(this), 1000);
	...
	setup_meta() {
		this.meta = nxenv.get_meta(this.doctype);

		if (this.meta.istable) {
			this.meta.in_dialog = 1;
		}

		this.perm = nxenv.perm.get_perm(this.doctype); // for create
	...
	setup() {
		this.fields = [];
		this.fields_dict = {};
		this.state_fieldname = nxenv.workflow.get_state_fieldname(this.doctype);
		...
		this.toolbar = new nxenv.ui.form.Toolbar({
			frm: this,
			page: this.page,
		});

		this.viewers = new nxenv.ui.form.FormViewers({
			frm: this,
		...
		this.script_manager = new nxenv.ui.form.ScriptManager({
			frm: this,
		});
		this.script_manager.setup();
		this.watch_model_updates();

		if (!this.meta.hide_toolbar && nxenv.boot.desk_settings.timeline) {
			// this.footer_tab = new nxenv.ui.form.Tab(this.layout, {
			// 	label: __("Activity"),
			// 	fieldname: 'timeline'
			// });

			this.footer = new nxenv.ui.form.Footer({
				frm: this,
	...
	add_form_keyboard_shortcuts() {
		...
		nxenv.ui.keys.add_shortcut({
			...
			condition: () => nxenv.model.can_print(this.doctype, this) && !this.meta.issingle,
		});
	...
	setup_std_layout() {
		...
		// layout
		this.layout = new nxenv.ui.form.Layout({
			parent: this.body,
		...
		this.dashboard = new nxenv.ui.form.Dashboard(dashboard_parent, this);

		this.tour = new nxenv.ui.form.FormTour({
			frm: this,
		});

		// workflow state
		this.states = new nxenv.ui.form.States({
			frm: this,
		});
	}
	...
	watch_model_updates() {
		...
		// on table fields
		var table_fields = nxenv.get_children("DocType", me.doctype, "fields", {
			fieldtype: ["in", nxenv.model.table_fields],
		});
	...
	setup_notify_on_rename() {
		...
		nxenv.realtime.on("doc_rename", (data) => {
			...
			if (data.doctype == this.doctype && data.old == this.docname) {
				...
				nxenv.set_route("Form", this.doctype, data.new);
			}
		});
	}
	...
	setup_file_drop() {
		...
		this.$wrapper.on("dragenter dragover", false).on("drop", function (e) {
			...
			if (me.doc.__islocal) {
				nxenv.msgprint(__("Please save before attaching."));
				throw "attach error";
			}

			new nxenv.ui.FileUploader({
				doctype: me.doctype,
				docname: me.docname,
				frm: me,
		...
		this.fields.map((field) => {
			if (field.df.fieldtype === "Markdown Editor") {
				this.set_df_property(field.df.fieldname, "autocompletions", () => {
					...
					return attachments
						.filter((file) => nxenv.utils.is_image_file(file.file_url))
						.map((file) => {
							return {
	...
	refresh(docname) {
		...
		if (this.docname) {
			...
			// set the doc
			this.doc = nxenv.get_doc(this.doctype, this.docname);

			// check permissions
			this.fetch_permissions();
			if (!this.has_read_permission()) {
				nxenv.show_not_permitted(__(this.doctype) + " " + __(cstr(this.docname)));
				return;
			}
			...
			// read only (workflow)
			this.read_only = nxenv.workflow.is_read_only(this.doctype, this.docname);
			if (this.read_only) {
				this.set_read_only();
				nxenv.show_alert(__("This form is not editable due to a Workflow."));
			}
	...
	setup_doctype_actions() {
		if (this.meta.actions) {
			for (let action of this.meta.actions) {
				nxenv.ui.form.on(this.doctype, "refresh", () => {
					if (!this.is_new()) {
						if (!action.hidden) {
	...
	execute_action(action) {
		if (typeof action === "string") {
			...
			if (typeof action === "string") {
				nxenv.throw(`Action ${action} not found`);
			}
		}
		if (action.action_type === "Server Action") {
			return nxenv.xcall(action.action, { doc: this.doc }).then((doc) => {
				if (doc.doctype) {
					// document is returned by the method,
					// apply the changes locally and refresh
					nxenv.model.sync(doc);
					this.refresh();
				}
			...
			return nxenv.set_route(action.action);
		}
	}
	...
	initialize_new_doc() {
		...
		this.script_manager.trigger("before_load", this.doctype, this.docname).then(() => {
			...
			nxenv.after_ajax(function () {
				me.trigger_link_fields();
			});

			nxenv.breadcrumbs.add(me.meta.module, me.doctype);
		});
	...
	render_form(switched) {
		if (!this.meta.istable) {
			...
			if (nxenv.boot.desk_settings.form_sidebar) {
				this.sidebar = new nxenv.ui.form.Sidebar({
					frm: this,
					page: this.page,
				});
				this.sidebar.make();
	...
	run_after_load_hook() {
		if (nxenv.route_hooks.after_load) {
			let route_callback = nxenv.route_hooks.after_load;
			delete nxenv.route_hooks.after_load;
	...
	refresh_header(switched) {
		...
		if (!this.meta.in_dialog || this.in_form) {
			nxenv.utils.set_title(this.meta.issingle ? this.doctype : this.docname);
		}
		...
		var after_save = function (r) {
			...
			if (!r.exc) {
				if (["Save", "Update", "Amend"].indexOf(save_action) !== -1) {
					nxenv.utils.play_sound("click");
				}

				me.script_manager.trigger("after_save");

				if (nxenv.route_hooks.after_save) {
					let route_callback = nxenv.route_hooks.after_save;
					delete nxenv.route_hooks.after_save;
			...
			nxenv
				.run_serially([
					...
					() => {
						...
						nxenv.ui.form.save(me, save_action, after_save, btn);
					},
				])
				.catch(fail);
		} else {
	...
	discard(btn, callback, on_error) {
		...
		return new Promise((resolve) => {
			nxenv.confirm(__("Discard {0}", [this.docname]), function () {
						...
						me.save(
							...
							function (r) {
								if (r.exc) {
									...
									nxenv.utils.play_sound("submit");
									callback && callback();
									me.script_manager
										.trigger("on_submit")
										.then(() => resolve(me))
										.then(() => {
											if (nxenv.route_hooks.after_submit) {
												let route_callback =
													nxenv.route_hooks.after_submit;
												delete nxenv.route_hooks.after_submit;
												route_callback(me);
											}
										});
								}
	...
	savecancel(btn, callback, on_error) {
		...
		nxenv
			.call({
				method: "nxenv.desk.form.linked_with.get_submitted_linked_docs",
				args: {
	...
	_cancel_all(r, btn, callback, on_error) {
		...
		for (let doctype of doctypes) {
			if (!me.ignore_doctypes_on_cancel_all.includes(doctype)) {
				let docnames = links
					...
					.map((link) => nxenv.utils.get_form_link(link.doctype, link.name, true))
					.join(", ");
		...
		let can_cancel = links.every((link) => nxenv.model.can_cancel(link.doctype));
		if (can_cancel) {
		...
		let d = new nxenv.ui.Dialog(
			{
				...
				fields: [
					{
						fieldtype: "HTML",
						options: `<p class="nxenv-confirm-message">${confirm_message}</p>`,
					},
				],
			},
			() => me.handle_save_fail(btn, on_error)
		...
		if (can_cancel) {
			d.set_primary_action(__("Cancel All"), () => {
				d.hide();
				nxenv.call({
					method: "nxenv.desk.form.linked_with.cancel_all_linked_docs",
					args: {
						docs: links,
						ignore_doctypes_on_cancel_all: me.ignore_doctypes_on_cancel_all || [],
					},
			...
			me.script_manager.trigger("before_cancel").then(() => {
				...
				var after_cancel = function (r) {
					if (r.exc) {
						...
						nxenv.utils.play_sound("cancel");
						me.refresh();
						callback && callback();
						me.script_manager.trigger("after_cancel");
					}
				};
				nxenv.ui.form.save(me, "cancel", after_cancel, btn);
			});
		};
	...
	_discard(btn, on_error, skip_confirm) {
		...
		const discard_doc = () => {
			...
			me.script_manager.trigger("before_discard").then(() => {
				...
				//nxenv.ui.form.discard(me, after_discard, btn);
				nxenv.call({
					freeze: true,
					method: "nxenv.desk.form.save.discard",
					args: {
						doctype: me.doc.doctype,
						name: me.doc.name,
					},
	...
	savetrash() {
		...
		nxenv.model.delete_doc(this.doctype, this.docname, function () {
			window.history.back();
		});
	}
	...
	amend_doc() {
		if (!this.fields_dict["amended_from"]) {
			nxenv.msgprint(__('"amended_from" field must be present to do an amendment.'));
			return;
		}

		nxenv
			.xcall("nxenv.client.is_document_amended", {
			...
			.then((is_amended) => {
				...
				var fn = function (newdoc) {
					...
					if (me.fields_dict && me.fields_dict["amendment_date"])
						newdoc.amendment_date = nxenv.datetime.obj_to_str(new Date());
				};
				this.copy_doc(fn, 1);
				nxenv.utils.play_sound("click");
			});
	...
	validate_form_action(action, resolve) {
		...
		var perms = nxenv.perm.get_perm(this.doc.doctype)[0];
		...
		if (
			(nxenv.workflow.is_read_only(this.doctype, this.docname) &&
	...
	show_submit_message() {
		if (
			...
			!this.is_dirty() &&
			!this.is_new() &&
			!nxenv.model.has_workflow(this.doctype) && // show only if no workflow
	...
	fetch_permissions() {
		...
		this.perm = nxenv.perm.get_perm(dt, this.doc);
	}
	...
	check_doctype_conflict(docname) {
		if (this.doctype == "DocType" && docname == "DocType") {
			nxenv.msgprint(__("Allowing DocType, DocType. Be careful!"));
		} else if (this.doctype == "DocType") {
			if (nxenv.views.formview[docname] || nxenv.pages["List/" + docname]) {
				window.location.reload();
				//	nxenv.msgprint(__("Cannot open {0} when its instance is open", ['DocType']))
				// throw 'doctype open conflict'
			}
		} else {
			if (
				nxenv.views.formview.DocType &&
				nxenv.views.formview.DocType.frm.opendocs[this.doctype]
			) {
				window.location.reload();
				//	nxenv.msgprint(__("Cannot open instance when its {0} is open", ['DocType']))
				// throw 'doctype open conflict'
			}
		}
	}
	...
	rename_notify(dt, old, name) {
		...
		// cleanup
		if (this && this.opendocs[old] && nxenv.meta.docfield_copy[dt]) {
			// delete docfield copy
			nxenv.meta.docfield_copy[dt][name] = nxenv.meta.docfield_copy[dt][old];
			delete nxenv.meta.docfield_copy[dt][old];
		}
		...
		nxenv.re_route[nxenv.router.get_sub_path()] = `${encodeURIComponent(
			nxenv.router.slug(this.doctype)
		)}/${encodeURIComponent(name)}`;
		!nxenv._from_link && nxenv.set_route("Form", this.doctype, name);
	}
	...
	print_doc() {
		...
		nxenv.set_route("print", this.doctype, this.doc.name);
	}

	navigate_records(prev) {
		let filters, sort_field, sort_order;
		let list_view = nxenv.get_list_view(this.doctype);
		if (list_view) {
			filters = list_view.get_filters_for_args();
			sort_field = list_view.sort_by;
			sort_order = list_view.sort_order;
		} else {
			let list_settings = nxenv.get_user_settings(this.doctype)["List"];
			if (list_settings) {
		...
		nxenv.call("nxenv.desk.form.utils.get_next", args).then((r) => {
			if (r.message) {
				nxenv.set_route("Form", this.doctype, r.message);
	...
	rename_doc() {
		nxenv.model.rename_doc(this.doctype, this.docname, () => this.refresh_header());
	}
	...
	email_doc(message) {
		new nxenv.views.CommunicationComposer({
			doc: this.doc,
			frm: this,
	...
	copy_doc(onload, from_amend) {
		...
		var newdoc = nxenv.model.copy_doc(this.doc, from_amend);
	...
	reload_doc() {
		...
		if (!this.doc.__islocal) {
			nxenv.model.remove_from_locals(this.doctype, this.docname);
			return nxenv.model.with_doc(this.doctype, this.docname, () => {
				this.refresh();
			});
		}
	}
	...
	has_perm(ptype) {
		return nxenv.perm.has_perm(this.doctype, 0, ptype, this.doc);
	}
	...
	get_docinfo() {
		return nxenv.model.docinfo[this.doctype][this.docname];
	}
	...
	set_footnote(txt) {
		this.footnote_area = nxenv.utils.set_footnote(this.footnote_area, this.body, txt);
	}
	...
	scroll_to_element() {
		if (nxenv.route_options && nxenv.route_options.scroll_to) {
			var scroll_to = nxenv.route_options.scroll_to;
			delete nxenv.route_options.scroll_to;
			...
			if (selector.length) {
				nxenv.utils.scroll_to(selector);
			}
		} else if (window.location.hash) {
			if ($(window.location.hash).length) {
				nxenv.utils.scroll_to(window.location.hash, true, 200, null, null, true);
			} else {
	...
	show_success_action() {
		...
		const success_action = new nxenv.ui.form.SuccessAction(this);
		success_action.show();
	}
	...
	set_currency_labels(fields_list, currency, parentfield) {
		...
		$.each(fields_list, function (i, fname) {
			var docfield = nxenv.meta.docfield_map[doctype][fname];
			if (docfield) {
	...
	field_map(fnames, fn) {
		...
		for (var i = 0, l = fnames.length; i < l; i++) {
			...
			var field = nxenv.meta.get_docfield(this.doctype, fieldname, this.docname);
			if (field) {
				fn(field);
	...
	get_docfield(fieldname1, fieldname2) {
		if (fieldname2) {
			...
			return nxenv.meta.get_docfield(doctype, fieldname2, this.docname);
		} else {
			// for parent
			return nxenv.meta.get_docfield(this.doctype, fieldname1, this.docname);
		}
	}
	...
	set_df_property(fieldname, property, value, docname, table_field, table_row_name = null) {
		...
		if (!docname || !table_field) {
			...
			const filtered_fields = nxenv.utils.filter_dict(grid.docfields, {
				fieldname: table_field,
			});
			if (filtered_fields.length) {
				df = nxenv.meta.get_docfield(
	...
	get_files() {
		return this.attachments
			? nxenv.utils.sort(this.attachments.get_attachments(), "file_name", "string")
			: [];
	}
	...
	clear_table(fieldname) {
		nxenv.model.clear_table(this.doc, fieldname);
	}

	add_child(fieldname, values) {
		var doc = nxenv.model.add_child(
			this.doc,
			nxenv.meta.get_docfield(this.doctype, fieldname).options,
			fieldname
		);
		if (values) {
	...
	set_value(field, value, if_missing, skip_dirty_trigger = false) {
		...
		var _set = function (f, v) {
			...
			if (fieldobj) {
				if (!if_missing || !nxenv.model.has_value(me.doctype, me.doc.name, f)) {
					if (nxenv.model.table_fields.includes(fieldobj.df.fieldtype) && $.isArray(v)) {
						// set entire child table from specified array as value
						nxenv.model.clear_table(me.doc, fieldobj.df.fieldname);

						const standard_fields = [
							...nxenv.model.std_fields_list,
							...nxenv.model.child_table_field_list,
						];
						v.forEach((d, idx) => {
							let child = nxenv.model.add_child(
								me.doc,
								fieldobj.df.options,
				...
				nxenv.msgprint(__("Field {0} not found.", [f]));
				throw "frm.set_value";
			}
		};
		...
		if (typeof field == "string") {
			...
			return nxenv.run_serially(tasks);
		}
	}
	...
	call(opts, args, callback) {
		...
		if (!opts.doc) {
			if (opts.method.indexOf(".") === -1)
				opts.method = nxenv.model.get_server_module_name(me.doctype) + "." + opts.method;
					...
					if (opts.child) {
						...
						if (opts.child) {
							var std_field_list = ["doctype"]
								.concat(nxenv.model.std_fields_list)
								.concat(nxenv.model.child_table_field_list);
	...
	set_read_only() {
		const docperms = nxenv.perm.get_perm(this.doc.doctype);
	...
	get_formatted(fieldname) {
		return nxenv.format(
			this.doc[fieldname],
			nxenv.meta.get_docfield(this.doctype, fieldname, this.docname),
			{ no_icon: true },
			this.doc
		);
	}
	...
	open_grid_row() {
		return nxenv.ui.form.get_open_grid_form();
	}
	...
	get_selected() {
		...
		var selected = {},
			me = this;
		nxenv.meta.get_table_fields(this.doctype).forEach(function (df) {
	...
	set_indicator_formatter(fieldname, get_color, get_text) {
		...
		var doctype;
		if (nxenv.meta.docfield_map[this.doctype][fieldname]) {
			doctype = this.doctype;
		} else {
			nxenv.meta.get_table_fields(this.doctype).every(function (df) {
				if (nxenv.meta.docfield_map[df.options][fieldname]) {
					doctype = df.options;
					return false;
				} else {
					return true;
		...
		nxenv.meta.docfield_map[doctype][fieldname].formatter = function (
			value,
			df,
			options,
			doc
			...
			if (value) {
				...
				if (get_text) {
					label = get_text(doc);
				} else if (nxenv.form.link_formatters[df.options]) {
					label = nxenv.form.link_formatters[df.options](value, doc, df);
				} else {
					label = value;
				}
				...
				return `
						<a class="indicator ${get_color(doc || {})}"
							href="/app/${nxenv.router.slug(df.options)}/${escaped_name}"
							data-doctype="${df.options}"
							data-name="${nxenv.utils.escape_html(value)}">
							${label}
						</a>
					`;
			} else {
	...
	can_create(doctype) {
		...
		if (!nxenv.model.can_create(doctype)) {
			return false;
		}
	...
	make_new(doctype) {
		...
		if (this.make_methods && this.make_methods[doctype]) {
			...
			nxenv.model.with_doctype(doctype, function () {
				let new_doc = nxenv.model.get_new_doc(doctype, null, null, true);

				// set link fields (if found)
				me.set_link_field(doctype, new_doc);

				nxenv.ui.form.make_quick_entry(doctype, null, null, new_doc);
				// nxenv.set_route('Form', doctype, new_doc.name);
			});
		}
	}
	...
	set_link_field(doctype, new_doc) {
		let me = this;
		nxenv.get_meta(doctype).fields.forEach(function (df) {
	...
	update_in_all_rows(table_fieldname, fieldname, value) {
		...
		nxenv.model
			...
			.filter((child) => !nxenv.model.has_value(child.doctype, child.name, fieldname))
			.forEach((child) =>
				nxenv.model.set_value(child.doctype, child.name, fieldname, value)
			);
	}
	...
	scroll_to_field(fieldname, focus = true) {
		...
		// scroll to input
		nxenv.utils.scroll_to($el, true, 15);
		...
		let control_element = $el.closest(".nxenv-control");
	...
	setup_docinfo_change_listener() {
		...
		if (this.doc && !this.is_new()) {
			nxenv.realtime.doc_subscribe(doctype, docname);
		}
		nxenv.realtime.off("docinfo_update");
		nxenv.realtime.on("docinfo_update", ({ doc, key, action = "update" }) => {
			if (
			...
			let doc_list = nxenv.model.docinfo[doctype][docname][key] || [];
			let docindex = doc_list.findIndex((old_doc) => {
				return old_doc.name === doc.name;
			});

			if (action === "add") {
				nxenv.model.docinfo[doctype][docname][key].push(doc);
			}
			if (docindex > -1) {
				if (action === "update") {
					nxenv.model.docinfo[doctype][docname][key].splice(docindex, 1, doc);
				}
				if (action === "delete") {
					nxenv.model.docinfo[doctype][docname][key].splice(docindex, 1);
				}
			}
	...
	set_fields_as_options(
		...
		return new Promise((resolve) => {
			nxenv.model.with_doctype(reference_doctype, () => {
				nxenv.get_meta(reference_doctype).fields.map((df) => {
					filter_function(df) &&
	...
	get_involved_users() {
		...
		return involved_users
			.uniqBy((u) => u)
			.filter((user) => !["Administrator", nxenv.session.user].includes(user))
			.filter(Boolean);
	}
	...
	show_submission_queue_banner() {
		...
		nxenv
			.call({
				method: "nxenv.core.doctype.submission_queue.submission_queue.get_latest_submissions",
