nxenv.ui.form.FormTour = class FormTour {
	constructor({ frm }) {
		this.frm = frm;
		this.driver_steps = [];
	}
	...
	init_driver() {
		...
		nxenv.router.on("change", () => this.driver.reset());
	...
	async init({ tour_name, on_finish }) {
		if (tour_name) {
			...
			const doctype_tour_exists = await nxenv.db.exists("Form Tour", this.frm.doctype);
			if (doctype_tour_exists) {
				this.tour = await nxenv.db.get_doc("Form Tour", this.frm.doctype);
			} else {
				this.tour = { steps: nxenv.tour[this.frm.doctype] };
			}
		}
	...
	get_step(step_info, on_next, on_prev) {
		...
		let element = `.nxenv-control[data-fieldname='${fieldname}']`;
		...
		if (is_table_field) {
			...
			element = `.grid-row-open .nxenv-control[data-fieldname='${fieldname}']`;
		}
	...
	add_new_row_step(step) {
		const $add_row = `.nxenv-control[data-fieldname='${step.fieldname}'] .grid-add-row`;
		const add_row_step = {
			element: $add_row,
	...
	expand_row_and_proceed(step, start_from) {
		...
		nxenv.utils.sleep(300).then(() => this.driver.start(start_from));
	}
	...
	open_first_row_of(fieldname) {
		...
		$($close_row).one("click", () => {
			...
			nxenv.utils.scroll_to(next_element, true, 150, null, () => {
				this.driver.moveNext();
				nxenv.flags.disable_auto_scroll = false;
			});
			nxenv.flags.disable_auto_scroll = true;
		});
	}
	...
	add_step_to_save() {
		...
		nxenv.ui.form.on(this.frm.doctype, "after_save", () => this.on_finish && this.on_finish());
	}
