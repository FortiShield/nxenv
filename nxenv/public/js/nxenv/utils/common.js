...
nxenv.avatar = function (
	...
	if (user) {
		// desk
		user_info = nxenv.user_info(user);
	} else {
		// website
		let full_name = title || nxenv.get_cookie("full_name");
		user_info = {
			image: image_url === null ? nxenv.get_cookie("user_image") : image_url,
			fullname: full_name,
			abbr: nxenv.get_abbr(full_name),
			color: nxenv.get_palette(full_name),
		};
	}
...
nxenv.get_avatar = function (css_class, title, image_url = null, remove_color, data_attributes) {
	if (!css_class) {
	...
	if (image_url) {
		...
		let abbr = nxenv.get_abbr(title);
		let style = "";
		if (!remove_color) {
			let color = nxenv.get_palette(title);
...
nxenv.avatar_group = function (users, limit = 4, options = {}) {
	...
	let html = display_users
		.map((user) =>
			nxenv.avatar(user, "avatar-small " + css_class, null, null, false, options.filterable)
		)
		.join("");
		...
		html = `
			...
			<span class="avatar avatar-small ${css_class}">
				<div class="avatar-frame standard-image avatar-extra-count"
					title="${extra_users.map((u) => nxenv.user_info(u).fullname).join(", ")}">
					+${extra_users.length}
				</div>
			</span>
		`;
		...
		avatar_action_html = `
			<span class="avatar avatar-small">
				<div class="avatar-frame avatar-action">
					${nxenv.utils.icon(options.action_icon, "sm")}
				</div>
			</span>
		`;
	}
...
nxenv.ui.scroll = function (element, animate, additional_offset) {
...
nxenv.get_palette = function (txt) {
	if (!txt) return nxenv.palette[8]; // breaks when undefined
	var idx = cint((parseInt(md5(txt).substr(4, 2), 16) + 1) / 5.33);
	return nxenv.palette[idx % 8];
};

nxenv.get_abbr = function (txt, max_length) {
	if (!txt) return "";
	var abbr = "";
...
nxenv.gravatars = {};
nxenv.get_gravatar = function (email_id, size = 0) {
	var param = size ? "s=" + size : "d=retro";
	if (!nxenv.gravatars[email_id]) {
		// TODO: check if gravatar exists
		nxenv.gravatars[email_id] =
			"https://secure.gravatar.com/avatar/" + md5(email_id) + "?" + param;
	}
	return nxenv.gravatars[email_id];
};
...
nxenv.get_cookie = function getCookie(name) {
	return nxenv.get_cookies()[name];
};

nxenv.get_cookies = function getCookies() {
...
nxenv.is_mobile = function () {
	return $(document).width() < 768;
};

nxenv.is_large_screen = function () {
	return $(document).height() > 1180;
};

nxenv.utils.xss_sanitise = function (string, options) {
...
nxenv.utils.sanitise_redirect = (url) => {
	const is_external = (() => {
		return (url) => {
			function domain(url) {
	...
	url = nxenv.utils.strip_url(url);

	return is_external(url)
		? ""
		: sanitise_javascript(nxenv.utils.xss_sanitise(url, { strategies: ["js"] }));
};

nxenv.utils.strip_url = (url) => {
...
nxenv.utils.new_auto_repeat_prompt = function (frm) {
	const fields = [
		{
			fieldname: "frequency",
			fieldtype: "Select",
		...
		},
		{
			...
			label: __("Start Date"),
			reqd: 1,
			default: nxenv.datetime.nowdate(),
		},
		{
			fieldname: "end_date",
			fieldtype: "Date",
	...
	nxenv.prompt(
		...
		function (values) {
			nxenv.call({
				method: "nxenv.automation.doctype.auto_repeat.auto_repeat.make_auto_repeat",
				args: {
...
nxenv.utils.get_page_view_count = function (route) {
	return nxenv.call("nxenv.website.doctype.web_page_view.web_page_view.get_page_view_count", {
		path: route,
	});
};
