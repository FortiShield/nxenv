...
nxenv.provide("nxenv.datetime");

nxenv.defaultDateFormat = "YYYY-MM-DD";
nxenv.defaultTimeFormat = "HH:mm:ss";
nxenv.defaultDatetimeFormat = nxenv.defaultDateFormat + " " + nxenv.defaultTimeFormat;
moment.defaultFormat = nxenv.defaultDateFormat;

nxenv.provide("nxenv.datetime");

$.extend(nxenv.datetime, {
	convert_to_user_tz: function (date, format) {
		...
		let date_obj = null;
		if (nxenv.boot.time_zone && nxenv.boot.time_zone.system && nxenv.boot.time_zone.user) {
			date_obj = moment
				.tz(date, nxenv.boot.time_zone.system)
				.clone()
				.tz(nxenv.boot.time_zone.user);
		} else {
			date_obj = moment(date);
		}

		return format === false ? date_obj : date_obj.format(nxenv.defaultDatetimeFormat);
	},
	...
	convert_to_system_tz: function (date, format) {
		...
		if (nxenv.boot.time_zone && nxenv.boot.time_zone.system && nxenv.boot.time_zone.user) {
			date_obj = moment
				.tz(date, nxenv.boot.time_zone.user)
				.clone()
				.tz(nxenv.boot.time_zone.system);
		} else {
			date_obj = moment(date);
		}
	...
	is_system_time_zone: function () {
		if (nxenv.boot.time_zone && nxenv.boot.time_zone.system && nxenv.boot.time_zone.user) {
			return (
				moment().tz(nxenv.boot.time_zone.system).utcOffset() ===
				moment().tz(nxenv.boot.time_zone.user).utcOffset()
			);
		}
	...
	is_timezone_same: function () {
		return nxenv.datetime.is_system_time_zone();
	},

	str_to_obj: function (d) {
		return moment(d, nxenv.defaultDatetimeFormat)._d;
	},
	...
	obj_to_user: function (d) {
		return moment(d).format(nxenv.datetime.get_user_date_fmt().toUpperCase());
	},
	...
	get_user_time_fmt: function () {
		return (nxenv.sys_defaults && nxenv.sys_defaults.time_format) || "HH:mm:ss";
	},

	get_user_date_fmt: function () {
		return (nxenv.sys_defaults && nxenv.sys_defaults.date_format) || "yyyy-mm-dd";
	},
	...
	str_to_user: function (val, only_time = false, only_date = false) {
		if (!val) return "";
		const user_date_fmt = nxenv.datetime.get_user_date_fmt().toUpperCase();
		const user_time_fmt = nxenv.datetime.get_user_time_fmt();
		let user_format = user_time_fmt;

		if (only_time) {
			let date_obj = moment(val, nxenv.defaultTimeFormat);
			return date_obj.format(user_format);
		} else if (only_date) {
			let date_obj = moment(val, nxenv.defaultDateFormat);
			return date_obj.format(user_date_fmt);
		} else {
			let date_obj = moment.tz(val, nxenv.boot.time_zone.system);
			if (typeof val !== "string" || val.indexOf(" ") === -1) {
				user_format = user_date_fmt;
			} else {
				user_format = user_date_fmt + " " + user_time_fmt;
			}
			return date_obj.clone().tz(nxenv.boot.time_zone.user).format(user_format);
		}
	},

	get_datetime_as_string: function (d) {
		let time_format = nxenv?.boot?.sysdefaults?.time_format || nxenv.defaultTimeFormat;
		let datetime_format = nxenv.defaultDateFormat + " " + time_format;
		return moment(d).format(datetime_format);
	},

	user_to_str: function (val, only_time = false) {
		var user_time_fmt = nxenv.datetime.get_user_time_fmt();
		if (only_time) {
			return moment(val, user_time_fmt).format(nxenv.defaultTimeFormat);
		}

		var user_fmt = nxenv.datetime.get_user_date_fmt().toUpperCase();
	...
	user_to_obj: function (d) {
		return nxenv.datetime.str_to_obj(nxenv.datetime.user_to_str(d));
	},
	...
	now_date: function (as_obj = false) {
		return nxenv.datetime._date(nxenv.defaultDateFormat, as_obj);
	},

	now_time: function (as_obj = false) {
		return nxenv.datetime._date(nxenv.defaultTimeFormat, as_obj);
	},

	now_datetime: function (as_obj = false) {
		return nxenv.datetime._date(nxenv.defaultDatetimeFormat, as_obj);
	},

	system_datetime: function (as_obj = false) {
		return nxenv.datetime._date(nxenv.defaultDatetimeFormat, as_obj, true);
	},

	_date: function (format, as_obj = false, system_time = false) {
		let time_zone = nxenv.boot.time_zone?.system || nxenv.sys_defaults.time_zone;
		...
		if (!system_time) {
			time_zone = nxenv.boot.time_zone?.user || time_zone;
		}
		let date = moment.tz(time_zone);

		return as_obj ? nxenv.datetime.moment_to_date_obj(date) : date.format(format);
	},
	...
	nowdate: function () {
		return nxenv.datetime.now_date();
	},
	...
	validate: function (d) {
		return moment(
			d,
			[nxenv.defaultDateFormat, nxenv.defaultDatetimeFormat, nxenv.defaultTimeFormat],
			true
		).isValid();
	},

	get_first_day_of_the_week_index() {
		const first_day_of_the_week = nxenv.sys_defaults.first_day_of_the_week || "Sunday";
