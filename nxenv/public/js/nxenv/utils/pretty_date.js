function prettyDate(date, mini) {
	...
	if (typeof date == "string") {
		date = nxenv.datetime.convert_to_user_tz(date);
		date = new Date(
			(date || "")
				.replace(/-/g, "/")
				.replace(/[TZ]/g, " ")
	...
	let diff =
		(new Date(nxenv.datetime.now_datetime().replace(/-/g, "/")).getTime() - date.getTime()) /
		1000;
...
window.comment_when = function (datetime, mini) {
	var timestamp = nxenv.datetime.str_to_user ? nxenv.datetime.str_to_user(datetime) : datetime;
	return (
		'<span class="nxenv-timestamp ' +
		(mini ? " mini" : "") +
		'" data-timestamp="' +
		datetime +
		'" title="' +
...
nxenv.datetime.comment_when = comment_when;
nxenv.datetime.prettyDate = prettyDate;

nxenv.datetime.refresh_when = function () {
	if (jQuery) {
		$(".nxenv-timestamp").each(function () {
