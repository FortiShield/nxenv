...
nxenv.provide("nxenv.tools");

nxenv.tools.downloadify = function (data, roles, title) {
	if (roles && roles.length && !has_common(roles, roles)) {
		nxenv.msgprint(
			__("Export not allowed. You need {0} role to export.", [nxenv.utils.comma_or(roles)])
		);
		return;
	}

	var filename = title + ".csv";
	var csv_data = nxenv.tools.to_csv(data);
...
nxenv.markdown = function (txt) {
	if (!nxenv.md2html) {
		nxenv.md2html = new showdown.Converter({ tables: true });
	}
	...
	return nxenv.md2html.makeHtml(txt);
};

nxenv.tools.to_csv = function (data) {
	var res = [];
