...
nxenv.ready(function () {
	let web_form_doc = nxenv.web_form_doc;
	let reference_doc = nxenv.reference_doc;
	...
	function show_login_prompt() {
		if (nxenv.session.user != "Guest" || !web_form_doc.login_required) return;
		const login_required = new nxenv.ui.Dialog({
	...
	function show_form() {
		...
		$(".file-size").each(function () {
			$(this).text(nxenv.form.formatters.FileSize($(this).text()));
		});
	}
