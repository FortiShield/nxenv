...
nxenv.provide("nxenv.web_form");
import EventEmitterMixin from "../../nxenv/event_emitter";

export default class WebForm extends nxenv.ui.FieldGroup {
	constructor(opts) {
		super();
		Object.assign(this, opts);
		nxenv.web_form = this;
		nxenv.web_form.events = {};
		Object.assign(nxenv.web_form.events, EventEmitterMixin);
	...
	make() {
		...
		nxenv.init_client_script && nxenv.init_client_script();
		this.setup_listeners();
		nxenv.web_form.events.trigger("after_load");
	...
	set_default_values() {
		...
		let values = nxenv.utils.get_query_params();
		delete values.new;
	...
	render_progress_dots() {
		...
		for (let i = 0; i <= this.page_breaks.length; i++) {
			let $dot = $(`<div class="slide-step">
				...
				<div class="slide-step-complete">${nxenv.utils.icon("tick", "xs")}</div>
	...
	is_next_section_empty(section) {
		...
		let visible_controls = _page.find(".nxenv-control:not(.hide-control)");
	...
	save() {
		...
		// Save
		window.saving = true;
		nxenv.form_dirty = false;

		nxenv.call({
			type: "POST",
			method: "nxenv.website.doctype.web_form.web_form.accept",
			args: {
				data: this.doc,
				web_form: this.name,
				for_payment,
			...
			callback: (response) => {
				...
				if (!response.exc) {
					...
					nxenv.web_form.events.trigger("after_save");
					...
					if (is_new && (response.message.attachment || response.message.file)) {
						nxenv.call({
							type: "POST",
							method: "nxenv.handler.upload_file",
							args: {
	...
	handle_success(data) {
		...
		if (this.success_url) {
			nxenv.utils.setup_timer(5, 0, $(".time"));
			setTimeout(() => {
