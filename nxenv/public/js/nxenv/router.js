...
nxenv.re_route = { "#login": "" };
nxenv.route_titles = {};
nxenv.route_flags = {};
nxenv.route_history = [];
nxenv.view_factory = {};
nxenv.view_factories = [];
nxenv.route_options = null;
nxenv.open_in_new_tab = false;
nxenv.route_hooks = {};
...
$("body").on("click", "a", function (e) {
	...
	if (nxenv.router.is_app_route(target_element.pathname)) {
		// target has "/app, this is a v2 style route.
		if (target_element.search) {
			nxenv.route_options = {};
			let params = new URLSearchParams(target_element.search);
			for (const [key, value] of params) {
				nxenv.route_options[key] = value;
			}
		}
		if (target_element.hash) {
			nxenv.route_hash = target_element.hash;
		}
...
nxenv.router = {
	...
	setup() {
		...
		for (let doctype of nxenv.boot.user.can_read) {
			this.routes[this.slug(doctype)] = { doctype: doctype };
		}
		if (nxenv.boot.doctype_layouts) {
			for (let doctype_layout of nxenv.boot.doctype_layouts) {
	...
	async route() {
		...
		if (nxenv.boot.setup_complete) {
			!nxenv.re_route["setup-wizard"] && (nxenv.re_route["setup-wizard"] = "app");
		} else if (!sub_path.startsWith("setup-wizard")) {
			nxenv.re_route["setup-wizard"] && delete nxenv.re_route["setup-wizard"];
			nxenv.set_route(["setup-wizard"]);
		}
	...
	async convert_to_standard_route(route) {
		...
		if (nxenv.workspaces[route[0]]) {
			// public workspace
			route = ["Workspaces", nxenv.workspaces[route[0]].name];
		} else if (route[0] == "private") {
			// private workspace
			let private_workspace = route[1] && `${route[1]}-${nxenv.user.name.toLowerCase()}`;
			if (!nxenv.workspaces[private_workspace]) {
				nxenv.msgprint(__("Workspace <b>{0}</b> does not exist", [route[1]]));
				return ["Workspaces"];
			}
			route = ["Workspaces", "private", nxenv.workspaces[private_workspace].name];
	...
	set_doctype_route(route) {
		...
		return nxenv.model.with_doctype(doctype_route.doctype).then(() => {
			// doctype route
			let meta = nxenv.get_meta(doctype_route.doctype);
			...
			} else if (nxenv.model.is_single(doctype_route.doctype)) {
	...
	set_history() {
		nxenv.route_history.push(this.current_route);
		nxenv.ui.hide_open_dialog();
	},

	async set_active_sidebar_item() {
		nxenv.app.sidebar.set_active_workspace_item();
	},
	...
	render_page() {
		...
		const factory = nxenv.utils.to_title_case(route[0]);

		if (route[1] && nxenv.views[factory + "Factory"]) {
			route[0] = factory;
			// has a view generator, generate!
			if (!nxenv.view_factory[factory]) {
				nxenv.view_factory[factory] = new nxenv.views[factory + "Factory"]();
			}

			nxenv.view_factory[factory].show();
		} else {
			// show page
			const route_name = nxenv.utils.xss_sanitise(route[0]);
			if (nxenv.views.pageview) {
				nxenv.views.pageview.show(route_name);
			}
		}
	},

	re_route(sub_path) {
		if (nxenv.re_route[sub_path] !== undefined) {
			...
			const re_route_val = this.get_sub_path(nxenv.re_route[sub_path]);
			if (re_route_val === this.current_sub_path) {
				window.history.back();
			} else {
				nxenv.set_route(re_route_val);
			}
	...
	set_title(sub_path) {
		if (nxenv.route_titles[sub_path]) {
			nxenv.utils.set_title(nxenv.route_titles[sub_path]);
		}
	},

	set_route() {
		// set the route (push state) with given arguments
		// example 1: nxenv.set_route('a', 'b', 'c');
		// example 2: nxenv.set_route(['a', 'b', 'c']);
		// example 3: nxenv.set_route('a/b/c');
		...
		return new Promise((resolve) => {
			...
			sub_path += nxenv.route_hash || "";
			nxenv.route_hash = null;
			if (nxenv.open_in_new_tab) {
				localStorage["route_options"] = JSON.stringify(nxenv.route_options);
				window.open(sub_path, "_blank");
				nxenv.open_in_new_tab = false;
			} else {
				try {
					const route_options = nxenv.route_options || {};
	...
	make_url(params) {
		...
		let private_home = `home-${nxenv.user.name.toLowerCase()}`;
		let default_workspace = nxenv.router.slug(nxenv.boot.user.default_workspace?.name || "");

		let workspace =
			nxenv.workspaces[default_workspace] ||
			nxenv.workspaces[private_home] ||
			nxenv.workspaces["home"] ||
			Object.values(nxenv.workspace_map).find((w) => w.app === nxenv.current_app) ||
			Object.values(nxenv.workspaces)[0];

		if (workspace) {
			return (
				"/app/" + (workspace.public ? "" : "private/") + nxenv.router.slug(workspace.name)
			);
		}
	...
	push_state(path, query_params = "") {
		if (window.location.pathname !== path || window.location.search !== query_params) {
			...
			const method = nxenv.route_flags.replace_route ? "replaceState" : "pushState";
	...
	set_route_options_from_url() {
		// set query parameters as nxenv.route_options
		let query_string = window.location.search;

		if (!nxenv.route_options) {
			nxenv.route_options = {};
		}

		if (localStorage.getItem("route_options")) {
			nxenv.route_options = JSON.parse(localStorage.getItem("route_options"));
...
nxenv.get_route = () => nxenv.router.current_route;
nxenv.get_route_str = () => nxenv.router.current_route.join("/");
nxenv.set_route = function () {
	return nxenv.router.set_route.apply(nxenv.router, arguments);
};

nxenv.get_prev_route = function () {
	if (nxenv.route_history && nxenv.route_history.length > 1) {
		return nxenv.route_history[nxenv.route_history.length - 2];
	} else {
		return [];
	}
};

nxenv.set_re_route = function () {
	var tmp = nxenv.router.get_sub_path();
	nxenv.set_route.apply(null, arguments);
	nxenv.re_route[tmp] = nxenv.router.get_sub_path();
};

nxenv.has_route_options = function () {
	return Boolean(Object.keys(nxenv.route_options || {}).length);
};

nxenv.utils.make_event_emitter(nxenv.router);
