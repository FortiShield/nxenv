...
*/
nxenv.model.DocTypeController = class DocTypeController extends nxenv.ui.form.Controller {
	setup() {
		// setup formatters for fieldtype
		nxenv.meta.docfield_map[
	...
	show_db_utilization() {
		...
		nxenv
			.xcall("nxenv.core.doctype.doctype.doctype.get_row_size_utilization", {
				doctype,
			})
			.then((r) => {
				if (r < 50.0) return;
	...
	setup_fetch_from_fields(doc, doctype, docname) {
		...
		let row = nxenv.get_doc(doctype, docname);
		...
		function update_fieldname_options() {
			...
			nxenv.model.with_doctype(link_doctype, () => {
				let fields = nxenv.meta
