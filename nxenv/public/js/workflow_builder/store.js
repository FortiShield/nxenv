...
export const useStore = defineStore("workflow-builder-store", () => {
	...
	async function fetch() {
		await nxenv.model.clear_doc("Workflow", workflow_name.value);
		await nxenv.model.with_doc("Workflow", workflow_name.value);

		workflow_doc.value = nxenv.get_doc("Workflow", workflow_name.value);
		await nxenv.model.with_doctype(workflow_doc.value.document_type);

		if (!workflowfields.value.length) {
			await nxenv.model.with_doctype("Workflow");
			workflowfields.value = nxenv.get_meta("Workflow").fields;
		}

		if (!statefields.value.length) {
			await nxenv.model.with_doctype("Workflow Document State");
			statefields.value = nxenv.get_meta("Workflow Document State").fields;
		}

		if (!transitionfields.value.length) {
			await nxenv.model.with_doctype("Workflow Transition");
			transitionfields.value = nxenv.get_meta("Workflow Transition").fields;
		}

		if (!workflow_doc_fields.value.length) {
			let doc_type = workflow_doc.value.document_type;
			await nxenv.model.with_doctype(doc_type);
			workflow_doc_fields.value = nxenv.meta
				.get_docfields(doc_type, null, {
					fieldtype: ["not in", nxenv.model.no_value_type],
				})
				.sort((a, b) => {
					if (a.label && b.label) {
	...
	async function save_changes() {
		...
		try {
			...
			await nxenv.call("nxenv.client.save", { doc });
			nxenv.toast(__("Workflow updated successfully"));
			fetch();
		} catch (e) {
			console.error(e);
		} finally {
	...
	function setup_breadcrumbs() {
		...
		`;
		nxenv.breadcrumbs.clear();
		nxenv.breadcrumbs.$breadcrumbs.append(breadcrumbs);
	}
