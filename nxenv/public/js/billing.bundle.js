const nxenvCloudBaseEndpoint = "https://nxenvcloud.com";

$(document).ready(function () {
	if (
		nxenv.boot.fc_communication_secret &&
		nxenv.boot.setup_complete === 1 &&
		!nxenv.is_mobile() &&
		nxenv.user.has_role("System Manager")
	) {
		nxenv.call({
			method: "nxenv.integrations.nxenv_providers.nxenvcloud_billing.current_site_info",
			callback: (r) => {
				...
				if (response.trial_end_date) {
					...
					$(".login-to-fc").on("click", function () {
						initiateRequestForLoginToNxenvCloud();
					});
				}
			},
		});
...
function initiateRequestForLoginToNxenvCloud() {
	nxenv.confirm(__("Are you sure you want to login to Nxenv Cloud dashboard?"), () => {
		requestLoginToFC();
	});
}

function requestLoginToFC(freezing_msg = "Initating login to Nxenv Cloud...") {
	nxenv.call({
		method: "nxenv.integrations.nxenv_providers.nxenvcloud_billing.send_verification_code",
		freeze: true,
		...
		error: function (r) {
			nxenv.throw(__("Failed to login to Nxenv Cloud. Please try again"));
		},
	});
}
...
function showFCLoginDialog(email) {
	...
	function verifyCode() {
		...
		nxenv.call({
			method: "nxenv.integrations.nxenv_providers.nxenvcloud_billing.verify_and_login",
			args: {
			...
			callback: function (r) {
				...
				if (message.login_token) {
					...
					window.open(
						`${nxenvCloudBaseEndpoint}/api/method/press.api.developer.saas.login_to_fc?token=${message.login_token}`,
						"_blank"
					);
					nxenv.msgprint({
						title: __("Nxenv Cloud Login Successful"),
						indicator: "green",
						message: `<p>${__(
							"You will be redirected to Nxenv Cloud soon."
						)}</p><p>${__(
							"If you haven't been redirected,"
						)} <a href="${nxenvCloudBaseEndpoint}/api/method/press.api.developer.saas.login_to_fc?token=${
							message.login_token
...
function addLoginToFCDropdownItem() {
	$(".dropdown-navbar-user .dropdown-menu .dropdown-item:nth-child(2)").after(
		`<div class="dropdown-item login-to-fc" target="_blank">Login to Nxenv Cloud</div>`
	);
}
