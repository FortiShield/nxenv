nxenv.ui.OnboardingTour = class OnboardingTour {
	constructor() {
		this.driver_steps = [];
	...
	init_driver() {
		this.driver = new nxenv.Driver({
			className: "nxenv-driver",
			allowClose: false,
			padding: 10,
			...
			opacity: 0.5,
			onHighlighted: (step) => {
				nxenv.ui.next_form_tour = step.options.step_info?.next_form_tour;
				const wait_for_node = setInterval(() => {
					...
					if (step.popover.closeBtnNode) {
						step.popover.closeBtnNode.onclick = () => {
							...
							!nxenv.boot.user.onboarding_status[this.tour.name] &&
								(nxenv.boot.user.onboarding_status[this.tour.name] = {});
							nxenv.boot.user.onboarding_status[this.tour.name].is_complete = true;
							if (!this.driver.hasNextStep()) {
								nxenv.boot.user.onboarding_status[
									this.tour.name
								].all_steps_completed = true;
							}

							nxenv.call({
								method: "nxenv.desk.doctype.form_tour.form_tour.update_user_status",
								args: {
									value: JSON.stringify(nxenv.boot.user.onboarding_status),
				...
				if ($input) nxenv.utils.sleep(200).then(() => $input.focus());
			},
		});
	}

	async init({ tour_name, start_step }) {
		this.tour = await nxenv.db.get_doc("Form Tour", tour_name);
		this.init_driver();
		this.build_steps();
	...
	get_step(step_info, on_next) {
		...
		return {
			...
			popover: {
				title,
				description,
				position: nxenv.router.slug(position || "Bottom"),
			},
			onNext: on_next,
			step_info: step_info,
			showButtons,
...
nxenv.ui.init_onboarding_tour = () => {
	...
	typeof nxenv.boot.onboarding_tours == "undefined" && nxenv.boot.onboarding_tours == [];
	typeof nxenv.boot.user.onboarding_status == "undefined" &&
		nxenv.boot.user.onboarding_status == {};
	let route = nxenv.router.current_route;
	if (route?.[0] === "") return;
	...
	if (route[0] != "dashboard-view") {
		nxenv.boot.onboarding_tours &&
			nxenv.boot.onboarding_tours.forEach((tour) => {
	...
	matching_tours = matching_tours.filter((tour) => {
		if (nxenv.boot.user.onboarding_status[tour[0]]?.is_complete == true) return false;
		return true;
	});
	matching_tours = matching_tours.map((tour) => {
		if (nxenv.boot.user.onboarding_status[tour[0]]?.steps_complete != undefined) {
			tour.push(nxenv.boot.user.onboarding_status[tour[0]].steps_complete);
		}
		return tour;
	});
	if (matching_tours.length == 0) return;
	let current_tour = matching_tours.find(
		(tour) => tour[0] == nxenv.ui.currentTourInstance?.tour?.name
	);
	let next_tour = matching_tours.find((tour) => tour[0] == nxenv.ui.next_form_tour);
	if (current_tour) {
		...
		nxenv.ui.next_form_tour = undefined;
	} else {
	...
	if (!tour_name) return;
	if (nxenv.ui.currentTourInstance?.driver) {
		nxenv.ui.currentTourInstance.driver_steps = [];
		nxenv.ui.currentTourInstance.driver.reset(true);
		nxenv.ui.currentTourInstance.update_driver_steps();
	}
	const tour = (nxenv.ui.currentTourInstance = new nxenv.ui.OnboardingTour());
