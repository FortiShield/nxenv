...
if (props.df.fieldtype === "Datetime") {
	let time_zone_text = nxenv.boot.time_zone
		? nxenv.boot.time_zone.user
		: nxenv.sys_defaults.time_zone;
...
<template>
	<div class="control nxenv-control" :class="{ editable: slots.label }">
		<!-- label -->
		...
		<div
			...
			v-html="nxenv.utils.icon('folder-normal', 'md')"
		/>
		<!-- phone selector icon -->
		<div
			v-if="df.fieldtype == 'Phone'"
			class="selected-phone no-value"
			v-html="nxenv.utils.icon('down', 'sm')"
		/>
	</div>
</template>
