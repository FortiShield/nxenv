...
function edit_html() {
	let d = new nxenv.ui.Dialog({
		...
		primary_action: ({ html }) => {
			html = nxenv.dom.remove_script_and_style(html);
			props.df["html"] = html;
			d.hide();
		},
	});
...
function get_all_columns() {
	let meta = nxenv.get_meta(props.df.options);
	let more_columns = [
		{
			label: __("Sr No."),
			value: "idx",
	...
	return more_columns.concat(
		meta.fields
			.map((tf) => {
				if (nxenv.model.no_value_type.includes(tf.fieldtype)) {
					return;
				}
				return {
					label: tf.label,
...
function get_column_to_add(fieldname) {
	...
	return {
		...nxenv.meta.get_docfield(props.df.options, fieldname),
		width: 10,
	};
}
