...
import nxenv
from nxenv import _
from nxenv.database.database import is_query_type
...
class RecorderConfig:
	...
	def __post_init__(self):
		if not (self.record_jobs or self.record_requests):
			nxenv.throw("You must record one of jobs or requests")

	def store(self):
		nxenv.cache.set_value(RECORDER_CONFIG_FLAG, self, expires_in_sec=RECORDER_AUTO_DISABLE)

	@classmethod
	def retrieve(cls):
		return nxenv.cache.get_value(RECORDER_CONFIG_FLAG) or cls()

	@staticmethod
	def delete():
		nxenv.cache.delete_value(RECORDER_CONFIG_FLAG)


def record_sql(*args, **kwargs):
	start_time = time.monotonic()
	result = nxenv.db._sql(*args, **kwargs)
	end_time = time.monotonic()

	query = getattr(nxenv.db, "last_query", None)
	if not query or isinstance(result, str):
		# run=0, doesn't actually run the query so last_query won't be present
		return result

	stack = []
	if nxenv.local._recorder.config.capture_stack:
	...
	nxenv.local._recorder.register(data)
	return result


def get_current_stack_frames():
	from nxenv.utils.safe_exec import SERVER_SCRIPT_FILE_PREFIX
...
def post_process():
	...
	"""
	nxenv.db.rollback()
	nxenv.db.begin(read_only=True)  # Explicitly start read only transaction

	config = RecorderConfig.retrieve()
	result = list(nxenv.cache.hgetall(RECORDER_REQUEST_HASH).values())
	...
	for request in result:
		for call in request["calls"]:
			...
			if config.explain and is_query_type(formatted_query, ("select", "update", "delete")):
				...
				try:
					call["explain_result"] = nxenv.db.sql(f"EXPLAIN {formatted_query}", as_dict=True)
				except Exception:
					pass
		mark_duplicates(request)
		nxenv.cache.hset(RECORDER_REQUEST_HASH, request["uuid"], request)
...
def record(force=False):
	flag_value = nxenv.client_cache.get_value(RECORDER_INTERCEPT_FLAG)
	if flag_value or force:
		nxenv.local._recorder = Recorder(force=force)
	elif flag_value is None:
		# Explicitly set it once so next requests can use client-side cache
		nxenv.client_cache.set_value(RECORDER_INTERCEPT_FLAG, False)


def dump():
	if hasattr(nxenv.local, "_recorder"):
		nxenv.local._recorder.dump()
...
class Recorder:
	def __init__(self, force=False):
		...
		if (
			...
			and nxenv.request
			and self.config.request_filter in nxenv.request.path
		):
			self.path = nxenv.request.path
			self.cmd = nxenv.local.form_dict.cmd or ""
			self.method = nxenv.request.method
			self.headers = dict(nxenv.local.request.headers)
			self.form_dict = nxenv.local.form_dict
			self.event_type = "HTTP Request"
		elif self.config.record_jobs and nxenv.job and self.config.jobs_filter in nxenv.job.method:
			self.event_type = "Background Job"
			self.path = nxenv.job.method
			self.cmd = None
			self.method = None
			self.headers = None
			self.form_dict = None
		...
		self.uuid = nxenv.generate_hash(length=10)
	...
	def dump(self):
		...
		nxenv.cache.hset(RECORDER_REQUEST_SPARSE_HASH, self.uuid, request_data)

		request_data["calls"] = self.calls
		request_data["headers"] = self.headers
		request_data["form_dict"] = self.form_dict
		request_data["profile"] = profiler_output
		nxenv.cache.hset(RECORDER_REQUEST_HASH, self.uuid, request_data)
	...
	@staticmethod
	def _patch_sql():
		nxenv.db._sql = nxenv.db.sql
		nxenv.db.sql = record_sql

	@staticmethod
	def _unpatch_sql():
		nxenv.db.sql = nxenv.db._sql
...
def do_not_record(function):
	...
	def wrapper(*args, **kwargs):
		if hasattr(nxenv.local, "_recorder"):
			nxenv.local._recorder.cleanup()
			del nxenv.local._recorder
...
def administrator_only(function):
	...
	def wrapper(*args, **kwargs):
		if nxenv.session.user != "Administrator":
			nxenv.throw(_("Only Administrator is allowed to use Recorder"))
		return function(*args, **kwargs)
...
def status(*args, **kwargs):
	return bool(nxenv.cache.get_value(RECORDER_INTERCEPT_FLAG))
...
def start(
	...
	**kwargs,
	...
	nxenv.client_cache.set_value(RECORDER_INTERCEPT_FLAG, True)
...
def stop(*args, **kwargs):
	nxenv.client_cache.set_value(RECORDER_INTERCEPT_FLAG, False)
	nxenv.enqueue(post_process, now=nxenv.flags.in_test)
...
def get(uuid=None, *args, **kwargs):
	if uuid:
		result = nxenv.cache.hget(RECORDER_REQUEST_HASH, uuid)
	else:
		result = list(nxenv.cache.hgetall(RECORDER_REQUEST_SPARSE_HASH).values())
	return result
...
def export_data(*args, **kwargs):
	return list(nxenv.cache.hgetall(RECORDER_REQUEST_HASH).values())
...
def delete(*args, **kwargs):
	nxenv.cache.delete_value(RECORDER_REQUEST_SPARSE_HASH)
	nxenv.cache.delete_value(RECORDER_REQUEST_HASH)
...
def record_queries(func: Callable):
	...
	def wrapped(*args, **kwargs):
		record(force=True)
		nxenv.local._recorder.path = f"Function call: {func.__module__}.{func.__qualname__}"
...
def import_data(file: str) -> None:
	file_doc = nxenv.get_doc("File", {"file_url": file})
	file_content = json.loads(file_doc.get_content())
	for request in file_content:
		nxenv.cache.hset(RECORDER_REQUEST_SPARSE_HASH, request["uuid"], request)
		nxenv.cache.hset(RECORDER_REQUEST_HASH, request["uuid"], request)
