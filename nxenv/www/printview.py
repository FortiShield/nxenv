...
import nxenv
from nxenv import _, cstr, get_module_path
from nxenv.core.doctype.access_log.access_log import make_access_log
from nxenv.core.doctype.document_share_key.document_share_key import is_expired
from nxenv.utils import cint, escape_html, strip_html
...
if TYPE_CHECKING:
	...
	from nxenv.printing.doctype.print_settings.print_settings import PrintSettings
...
def get_context(context) -> PrintContext:
	...
	if not ((nxenv.form_dict.doctype and nxenv.form_dict.name) or nxenv.form_dict.doc):
		return PrintContext(
			print_style="",
			comment="",
			title="Error",
...
<pre>{escape_html(nxenv.as_json(nxenv.form_dict, indent=2))}</pre>
""",
		)

	if nxenv.form_dict.doc:
		doc = nxenv.form_dict.doc
	else:
		doc = nxenv.get_doc(nxenv.form_dict.doctype, nxenv.form_dict.name)

	set_link_titles(doc)

	settings = nxenv.parse_json(nxenv.form_dict.settings)

	letterhead = nxenv.form_dict.letterhead or None
	...
	make_access_log(
		doctype=nxenv.form_dict.doctype, document=nxenv.form_dict.name, file_type="PDF", method="Print"
	)
	...
	body = get_rendered_template(
		...
		meta=meta,
		trigger_print=nxenv.form_dict.trigger_print,
		no_letterhead=nxenv.form_dict.no_letterhead,
		letterhead=letterhead,
		settings=settings,
	)
	print_style = get_print_style(nxenv.form_dict.style, print_format)

	return {
		"body": body,
		"print_style": print_style,
		"comment": nxenv.session.user,
		"title": nxenv.utils.strip_html(cstr(doc.get_title() or doc.name)),
		"lang": nxenv.local.lang,
		"layout_direction": "rtl" if is_rtl() else "ltr",
		"doctype": nxenv.form_dict.doctype,
		"name": nxenv.form_dict.name,
		"key": nxenv.form_dict.get("key"),
...
def get_print_format_doc(print_format_name: str, meta: "Meta") -> Optional["PrintFormat"]:
	...
	if not print_format_name:
		print_format_name = nxenv.form_dict.format or meta.default_print_format or "Standard"

	if print_format_name == "Standard":
		return None
	else:
		try:
			return nxenv.get_doc("Print Format", print_format_name)
...
def get_rendered_template(
	...
	print_settings = nxenv.get_single("Print Settings").as_dict()
	...
	if not nxenv.flags.ignore_print_permissions:
		validate_print_permission(doc)

	if doc.meta.is_submittable:
		if doc.docstatus.is_draft() and not cint(print_settings.allow_print_for_draft):
			nxenv.throw(_("Not allowed to print draft documents"), nxenv.PermissionError)

		if doc.docstatus.is_cancelled() and not cint(print_settings.allow_print_for_cancelled):
			nxenv.throw(_("Not allowed to print cancelled documents"), nxenv.PermissionError)
	...
	if print_format:
		...
		template = None
		if hook_func := nxenv.get_hooks("get_print_format_template"):
			template = nxenv.get_attr(hook_func[-1])(jenv=jenv, print_format=print_format)
	...
	letter_head = nxenv._dict(get_letter_head(doc, no_letterhead, letterhead) or {})

	if letter_head.content:
		letter_head.content = nxenv.utils.jinja.render_template(letter_head.content, {"doc": doc.as_dict()})
	...
	if letter_head.footer:
		letter_head.footer = nxenv.utils.jinja.render_template(letter_head.footer, {"doc": doc.as_dict()})
	...
	hook_func = nxenv.get_hooks("pdf_body_html")
	html = nxenv.get_attr(hook_func[-1])(jenv=jenv, template=template, print_format=print_format, args=args)
...
def set_title_values_for_link_and_dynamic_link_fields(
	...
	for field in meta.get_link_fields() + meta.get_dynamic_link_fields():
		...
		link_title = nxenv.get_cached_value(doctype, doc.get(field.fieldname), meta.title_field)
		if parent_doc:
...
def set_title_values_for_table_and_multiselect_fields(meta: "Meta", doc: "Document") -> None:
	for field in meta.get_table_fields():
		...
		_meta = nxenv.get_meta(field.options)
	...
	for field in doc.meta.fields:
		if field.fieldtype == "Text Editor":
			...
			if value and "<!-- markdown -->" in value:
				doc.set(field.fieldname, nxenv.utils.md_to_html(value))
...
def get_html_and_style(
	...
	if isinstance(name, str):
		document = nxenv.get_doc(doc, name)
	else:
		document = nxenv.get_doc(json.loads(doc))
	...
	try:
		html = get_rendered_template(
			...
			settings=nxenv.parse_json(settings),
		)
	except nxenv.TemplateNotFoundError:
...
def get_rendered_raw_commands(doc: str, name: str | None = None, print_format: str | None = None) -> dict:
	...
	if not print_format or (print_format and not print_format.raw_printing):
		nxenv.throw(
			_("{0} is not a raw printing format.").format(print_format), nxenv.TemplateNotFoundError
		)
...
def validate_print_permission(doc: "Document") -> None:
	if nxenv.has_website_permission(doc):
		return

	for ptype in ("read", "print"):
		if nxenv.has_permission(doc.doctype, ptype, doc):
			return

	if (key := nxenv.form_dict.key) and isinstance(key, str):
		validate_key(key, doc)
		return

	nxenv.throw(_("{0} {1} not found").format(_(doc.doctype), doc.name), nxenv.DoesNotExistError)


def validate_key(key: str, doc: "Document") -> None:
	document_key_expiry = nxenv.get_cached_value(
		"Document Share Key",
	...
	if document_key_expiry is not None:
		if is_expired(document_key_expiry[0]):
			raise nxenv.exceptions.LinkExpired
		else:
			return

	# TODO: Deprecate this! kept it for backward compatibility
	if nxenv.get_system_settings("allow_older_web_view_links") and key == doc.get_signature():
		return

	raise nxenv.exceptions.InvalidKeyError
...
def get_print_format(doctype: str, print_format: "PrintFormat") -> str:
	if print_format.disabled:
		nxenv.throw(_("Print Format {0} is disabled").format(print_format.name), nxenv.DoesNotExistError)

	# server, find template
	module = print_format.module or nxenv.db.get_value("DocType", doctype, "module")
	path = os.path.join(
		get_module_path(module, "Print Format", print_format.name),
		nxenv.scrub(print_format.name) + ".html",
	)
	...
	if os.path.exists(path):
		...
		nxenv.throw(_("No template found at path: {0}").format(path), nxenv.TemplateNotFoundError)
...
def make_layout(doc: "Document", meta: "Meta", format_data=None) -> list:
	...
	for df in format_data or meta.fields:
		...
		if df.fieldtype == "Signature" and not doc.get(df.fieldname):
			placeholder_image = "/assets/nxenv/images/signature-placeholder.png"
...
def get_print_style(
	...
	css = nxenv.get_template("templates/styles/standard.css").render(context)

	if style and nxenv.db.exists("Print Style", style):
		css = css + "\n" + nxenv.db.get_value("Print Style", style, "css")
...
def get_visible_columns(data: list, table_meta: "Meta", df: "DocField") -> list["DocField"]:
	...
	columns = []
	doc = data[0] or nxenv.new_doc(df.options)
