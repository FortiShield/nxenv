...
def get_context(context):
	...
	for token in active_tokens:
		...
		app = {
			...
			"app_name": nxenv.db.get_value("OAuth Client", token.get("client"), "app_name"),
			"creation": creation,
		}
		client_apps.append(app)

	app = None
	if "app" in nxenv.form_dict:
		app = nxenv.get_doc("OAuth Client", nxenv.form_dict.app)
		app = app.__dict__
...
def get_first_login(client):
	login_date = nxenv.get_all(
		"OAuth Bearer Token",
		filters=[["user", "=", nxenv.session.user], ["client", "=", client]],
		fields=["creation"],
		order_by="creation",
		limit=1,
	)
...
def delete_client(client_id: str):
	active_client_id_tokens = nxenv.get_all(
		"OAuth Bearer Token", filters=[["user", "=", nxenv.session.user], ["client", "=", client_id]]
	)
	for token in active_client_id_tokens:
		nxenv.delete_doc("OAuth Bearer Token", token.get("name"), ignore_permissions=True)
