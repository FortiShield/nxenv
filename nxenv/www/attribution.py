...
import nxenv
from nxenv import _
from nxenv.permissions import is_system_user


def get_context(context):
	if not is_system_user():
		nxenv.throw(_("You need to be a system user to access this page."), nxenv.PermissionError)
...
def get_js_deps(app: str) -> list[dict]:
	package_json = Path(nxenv.get_app_path(app, "..", "package.json"))
...
def get_pyproject_info(app: str) -> dict:
	pyproject_toml = Path(nxenv.get_app_path(app, "..", "pyproject.toml"))
