"""This file houses all Nxenv specific optimizations and hooks that run on startup or during fork.
...
def optimize_all():
	...
	# Note:
	# - This function is ALWAYS executed as soon as `import nxenv` ends.
...
def optimize_gc_parameters():
	from nxenv.utils import sbool

	if not bool(sbool(os.environ.get("NXENV_TUNE_GC", True))):
		return

	# generational GC gets triggered after certain allocs (g0) which is 700 by default.
	# This number is quite small for nxenv where a single query can potentially create 700+
	# objects easily.
...
def freeze_gc():
	...
	#
	# Refer to issue for more info: https://github.com/nxenv/nxenv/issues/18927
	gc.collect()
	gc.freeze()
...
def optimize_for_gil_contention():
	if not os.environ.get("NXENV_PERF_PIN_WORKERS"):
		return

	if "gunicorn" not in str(sys.argv[0]):
		return

	if os.environ.get("NXENV_PERF_PIN_WORKERS_DETERMINISTIC"):
		# Ensure same pinning order every time.
		# This is only useful for nxclimarking, DO NOT enable this in production.
		global worker_num
		worker_num = 0
