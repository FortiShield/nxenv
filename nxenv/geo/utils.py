...
def merge_location_features_in_one(coords):
	...
	for element in coords:
		geojson_loc = nxenv.parse_json(element["location"])
		if not geojson_loc:
			continue
...
def return_location(doctype, filters_sql):
	...
	if filters_sql:
		try:
			...
			nxenv.msgprint(nxenv._("This Doctype does not contain location fields"), raise_exception=True)
			return
	else:
		coords = nxenv.get_all(doctype, fields=["name", "location"])
	return coords
...
def return_coordinates(doctype, filters_sql):
	...
	if filters_sql:
		try:
			...
			nxenv.msgprint(
				nxenv._("This Doctype does not contain latitude and longitude fields"), raise_exception=True
			)
			return
	else:
		coords = nxenv.get_all(doctype, fields=["name", "latitude", "longitude"])
	return coords
...
def get_coords_conditions(doctype, filters=None):
	...
	if not nxenv.has_permission(doctype):
		nxenv.throw(nxenv._("Not Permitted"), nxenv.PermissionError)
