...
import nxenv
from nxenv.geo.doctype.country.country import (
	get_countries_and_currencies,
	import_country_and_currency,
)
from nxenv.geo.doctype.currency.currency import enable_default_currencies
from nxenv.tests import IntegrationTestCase, UnitTestCase


def get_table_snapshot(doctype):
	data = nxenv.db.sql(f"select * from `tab{doctype}` order by name", as_dict=True)
...
class TestCountry(IntegrationTestCase):
	def test_bulk_insert_correctness(self):
		def clear_tables():
			nxenv.db.delete("Currency")
			nxenv.db.delete("Country")
