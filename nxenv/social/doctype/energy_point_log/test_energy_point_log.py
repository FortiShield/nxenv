...
import nxenv
from nxenv.desk.form.assign_to import add as assign_to
from nxenv.desk.page.user_profile.user_profile import get_energy_points_heatmap_data
...
class TestEnergyPointLog(IntegrationTestCase):
	...
	def setUpClass(cls):
		super().setUpClass()
		settings = nxenv.get_single("Energy Point Settings")
		settings.enabled = 1
		settings.save()
	...
	def setUp(self):
		nxenv.cache.delete_value("energy_point_rule_map")

	def tearDown(self):
		nxenv.set_user("Administrator")
		nxenv.db.delete("Energy Point Log")
		nxenv.db.delete("Energy Point Rule")
		nxenv.cache.delete_value("energy_point_rule_map")
	...
	def test_revert_points_on_cancelled_doc(self):
		...
		energy_point_logs = nxenv.get_all("Energy Point Log")
		...
		energy_point_logs = nxenv.get_all("Energy Point Log", fields=["reference_name", "type", "reverted"])
	...
	def test_allow_creation_of_new_log_if_the_previous_log_was_reverted(self):
		...
		log_name = nxenv.db.exists("Energy Point Log", {"reference_name": created_todo.name})
		nxenv.get_doc("Energy Point Log", log_name).revert("Just for test")
...
def create_energy_point_rule_for_todo(
	...
	name = "ToDo Closed"
	point_rule_exists = nxenv.db.exists("Energy Point Rule", name)

	if point_rule_exists:
		return nxenv.get_doc("Energy Point Rule", name)
