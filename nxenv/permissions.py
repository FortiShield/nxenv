...
def print_has_permission_check_logs(func):
	...
	def inner(*args, **kwargs):
		...
		self_perm_check = True if not kwargs.get("user") else kwargs.get("user") == nxenv.session.user

		if print_logs:
			nxenv.flags["has_permission_check_logs"] = []
		...
		if not result and self_perm_check and print_logs:
			msgprint(("<br>").join(nxenv.flags.get("has_permission_check_logs", [])))

		if print_logs:
			nxenv.flags.pop("has_permission_check_logs", None)
		return result
...
def _debug_log(log: str):
	if not hasattr(nxenv.local, "permission_debug_log"):
		nxenv.local.permission_debug_log = []
	nxenv.local.permission_debug_log.append(log)


def _pop_debug_log() -> list[str]:
	if log := getattr(nxenv.local, "permission_debug_log", None):
		del nxenv.local.permission_debug_log
		return log
	return []
...
def has_permission(
	...
	*,
	...
	:param print_logs: If True, will display a message using nxenv.msgprint
	...
	if ptype == "share" and nxenv.get_system_settings("disable_document_sharing"):
	...
	if doc:
		if isinstance(doc, str | int):
			doc = nxenv.get_doc(meta.name, doc)
		...
		if not perm:
			...
			msg = _("User {0} does not have access to this document").format(nxenv.bold(user))
			if nxenv.has_permission(doc.doctype):
		...
		debug and _debug_log(
			...
			+ nxenv.as_json(role_permissions, indent=8)
		)
		...
		if not perm:
			push_perm_check_log(
				_("User {0} does not have doctype access via role permission for document {1}").format(
					nxenv.bold(user), nxenv.bold(_(doctype))
				),
				debug=debug,
			)
	...
	def false_if_not_shared():
		...
		elif nxenv.share.get_shared(doctype, user, rights=rights, limit=1):
...
def get_doc_permissions(doc, user=None, ptype=None, debug=False):
	...
	meta = nxenv.get_meta(doc.doctype)
	...
	debug and _debug_log(
		"User has following permissions using role permission system: "
		+ nxenv.as_json(permissions, indent=8)
	)
	...
	if permissions.get("has_if_owner_enabled"):
		...
		debug and _debug_log(
			"User is owner of document, so permissions are updated to: " + nxenv.as_json(permissions)
		)
...
def get_role_permissions(doctype_meta, user=None, is_owner=None, debug=False):
	...
	if isinstance(doctype_meta, str):
		doctype_meta = nxenv.get_meta(doctype_meta)  # assuming doctype name was passed
	...
	if not nxenv.local.role_permissions.get(cache_key) or debug:
		perms = nxenv._dict(if_owner={})
		...
		nxenv.local.role_permissions[cache_key] = perms

	return nxenv.local.role_permissions[cache_key]


def get_user_permissions(user):
	from nxenv.core.doctype.user_permission.user_permission import get_user_permissions
...
def has_user_permission(doc, user=None, debug=False):
	...
	apply_strict_user_permissions = (
		False if doc.meta.issingle else nxenv.get_system_settings("apply_strict_user_permissions")
	)
	...
	def check_user_permission_on_link_fields(d):
		...
		meta = nxenv.get_meta(d.get("doctype"))
...
def has_controller_permissions(doc, ptype, user=None, debug=False) -> bool:
	...
	hooks = nxenv.get_hooks("has_permission")
	methods = hooks.get(doc.doctype, []) + hooks.get("*", [])

	for method in reversed(methods):
		controller_permission = nxenv.call(method, doc=doc, ptype=ptype, user=user, debug=debug)
...
def get_all_perms(role):
	...
	perms = nxenv.get_all("DocPerm", fields="*", filters=dict(role=role))
	custom_perms = nxenv.get_all("Custom DocPerm", fields="*", filters=dict(role=role))
	doctypes_with_custom_perms = nxenv.get_all("Custom DocPerm", pluck="parent", distinct=True)
...
def get_roles(user=None, with_standard=True):
	...
	def get():
		if user == "Administrator":
			return nxenv.get_all("Role", pluck="name")  # return all available roles
		else:
	...
	roles = nxenv.cache.hget("roles", user, get)
...
def get_perms_for(roles, perm_doctype="DocPerm"):
	...
	return nxenv.get_all(perm_doctype, fields=["*"], filters=filters)


def get_doctypes_with_custom_docperms():
	"""Return all the doctypes with Custom Docperms."""

	doctypes = nxenv.get_all("Custom DocPerm", fields=["parent"], distinct=1)
...
def add_user_permission(
	...
	"""Add user permission"""
	from nxenv.core.doctype.user_permission.user_permission import user_permission_exists

	if not user_permission_exists(user, doctype, name, applicable_for):
		if not nxenv.db.exists(doctype, name):
			nxenv.throw(_("{0} {1} not found").format(_(doctype), name), nxenv.DoesNotExistError)
...
def remove_user_permission(doctype, name, user):
	user_permission_name = nxenv.db.get_value(
		"User Permission", dict(user=user, allow=doctype, for_value=name)
	)
	nxenv.delete_doc("User Permission", user_permission_name, force=True)
...
def clear_user_permissions_for_doctype(doctype, user=None):
	...
	if user:
		filters["user"] = user
	user_permissions_for_doctype = nxenv.get_all("User Permission", filters=filters)
	for d in user_permissions_for_doctype:
		nxenv.delete_doc("User Permission", d.name, force=True)


def can_import(doctype, raise_exception=False):
	if not ("System Manager" in nxenv.get_roles() or has_permission(doctype, "import")):
		if raise_exception:
			raise nxenv.PermissionError(f"You are not allowed to import: {doctype}")
		else:
			return False
	return True
...
def can_export(doctype, raise_exception=False, is_owner=False):
	if "System Manager" in nxenv.get_roles():
		return True
	else:
		role_permissions = nxenv.permissions.get_role_permissions(doctype, is_owner=is_owner)
		has_access = role_permissions.get("export") or role_permissions.get("if_owner").get("export")
		if not has_access and raise_exception:
			raise nxenv.PermissionError(_("You are not allowed to export {} doctype").format(doctype))
		return has_access
...
def update_permission_property(
	...
	from nxenv.core.doctype.custom_docperm.custom_docperm import update_custom_docperm
	from nxenv.core.doctype.doctype.doctype import validate_permissions_for_doctype

	out = setup_custom_perms(doctype)

	custom_docperm = nxenv.db.get_value(
...
def setup_custom_perms(parent):
	...
	if not nxenv.db.exists("Custom DocPerm", dict(parent=parent)):
		copy_perms(parent)
		return True
...
def copy_perms(parent):
	...
	for d in nxenv.get_all("DocPerm", fields="*", filters=dict(parent=parent)):
		custom_perm = nxenv.new_doc("Custom DocPerm")
...
def reset_perms(doctype):
	...
	for custom_docperm in nxenv.get_all("Custom DocPerm", filters={"parent": doctype}, pluck="name"):
		nxenv.delete_doc("Custom DocPerm", custom_docperm, ignore_permissions=True, force=True)


def get_linked_doctypes(dt: str) -> list:
	meta = nxenv.get_meta(dt)
	linked_doctypes = [dt] + [
		d.options
		for d in meta.get(
			"fields",
...
def push_perm_check_log(log, debug=False):
	...
	if nxenv.flags.get("has_permission_check_logs") is None:
		return

	nxenv.flags.get("has_permission_check_logs").append(log)
...
def has_child_permission(
	...
	*,
	...
	if not parent_doctype:
		push_perm_check_log(
			_("Please specify a valid parent DocType for {0}").format(nxenv.bold(child_doctype)),
			debug=debug,
		)
		return False

	parent_meta = nxenv.get_meta(parent_doctype)
	...
	if parent_meta.istable or not (
		...
		push_perm_check_log(
			_("{0} is not a valid parent DocType for {1}").format(
				nxenv.bold(parent_doctype), nxenv.bold(child_doctype)
			),
			debug=debug,
		)
		return False
	...
	if child_doc:
		...
		if not parentfield:
			push_perm_check_log(
				_("Parentfield not specified in {0}: {1}").format(
					nxenv.bold(child_doctype), nxenv.bold(child_doc.name)
				),
				debug=debug,
			)
			return False
		...
		if parentfield not in valid_parentfields:
			push_perm_check_log(
				_("{0} is not a valid parentfield for {1}").format(
					nxenv.bold(parentfield), nxenv.bold(child_doctype)
				),
				debug=debug,
			)
			return False
		...
		if permlevel > 0 and permlevel not in accessible_permlevels:
			push_perm_check_log(
				_("Insufficient Permission Level for {0}").format(nxenv.bold(parent_doctype)), debug=debug
			)
			debug and _debug_log(
...
def is_system_user(user: str | None = None) -> bool:
	return nxenv.get_cached_value("User", user or nxenv.session.user, "user_type") == "System User"
