nxenv.ready(() => {
	...
	nxenv.require("controls.bundle.js", () => {
...
const setup_socket_io = () => {
	nxenv.realtime.init(window.socketio_port || "9000");

	nxenv.realtime.on("publish_message", (data) => {
		publish_message(data);
	});

	nxenv.realtime.on("update_message", (data) => {
		update_message(data);
	});

	nxenv.realtime.socket.on("delete_message", (data) => {
		delete_message(data);
	});
};
...
const insert_message = (data) => {
	...
	if ($(`.discussion-on-page[data-topic=${topic.name}]`).length) {
		...
		if (topic.owner == nxenv.session.user) {
	...
	} else if (topic.owner == nxenv.session.user && document_match_found) {
...
const submit_discussion = (e) => {
	...
	if (strip_html(reply).trim() != "" || reply.includes("img")) {
		...
		nxenv.call({
			method: "nxenv.website.doctype.discussion_topic.discussion_topic.submit_discussion",
			args: {
...
const get_color_from_palette = (element) => {
	const palette = nxenv.get_palette(element.attr("title"));
...
const delete_reply = (e) => {
	nxenv.call({
		method: "nxenv.website.doctype.discussion_reply.discussion_reply.delete_message",
		args: {
...
const hide_actions_on_conditions = (template, owner) => {
	...
	nxenv.session.user != owner && $template.find(".dropdown").remove();
	return $template.prop("outerHTML");
};
...
const make_comment_editor = (element) => {
	this.comment_editor = new nxenv.ui.FieldGroup({
		fields: [
			{
	...
	element.find(".nxenv-control").removeClass("hide-control");
