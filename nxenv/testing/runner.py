"""
This module contains the TestRunner class, which is responsible for executing test suites in Nxenv.
...
- Integration with Nxenv's configuration and environment setup
...
Usage:
The TestRunner is typically instantiated and used by Nxenv's test discovery and execution system.
...
class TestRunner(unittest.TextTestRunner):
	...
	def _prepare_category(self, category, suite, app):
		from nxenv.deprecation_dumpster import get_compat_nxenv_test_case_preparation

		dispatcher = {
			"integration": self.integration_preparation,
			"old-nxenv-test-class-category": get_compat_nxenv_test_case_preparation(self.cfg),
