"""
Nxenv Testing Module

This module provides a comprehensive framework for running tests in Nxenv applications.
...
- Integration with Nxenv's hooks and test record creation system

Usage:
This module is typically used by Nxenv's CLI commands for running tests, but can also
...
from nxenv.utils.logger import create_handler as createNxenvFileHandler
...
LOGGING_CONFIG = {
	...
	"loggers": {
		f"{__name__}": {
			"handlers": [],  # only log to the nxenv handler
			"propagate": False,
		},
	},
}
...
handlers = createNxenvFileHandler(__name__)
