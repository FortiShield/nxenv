"""
This module provides functionality for discovering and organizing tests in the Nxenv framework.
...
logger = logging.getLogger("nxenv.testing.discovery")
...
def discover_doctype_tests(doctypes: list[str], runner, app: str, force: bool = False) -> "TestRunner":
	...
	for doctype in doctypes:
		try:
			...
			doctype_app = nxenv.db.get_value("Module Def", module, "app_name")
			if app is None:
				_app = doctype_app
			...
			test_module = nxenv.modules.utils.get_module_name(doctype, module, "test_")
			force and nxenv.db.delete(doctype)
	...
	for test in runner._iterate_suite(test_suite):
		...
		match test:
			...
			case _:
				...
				if any(b.__name__ == "NxenvTestCase" for b in test.__class__.__bases__):
					from nxenv.deprecation_dumpster import deprecation_warning

					deprecation_warning(
						"2024-20-08",
						"v17",
						"accurate categorization of NxenvTestCase will be removed from this runner",
					)
					category = "old-nxenv-test-class-category"
