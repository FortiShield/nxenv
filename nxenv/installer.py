...
import nxenv
from nxenv.defaults import _clear_cache
from nxenv.utils import cint, is_git_url
...
def _is_scheduler_enabled(site) -> bool:
	...
	try:
		nxenv.init(site)
		nxenv.connect()
		enable_scheduler = cint(nxenv.db.get_single_value("System Settings", "enable_scheduler"))
	except Exception:
		pass
	finally:
		nxenv.db.close()
...
def _new_site(
	...
	"""Install a new Nxenv site"""

	from nxenv.utils import scheduler
	...
	if not db_name:
		db_name = f"_{nxenv.generate_hash(length=16)}"
	...
	if rollback_callback:
		rollback_callback.add(lambda: shutil.rmtree(nxenv.get_site_path()))

	with filelock("nxcli_new_site", timeout=1):
		install_db(
		...
		apps_to_install = ["nxenv"] + (nxenv.conf.get("install_apps") or []) + (list(install_apps or []))
	...
	scheduler_status = "disabled" if nxenv.utils.scheduler.is_scheduler_disabled() else "enabled"
...
def install_db(
	...
	from nxenv.database import bootstrap_database, drop_user_and_database, setup_database
	...
	nxenv.flags.in_install_db = True

	if root_login:
		nxenv.flags.root_login = root_login

	if root_password:
		nxenv.flags.root_password = root_password
	...
	nxenv.conf.admin_password = nxenv.conf.admin_password or admin_password

	remove_missing_apps()

	nxenv.db.create_auth_table()
	nxenv.db.create_global_search_table()
	nxenv.db.create_user_settings_table()

	nxenv.flags.in_install_db = False
...
def find_org(org_repo: str) -> tuple[str, str]:
	...
	find_org()
	ref -> https://github.com/nxenv/nxcli/blob/develop/nxcli/utils/__init__.py#L390
	...
	from nxenv.exceptions import InvalidRemoteException
...
def fetch_details_from_tag(_tag: str) -> tuple[str, str, str]:
	...
	ref -> https://github.com/nxenv/nxcli/blob/develop/nxcli/utils/__init__.py#L403
...
def parse_app_name(name: str) -> str:
	...
	ref -> https://github.com/nxenv/nxcli/blob/develop/nxcli/app.py#L114
...
def install_app(name, verbose=False, set_as_patched=True, force=False):
	...
	from nxenv.model.sync import sync_for
	from nxenv.modules.utils import sync_customizations
	from nxenv.utils.fixtures import sync_fixtures

	nxenv.flags.in_install = name
	nxenv.flags.ignore_in_install = False

	nxenv.clear_cache()
	app_hooks = nxenv.get_hooks(app_name=name)
	installed_apps = nxenv.get_installed_apps()
	...
	if name not in nxenv.get_all_apps():
	...
	for before_install in app_hooks.before_install or []:
		out = nxenv.get_attr(before_install)()
		if out is False:
			return

	for fn in nxenv.get_hooks("before_app_install"):
		nxenv.get_attr(fn)(name)
	...
	nxenv.get_doc("Portal Settings", "Portal Settings").sync_menu()

	if set_as_patched:
		set_all_patches_as_completed(name)

	for after_install in app_hooks.after_install or []:
		nxenv.get_attr(after_install)()

	for fn in nxenv.get_hooks("after_app_install"):
		nxenv.get_attr(fn)(name)
	...
	for after_sync in app_hooks.after_sync or []:
		nxenv.get_attr(after_sync)()  #

	nxenv.flags.in_install = False
...
def add_to_installed_apps(app_name, rebuild_website=True):
	...
	if app_name not in installed_apps:
		...
		nxenv.db.set_global("installed_apps", json.dumps(installed_apps))
		nxenv.db.commit()
...
def remove_app(app_name, dry_run=False, yes=False, no_backup=False, force=False):
	...
	app_hooks = nxenv.get_hooks(app_name=app_name)

	# dont allow uninstall app if not installed unless forced
	if not force:
		if app_name not in nxenv.get_installed_apps():
	...
	for app in nxenv.get_installed_apps():
		if app != app_name:
			hooks = nxenv.get_hooks(app_name=app)
	...
	nxenv.flags.in_uninstall = True

	for before_uninstall in app_hooks.before_uninstall or []:
		nxenv.get_attr(before_uninstall)()

	for fn in nxenv.get_hooks("before_app_uninstall"):
		nxenv.get_attr(fn)(app_name)

	modules = nxenv.get_all("Module Def", filters={"app_name": app_name}, pluck="name")
	...
	for after_uninstall in app_hooks.after_uninstall or []:
		nxenv.get_attr(after_uninstall)()

	for fn in nxenv.get_hooks("after_app_uninstall"):
		nxenv.get_attr(fn)(app_name)

	click.secho(f"Uninstalled App {app_name} from Site {site}", fg="green")
	nxenv.flags.in_uninstall = False
...
def _delete_modules(modules: list[str], dry_run: bool) -> list[str]:
	...
	for module_name in modules:
		...
		for doctype in nxenv.get_all(
			...
			if not dry_run:
				if doctype.issingle:
					nxenv.delete_doc("DocType", doctype.name, ignore_on_trash=True, force=True)
				else:
		...
		if not dry_run:
			nxenv.delete_doc("Module Def", module_name, ignore_on_trash=True, force=True)
...
def _delete_linked_documents(module_name: str, doctype_linkfield_map: dict[str, str], dry_run: bool) -> None:
	...
	for doctype, fieldname in doctype_linkfield_map.items():
		for record in nxenv.get_all(doctype, filters={fieldname: module_name}, pluck="name"):
			print(f"* removing {doctype} '{record}'...")
			if not dry_run:
				nxenv.delete_doc(doctype, record, ignore_on_trash=True, force=True)
...
def _get_module_linked_doctype_field_map() -> dict[str, str]:
	...
	linked_doctypes = nxenv.get_all(
		"DocField",
		filters={"fieldtype": "Link", "options": "Module Def"},
		fields=["parent", "fieldname"],
	)
	existing_linked_doctypes = [d for d in linked_doctypes if nxenv.db.exists("DocType", d.parent)]
...
def _delete_doctypes(doctypes: list[str], dry_run: bool) -> None:
	for doctype in set(doctypes):
		...
		if not dry_run:
			nxenv.delete_doc("DocType", doctype, ignore_on_trash=True, force=True)
			nxenv.db.sql_ddl(f"DROP TABLE IF EXISTS `tab{doctype}`")
...
def set_all_patches_as_completed(app):
	from nxenv.modules.patch_handler import get_patches_from_app

	patches = get_patches_from_app(app)
	for patch in patches:
		nxenv.get_doc({"doctype": "Patch Log", "patch": patch}).insert(ignore_permissions=True)
	nxenv.db.commit()


def init_singles():
	singles = nxenv.get_all("DocType", filters={"issingle": True}, pluck="name")
	for single in singles:
		if nxenv.db.get_singles_dict(single):
			continue

		try:
			doc = nxenv.new_doc(single)
			doc.flags.ignore_mandatory = True
			doc.flags.ignore_validate = True
			doc.save()
		except (ImportError, nxenv.DoesNotExistError):
...
def make_conf(
	...
	sites_path = nxenv.local.sites_path
	nxenv.destroy()
...
def make_site_config(
	...
	nxenv.create_folder(os.path.join(nxenv.local.site_path))
...
def update_site_config(key, value, validate=True, site_config_path=None):
	...
	from nxenv.config import clear_site_config_cache
	from nxenv.utils.synchronization import filelock
...
def _update_config_file(key: str, value, config_file: str):
	...
	if hasattr(nxenv.local, "conf"):
		nxenv.local.conf[key] = value


def get_site_config_path():
	return os.path.join(nxenv.local.site_path, "site_config.json")
...
def make_site_dirs():
	for dir_path in [
		...
		path = nxenv.get_site_path(dir_path)
		os.makedirs(path, exist_ok=True)


def add_module_defs(app, ignore_if_duplicate=False):
	modules = nxenv.get_module_list(app)
	for module in modules:
		d = nxenv.new_doc("Module Def")
		d.app_name = app
...
def remove_missing_apps():
	...
	apps = ("nxenv_subscription", "shopping_cart")
	installed_apps = json.loads(nxenv.db.get_global("installed_apps") or "[]")
	for app in apps:
...
def extract_files(site_name, file_path):
	...
	from nxenv.utils import get_nxcli_relative_path

	file_path = get_nxcli_relative_path(file_path)

	# Need to do nxenv.init to maintain the site locals
	nxenv.init(site_name)
	abs_site_path = os.path.abspath(nxenv.get_site_path())
...
def is_downgrade(sql_file_path, verbose=False):
	"""Check if input db backup will get downgraded on current nxcli
	...
	current_version = Version(nxenv.__version__)
	...
	if verbose and is_downgrade:
		print(f"Your site is currently on Nxenv {current_version} and your backup is {backup_version}.")

	return is_downgrade


def get_old_backup_version(sql_file_path: str) -> Version | None:
	"""Return the nxenv version used to create the specified database dump.

	This methods supports older versions of Nxenv wich used a different format.
	"""
	header = get_db_dump_header(sql_file_path).split("\n")
	if match := re.search(r"Nxenv (\d+\.\d+\.\d+)", header[0]):
...
def get_backup_version(sql_file_path: str) -> Version | None:
	...
	header = get_db_dump_header(sql_file_path).split("\n")
	metadata = ""
	if "begin nxenv metadata" in header[0]:
		for line in header[1:]:
			if "end nxenv metadata" in line:
				break
			metadata += line.replace("--", "").strip() + "\n"
		parser = configparser.ConfigParser()
		parser.read_string(metadata)
		return Version(parser["nxenv"]["version"])
...
def partial_restore(sql_file_path, verbose=False):
	if nxenv.conf.db_type == "mariadb":
		from nxenv.database.mariadb.setup_db import import_db_from_sql
	elif nxenv.conf.db_type == "postgres":
		import warnings

		from nxenv.database.postgres.setup_db import import_db_from_sql
...
def validate_database_sql(path: str, _raise: bool = True) -> None:
	...
	if os.path.getsize(path):
		...
		try:
			nxenv.utils.execute_in_shell(f"{executable} -m1 __Auth {path}", check_exit_code=True)
			return
		except Exception:
	...
	if _raise:
		raise nxenv.InvalidDatabaseFile
