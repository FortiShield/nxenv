...
from nxenv.desk.form.document_follow import is_document_followed
from nxenv.model.utils.user_settings import get_user_settings
from nxenv.permissions import get_doc_permissions, has_permission
...
def getdoc(doctype, name):
	...
	if (key := nxenv.can_cache_doc((doctype, name))) and nxenv.cache.exists(key):
		nxenv._set_document_in_cache(key, doc)
	...
	doc.add_seen()
	set_link_titles(doc)
	if nxenv.response.docs is None:
		nxenv.local.response = _dict({"docs": []})
...
def getdoctype(doctype, with_parent=False, cached_timestamp=None):
	...
	if with_parent and (parent_dt := nxenv.model.meta.get_parent_dt(doctype)):
		docs = get_meta_bundle(parent_dt)
		nxenv.response["parent_dt"] = parent_dt

	if not docs:
		docs = get_meta_bundle(doctype)

	nxenv.response["user_settings"] = get_user_settings(parent_dt or doctype)

	if cached_timestamp and docs[0].modified == cached_timestamp:
		return "use_cache"

	nxenv.response.docs.extend(docs)


def get_meta_bundle(doctype):
	bundle = [nxenv.desk.form.meta.get_meta(doctype)]
	bundle.extend(
		nxenv.desk.form.meta.get_meta(df.options)
		for df in bundle[0].fields
		if df.fieldtype in nxenv.model.table_fields
	)
	return bundle
...
def get_docinfo(doc=None, doctype=None, name=None):
	from nxenv.share import _get_users as get_docshares
	...
	docinfo.update(
		{
			...
			"is_document_followed": is_document_followed(doc.doctype, doc.name, nxenv.session.user),
	...
	nxenv.response["docinfo"] = docinfo
...
def add_comments(doc, docinfo):
	...
	for c in comments:
		match c.comment_type:
			case "Comment":
				c.content = nxenv.utils.markdown(c.content)
...
def get_error_log_exists(doc: "Document") -> bool:
	if has_permission("Error Log", print_logs=False):
		return nxenv.db.exists("Error Log", {"reference_doctype": doc.doctype, "reference_name": doc.name})
	return False


def get_webhook_request_log_exists(doc: "Document") -> bool:
	if has_permission("Webhook Request Log", print_logs=False):
		return nxenv.db.exists(
...
def get_comments(doctype: str, name: str, comment_type: str | list[str] = "Comment") -> list[nxenv._dict]:
...
def get_additional_timeline_content(doctype, docname):
	contents = []
	hooks = nxenv.get_hooks().get("additional_timeline_content", {})
	...
	for method in methods_for_all_doctype + methods_for_current_doctype:
		contents.extend(nxenv.get_attr(method)(doctype, docname) or [])
...
def get_title_values_for_link_and_dynamic_link_fields(doc, link_fields=None):
	...
	for field in link_fields:
		...
		link_title = nxenv.db.get_value(doctype, doc_fieldvalue, meta.title_field, cache=True, order_by=None)
...
def send_link_titles(link_titles):
	"""Append link titles dict in `nxenv.local.response`."""
	if "_link_titles" not in nxenv.local.response:
		nxenv.local.response["_link_titles"] = {}

	nxenv.local.response["_link_titles"].update(link_titles)
...
def update_user_info(docinfo):
	...
	nxenv.utils.add_user_info(users, docinfo.user_info)
