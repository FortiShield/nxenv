...
import nxenv
from nxenv import _
from nxenv.build import scrub_html_template
from nxenv.model.meta import Meta
from nxenv.model.utils import render_include
from nxenv.modules import get_module_path, load_doctype_module, scrub
from nxenv.utils import get_nxcli_path, get_html_format
...
def get_meta(doctype, cached=True) -> "FormMeta":
	...
	cached = cached and not nxenv.conf.developer_mode
	if cached:
		meta = nxenv.cache.hget("doctype_form_meta", doctype)
		if not meta:
			# Cache miss - explicitly get meta from DB to avoid
			meta = FormMeta(doctype, cached=False)
			nxenv.cache.hset("doctype_form_meta", doctype, meta)
	else:
...
class FormMeta(Meta):
	def __init__(self, doctype, *, cached=True):
		self.__dict__.update(nxenv.get_meta(doctype, cached=cached).__dict__)
		self.load_assets()
	...
	def add_code(self):
		...
		system_country = nxenv.get_system_settings("country")
	...
	def _add_code(self, path, fieldname):
		...
		if js:
			...
			asset_path = path.replace(nxcli_path, "")
			comment = f"\n\n/* Adding {asset_path} */\n\n"
...
"""
	...
	def add_search_fields(self):
		...
		for df in self.get("fields", {"fieldtype": "Link", "options": ["!=", "[Select]"]}):
			if df.options:
				try:
					search_fields = nxenv.get_meta(df.options).search_fields
	...
	def _show_missing_doctype_msg(self, df):
		...
		msg = _("Field {0} is referring to non-existing doctype {1}.").format(
			nxenv.bold(df.fieldname), nxenv.bold(df.options)
		)
		...
		nxenv.throw(msg, title=_("Missing DocType"))

	def add_linked_document_type(self):
		for df in self.get("fields", {"fieldtype": "Link"}):
			if df.options:
				try:
					df.linked_document_type = nxenv.get_meta(df.options).document_type
				except nxenv.DoesNotExistError:
					self._show_missing_doctype_msg(df)

	def load_print_formats(self):
		print_formats = nxenv.db.sql(
	...
	def load_workflows(self):
		...
		if workflow_name and nxenv.db.exists("Workflow", workflow_name):
			workflow = nxenv.get_doc("Workflow", workflow_name)
			workflow_docs.append(workflow)

			workflow_docs.extend(nxenv.get_doc("Workflow State", d.state) for d in workflow.get("states"))
		...
		if not self.custom:
			...
			if hasattr(module, "form_grid_templates"):
				for key, path in module.form_grid_templates.items():
					templates[key] = get_html_format(nxenv.get_app_path(app, path))
	...
	def load_workspaces(self):
		Shortcut = nxenv.qb.DocType("Workspace Shortcut")
		Workspace = nxenv.qb.DocType("Workspace")
		shortcut = (
			nxenv.qb.from_(Shortcut)
		...
		if shortcut:
			...
			Link = nxenv.qb.DocType("Workspace Link")
			link = (
				nxenv.qb.from_(Link)
				.select(Link.parent)
				.inner_join(Workspace)
...
def get_code_files_via_hooks(hook, name):
	...
	for app_name in nxenv.get_installed_apps():
		code_hook = nxenv.get_hooks(hook, default={}, app_name=app_name)
		if not code_hook:
			continue
		...
		for file in files:
			path = nxenv.get_app_path(app_name, *file.strip("/").split("/"))
