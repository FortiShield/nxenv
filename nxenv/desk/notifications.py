...
def get_notifications():
	...
	if nxenv.flags.in_install or not nxenv.get_system_settings("setup_complete"):
		return out
	...
	for name in groups:
		count = nxenv.cache.hget("notification_count:" + name, nxenv.session.user)
...
def get_notifications_for_doctypes(config, notification_count):
	...
	can_read = nxenv.get_user().get_can_read()
	open_count_doctype = {}
			...
			if d in notification_count:
				...
				try:
					if isinstance(condition, dict):
						result = nxenv.get_list(
							d, fields=["count(*) as count"], filters=condition, ignore_ifnull=True
						)[0].count
					else:
						result = nxenv.get_attr(condition)()

				except nxenv.PermissionError:
					nxenv.clear_messages()
					pass
				# nxenv.msgprint("Permission Error in notifications for {0}".format(d))
					...
					nxenv.cache.hset("notification_count:" + d, nxenv.session.user, result)
...
def clear_notifications(user=None):
	...
	if user:
		nxenv.cache.hdel_names([f"notification_count:{name}" for name in groups], user)
	else:
		nxenv.cache.delete_value([f"notification_count:{name}" for name in groups])


def clear_notification_config(user):
	nxenv.cache.hdel("notification_config", user)


def delete_notification_count_for(doctype):
	nxenv.cache.delete_key("notification_count:" + doctype)
...
def get_notification_info():
	...
	conditions = {}
	module_doctypes = {}
	doctype_info = dict(nxenv.db.sql("""select name, module from tabDocType"""))
...
def get_notification_config():
	user = nxenv.session.user or "Guest"
	...
	return nxenv.cache.hget("notification_config", user, _get)
...
def get_open_count(doctype: str, name: str, items=None):
	...
	if nxenv.flags.in_migrate or nxenv.flags.in_install:
		return {"count": []}
	...
	if not meta.custom:
		module = nxenv.get_meta_module(doctype)
...
def notify_mentions(ref_doctype, ref_name, content):
	if ref_doctype and ref_name and content:
		...
		sender_fullname = get_fullname(nxenv.session.user)
		...
		notification_message = _("""{0} mentioned you in a comment in {1} {2}""").format(
			nxenv.bold(sender_fullname), nxenv.bold(ref_doctype), get_title_html(title)
		)
...
def extract_mentions(txt):
	...
	for mention in soup.find_all(class_="mention"):
		if mention.get("data-is-group") == "true":
			try:
				user_group = nxenv.get_cached_doc("User Group", mention["data-id"])
