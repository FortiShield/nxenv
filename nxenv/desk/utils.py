...
def validate_route_conflict(doctype, name):
	...
	for _doctype in ["Page", "Workspace", "DocType"]:
		all_names.extend(
			[slug(d) for d in nxenv.get_all(_doctype, pluck="name") if (doctype != _doctype and d != name)]
		)

	if slug(name) in all_names:
		nxenv.msgprint(nxenv._("Name already taken, please set a new name"))
		raise nxenv.NameError
...
def pop_csv_params(form_dict):
	...
	from nxenv.utils.data import cint, cstr
...
def provide_binary_file(filename: str, extension: str, content: bytes) -> None:
	...
	nxenv.response["filecontent"] = content
	nxenv.response["filename"] = f"{_(filename)}.{extension}"
