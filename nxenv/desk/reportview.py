...
from nxenv.model import child_table_fields, default_fields, get_permitted_fields, optional_fields
...
from nxenv.utils import add_user_info, cint, format_duration
...
def get():
	...
	if is_virtual_doctype(args.doctype):
		...
		data = compress(nxenv.call(controller.get_list, args=args, **args))
	else:
...
def get_list():
	...
	if is_virtual_doctype(args.doctype):
		controller = get_controller(args.doctype)
		data = nxenv.call(controller.get_list, args=args, **args)
	else:
		# uncompressed (refactored from nxenv.model.db_query.get_list)
		data = execute(**args)
...
def get_count() -> int:
	...
	if is_virtual_doctype(args.doctype):
		controller = get_controller(args.doctype)
		count = nxenv.call(controller.get_count, args=args, **args)
	else:
		...
		if args.limit:
			...
			count = nxenv.db.sql(f"""select count(*) from ( {partial_query} ) p""")[0][0]

			if count == args.limit:
				nxenv.response.can_cache = True
		else:
...
def get_form_params():
	...
	data = nxenv._dict(nxenv.local.form_dict)
	clean_params(data)
	validate_args(data)
	return data
	...
	if isinstance(filters, list):
		...
		for condition in filters:
			if len(condition) == 3:
				...
				if is_standard(fieldname):
					continue
				meta = nxenv.get_meta(condition[0])
...
def setup_group_by(data):
	...
	if data.group_by and data.aggregate_function:
		if data.aggregate_function.lower() not in ("count", "sum", "avg"):
			nxenv.throw(_("Invalid aggregate function"))

		if nxenv.db.has_column(data.aggregate_on_doctype, data.aggregate_on_field):
			data.fields.append(
...
def raise_invalid_field(fieldname):
	nxenv.throw(_("Field not permitted in query") + f": {fieldname}", nxenv.DataError)
...
def get_meta_and_docfield(fieldname, data):
	...
	meta = nxenv.get_meta(parenttype)
...
def get_parenttype_and_fieldname(field, data):
	if "." in field:
		...
		parenttype = parts[0]
		fieldname = parts[1]
		df = nxenv.get_meta(data.doctype).get_field(parenttype)
...
def compress(data, args=None):
	...
	from nxenv.desk.query_report import add_total_row
	...
	if args.get("add_total_row"):
		meta = nxenv.get_meta(args.doctype)
...
def save_report(name, doctype, report_settings):
	...
	if nxenv.db.exists("Report", name):
		report = nxenv.get_doc("Report", name)
		if report.is_standard == "Yes":
			nxenv.throw(_("Standard Reports cannot be edited"))

		if report.report_type != "Report Builder":
			nxenv.throw(_("Only reports of type Report Builder can be edited"))

		if report.owner != nxenv.session.user and not report.has_permission("write"):
			nxenv.throw(_("Insufficient Permissions for editing Report"), nxenv.PermissionError)
	else:
		report = nxenv.new_doc("Report")
	...
	nxenv.msgprint(
		_("Report {0} saved").format(nxenv.bold(report.name)),
		indicator="green",
		alert=True,
	)
	return report.name
...
def delete_report(name):
	...
	report = nxenv.get_doc("Report", name)
	if report.is_standard == "Yes":
		nxenv.throw(_("Standard Reports cannot be deleted"))

	if report.report_type != "Report Builder":
		nxenv.throw(_("Only reports of type Report Builder can be deleted"))

	if report.owner != nxenv.session.user and not report.has_permission("delete"):
		nxenv.throw(_("Insufficient Permissions for deleting Report"), nxenv.PermissionError)

	report.delete(ignore_permissions=True)
	nxenv.msgprint(
		_("Report {0} deleted").format(nxenv.bold(report.name)),
		indicator="green",
		alert=True,
	)
...
def export_query():
	...
	from nxenv.desk.utils import get_csv_bytes, pop_csv_params, provide_binary_file
	...
	if not nxenv.permissions.can_export(doctype):
		if nxenv.permissions.can_export(doctype, is_owner=True):
			for row in ret:
				if row[-1] != nxenv.session.user:
	...
	if nxenv.local.lang == "en" or not translate_values:
	...
	if file_format_type == "CSV":
		...
		content = get_csv_bytes(
			[[handle_html(nxenv.as_unicode(v)) if isinstance(v, str) else v for v in r] for r in data],
			csv_params,
		)
	elif file_format_type == "Excel":
		from nxenv.utils.xlsxutils import make_xlsx
...
def get_field_info(fields, doctype):
	...
	for key in fields:
		...
		if parenttype == doctype and fieldname == "name":
			...
			df = nxenv.get_meta(parenttype).get_field(fieldname)
			if df and df.fieldtype in ("Data", "Select", "Small Text", "Text"):
				name = df.name
				label = _(df.label)
				fieldtype = df.fieldtype
				translatable = getattr(df, "translatable", False)
			elif df and df.fieldtype == "Link" and nxenv.get_meta(df.options).translated_doctype:
				name = df.name
				label = _(df.label)
				fieldtype = df.fieldtype
				translatable = True
...
def delete_items():
	...
	items = sorted(json.loads(nxenv.form_dict.get("items")), reverse=True)
	doctype = nxenv.form_dict.get("doctype")

	if len(items) > 10:
		nxenv.enqueue("nxenv.desk.reportview.delete_bulk", doctype=doctype, items=items)
	else:
...
def delete_bulk(doctype, items):
	...
	for i, d in enumerate(items):
		try:
			nxenv.flags.in_bulk_delete = True
			nxenv.delete_doc(doctype, d)
			if len(items) >= 5:
...
def get_sidebar_stats(stats, doctype, filters=None):
	...
	if is_virtual_doctype(doctype):
		...
		data = nxenv.call(controller.get_stats, args=args, **args)
	else:
...
def get_stats(stats, doctype, filters=None):
	...
	try:
		db_columns = nxenv.db.get_table_columns(doctype)
	except (nxenv.db.InternalError, nxenv.db.ProgrammingError):
