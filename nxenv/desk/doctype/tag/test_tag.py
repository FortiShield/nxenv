import nxenv
from nxenv.desk.doctype.tag.tag import add_tag
from nxenv.desk.reportview import get_stats
...
class TestTag(IntegrationTestCase):
	def setUp(self) -> None:
		nxenv.db.delete("Tag")
		nxenv.db.sql("UPDATE `tabDocType` set _user_tags=''")

	def test_tag_count_query(self):
		self.assertDictEqual(
			get_stats('["_user_tags"]', "DocType"),
			{"_user_tags": [["No Tags", nxenv.db.count("DocType")]]},
		)
		...
		self.assertDictEqual(
			get_stats('["_user_tags"]', "DocType"),
			{"_user_tags": [["Standard", 2], ["No Tags", nxenv.db.count("DocType") - 2]]},
		)
