...
from nxenv.utils import unique
...
def check_user_tags(dt):
	...
	try:
		doctype = DocType(dt)
		nxenv.qb.from_(doctype).select(doctype._user_tags).limit(1).run()
...
def get_tagged_docs(doctype, tag):
	...
	return (nxenv.qb.from_(doctype).where(doctype._user_tags.like(tag)).select(doctype.name)).run()


@nxenv.whitelist()
def get_tags(doctype, txt):
	tag = nxenv.get_list("Tag", filters=[["name", "like", f"%{txt}%"]])
...
class DocTags:
	...
	def get_tag_fields(self):
		...
		return nxenv.db.get_value("DocType", self.dt, "tag_fields")

	def get_tags(self, dn):
		"""Return tag for a particular item."""
		return (nxenv.db.get_value(self.dt, dn, "_user_tags", ignore=1) or "").strip()
	...
	def add(self, dn, tag):
		...
		if tag not in tl:
			tl.append(tag)
			if not nxenv.db.exists("Tag", tag):
				nxenv.get_doc({"doctype": "Tag", "name": tag}).insert(ignore_permissions=True)
			self.update(dn, tl)
...
def delete_tags_for_document(doc):
	...
	"""
	if not nxenv.db.table_exists("Tag Link"):
		return

	nxenv.db.delete("Tag Link", {"document_type": doc.doctype, "document_name": doc.name})
...
def update_tags(doc, tags):
	...
	for tag in deleted_tags:
		nxenv.db.delete("Tag Link", {"document_type": doc.doctype, "document_name": doc.name, "tag": tag})
...
def get_tags_list_for_awesomebar():
	return nxenv.get_list("Tag", pluck="name", order_by=None)
