...
def fetch_changelog_feed():
	...
	for fn in nxenv.get_hooks("get_changelog_feed"):
		try:
			cache_key = f"changelog_feed::{fn}"
			changelog_feed = nxenv.cache.get_value(cache_key, shared=True)
			if changelog_feed is None:
				changelog_feed = nxenv.call(fn, since=since)[:20] or []
			...
			for feed_item in changelog_feed:
				...
				if not nxenv.db.exists("Changelog Feed", feed):
					nxenv.new_doc("Changelog Feed").update(feed).insert()
		except Exception:
			nxenv.log_error(f"Failed to fetch changelog from {fn}")
			# don't retry if it's broken for 1 week
			nxenv.cache.set_value(cache_key, [], expires_in_sec=7 * 24 * 60 * 60, shared=True)
...
def get_changelog_feed_items():
	...
	feed = nxenv.get_all(
		...
		filters={"app_name": ("in", nxenv.get_installed_apps())},
		order_by="posting_timestamp desc",
		limit=20,
	)
	for f in feed:
...
def _app_title(app_name):
	try:
		return nxenv.get_hooks("app_title", app_name=app_name)[0]
	except Exception:
		return app_name


def get_feed(since):
	"""'What's New' feed implementation for Nxenv"""
	r = requests.get(f"https://nxenv.io/api/method/changelog_feed?since={since}")
	r.raise_for_status()
