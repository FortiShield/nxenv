...
def save_listview_settings(doctype, listview_settings, removed_listview_fields):
	listview_settings = nxenv.parse_json(listview_settings)
	removed_listview_fields = nxenv.parse_json(removed_listview_fields)

	if nxenv.get_all("List View Settings", filters={"name": doctype}):
	...
	return {"meta": nxenv.get_meta(doctype, False), "listview_settings": doc}


def set_listview_fields(doctype, listview_fields, removed_listview_fields):
	meta = nxenv.get_meta(doctype)

	listview_fields = [f.get("fieldname") for f in nxenv.parse_json(listview_fields) if f.get("fieldname")]
...
def set_in_list_view_property(doctype, field, value):
	...
	property_setter = nxenv.db.get_value(
		"Property Setter",
		{"doc_type": doctype, "field_name": field.fieldname, "property": "in_list_view"},
	)
	if property_setter:
		doc = nxenv.get_doc("Property Setter", property_setter)
		doc.value = value
		doc.save()
	else:
...
def get_default_listview_fields(doctype):
	meta = nxenv.get_meta(doctype)
	path = nxenv.get_module_path(
		nxenv.scrub(meta.module), "doctype", nxenv.scrub(meta.name), nxenv.scrub(meta.name) + ".json"
	)
	doctype_json = nxenv.get_file_json(path)
