...
from nxenv.desk.doctype.dashboard_chart.dashboard_chart import get
from nxenv.tests import IntegrationTestCase, UnitTestCase
from nxenv.utils import formatdate, get_last_day, getdate
from nxenv.utils.dateutils import get_period, get_period_ending
...
class TestDashboardChart(IntegrationTestCase):
	...
	def test_dashboard_chart(self):
		if nxenv.db.exists("Dashboard Chart", "Test Dashboard Chart"):
			nxenv.delete_doc("Dashboard Chart", "Test Dashboard Chart")
	...
	def test_empty_dashboard_chart(self):
		if nxenv.db.exists("Dashboard Chart", "Test Empty Dashboard Chart"):
			nxenv.delete_doc("Dashboard Chart", "Test Empty Dashboard Chart")

		nxenv.db.delete("Error Log")
	...
	def test_chart_wih_one_value(self):
		if nxenv.db.exists("Dashboard Chart", "Test Empty Dashboard Chart 2"):
			nxenv.delete_doc("Dashboard Chart", "Test Empty Dashboard Chart 2")

		nxenv.db.delete("Error Log")

		# create one data point
		nxenv.get_doc(doctype="Error Log", creation="2018-06-01 00:00:00").insert()
	...
	def test_group_by_chart_type(self):
		if nxenv.db.exists("Dashboard Chart", "Test Group By Dashboard Chart"):
			nxenv.delete_doc("Dashboard Chart", "Test Group By Dashboard Chart")

		nxenv.get_doc({"doctype": "ToDo", "description": "test"}).insert()
		...
		todo_status_count = nxenv.db.count("ToDo", {"status": result.get("labels")[0]})
	...
	def test_daily_dashboard_chart(self):
		...
		if nxenv.db.exists("Dashboard Chart", "Test Daily Dashboard Chart"):
			nxenv.delete_doc("Dashboard Chart", "Test Daily Dashboard Chart")
	...
	def test_weekly_dashboard_chart(self):
		...
		if nxenv.db.exists("Dashboard Chart", "Test Weekly Dashboard Chart"):
			nxenv.delete_doc("Dashboard Chart", "Test Weekly Dashboard Chart")
	...
	def test_avg_dashboard_chart(self):
		...
		if nxenv.db.exists("Dashboard Chart", "Test Average Dashboard Chart"):
			nxenv.delete_doc("Dashboard Chart", "Test Average Dashboard Chart")
	...
	def test_user_date_label_dashboard_chart(self):
		nxenv.delete_doc_if_exists("Dashboard Chart", "Test Dashboard Chart Date Label")
		...
		with patch.object(nxenv.utils.data, "get_user_date_format", return_value="dd.mm.yyyy"):
		...
		with patch.object(nxenv.utils.data, "get_user_date_format", return_value="mm-dd-yyyy"):
			result = get(chart_name="Test Dashboard Chart Date Label")
...
def create_new_record(doctype_name, title, date, number):
	...
	if not nxenv.db.exists(doctype_name, {"title": doc.title}):
