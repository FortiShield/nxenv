...
import nxenv
from nxenv import _
from nxenv.boot import get_allowed_report_names
from nxenv.model.document import Document
from nxenv.model.naming import append_number_if_name_exists
from nxenv.modules.export_file import export_to_files
from nxenv.utils import cint, get_datetime, getdate, has_common, now_datetime, nowdate
from nxenv.utils.dashboard import cache_source
from nxenv.utils.data import format_date
from nxenv.utils.dateutils import (
...
def get_permission_query_conditions(user):
	...
	allowed_doctypes = [nxenv.db.escape(doctype) for doctype in nxenv.permissions.get_doctypes_with_read()]
	allowed_reports = [nxenv.db.escape(report) for report in get_allowed_report_names()]
	allowed_modules = [
		nxenv.db.escape(module.get("module_name")) for module in get_modules_from_all_apps_for_user()
	]
...
def has_permission(doc, ptype, user):
	...
	if doc.roles:
		...
		allowed_doctypes = nxenv.permissions.get_doctypes_with_read()
...
def get(
	...
	if chart_name:
		chart: DashboardChart = nxenv.get_doc("Dashboard Chart", chart_name)
	else:
		chart = nxenv._dict(nxenv.parse_json(chart))
	...
	filters = nxenv.parse_json(filters) or nxenv.parse_json(chart.filters_json)
	if not filters:
		filters = []
...
def create_dashboard_chart(args):
	...
	doc = nxenv.new_doc("Dashboard Chart")
	...
	if nxenv.db.exists("Dashboard Chart", args.chart_name):
...
def add_chart_to_dashboard(args):
	...
	dashboard = nxenv.get_doc("Dashboard", args.dashboard)
	dashboard_link = nxenv.new_doc("Dashboard Chart Link")
	dashboard_link.chart = args.chart_name or args.name

	if args.set_standard and dashboard.is_standard:
		chart = nxenv.get_doc("Dashboard Chart", dashboard_link.chart)
		chart.is_standard = 1
...
def get_group_by_chart_config(chart, filters) -> dict | None:
	...
	group_by_field_field = nxenv.get_meta(doctype).get_field(
		group_by_field
	)  # get info about @group_by_field

	if data and group_by_field_field.fieldtype == "Link":  # if @group_by_field is link
		title_field = nxenv.get_meta(group_by_field_field.options)  # get title field
		if title_field.title_field:  # if has title_field
			for item in data:  # replace chart labels from name to title value
				item.name = nxenv.get_value(group_by_field_field.options, item.name, title_field.title_field)
...
def get_charts_for_user(doctype, txt, searchfield, start, page_len, filters):
	or_filters = {"owner": nxenv.session.user, "is_public": 1}
	return nxenv.db.get_list(
...
class DashboardChart(Document):
	...
	if TYPE_CHECKING:
		...
		from nxenv.desk.doctype.dashboard_chart_field.dashboard_chart_field import DashboardChartField
	...
	def on_update(self):
		nxenv.cache.delete_key(f"chart-data:{self.name}")
	...
	def validate(self):
		if not nxenv.conf.developer_mode and self.is_standard:
			nxenv.throw(_("Cannot edit Standard charts"))
	...
	def check_required_field(self):
		if not self.document_type:
			nxenv.throw(_("Document type is required to create a dashboard chart"))

		if (
			self.document_type
			and nxenv.get_meta(self.document_type).istable
			and not self.parent_document_type
		):
			nxenv.throw(_("Parent document type is required to create a dashboard chart"))

		if self.chart_type == "Group By":
			if not self.group_by_based_on:
				nxenv.throw(_("Group By field is required to create a dashboard chart"))
			if self.group_by_type in ["Sum", "Average"] and not self.aggregate_function_based_on:
				nxenv.throw(_("Aggregate Function field is required to create a dashboard chart"))
		else:
			if not self.based_on:
				nxenv.throw(_("Time series based on is required to create a dashboard chart"))

	def check_document_type(self):
		if nxenv.get_meta(self.document_type).issingle:
			nxenv.throw(_("You cannot create a dashboard chart from single DocTypes"))
	...
	def validate_custom_options(self):
		if self.custom_options:
			try:
				...
				nxenv.throw(_("Invalid json added in the custom options: {0}").format(error))
