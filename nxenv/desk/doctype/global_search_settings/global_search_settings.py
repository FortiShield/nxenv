...
class GlobalSearchSettings(Document):
	...
	if TYPE_CHECKING:
		from nxenv.desk.doctype.global_search_doctype.global_search_doctype import GlobalSearchDocType
	...
	def validate(self):
		...
		for dt in self.allowed_in_global_search:
			...
			if nxenv.get_meta(dt.document_type).module == "Core":
				core_dts.append(dt.document_type)

			dts.append(dt.document_type)

		if core_dts:
			core_dts = ", ".join(nxenv.bold(dt) for dt in core_dts)
			nxenv.throw(_("Core Modules {0} cannot be searched in Global Search.").format(core_dts))

		if repeated_dts:
			repeated_dts = ", ".join([nxenv.bold(dt) for dt in repeated_dts])
			nxenv.throw(_("Document Type {0} has been repeated.").format(repeated_dts))

		# reset cache
		nxenv.cache.hdel("global_search", "search_priorities")


def get_doctypes_for_global_search():
	def get_from_db():
		doctypes = nxenv.get_all("Global Search DocType", fields=["document_type"], order_by="idx ASC")
		return [d.document_type for d in doctypes] or []

	return nxenv.cache.hget("global_search", "search_priorities", get_from_db)
...
def update_global_search_doctypes():
	...
	installed_apps = [app for app in nxenv.get_installed_apps() if app]
	active_domains = [domain for domain in nxenv.get_active_domains() if domain]
	active_domains.append("Default")

	for app in installed_apps:
		search_doctypes = nxenv.get_hooks(hook="global_search_doctypes", app_name=app)
	...
	doctype_list = {dt.name for dt in nxenv.get_all("DocType")}
	...
	global_search_settings = nxenv.get_single("Global Search Settings")
