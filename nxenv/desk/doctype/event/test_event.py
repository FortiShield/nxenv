...
class TestEvent(IntegrationTestCase):
	...
	def test_allowed_public(self):
		nxenv.set_user(self.test_user)
		doc = nxenv.get_doc("Event", nxenv.db.get_value("Event", {"subject": "_Test Event 1"}))
		self.assertTrue(nxenv.has_permission("Event", doc=doc))

	def test_not_allowed_private(self):
		nxenv.set_user(self.test_user)
		doc = nxenv.get_doc("Event", nxenv.db.get_value("Event", {"subject": "_Test Event 2"}))
		self.assertFalse(nxenv.has_permission("Event", doc=doc))

	def test_allowed_private_if_in_event_user(self):
		name = nxenv.db.get_value("Event", {"subject": "_Test Event 3"})
		nxenv.share.add("Event", name, self.test_user, "read")
		nxenv.set_user(self.test_user)
		doc = nxenv.get_doc("Event", name)
		self.assertTrue(nxenv.has_permission("Event", doc=doc))
		nxenv.set_user("Administrator")
		nxenv.share.remove("Event", name, self.test_user)
	...
	def test_revert_logic(self):
		ev = nxenv.get_doc(self.globalTestRecords["Event"][0]).insert()
		name = ev.name

		nxenv.delete_doc("Event", ev.name)
