...
nxenv.ui.form.on("Event", {
	...
	refresh: function (frm) {
		if (frm.doc.event_participants) {
			frm.doc.event_participants.forEach((value) => {
				frm.add_custom_button(
					...
					function () {
						nxenv.set_route("Form", value.reference_doctype, value.reference_docname);
					},
					__("Participants")
				);
			});
		...
		frm.add_custom_button(
			__("Add Contacts"),
			function () {
				new nxenv.desk.eventParticipants(frm, "Contact");
			},
			__("Add Participants")
		);
		...
		if (
			...
			nxenv.datetime.now_date() <= ends_on_date
		) {
...
nxenv.ui.form.on("Event Participants", {
	event_participants_remove: function (frm, cdt, cdn) {
		if (cdt && !cdn.includes("new-event-participants")) {
			nxenv.call({
				type: "POST",
				method: "nxenv.desk.doctype.event.event.delete_communication",
				args: {
					event: frm.doc,
...
nxenv.desk.eventParticipants = class eventParticipants {
