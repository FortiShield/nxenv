...
import nxenv
from nxenv import _
from nxenv.contacts.doctype.contact.contact import get_default_contact
...
from nxenv.utils.user import get_enabled_system_users
...
class Event(Document):
	...
	if TYPE_CHECKING:
		from nxenv.desk.doctype.event_participants.event_participants import EventParticipants
	...
	def validate(self):
		...
		if self.repeat_on == "Daily" and self.ends_on and getdate(self.starts_on) != getdate(self.ends_on):
			nxenv.throw(_("Daily Events should finish on the Same Day."))

		if self.sync_with_google_calendar and not self.google_calendar:
			nxenv.throw(_("Select Google Calendar to which event should be synced."))
	...
	def on_trash(self):
		...
		if communications:
			for communication in communications:
				nxenv.delete_doc_if_exists("Communication", communication.name, force=True)
	...
	def sync_communication(self):
		if self.event_participants:
			for participant in self.event_participants:
				...
				if comms := nxenv.get_all("Communication", filters=filters, fields=["name"], distinct=True):
					for comm in comms:
						communication = nxenv.get_doc("Communication", comm.name)
						self.update_communication(participant, communication)
				else:
					meta = nxenv.get_meta(participant.reference_doctype)
	...
	def update_communication(self, participant, communication):
		...
		communication.sender_full_name = nxenv.utils.get_fullname(self.owner)
		communication.reference_doctype = self.doctype
		communication.reference_name = self.name
	...
	def set_participants_email(self):
		for participant in self.event_participants:
			...
			participant.email = (
				nxenv.get_value("Contact", participant_contact, "email_id") if participant_contact else None
			)


@nxenv.whitelist()
def delete_communication(event, reference_doctype, reference_docname):
	deleted_participant = nxenv.get_doc(reference_doctype, reference_docname)
	...
	comms = nxenv.get_list("Communication", filters=filters, fields=["name"])

	if comms:
		deletion = []
		for comm in comms:
			delete = nxenv.get_doc("Communication", comm.name).delete()
...
def get_permission_query_conditions(user):
	...
	return f"""(`tabEvent`.`event_type`='Public' or `tabEvent`.`owner`={nxenv.db.escape(user)})"""
...
def send_event_digest():
	...
	for user in users:
		...
		if events:
			nxenv.set_user_lang(user.name, user.language)
			...
			nxenv.sendmail(
				...
				subject=nxenv._("Upcoming Events for Today"),
				template="upcoming_events",
				args={
					"events": events,
				},
				header=[nxenv._("Events in Today's Calendar"), "blue"],
			)


@nxenv.whitelist()
def get_events(start, end, user=None, for_reminder=False, filters=None) -> list[nxenv._dict]:
	if not user:
		user = nxenv.session.user
...
def delete_events(ref_type, ref_name, delete_event=False):
	...
	if participations:
		for participation in participations:
			if delete_event:
				nxenv.delete_doc("Event", participation.parent, for_reload=True)
			else:
				total_participants = nxenv.get_all(
					"Event Participants", filters={"parenttype": "Event", "parent": participation.parent}
				)

				if len(total_participants) <= 1:
					nxenv.db.delete("Event", {"name": participation.parent})
					nxenv.db.delete("Event Participants", {"name": participation.name})


# Close events if ends_on or repeat_till is less than now_datetime
def set_status_of_events():
	events = nxenv.get_list("Event", filters={"status": "Open"}, fields=["name", "ends_on", "repeat_till"])
	for event in events:
		if (event.ends_on and getdate(event.ends_on) < getdate(nowdate())) or (
			...
			nxenv.db.set_value("Event", event.name, "status", "Closed")
