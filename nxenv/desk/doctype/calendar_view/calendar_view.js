...
nxenv.ui.form.on("Calendar View", {
	onload: function (frm) {
	...
	refresh: function (frm) {
		if (!frm.is_new()) {
			frm.add_custom_button(__("Show Calendar"), () =>
				nxenv.set_route("List", frm.doc.reference_doctype, "Calendar", frm.doc.name)
			);
		}
	},
	reference_doctype: function (frm) {
		...
		nxenv.model.with_doctype(reference_doctype, () => {
			const meta = nxenv.get_meta(reference_doctype);

			const subject_options = meta.fields
				.filter((df) => !nxenv.model.no_value_type.includes(df.fieldtype))
				.map((df) => df.fieldname);
