...
from nxenv.query_builder import Criterion
from nxenv.query_builder.utils import DocType
from nxenv.utils import cint, flt
...
class NumberCard(Document):
	...
	def autoname(self):
		...
		if nxenv.db.exists("Number Card", self.name):
	...
	def validate(self):
		if self.type == "Document Type":
			if not (self.document_type and self.function):
				nxenv.throw(_("Document Type and Function are required to create a number card"))

			if self.function != "Count" and not self.aggregate_function_based_on:
				nxenv.throw(_("Aggregate Field is required to create a number card"))

			if nxenv.get_meta(self.document_type).istable and not self.parent_document_type:
				nxenv.throw(_("Parent Document Type is required to create a number card"))

		elif self.type == "Report":
			if not (self.report_name and self.report_field and self.function):
				nxenv.throw(_("Report Name, Report Field and Fucntion are required to create a number card"))

		elif self.type == "Custom":
			if not self.method:
				nxenv.throw(_("Method is required to create a number card"))
...
def has_permission(doc, ptype, user):
	...
	if doc.type == "Report":
		...
		allowed_doctypes = tuple(nxenv.permissions.get_doctypes_with_read())
...
def get_result(doc, filters, to_date=None):
	...
	if not filters:
		...
		filters = nxenv.parse_json(filters)
...
def get_percentage_difference(doc, filters, result):
	doc = nxenv.parse_json(doc)
	result = nxenv.parse_json(result)

	doc = nxenv.get_doc("Number Card", doc.name)
...
def calculate_previous_result(doc, filters):
	from nxenv.utils import add_to_date

	current_date = nxenv.utils.now()
...
def get_cards_for_user(doctype, txt, searchfield, start, page_len, filters):
	...
	condition_query = nxenv.qb.get_query(
		doctype,
		filters=filters,
	...
	return (
		...
		.where((numberCard.owner == nxenv.session.user) | (numberCard.is_public == 1))
...
def create_report_number_card(args):
	...
	args.name = card.name
	if args.dashboard:
		add_card_to_dashboard(nxenv.as_json(args))
...
def add_card_to_dashboard(args):
	...
	dashboard_link = nxenv.new_doc("Number Card Link")
	dashboard_link.card = args.name

	if args.set_standard and dashboard.is_standard:
		card = nxenv.get_doc("Number Card", dashboard_link.card)
		card.is_standard = 1
