...
import nxenv
from nxenv.core.doctype.scheduled_job_type.scheduled_job_type import ScheduledJobType
from nxenv.model.document import Document
from nxenv.utils.background_jobs import get_queue, get_queue_list, get_redis_conn
from nxenv.utils.caching import redis_cache
from nxenv.utils.data import add_to_date
from nxenv.utils.scheduler import get_scheduler_status, get_scheduler_tick
	...
	def suppress_exception(func: Callable):
		...
		def wrapper(*args, **kwargs):
			try:
				...
				nxenv.log(nxenv.get_traceback())
				# nosemgrep
				nxenv.msgprint(
					f"System Health check step {nxenv.bold(step)} failed: {e}", alert=True, indicator="red"
				)
...
class SystemHealthReport(Document):
	...
	if TYPE_CHECKING:
		from nxenv.desk.doctype.system_health_report_errors.system_health_report_errors import (
			SystemHealthReportErrors,
		)
		from nxenv.desk.doctype.system_health_report_failing_jobs.system_health_report_failing_jobs import (
			SystemHealthReportFailingJobs,
		)
		from nxenv.desk.doctype.system_health_report_queue.system_health_report_queue import (
			SystemHealthReportQueue,
		)
		from nxenv.desk.doctype.system_health_report_tables.system_health_report_tables import (
			SystemHealthReportTables,
		)
		from nxenv.desk.doctype.system_health_report_workers.system_health_report_workers import (
	...
	def fetch_background_jobs(self):
		...
		self.test_job_id = nxenv.enqueue("nxenv.ping", at_front=True).id
		self.background_jobs_check = "queued"
		self.scheduler_status = get_scheduler_status().get("status")
		workers = nxenv.get_all("RQ Worker")
	...
	def fetch_scheduler(self):
		...
		failing_jobs = nxenv.db.multisql(
			{
		...
		for job_type in nxenv.get_all(
			"Scheduled Job Type",
			filters={"stopped": 0, "last_execution": ("<", threshold)},
			fields="*",
			order_by="last_execution asc",
		):
			job_type: ScheduledJobType = nxenv.get_doc(doctype="Scheduled Job Type", **job_type)
	...
	def fetch_email_stats(self):
		...
		self.total_outgoing_emails = nxenv.db.count("Email Queue", filters)
		self.pending_emails = nxenv.db.count("Email Queue", {"status": "Not Sent", **filters})
		self.failed_emails = nxenv.db.count("Email Queue", {"status": "Error", **filters})
		self.unhandled_emails = nxenv.db.count("Unhandled Email", filters)
		self.handled_emails = nxenv.db.count(
			"Communication",
	...
	def fetch_errors(self):
		...
		filters = {"creation": (">", threshold), "modified": (">", threshold)}
		self.total_errors = nxenv.db.count("Error Log", filters)

		top_errors = nxenv.db.sql(
	...
	def fetch_database_details(self):
		...
		self.database = nxenv.db.db_type
		self.db_storage_usage = sum(table.size for table in data)
		for row in data[:5]:
			self.append("top_db_tables", row)
		self.database_version = nxenv.db.sql("select version()")[0][0]

		if nxenv.db.db_type == "mariadb":
			self.bufferpool_size = nxenv.db.sql("show variables like 'innodb_buffer_pool_size'")[0][1]
			self.binary_logging = nxenv.db.sql("show variables like 'log_bin'")[0][1]

	@health_check("Cache")
	def fetch_cache_details(self):
		self.cache_keys = len(nxenv.cache.get_keys(""))
		self.cache_memory_usage = nxenv.cache.execute_command("INFO", "MEMORY").get("used_memory_human")

	@health_check("Storage")
	def fetch_storage_details(self):
		from nxenv.desk.page.backups.backups import get_context
	...
	def fetch_user_stats(self):
		...
		self.total_users = nxenv.db.count("User", {"enabled": 1})
		self.new_users = nxenv.db.count("User", {"enabled": 1, "creation": (">", threshold)})
		self.failed_logins = nxenv.db.count(
			"Activity Log",
			{
				"operation": "login",
				"status": "Failed",
		...
		self.active_sessions = nxenv.db.count("Sessions")
...
def get_job_status(job_id: str | None = None):
	nxenv.only_for("System Manager")
	try:
		return nxenv.get_doc("RQ Job", job_id).status
	except Exception:
...
def _get_directory_size(*path):
	folder = os.path.abspath(nxenv.get_site_path(*path))
