...
nxenv.ui.form.on("System Health Report", {
	onload(frm) {
		let poll_attempts = 0;
		const interval = setInterval(() => {
			nxenv
				.xcall(
					"nxenv.desk.doctype.system_health_report.system_health_report.get_job_status",
	...
	refresh(frm) {
		...
		nxenv.realtime.on("pong", () => {
			frm.set_value("socketio_ping_check", "Pass");
			frm.set_value(
				"socketio_transport_mode",
				nxenv.realtime.socket.io?.engine?.transport?.name
			);
		});
		nxenv.realtime.emit("ping");
		frm.disable_save();
	...
	setup_highlight(frm) {
		...
		const update_fields = () => {
			if (!nxenv.get_route().includes(frm.doc.name)) {
				clearInterval(interval);
			}
