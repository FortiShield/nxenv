nxenv.provide("nxenv.setup");
nxenv.provide("nxenv.setup.events");
nxenv.provide("nxenv.ui");
...
nxenv.setup = {
	...
	on: function (event, fn) {
		if (!nxenv.setup.events[event]) {
			nxenv.setup.events[event] = [];
		}
		nxenv.setup.events[event].push(fn);
	},
	add_slide: function (slide) {
		nxenv.setup.slides.push(slide);
	},

	remove_slide: function (slide_name) {
		nxenv.setup.slides = nxenv.setup.slides.filter((slide) => slide.name !== slide_name);
	},

	run_event: function (event) {
		$.each(nxenv.setup.events[event] || [], function (i, fn) {
			fn();
		});
	},
};

nxenv.pages["setup-wizard"].on_page_load = function (wrapper) {
	if (nxenv.boot.setup_complete) {
		window.location.href = nxenv.boot.apps_data.default_path || "/app";
	}
	let requires = nxenv.boot.setup_wizard_requires || [];
	nxenv.require(requires, function () {
		nxenv.call({
			method: "nxenv.desk.page.setup_wizard.setup_wizard.load_languages",
			freeze: true,
			callback: function (r) {
				nxenv.setup.data.lang = r.message;

				nxenv.setup.run_event("before_load");
				var wizard_settings = {
					parent: wrapper,
					slides: nxenv.setup.slides,
					slide_class: nxenv.setup.SetupWizardSlide,
					unidirectional: 1,
					done_state: 1,
				};
				nxenv.wizard = new nxenv.setup.SetupWizard(wizard_settings);
				nxenv.setup.run_event("after_load");
				nxenv.wizard.show_slide(cint(nxenv.get_route()[1]));
			},
		});
	});
};

nxenv.pages["setup-wizard"].on_page_show = function () {
	nxenv.wizard && nxenv.wizard.show_slide(cint(nxenv.get_route()[1]));
};

nxenv.setup.on("before_load", function () {
	// load slides
	nxenv.setup.slides_settings.forEach((s) => {
		if (!(s.name === "user" && nxenv.boot.developer_mode)) {
			// if not user slide with developer mode
			nxenv.setup.add_slide(s);
		}
	});
});

nxenv.setup.SetupWizard = class SetupWizard extends nxenv.ui.Slides {
	constructor(args = {}) {
		super(args);
		$.extend(this, args);
		...
		this.welcomed = true;
		nxenv.set_route("setup-wizard/0");
	}
	...
	handle_enter_press(e) {
		if (e.which === nxenv.ui.keyCode.ENTER) {
			let $target = $(e.target);
	...
	before_show_slide() {
		if (!this.welcomed) {
			nxenv.set_route(this.page_name);
			return false;
		}
		return true;
	}
	...
	show_slide(id) {
		...
		super.show_slide(id);
		nxenv.set_route(this.page_name, cstr(id));
	}
	...
	refresh_slides() {
		...
		nxenv.setup.slides = this.get_setup_slides_filtered_by_domain();

		this.slides = nxenv.setup.slides;
		nxenv.setup.run_event("after_load");
	...
	action_on_complete() {
		nxenv.telemetry.capture("initated_client_side", "setup");
		...
		return nxenv.call({
			method: "nxenv.desk.page.setup_wizard.setup_wizard.setup_complete",
	...
	post_setup_success() {
		...
		if (nxenv.setup.welcome_page) {
			localStorage.setItem("session_last_route", nxenv.setup.welcome_page);
		}
		setTimeout(function () {
			// Reload
			window.location.href = nxenv.boot.apps_data.default_path || "/app";
	...
	abort_setup(fail_msg) {
		...
		fail_msg = fail_msg
			? fail_msg
			: nxenv.last_response.setup_wizard_failure_message
			? nxenv.last_response.setup_wizard_failure_message
	...
	listen_for_setup_stages() {
		nxenv.realtime.on("setup_task", (data) => {
	...
	get_setup_slides_filtered_by_domain() {
		let filtered_slides = [];
		nxenv.setup.slides.forEach(function (slide) {
			if (nxenv.setup.domains) {
				let active_domains = nxenv.setup.domains;
				if (
					!slide.domains ||
	...
	attach_abort_button() {
		...
		this.$abort_btn.on("click", () => {
			...
			this.container.show();
			nxenv.set_route(this.page_name, this.slides.length - 1);
		});
...
nxenv.setup.SetupWizardSlide = class SetupWizardSlide extends nxenv.ui.Slide {
	...
	set_init_values() {
		let me = this;
		// set values from nxenv.setup.values
		if (nxenv.wizard.values && this.fields) {
			this.fields.forEach(function (f) {
				var value = nxenv.wizard.values[f.fieldname];
				if (value) {
	...
	setup_telemetry_events() {
		let me = this;
		this.fields.filter(nxenv.model.is_value_type).forEach((field) => {
			field.fieldname &&
				me.get_input(field.fieldname)?.on?.("change", function () {
					nxenv.telemetry.capture(`${field.fieldname}_set`, "setup");
					if (
						field.fieldname == "enable_telemetry" &&
						!me.get_value("enable_telemetry")
					) {
						nxenv.telemetry.disable();
					}
				});
		});
	}
...
// Nxenv slides settings
// ======================================================
nxenv.setup.slides_settings = [
	{
		...
		fields: [
			...
			},
			{
				...
				fieldtype: "Check",
				default: cint(nxenv.telemetry.can_enable()),
				depends_on: "eval:nxenv.telemetry.can_enable()",
			},
		],

		onload: function (slide) {
			if (nxenv.setup.data.regional_data) {
				this.setup_fields(slide);
			} else {
				nxenv.setup.utils.load_regional_data(slide, this.setup_fields);
			}
			if (!slide.get_value("language")) {
				let session_language =
					nxenv.setup.utils.get_language_name_from_code(
						nxenv.boot.lang || navigator.language
					) || "English";
				let language_field = slide.get_field("language");

				language_field.set_input(session_language);
				if (!nxenv.setup._from_load_messages) {
					language_field.$input.trigger("change");
				}
				delete nxenv.setup._from_load_messages;
				moment.locale("en");
			}
			nxenv.setup.utils.bind_region_events(slide);
			nxenv.setup.utils.bind_language_events(slide);
		},

		setup_fields: function (slide) {
			nxenv.setup.utils.setup_region_fields(slide);
			nxenv.setup.utils.setup_language_field(slide);
		},
	},
	{
		// Profile slide
		...
		onload: function (slide) {
			if (nxenv.session.user !== "Administrator") {
				const { first_name, last_name, email } = nxenv.boot.user;
				...
				nxenv.setup.utils.load_user_details(slide, this.setup_fields);
			}
		},

		setup_fields: function (slide) {
			if (nxenv.setup.data.full_name) {
				slide.form.fields_dict.full_name.set_input(nxenv.setup.data.full_name);
			}
			if (nxenv.setup.data.email) {
				let email = nxenv.setup.data.email;
				slide.form.fields_dict.email.set_input(email);
			}
		},
	},
...
nxenv.setup.utils = {
	load_regional_data: function (slide, callback) {
		nxenv.call({
			method: "nxenv.geo.country_info.get_country_timezone_info",
			callback: function (data) {
				nxenv.setup.data.regional_data = data.message;
				callback(slide);
			},
		});
	},
	...
	load_user_details: function (slide, callback) {
		nxenv.call({
			method: "nxenv.desk.page.setup_wizard.setup_wizard.load_user_details",
			freeze: true,
			callback: function (r) {
				nxenv.setup.data.full_name = r.message.full_name;
				nxenv.setup.data.email = r.message.email;
				callback(slide);
			},
		});
	},
	...
	setup_language_field: function (slide) {
		...
		language_field.df.options = nxenv.setup.data.lang.languages;
	...
	setup_region_fields: function (slide) {
		...
		*/
		let data = nxenv.setup.data.regional_data;
		...
		slide
			...
			.empty()
			.add_options(
				nxenv.utils.unique($.map(data.country_info, (opts) => opts.currency).sort())
			);

		slide.get_input("timezone").empty().add_options(data.all_timezones);

		slide.get_field("currency").set_input(nxenv.wizard.values.currency);
		slide.get_field("timezone").set_input(nxenv.wizard.values.timezone);

		// set values if present
		let country =
			nxenv.wizard.values.country ||
			data.default_country ||
			guess_country(nxenv.setup.data.regional_data.country_info);
		...
		slide
			...
			.on("change", function () {
				...
				slide.language_call_timeout = setTimeout(() => {
					...
					nxenv._messages = {};
					nxenv.call({
						method: "nxenv.desk.page.setup_wizard.setup_wizard.load_messages",
						freeze: true,
						args: {
							language: lang,
						},
						callback: function () {
							nxenv.setup._from_load_messages = true;
							nxenv.wizard.refresh_slides();
						},
					});
				}, 500);
			});
	...
	get_language_name_from_code: function (language_code) {
		return nxenv.setup.data.lang.codes_to_names[language_code] || "English";
	},
	...
	bind_region_events: function (slide) {
		...
		*/
		slide.get_input("country").on("change", function () {
			...
			nxenv.boot.sysdefaults.date_format =
		...
		slide.get_input("currency").on("change", function () {
			...
			if (!currency) return;
			nxenv.model.with_doc("Currency", currency, function () {
				nxenv.provide("locals.:Currency." + currency);
				let currency_doc = nxenv.model.get_doc("Currency", currency);
				...
				nxenv.boot.sysdefaults.number_format = number_format;
