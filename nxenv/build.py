...
def build_missing_files():
	...
	for type in ["css", "js"]:
		folder = os.path.join(sites_path, "assets", "nxenv", "dist", type)
	...
	assets_json = nxenv.read_file("assets/assets.json")
	if assets_json:
		assets_json = nxenv.parse_json(assets_json)

		for bundle_file, output_file in assets_json.items():
			if not output_file.startswith("/assets/nxenv"):
				continue
		...
		if missing_assets:
			...
			files_to_build = ["nxenv/" + name for name in missing_assets]
			bundle(build_mode, files=files_to_build)
	else:
		# no assets.json, run full build
		bundle(build_mode, apps="nxenv")


def get_assets_link(nxenv_head) -> str:
	import requests

	tag = getoutput(
		r"cd ../apps/nxenv && git show-ref --tags -d | grep {} | sed -e 's,.*"
		r" refs/tags/,,' -e 's/\^{{}}//'".format(nxenv_head)
	)

	if tag:
		# if tag exists, download assets from github release
		url = f"https://github.com/nxenv/nxenv/releases/download/{tag}/assets.tar.gz"
	else:
		url = f"http://assets.nxenvframework.com/{nxenv_head}.tar.gz"

	if not requests.head(url):
		reference = f"Release {tag}" if tag else f"Commit {nxenv_head}"
		raise AssetsDontExistError(f"Assets for {reference} don't exist")

	return url


def fetch_assets(url, nxenv_head):
	click.secho("Retrieving assets...", fg="yellow")

	prefix = mkdtemp(prefix="nxenv-assets-", suffix=nxenv_head)
	assets_archive = download_file(url, prefix)

	if not assets_archive:
		raise AssetsNotDownloadedError(f"Assets could not be retrived from {url}")

	click.echo(click.style("✔", fg="green") + f" Downloaded Nxenv assets from {url}")
...
def setup_assets(assets_archive):
	...
	with tarfile.open(assets_archive) as tar:
		for file in tar:
			if not file.isdir():
				dest = "." + file.name.replace("./nxenv-nxcli/sites", "")
...
def download_nxenv_assets(verbose=True) -> bool:
	"""Download and set up Nxenv assets if they exist based on the current commit HEAD.
	Return True if correctly setup else return False.
	"""
	nxenv_head = getoutput("cd ../apps/nxenv && git rev-parse HEAD")

	if not nxenv_head:
		return False

	try:
		url = get_assets_link(nxenv_head)
		assets_archive = fetch_assets(url, nxenv_head)
		...
		# TODO: log traceback in nxcli.log
...
def symlink(target, link_name, overwrite=False):
	...
	while True:
		temp_link_name = f"tmp{nxenv.generate_hash()}"
...
def setup():
	...
	for app in nxenv.get_all_apps(True):
		try:
			pymodules.append(nxenv.get_module(app))
		except ImportError:
			pass
	app_paths = [os.path.dirname(pymodule.__file__) for pymodule in pymodules]
	assets_path = os.path.join(nxenv.local.sites_path, "assets")
...
def bundle(
	...
	nxenv_app_path = nxenv.get_app_source_path("nxenv")
	nxenv.commands.popen(command, cwd=nxenv_app_path, env=get_node_env(), raise_err=True)
...
def watch(apps=None):
	...
	live_reload = nxenv.utils.cint(os.environ.get("LIVE_RELOAD", nxenv.conf.live_reload))
	...
	check_node_executable()
	nxenv_app_path = nxenv.get_app_source_path("nxenv")
	nxenv.commands.popen(command, cwd=nxenv_app_path, env=get_node_env())
...
def generate_assets_map():
	...
	for app_name in nxenv.get_all_apps():
		app_doc_path = None

		pymodule = nxenv.get_module(app_name)
...
def html_to_js_template(path, content):
	"""Return HTML template content as Javascript code, by adding it to `nxenv.templates`."""
	return """nxenv.templates["{key}"] = '{content}';\n""".format(
