nxenv.pages["print"].on_page_load = function (wrapper) {
	nxenv.ui.make_app_page({
		parent: wrapper,
	});

	let print_view = new nxenv.ui.form.PrintView(wrapper);

	$(wrapper).bind("show", () => {
		const route = nxenv.get_route();
		const doctype = route[1];
		const docname = route.slice(2).join("/");
		if (!nxenv.route_options || !nxenv.route_options.frm) {
			nxenv.model.with_doc(doctype, docname, () => {
				let frm = { doctype: doctype, docname: docname };
				frm.doc = nxenv.get_doc(doctype, docname);
				nxenv.model.with_doctype(doctype, () => {
					frm.meta = nxenv.get_meta(route[1]);
					print_view.show(frm);
				});
			});
		} else {
			print_view.frm = nxenv.route_options.frm.doctype
				? nxenv.route_options.frm
				: nxenv.route_options.frm.frm;
			nxenv.route_options.frm = null;
			print_view.show(print_view.frm);
		}
	});
};

nxenv.ui.form.PrintView = class {
	constructor(wrapper) {
	...
	make() {
		this.print_wrapper = this.page.main.empty().html(
			`<div class="print-preview-wrapper"><div class="print-preview">
				${nxenv.render_template("print_skeleton_loading")}
		...
		this.print_settings = nxenv.model.get_doc(":Print Settings", "Print Settings");
		this.setup_menu();
		this.setup_toolbar();
		this.setup_sidebar();
	...
	add_sidebar_item(df, is_dynamic) {
		...
		let field = nxenv.ui.form.make_control({
			df: df,
	...
	setup_menu() {
		...
		if (nxenv.model.can_create("Print Format")) {
	...
	show(frm) {
		...
		// print designer link
		if (Object.keys(nxenv.boot.versions).includes("print_designer")) {
			...
			this.page.add_inner_message(`
			<a style="line-height: 2.4" href="https://nxenvcloud.com/marketplace/apps/print_designer?utm_source=framework-desk&utm_medium=print-view&utm_campaign=try-link">
				${__("Try the new Print Designer")}
			</a>
			`);
		}
	...
	set_breadcrumbs() {
		nxenv.breadcrumbs.add(this.frm.meta.module, this.frm.doctype);
	}
	...
	setup_additional_settings() {
		...
		nxenv
			.xcall("nxenv.printing.page.print.print.get_print_settings_to_show", {
	...
	edit_print_format() {
		...
		if (is_standard_but_editable) {
			nxenv.set_route("Form", "Print Format", print_format.name);
			return;
		}
		if (is_custom_format) {
			if (print_format.print_format_builder_beta) {
				nxenv.set_route("print-format-builder-beta", print_format.name);
			} else {
				nxenv.set_route("print-format-builder", print_format.name);
			}
			return;
		}
		...
		nxenv.prompt(
			...
			(data) => {
				...
				nxenv.set_route("print-format-builder");
	...
	setup_keyboard_shortcuts() {
		this.wrapper.find(".print-toolbar a.btn-default").each((i, el) => {
			nxenv.ui.keys.get_shortcut_group(this.frm.page).add($(el));
		});
	}
	...
	set_default_print_language() {
		let print_format = this.get_print_format();
		this.lang_code =
			this.frm.doc.language || print_format.default_print_language || nxenv.boot.lang;
	...
	preview() {
		...
		this.get_print_html((out) => {
			...
			const print_height_inches = nxenv.dom.pixel_to_inches(print_height);
	...
	setup_print_format_dom(out, $print_format) {
		...
		let base_url = nxenv.urllib.get_base_url();
		let print_css = nxenv.assets.bundled_asset(
			"print.bundle.css",
			nxenv.utils.is_rtl(this.lang_code)
		);
		this.$print_format_body
			.find("html")
			.attr("dir", nxenv.utils.is_rtl(this.lang_code) ? "rtl" : "ltr");
	...
	go_to_form_view() {
		...
		nxenv.set_route("Form", this.frm.doctype, this.frm.docname);
	}
			...
			if (me.is_raw_printing()) {
				me.get_raw_commands(function (out) {
					nxenv.ui.form
						...
						.then(nxenv.ui.form.qz_success)
						.catch((err) => {
							nxenv.ui.form.qz_fail(err);
						});
				});
			} else {
				nxenv.show_alert(
	...
	print_by_server() {
		...
		if (localStorage.getItem("network_printer")) {
			nxenv.call({
				method: "nxenv.utils.print_format.print_by_server",
				args: {
	...
	network_printer_setting_dialog(callback) {
		nxenv.call({
			method: "nxenv.printing.doctype.network_printer_settings.network_printer_settings.get_network_printer_settings",
			callback: function (r) {
				if (r.message) {
	...
	async is_wkhtmltopdf_valid() {
		const is_valid = await nxenv.xcall("nxenv.utils.pdf.is_wkhtmltopdf_valid");
	...
	render_pdf() {
		...
		if (print_format.print_format_builder_beta) {
			...
			let w = window.open(`/api/method/nxenv.utils.weasyprint.download_pdf?${params}`);
			if (!w) {
				nxenv.msgprint(__("Please enable pop-ups"));
				return;
			}
		} else {
			this.is_wkhtmltopdf_valid();
			this.render_page("/api/method/nxenv.utils.print_format.download_pdf?");
		}
	}
	...
	get_print_html(callback) {
		...
		this._req = nxenv.call({
			method: "nxenv.www.printview.get_html_and_style",
			args: {
				doc: this.frm.doc,
	...
	get_raw_commands(callback) {
		...
		nxenv.call({
			method: "nxenv.www.printview.get_rendered_raw_commands",
			args: {
				doc: this.frm.doc,
				print_format: this.selected_format(),
				_lang: this.lang_code,
	...
	set_style(style) {
		nxenv.dom.set_style(style || nxenv.boot.print_css, "print-style");
	}
	...
	printer_setting_dialog() {
		...
		this.printer_list = [];
		nxenv.ui.form.qz_get_printer_list().then((data) => {
						...
						fields: [
							{
								...
								fieldname: "print_format",
								default: 0,
								options: nxenv.meta.get_print_formats(this.frm.doctype),
								read_only: 0,
								in_list_view: 1,
								label: __("Print Format"),
							},
			...
			if (!(this.printer_list && this.printer_list.length)) {
				nxenv.throw(__("No Printer is Available."));
			}
		});
	}
};
