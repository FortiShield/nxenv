nxenv.pages["print-format-builder"].on_page_load = function (wrapper) {
	nxenv.print_format_builder = new nxenv.PrintFormatBuilder(wrapper);
	nxenv.breadcrumbs.add("Setup", "Print Format");
};

nxenv.pages["print-format-builder"].on_page_show = function (wrapper) {
	var route = nxenv.get_route();
	if (route.length > 1) {
		nxenv.model.with_doc("Print Format", route[1], function () {
			nxenv.print_format_builder.print_format = nxenv.get_doc("Print Format", route[1]);
			nxenv.print_format_builder.refresh();
		});
	} else if (nxenv.route_options) {
		if (nxenv.route_options.make_new) {
			let { doctype, name, based_on, beta } = nxenv.route_options;
			nxenv.route_options = null;
			nxenv.print_format_builder.setup_new_print_format(doctype, name, based_on, beta);
		} else {
			nxenv.print_format_builder.print_format = nxenv.route_options.doc;
			nxenv.route_options = null;
			nxenv.print_format_builder.refresh();
		}
	}
...
nxenv.PrintFormatBuilder = class PrintFormatBuilder {
	constructor(parent) {
		this.parent = parent;
		this.make();
		this.refresh();
	...
	make() {
		this.page = nxenv.ui.make_app_page({
			parent: this.parent,
	...
	show_start() {
		this.page.main.html(nxenv.render_template("print_format_builder_start", {}));
	...
	start_edit_print_format() {
		...
		var me = this;
		this.print_format_input = nxenv.ui.form.make_control({
		...
		this.page.main.find(".btn-edit-print-format").on("click", function () {
			...
			if (!name) return;
			nxenv.model.with_doc("Print Format", name, function (doc) {
				nxenv.set_route("print-format-builder", name);
			});
		});
	}
	start_new_print_format() {
		var me = this;
		this.doctype_input = nxenv.ui.form.make_control({
		...
		this.name_input = nxenv.ui.form.make_control({
		...
		this.page.main.find(".btn-new-print-format").on("click", function () {
			...
			if (!(doctype && name)) {
				nxenv.msgprint(__("Both DocType and Name required"));
				return;
			}
	...
	setup_new_print_format(doctype, name, based_on, beta) {
		nxenv.call({
			method: "nxenv.printing.page.print_format_builder.print_format_builder.create_custom_format",
			args: {
				doctype: doctype,
				name: name,
				based_on: based_on,
	...
	setup_print_format() {
		var me = this;
		nxenv.model.with_doctype(this.print_format.doc_type, function (doctype) {
			me.meta = nxenv.get_meta(me.print_format.doc_type);
			me.setup_sidebar();
			me.render_layout();
			...
			me.page.add_inner_button(__("Edit Properties"), function () {
				nxenv.set_route("Form", "Print Format", me.print_format.name);
			});
		});
	}
	setup_sidebar() {
		...
		this.page.sidebar.html(
			$(nxenv.render_template("print_format_builder_sidebar", { fields: fields }))
		);
	...
	render_layout() {
		...
		this.prepare_data();
		$(
			nxenv.render_template("print_format_builder_layout", {
		...
		$.each(this.data, function (i, f) {
			...
			if (!f.name && f.fieldname) {
				...
				if (f.fieldname === "_custom_html") {
					...
					f = $.extend(
						nxenv.meta.get_docfield(me.print_format.doc_type, f.fieldname) || {},
						f
					);
				}
			}
			...
			if (f.fieldtype === "Section Break") {
				...
				set_column();
			} else if (!nxenv.model.layout_fields.includes(f.fieldtype)) {
	...
	init_visible_columns(f) {
		f.visible_columns = [];
		$.each(nxenv.get_meta(f.options).fields, function (i, _f) {
			if (
			...
			onAdd: function (evt) {
				...
				if (!$item.hasClass("print-format-builder-field")) {
					...
					if (fieldname === "_custom_html") {
						...
						field = nxenv.meta.get_docfield(me.print_format.doc_type, fieldname);
					}

					var html = nxenv.render_template("print_format_builder_field", {
						field: field,
						me: me,
					});
	...
	setup_add_section() {
		...
		this.page.main.find(".print-format-builder-add-section").on("click", function () {
			...
			var $section = $(
				nxenv.render_template("print_format_builder_section", {
					section: section,
					me: me,
				})
	...
	setup_column_selector() {
		...
		this.page.main.on("click", ".select-columns", function () {
			...
			var doc_fields = nxenv.get_meta(doctype).fields;
			...
			// render checkboxes
			$(
				nxenv.render_template("print_format_builder_column_selector", {
					fields: fields,
	...
	save_print_format() {
		...
		nxenv.call({
			...
			callback: function (r) {
				...
				nxenv.show_alert({ message: __("Saved"), indicator: "green" });
			},
		});
	}
};
