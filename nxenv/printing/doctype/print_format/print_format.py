...
from nxenv.utils.weasyprint import download_pdf, get_html
...
class PrintFormat(Document):
	...
	def validate(self):
		if (
			...
			and not nxenv.flags.in_install
			and not nxenv.flags.in_test
		):
			nxenv.throw(nxenv._("Standard Print Format cannot be updated"))

		# old_doc_type is required for clearing item cache
		self.old_doc_type = nxenv.db.get_value("Print Format", self.name, "doc_type")
		...
		if self.custom_format and self.raw_printing and not self.raw_commands:
			nxenv.throw(_("{0} are required").format(nxenv.bold(_("Raw Commands"))), nxenv.MandatoryError)

		if self.custom_format and not self.html and not self.raw_printing:
			nxenv.throw(_("{0} is required").format(nxenv.bold(_("HTML"))), nxenv.MandatoryError)
	...
	def on_update(self):
		if hasattr(self, "old_doc_type") and self.old_doc_type:
			nxenv.clear_cache(doctype=self.old_doc_type)
		if self.doc_type:
...
def make_default(name):
	...
	nxenv.has_permission("Print Format", "write")

	print_format = nxenv.get_doc("Print Format", name)

	doctype = nxenv.get_doc("DocType", print_format.doc_type)
	if doctype.custom:
	...
	nxenv.msgprint(
		nxenv._("{0} is now default print format for {1} doctype").format(
			nxenv.bold(name), nxenv.bold(print_format.doc_type)
		)
	)
