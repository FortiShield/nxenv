...
def setup_database(force, verbose=None, mariadb_user_host_login_scope=None):
	...
	if nxenv.conf.db_type == "postgres":
		import nxenv.database.postgres.setup_db

		return nxenv.database.postgres.setup_db.setup_database()
	else:
		import nxenv.database.mariadb.setup_db

		return nxenv.database.mariadb.setup_db.setup_database(force, verbose, mariadb_user_host_login_scope)
...
def bootstrap_database(verbose=None, source_sql=None):
	...
	if nxenv.conf.db_type == "postgres":
		...
		return nxenv.database.postgres.setup_db.bootstrap_database(verbose, source_sql)
	else:
		import nxenv.database.mariadb.setup_db

		return nxenv.database.mariadb.setup_db.bootstrap_database(verbose, source_sql)
...
def drop_user_and_database(db_name, db_user):
	...
	if nxenv.conf.db_type == "postgres":
		...
		return nxenv.database.postgres.setup_db.drop_user_and_database(db_name, db_user)
	else:
		import nxenv.database.mariadb.setup_db

		return nxenv.database.mariadb.setup_db.drop_user_and_database(db_name, db_user)
...
def get_db(socket=None, host=None, user=None, password=None, port=None, cur_db_name=None):
	...
	if nxenv.conf.db_type == "postgres":
		import nxenv.database.postgres.database

		return nxenv.database.postgres.database.PostgresDatabase(
			socket, host, user, password, port, cur_db_name
		)
	else:
		import nxenv.database.mariadb.database

		return nxenv.database.mariadb.database.MariaDBDatabase(
			socket, host, user, password, port, cur_db_name
		)
