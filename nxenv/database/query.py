...
import nxenv
from nxenv import _
from nxenv.database.operator_map import OPERATOR_MAP
from nxenv.database.schema import SPECIAL_CHAR_PATTERN
from nxenv.database.utils import DefaultOrderBy, FilterValue, convert_to_value, get_doctype_name
from nxenv.query_builder import Criterion, Field, Order, functions
from nxenv.query_builder.functions import Function, SqlFunctions
from nxenv.query_builder.utils import PseudoColumnMapper
from nxenv.utils.data import MARIADB_SPECIFIC_COMMENT
...
# less restrictive version of nxenv.core.doctype.doctype.doctype.START_WITH_LETTERS_PATTERN
...
class Engine:
	def get_query(
		...
		*,
		...
		self.is_mariadb = nxenv.db.db_type == "mariadb"
		self.is_postgres = nxenv.db.db_type == "postgres"
		...
		if isinstance(table, Table):
			...
			self.table = nxenv.qb.DocType(table)

		if update:
			self.query = nxenv.qb.update(self.table)
		elif into:
			self.query = nxenv.qb.into(self.table)
		elif delete:
			self.query = nxenv.qb.from_(self.table).delete()
		else:
			self.query = nxenv.qb.from_(self.table)
	...
	def validate_doctype(self):
		if not TABLE_NAME_PATTERN.match(self.doctype):
			nxenv.throw(_("Invalid DocType: {0}").format(self.doctype))
	...
	def _apply_filter(
		...
		if not isinstance(_field, str):
			...
			nxenv.throw(_("Invalid filter: {0}").format(_field))
		elif not doctype or doctype == self.doctype:
			_field = self.table[field]
		elif doctype:
			_field = nxenv.qb.DocType(doctype)[field]
		...
		if _operator in OPERATOR_MAP["nested_set"]:
			...
			_df = nxenv.get_meta(self.doctype).get_field(field)
...
class Permission:
	...
	def check_permissions(cls, query, **kwargs):
		...
		for dt in doctype:
			...
			) and not nxenv.has_permission(
				dt,
				"read",
				user=kwargs.get("user"),
				parent_doctype=kwargs.get("parent_doctype"),
			):
				nxenv.throw(_("Insufficient Permission for {0}").format(nxenv.bold(dt)))
	...
	def parse(field: str, doctype: str):
		if "." in field:
			...
			if field.startswith("`tab") or field.startswith('"tab'):
				...
				linked_field = nxenv.get_meta(doctype).get_field(linked_fieldname)
				linked_doctype = linked_field.options
				if linked_field.fieldtype == "Link":
					return LinkTableField(linked_doctype, fieldname, doctype, linked_fieldname, alias=alias)
				elif linked_field.fieldtype in nxenv.model.table_fields:
...
class ChildTableField(DynamicTableField):
	def __init__(
		...
		self.alias = alias
		self.parent_doctype = parent_doctype
		self.table = nxenv.qb.DocType(self.doctype)
		self.field = self.table[self.fieldname]

	def apply_select(self, query: QueryBuilder) -> QueryBuilder:
		table = nxenv.qb.DocType(self.doctype)
	...
	def apply_join(self, query: QueryBuilder) -> QueryBuilder:
		table = nxenv.qb.DocType(self.doctype)
		main_table = nxenv.qb.DocType(self.parent_doctype)
...
class ChildQuery:
	def __init__(
		...
		field = nxenv.get_meta(parent_doctype).get_field(fieldname)
		if field.fieldtype not in nxenv.model.table_fields:
			return
		self.fieldname = fieldname
		self.fields = fields
		self.parent_doctype = parent_doctype
...
def get_nested_set_hierarchy_result(doctype: str, name: str, hierarchy: str) -> list[str]:
	...
	table = nxenv.qb.DocType(doctype)
	try:
		lft, rgt = nxenv.qb.from_(table).select("lft", "rgt").where(table.name == name).run()[0]
	except IndexError:
		lft, rgt = None, None
