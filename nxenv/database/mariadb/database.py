...
from nxenv.database.mariadb.schema import MariaDBTable
from nxenv.utils import UnicodeWithAttrs, cstr, get_datetime, get_table_name
...
class MariaDBConnectionUtil:
	...
	def get_connection_settings(self) -> dict:
		...
		if nxenv.conf.local_infile:
			conn_settings["local_infile"] = nxenv.conf.local_infile

		if nxenv.conf.db_ssl_ca and nxenv.conf.db_ssl_cert and nxenv.conf.db_ssl_key:
			conn_settings["ssl"] = {
				"ca": nxenv.conf.db_ssl_ca,
				"cert": nxenv.conf.db_ssl_cert,
				"key": nxenv.conf.db_ssl_key,
			}
...
class MariaDBDatabase(MariaDBConnectionUtil, MariaDBExceptionUtil, Database):
	...
	def escape(s, percent=True):
		...
		# Also, given we're promoting use of the PyPika builder via nxenv.qb, the use
		# of this method should be limited.

		# pymysql expects unicode argument to escape_string with Python 3
		s = nxenv.as_unicode(escape_string(nxenv.as_unicode(s)), "utf-8").replace("`", "\\`")
	...
	def get_column_index(self, table_name: str, fieldname: str, unique: bool = False) -> nxenv._dict | None:
	...
	def get_tables(self, cached=True):
		...
		if cached:
			tables = nxenv.cache.get_value("db_tables")
			to_query = not tables
		...
		if to_query:
			...
			tables = (
				...
				.where(information_schema.tables.table_schema == nxenv.db.cur_db_name)
				.run(pluck=True)
			)
			nxenv.cache.set_value("db_tables", tables)
