import nxenv
from nxenv import _
from nxenv.database.schema import DBTable, get_definition
...
class PostgresTable(DBTable):
	def create(self):
		varchar_len = nxenv.db.VARCHAR_LEN
		...
		if not self.meta.issingle and self.meta.autoname == "autoincrement":
			nxenv.db.create_sequence(self.doctype, check_not_exists=True)
	...
	def create_indexes(self):
		...
		if create_index_query:
			# nosemgrep
			nxenv.db.sql(create_index_query)
	...
	def alter(self):
		...
		for col in self.set_default:
			...
			if col.fieldtype in ("Check", "Int"):
				...
				col_default = f"{nxenv.db.escape(col.default)}"
		...
		try:
			if query:
				...
				# nosemgrep
				nxenv.db.sql(final_alter_query)
			if change_nullability:
				# nosemgrep
				nxenv.db.sql(f"ALTER TABLE `{self.table_name}` {','.join(change_nullability)}")
			if create_contraint_query:
				# nosemgrep
				nxenv.db.sql(create_contraint_query)
			if drop_contraint_query:
				# nosemgrep
				nxenv.db.sql(drop_contraint_query)
		except Exception as e:
			# sanitize
			if nxenv.db.is_duplicate_fieldname(e):
				nxenv.throw(str(e))
			elif nxenv.db.is_duplicate_entry(e):
	...
	def alter_primary_key(self) -> str | None:
		...
		if autoname == "UUID" and nxenv.db.get_column_type(self.doctype, "name") != "uuid":
			if not nxenv.db.get_value(self.doctype, {}, order_by=None):
		...
		if autoname != "UUID" and nxenv.db.get_column_type(self.doctype, "name") == "uuid":
			return f"alter column `name` TYPE varchar({nxenv.db.VARCHAR_LEN})"
