...
import nxenv
from nxenv import _
from nxenv.utils import cint, cstr, flt
...
class DBTable:
	def __init__(self, doctype, meta=None):
		...
		self.meta = meta or nxenv.get_meta(doctype, False)
	...
	def sync(self):
		...
		if self.is_new():
			...
			nxenv.client_cache.delete_value(f"table_columns::{self.table_name}")
			self.alter()
	...
	def get_column_definitions(self):
		column_list = [*nxenv.db.DEFAULT_COLUMNS]
		ret = []
	...
	def get_index_definitions(self):
		return [
			...
			if (
				col.set_index
				and not col.unique
				and col.fieldtype in nxenv.db.type_map
				and nxenv.db.type_map.get(col.fieldtype)[0] not in ("text", "longtext")
			)
		]
	...
	def get_columns_from_docfields(self):
		...
		if not self.meta.get("istable"):
			for fieldname in nxenv.db.OPTIONAL_COLUMNS:
	...
	def validate(self):
		...
		columns = [
			nxenv._dict({"fieldname": f, "fieldtype": "Data"}) for f in nxenv.db.STANDARD_VARCHAR_COLUMNS
		]
		if self.meta.get("istable"):
			columns += [
				nxenv._dict({"fieldname": f, "fieldtype": "Data"}) for f in nxenv.db.CHILD_TABLE_COLUMNS
			]
		...
		for col in columns:
			if len(col.fieldname) >= 64:
				nxenv.throw(
					_("Fieldname is limited to 64 characters ({0})").format(nxenv.bold(col.fieldname))
				)

			if "varchar" in nxenv.db.type_map.get(col.fieldtype, ()):
				# validate length range
				new_length = cint(col.length) or cint(nxenv.db.VARCHAR_LEN)
				if not (1 <= new_length <= 1000):
					nxenv.throw(_("Length of {0} should be between 1 and 1000").format(col.fieldname))
				...
				if cint(current_length) != cint(new_length):
					try:
						...
						max_length = nxenv.db.sql(
							f"""SELECT MAX(CHAR_LENGTH(`{col.fieldname}`)) FROM `tab{self.doctype}`"""
						)

					except nxenv.db.InternalError as e:
						if nxenv.db.is_missing_column(e):
					...
					if max_length and max_length[0][0] and max_length[0][0] > new_length:
						...
						nxenv.msgprint(info_message)

	def is_new(self):
		return self.table_name not in nxenv.db.get_tables()

	def setup_table_columns(self):
		# TODO: figure out a way to get key data
		for c in nxenv.db.get_table_columns_description(self.table_name):
...
class DbColumn:
	...
	def get_definition(self, for_modification=False):
		...
		if self.fieldtype in ("Check", "Int"):
			...
			self.default
			and (self.default not in nxenv.db.DEFAULT_SHORTCUTS)
			and not cstr(self.default).startswith(":")
		):
			default = nxenv.db.escape(self.default)

		if self.not_nullable and null:
			if default is None:
				default = get_not_null_defaults(self.fieldtype)
				if isinstance(default, str):
					default = nxenv.db.escape(default)
			null = False
	...
	if special_characters := SPECIAL_CHAR_PATTERN.findall(n):
		...
		nxenv.throw(
			_("Fieldname {0} cannot have special characters like {1}").format(
				nxenv.bold(cstr(n)), special_characters
			),
			nxenv.db.InvalidColumnName,
		)
	return n


def validate_column_length(fieldname):
	if len(fieldname) > nxenv.db.MAX_COLUMN_LENGTH:
		nxenv.throw(_("Fieldname is limited to 64 characters ({0})").format(fieldname))


def get_definition(fieldtype, precision=None, length=None, *, options=None):
	d = nxenv.db.type_map.get(fieldtype)
	...
	if (
		...
		and nxenv.db.exists("DocType", options)
		and nxenv.get_meta(options).autoname == "UUID"
	):
		d = ("uuid", None)
	...
	if fieldtype == "Int" and length and length > 11:
		...
		d = nxenv.db.type_map.get("Long Int")
	...
	if size:
		...
		if length:
			if coltype == "varchar":
				...
				# NOTE: this will only be applicable for mariadb as nxenv implements int
