...
def add_docshare(
	...
	if share_name:
		doc = nxenv.get_doc("DocShare", share_name)
	else:
		doc = nxenv.new_doc("DocShare")
	...
	if nxenv.get_cached_value("User", user, "follow_shared_documents"):
...
def remove(doctype, name, user, flags=None):
	share_name = nxenv.db.get_value("DocShare", {"user": user, "share_name": name, "share_doctype": doctype})

	if share_name:
		nxenv.delete_doc("DocShare", share_name, flags=flags)
...
def set_docshare_permission(doctype, name, user, permission_to, value=1, everyone=0, flags=None):
	...
	if not share_name:
		...
		share = nxenv.get_doc("DocShare", share_name)
		if flags:
...
def get_shared_doctypes(user=None):
	...
	if not user:
		user = nxenv.session.user
	table = nxenv.qb.DocType("DocShare")
	query = (
...
def check_share_permission(doctype, name):
	...
	if not nxenv.has_permission(doctype, ptype="share", doc=name):
		nxenv.throw(
			_("No permission to {0} {1} {2}").format("share", _(doctype), name), nxenv.PermissionError
		)
...
def notify_assignment(shared_by, doctype, doc_name, everyone, notify=0):
	...
	reference_user = get_fullname(nxenv.session.user)
	notification_message = _("{0} shared a document {1} {2} with you").format(
		nxenv.bold(reference_user), nxenv.bold(_(doctype)), get_title_html(title)
	)
