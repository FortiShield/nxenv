# Copyright (c) 2013, Nxenv and contributors
# License: MIT. See LICENSE

import nxenv
from nxenv import _
from nxenv.rate_limiter import rate_limit
from nxenv.utils import cint, is_markdown, markdown
from nxenv.website.utils import get_comment_list
...
class HelpArticle(WebsiteGenerator):
	...
	def set_route(self):
		...
		if not self.route:
			self.route = "/".join(
				[nxenv.get_value("Help Category", self.category, "route"), self.scrub(self.title)]
			)
	...
	def update_category(self):
		cnt = nxenv.db.count("Help Article", filters={"category": self.category, "published": 1})
		cat = nxenv.get_doc("Help Category", self.category)
	...
	def get_context(self, context):
		...
		context.category = nxenv.get_doc("Help Category", self.category)
...
def get_list_context(context=None):
	...
	category = nxenv.db.get_value("Help Category", {"route": nxenv.local.path})
	...
	list_context = nxenv._dict(
		...
		hide_filters=True,
		filters=filters,
		category=nxenv.local.form_dict.category,
		no_breadcrumbs=True,
	)

	if nxenv.local.form_dict.txt:
		list_context.blog_subtitle = _('Filtered by "{0}"').format(nxenv.local.form_dict.txt)
	#
	# list_context.update(nxenv.get_doc("Blog Settings", "Blog Settings").as_dict())
	return list_context
...
def get_sidebar_items():
	...
	return nxenv.cache.get_value("knowledge_base:category_sidebar", _get)


def clear_knowledge_base_cache():
	nxenv.cache.delete_value("knowledge_base:category_sidebar")
	nxenv.cache.delete_value("knowledge_base:faq")
...
def add_feedback(article: str, helpful: str):
	...
	value = cint(nxenv.db.get_value("Help Article", article, field))
	nxenv.db.set_value("Help Article", article, field, value + 1, update_modified=False)
