...
class WebsiteTheme(Document):
	...
	if TYPE_CHECKING:
		...
		from nxenv.website.doctype.website_theme_ignore_app.website_theme_ignore_app import (
	...
	def on_update(self):
		if (
			not self.custom
			and nxenv.local.conf.get("developer_mode")
	...
	def is_standard_and_not_valid_user(self):
		return (
			...
			and not (nxenv.flags.in_import or nxenv.flags.in_test or nxenv.flags.in_migrate)
		)

	def on_trash(self):
		if self.is_standard_and_not_valid_user():
			nxenv.throw(_("You are not allowed to delete a standard Website Theme"), nxenv.PermissionError)

	def validate_if_customizable(self):
		if self.is_standard_and_not_valid_user():
			nxenv.throw(_("Please Duplicate this Website Theme to customize."))
	...
	def clear_cache_if_current_theme(self):
		if nxenv.flags.in_install == "nxenv":
			return
		website_settings = nxenv.get_doc("Website Settings", "Website Settings")
	...
	def generate_bootstrap_theme(self):
		...
		folder_path = abspath(nxenv.utils.get_files_path("website_theme", is_private=False))
		# create folder if not exist
		nxenv.create_folder(folder_path)

		if self.custom:
			self.delete_old_theme_files(folder_path)

		# add a random suffix
		suffix = nxenv.generate_hash(length=8) if self.custom else "style"
		file_name = nxenv.scrub(self.name) + "_" + suffix + ".css"
		...
		process = Popen(command, cwd=nxenv.get_app_source_path("nxenv"), stdout=PIPE, stderr=PIPE)

		stderr = process.communicate()[1]

		if stderr:
			stderr = nxenv.safe_decode(stderr)
			stderr = stderr.replace("\n", "<br>")
			nxenv.throw(f'<div style="font-family: monospace;">{stderr}</div>')
		else:
			self.theme_url = "/files/website_theme/" + file_name

		nxenv.msgprint(_("Compiled Successfully"), alert=True)
	...
	def delete_old_theme_files(self, folder_path):
		...
		for fname in os.listdir(folder_path):
			if fname.startswith(nxenv.scrub(self.name) + "_") and fname.endswith(".css"):
...
def get_active_theme() -> Optional["WebsiteTheme"]:
	if website_theme := nxenv.get_website_settings("website_theme"):
		try:
			return nxenv.client_cache.get_doc("Website Theme", website_theme)
...
def get_scss(website_theme):
	...
	return nxenv.render_template("nxenv/website/doctype/website_theme/website_theme_template.scss", context)
...
def get_scss_paths():
	...
	If `$NXCLI_PATH/apps/nxenv/nxenv/public/scss/website[.bundle].scss` exists, the
	returned set will contain 'nxenv/public/scss/website[.bundle]'.
	"""
	import_path_list = []
	...
	for app in nxenv.get_installed_apps():
		for scss_file in scss_files:
			full_path = nxenv.get_app_path(app, scss_file)
...
def after_migrate():
	...
	the end of every `nxcli migrate`.
	"""
	website_theme = nxenv.db.get_single_value("Website Settings", "website_theme")
	if not website_theme or website_theme == "Standard":
		return

	doc = nxenv.get_doc("Website Theme", website_theme)
