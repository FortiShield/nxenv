...
from nxenv.website.doctype.web_form.web_form import accept
...
class TestWebForm(IntegrationTestCase):
	def setUp(self):
		nxenv.conf.disable_website_cache = True

	def tearDown(self):
		nxenv.conf.disable_website_cache = False
	...
	def test_accept(self):
		...
		self.event_name = nxenv.db.get_value("Event", {"subject": "_Test Event Web Form"})
	...
	def test_edit(self):
		...
		self.assertNotEqual(
			nxenv.db.get_value("Event", self.event_name, "description"), doc.get("description")
		)

		accept("manage-events", json.dumps(doc))

		self.assertEqual(nxenv.db.get_value("Event", self.event_name, "description"), doc.get("description"))
	...
	def test_webform_html_meta_is_added(self):
		...
		self.assertIn(
			self.normalize_html('<meta property="og:image" content="https://nxenv.io/files/nxenv.png">'),
			content,
		)
