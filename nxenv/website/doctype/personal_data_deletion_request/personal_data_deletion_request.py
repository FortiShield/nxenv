...
from nxenv.desk.doctype.notification_settings.notification_settings import is_email_notifications_enabled
from nxenv.model.document import Document
from nxenv.utils import get_datetime, get_fullname, time_diff_in_hours
...
class PersonalDataDeletionRequest(Document):
	...
	if TYPE_CHECKING:
		...
		from nxenv.website.doctype.personal_data_deletion_step.personal_data_deletion_step import (
	...
	def __init__(self, *args, **kwargs):
		...
		self.user_data_fields = nxenv.get_hooks("user_data_fields")
	...
	def autoname(self):
		from nxenv.model.naming import set_name_from_naming_options
		...
		domain = nxenv.local.site.replace("_", "-")
		...
		nxenv.utils.validate_email_address(self.email, throw=True)

	def after_insert(self):
		self.send_verification_mail()

	def generate_url_for_confirmation(self):
		params = {"email": self.email, "name": self.name, "host_name": nxenv.local.site}
		api = nxenv.utils.get_url(
			"/api/method/nxenv.website.doctype.personal_data_deletion_request"
	...
	def disable_user(self):
		user = nxenv.get_doc("User", self.email)
		user.enabled = False
		user.save()
	...
	def notify_system_managers(self):
		...
		nxenv.sendmail(
			...
			args={"user": self.email, "url": nxenv.utils.get_url(self.get_url())},
	...
	def validate_data_anonymization(self):
		...
		if self.status != "Pending Approval":
			nxenv.throw(_("This request has not yet been approved by the user."))
	...
	def generate_anonymization_dict(self, ref):
		...
		meta = nxenv.get_meta(ref["doctype"])
	...
	def redact_doc(self, doc, ref):
		...
		if ref.get("rename") and doc["name"] != self.anon:
			nxenv.rename_doc(ref["doctype"], doc["name"], self.anon, force=True, show_alert=False)
	...
	def _anonymize_data(self, email=None, anon=None, set_data=True, commit=False):
		...
		nxenv.rename_doc("User", email, anon, force=True, show_alert=False)
	...
	def __redact_partial_match_data(self, doctype):
		...
		for df in nxenv.get_meta(doctype["doctype"]).fields:
...
def process_data_deletion_request():
	auto_account_deletion = nxenv.db.get_single_value("Website Settings", "auto_account_deletion")
	if auto_account_deletion < 1:
		return

	requests = nxenv.get_all(
		"Personal Data Deletion Request", filters={"status": "Pending Approval"}, pluck="name"
	)

	for request in requests:
		doc = nxenv.get_doc("Personal Data Deletion Request", request)
...
def confirm_deletion(email, name, host_name):
	...
	doc = nxenv.get_doc("Personal Data Deletion Request", name)
	host_name = nxenv.utils.get_url()
