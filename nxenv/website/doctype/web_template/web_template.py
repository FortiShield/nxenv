...
from nxenv.modules.export_file import get_module_path, scrub_dt_dn, write_document_file
...
class WebTemplate(Document):
	...
	if TYPE_CHECKING:
		...
		from nxenv.website.doctype.web_template_field.web_template_field import WebTemplateField
	...
	def validate(self):
		if self.standard and not nxenv.conf.developer_mode and not nxenv.flags.in_patch:
			nxenv.throw(_("Enable developer mode to create a standard Web Template"))

		for field in self.fields:
			if not field.fieldname:
				field.fieldname = nxenv.scrub(field.label)
	...
	def on_trash(self):
		if nxenv.conf.developer_mode and self.standard:
	...
	def get_template_path(self):
		...
		file_name = nxenv.scrub(self.name) + ".html"
	...
	def render(self, values=None):
		if not values:
			values = {}
		values = nxenv.parse_json(values)
		values.update({"values": values})
		template = self.get_template(self.standard)

		return nxenv.render_template(template, values)
