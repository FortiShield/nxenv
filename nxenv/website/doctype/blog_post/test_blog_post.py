...
from nxenv.utils import random_string, set_request
from nxenv.website.doctype.blog_post.blog_post import get_blog_list
...
class TestBlogPost(IntegrationTestCase):
	...
	def test_generator_not_found(self):
		pages = nxenv.get_all("Blog Post", fields=["name", "route"], filters={"published": 0}, limit=1)

		route = f"test-route-{nxenv.generate_hash(length=5)}"

		nxenv.db.set_value("Blog Post", pages[0].name, "route", route)
	...
	def test_category_link(self):
		...
		blog_page_html = nxenv.safe_decode(blog_page_response.get_data())
		...
		cached_value = nxenv.db.value_cache.get(("DocType", "Blog Post", "name"))
		nxenv.db.value_cache[("DocType", "Blog Post", "name")] = (("Blog Post",),)
		...
		category_page_html = nxenv.safe_decode(category_page_response.get_data())
		# Category page should contain the blog post title
		self.assertIn(blog.title, category_page_html)

		# Cleanup
		nxenv.db.value_cache[("DocType", "Blog Post", "name")] = cached_value
		nxenv.delete_doc("Blog Post", blog.name)
		nxenv.delete_doc("Blog Category", blog.blog_category)
	...
	def test_blog_pagination(self):
		...
		filters = nxenv._dict({"blog_category": scrub(category_title)})
		...
		for blog in blogs:
			nxenv.delete_doc(blog.doctype, blog.name)
		nxenv.delete_doc("Blog Category", blogs[0].blog_category)

	def test_caching(self):
		# to enable caching
		nxenv.flags.force_website_cache = True
		print(nxenv.session.user)
	...
	def test_like_dislike(self):
		...
		nxenv.db.delete("Comment", {"comment_type": "Like", "reference_doctype": "Blog Post"})

		from nxenv.templates.includes.likes.likes import like
...
def make_test_blog(category_title="Test Blog Category"):
	...
	if not nxenv.db.exists("Blog Category", category_name):
		nxenv.get_doc(doctype="Blog Category", title=category_title).insert()
	if not nxenv.db.exists("Blogger", "test-blogger"):
		nxenv.get_doc(doctype="Blogger", short_name="test-blogger", full_name="Test Blogger").insert()
