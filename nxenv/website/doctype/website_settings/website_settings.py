...
from nxenv.utils import cint, encode, get_request_site_address
from nxenv.website.utils import get_boot_data
...
class WebsiteSettings(Document):
	...
	if TYPE_CHECKING:
		...
		from nxenv.website.doctype.top_bar_item.top_bar_item import TopBarItem
		from nxenv.website.doctype.website_route_redirect.website_route_redirect import (
	...
	def validate_google_settings(self):
		if self.enable_google_indexing and not nxenv.db.get_single_value("Google Settings", "enable"):
		...
		for idx, row in enumerate(self.route_redirects):
			try:
				...
				if not nxenv.flags.in_migrate:
					nxenv.throw(_("Invalid redirect regex in row #{}: {}").format(idx, str(e)))
	...
	def get_access_token(self):
		...
		if not self.indexing_refresh_token:
			button_label = nxenv.bold(_("Allow API Indexing Access"))
...
def get_website_settings(context=None):
	...
	context = nxenv._dict(context or {})
	settings: WebsiteSettings = nxenv.client_cache.get_doc("Website Settings")
	...
	if context.disable_website_theme:
		context.theme = nxenv._dict()

	else:
		from nxenv.website.doctype.website_theme.website_theme import get_active_theme

		context.theme = get_active_theme() or nxenv._dict()

	if not context.get("favicon"):
		context["favicon"] = "/assets/nxenv/images/nxenv-favicon.svg"
	...
	context.read_only_mode = nxenv.flags.read_only
...
def get_auto_account_deletion():
	return nxenv.db.get_single_value("Website Settings", "auto_account_deletion")
