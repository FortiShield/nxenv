...
class PortalSettings(Document):
	...
	if TYPE_CHECKING:
		...
		from nxenv.website.doctype.portal_menu_item.portal_menu_item import PortalMenuItem
	...
	def sync_menu(self):
		...
		dirty = False
		for item in nxenv.get_hooks("standard_portal_menu_items"):
			if item.get("role") and not nxenv.db.exists("Role", item.get("role")):
				nxenv.get_doc({"doctype": "Role", "role_name": item.get("role"), "desk_access": 0}).insert()
	...
	def remove_deleted_doctype_items(self):
		existing_doctypes = set(nxenv.get_list("DocType", pluck="name"))
	...
	def validate(self):
		if nxenv.request and self.default_portal_home:
