...
nxenv.provide("website");
nxenv.provide("nxenv.awesome_bar_path");
window.cur_frm = null;
...
$.extend(nxenv, {
	...
	require: async function (links, callback) {
		...
		links = links.map((link) => nxenv.bundled_asset(link));
	...
	bundled_asset(path, is_rtl = null) {
		if (!path.startsWith("/assets") && path.includes(".bundle.")) {
			...
			path = nxenv.boot.assets_json[path] || path;
			return path;
		}
		return path;
	},
	add_asset_to_head(link) {
		return new Promise((resolve) => {
			if (nxenv._assets_loaded.includes(link)) return resolve();
			let el;
			...
			el.onload = () => {
				nxenv._assets_loaded.push(link);
				resolve();
			};
		});
	},
	...
	call: function (opts) {
		...
		return $.ajax({
			...
			statusCode: opts.statusCode || {
				404: function () {
					nxenv.msgprint(__("Not found"));
				},
				403: function () {
					nxenv.msgprint(__("Not permitted"));
				},
				200: function (data) {
			...
			nxenv.process_response(opts, data);
		});
	},
	...
	process_response: function (opts, data) {
		...
		if (data._server_messages) {
			...
			if (opts.error_msg) {
				...
				nxenv.msgprint(server_messages);
			}
		}
	...
	has_permission: function (doctype, docname, perm_type, callback) {
		return nxenv.call({
			type: "GET",
			method: "nxenv.client.has_permission",
			no_spinner: true,
	...
	render_user: function () {
		if (nxenv.is_user_logged_in()) {
			$(".btn-login-area").toggle(false);
			$(".logged-in").toggle(true);
			$(".user-image").attr("src", nxenv.get_cookie("user_image"));

			$(".user-image-wrapper").html(
				nxenv.avatar(null, "avatar-medium", null, null, null, true)
			);
			$(".user-image-sidebar").html(
				nxenv.avatar(null, "avatar-medium", null, null, null, true)
			);
			$(".user-image-myaccount").html(
				nxenv.avatar(null, "avatar-large", null, null, null, true)
			);
		}
	},
	freeze_count: 0,
	...
	unfreeze: function () {
		if (!nxenv.freeze_count) return; // anything open?
		nxenv.freeze_count--;
		if (!nxenv.freeze_count) {
	...
	trigger_ready: function () {
		nxenv.ready_events.forEach(function (fn) {
			fn();
		});
	},
	...
	bind_filters: function () {
		...
		$(".filter").each(function () {
			...
			var val = nxenv.utils.get_url_arg(key).replace(/\+/g, " ");
	...
	bind_navbar_search: function () {
		nxenv.get_navbar_search().on("keypress", function (e) {
			var val = $(this).val();
			if (e.which === 13 && val) {
				$(this).val("").blur();
				nxenv.do_search(val);
	...
	do_search: function (val) {
		var path =
			(nxenv.awesome_bar_path && nxenv.awesome_bar_path[location.pathname]) ||
			window.search_path ||
			location.pathname;
	...
	set_search_path: function (path) {
		nxenv.awesome_bar_path[location.pathname] = path;
	},
	...
	is_user_logged_in: function () {
		return nxenv.get_cookie("user_id") !== "Guest" && nxenv.session.user !== "Guest";
	},
	...
	show_language_picker() {
		if (nxenv.session.user === "Guest" && window.show_language_picker) {
			nxenv
				.call("nxenv.translate.get_all_languages", {
					with_language_name: true,
				})
				.then((res) => {
					let language_list = res.message;
					let language = nxenv.get_cookie("preferred_language");
					let language_codes = [];
...
nxenv.setup_search = function (target, search_scope) {
	...
	$input.on(
		...
		nxenv.utils.debounce(() => {
			...
			nxenv
				.call({
					method: "nxenv.search.web_search",
					args: {
...
window.valid_email = function (id) {
	// copied regex from nxenv/utils.js validate_type
...
$(document).ready(function () {
	window.full_name = nxenv.get_cookie("full_name");
	var logged_in = nxenv.is_user_logged_in();
	$("#website-login").toggleClass("hide", logged_in ? true : false);
	$("#website-post-login").toggleClass("hide", logged_in ? false : true);
	$(".logged-in").toggleClass("hide", logged_in ? false : true);

	nxenv.bind_navbar_search();

	// add apps link
	let apps = nxenv.boot?.apps_data?.apps;
	let obj = {
		label: __("Apps"),
		route: "/apps",
	};
	if (apps?.length) {
		...
		let is_desk_apps = nxenv.boot?.apps_data?.is_desk_apps;
		!is_desk_apps && nxenv.add_apps(obj);
	}

	// switch to app link
	if (nxenv.get_cookie("system_user") === "yes" && logged_in) {
		nxenv.add_switch_to_desk();
	}
...
$(document).on("page-change", function () {
	...
	$.extend(nxenv, nxenv.get_cookies());
	nxenv.session = { user: nxenv.user_id };

	nxenv.datetime.refresh_when();
	nxenv.trigger_ready();
	nxenv.bind_filters();
	nxenv.highlight_code_blocks();
	nxenv.add_link_to_headings();
	nxenv.make_navbar_active();
	// scroll to hash
...
nxenv.ready(function () {
	nxenv.show_language_picker();
	nxenv.setup_videos();
	nxenv.realtime.init(window.socketio_port, true); // lazy connection
});
