...
def delete_page_cache(path=None):
	if path:
		nxenv.cache.delete_value(f"{WEBSITE_PAGE_CACHE_PREFIX}{path}")
	else:
		nxenv.cache.delete_keys(WEBSITE_PAGE_CACHE_PREFIX)
...
def can_cache(no_cache=False):
	if nxenv.flags.force_website_cache:
		return True
	if nxenv.conf.disable_website_cache or nxenv.conf.developer_mode:
		return False
	if getattr(nxenv.local, "no_cache", False):
		return False
	return not no_cache


def get_comment_list(doctype, name):
	comments = nxenv.get_all(
		"Comment",
		...
		or_filters=[["owner", "=", nxenv.session.user], ["published", "=", 1]],
	)

	communications = nxenv.get_all(
		"Communication",
		fields=[
			"name",
			"creation",
		...
		or_filters=[
			["recipients", "like", f"%{nxenv.session.user}%"],
			["cc", "like", f"%{nxenv.session.user}%"],
			["bcc", "like", f"%{nxenv.session.user}%"],
		],
	)
...
def get_home_page():
	if nxenv.local.flags.home_page and not nxenv.flags.in_test:
		return nxenv.local.flags.home_page
	...
	def _get_home_page():
		...
		if nxenv.session.user != "Guest":
			# by role
			for role in nxenv.get_roles():
				home_page = nxenv.db.get_value("Role", role, "home_page")
				if home_page:
					break

			# portal default
			if not home_page:
				home_page = nxenv.db.get_single_value("Portal Settings", "default_portal_home")
		...
		# global
		if not home_page:
			home_page = nxenv.db.get_single_value("Website Settings", "home_page")

		if not home_page:
			home_page = "login" if nxenv.session.user == "Guest" else "me"
	...
	return nxenv.cache.hget("home_page", nxenv.session.user, _get_home_page)


def get_home_page_via_hooks():
	home_page = None

	home_page_method = nxenv.get_hooks("get_website_user_home_page")
	if home_page_method:
		home_page = nxenv.get_attr(home_page_method[-1])(nxenv.session.user)
	elif nxenv.get_hooks("website_user_home_page"):
		home_page = nxenv.get_hooks("website_user_home_page")[-1]

	if not home_page:
		role_home_page = nxenv.get_hooks("role_home_page")
		if role_home_page:
			for role in nxenv.get_roles():
	...
	if not home_page:
		home_page = nxenv.get_hooks("home_page")
		if home_page:
...
def get_boot_data():
	...
	return {
		...
		"sysdefaults": {
			"float_precision": cint(nxenv.get_system_settings("float_precision")) or 3,
			"date_format": get_date_format(),
			"time_format": get_time_format(),
			"first_day_of_the_week": get_first_day_of_the_week(),
			"number_format": get_number_format().string,
			"currency": nxenv.get_system_settings("currency"),
		},
		"time_zone": {
			"system": get_system_timezone(),
			"user": nxenv.get_cached_value("User", nxenv.session.user, "time_zone")
			or get_system_timezone(),
		},
		"assets_json": get_assets_json(),
		"sitename": nxenv.local.site,
	}


def is_signup_disabled():
	return nxenv.get_website_settings("disable_signup")
...
def get_toc(route, url_prefix=None, app=None):
	...
	return nxenv.get_template("templates/includes/full_index.html").render(
...
def get_full_index(route=None, app=None):
	...
	from nxenv.website.router import get_pages

	if not nxenv.local.flags.children_map:
		...
		children_map = nxenv.cache.get_value("website_full_index", _build)

		nxenv.local.flags.children_map = children_map

	return nxenv.local.flags.children_map
...
def clear_cache(path=None):
	...
	if path:
		nxenv.cache.hdel("website_redirects", path)
		delete_page_cache(path)
	else:
		clear_sitemap()
		nxenv.clear_cache("Guest")
		delete_page_cache()
		keys += [
			"portal_menu_items",
			"home_page",
	...
	nxenv.cache.delete_value(keys)

	for method in nxenv.get_hooks("website_clear_cache"):
		nxenv.get_attr(method)(path)
...
def get_sidebar_items(parent_sidebar, basepath=None):
	...
	hooks = nxenv.get_hooks("look_for_sidebar_json")
	look_for_sidebar_json = hooks[0] if hooks else nxenv.flags.look_for_sidebar
...
def get_portal_sidebar_items():
	sidebar_items = nxenv.cache.hget("portal_menu_items", nxenv.session.user)
	if sidebar_items is None:
		sidebar_items = []
		roles = nxenv.get_roles()
		portal_settings = nxenv.get_doc("Portal Settings", "Portal Settings")
		...
		items_via_hooks = nxenv.get_hooks("portal_menu_items")
		if items_via_hooks:
			for i in items_via_hooks:
				i["enabled"] = 1
			add_items(sidebar_items, items_via_hooks)

		nxenv.cache.hset("portal_menu_items", nxenv.session.user, sidebar_items)
...
def get_sidebar_items_from_sidebar_file(basepath, look_for_sidebar_json):
	...
	with open(sidebar_json_path) as sidebarfile:
		try:
			...
			nxenv.throw("Invalid Sidebar JSON at " + sidebar_json_path)
...
def cache_html(func):
	...
	def cache_html_decorator(*args, **kwargs):
		...
		if can_cache():
			html = None
			page_cache = nxenv.cache.get_value(cache_key)
			if page_cache and nxenv.local.lang in page_cache:
				html = page_cache[nxenv.local.lang]
			if html:
				nxenv.local.response.from_cache = True
				nxenv.local.response.can_cache = True
				return html
		html = func(*args, **kwargs)
		context = args[0].context
		if can_cache(context.no_cache):
			page_cache = nxenv.cache.get_value(cache_key) or {}
			page_cache[nxenv.local.lang] = html
			nxenv.cache.set_value(cache_key, page_cache, expires_in_sec=30 * 60)
			nxenv.local.response.can_cache = True
...
def build_response(path, data, http_status_code, headers: dict | None = None):
	...
	response.headers["X-From-Cache"] = nxenv.local.response.from_cache or False
...
def add_preload_for_bundled_assets(response):
	links = [f"<{css}>; rel=preload; as=style" for css in nxenv.local.preload_assets["style"]]
	links.extend(f"<{js}>; rel=preload; as=script" for js in nxenv.local.preload_assets["script"])

	version = get_build_version()
	links.extend(
		f"<{svg}?v={version}>; rel=preload; as=fetch; crossorigin"
		for svg in nxenv.local.preload_assets["icons"]
	)
