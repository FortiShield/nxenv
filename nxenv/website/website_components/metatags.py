...
class MetaTags:
	def __init__(self, path, context):
		...
		self.tags = nxenv._dict(self.context.get("metatags") or {})
	...
	def init_metatags_from_context(self):
		...
		if self.tags.get("image"):
			self.tags["image"] = nxenv.utils.get_url(self.tags["image"])

		self.tags["language"] = nxenv.local.lang or "en"
	...
	def set_metatags_from_website_route_meta(self):
		...
		if route == "":
			# homepage
			route = nxenv.get_website_settings("home_page")

		route_exists = route and not route.endswith((".js", ".css")) and has_meta_tags(route)

		if route_exists:
			website_route_meta = nxenv.get_doc("Website Route Meta", route)
...
def has_meta_tags(route):
	return bool(nxenv.db.exists("Website Route Meta", route))
