...
from nxenv.website.utils import can_cache
...
class NotFoundPage(TemplatePage):
	def __init__(self, path, http_status_code=None):
		...
		self.request_url = nxenv.local.request.url if hasattr(nxenv.local, "request") else ""
		path = "404"
	...
	def render(self):
		if self.can_cache_404():
			nxenv.cache.hset("website_404", self.request_url, True)
		return super().render()
