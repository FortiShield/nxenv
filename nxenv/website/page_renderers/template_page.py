...
import nxenv
from nxenv.website.page_renderers.base_template_page import BaseTemplatePage
from nxenv.website.router import get_base_template, get_page_info
...
class TemplatePage(BaseTemplatePage):
	...
	def set_template_path(self):
		...
		for app in reversed(nxenv.get_installed_apps()):
	...
	def get_index_path_options(search_path):
		return (
			nxenv.as_unicode(f"{search_path}{d}") for d in ("", ".html", ".md", "/index.html", "/index.md")
		)
	...
	def update_context(self):
		...
		self.context.build_version = nxenv.utils.get_build_version()

		if self.pymodule_name:
			self.pymodule = nxenv.get_module(self.pymodule_name)
		...
		self.context.developer_mode = nxenv.conf.developer_mode
	...
	def get_raw_template(self):
		return nxenv.get_jloader().get_source(nxenv.get_jenv(), self.context.template)[0]
	...
	def convert_from_markdown(self):
		if self.template_path.endswith(".md"):
			self.source = nxenv.utils.md_to_html(self.source)
	...
	def set_standard_path(self, path):
		self.app = "nxenv"
		self.app_path = nxenv.get_app_path("nxenv")
		self.path = path
	...
	def set_user_info(self):
		from nxenv.utils.user import get_fullname_and_avatar

		info = get_fullname_and_avatar(nxenv.session.user)
