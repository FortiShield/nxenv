...
def pass_context(f):
	...
	def _func(ctx, *args, **kwargs):
		...
		try:
			...
			nxenv.exceptions.SiteNotSpecifiedError,
			nxenv.exceptions.IncorrectSitePath,
			nxenv.exceptions.CommandFailedError,
		) as e:
...
def get_site(context, raise_err=True):
	try:
		...
		if raise_err:
			raise nxenv.SiteNotSpecifiedError
		return None
...
def get_commands():
	...
	clickable_link = "https://nxenvframework.com/docs"
	all_commands = (
		scheduler_commands
		+ site_commands
		+ testing_commands
