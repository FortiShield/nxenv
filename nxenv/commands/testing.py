...
if TYPE_CHECKING:
	...
	from nxenv.testing import TestRunner
...
def main(
	...
	from nxenv.testing.environment import _cleanup_after_tests, _initialize_test_environment

	if debug and not debug_exceptions:
		debug_exceptions = (Exception,)

	testing_module_logger = logging.getLogger("nxenv.testing")
	...
	try:
		...
		if doctype or doctype_list_path:
			...
			apps = [app] if app else nxenv.get_installed_apps()
...
def _load_doctype_list(doctype_list_path):
	...
	with open(nxenv.get_app_path(app, path)) as f:
...
def _run_module_def_tests(app, module_def, runner: "TestRunner", force) -> "TestRunner":
	...
	from nxenv.testing import discover_doctype_tests
...
def _get_doctypes_for_module_def(app, module_def):
	...
	doctypes = []
	doctypes_ = nxenv.get_list(
		"DocType",
		filters={"module": module_def, "istable": 0},
		fields=["name", "module"],
		as_list=True,
	)
	from nxenv.modules import get_module_name
...
def run_tests(
	...
	with CodeCoverage(coverage, app):
		...
		nxenv.init(site)
		allow_tests = nxenv.get_conf().allow_tests

		if not (allow_tests or os.environ.get("CI")):
			click.secho("Testing is disabled for the site!", bold=True)
			click.secho("You can enable tests by entering following command:")
			click.secho(f"nxcli --site {site} set-config allow_tests true", fg="green")
			return
...
@click.option("--app", help="For App", default="nxenv")
...
def run_parallel_tests(
	...
	with CodeCoverage(with_coverage, app) as cc:
		site = get_site(context)
		if use_orchestrator:
			from nxenv.parallel_test_runner import ParallelTestWithOrchestrator

			runner = ParallelTestWithOrchestrator(app, site=site)
		else:
			from nxenv.parallel_test_runner import ParallelTestRunner
...
def run_ui_tests(
	...
	site = get_site(context)
	nxenv.init(site)
	app_base_path = nxenv.get_app_source_path(app)
	site_url = nxenv.utils.get_site_url(site)
	admin_password = nxenv.get_conf().admin_password
	...
	node_bin = subprocess.getoutput("(cd ../nxenv && yarn bin)")
	...
	if not (
		...
		nxenv.commands.popen(f"(cd ../nxenv && yarn add {packages} --no-lockfile)")
	...
	try:
		nxenv.commands.popen(formatted_command, cwd=app_base_path, raise_err=True)
