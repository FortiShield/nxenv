...
nxenv.provide("nxenv.auto_repeat");

nxenv.ui.form.on("Auto Repeat", {
	setup: function (frm) {
		frm.fields_dict["reference_doctype"].get_query = function () {
			return {
				query: "nxenv.automation.doctype.auto_repeat.auto_repeat.get_auto_repeat_doctypes",
			};
		};
	...
	refresh: function (frm) {
		...
		if (!frm.is_dirty()) {
			...
			frm.add_custom_button(label, () =>
				nxenv.set_route("List", frm.doc.reference_doctype, { auto_repeat: frm.doc.name })
			);
		}

		// auto repeat schedule
		nxenv.auto_repeat.render_schedule(frm);
	...
	toggle_submit_on_creation: function (frm) {
		...
		if (frm.doc.reference_doctype) {
			nxenv.model.with_doctype(frm.doc.reference_doctype, () => {
				let meta = nxenv.get_meta(frm.doc.reference_doctype);
	...
	template: function (frm) {
		if (frm.doc.template) {
			nxenv.model.with_doc("Email Template", frm.doc.template, () => {
				let email_template = nxenv.get_doc("Email Template", frm.doc.template);
	...
	preview_message: function (frm) {
		if (frm.doc.message) {
			nxenv.call({
				method: "nxenv.automation.doctype.auto_repeat.auto_repeat.generate_message_preview",
				args: {
				...
				callback: function (r) {
					if (r.message) {
						nxenv.msgprint(r.message.message, r.message.subject);
					}
				},
			});
		} else {
			nxenv.msgprint(__("Please setup a message first"), __("Message not setup"));
		}
	},
});

nxenv.auto_repeat.render_schedule = function (frm) {
	if (!frm.is_dirty() && frm.doc.status !== "Disabled") {
		frm.call("get_auto_repeat_schedule").then((r) => {
			frm.dashboard.reset();
			frm.dashboard.add_section(
				nxenv.render_template("auto_repeat_schedule", {
