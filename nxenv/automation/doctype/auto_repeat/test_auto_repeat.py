...
import nxenv
from nxenv.automation.doctype.auto_repeat.auto_repeat import (
...
from nxenv.utils import add_days, add_months, getdate, today

if TYPE_CHECKING:
	from nxenv.custom.doctype.custom_field.custom_field import CustomField
...
def add_custom_fields() -> "CustomField":
	...
	return create_custom_field("ToDo", df) or nxenv.get_doc(
...
class TestAutoRepeat(IntegrationTestCase):
	...
	def test_daily_auto_repeat(self):
		...
		todo = nxenv.get_doc(doc.reference_doctype, doc.reference_document)
		self.assertEqual(todo.auto_repeat, doc.name)

		new_todo = nxenv.db.get_value("ToDo", {"auto_repeat": doc.name, "name": ("!=", todo.name)}, "name")

		new_todo = nxenv.get_doc("ToDo", new_todo)
	...
	def monthly_auto_repeat(self, doctype, docname, start_date, end_date=None):
		...
		data = get_auto_repeat_entries(getdate(today()))
		create_repeated_entries(data)
		docnames = nxenv.get_all(doc.reference_doctype, {"auto_repeat": doc.name})
		self.assertEqual(len(docnames), 1)

		doc = nxenv.get_doc("Auto Repeat", doc.name)
	...
	def test_email_notification(self):
		...
		email_queue = nxenv.db.exists("Email Queue", dict(reference_doctype="ToDo", reference_name=new_todo))
	...
	def test_submit_on_creation(self):
		...
		submittable_doc = nxenv.get_doc(doctype=doctype, test="test submit on creation").insert()
	...
	return nxenv.get_doc(
		{
			...
			"reference_document": args.reference_document or nxenv.db.get_value("ToDo", "name"),
...
def create_submittable_doctype(doctype, submit_perms=1):
	if nxenv.db.exists("DocType", doctype):
		return
	else:
		doc = nxenv.get_doc(
			{
