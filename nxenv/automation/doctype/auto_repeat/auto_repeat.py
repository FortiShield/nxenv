...
import nxenv
from nxenv import _
from nxenv.automation.doctype.assignment_rule.assignment_rule import get_repeated
from nxenv.contacts.doctype.contact.contact import (
	get_contacts_linked_from,
	get_contacts_linking_to,
)
from nxenv.core.doctype.communication.email import make
...
from nxenv.utils.background_jobs import get_jobs
...
class AutoRepeat(Document):
	...
	if TYPE_CHECKING:
		from nxenv.automation.doctype.auto_repeat_day.auto_repeat_day import AutoRepeatDay
	...
	def on_update(self):
		nxenv.get_doc(self.reference_doctype, self.reference_document).notify_update()

	def on_trash(self):
		nxenv.db.set_value(self.reference_doctype, self.reference_document, "auto_repeat", "")
		nxenv.get_doc(self.reference_doctype, self.reference_document).notify_update()
	...
	def validate_reference_doctype(self):
		if nxenv.flags.in_test or nxenv.flags.in_patch:
			return
		if not nxenv.get_meta(self.reference_doctype).allow_auto_repeat:
			nxenv.throw(
	...
	def validate_submit_on_creation(self):
		if self.submit_on_creation and not nxenv.get_meta(self.reference_doctype).is_submittable:
			nxenv.throw(
				_("Cannot enable {0} for a non-submittable doctype").format(
					nxenv.bold(_("Submit on Creation"))
				)
			)
		...
		if self.end_date == self.start_date:
			nxenv.throw(
				_("{0} should not be same as {1}").format(
					nxenv.bold(_("End Date")), nxenv.bold(_("Start Date"))
				)
			)
			...
			if self.recipients:
				...
				for email in email_list:
					if not validate_email_address(email):
						nxenv.throw(_("{0} is an invalid email address in 'Recipients'").format(email))
			else:
				nxenv.throw(_("'Recipients' not specified"))
		...
		if len(set(auto_repeat_days)) != len(auto_repeat_days):
			...
			nxenv.throw(
				_("Auto Repeat Day{0} {1} has been repeated.").format(
					plural, nxenv.bold(", ".join(repeated_days))
				)
			)

	def update_auto_repeat_id(self):
		# check if document is already on auto repeat
		auto_repeat = nxenv.db.get_value(self.reference_doctype, self.reference_document, "auto_repeat")
		if auto_repeat and auto_repeat != self.name and not nxenv.flags.in_patch:
			nxenv.throw(
				_("The {0} is already on auto repeat {1}").format(self.reference_document, auto_repeat)
			)
		else:
			nxenv.db.set_value(self.reference_doctype, self.reference_document, "auto_repeat", self.name)
	...
	def create_documents(self):
		try:
			...
			if self.reference_document and not nxenv.flags.in_test:
				self.notify_error_to_user(error_log)

	def make_new_document(self):
		reference_doc = nxenv.get_doc(self.reference_doctype, self.reference_document)
		new_doc = nxenv.copy_doc(reference_doc, ignore_no_copy=False)
	...
	def update_doc(self, new_doc, reference_doc):
		...
		auto_repeat_doc = nxenv.get_doc("Auto Repeat", self.name)
	...
	def send_notification(self, new_doc):
		...
		if not self.subject:
			...
			subject = nxenv.render_template(self.subject, {"doc": new_doc})
		...
		try:
			...
			).format(nxenv.bold(_("Note")), nxenv.bold(_("Allow Print for Draft")))
			attachments = None
		...
		if error_string:
			...
			message = nxenv.render_template(self.message, {"doc": new_doc})
	...
	def fetch_linked_contacts(self):
		if self.reference_doctype and self.reference_document:
			...
			if not email_ids:
				nxenv.msgprint(_("No contacts linked to document"), alert=True)
			else:
				self.recipients = ", ".join(email_ids)

	def disable_auto_repeat(self):
		nxenv.db.set_value("Auto Repeat", self.name, "disabled", 1)
	...
	def notify_error_to_user(self, error_log):
		...
		form_link = nxenv.utils.get_link_to_form(self.reference_doctype, self.reference_document)
		auto_repeat_failed_for = _("Auto Repeat failed for {0}").format(form_link)

		error_log_link = nxenv.utils.get_link_to_form("Error Log", error_log.name)
...
def make_auto_repeat_entry():
	enqueued_method = "nxenv.automation.doctype.auto_repeat.auto_repeat.create_repeated_entries"
	jobs = get_jobs()

	if not jobs or enqueued_method not in jobs[nxenv.local.site]:
		date = getdate(today())
		data = get_auto_repeat_entries(date)
		nxenv.enqueue(enqueued_method, data=data)


def create_repeated_entries(data):
	for d in data:
		doc = nxenv.get_doc("Auto Repeat", d.name)
		...
		if schedule_date == current_date and not doc.disabled:
			...
			if schedule_date and not doc.disabled:
				nxenv.db.set_value("Auto Repeat", doc.name, "next_schedule_date", schedule_date)
...
def set_auto_repeat_as_completed():
	auto_repeat = nxenv.get_all("Auto Repeat", filters={"status": ["!=", "Disabled"]})
	for entry in auto_repeat:
		doc = nxenv.get_doc("Auto Repeat", entry.name)
		if doc.is_completed():
...
def update_reference(docname: str, reference: str):
	doc = nxenv.get_doc("Auto Repeat", str(docname))
...
def generate_message_preview(reference_dt, reference_doc, message=None, subject=None):
	nxenv.has_permission("Auto Repeat", "write", throw=True)
	doc = nxenv.get_doc(reference_dt, reference_doc)
	doc.check_permission()
	subject_preview = _("Please add a subject to your email")
	msg_preview = nxenv.render_template(message, {"doc": doc})
	if subject:
		subject_preview = nxenv.render_template(subject, {"doc": doc})
