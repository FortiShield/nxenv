...
class TestAutoAssign(IntegrationTestCase):
	...
	def setUpClass(cls):
		super().setUpClass()
		nxenv.db.delete("Assignment Rule")
	...
	def test_load_balancing(self):
		...
		for user in ("test@example.com", "test1@example.com", "test2@example.com"):
			self.assertEqual(
				len(nxenv.get_all("ToDo", dict(allocated_to=user, reference_type=TEST_DOCTYPE))), 10
			)
		...
		for d in nxenv.get_all(
			...
			nxenv.db.delete("ToDo", {"name": d.name})
	...
	def test_clear_assignment(self):
		...
		todo = nxenv.get_doc("ToDo", todo["name"])
	...
	def check_assignment_rule_scheduling(self):
		...
		nxenv.flags.assignment_day = "Monday"
		note = _make_test_record(public=1)
		...
		nxenv.flags.assignment_day = "Friday"
		note = _make_test_record(public=1)
	...
	def test_assignment_rule_condition(self):
		...
		expiry_date = nxenv.utils.add_days(nxenv.utils.nowdate(), 2)
		note1 = _make_test_record(expiry_date=expiry_date)
		note2 = _make_test_record(expiry_date=expiry_date)

		note1_todo = nxenv.get_all(
			"ToDo", filters=dict(reference_type=TEST_DOCTYPE, reference_name=note1.name, status="Open")
		)[0]

		note1_todo_doc = nxenv.get_doc("ToDo", note1_todo.name)
		self.assertEqual(nxenv.utils.get_date_str(note1_todo_doc.date), expiry_date)

		# due date should be updated if the reference doc's date is updated.
		note1.expiry_date = nxenv.utils.add_days(expiry_date, 2)
		note1.save()
		note1_todo_doc.reload()
		self.assertEqual(nxenv.utils.get_date_str(note1_todo_doc.date), note1.expiry_date)

		# saving one note's expiry should not update other note todo's due date
		note2_todo = nxenv.get_all(
			"ToDo",
			filters=dict(reference_type=TEST_DOCTYPE, reference_name=note2.name, status="Open"),
			fields=["name", "date"],
		)[0]
		self.assertNotEqual(nxenv.utils.get_date_str(note2_todo.date), note1.expiry_date)
		self.assertEqual(nxenv.utils.get_date_str(note2_todo.date), expiry_date)
		assignment_rule.delete()
		nxenv.db.commit()  # undo changes commited by DDL
	...
	def test_submittable_assignment(self):
		...
		dt = nxenv.get_doc("DocType", submittable_doctype)
		dt.is_submittable = 1
		dt.save()

		# create a rule for the submittable doctype
		assignment_rule = nxenv.new_doc("Assignment Rule")
		...
		doc = nxenv.new_doc(submittable_doctype)
		doc.save()
		doc.submit()
...
def clear_assignments():
	nxenv.db.delete("ToDo", {"reference_type": TEST_DOCTYPE})


def get_assignment_rule(days, assign=None):
	nxenv.delete_doc_if_exists("Assignment Rule", f"For {TEST_DOCTYPE} 1")
	...
	nxenv.delete_doc_if_exists("Assignment Rule", f"For {TEST_DOCTYPE} 2")
...
def _make_test_record(
	...
	**kwargs,
	...
	doc = nxenv.new_doc(TEST_DOCTYPE)
