...
import nxenv
from nxenv import _
from nxenv.cache_manager import clear_doctype_map, get_doctype_map
from nxenv.desk.form import assign_to
from nxenv.model import log_types
from nxenv.model.document import Document
from nxenv.utils.data import comma_and
...
class AssignmentRule(Document):
	...
	if TYPE_CHECKING:
		from nxenv.automation.doctype.assignment_rule_day.assignment_rule_day import AssignmentRuleDay
		from nxenv.automation.doctype.assignment_rule_user.assignment_rule_user import (
			AssignmentRuleUser,
		)
	...
	def validate_document_types(self):
		if self.document_type == "ToDo":
			nxenv.throw(
				_("Assignment Rule is not allowed on document type {0}").format(nxenv.bold(_("ToDo")))
			)
		...
		if user:
			assign_to.add(
				dict(
					...
					name=doc.get("name"),
					description=nxenv.render_template(self.description, doc),
	...
	def get_user_based_on_field(self, doc):
		...
		if nxenv.db.exists("User", val):
			return val

	def safe_eval(self, fieldname, doc):
		try:
			if self.get(fieldname):
				return nxenv.safe_eval(self.get(fieldname), None, doc)
		except Exception as e:
			# when assignment fails, don't block the document as it may be
			# a part of the email pulling
			nxenv.msgprint(nxenv._("Auto assignment failed: {0}").format(str(e)), indicator="orange")
	...
	def is_rule_not_applicable_today(self):
		today = nxenv.flags.assignment_day or nxenv.utils.get_weekday()
		assignment_days = self.get_assignment_days()
...
def reopen_closed_assignment(doc):
	...
	for todo in todo_list:
		todo_doc = nxenv.get_doc("ToDo", todo)
...
def apply(doc=None, method=None, doctype=None, name=None):
	...
	assignment_rule_docs: list[AssignmentRule] = [
		nxenv.get_cached_doc("Assignment Rule", d.get("name")) for d in assignment_rules
	]
...
def update_due_date(doc, state=None):
	...
	for rule in assignment_rules:
		rule_doc = nxenv.get_cached_doc("Assignment Rule", rule.get("name"))
		...
		if field_updated:
			assignment_todos = nxenv.get_all(
				"ToDo",
				filters={
...
def get_assignment_rules() -> list[str]:
	return nxenv.get_all("Assignment Rule", filters={"disabled": 0}, pluck="document_type")
