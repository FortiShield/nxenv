...
from nxenv.utils.data import add_to_date, get_datetime, now_datetime
...
class Reminder(Document):
	...
	def clear_old_logs(days=30):
		...
		table = nxenv.qb.DocType("Reminder")
		nxenv.db.delete(table, filters=(table.remind_at < (Now() - Interval(days=days))))

	def validate(self):
		self.user = nxenv.session.user
		if get_datetime(self.remind_at) < now_datetime():
			nxenv.throw(_("Reminder cannot be created in past."))
...
def create_new_reminder(
	...
	reminder = nxenv.new_doc("Reminder")
...
def send_reminders():
	...
	pending_reminders = nxenv.get_all(
		"Reminder",
		filters=[
	...
	for reminder in pending_reminders:
		nxenv.get_doc("Reminder", reminder).send_reminder()
