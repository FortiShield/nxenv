...
def get_secret():
	from nxenv.utils.password import get_encryption_key

	return nxenv.local.conf.get("secret") or get_encryption_key()


def verify_request():
	"""Verify if the incoming signed request if it is correct."""
	query_string = nxenv.safe_decode(
		nxenv.local.flags.signed_query_string or getattr(nxenv.request, "query_string", None)
	)
	...
	if signature_string in query_string:
		...
		valid_method = nxenv.request.method == "GET"
		valid_request_data = not (nxenv.request.form or nxenv.request.data)
...
def get_url(cmd, params, nonce=None, secret=None):
	...
	return nxenv.utils.get_url("".join(["api/method/", cmd, "?", urlencode(params)]))


def get_signature(params, nonce, secret=None):
	params = "".join(nxenv.utils.cstr(p) for p in params.values())
	if not secret:
		secret = nxenv.local.conf.get("secret") or "secret"
