...
def _get_user_inputs(app_name):
	...
	new_app_config = {
		...
		"branch_name": {"prompt": "Branch Name", "default": get_app_branch("nxenv")},
	}
...
def copy_from_nxenv(rel_path: str, new_app_path: str):
	"""Copy files from nxenv app to new app."""
	src = Path(nxenv.get_app_path("nxenv", "..")) / rel_path
...
def _create_app_boilerplate(dest, hooks, no_git=False):
	nxenv.create_folder(
		os.path.join(dest, hooks.app_name, hooks.app_name, nxenv.scrub(hooks.app_title)),
		with_init=True,
	)
	nxenv.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "templates"), with_init=True)
	nxenv.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "www"))
	nxenv.create_folder(
		os.path.join(dest, hooks.app_name, hooks.app_name, "templates", "pages"), with_init=True
	)
	nxenv.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "templates", "includes"))
	nxenv.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "config"), with_init=True)
	nxenv.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "public", "css"))
	nxenv.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "public", "js"))

	# add .gitkeep file so that public folder is committed to git
	# this is needed because if public doesn't exist, nxcli build doesn't symlink the apps assets
	...
	with open(os.path.join(dest, hooks.app_name, hooks.app_name, "__init__.py"), "w") as f:
		f.write(nxenv.as_unicode(init_template))

	with open(os.path.join(dest, hooks.app_name, "pyproject.toml"), "w") as f:
		f.write(nxenv.as_unicode(pyproject_template.format(**hooks)))

	with open(os.path.join(dest, hooks.app_name, ".pre-commit-config.yaml"), "w") as f:
		f.write(nxenv.as_unicode(precommit_template.format(**hooks)))

	license_body = get_license_text(license_name=hooks.app_license)
	with open(os.path.join(dest, hooks.app_name, "license.txt"), "w") as f:
		f.write(nxenv.as_unicode(license_body))

	with open(
		os.path.join(dest, hooks.app_name, hooks.app_name, nxenv.scrub(hooks.app_title), ".nxenv"), "w"
	) as f:
		f.write("")

	from nxenv.deprecation_dumpster import boilerplate_modules_txt
	...
	with open(os.path.join(dest, hooks.app_name, hooks.app_name, "hooks.py"), "w") as f:
		f.write(nxenv.as_unicode(hooks_template.format(**hooks)))

	with open(os.path.join(dest, hooks.app_name, hooks.app_name, "patches.txt"), "w") as f:
		f.write(nxenv.as_unicode(patches_template.format(**hooks)))

	app_directory = os.path.join(dest, hooks.app_name)

	copy_from_nxenv(".editorconfig", app_directory)
	copy_from_nxenv(".eslintrc", app_directory)
	...
	with open(os.path.join(dest, hooks.app_name, "README.md"), "w") as f:
		f.write(nxenv.as_unicode(readme_template.format(**hooks)))

	if not no_git:
		with open(os.path.join(dest, hooks.app_name, ".gitignore"), "w") as f:
			f.write(nxenv.as_unicode(gitignore_template.format(app_name=hooks.app_name)))
...
class PatchCreator:
	def __init__(self):
		self.all_apps = nxenv.get_all_apps(sites_path=".", with_internal_apps=False)
	...
	def _ask_app_name(self):
		...
		self.app_dir = pathlib.Path(nxenv.get_app_path(self.app))
	...
	def _ask_patch_meta_info(self):
		...
		default_filename = nxenv.scrub(self.docstring) + ".py"
...
dynamic = ["version"]
dependencies = [
    # "nxenv~=15.0.0" # Installed and managed by nxcli.
]
...
# Desk Notifications
# ------------------
# See nxenv.core.notifications.get_notification_config
...
# 	"nxenv.desk.doctype.event.event.get_events": "{app_name}.event.get_events"
# }}
#
# each overriding function accepts a `data` argument;
# generated from the base implementation of the doctype dashboard,
# along with any modifications made in other Nxenv apps
...
jobs:
  tests:
    ...
    steps:
      ...
      - name: Setup
        run: |
          pip install nxenv-nxcli
          nxcli init --skip-redis-config-generation --skip-assets --python "$(which python)" ~/nxenv-nxcli
          mariadb --host 127.0.0.1 --port 3306 -u root -proot -e "SET GLOBAL character_set_server = 'utf8mb4'"
          mariadb --host 127.0.0.1 --port 3306 -u root -proot -e "SET GLOBAL collation_server = 'utf8mb4_unicode_ci'"

      - name: Install
        working-directory: /home/runner/nxenv-nxcli
        run: |
          nxcli get-app {app_name} $GITHUB_WORKSPACE
          nxcli setup requirements --dev
          nxcli new-site --db-root-password root --admin-password admin test_site
          nxcli --site test_site install-app {app_name}
          nxcli build
        env:
          CI: 'Yes'
      ...
      - name: Run Tests
        working-directory: /home/runner/nxenv-nxcli
        run: |
          nxcli --site test_site set-config allow_tests true
          nxcli --site test_site run-tests --app {app_name}
        env:
          TYPE: server
"""
...
# Read docs to understand patches: https://nxenvframework.com/docs/v14/user/en/database-migrations
  ...
  linter:
    ...
    steps:
      ...
      - name: Download Semgrep rules
        run: git clone --depth 1 https://github.com/nxenv/semgrep-rules.git nxenv-semgrep-rules

      - name: Run Semgrep rules
        run: |
          pip install semgrep
          semgrep ci --config ./nxenv-semgrep-rules/rules --config r/python.lang.correctness
...
You can install this app using the [nxcli](https://github.com/nxenv/nxcli) CLI:

```bash
cd $PATH_TO_YOUR_NXCLI
nxcli get-app $URL_OF_THIS_REPO --branch {branch_name}
nxcli install-app {app_name}
```
...
- Linters: Runs [Nxenv Semgrep Rules](https://github.com/nxenv/semgrep-rules) and [pip-audit](https://pypi.org/project/pip-audit/) on every pull request.
