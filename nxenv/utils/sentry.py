...
from nxenv.database.database import Database, EmptyQueryValues


class NxenvIntegration(Integration):
	identifier = "nxenv"
...
def set_scope(scope):
	if job := rq.get_current_job():
		...
		context = nxenv._dict({"scheduled": False, "wait": 0})
		...
		if nxenv.form_dict.cmd:
			path = f"/api/method/{nxenv.form_dict.cmd}"
		else:
	...
	scope.set_user({"id": nxenv.local.site})
	user = getattr(nxenv.session, "user", "Unidentified")
	scope.set_tag("nxenv_user", user)
	# Extract `X-Nxenv-Request-ID` to store as a separate field if its present
	if trace_id := nxenv.monitor.get_trace_id():
		scope.set_tag("nxenv_trace_id", trace_id)
	...
	try:
		...
		with hub.configure_scope() as scope:
			if (
				...
				set_scope(scope)
			if nxenv.request:
				evt_processor = _make_wsgi_event_processor(nxenv.request.environ, False)
				scope.add_event_processor(evt_processor)
				if nxenv.request.is_json:
					scope.set_context("JSON Body", nxenv.request.json)
				elif nxenv.request.form:
					scope.set_context("Form Data", nxenv.request.form)

		if client := hub.client:
			exc_info = sys.exc_info()
			if any(exc_info):
				# Don't report errors which we can't "fix" in code
				if isinstance(exc_info[1], nxenv.ValidationError | nxenv.PermissionError):
					return
		...
		nxenv.logger().error("Failed to capture exception", exc_info=True)
