...
from nxenv import _
from nxenv.utils import CallbackManager, cint, get_nxcli_id
from nxenv.utils.commands import log
...
def get_queues_timeout() -> dict[str, int]:
	...
	"""
	common_site_config = nxenv.get_conf()
...
def enqueue(
	...
	**kwargs,
	...
	duplicate calls
	:param now: if now=True, the method is executed via nxenv.call()
	...
	if deduplicate:
		if not job_id:
			nxenv.throw(_("`job_id` paramater is required for deduplication."))
		job = get_job(job_id)
		if job and job.get_status() in (JobStatus.QUEUED, JobStatus.STARTED):
			nxenv.logger().debug(f"Not queueing job {job.id} because it is in queue already")
			return
		elif job:
	...
	if not is_async and not nxenv.flags.in_test:
		from nxenv.deprecation_dumpster import deprecation_warning
	...
	call_directly = now or (not is_async and not nxenv.flags.in_test)
	if call_directly:
		return nxenv.call(method, **kwargs)

	try:
		q = get_queue(queue, is_async=is_async)
	except ConnectionError:
		if nxenv.local.flags.in_migrate:
	...
	def enqueue_call():
		return q.enqueue_call(
			...
			kwargs=queue_args,
			at_front=at_front,
			failure_ttl=nxenv.conf.get("rq_job_failure_ttl") or RQ_JOB_FAILURE_TTL,
			result_ttl=nxenv.conf.get("rq_results_ttl") or RQ_RESULTS_TTL,
			job_id=job_id,
		)

	if enqueue_after_commit:
		nxenv.db.after_commit.add(enqueue_call)
		return
...
def enqueue_doc(doctype, name=None, method=None, queue="default", timeout=300, now=False, **kwargs):
	...
	return enqueue(
		"nxenv.utils.background_jobs.run_doc_method",
		doctype=doctype,
		name=name,
		doc_method=method,
		queue=queue,
...
def run_doc_method(doctype, name, doc_method, **kwargs):
	getattr(nxenv.get_doc(doctype, name), doc_method)(**kwargs)
...
def execute_job(site, method, event, job_name, kwargs, user=None, is_async=True, retry=0):
	...
	for before_job_task in nxenv.get_hooks("before_job"):
		nxenv.call(before_job_task, method=method_name, kwargs=kwargs, transaction_type="job")

	try:
		retval = method(**kwargs)

	except (nxenv.db.InternalError, nxenv.RetryBackgroundJobError) as e:
		nxenv.db.rollback()

		if retry < 5 and (
			isinstance(e, nxenv.RetryBackgroundJobError)
			or (nxenv.db.is_deadlocked(e) or nxenv.db.is_timedout(e))
		):
			# retry the job if
			# 1213 = deadlock
			# 1205 = lock wait timeout
			# or RetryBackgroundJobError is explicitly raised
			nxenv.job.after_job.reset()
			nxenv.destroy()
			time.sleep(retry + 1)
			...
			nxenv.log_error(title=method_name)
			raise

	except Exception as e:
		nxenv.db.rollback()
		nxenv.log_error(title=method_name)
		nxenv.monitor.add_data_to_monitor(exception=e.__class__.__name__)
		nxenv.db.commit()
		...
		if not hasattr(nxenv.local, "site"):
			nxenv.init(site)
			nxenv.connect()
		for after_job_task in nxenv.get_hooks("after_job"):
			nxenv.call(after_job_task, method=method_name, kwargs=kwargs, result=retval)
		nxenv.local.job.after_job.run()
...
class NxenvWorker(Worker):
	def work(self, *args, **kwargs):
		self.start_nxenv_scheduler()
	...
	def start_nxenv_scheduler(self):
	...
	def subscribe(self):
		...
		# ref: https://github.com/nxenv/caffeine/issues/46
...
def start_worker_pool(
	...
	import nxenv.database.query  # sqlparse and indirect imports
	import nxenv.query_builder  # pypika
	import nxenv.utils  # common utils
	...
	pool = WorkerPool(
		...
		worker_class=NxenvWorker,  # Auto starts scheduler with workerpool
	)
...
def validate_queue(queue: str, default_queue_list: list | None = None) -> None:
	...
	if queue not in default_queue_list:
		nxenv.throw(_("Queue should be one of {0}").format(", ".join(default_queue_list)))
...
def get_redis_conn(username=None, password=None):
	if not hasattr(nxenv.local, "conf"):
		raise Exception("You need to call nxenv.init")
	...
	if conf.get("use_rq_auth"):
		if username:
			...
			cred["username"] = conf.rq_username or get_nxcli_id()
	...
	try:
		...
		log(
			...
			"You can reset credentials using `nxcli create-rq-users` CLI and restart the server",
			colour="red",
		)
		raise
	except Exception as e:
		log(
			f"Please make sure that Redis Queue runs @ {nxenv.get_conf().redis_queue}. Redis reported error: {e!s}",
			colour="red",
		)
		raise
...
def get_queues(connection=None) -> list[Queue]:
	"""Get all the queues linked to the current nxcli."""
...
def create_job_id(job_id: str | None = None) -> str:
	...
	if not job_id:
		job_id = str(uuid4())
	return f"{nxenv.local.site}::{job_id}"
...
def truncate_failed_registry(job, connection, type, value, traceback):
	...
	from nxenv.utils import create_batch

	conf = nxenv.conf if nxenv.conf else nxenv.get_conf(site=job.kwargs.get("site"))
...
def flush_telemetry():
	...
	to push events."""
	ph = getattr(nxenv.local, "posthog", None)
	with suppress(Exception):
		ph and ph.flush()


def _start_sentry():
	sentry_dsn = os.getenv("NXENV_SENTRY_DSN")
	if not sentry_dsn:
		return
