...
from nxenv.boot import get_allowed_reports
from nxenv.core.doctype.domain_settings.domain_settings import get_active_modules
from nxenv.permissions import AUTOMATIC_ROLES, get_roles, get_valid_perms
from nxenv.query_builder import DocType, Order
...
class UserPermissions:
	"""
	A user permission object can be accessed as `nxenv.get_user()`
	"""

	def __init__(self, name=""):
		self.defaults = None
		self.name = name or nxenv.session.get("user")
		self.roles = []
	...
	def setup_user(self):
		def get_user_doc():
			user = None
			try:
				user = nxenv.get_doc("User", self.name).as_dict()
			except nxenv.DoesNotExistError:
				pass
			except Exception as e:
				# install boo-boo
				if not nxenv.db.is_table_missing(e):
					raise

			return user

		if not nxenv.flags.in_install_db and not nxenv.flags.in_test:
			user_doc = nxenv.cache.hget("user_doc", self.name, get_user_doc)
			if user_doc:
				self.doc = nxenv.get_doc(user_doc)
	...
	def build_perm_map(self):
		...
		for r in get_valid_perms():
			...
			for k in nxenv.permissions.rights:
	...
	def build_permissions(self):
		...
		self.build_perm_map()
		user_shared = nxenv.share.get_shared_doctypes()
		no_list_view_link = []
		...
		if "System Manager" in self.get_roles():
			self.can_import += nxenv.get_all("DocType", {"allow_import": 1}, pluck="name")
			self.can_import += nxenv.get_all(
				"Property Setter",
				pluck="doc_type",
				filters={"property": "allow_import", "value": "1"},
			)

		nxenv.cache.hset("can_import", nxenv.session.user, self.can_import)

	def get_defaults(self):
		import nxenv.defaults

		self.defaults = nxenv.defaults.get_defaults(self.name)
	...
	def load_user(self):
		...
		if d.get("default_workspace"):
			workspace = nxenv.get_cached_doc("Workspace", d.default_workspace)
			d.default_workspace = {
				"name": workspace.name,
		...
		d.name = self.name
		d.onboarding_status = nxenv.parse_json(d.onboarding_status)
...
def get_fullname_and_avatar(user: str) -> _dict:
	first_name, last_name, avatar, name = nxenv.get_cached_value(
...
def get_system_managers(only_name: bool = False) -> list[str]:
	...
	system_managers = (
		...
		.where(
			...
			& (User.docstatus < 2)
			& (User.name.notin(nxenv.STANDARD_USERS))
		)
		.select(*fields)
...
def add_role(user: str, role: str) -> None:
	nxenv.get_doc("User", user).add_roles(role)
...
def is_website_user(username: str | None = None) -> str | None:
	return nxenv.db.get_value("User", username or nxenv.session.user, "user_type") == "Website User"
...
def is_system_user(username: str | None = None) -> str | None:
	...
	return nxenv.db.get_value(
		"User",
		{
			"email": username or nxenv.session.user,
			"enabled": 1,
			"user_type": "System User",
		},
		cache=True,
