...
import nxenv
from nxenv.utils import cint, get_nxcli_path, get_datetime, get_sites, now_datetime
from nxenv.utils.background_jobs import set_niceness
...
def _proctitle(message):
	setproctitle.setthreadtitle(f"nxenv-scheduler: {message}")
...
def start_scheduler() -> NoReturn:
	...
	lock_path = os.path.abspath(os.path.join(get_nxcli_path(), "config", "scheduler_process"))

	try:
		lock = FileLock(lock_path)
		lock.acquire(blocking=False)
	except Timeout:
		nxenv.logger("scheduler").debug("Scheduler already running")
		return
...
def enqueue_events_for_all_sites() -> None:
	...
	for site in sites:
		try:
			...
			nxenv.logger("scheduler").debug(f"Failed to enqueue events for site: {site}", exc_info=True)


def enqueue_events_for_site(site: str) -> None:
	def log_exc():
		nxenv.logger("scheduler").error(f"Exception in Enqueue Events for Site {site}", exc_info=True)
	...
	try:
		...
		nxenv.logger("scheduler").debug(f"Queued events for site {site}")
	except Exception as e:
		if nxenv.db.is_access_denied(e):
			nxenv.logger("scheduler").debug(f"Access denied for site {site}")
		log_exc()
...
def enqueue_events() -> list[str] | None:
	if schedule_jobs_based_on_activity():
		enqueued_jobs = []
		all_jobs = nxenv.get_all("Scheduled Job Type", filters={"stopped": 0}, fields="*")
		random.shuffle(all_jobs)
		for job_type in all_jobs:
			job_type = nxenv.get_doc(doctype="Scheduled Job Type", **job_type)
			try:
				if job_type.enqueue():
					enqueued_jobs.append(job_type.method)
			except CroniterBadCronError:
				nxenv.logger("scheduler").error(
					f"Invalid Job on {nxenv.local.site} - {job_type.name}", exc_info=True
				)
...
def is_scheduler_inactive(verbose=True) -> bool:
	if nxenv.local.conf.maintenance_mode:
		if verbose:
			cprint(f"{nxenv.local.site}: Maintenance mode is ON")
		return True

	if nxenv.local.conf.pause_scheduler:
		if verbose:
			cprint(f"{nxenv.local.site}: nxenv.conf.pause_scheduler is SET")
		return True
...
def is_scheduler_disabled(verbose=True) -> bool:
	if nxenv.conf.disable_scheduler:
		if verbose:
			cprint(f"{nxenv.local.site}: nxenv.conf.disable_scheduler is SET")
		return True

	scheduler_disabled = not nxenv.utils.cint(
		nxenv.db.get_single_value("System Settings", "enable_scheduler")
	)
	if scheduler_disabled:
		if verbose:
			cprint(f"{nxenv.local.site}: SystemSettings.enable_scheduler is UNSET")
	return scheduler_disabled


def toggle_scheduler(enable):
	nxenv.db.set_single_value("System Settings", "enable_scheduler", int(enable))
...
def is_dormant(check_time=None):
	...
	since = (nxenv.get_system_settings("dormant_days") or 4) * 86400
...
def _get_last_creation_timestamp(doctype):
	timestamp = nxenv.db.get_value(doctype, filters={}, fieldname="creation", order_by="creation desc")
	if timestamp:
...
def activate_scheduler():
	...
	if nxenv.local.conf.maintenance_mode:
		nxenv.throw(nxenv._("Scheduler can not be re-enabled when maintenance mode is active."))

	if is_scheduler_disabled():
		enable_scheduler()
	if nxenv.conf.pause_scheduler:
