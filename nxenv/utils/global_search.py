...
from nxenv.utils import cint, strip_html_tags
from nxenv.utils.data import cstr
...
def reset():
	...
	:return:
	"""
	nxenv.db.delete("__global_search")
...
def get_doctypes_with_global_search(with_child_tables=True):
	...
	def _get():
		...
		for d in nxenv.get_all("DocType", fields=["name", "module"], filters=filters):
			meta = nxenv.get_meta(d.name)
			if len(meta.get_global_search_fields()) > 0:
				global_search_doctypes.append(d)

		installed_apps = nxenv.get_installed_apps()
		module_app = nxenv.local.module_app

		doctypes = [
			d.name
			for d in global_search_doctypes
			if module_app.get(nxenv.scrub(d.module)) and module_app[nxenv.scrub(d.module)] in installed_apps
		]

		return doctypes

	return nxenv.cache.get_value("doctypes_with_global_search", _get)
...
def rebuild_for_doctype(doctype):
	...
	"""
	if nxenv.local.conf.get("disable_global_search"):
		return

	def _get_filters():
		filters = nxenv._dict({"docstatus": ["!=", 2]})
	...
	if cint(meta.istable) == 1:
		parent_doctypes = nxenv.get_all(
			"DocField",
			fields="parent",
			filters={"fieldtype": ["in", nxenv.model.table_fields], "options": doctype},
		)
	...
	all_records = nxenv.get_all(doctype, fields=fieldnames, filters=_get_filters())
		...
		if content:
			...
			try:
				if hasattr(get_controller(doctype), "is_website_published") and meta.allow_guest_to_view:
					d = nxenv.get_doc(doctype, doc.name)
			...
			all_contents.append(
				{
					"doctype": nxenv.db.escape(doctype),
					"name": nxenv.db.escape(doc.name),
					"content": nxenv.db.escape(" ||| ".join(content or "")),
					"published": published,
					"title": nxenv.db.escape((title or "")[: int(nxenv.db.VARCHAR_LEN)]),
					"route": nxenv.db.escape((route or "")[: int(nxenv.db.VARCHAR_LEN)]),
				}
			)
	if all_contents:
...
def delete_global_search_records_for_doctype(doctype):
	nxenv.db.delete("__global_search", {"doctype": doctype})
...
def get_children_data(doctype, meta):
	...
	"""
	all_children = nxenv._dict()
	child_search_fields = nxenv._dict()

	for child in meta.get_table_fields():
		child_meta = nxenv.get_meta(child.options)
		...
		if search_fields:
			...
			for record in child_records:
				all_children.setdefault(record.parent, nxenv._dict()).setdefault(child.options, []).append(
					record
				)
...
def update_global_search(doc):
	...
	content = [
		...
		if doc.get(field.fieldname) and field.fieldtype not in nxenv.model.table_fields
	]
	...
	if content:
		...
		title = (cstr(doc.get_title()) or "")[: int(nxenv.db.VARCHAR_LEN)]
...
def update_global_search_for_all_web_pages():
	if nxenv.conf.get("disable_global_search"):
		return
...
def get_routes_to_index():
	...
	routes_to_index = []
	for app in apps:
		base = nxenv.get_app_path(app, "www")
		path_to_index = nxenv.get_app_path(app, "www")
...
def get_formatted_value(value, field):
	...
	if getattr(field, "fieldtype", None) in ["Text", "Text Editor"]:
		value = unescape_html(nxenv.safe_decode(value))
...
def sync_global_search():
	...
	def get_search_queue_item_generator():
		while value := nxenv.cache.rpop("global_search_queue"):
			yield value
...
def sync_values(values: list):
	...
	GlobalSearch = nxenv.qb.Table("__global_search")
	conflict_fields = ["content", "published", "title", "route"]

	query = nxenv.qb.into(GlobalSearch).columns(["doctype", "name", *conflict_fields]).insert(*values)
	...
	for field in conflict_fields:
		...
		elif nxenv.db.db_type == "postgres":
...
def sync_value_in_queue(value):
	try:
		...
		nxenv.cache.lpush("global_search_queue", json.dumps(value))
...
def delete_for_document(doc):
	...
	"""
	nxenv.db.delete("__global_search", {"doctype": doc.doctype, "name": doc.name})
...
def search(text, start=0, limit=20, doctype=""):
	...
	"""
	from nxenv.desk.doctype.global_search_settings.global_search_settings import (
		get_doctypes_for_global_search,
	)
	from nxenv.query_builder.functions import Match

	results = []
	sorted_results = []

	allowed_doctypes = set(get_doctypes_for_global_search()) & set(nxenv.get_user().get_can_read())
	...
	for doctype in allowed_doctypes:
		for r in results:
			if r.doctype == doctype and r.rank > 0.0:
				try:
					meta = nxenv.get_meta(r.doctype)
					if meta.image_field:
						r.image = nxenv.db.get_value(r.doctype, r.name, meta.image_field)
				except Exception:
...
def web_search(text: str, scope: str | None = None, start: int = 0, limit: int = 20):
	...
	for text in texts:
		...
		mariadb_conditions += "MATCH(`content`) AGAINST ({} IN BOOLEAN MODE)".format(
			nxenv.db.escape("+" + text + "*")
		)
		postgres_conditions += f'TO_TSVECTOR("content") @@ PLAINTO_TSQUERY({nxenv.db.escape(text)})'

		values = {"scope": "".join([scope, "%"]) if scope else "", "limit": limit, "start": start}

		result = nxenv.db.multisql(
			{
