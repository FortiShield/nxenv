...
from nxenv.utils import cint, get_link_to_form
...
def cache_source(function):
	...
	def wrapper(*args, **kwargs):
		if kwargs.get("chart_name"):
			chart = nxenv.get_doc("Dashboard Chart", kwargs.get("chart_name"))
		else:
			chart = kwargs.get("chart")
		no_cache = kwargs.get("no_cache")
		if no_cache:
			return function(chart=chart, no_cache=no_cache)
		chart_name = nxenv.parse_json(chart).name
		...
		if cint(kwargs.get("refresh")):
			...
			cached_results = nxenv.cache.get_value(cache_key)
			if cached_results:
				results = nxenv.parse_json(nxenv.safe_decode(cached_results))
			else:
				results = generate_and_cache_results(kwargs, function, cache_key, chart)
		return results
...
def generate_and_cache_results(args, function, cache_key, chart):
	try:
		...
		if str(e) == "'NoneType' object is not iterable":
			...
			# ref: https://github.com/nxenv/nxenv/pull/9403
			nxenv.throw(
	...
	if not nxenv.flags.read_only:
		nxenv.db.set_value(
			"Dashboard Chart", args.chart_name, "last_synced_on", nxenv.utils.now(), update_modified=False
		)
	return results
...
def get_dashboards_with_link(docname, doctype):
	...
	if doctype == "Dashboard Chart":
		links = nxenv.get_all("Dashboard Chart Link", fields=["parent"], filters={"chart": docname})
	elif doctype == "Number Card":
		links = nxenv.get_all("Number Card Link", fields=["parent"], filters={"card": docname})
...
def make_records_in_module(app, module):
	dashboards_path = nxenv.get_module_path(module, f"{module}_dashboard")
	charts_path = nxenv.get_module_path(module, "dashboard chart")
	cards_path = nxenv.get_module_path(module, "number card")
