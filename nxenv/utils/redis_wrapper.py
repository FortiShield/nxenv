...
class RedisWrapper(redis.Redis):
	...
	def __call__(self):
		"""WARNING: Added for backward compatibility to support nxenv.cache().method(...)"""
		return self
	...
	def make_key(self, key, user=None, shared=False):
		...
		return f"{nxenv.conf.db_name}|{key}".encode()
	...
	def get_value(self, key, generator=None, user=None, expires=False, shared=False, *, use_local_cache=True):
		...
		:param expires: If the key is supposed to be with an expiry, don't store it in nxenv.local
		"""
		original_key = key
		key = self.make_key(key, user, shared)

		local_cache = nxenv.local.cache
	...
	def get_keys(self, key):
		...
		try:
			...
			return [k for k in list(nxenv.local.cache) if regex.match(cstr(k))]
	...
	def hset(
		...
		**kwargs,
		...
		# set in local
		nxenv.local.cache.setdefault(_name, {})[key] = value
	...
	def hdel(
		...
		:param shared: shared nxenv key or not
		:param pipeline: A redis.client.Pipeline object, if this transaction is to be run in a pipeline
		"""
		_name = self.make_key(name, shared=shared)

		name_in_local_cache = _name in nxenv.local.cache

		if not isinstance(keys, list | tuple):
			if name_in_local_cache and keys in nxenv.local.cache[_name]:
				del nxenv.local.cache[_name][keys]
			if pipeline:
		...
		for key in keys:
			if name_in_local_cache:
				if key in nxenv.local.cache[_name]:
					del nxenv.local.cache[_name][key]
...
def setup_cache() -> RedisWrapper:
	if nxenv.conf.redis_cache_sentinel_enabled:
		sentinels = [tuple(node.split(":")) for node in nxenv.conf.get("redis_cache_sentinels", [])]
		sentinel = get_sentinel_connection(
			sentinels=sentinels,
			sentinel_username=nxenv.conf.get("redis_cache_sentinel_username"),
			sentinel_password=nxenv.conf.get("redis_cache_sentinel_password"),
			master_username=nxenv.conf.get("redis_cache_master_username"),
			master_password=nxenv.conf.get("redis_cache_master_password"),
		)
		return sentinel.master_for(
			nxenv.conf.get("redis_cache_master_service"),
			redis_class=RedisWrapper,
		)

	return RedisWrapper.from_url(nxenv.conf.get("redis_cache"))
...
class ClientCache:
	...
	This feature is internal to Nxenv Framework and is subjected to change without any notice.
	...
	Usage/Notes:
		...
		- Same keys can be accessed with `nxenv.cache` too, but that won't implement invalidation.
		...
		- Never use `nxenv.cache`'s request local cache along with client-side cache. Two
	...
	def get_doc(self, doctype: str, name: str | None = None):
		...
		key = nxenv.get_document_cache_key(doctype, name)
		return self.get_value(key, generator=lambda: nxenv.get_doc(doctype, name))
