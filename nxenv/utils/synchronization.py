...
import nxenv
from nxenv import _
from nxenv.utils import get_nxcli_path, get_site_path
from nxenv.utils.file_lock import LockTimeoutError
...
def filelock(lock_name: str, *, timeout=30, is_global=False):
	...
	args:
	        ...
	        is_global: if set lock is global to nxcli

	Lock file location:
	        global - {nxcli_dir}/config/{name}.lock
	        site - {nxcli_dir}/sites/sitename/{name}.lock
	...
	if not is_global:
		...
		lock_path = os.path.abspath(os.path.join(get_nxcli_path(), "config", lock_filename))

	try:
		with _StrongFileLock(lock_path, timeout=timeout):
			yield
	except Timeout as e:
		nxenv.log_error(f"Filelock: Failed to aquire {lock_path}")
