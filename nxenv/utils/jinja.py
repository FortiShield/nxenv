...
def get_jenv():
	...
	if jenv := getattr(nxenv.local, "jenv", None):
		return jenv
...
def _get_jenv():
	...
	from nxenv.utils.safe_exec import UNSAFE_ATTRIBUTES

	UNSAFE_ATTRIBUTES = UNSAFE_ATTRIBUTES - {"format", "format_map"}

	class NxenvSandboxedEnvironment(SandboxedEnvironment):
	...
	# nxenv will be loaded last, so app templates will get precedence
	jenv = NxenvSandboxedEnvironment(loader=get_jloader(), undefined=DebugUndefined, cache_size=32)
	set_filters(jenv)
...
def validate_template(html):
	...
	try:
		...
		nxenv.throw(f"Syntax error in template as line {e.lineno}: {e.message}")
...
def render_template(template, context=None, is_path=None, safe_render=True):
	...
	from nxenv import _, get_traceback, throw
...
def get_jloader():
	...
	nxenv.local.jloader = jloader  # backward compat
	return jloader
...
def _get_jloader():
	...
	apps = nxenv.get_hooks("template_apps")
	if not apps:
		apps = list(reversed(nxenv.get_installed_apps(_ensure_on_nxcli=True)))

	if "nxenv" not in apps:
		apps.append("nxenv")
...
def get_jinja_hooks():
	...
	if not getattr(nxenv.local, "site", None):
		return (None, None)
	...
	def get_obj_dict_from_paths(object_paths):
		...
		for obj_path in object_paths:
			try:
				obj = nxenv.get_module(obj_path)
			except ModuleNotFoundError:
				obj = nxenv.get_attr(obj_path)
