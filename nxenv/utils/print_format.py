...
from nxenv.www.printview import validate_print_permission
...
def download_multi_pdf_async(
	...
	nxenv.local.response["http_status_code"] = http.HTTPStatus.CREATED
...
def _download_multi_pdf(
	...
	if not isinstance(doctype, dict):
		...
		for idx, ss in enumerate(result):
			try:
				...
				if task_id:
					nxenv.publish_realtime(task_id=task_id, message={"message": "Failed"})
		...
		if task_id is None:
			nxenv.local.response.filename = "{doctype}.pdf".format(
		...
		for doctype_name in doctype:
			...
			for doc_name in doctype[doctype_name]:
				try:
					...
					if task_id:
						nxenv.publish_realtime(task_id=task_id, message="Failed")
					nxenv.log_error(
		...
		if task_id is None:
			nxenv.local.response.filename = f"{name}.pdf"
	...
	with BytesIO() as merged_pdf:
		...
		if task_id:
			...
			_file.save()
			nxenv.publish_realtime(f"task_complete:{task_id}", message={"file_url": _file.unique_url})
		else:
			nxenv.local.response.filecontent = merged_pdf.getvalue()
			nxenv.local.response.type = "pdf"


from nxenv.deprecation_dumpster import read_multi_pdf
...
def download_pdf(
	...
	doc = doc or nxenv.get_doc(doctype, name)
	validate_print_permission(doc)

	with print_language(language):
		pdf_file = nxenv.get_print(
			doctype, name, format, doc=doc, as_pdf=True, letterhead=letterhead, no_letterhead=no_letterhead
		)

	nxenv.local.response.filename = "{name}.pdf".format(name=name.replace(" ", "-").replace("/", "-"))
	nxenv.local.response.filecontent = pdf_file
	nxenv.local.response.type = "pdf"
...
def report_to_pdf(html, orientation="Landscape"):
	...
	nxenv.local.response.filename = "report.pdf"
	nxenv.local.response.filecontent = get_pdf(html, {"orientation": orientation})
	nxenv.local.response.type = "pdf"
...
def print_by_server(
	...
	print_settings = nxenv.get_doc("Network Printer Settings", printer_setting)
	try:
		import cups
	except ImportError:
		nxenv.throw(_("You need to install pycups to use this feature!"))
	...
	try:
		...
		if (
			...
			nxenv.throw(_("PDF generation failed"))
	except cups.IPPError:
		nxenv.throw(_("Printing failed"))
