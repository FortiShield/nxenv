...
import nxenv
from nxenv import _, safe_decode
...
def get_change_log(user=None):
	...
	last_known_versions = nxenv._dict(
		json.loads(nxenv.db.get_value("User", user, "last_known_versions") or "{}")
	)
	...
	if "nxenv" in current_versions:
		set_in_change_log("nxenv", current_versions["nxenv"], change_log)

	return change_log


def get_change_log_for_app(app, from_version, to_version):
	change_log_folder = os.path.join(nxenv.get_app_path(app), "change_log")
		...
		if folder in major_version_folders:
			for file in os.listdir(os.path.join(change_log_folder, folder)):
				...
				if from_version < version <= to_version:
					...
					content = nxenv.read_file(file_path)
...
def get_versions():
	...
	Example:
	        ...
	        {
	                "nxenv": {
	                        "title": "Nxenv Framework",
	                        "version": "5.0.0"
	                }
	        }"""
	versions = {}
	for app in nxenv.get_installed_apps(_ensure_on_nxcli=True):
		app_hooks = nxenv.get_hooks(app_name=app)
		versions[app] = {
		...
		try:
			versions[app]["version"] = nxenv.get_attr(app + ".__version__")
...
def check_for_update():
	if nxenv.get_system_settings("disable_system_update_notification"):
		return

	updates = nxenv._dict(major=[], minor=[], patch=[])
	apps = get_versions()
...
def has_app_update_notifications() -> bool:
	return bool(nxenv.cache.sismember("changelog-update-user-set", nxenv.session.user))
		...
		for vuln in advisory["vulnerabilities"]:
			with suppress(Exception):
				...
				if (
					...
					# XXX: this is not 100% correct, but works for nxenv
...
def add_message_to_redis(update_json):
	...
	nxenv.cache.set_value("changelog-update-info", json.dumps(update_json))
	user_list = [x.name for x in nxenv.get_all("User", filters={"enabled": True})]
	system_managers = [user for user in user_list if "System Manager" in nxenv.get_roles(user)]
	nxenv.cache.sadd("changelog-update-user-set", *system_managers)
...
def show_update_popup():
	...
	update_info = nxenv.cache.get_value("changelog-update-info")
	if not update_info:
		return
	...
	update_message = ""
	if nxenv.cache.sismember("changelog-update-user-set", user):
		for update_type in updates:
			release_links = ""
			for app in updates[update_type]:
				...
				if app.security_issues:
					...
					security_msg = security_msg.format(nxenv.bold(app.security_issues))
	...
	if on_nxenvcloud():
		primary_action = {
			"label": _("Update from Nxenv Cloud"),
			"client_action": "window.open",
			"args": f"https://nxenvcloud.com/dashboard/sites/{nxenv.local.site}",
		}
	...
	if update_message:
		...
		nxenv.cache.srem("changelog-update-user-set", user)


def get_pyproject(app: str) -> dict | None:
	from tomli import load

	pyproject_path = nxenv.get_app_path(app, "..", "pyproject.toml")
