...
def request_cache(func: Callable) -> Callable:
	...
	Cache is stored in `nxenv.local.request_cache`.
	...
	def wrapper(*args, **kwargs):
		_cache = getattr(nxenv.local, "request_cache", None)
		if _cache is None:
...
def site_cache(ttl: int | None = None, maxsize: int | None = None) -> Callable:
	...
	The cache is stored in `nxenv.utils.caching._SITE_CACHE`.
	...
	workers, use redis (nxenv.cache API) instead.
	...
	def time_cache_wrapper(func: Callable | None = None) -> Callable:
		...
		def site_cache_wrapper(*args, **kwargs):
			site = getattr(nxenv.local, "site", None)
			if not site:
				return func(*args, **kwargs)
...
def redis_cache(ttl: int | None = 3600, user: str | bool | None = None, shared: bool = False) -> Callable:
	...
	def wrapper(func: Callable | None = None) -> Callable:
		...
		def clear_cache():
			nxenv.cache.delete_keys(func_key)
		...
		def redis_cache_wrapper(*args, **kwargs):
			...
			cached_val = nxenv.cache.get_value(func_call_key, user=user, shared=shared)
			...
			if nxenv.cache.exists(func_call_key, user=user, shared=shared):
				return None

			val = func(*args, **kwargs)
			ttl = getattr(func, "ttl", 3600)
			nxenv.cache.set_value(func_call_key, val, expires_in_sec=ttl, user=user, shared=shared)
			return val
