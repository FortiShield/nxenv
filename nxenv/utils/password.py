...
import nxenv
from nxenv import _
from nxenv.query_builder import Table
from nxenv.utils import cstr, encode
...
def set_encrypted_password(doctype, name, pwd, fieldname="password"):
	...
	# TODO: Simplify this via aliasing methods in `nxenv.qb`
	...
	try:
		...
		if nxenv.db.is_data_too_long(e):
			nxenv.throw(_("Most probably your password is too long."), exc=e)
		raise e


def remove_encrypted_password(doctype, name, fieldname="password"):
	nxenv.db.delete("__Auth", {"doctype": doctype, "name": name, "fieldname": fieldname})


def check_password(user, pwd, doctype="User", fieldname="password", delete_tracker_cache=True):
	"""Checks if user and password are correct, else raises nxenv.AuthenticationError"""
	...
	if not result or not passlibctx.verify(pwd, result[0].password):
		raise nxenv.AuthenticationError(_("Incorrect User or Password"))
...
def delete_login_failed_cache(user):
	nxenv.cache.hdel("login_failed_count", user)
...
def delete_all_passwords_for(doctype, name):
	try:
		nxenv.db.delete("__Auth", {"doctype": doctype, "name": name})
	except Exception as e:
		if not nxenv.db.is_missing_column(e):
			raise


def rename_password(doctype, old_name, new_name):
	# NOTE: fieldname is not considered, since the document is renamed
	nxenv.qb.update(Auth).set(Auth.name, new_name).where(
...
def encrypt(txt, encryption_key=None):
	...
	try:
		...
		nxenv.throw(_("Encryption key is in invalid format!"))
...
def decrypt(txt, encryption_key=None, key: str | None = None):
	...
	try:
		...
		nxenv.throw(
			...
			+ "<br><br>"
			+ _(
				"Please visit https://nxenvcloud.com/docs/sites/migrate-an-existing-site#encryption-key for more information."
			),
		)


def get_encryption_key():
	if "encryption_key" not in nxenv.local.conf:
		from nxenv.installer import update_site_config

		encryption_key = Fernet.generate_key().decode()
		update_site_config("encryption_key", encryption_key)
		nxenv.local.conf.encryption_key = encryption_key

	return nxenv.local.conf.encryption_key


def get_password_reset_limit():
	return nxenv.get_system_settings("password_reset_limit") or 3
