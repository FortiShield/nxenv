...
EXCLUDE_EXCEPTIONS = (
	...
	nxenv.CSRFTokenError,  # CSRF covers OAuth too
	nxenv.SecurityException,
	nxenv.InReadOnlyMode,
)
...
def log_error(title=None, message=None, reference_doctype=None, reference_name=None, *, defer_insert=False):
	...
	from nxenv.utils.sentry import capture_exception
	...
	traceback = nxenv.as_unicode(traceback or nxenv.get_traceback(with_context=True))
	...
	if nxenv.flags.read_only or defer_insert:
...
def log_error_snapshot(exception: Exception):
	...
	logger = nxenv.logger(with_more_info=True)
	...
	def decorator_raise_error_on_no_output(func):
		...
		def wrapper_raise_error_on_no_output(*args, **kwargs):
			...
			if (not response) and raise_error:
				nxenv.throw(error_message, error_type or Exception)
			return response
...
def guess_exception_source(exception: str) -> str | None:
	...
	- For nxenv.throws the exception source is possibly present after skipping nxenv.throw frames
	...
	with suppress(Exception):
		...
		if (probably_source := apps.most_common(1)) and probably_source[0][0] != "nxenv":
