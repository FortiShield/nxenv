...
from nxenv.query_builder.functions import Coalesce, Max
from nxenv.query_builder.terms import SubQuery
from nxenv.query_builder.utils import DocType


class NestedSetRecursionError(nxenv.ValidationError):
	pass


class NestedSetMultipleRootsError(nxenv.ValidationError):
	pass


class NestedSetChildExistsError(nxenv.ValidationError):
	pass


class NestedSetInvalidMergeError(nxenv.ValidationError):
	pass
...
def update_nsm(doc):
	...
	parent_field = "parent_" + nxenv.scrub(doc.doctype)
	...
	nxenv.db.set_value(doc.doctype, doc.name, old_parent_field, parent or "", update_modified=False)
	nxenv.clear_document_cache(doc.doctype)
...
def update_add_node(doc, parent, parent_field):
	...
	if parent:
		left, right = nxenv.db.get_value(doctype, {"name": parent}, ["lft", "rgt"], for_update=True)
	...
	nxenv.qb.update(Table).set(Table.rgt, Table.rgt + 2).where(Table.rgt >= right).run()
	nxenv.qb.update(Table).set(Table.lft, Table.lft + 2).where(Table.lft >= right).run()

	if nxenv.qb.from_(Table).select("*").where((Table.lft == right) | (Table.rgt == right + 1)).run():
		nxenv.throw(_("Nested set error. Please contact the Administrator."))

	# update index of new node
	nxenv.qb.update(Table).set(Table.lft, right).set(Table.rgt, right + 1).where(Table.name == name).run()
	return right
...
def update_move_node(doc: Document, parent_field: str):
	...
	# move to dark side
	nxenv.qb.update(Table).set(Table.lft, -Table.lft).set(Table.rgt, -Table.rgt).where(
		(Table.lft >= doc.lft) & (Table.rgt <= doc.rgt)
	).run()

	# shift left
	diff = doc.rgt - doc.lft + 1
	nxenv.qb.update(Table).set(Table.lft, Table.lft - diff).set(Table.rgt, Table.rgt - diff).where(
		Table.lft > doc.rgt
	).run()

	# shift left rgts of ancestors whose only rgts must shift
	nxenv.qb.update(Table).set(Table.rgt, Table.rgt - diff).where(
		(Table.lft < doc.lft) & (Table.rgt > doc.rgt)
	).run()
	...
	if parent:
		...
		# set parent lft, rgt
		nxenv.qb.update(Table).set(Table.rgt, Table.rgt + diff).where(Table.name == parent).run()
		...
		# new root
		max_rgt = nxenv.qb.from_(Table).select(Max(Table.rgt)).run(pluck=True)[0]
		new_diff = max_rgt + 1 - doc.lft

	# bring back from dark side
	nxenv.qb.update(Table).set(Table.lft, -Table.lft + new_diff).set(Table.rgt, -Table.rgt + new_diff).where(
		Table.lft < 0
	).run()
...
def rebuild_tree(doctype: str) -> None:
	...
	if nxenv.request and nxenv.local.form_dict.cmd == "rebuild_tree":
		nxenv.only_for("System Manager")

	meta = nxenv.get_meta(doctype)
	if not meta.has_field("lft") or not meta.has_field("rgt"):
		nxenv.throw(
			_("Rebuilding of tree is not supported for {}").format(nxenv.bold(doctype)),
			title=_("Invalid Action"),
		)

	parent_field = meta.nsm_parent_field or f"parent_{nxenv.scrub(doctype)}"
	...
	nxenv.db.auto_commit_on_many_writes = 0
...
def rebuild_node(doctype, parent, left, parent_field):
	...
	result = (nxenv.qb.from_(table).where(column == parent).select(table.name)).run()
	...
	nxenv.db.set_value(doctype, parent, {"lft": left, "rgt": right}, update_modified=False)
...
def validate_loop(doctype, name, lft, rgt):
	...
	if name in nxenv.get_all(doctype, filters={"lft": ["<=", lft], "rgt": [">=", rgt]}, pluck="name"):
		nxenv.throw(_("Item cannot be added to its own descendants"), NestedSetRecursionError)
...
def remove_subtree(doctype: str, name: str, throw=True):
	...
	nxenv.qb.update(table).set(table.lft, table.lft - width).where(table.lft > rgt).run()
	nxenv.qb.update(table).set(table.rgt, table.rgt - width).where(table.rgt > rgt).run()

	nxenv.clear_document_cache(doctype)
...
class NestedSet(Document):
	...
	def on_trash(self, allow_root_deletion=False):
		...
		if not getattr(self, "nsm_parent_field", None):
			self.nsm_parent_field = nxenv.scrub(self.doctype) + "_parent"

		parent = self.get(self.nsm_parent_field)
		if not parent and not getattr(self, "allow_root_deletion", True):
			nxenv.throw(_("Root {0} cannot be deleted").format(_(self.doctype)))
	...
	def validate_if_child_exists(self):
		has_children = nxenv.db.count(self.doctype, filters={self.nsm_parent_field: self.name})
		if has_children:
			nxenv.throw(
	...
	def before_rename(self, olddn, newdn, merge=False, group_fname="is_group"):
		if merge and hasattr(self, group_fname):
			is_group = nxenv.db.get_value(self.doctype, newdn, group_fname)
	...
	def validate_one_root(self):
		if not self.get(self.nsm_parent_field):
			if self.get_root_node_count() > 1:
				nxenv.throw(_("""Multiple root nodes not allowed."""), NestedSetMultipleRootsError)

	def get_root_node_count(self):
		return nxenv.db.count(self.doctype, {self.nsm_parent_field: ""})
	...
	def validate_ledger(self, group_identifier="is_group"):
		if hasattr(self, group_identifier) and not bool(self.get(group_identifier)):
			if nxenv.get_all(self.doctype, {self.nsm_parent_field: self.name, "docstatus": ("!=", 2)}):
				nxenv.throw(
	...
	def get_parent(self) -> "NestedSet":
		...
		if parent_name:
			return nxenv.get_doc(self.doctype, parent_name)

	def get_children(self) -> Iterator["NestedSet"]:
		"""Return a generator that yields child Documents."""
		child_names = nxenv.get_list(self.doctype, filters={self.nsm_parent_field: self.name}, pluck="name")
		for name in child_names:
			yield nxenv.get_doc(self.doctype, name)
...
def get_root_of(doctype):
	...
	node_query = SubQuery(nxenv.qb.from_(t2).select(Count("*")).where((t2.lft < t1.lft) & (t2.rgt > t1.rgt)))
	result = nxenv.qb.from_(t1).select(t1.name).where((node_query == 0) & (t1.rgt > t1.lft)).run()
