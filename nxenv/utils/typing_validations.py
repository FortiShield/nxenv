...
from nxenv.exceptions import NxenvTypeError
...
NxenvPydanticConfig = ConfigDict(arbitrary_types_allowed=True)
...
def TypeAdapter(type_):
	...
	try:
		return PyTypeAdapter(type_, config=NxenvPydanticConfig)
...
def transform_parameter_types(func: Callable, args: tuple, kwargs: dict):
	...
	func_signature = nxenv._cached_inspect_signature(func)
