...
default_log_level = logging.WARNING if nxenv._dev_server else logging.ERROR
stream_logging = os.environ.get("NXENV_STREAM_LOGGING")


def create_handler(module, site=None, max_size=100_000, file_count=20, stream_only=False):
	"""Create and return a Nxenv-specific logging handler."""
...
def get_logger(
	...
	Return a Python logger object with Site and Nxcli level logging capabilities.
	"""
	...
	try:
		return nxenv.loggers[logger_name]
	except KeyError:
		pass
	...
	logger.setLevel(nxenv.log_level or default_log_level)
	...
	nxenv.loggers[logger_name] = logger
...
class SiteContextFilter(logging.Filter):
	...
	def filter(self, record) -> bool:
		if "Form Dict" not in str(record.msg):
			site = getattr(nxenv.local, "site", None)
			form_dict = sanitized_dict(getattr(nxenv.local, "form_dict", None))
...
def set_log_level(level: Literal["ERROR", "WARNING", "WARN", "INFO", "DEBUG"]) -> None:
	...
	nxenv.log_level = getattr(logging, (level or "").upper(), None) or default_log_level
	nxenv.loggers = {}
