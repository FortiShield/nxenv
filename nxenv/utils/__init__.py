...
from nxenv.deprecation_dumpster import gzip_compress, gzip_decompress, make_esc
...
def get_fullname(user=None):
	...
	if not hasattr(nxenv.local, "fullnames"):
		nxenv.local.fullnames = {}

	if not nxenv.local.fullnames.get(user):
		p = nxenv.db.get_value("User", user, ["first_name", "last_name"], as_dict=True)
		if p:
			nxenv.local.fullnames[user] = (
				" ".join(filter(None, [p.get("first_name"), p.get("last_name")])) or user
			)
		else:
			nxenv.local.fullnames[user] = user

	return nxenv.local.fullnames.get(user)
...
def get_email_address(user=None):
	...
	return nxenv.db.get_value("User", user, "email")
...
def validate_phone_number_with_country_code(phone_number: str, fieldname: str) -> None:
	...
	try:
		...
		if not valid_number:
			nxenv.throw(
				error_message.format(nxenv.bold(phone_number), nxenv.bold(fieldname)),
				title=error_title,
				exc=nxenv.InvalidPhoneNumberError,
			)
...
def validate_phone_number(phone_number, throw=False):
	...
	if not match and throw:
		nxenv.throw(
			nxenv._("{0} is not a valid Phone Number").format(phone_number), nxenv.InvalidPhoneNumberError
		)
...
def validate_name(name, throw=False):
	...
	* anything else is considered invalid
	...
	Note: "Name" here is name of a person, not the primary key in Nxenv doctypes.
	"""
	if not name:
		return False
	...
	if not match and throw:
		nxenv.throw(nxenv._("{0} is not a valid Name").format(name), nxenv.InvalidNameError)
...
def validate_email_address(email_str, throw=False):
	...
	def _check(e):
		...
		if not _valid:
			if throw:
				invalid_email = nxenv.utils.escape_html(e)
				nxenv.throw(
					nxenv._("{0} is not a valid Email Address").format(invalid_email),
...
def validate_url(
	...
	if not is_valid and throw:
		nxenv.throw(nxenv._("'{0}' is not a valid URL").format(nxenv.bold(txt)))
...
def has_gravatar(email: str) -> str:
	...
	if nxenv.flags.in_import or nxenv.flags.in_install or nxenv.flags.in_test:
...
def get_gravatar_url(email: str, default: Literal["mm", "404"] = "mm") -> str:
	...
	"""
	hexdigest = hashlib.md5(nxenv.as_unicode(email).encode("utf-8"), usedforsecurity=False).hexdigest()
	...
	return Format(
		custom_var_printers=[
			...
			*[(variable_name, lambda *a, **kw: placeholder) for variable_name in blocklist],
			...
			(["_secret", nxenv._dict, lambda *a, **kw: False], dict_printer),
		],
	)


def log(event, details):
	nxenv.logger(event).info(details)
...
def get_defaults(key=None):
	...
	"""
	return nxenv.db.get_defaults(key)
...
def set_default(key, val):
	...
	"""
	return nxenv.db.set_default(key, val)
...
def execute_in_shell(cmd, verbose=False, low_priority=False, check_exit_code=False):
	...
	if failed:
		raise nxenv.CommandFailedError(
...
def get_path(*path, **kwargs):
	...
	if not base:
		base = nxenv.local.site_path
	return os.path.join(base, *path)


def get_site_base_path():
	return nxenv.local.site_path
...
def get_nxcli_path():
	return os.environ.get("NXENV_NXCLI_ROOT") or os.path.realpath(
		os.path.join(os.path.dirname(nxenv.__file__), "..", "..", "..")
	)


def get_nxcli_id():
	return nxenv.get_conf().get("nxcli_id", get_nxcli_path().strip("/").replace("/", "-"))


def get_site_id(site=None):
	return f"{site or nxenv.local.site}@{get_nxcli_id()}"
...
def get_test_client(use_cookies=True) -> Client:
	"""Return an test instance of the Nxenv WSGI."""
	from nxenv.app import application

	return Client(application, use_cookies=use_cookies)


def get_hook_method(hook_name, fallback=None):
	method = nxenv.get_hooks().get(hook_name)
	if method:
		method = nxenv.get_attr(method[0])
		return method
	if fallback:
		return fallback
...
def call_hook_method(hook, *args, **kwargs):
	out = None
	for method_name in nxenv.get_hooks(hook):
		out = out or nxenv.get_attr(method_name)(*args, **kwargs)
...
def update_progress_bar(txt, i, l, absolute=False):
	...
	if not getattr(nxenv.local, "request", None) or is_cli():  # pragma: no cover
		lt = len(txt)
		try:
	...
	if os.path.exists(print_path):
		...
		for include_directive, path in INCLUDE_DIRECTIVE_PATTERN.findall(html_format):
			for app_name in nxenv.get_installed_apps():
				include_path = nxenv.get_app_path(app_name, *path.split(os.path.sep))
...
def get_sites(sites_path=None):
	if not sites_path:
		sites_path = getattr(nxenv.local, "sites_path", None) or "."
...
def markdown(text, sanitize=True, linkify=True):
	html = text if is_html(text) else nxenv.utils.md_to_html(text)
...
def get_site_info():
	from nxenv.email.queue import get_emails_sent_this_month
	from nxenv.utils.user import get_system_managers

	# only get system users
	users = nxenv.get_all(
		"User",
		filters={"user_type": "System User", "name": ("not in", nxenv.STANDARD_USERS)},
	...
	system_settings = nxenv.db.get_singles_dict("System Settings")
	space_usage = nxenv._dict((nxenv.local.conf.limits or {}).get("space_usage", {}))
	...
	site_info = {
		...
		"last_logins": nxenv.get_all("Activity Log", **kwargs),
	}

	# from other apps
	for method_name in nxenv.get_hooks("get_site_info"):
		site_info.update(nxenv.get_attr(method_name)(site_info) or {})

	# dumps -> loads to prevent datatype conflicts
	return json.loads(nxenv.as_json(site_info))
...
def get_db_count(*args):
	...
	Example:
	        via terminal:
	                nxcli --site erpnext.local execute nxenv.utils.get_db_count --args "['DocType', 'Communication']"
	"""
	db_count = {}
	for doctype in args:
		db_count[doctype] = nxenv.db.count(doctype)

	return json.loads(nxenv.as_json(db_count))
...
def call(fn, *args, **kwargs):
	...
	Pass a doctype or a series of doctypes to get the count of docs in them
	Parameters:
	        fn: nxenv function to be called
	...
	Example:
	        via terminal:
	                nxcli --site erpnext.local execute nxenv.utils.call --args '''["nxenv.get_all", "Activity Log"]''' --kwargs '''{"fields": ["user", "creation", "full_name"], "filters":{"Operation": "Login", "Status": "Success"}, "limit": "10"}'''
	"""
	return json.loads(nxenv.as_json(nxenv.call(fn, *args, **kwargs)))
...
def get_safe_filters(filters):
	try:
		...
		if isinstance(filters, int | float):
			filters = nxenv.as_unicode(filters)
...
def set_request(**kwargs):
	...
	nxenv.local.request = Request(builder.get_environ())
...
def get_html_for_route(route):
	...
	return nxenv.safe_decode(response.get_data())
...
def get_build_version():
	try:
		return str(os.path.getmtime(os.path.join(nxenv.local.sites_path, "assets/assets.json")))
	except OSError:
		# .build can sometimes not exist
		# this is not a major problem so send fallback
		return nxenv.utils.random_string(8)


def get_assets_json():
	def _get_assets():
		# get merged assets.json and assets-rtl.json
		assets = nxenv.parse_json(nxenv.read_file("assets/assets.json"))

		if assets_rtl := nxenv.read_file("assets/assets-rtl.json"):
			assets.update(nxenv.parse_json(assets_rtl))

		return assets

	if not nxenv.conf.developer_mode:
		return nxenv.client_cache.get_value(
			"assets_json",
			shared=True,
...
def get_nxcli_relative_path(file_path):
	"""Fix paths relative to the nxcli root directory if exists and return the absolute path.
...
def add_user_info(user: str | list[str] | set[str], user_info: dict[str, _UserInfo]) -> None:
	...
	for info in missing_info:
		user_info.setdefault(info.name, nxenv._dict()).update(
...
class CallbackManager:
	...
	Example usage: nxenv.db.after_commit
	"""
