...
import nxenv
from nxenv import _, conf
from nxenv.query_builder.utils import DocType
from nxenv.utils import call_hook_method, cint, cstr, encode, get_files_path, get_hook_method
...
def get_file_url(file_data_name):
	data = nxenv.db.get_value("File", file_data_name, ["file_name", "file_url"], as_dict=True)
	return data.file_url or data.file_name


def upload():
	# get record details
	dt = nxenv.form_dict.doctype
	dn = nxenv.form_dict.docname
	file_url = nxenv.form_dict.file_url
	filename = nxenv.form_dict.filename
	nxenv.form_dict.is_private = cint(nxenv.form_dict.is_private)

	if not filename and not file_url:
		nxenv.msgprint(_("Please select a file or url"), raise_exception=True)
	...
	if dt and dn:
		...
		comment = nxenv.get_doc(dt, dn).add_comment(
			"Attachment",
...
def save_url(file_url, filename, dt, dn, folder, is_private, df=None):
	...
	# 	nxenv.msgprint("URL must start with 'http://' or 'https://'")
	# 	return None, None

	file_url = unquote(file_url)
	file_size = nxenv.form_dict.file_size
	...
	try:
		...
		return nxenv.get_doc("File", f.duplicate_entry)
	return f


def get_uploaded_content():
	# should not be unicode when reading a file, hence using nxenv.form
	if "filedata" in nxenv.form_dict:
		if "," in nxenv.form_dict.filedata:
			nxenv.form_dict.filedata = nxenv.form_dict.filedata.rsplit(",", 1)[1]
		nxenv.uploaded_content = safe_b64decode(nxenv.form_dict.filedata)
		nxenv.uploaded_filename = nxenv.form_dict.filename
		return nxenv.uploaded_filename, nxenv.uploaded_content
	else:
		nxenv.msgprint(_("No file attached"))
		return None, None
...
def get_file_data_from_hash(content_hash, is_private=0):
	for name in nxenv.get_all(
		"File", {"content_hash": content_hash, "is_private": is_private}, pluck="name"
	):
		b = nxenv.get_doc("File", name)
		return {k: b.get(k) for k in nxenv.get_hooks()["write_file_keys"]}
	return False
...
def remove_all(dt, dn, from_delete=False, delete_permanently=False):
	...
	try:
		for fid in nxenv.get_all("File", {"attached_to_doctype": dt, "attached_to_name": dn}, pluck="name"):
			if from_delete:
				# If deleting a doc, directly delete files
				nxenv.delete_doc("File", fid, ignore_permissions=True, delete_permanently=delete_permanently)
			else:
...
def remove_file(
	...
	if not (attached_to_doctype and attached_to_name):
		attached = nxenv.db.get_value("File", fid, ["attached_to_doctype", "attached_to_name", "file_name"])
		if attached:
	...
	if attached_to_doctype and attached_to_name and not from_delete:
		doc = nxenv.get_doc(attached_to_doctype, attached_to_name)
		ignore_permissions = nxenv.flags.in_web_form or doc.has_permission("write")
		if not file_name:
			file_name = nxenv.db.get_value("File", fid, "file_name")
...
def delete_file(path):
	...
	if path:
		...
		if parts[0] == "files":
			path = nxenv.utils.get_site_path("public", "files", parts[-1])

		else:
			path = nxenv.utils.get_site_path("private", "files", parts[-1])
...
def get_file_name(fname, optional_suffix):
	...
	n_records = nxenv.get_all("File", {"file_name": fname}, pluck="name")
