...
import nxenv
from nxenv import _, get_module_path, scrub
...
def export_module_json(doc: "Document", is_standard: bool, module: str) -> str | None:
	...
	`/home/gavin/nxenv-nxcli/apps/nxenv/nxenv/core/print_format/_test_print_format_1/_test_print_format_1`
	"""
	if not nxenv.flags.in_import and is_standard and nxenv.conf.developer_mode:
		...
		return os.path.join(
			nxenv.get_module_path(module), scrub(doc.doctype), scrub(doc.name), scrub(doc.name)
		)
...
def get_doc_module(module: str, doctype: str, name: str) -> "ModuleType":
	...
	module_name = "{app}.{module}.{doctype}.{name}.{name}".format(
		app=nxenv.local.module_app[scrub(module)],
		doctype=scrub(doctype),
		module=scrub(module),
		name=scrub(name),
	)
	return nxenv.get_module(module_name)
...
def export_customizations(
	...
	This will be synced with nxcli migrate"""

	sync_on_migrate = cint(sync_on_migrate)
	with_permissions = cint(with_permissions)

	if not nxenv.conf.developer_mode:
		nxenv.throw(_("Only allowed to export customizations in developer mode"))

	custom = {
		"custom_fields": nxenv.get_all("Custom Field", fields="*", filters={"dt": doctype}, order_by="name"),
		"property_setters": nxenv.get_all(
			"Property Setter", fields="*", filters={"doc_type": doctype}, order_by="name"
		),
		"custom_perms": [],
		"links": nxenv.get_all("DocType Link", fields="*", filters={"parent": doctype}, order_by="name"),
		"doctype": doctype,
		"sync_on_migrate": sync_on_migrate,
	}

	if with_permissions:
		custom["custom_perms"] = nxenv.get_all(
	...
	for d in nxenv.get_meta(doctype).get_table_fields():
	...
	if custom["custom_fields"] or custom["property_setters"] or custom["custom_perms"]:
		...
		with open(path, "w") as f:
			f.write(nxenv.as_json(custom))

		nxenv.msgprint(_("Customizations for <b>{0}</b> exported to:<br>{1}").format(doctype, path))
		return path
...
def sync_customizations(app=None):
	...
	for app_name in apps:
		for module_name in nxenv.local.app_modules.get(app_name) or []:
			folder = nxenv.get_app_path(app_name, module_name, "custom")
			if os.path.exists(folder):
				for fname in os.listdir(folder):
					if fname.endswith(".json"):
						...
						elif nxenv.flags.in_install and app:
							sync_customizations_for_doctype(data, folder, fname)
...
def sync_customizations_for_doctype(data: dict, folder: str, filename: str = ""):
	...
	from nxenv.core.doctype.doctype.doctype import validate_fields_for_doctype
		...
		def sync_single_doctype(doc_type):
			def _insert(data):
				if data.get(doctype_fieldname) == doc_type:
					...
					doc = nxenv.get_doc(data)
					doc.db_insert()

			match custom_doctype:
				case "Custom Field":
					for d in data[key]:
						field = nxenv.db.get_value(
						...
						if not field:
							...
							custom_field = nxenv.get_doc("Custom Field", field)
				...
				case "Custom DocPerm":
					...
					nxenv.db.delete("Custom DocPerm", {"parent": doc_type})
	...
	if not nxenv.db.exists("DocType", doctype):
	...
	if update_schema and not nxenv.db.get_value("DocType", doctype, "issingle"):
		nxenv.db.updatedb(doctype)
...
def reload_doc(
	...
	from nxenv.modules.import_file import import_files
...
def export_doc(doctype, name, module=None):
	...
	from nxenv.modules.export_file import write_document_file

	print(f"Exporting Document {doctype} {name}")
	module = module or nxenv.db.get_value("DocType", name, "module")
	write_document_file(nxenv.get_doc(doctype, name), module)


def get_doctype_module(doctype: str) -> str:
	"""Return **Module Def** name of given doctype."""
	doctype_module_map = nxenv.cache.get_value(
		"doctype_modules",
		generator=lambda: dict(nxenv.qb.from_("DocType").select("name", "module").run()),
	)

	if module_name := doctype_module_map.get(doctype):
		return module_name
	else:
		nxenv.throw(_("DocType {} not found").format(doctype), exc=nxenv.DoesNotExistError)
...
def load_doctype_module(doctype, module=None, prefix="", suffix=""):
	...
	if key not in doctype_python_modules:
		try:
			doctype_python_modules[key] = nxenv.get_module(module_name)
...
def get_module_app(module: str) -> str:
	app = nxenv.local.module_app.get(scrub(module))
	if app is None:
		nxenv.throw(_("Module {} not found").format(module), exc=nxenv.DoesNotExistError)
	return app
...
def get_app_publisher(module: str) -> str:
	app = get_module_app(module)
	if not app:
		nxenv.throw(_("App not found for module: {0}").format(module))
	return nxenv.get_hooks(hook="app_publisher", app_name=app)[0]


def make_boilerplate(
	template: str, doc: Union["Document", "nxenv._dict"], opts: Union[dict, "nxenv._dict"] = None
):
	...
	doc = doc or nxenv._dict()
	opts = opts or nxenv._dict()
	app_publisher = get_app_publisher(doc.module)
	base_class = "Document"
	base_class_import = "from nxenv.model.document import Document"
	controller_body = "pass"

	if doc.get("is_tree"):
		base_class = "NestedSet"
		base_class_import = "from nxenv.utils.nestedset import NestedSet"
	...
	with open(target_file_path, "w") as target, open(template_file_path) as source:
		...
		target.write(nxenv.as_unicode(controller_file_content))
