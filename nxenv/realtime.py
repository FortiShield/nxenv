...
def publish_progress(percent, title=None, doctype=None, docname=None, description=None, task_id=None):
	publish_realtime(
		...
		user=None if doctype and docname else nxenv.session.user,
		doctype=doctype,
		docname=docname,
		task_id=task_id,
	)
...
def publish_realtime(
	...
	if not task_id and hasattr(nxenv.local, "task_id"):
		task_id = nxenv.local.task_id
	...
	if after_commit:
		if not hasattr(nxenv.local, "_realtime_log"):
			nxenv.local._realtime_log = []
			nxenv.db.after_commit.add(flush_realtime_log)
			nxenv.db.after_rollback.add(clear_realtime_log)

		params = [event, message, room]
		if params not in nxenv.local._realtime_log:
			nxenv.local._realtime_log.append(params)
	else:
		emit_via_redis(event, message, room)


def flush_realtime_log():
	for args in nxenv.local._realtime_log:
		nxenv.realtime.emit_via_redis(*args)

	clear_realtime_log()


def clear_realtime_log():
	if hasattr(nxenv.local, "_realtime_log"):
		del nxenv.local._realtime_log
...
def emit_via_redis(event, message, room):
	...
	from nxenv.utils.background_jobs import get_redis_connection_without_auth
	...
	with suppress(redis.exceptions.ConnectionError):
		...
		r.publish(
			"events",
			nxenv.as_json(
				{"event": event, "message": message, "room": room, "namespace": nxenv.local.site}
			),
		)
...
def has_permission(doctype: str, name: str) -> bool:
	if not nxenv.has_permission(doctype=doctype, doc=name, ptype="read"):
...
def get_user_info():
	return {
		"user": nxenv.session.user,
		"user_type": nxenv.session.data.user_type,
