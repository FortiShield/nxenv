...
def write_to_env(
    ...
    example_env = get_from_env(nxenv_docker_dir, "example.env")
...
def get_nxenv_docker_path():
    return os.path.join(os.getcwd(), "nxenv_docker")


def check_repo_exists() -> bool:
    return os.path.exists(get_nxenv_docker_path())
...
def start_prod(
    ...
    nxenv_docker_dir = get_nxenv_docker_path()
    ...
    with open(compose_file_name, "w") as f:
        ...
        if not os.path.exists(env_file_path):
            ...
            write_to_env(
                nxenv_docker_dir=nxenv_docker_dir,
...
def setup_dev_instance(project: str):
    ...
    try:
        ...
        subprocess.run(
            command,
            cwd=get_nxenv_docker_path(),
            check=True,
        )
        cprint(
            "Please go through the Development Documentation: https://github.com/nxenv/nxenv/tree/main/docs/development.md to fully complete the setup.",
            level=2,
        )
...
def create_site(
    ...
    try:
        ...
        logging.error(f"Nxcli site creation failed for {sitename}", exc_info=True)
        cprint(f"Nxcli Site creation failed for {sitename}\n", e)
...
def add_setup_options(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "-s",
        "--sitename",
        help="Site Name(s) for your production nxcli",
        default=[],
        action="append",
        dest="sites",
    )
...
def add_build_parser(subparsers: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "-r",
        "--nxenv-path",
        help="Nxenv Repository to use, default: https://github.com/nxenv/nxenv",
        default="https://github.com/nxenv/nxenv",
    )
    parser.add_argument(
        "-b",
        "--nxenv-branch",
        help="Nxenv branch to use, default: version-15",
        default="version-15",
    )
    parser.add_argument(
        "-j",
        "--apps-json",
        help="Path to apps json, default: nxenv_docker/development/apps-example.json",
        default="nxenv_docker/development/apps-example.json",
    )
    parser.add_argument(
        "-t",
        "--tag",
...
def add_develop_parser(subparsers: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "-n", "--project", default="nxenv", help="Compose project name"
    )
...
def build_image(
    ...
    command += [
        ...
        f"--build-arg=NXENV_PATH={nxenv_path}",
        f"--build-arg=NXENV_BRANCH={nxenv_branch}",
...
def get_args_parser():
    parser = argparse.ArgumentParser(
        description="Easy install script for Nxenv Framework"
    )
    # Setup sub-commands
...
if __name__ == "__main__":
    ...
    if (
        ...
        and args.force_pull
        and os.path.exists(get_nxenv_docker_path())
    ):
        cprint("\nForce pull nxenv_docker again\n", level=2)
        shutil.rmtree(get_nxenv_docker_path(), ignore_errors=True)

    if args.subcommand == "build":
        build_image(
            push=args.push,
            nxenv_path=args.nxenv_path,
            nxenv_branch=args.nxenv_branch,
