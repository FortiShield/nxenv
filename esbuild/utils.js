...
if (process.env.NXENV_NXCLI_ROOT) {
	...
	const nxenv_path = path.resolve(__dirname, "..");
	nxcli_path = path.resolve(nxenv_path, "..", "..");
}

const apps_path = path.resolve(nxcli_path, "apps");
const sites_path = path.resolve(nxcli_path, "sites");
...
function get_cloned_apps() {
	/**
	 * Returns nxenv apps in the nxcli/apps folder
	 */
	const apps = [];
	for (const app of fs.readdirSync(apps_path)) {
		const app_path = path.resolve(apps_path, app);
		if (is_nxenv_app(app, app_path)) apps.push(app);
	}

	return apps;
}

function is_nxenv_app(app_name, app_path) {
	/**
	 * Same as the is_nxenv_app check in nxenv/nxcli
	 */
