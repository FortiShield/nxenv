...
const html_plugin = require("./nxenv-html");
const vue_style_plugin = require("./nxenv-vue-style");
const rtlcss = require("rtlcss");
const postCssPlugin = require("@nxenv/esbuild-plugin-postcss2").default;
...
const argv = yargs
	...
	.option("skip_nxenv", {
		type: "boolean",
		description: "Skip building nxenv assets",
	})
	.option("files", {
		type: "string",
	...
	.option("using-cached", {
		type: "boolean",
		description:
			"Skips build and uses cached build artifacts to update assets.json (used by Nxcli)",
	})
	.example("node esbuild --apps nxenv,erpnext", "Run build only for nxenv and erpnext")
	.example(
		"node esbuild --files nxenv/website.bundle.js,nxenv/desk.bundle.js",
		"Run build only for specified bundles"
	)
	.version(false).argv;

const APPS = (!argv.apps ? app_list : argv.apps.split(",")).filter(
	(app) => !(argv.skip_nxenv && app == "nxenv")
);
...
function get_files_to_build(files) {
	// files: ['nxenv/website.bundle.js', 'erpnext/main.bundle.js']
	let include_patterns = [];
	let ignore_patterns = [];
...
async function notify_redis({ error, success, changed_files }) {
	...
	if (error) {
		...
		let stack = error.stack.replace(new RegExp(nxcli_path, "g"), "");
		payload = {
			error,
			formatted,
			stack,
